...
if(isset($_POST['hapus'])){
	...
	$sqlIns="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Hapus Kib Gedung','update as_seri2 set isaktif=0, tgl_hapus=$tgl, ket_hapus=$alasan where idseri=$id','".$_SERVER['REMOTE_ADDR']."')";
		mysql_query($sqlIns);
		
	$sqlupdate="update as_seri2 set isaktif='0', tgl_hapus='$tgl', ket_hapus='$alasan' where idseri='$id'";
	$rs=mysql_query($sqlupdate);
	if($rs!=mysql_error()){
        echo "<script>window.opener.rek.loadURL('gedung_utils.php?kode=true','','GET');</script>";
