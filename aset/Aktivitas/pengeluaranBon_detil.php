    ...
    if($act == 'add') {
        for($i=0; $i<count($data_submit)-1; $i++) {
            ...
            //$quKeluar = "UPDATE as_keluar SET tgl_gd='$txtTgl',no_gd='$txtNoPen',jumlah_keluar='$jmlk',petugas_gudang='$petugas_gudang' WHERE klr_id='$idklr'";
			 $sqlIns="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Update Pengeluaran Bon','UPDATE as_keluar SET tgl_gd=$txtTgl,no_gd=$txtNoPen,petugas_gudang=$petugas_gudang WHERE klr_id=$idklr','".$_SERVER['REMOTE_ADDR']."')";
					mysql_query($sqlIns);
			$quKeluar = "UPDATE as_keluar SET tgl_gd='$txtTgl',no_gd='$txtNoPen',jml_klr='$jmlb',jml_batal='$jmlbatal',petugas_gudang='$petugas_gudang' WHERE klr_id='$idklr'";
            ...
            while($rwMasuk = mysql_fetch_array($rsMasuk)) {
                ...
                else if($jmlKeluar<$rwMasuk['sisa']&&$jmlKeluar>0) {
                    $jml = $jumlahbon;
                    $jumlahbon = 0;
                }*/
				if($utang>=$rwMasuk['sisa']) 
				{
                    
                    $jml = $rwMasuk['sisa'];
					$utang = $utang-$rwMasuk['sisa']; 
                }
                else if($utang<$rwMasuk['sisa']) 
				{
					if($utang!=0)
					{
						$jml = $utang; 
                ...
                $nilai = $rwMasuk['harga_unit'];
				 $sqlIns1="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Insert Pengeluaran Bon','INSERT INTO as_operasi (klr_id,msk_id,waktu_opr,barang_id,jml,satuan,harga_satuan,subtotal)
		values($idklr,".$rwMasuk['msk_id'].",now(),$idbarang,$$jml,".$rwMasuk['satuan_unit'].",".$rwMasuk['harga_unit'].",".($jml*$rwMasuk['harga_unit']).")','".$_SERVER['REMOTE_ADDR']."')";
					mysql_query($sqlIns1);
		...
		values('$idklr','".$rwMasuk['msk_id']."',now(),'$idbarang','$$jml','".$rwMasuk['satuan_unit']."','".$rwMasuk['harga_unit']."','".($jml*$rwMasuk['harga_unit'])."')";
				...
				$sql = "update as_masuk set sisa = '".($rwMasuk['sisa']-$jml)."' where msk_id='".$rwMasuk['msk_id']."'";
				mysql_query($sql);
            }
            ...
            //mysql_free_result($rsOperasi);
			 $sqlIns2="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Update as_keluar','UPDATE as_keluar SET nilai=$hargaTotal,stt=1 WHERE klr_id=$idklr','".$_SERVER['REMOTE_ADDR']."')";
					mysql_query($sqlIns2);
					
            $quKeluar1 = "UPDATE as_keluar SET nilai='$hargaTotal',stt='1' WHERE klr_id='$idklr'";
            mysql_query($quKeluar1);

            $qsKeluar2 = "select klr_id,jml_klr,nilai,k.unit_id,barang_id,lokasi_id,satuan,mu.namaunit
            ...
            $tipe='1';//tipe keluar
			 $sqlIns3="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Update as_keluar','insert into as_kstok (waktu,barang_id,klr_id,jml_awal,jml_keluar,jml_sisa
		    ,nilai_awal,nilai_keluar,nilai_sisa,ket,tipe) 
		    values (now(),".$rwKeluar2['barang_id'].",".$rwKeluar2['klr_id'].",".$jmlSisa.",".$rwKeluar2['jml_klr'].",(jml_awal+jml_masuk-jml_keluar),
		    ".$nilaiSisa.",".$rwKeluar2['nilai'].",(nilai_awal+nilai_masuk-nilai_keluar),".$rwKeluar2['namaunit'].",$tipe)','".$_SERVER['REMOTE_ADDR']."')";
					mysql_query($sqlIns3);
	    ...
	    ('".$rwKeluar2['unit_id']."','".$rwKeluar2['lokasi_id']."','".$wMasuk['tgl_faktur']."','T','".$wMasuk['kode_transaksi']."','".$wMasuk['no_faktur']."','".$wMasuk['vendor_id']."', '$idbarang','".$rwKeluar2['satuan']."','0','".$rwKeluar2['jml_klr']."','IDR','1','0')";
           ...
           $sSeri = "SELECT noseri FROM as_seri2 where idbarang='$idbarang' and thn_pengadaan='$thnPengadaan'  ORDER BY noseri DESC LIMIT 1";
            ...
            $wSeri = mysql_fetch_array($rSeri);
			 ...
			 $sqlIns4="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Update as_keluar','INSERT INTO as_seri2 (harga_perolehan,nilaibuku,ms_idunit,ms_idlokasi,noseri,thn_pengadaan,idbarang,jenis_kib)VALUES
		    ($nilai,$nilai,".$rwKeluar2['unit_id'].",".$rwKeluar2['lokasi_id'].",".($wSeri['noseri']+1).",".$thnPengadaan.",".$idbarang.",".$kib.")','".$_SERVER['REMOTE_ADDR']."')";
					mysql_query($sqlIns4);
...
<html>
            ...
            <table border="0" width="1000" cellpadding="0" cellspacing="2" bgcolor="#FFFBF0">
                ...
                <form id="form1" name="form1" action="" method="post" onSubmit="save()">
                    ...
                    <tr>
                        ...
                        <td width="20%">&nbsp;:&nbsp;
                            <input id="txtNoPen"  name="txtNoPen" value='<?php if($po_rekanan[7]==''){echo $noterima;}else{echo $po_rekanan[7];} ?>'class="txtinput" size="20" /></td>
                        <td width="5%">&nbsp;</td>
                        <td width="5%">&nbsp;</td>
                        <td width="5%">&nbsp;</td>
                        <td width="5%">&nbsp;</td>
                        ...
                        <td width="30%">&nbsp;:&nbsp;
                            ...
                            ?>
                            <input type="text" value="<?php echo $val;?>" <?php echo $disabled; ?> size="30" name="txtNoPo" id="txtNoPo"  class="txtinput" onKeyUp="suggest(event,this);" autocomplete="off" placeholder="Ketikkan No Bon"/></td>
                        <td width="5%">&nbsp;</td>
                    </tr>
                    <tr>
                        <td width="5%">&nbsp;</td>
                        ...
                        <td width="16%">&nbsp;:&nbsp;
                            <input id="txtTgl" name="txtTgl" value="<?php if(isset($tgl) ) echo $tgl; else echo $date_now;?>" size="10" class="txtcenter"/>&nbsp;
                        ...
                        <td width="17%">&nbsp;:&nbsp;
                            <input type="text" size="30" id="txtUntuk" name="txtUntuk" class="txtinput" value="<?php echo (($_GET['act'] == 'edit')?$peruntukan:$po_rekanan[2]);?>" <?php if($_GET['act'] == 'edit') echo 'readonly'?>/>
                        </td>
                        <td width="5%">&nbsp;</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>&nbsp;:&nbsp;
                            <input type="text" id="txtPetGud" name="txtPetGud" class="txtinput" value="<?php if($_GET['act'] == 'edit') echo (($_GET['act'] == 'edit')?$petugas_gudang:$po_rekanan[3]); else echo $_SESSION['userid']; ?>" <?php if($_GET['act'] == 'edit') echo 'readonly'?>/>
                        </td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                    ...
                    <tr>
                        ...
                        <td colspan="8" align="center">
                            <table id="tblpenerimaan" width="100%" border="0" cellpadding="1" cellspacing="0">
                                <tr class="headtable">
                                  ...
                                  <td id="uraian" width="" class="tblheader"> <p>Uraian</p></td>
                                    <td id="satuan" width="" class="tblheader">Satuan</td>
									<td width=""  id="jmlKlr" class="tblheader">Jumlah Bon Dipesan</td>
									<td width=""  id="jmlKlr" class="tblheader">Jumlah Bon Keluar</td>
                                ...
                                if(isset($_GET['act']) && $_GET['act'] == 'edit' || isset($no_po)) {
                                    if($_GET['act'] == 'edit') {
                                        $sql = "SELECT klr_id,k.barang_id,kodebarang,namabarang,jml_klr,satuan,klr_uraian
						...
						WHERE tgl_transaksi='".$terima_po[0]."' AND kode_transaksi='".$terima_po[1]."'";                                        
                                        ...
                                        if($no_po != '') {
                                             $sql = "SELECT klr_id,k.barang_id,kodebarang,namabarang,k.jml_klr,satuan,klr_uraian
						FROM as_keluar k INNER JOIN as_ms_barang b 
						ON k.barang_id=b.idbarang
						WHERE tgl_transaksi='$po_rekanan[0]' AND kode_transaksi='$po_rekanan[1]'";
                                ...
                                <tr class="itemtableA" onMouseOver="this.className='itemtableAMOver'" onMouseOut="this.className='itemtableA'">
									...
									<td class="tdisi" align="center"><input type="textbox" style="text-align:center" size="5" id="jmlKLr<?=$i;?>" readonly value="<?php echo $jumlahkeluar;?>" /></td>
									<td class="tdisi" align="center"><input type="textbox" style="text-align:center" size="5" id="jmlKLr" onKeyUp="jmlCek(this,'<?=$i;?>');"  value="<?php echo $jumlahkeluar;?>" />
									<input type="hidden" size="5" id="jmlKLrTemp" value="<?php echo $jumlahkeluar;?>" /></td>
    ...
    <script type="text/javascript" language="javascript">
		...
		function jmlCek(a,b){
		//alert(document.getElementById("jmlKLr"+b).value);
		if(a.value*0==0){
			if(parseInt(a.value) > parseInt(document.getElementById("jmlKLr"+b).value)){
				alert("Data Tidak Boleh Besar Dari Jumlah Bon");
				a.value=document.getElementById("jmlKLr"+b).value;
			}
		}else{
		a.value=document.getElementById("jmlKLr"+b).value;
		}
            ...
            if(document.forms[0].chkItem.length){
                for(var i=0; i<document.forms[0].chkItem.length; i++){				
                    if(document.forms[0].chkItem[i].checked==true){
                        data_submit += document.forms[0].chkItem[i].value
							...
							+'*-*'+document.forms[0].jmlKLrTemp[i].value
							//jmlKLrTemp
                ...
                if(document.forms[0].chkItem.checked==true){
                    data_submit = document.forms[0].chkItem.value
						...
						+'*-*'+document.forms[0].jmlKLrTemp.value
