    ...
    if($act == 'add') {
        for($i=0; $i<count($data_submit)-1; $i++) {
            ...
            if($chkItem2 == 'true') {
                ...
                $qty_kemasan += $hid_qty_kemasan_terima;
            ...
            if($penerimaan == 'true') {
                $query = "insert into as_penerimaan (ms_barang_id,fk_minta_id, noterima, suplier_id, user_id_terima, tgl_act, tgl, tgl_j_tempo
                    , hari_j_tempo, qty_kemasan, kemasan, harga_kemasan, qty_per_kemasan, qty_satuan, satuan, nilai_pajak, unit_id_terima
                    ,harga_beli_total, harga_beli_satuan, diskon, diskon_total, nobukti, status) values
                    ('$chkItem1[1]', '$chkItem1[0]', '$txtNoPen', '$cmbNoPo[1]', '$user_id', now(), '$txtTgl', ADDDATE('$txtTgl', INTERVAL $txtTempo DAY)
                    , $txtTempo, '$qty_kemasan', '$kemasan', '$h_kemasan', '$qty_per_kemasan', '$qty_satuan', '$satuan', '$pajak', '$unit_terima'
                    , '$txtTotal', '$h_satuan', '$diskon', '$txtDiskon', '$txtFaktur', 1)";
                mysql_query($query);
...
?>
            ...
            <table border="0" width="1000" cellpadding="0" cellspacing="2" bgcolor="#FFFBF0">
                ...
                <form id="form1" name="form1" action="" method="post">
                    ...
                    <tr>
                        ...
                        <td width="5%">&nbsp;</td>
                        <td width="10%">&nbsp;No Kirim</td>
                        <td width="17%">&nbsp;:&nbsp;
                            <input id="nokirim" name="nokirim" class="txtright" align="right" value="<?php echo $nokirim; ?>" readonly size="15" />
                        </td>
                        <td width="5%">&nbsp;</td>
                        <td width="8%">&nbsp;Unit Tujuan</td>
                        <td width="17%">&nbsp;:&nbsp;
                            <select id="unit" name="unit">
                                ...
                                while($row = mysql_fetch_array($rs)){
                                    ?>
                                <option value="<?php echo $row['idunit']; ?>"><?php echo $row['kodeunit'].' - '.$row['namaunit']; ?></option>
                                    <?php
                                }
                                ?>
                            </select>
                            ...
                            <table id="tblpenerimaan" width="100%" border="0" cellpadding="1" cellspacing="0">
                                ...
                                <tr class="itemtableA" onMouseOver="this.className='itemtableAMOver'" onMouseOut="this.className='itemtableA'">
                                    ...
                                    <td class="tdisi" align="center">
                                        ...
                                        <input type="hidden" id="hid_qty_kemasan_terima" name="hid_qty_kemasan_terima" value="<?php echo $qty_kemasan_terima; ?>" />
                    ...
                    <tr>
                        ...
                        <td colspan="4" align="left">&nbsp;
                            <button type="reset" onClick="location='pemakaianBrg.php'">
    ...
    <script type="text/javascript" language="javascript">
            ...
            if(document.forms[0].chkItem.length){
                for(var i=0; i<document.forms[0].chkItem.length; i++){
                    data_submit += document.forms[0].chkItem[i].value+'*-*'+document.forms[0].chkItem[i].checked+'*-*'+document.forms[0].qty_kemasan[i].value
                        ...
                        +'*-*'+document.forms[0].diskon[i].value+'*-*'+document.forms[0].hid_qty_kemasan[i].value+'*-*'+document.forms[0].hid_qty_kemasan_terima[i].value+'*|*';
                }
            }
            else{
                data_submit = document.forms[0].chkItem.value+'*-*'+document.forms[0].chkItem.checked+'*-*'+document.forms[0].qty_kemasan.value
                    ...
                    +'*-*'+document.forms[0].diskon.value+'*-*'+document.forms[0].hid_qty_kemasan.value+'*-*'+document.forms[0].hid_qty_kemasan_terima.value+'*|*';
            }
        ...
        function set()
        {
            ...
            //alert("utils.php?pilihan=detailPenerimaan&nopo="+nopo);
