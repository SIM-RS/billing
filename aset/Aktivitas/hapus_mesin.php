...
if($_POST['act']=='hapus'){
		...
		$sqlIns="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Hapus Kib Mesin','update as_seri2 set isaktif = 0, tgl_hapus = '$tgl', t_userid = '$t_userid'
                        , t_updatetime = '$t_updatetime', ket_hapus='$alasan'
                        where idseri = $id','".$_SERVER['REMOTE_ADDR']."')";
		mysql_query($sqlIns);
		...
		$sql_del = "update as_seri2 set isaktif = 0, tgl_hapus = '$tgl', t_userid = '$t_userid'
				...
				if($res>0){
				echo "<script>alert('data berhasil dihapus');window.close();window.opener.grid.loadURL('utils_seri.php?pilihan=mesin','','GET');</script>";
				}else{
...
<td align="center">
	...
	<tr>
		<td colspan="2" class="header">.: Form Penghapusan KIB Alat/Mesin :.</td>
	</tr>
	<tr>
