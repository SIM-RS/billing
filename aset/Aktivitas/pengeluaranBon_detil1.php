...
if(isset($_POST['act']) && $_POST['act'] != '') {
    ...
    if($act == 'add') {
        for($i=0; $i<count($data_submit)-1; $i++) {
            ...
            $quKeluar = "UPDATE as_keluar SET tgl_gd='$txtTgl',no_gd='$txtNoPen',jml_klr='$jumlahbon',petugas_gudang='$petugas_gudang' WHERE klr_id='$idklr'";
...
if($_GET['id'] != '' && isset($_GET['id']) && $_GET['act'] == 'edit') {
   ...
   order by tgl_act ";
    ...
    $sql="select kode_transaksi from as_keluar where month(tgl_transaksi)='$th[1]' and year(tgl_transaksi)='$th[2]'  order by klr_id desc limit 1";
    $rs1=mysql_query($sql);
    if ($rows1=mysql_fetch_array($rs1)) {
        $noterima=$rows1["kode_transaksi"];
...
<html>
                    ...
                    <tr>
                        ...
                        <td colspan="8" align="center">
                            <table id="tblpenerimaan" width="100%" border="0" cellpadding="1" cellspacing="0">
                                <tr class="headtable">
                                    <td width="43" height="25" class="tblheaderkiri">No</td>
                                    <td id="kodebarang" width="62" class="tblheader">
                                  <input type="checkbox" name="chkAll" id="chkAll" value="" <?php if($_GET['act'] == 'edit') echo 'checked disabled';?> onClick="fCheckAll(chkAll,chkItem);HitunghTot();" style="cursor:pointer" title="Pilih Semua" />                                    </td>
                                    <td width="101"  id="kodebarang" class="tblheader">Kode Barang</td>
                                  <td id="namabarang" width="302" class="tblheader"> <p>Nama Barang</p></td>
                                  <td id="uraian" width="145" class="tblheader"> <p>Uraian</p></td>
                                    <td id="satuan" width="71" class="tblheader">Satuan</td>
									<td width="56"  id="jmlKlr" class="tblheader">Jumlah Bon</td>
									<td id="stok" width="56" class="tblheader">Stok</td>
                                    <td width="55"  id="jumlahbon" class="tblheader">Jml Keluar</td>
                                </tr>
                                <?php
						...
						WHERE tgl_transaksi='$po_rekanan[0]' AND kode_transaksi='$po_rekanan[1]'";
                                ...
                                <tr class="itemtableA" onMouseOver="this.className='itemtableAMOver'" onMouseOut="this.className='itemtableA'">
									...
									<td class="tdisi" align="center"><input type="textbox" size="5" id="jmlKLr" readonly value="<?php echo $jumlahkeluar;?>" /></td>
                                    <td class="tdisi" align="center"><input type="textbox" size="5" id="stok" readonly value="<?php echo $stok;?>" /></td>
                                    <td class="tdisi" align="center"><input type="textbox" size="5" id="jumlahkeluar" onKeyUp="cek()" value="<?php echo $jumlahkeluar;?>" /></td>
                                </tr>
                                        <?php
                                    }
    ...
    <script type="text/javascript" language="javascript">
		...
		function cek(){
		 ...
		 if(isNaN(document.getElementById('jumlahkeluar').value)){
		 	alert("Inputan harus diisi numeric !");
			location="pengeluaranBon_detil.php?act=<?php echo $act;?>&no_po=<? echo $no_po ?>";
		 }
		 else if(document.getElementById('jumlahkeluar').value > document.getElementById('jmlKLr').value){
