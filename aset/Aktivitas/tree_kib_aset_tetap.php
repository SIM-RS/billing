                    ...
                    <td nowrap>
                        ...
                        /*if(isset($_GET['idunit']) && $_GET['idunit'] != ''){
                            ...
                            while ($rows=mysql_fetch_array($rs)) {
                      		...
                      		$ss = substr($kdbrg,0,2);
                           $strSQL = "select * from as_ms_barang where tipe = 1 and kodebarang like '$ss%' order by kodebarang";
                            $rs = mysql_query($strSQL);
                            while ($rows=mysql_fetch_array($rs)) {
                                $c_level = $rows["level"];
                                $mpkode=trim($rows['kodebarang']);
                                $tree[$cnt][0]= $c_level;
                                $islast = $rows["islast"];
                                if($c_level<2 || substr($rows["kodebarang"],0,5)==substr($kdbrg,0,5))
                                $tree[$cnt][1]= $rows["kodebarang"]." - ".($mpkode==""?"":$mpkode." - ").$rows["namabarang"];
                                ...
                                if ($tree[$cnt][0] > $maxlevel)
                                    $maxlevel=$tree[$cnt][0];
                                	//	if(substr($rows["kodebarang"],0,5)==substr($kdbrg,0,5))
												if($c_level<2 || substr($rows["kodebarang"],0,5)==substr($kdbrg,0,5))															
												$cnt++;                                			
