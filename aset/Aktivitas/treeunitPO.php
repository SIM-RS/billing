...
<html>
                    ...
                    <td nowrap>
                        ...
                        while ($rows=mysql_fetch_array($rs)) {
                          ...
                          //  if ($islast>0){
                           ...
                           // $tree[$cnt][1]= '<input type="checkbox" id="pilih" name="pilih" onchange="pilih(this,'.$t.')" >'.($mpkode==""?"":$mpkode." - ").$rows["namaunit"].'<input type="hidden" id="nl" name="nl" size="1">';
									//	$t++;
                           // }else 
                            $tree[$cnt][1]= ($mpkode==""?"":$mpkode." - ").$rows["namaunit"];
                            
                            $ar = $no."*|*".$rows["idunit"]."*|*".$rows["namaunit"];
                            if ($islast>0)
                               $tree[$cnt][2]= "javascript:window.opener.fsetUnit('".$ar."');window.close();";
    ...
    function pilih(v,t){
    	if(v.checked==true){
			//	document.getElementById('nl'+t).type = 'text';    		
    		}
