...
$sql = "select 0 as id,msk.tgl_terima,msk.no_gudang, po.no_po,msk.no_faktur,msk.tgl_faktur,rek.namarekanan
	...
	$filter2 $filter AND po.ms_barang_id in 
			(select idbarang from as_ms_barang where kodebarang like '01%' or kodebarang like '03%' 
			or kodebarang like '04%' or kodebarang like '06%' and tipe='1') group by msk.tgl_terima,msk.no_gudang
	order by $sorting";
...
while($rows = mysql_fetch_array($rs)) {
        ...
        $dt .= $rows['id']."|".$rows['no_gudang'].chr(3).$i.chr(3).tglSQL($rows['tgl_faktur']).chr(3).$rows['no_faktur'].chr(3).$rows['no_po'].chr(3).$rows['no_gudang'].chr(3).$rows['namarekanan'].chr(6);
}
