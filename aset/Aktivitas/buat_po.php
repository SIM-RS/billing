...
if(isset($act) && $act!=''){
		...
		for($ro=0;$ro<(count($data));$ro++){
				...
				$id = $ri[0];
				$utk=$ri[1];
				 $sqlIns="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Update Buat PO','update as_po set no_po = $no_po,judul=$jdl,tgl_po=$tgl_po,peruntukan=$utk,status=1 where id=$id','".$_SERVER['REMOTE_ADDR']."')";
					mysql_query($sqlIns);
