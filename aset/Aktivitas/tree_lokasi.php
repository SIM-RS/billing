...
<html>
        ...
        <div align="center">
            <table border=1 cellspacing=0 width="98%">
                           ...
                           $strSQL = "select * from as_lokasi order by kodelokasi";
                            ...
                            while ($rows=mysql_fetch_array($rs)) {
                                ...
                                $tree[$cnt][1]= $rows["kodelokasi"]." - ".$rows["namalokasi"];
											if($par[3]!=''){
												$tm = ";window.opener.tampilkan();window.opener.document.getElementById('view').disabled = false";
												}                    
                                if ($islast > 0){
                                    $arfvalue = $i."*|*".$rows['idlokasi']."*|*".$rows['kodelokasi']."*|*".$rows['namalokasi'];
									$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['idlokasi']."*|*".$par[1]."*-*".$rows['kodelokasi']."*|*".$par[2]."*-*".$rows['namalokasi']."')".$tm.";window.close();";
                               }
                                else
                                    $tree[$cnt][2] = null;
               ...
               <tr>
					...
					<?php 
					if($par[3]=='kosongkan'){
						echo '<input type="button" value="Kosongkan" onclick="kosong()" />';
						}					
					?>
					</td>               
               </tr>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function kosong(){
				window.opener.document.getElementById('idlokasi2').value = '';
				window.opener.document.getElementById('kodelokasi').value = '';
				window.opener.document.getElementById('namalokasi').value = ''; 
				window.opener.document.getElementById('view').disabled = true; 
				window.close();       	
        	}
    </script>
</html>
