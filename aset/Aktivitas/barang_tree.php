    ...
    <body style="border-width:0px;" bgcolor="#CCCCCC" topmargin="0" leftmargin="0" onLoad="javascript:if (window.focus) window.focus();" onBlur="window.close();">
        <div align="center">
            <table border=1 cellspacing=0 width="98%">
                ...
                <tr>
                <td>	<select id="cmbfilter" name="cmbfilter" onChange="window.location='barang_tree.php?par=<?=$par1;?>&i=<?=$i;?>&cmb='+this.value;">
                ...
                <tr bgcolor="whitesmoke">
                    <td nowrap>
                        ...
                        while ($rows=mysql_fetch_array($rs)) {
							...
							$islast = $rows["islast"];
                            $tree[$cnt][0]= $c_level;
                            if ($islast>0) $tree[$cnt][1]= ($mpkode==""?"":$mpkode." - ").$rows["namabarang"].'('.$rows["idsatuan"].')';
										else $tree[$cnt][1]= ($mpkode==""?"":$mpkode." - ").$rows["namabarang"];
