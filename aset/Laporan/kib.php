<?php
session_start();
include '../koneksi/konek.php';
if(!isset($_SESSION['userid']) || $_SESSION['userid'] == '') {
    echo "<script>alert('Anda belum login atau session anda habis, silakan login ulang.');
                        window.location='$def_loc';
                        </script>";
}
$unit_opener="par=idunit*kodeunit*namaunit*idlokasi*kodelokasi";
?>

    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1" />
        <script type="text/javascript" language="JavaScript" src="../theme/js/mod.js"></script>
        <link type="text/css" rel="stylesheet" href="../theme/mod.css"/>
        <link type="text/css" rel="stylesheet" href="../default.css"/>
        <title>.: Laporan KIB :.</title>
    </head>

    <body>
        <div id="tree" style="display: none;">
            
        </div>    
        
        <div align="center">
            <?php
            include '../header.php';
            ?>
            <table width="1000" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0">
                <tr>
                    <td style="padding-top: 10px;" align="center">
                    <script>
                                        function showtree(){
                                            if(document.getElementById('treediv').style.display=='none'){
                                                document.getElementById('treediv').style.display='inline';
                                                }    
                                             else{
                                                document.getElementById('treediv').style.display='none';
                                                                                            
                                                                                             
                                             }                  
                                        }
                                    </script>
                    <iframe id="treediv" scrolling="auto" style="display: none;" width="1000" src="../Aktivitas/unit_tree.php?<?php echo $unit_opener; ?>"></iframe>                                        
                        <?php
                        if ($_GET["par"]=="1") {
                            ?>
                                                        
                        <form id="form1" name="form1" action="lap_inv.php" method="post" target="Laporan Kartu Inventaris Barang">
                            <table width="600" border="0" cellspacing="0" cellpadding="4">
                                <tr align="center">
                                    <td colspan="2" class="header">
                                        <strong><font size="2">Laporan Kartu Inventaris Barang</font></strong>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="label"><strong>Unit / Satuan Kerja </strong></td>
                                    <td class="content">
                                                                        
                                        <input name="kodeunit" type="text" class="txtmustfilled" id="kodeunit" size="15" maxlength="15" readonly style="text-align:left;" value="12.13.13.07.02"/>
                                        <input name="namaunit" type="text" class="txtmustfilled" id="namaunit" size="25" maxlength="25" readonly style="text-align:left;" value="RSUD Sidoarjo"/>
                                        <!--img alt="tree" title='daftar unit kerja'  style="cursor:pointer" border=0 src="../images/view_tree.gif" align="absbottom" onClick="OpenWnd('../Aktivitas/unit_tree.php?<?php //echo $unit_opener; ?>',800,500,'msma',true)" /-->
                                        <!--img alt="tree" title='daftar unit kerja'  style="cursor:pointer" border=0 src="../images/view_tree.gif" align="absmiddle" onclick="if (document.getElementById('kodeunit').value != '0') {show_treeunit();}" /-->
                                        <!--input type="checkbox" name="chkall" value="checkbox" onClick="if (this.checked==true) {document.getElementById('idunit').value='0';document.getElementById('kodeunit').value='0';}else{document.getElementById('idunit').value='';document.getElementById('kodeunit').value='';}" />
                                        Semua Unit Kerja<br /-->
                                                                                
                                    </td>
                                </tr>
                                <tr>
                                    <td class="label">
                                        <strong>Kategori</strong>
                                    </td>
                                    <td class="content">
                                        <select name="kategori_kib" class="txt" id="kategori_kib">
                                            <option value="A. TANAH">A. TANAH</option>
                                            <option value="B. PERALATAN DAN MESIN">B. PERALATAN DAN MESIN</option>
                                            <option value="C. GEDUNG DAN BANGUNAN">C. GEDUNG DAN BANGUNAN</option>
                                            <option value="D. JALAN, IRIGASI DAN JARINGAN">D. JALAN, IRIGASI DAN JARINGAN</option>
                                            <option value="E. ASET TETAP LAINNYA">E. ASET TETAP LAINNYA</option>
                                            <!--option value="F. KONSTRUKSI DALAM PENGERJAAN">F. KONSTRUKSI DALAM PENGERJAAN</option-->
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    <td width="139" class="label"><strong>Perolehan</strong></td>
                                    <td width="439" class="content">
                                        <select name="cmbperolehan" class="txt" id="cmbperolehan" tabindex="2" onChange="if (this.value==1){document.getElementById('a').style.display='none';}else{document.getElementById('a').style.display='block';}">
                                            <option value="1">Semua</option>
                                            <option value="2">Periode</option>
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="label"><strong>Tanggal Perolehan</strong></td>
                                    <td class="content">
                                        <div id="a" style="display:none">
                                            <input name="tglawal" type="text" class="txt" id="tglawal" tabindex="4" value="<?php echo date('d-m-Y');?>" size="15" maxlength="15" readonly />
                                            <img alt="calender" style="cursor:pointer" border=0 src="../images/cal.gif" align="absbottom" onClick="gfPop.fPopCalendar(document.getElementById('tglawal'),depRange);" />
                                            <!--img alt="calender" name="imgtglawal" style="cursor:pointer" border=0 src="../images/cal.gif" align="absbottom" onclick="javascript:show_calendar('form1.tglawal','dd-mm-yyyy',null,null,window.event);" /-->
                                            &nbsp;s/d&nbsp;
                                            <input name="tglakhir" type="text" class="txt" id="tglakhir" tabindex="4" value="<?php echo date('d-m-Y');?>" size="15" maxlength="15" readonly />
                                            <img alt="calender" style="cursor:pointer" border=0 src="../images/cal.gif" align="absbottom" onClick="gfPop.fPopCalendar(document.getElementById('tglakhir'),depRange);" />
                                            <!--img alt="calender" name="imgtglakhir" style="cursor:pointer" border=0 src="../images/cal.gif" align="absbottom" onclick="javascript:show_calendar('form1.tglakhir','dd-mm-yyyy',null,null,window.event);" /-->
                                            <font size=1 color=gray><i>(dd-mm-yyyy)</i></font>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="label"><strong>Format Laporan </strong></td>
                                    <td class="content"><select name="formatlap" class="txt" id="formatlap" tabindex="7">
                                            <option value="HTML">HTML</option>
                                            <option value="XLS">EXCEL</option>
                                            <option value="WORD">WORD</option>
                                        </select></td>
                                        
                                </tr>
                                <tr align="center">
                                    <td colspan="2" class="header2"><input name="submit2" type="button" id="submit2" value="Tampilkan" onClick="
if(document.getElementById('kategori_kib').value=='A. TANAH'){
window.open('lap_kibTanah.php?kode='+document.getElementById('kodeunit').value+'&jenislap='+document.getElementById('formatlap').value);
}else if(document.getElementById('kategori_kib').value=='B. PERALATAN DAN MESIN'){
window.open('../Aktivitas/laporan_mesin.php?kdunit='+document.getElementById('kodeunit').value+'&jenislap='+document.getElementById('formatlap').value);
}else if(document.getElementById('kategori_kib').value=='C. GEDUNG DAN BANGUNAN'){
window.open('../Aktivitas/gedung_laporan.php?kode='+document.getElementById('kodeunit').value+'&jenislap='+document.getElementById('formatlap').value);
}else if(document.getElementById('kategori_kib').value=='D. JALAN, IRIGASI DAN JARINGAN'){
window.open('../Aktivitas/jln_laporan.php?kode='+document.getElementById('kodeunit').value+'&jenislap='+document.getElementById('formatlap').value);
}else if(document.getElementById('kategori_kib').value=='E. ASET TETAP LAINNYA'){
window.open('../Aktivitas/laporan_aset_tetap.php?kdunit='+document.getElementById('kodeunit').value+'&jenislap='+document.getElementById('formatlap').value);
}
"                                         /></td>
                                </tr>
                            </table>
                          <input name="idunit" type="hidden" id="idunit" value="" />
                            <input name="namaunit" type="hidden" id="namaunit" value="" />
                            <input name="idlokasi" type="hidden" id="idlokasi" value="" />
                            <input name="kodelokasi" type="hidden" id="kodelokasi" value="" />
                        </form>
                            <?php
                        } else {
                            ?>
                        <form id="form1" name="form1"  action="lap_buku_inv.php" method="post" target="Laporan Buku Inventaris">
                            <table width="600" border="0" cellspacing="0" cellpadding="4">
                                <tr align="center">
                                    <td colspan="2" class="header">
                                        <strong><font size="2">Laporan Buku Inventaris</font></strong>
                                    </td>
                                </tr>                                                                                                                                       
                                <tr>
                                    <td class="label"><strong>Unit / Satuan Kerja </strong></td>                                                                            
                                    <td class="content">
                                   
                                        <input name="kodeunit" type="text" class="txt" id="kodeunit" size="15" maxlength="15" readonly style="text-align:left;background-color:#D5FDFD;" value="12.13.13.07" />
                                         <input name="namaunit" type="text" class="txt" id="namaunit" size="25" maxlength="25" readonly style="text-align:left;background-color:#D5FDFD;" value="Rsud Sidoarjo" />
                                        <!--img alt="tree" title='daftar unit kerja'  style="cursor:pointer" border=0 src="../images/view_tree.gif" align="absmiddle" onClick="OpenWnd('../Aktivitas/unit_tree.php?<?php echo $unit_opener; ?>',800,500,'msma',true)" />
                                        <input type="checkbox" name="chkall" value="checkbox" onClick="if (this.checked==true) {document.getElementById('idunit').value='0';document.getElementById('kodeunit').value='0';}else{document.getElementById('idunit').value='';document.getElementById('kodeunit').value='';}" /-->
                                        
                                    </td>
                                </tr>
                                <tr>
                                    <td class="label"><strong>Kategori</strong></td>
                                    <td class="content">
                                        <select name="cmb_buku_inv" class="txt">
                                            <option value="1">Buku Inventaris</option>
                                            <option value="2">Rekap Buku Inventaris</option>
                                        </select>
                                    </td>
                                </tr>
                               
                                <tr>
                                    <td class="label"><strong>Format Laporan </strong></td>
                                    <td class="content">
                                        <select name="formatlap" class="txt" id="formatlap" tabindex="7">
                                            <option value="HTML">HTML</option>
                                            <option value="XLS">EXCEL</option>
                                            <option value="WORD">WORD</option>
                                        </select>
                                    </td>
                                </tr>
                                <tr align="center">
                                    <td colspan="2" class="header2">
                                        <input name="submit" type="submit" id="submit" value="Tampilkan" onClick=""/>
                                    </td>
                                </tr>
                            </table>
                            <input name="idunit" type="hidden" id="idunit" value="" />
                            <input name="namaunit" type="hidden" id="namaunit" value="" />
                            <input name="idlokasi" type="hidden" id="idlokasi" value="" />
                            <input name="kodelokasi" type="hidden" id="kodelokasi" value="" />
                        </form>
                            <?php
                        }
                        ?>
                    </td>
                </tr>
                <tr>
                    <td>
                        <?php
                        include '../footer.php';
                        ?>
                    </td>
                </tr>
            </table>
        </div>
        <iframe height="193" width="168" name="gToday:normal:agenda.js"
                id="gToday:normal:agenda.js"
                src="../theme/popcjs.php" scrolling="no"
                frameborder="1"
                style="border:1px solid medium ridge; position: absolute; z-index: 65535; left: 100px; top: 50px; visibility: hidden">
        </iframe>
    </body>
    <script type="text/JavaScript" language="JavaScript">
        var arrRange=depRange=[];

    </script>
</html>
