...
if(isset($_POST['act']) && $_POST['act'] != '') {
	...
	$sqlIns="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Insert Master Barang','insert into as_ms_barang (kodebarang,namabarang,tipebarang,level,metodedepresiasi,stddeppct,akunaset,akunpengadaan,akunpemakaian,akunhapus,idsatuan,t_userid,t_updatetime,t_ipaddress,tipe)values(".$kodebarang.",".$namabarang.", ".$tipebarang.", ".$level.", ".$metodedepresiasi.",".$stddeppct.",".$akunaset.",".$akundep.",".$akunakdep.",".$akundisloss.",".$idsatuan.",". $t_userid.", now(), ".$t_ipaddress.", ".$tipe.")','".$_SERVER['REMOTE_ADDR']."')";
        ...
        $query = "insert into as_ms_barang (kodebarang,namabarang,tipebarang,level,metodedepresiasi,stddeppct
            ,akunaset,akunpengadaan,akunpemakaian,akunhapus,idsatuan,t_userid,t_updatetime,t_ipaddress,tipe,isbrg_aktif,kemasan)
                values
                ('$kodebarang', '$namabarang', '$tipebarang', '$level', '$metodedepresiasi','$stddeppct'
                , '$akunaset', '$akundep', '$akunakdep', '$akundisloss', '$idsatuan', '$t_userid', now(), '$t_ipaddress', '$tipe', '$aktif','$kemasan')";
    }
    else if($act == 'edit') 
	{
	$sqlIns="insert into user_log (log_user,log_time,log_action,log_query,log_ip) values ('".$_SESSION['id_user']."',sysdate(),'Update Master Barang','update as_ms_barang set kodebarang = ".$kodebarang.", namabarang = ".$namabarang.", tipebarang = ".$tipebarang.", level = ".$level."
        ...
        $query = "update as_ms_barang set kodebarang = '$kodebarang', namabarang = '$namabarang', tipebarang = '$tipebarang', level = '$level'
                ...
                , akunhapus = '$akundisloss', idsatuan = '$idsatuan', t_userid = '$t_userid', t_updatetime = now(), t_ipaddress = '$t_ipaddress', tipe = '$tipe',isbrg_aktif = '$aktif',kemasan='$kemasan'
            where idbarang = $id"; 
    }
    ...
    if($act == 'edit') 
	{
        if($res > 0) 
		{
           echo "<script>
                ...
                window.location = '$back_addr?tipe=".$tipe."';
                </script>";
        }
        else if($res == 0) 
		{
...
?>
    ...
    <body>
        <div align="center">
            ...
            if($act == 'edit') {
                $par .= "&idbarang=$id";
                $query = "SELECT idbarang,kodebarang,namabarang,idsatuan,akunpemakaian ,akunhapus,tipebarang,level,mru,statuskode,metodedepresiasi,akunpengadaan,stddeppct,lifetime,akunaset,akundep,akunakdep,akundisloss,akundisgain,akunmaintenance,akunlease,isbrg_aktif,kemasan FROM as_ms_barang WHERE idbarang = '$id'";
                $rs = mysql_query($query);
                            ...
                            <table width="625" border="0" cellspacing="0" cellpadding="2" align="center">
                                <tr>
                                    <td height="30" colspan="2" valign="bottom" align="right">
                                        <button class="Enabledbutton" id="backbutton" onClick="location='<?php echo $back_addr."?tipe=".$_GET['tipe'];?>'" title="Back" style="cursor:pointer">
                                ...
                                <form action="" id="form1" name="form1" method="post">
                                    ...
                                    <tr>
                                        ...
                                        <td class="content">&nbsp;
                                            <select name="satuan" id="satuan" class="txt">
                                                <?php
                                                $query = "select idsatuan from as_ms_satuan order by idsatuan";
                                                $rs = mysql_query($query);
									...
									<tr>
                                      ...
                                      <td class="content">&nbsp;
                                        <select name="kemasan" id="kemasan" class="txt">
                                          <?php
                                                $query = "select idsatuan from as_ms_satuan order by idsatuan;";
                                                $rs = mysql_query($query);
                                                while($row = mysql_fetch_array($rs)) {
                                ...
                                <tr>
                                    <td colspan="2" class="header2">Aktif /Non Aktif </td>
                                </tr>
                                <tr>
                                        <td height="20" class="label">Aktif</td>
                                        <td class="content">&nbsp;<input type="checkbox" name="aktif" id="aktif" <? if($rows['isbrg_aktif']==1){echo "checked value='1'";}else if($rows['isbrg_aktif']==0){echo"value='0'";}?> onClick="cek_aktif(this.value)"></td>
                              </tr>
                                <tr>
                                    <td colspan="2">&nbsp;
                                    </td>
		...
		function cek_aktif(val)
		{
			if(val=="0")
			{
				document.getElementById('aktif').value=1;
				 ...
				 //alert(document.getElementById('aktif').value);
			}
			else if(val=="1")
			{
