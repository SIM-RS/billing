...
function fSetNilaiArray(obj,par,idx){
var tpar=par;
var vtpar;
	while (tpar.indexOf(String.fromCharCode(5))>0){
		tpar=tpar.replace(String.fromCharCode(5),"'");
	...
	//for (var i=0;i<tpar.length;i++){
		vtpar=tpar[0].split('*-*');
		obj.document.forms[0].idbarang[idx].value=vtpar[1];
		vtpar=tpar[1].split('*-*');
		obj.document.forms[0].kodebarang[idx].value=vtpar[1];
		vtpar=tpar[2].split('*-*');
		obj.document.forms[0].namabarang[idx].value=vtpar[1];
