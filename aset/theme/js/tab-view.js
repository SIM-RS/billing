...
function TabView(id){
	...
	this.obj.cellSpacing = 0; this.obj.cellPadding = 0; this.obj.border = "1px";
	...
	this.box = document.createElement("DIV");
	this.box.id='divBlank_'+id;this.box.style.height='1px';this.box.style.position="absolute";this.box.style.border="solid 1px";this.box.style.borderTop="none";this.box.style.borderBottom="none";this.box.style.borderLeft="none";this.box.style.borderRight="none"; this.box.className = "TabOut";
	this.tab.appendChild(this.box);
	this.setTabCaption=function(str){
		var cpage=document.getElementById('page_'+id);
		var ctable=document.getElementById("TabTable_"+id);
		var cRow=ctable.insertRow(0);cRow.className = "TabOut";cRow.align = "center";
		...
		for (var i=0;i<this.TabCaptionArr.length;i++){
			...
			cRow.childNodes[i].onclick=function(){TabViewSwitch(this);}
			this.box = document.createElement("DIV");
			this.box.id='page_'+id+"_"+i;this.box.innerHTML="<br>This is Page "+(i+1);
...
var cdiv=document.getElementById("divBlank_"+Tabs.id.substr(0,Tabs.id.length-2));
	...
	for (var i=0;i<tr.cells.length;i++){
		if (tr.cells[i]==Tabs){
			...
			Tabs.onclick="";
			cdiv.style.width=parseInt(Tabs.style.width.substr(0,Tabs.style.width.length-2))+"px";
			var p=GetXY(Tabs);
			cdiv.style.left=parseInt(p[0])-1+"px";
			...
			tr.cells[i].onclick=function(){TabViewSwitch(this)};
