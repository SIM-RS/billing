...
function popupReport(p,q)
{
	...
	document.getElementById('trJns').style.visibility = "collapse";
	document.getElementById('trTmp').style.visibility = "collapse";
	document.getElementById('trPenjamin').style.visibility = "collapse";
	showTime(0);
	...
	switch (p){
		...
		case 114:
			...
			$('popup_div1').popup.show();
			if (q==1){
				document.form1.action='laporan/buku_jurnal_pembelian.php';
			}else{
				document.form1.action='../laporan/buku_jurnal_pembelian.php';
			}
			break;
		...
		case 115:
			...
			$('popup_div1').popup.show();
			if (q==1){
				document.form1.action='laporan/beban_lay.php';
			}else{
				document.form1.action='../laporan/beban_lay.php';
			}
			break;
...
/* Fungsi report billing */
function isiCombo(id,val,defaultId,targetId,fromHome,all_ = 0,evloaded)
{
	if(targetId=='' || targetId==undefined)
		targetId=id;
	var point = (fromHome == 1 ? '../billing/' : '../../billing/');
	var _all = (all_ == 1 ? "&all=1" : "");
	Request(point+'combo_utils.php?id='+id+'&value='+ val + _all + '&defaultId='+defaultId,id,'','GET',evloaded);
}
	
function change(fromHome){
	isiCombo('TmpLayanan',document.getElementById('JnsLayanan').value,'','',fromHome);
}
...
function popBilling(id, fromHome){
	switch(id){
		case 1:
			isiCombo('JnsLayanan','','','',fromHome,0, function(){
				isiCombo('TmpLayanan',document.getElementById('JnsLayanan').value,'','',fromHome);
				isiCombo('StatusPas','','','',fromHome,1);
			});
			...
			document.getElementById('trJns').style.visibility = "visible";
			document.getElementById('trTmp').style.visibility = "visible";
			document.getElementById('trPenjamin').style.visibility = "visible";
			
			document.getElementById('txtJam1').style.display = "none";
			document.getElementById('txtJam2').style.display = "none";
			new Popup('popup_div1',null,{modal:true,position:'center',duration:0.5})
			$('popup_div1').popup.show();
			if(fromHome == 1){
				var url = "../billing/laporan/tempatlayanan/";
			} else {
				var url = "../../billing/laporan/tempatlayanan/";
			}
			document.getElementById('form1').action=url+'Pendapatan.php';
			break;
		case 2:
			isiCombo('cmbKasir2','','','',fromHome,0,function(){
				isiCombo('nmKsr',document.getElementById('cmbKasir2').value,'','',fromHome,1);
			});
			...
			$('popup_div1').popup.show();
			if(fromHome == 1){
				var url = "../billing/laporan/kasir/";
			} else {
				var url = "../../billing/laporan/kasir/";
			}
            document.getElementById('form1').action=url+'PenerimaanKasir_new_ak.php';
			break;
	}
}