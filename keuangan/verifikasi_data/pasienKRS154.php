...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	var url,tmp;
		...
		{		
			 timeFil = document.getElementById('cmbTime').value;
            kso = document.getElementById('cmbKso').value;
            var url = "pasienKRS_utils.php?grid=1Rkp&tipe=<?php echo $tipe;?>&tipePend=0&kso="+kso+"&filter="+a.getFilter()+"&sorting="+a.getSorting()+"&page="+a.getPage();
            var tglA,tglZ;
            a.loadURL(url,"","GET");
		}else if (grd=="gridbox"){
			timeFil = document.getElementById('cmbTime').value;
            kso = document.getElementById('cmbKso').value;
            var url = "pasienKRS_utils.php?grid=piutangDetail&tipe=<?php echo $tipe;?>&tipePend=0&kso="+kso+"&filter="+a.getFilter()+"&sorting="+a.getSorting()+"&page="+a.getPage();
            var tglA,tglZ;
            a.loadURL(url,"","GET")
		}
		...
		function ambilDataDetail(){
			...
			url="pasienKRS_utils.php?grid=piutangDetail&txtTgl="+TglA+"&txtTgl2="+TglB+"&kso="+kso+"&posting="+cmbPost+"&tglslip="+tglslip+"&kasir="+tmp[8]+"&slipke="+tmp[9];
			//alert(url);
			a.loadURL(url,'','GET');
		...
		var cekbox=(p==true)?1:0;
			...
			//for (var i=0;i<a.getMaxRow();i++){
				aRkp.cellsSetValue(i+1,5,cekbox);
		...
		function kirim(){
			...
			url="pasienKRS_utils.php?grid=1Rkp&txtTgl="+TglA+"&txtTgl2="+TglB+"&kso="+kso+"&posting="+cmbPost+"&tglslip="+tglslip;
			//url="pasienKRS_utils.php?grid=1&txtTgl="+TglA+"&txtTgl2="+TglB+"&kso="+kso+"&posting="+cmbPost;
			//alert(url);
			if (tmpTglB>=tmpTglA){
				document.getElementById('btnVerifikasi').disabled=true;
				ShowLoadImg();
		...
		//var url2;
			...
			for (var i=0;i<aRkp.getMaxRow();i++){
			//for (var i=0;i<a.getMaxRow();i++){
				if (aRkp.obj.childNodes[0].childNodes[i].childNodes[4].childNodes[0].checked){
			...
			if (tmp!="")
			{
				...
				{
					url="pasienKRS_utils.php?grid=1Rkp&act=verifikasi&userId=<?php echo $userId; ?>&txtTgl="+txtTglA+"&txtTgl2="+txtTglB+"&kso="+kso+"&posting="+document.getElementById('cmbPost').value+"&fdata="+tmp+"&no_slip="+no_slip;
		...
		aRkp.setColHeader("NO,TANGGAL PULANG,NAMA KASIR,NILAI,VERIFIKASI<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
		aRkp.setCellType("txt,txt,txt,txt,chk");
		//aRkp.setSubTotal(",,,SubTotal :&nbsp;,0,");
		//aRkp.setSubTotal(",,,SubTotal :&nbsp;,0,,");
		aRkp.setSubTotal(",,,,,,");
		aRkp.setIDColHeader(",,,mp.no_rm,,,");
		aRkp.setColWidth("35,100,450,120,80");//35,80,65,100,480,100,70
		aRkp.setCellAlign("center,center,center,right,center");
		aRkp.setSubTotalAlign("center,center,center,center,center");
		...
		aRkp.baseURL("pasienKRS_utils.php?grid=1Rkp&tipe=<?php echo $tipe;?>&tipePend=0&kso="+kso+"&waktu="+timeFil+"&bln="+bln+"&thn="+thn+"&tglslip="+tglslip+"&posting="+document.getElementById('cmbPost').value);
        aRkp.Init();
		...
		a.baseURL("pasienKRS_utils.php?grid=piutangDetail&tipe=<?php echo $tipe;?>&tipePend=0&kso="+kso+"&waktu="+timeFil+"&bln="+bln+"&thn="+thn+"&tglslip="+tglslip+"&posting="+document.getElementById('cmbPost').value);
