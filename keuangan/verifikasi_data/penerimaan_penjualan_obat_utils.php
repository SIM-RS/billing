		...
		if ($posting==0)
		{
			...
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				if ($cNilai>0){
					...
					$sql="UPDATE $dbapotek.a_pengembalian_uang SET VERIF=1,USER_VERIF='$userId',TGL_VERIF=NOW() WHERE idBalik='$IdKreditUtang' AND VERIF=0";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}
				...
				if (mysql_errno()==0 && mysql_affected_rows()>0)
				{
					...
					if (mysql_errno()>0){
						if ($cNilai>0){
							...
							$sql="UPDATE $dbapotek.a_pengembalian_uang SET VERIF=0,USER_VERIF='0',TGL_VERIF=NULL WHERE idBalik='$IdKreditUtang' AND VERIF=1";
							//echo $sql."<br>";
							$rsPost=mysql_query($sql);
						}
			...
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				/*if ($cNilai>0){
					//===Cek Sudah Posting apa belum===$dbkeuangan.k_transaksi
					...
					$sql="SELECT * FROM $dbapotek.a_pengembalian_uang WHERE unit_id='$IdUnit' AND DATE(tgl_act)='$tanggal' AND no_penjualan='$NoPenjualan' AND no_rm='$NoRm' AND shift='$ShiftResep' AND VERIF=1 AND POSTING=0";
					//echo $sql.";<br>";
				}
				
				$rsPost=mysql_query($sql);
				if (mysql_num_rows($rsPost)>0){*/
					//============Cek Sudah Dimasukkan ke BKM / Posting apa Belum========
					$sql="SELECT * FROM k_transaksi WHERE id_trans='$idTrans_Penerimaan_Kasir_Farmasi' AND (bkm_detail_id='0' AND posting=0) 
						AND tgl='$tanggal' AND no_faktur='$NoPenjualan' AND no_terima='$NoRm' 
						AND unit_id_billing='$IdUnitBilling' AND kasir_id='$UserEntryResep' AND unit_id='$IdUnit'";
					//echo $sql.";<br>";
					$rsPost=mysql_query($sql);
					if (mysql_num_rows($rsPost)>0){
						if ($cNilai>0){
						...
						{
							if ($cNilai>0){
								$sql="DELETE FROM k_transaksi WHERE id_trans='$idTrans_Penerimaan_Kasir_Farmasi' AND tgl='$tanggal' AND no_faktur='$NoPenjualan' AND no_terima='$NoRm' AND unit_id_billing='$IdUnitBilling' AND kasir_id='$UserEntryResep' AND unit_id='$IdUnit' AND nilai_sim='$cNilai'";
								//echo $sql."<br>";
							}else{
								$sql="DELETE FROM k_transaksi WHERE id_trans='$idTrans_Penerimaan_Kasir_Farmasi' AND tgl='$tanggal' AND no_faktur='$NoPenjualan' AND no_terima='$NoRm' AND unit_id_billing='$IdUnitBilling' AND kasir_id='$UserEntryResep' AND unit_id='$IdUnit' AND nilai_sim='$cNilai'";
								//echo $sql."<br>";
						...
						$msg="Proses unVerifikasi Gagal !. Transaksi Sudah Dimasukkan ke BKM atau Sudah Diposting, Jadi Tidak Bisa UnVerifikasi.";
					}
				//}else{
...
if($statusProses=='Error') {
    ...
    if ($grd == "penerimaanPenjualanObat") {
			...
			if ($shift!=0){
				...
				$fFarShift=" AND apu.shift='$shift'";
			}
			
			$fposting=" AND ku.VERIF = $posting";
			$fFarposting=" AND apu.VERIF = $posting";
			...
			/*$sql="SELECT DISTINCT
					...
					GROUP BY ku.FK_NO_PENJUALAN, ku.UNIT_ID, ku.NO_PELAYANAN, ku.NORM, ku.SHIFT";*/
			...
			$sql="SELECT * FROM 
					...
					WHERE (ku.BILLING_BAYAR_OBAT_ID = 0
						...
						AND DATE(ku.TGL_BAYAR) = '$tanggal'
						$fposting
						$fFarmasi $fShift $fkso
					GROUP BY ku.FK_NO_PENJUALAN, ku.UNIT_ID, ku.NO_PELAYANAN, ku.NORM, ku.SHIFT
				UNION
				SELECT DISTINCT
				  apu.idBalik         AS ID,
				  apu.no_pengembalian AS NO_BAYAR,
				  apu.nilai * -1           AS HARGA_TOTAL,
				  kt.no_bukti,
				  SUM(FLOOR((rp.qty_retur)*ap.HARGA_NETTO)) * -1 AS HARGA_NETTO,
				  0                   AS BILLING_BAYAR_OBAT_ID,
				  DATE_FORMAT(apu.tgl_act,'%d/%m/%Y') AS tgl1,
				  apu.tgl_act         AS TGL,
				  ap.UNIT_ID,
				  au.UNIT_NAME        AS apotek,
				  ap.NO_PENJUALAN,
				  ...
				  aunit.UNIT_NAME,
				  aunit.unit_billing
				FROM $dbapotek.a_pengembalian_uang apu
				  INNER JOIN $dbapotek.a_return_penjualan rp
					ON apu.no_retur = rp.no_retur
				  INNER JOIN $dbapotek.a_penjualan ap
				  ...
				  LEFT JOIN $dbapotek.a_unit aunit
					ON (ap.RUANGAN = aunit.UNIT_ID)
				  LEFT JOIN k_transaksi kt
					ON (apu.idBalik = kt.fk_id)
				WHERE DATE(apu.tgl_act) = '$tanggal'
					/*AND apu.VERIF = 0*/
					$fFarposting 
					$fFarmasi $fFarShift $fkso 
				GROUP BY ap.NO_PENJUALAN, ap.UNIT_ID, apu.no_pelayanan, ap.NO_PASIEN, apu.shift) AS gab
				WHERE 1=1 $filter";
