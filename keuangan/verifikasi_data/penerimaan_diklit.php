				...
				<td align="center" >
					<table border="0" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td>:&nbsp;<input id="tgl_s" name="tgl_s" readonly size="11" class="txtcenter" type="text" value="<?php echo $tgl; ?>" />&nbsp;<input type="button" name="btnTgl_s" value="&nbsp;V&nbsp;" class="tblBtn" onClick="gfPop.fPopCalendar(document.getElementById('tgl_s'),depRange);"/> s/d <input id="tgl_d" name="tgl_d" readonly size="11" class="txtcenter" type="text" value="<?php echo $tgl; ?>" />&nbsp;<input type="button" name="btnTgl_d" value="&nbsp;V&nbsp;" class="tblBtn" onClick="gfPop.fPopCalendar(document.getElementById('tgl_d'),depRange);"/></td>
						</tr>
					...
					<table border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td height="30" style="padding-left:40px;">
								<select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="gantiPosting(this.value)">
									<option value="0">BELUM VERIFIKASI</option>
									<option value="1">SUDAH VERIFIKASI</option>
								</select>
								<button style="" disabled id="cetakDiklit" onclick="cetak()"><img width="16px" alt="Cetak Laporan Pendapatan Parkir" src="../icon/printer.png">&nbsp;Cetak Laporan</button>
							</td>
...
<script type="text/javascript">
	...
	a1.setColHeader("No,Tanggal<br />Bayar,No Induk,Nama,No  Kwitansi,Jenis Bayar,Periode Bayar,Nilai,Pendanaan,Penerima,VERIF<br/><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>");
	...
	a1.setColWidth("30,80,100,150,120,150,100,100,100,100,50");
	...
	a1.baseURL("penerimaan_diklit_utils.php?grd=penerimaanDiklit&tanggal="+document.getElementById('tgl').value+"&tgl_s="+document.getElementById('tgl_s').value+"&tgl_d="+document.getElementById('tgl_d').value+"&posting="+document.getElementById('cmbPost').value+"&no_slip="+document.getElementById('noSlip').value); //+"&kasir="+document.getElementById('cmbKasir').value
	a1.Init();
	
	function goFilterAndSort(grd){
	var url;
		if (grd=="gridbox"){
			url="penerimaan_diklit_utils.php?grd=penerimaanDiklit&tanggal="+document.getElementById('tgl').value+"&tgl_s="+document.getElementById('tgl_s').value+"&tgl_d="+document.getElementById('tgl_d').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage()+"&no_slip="+document.getElementById('noSlip').value; //+"&kasir="+document.getElementById('cmbKasir').value
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
...
/* 				if (idata[3]=="1"){
		...
		if (tmp!=""){
			...
			else
			{
				...
				Request(url,'hasilAjax',"","GET",function(v){
					var hasil = v.split('|');
					if(hasil[0] == 'sukses'){
						alert('Proses '+hasil[1]+' Data Berhasil!');
					} else {
						alert('Proses '+hasil[1]+' Data Gagal!');
					}
					kirim();
					document.getElementById('btnVerifikasi').disabled=false;
	...
	function gantiPosting(par){
		...
		if(par==0){
			...
			document.getElementById('btnVerifikasi').disabled = false;
			document.getElementById('cetakDiklit').disabled = true;
		}else{
			document.getElementById('spnBtnVer').innerHTML='&nbsp;UnVerifikasi';
			document.getElementById('chkAll').disabled = false;
			document.getElementById('btnVerifikasi').disabled = false;
			document.getElementById('cetakDiklit').disabled = false;
		}
		kirim();
	}
	
	function chkKlik(p){
		var posting = jQuery('#cmbPost').val();
		var cekbox=(p==true)?1:0;
		//alert(p);
		for (var i=0;i<a1.getMaxRow();i++){
			if(posting == '1'){
				if(!a1.obj.childNodes[0].childNodes[i].childNodes[10].childNodes[0].disabled){
	...
	function kirim(){
		...
		url="penerimaan_diklit_utils.php?grd=penerimaanDiklit&tanggal="+document.getElementById('tgl').value+"&tgl_s="+document.getElementById('tgl_s').value+"&tgl_d="+document.getElementById('tgl_d').value+"&posting="+cmbPost+"&no_slip="+document.getElementById('noSlip').value; //+"&kasir="+document.getElementById('cmbKasir').value
		//alert(url);
		a1.loadURL(url,"","GET"); 
	}
	...
	function cetak(){
		...
		var cmbPost = document.getElementById('cmbPost').value;
		window.open("../penerimaan/cetak_diklit.php?tgl_s="+tgl_s+"&tgl_d="+tgl_d+"&no_slip="+document.getElementById('noSlip').value+"&posting="+cmbPost+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting(), "", "scrollbars=yes, width=1200, height=600, left=0");
	}
