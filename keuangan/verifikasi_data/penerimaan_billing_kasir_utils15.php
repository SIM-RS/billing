...
switch(strtolower($_REQUEST['act'])) {
    ...
    case 'koreksi_distribusi':
		...
		for ($i=0;$i<count($arfdata);$i++)
		{
			...
			$sql="SELECT * FROM $dbbilling.b_bayar WHERE id='$idBayar' AND posting=0";
			//echo $sql.";<br>";
			...
			if (mysql_num_rows($rsPost)>0){
				...
				if (($nilaiBayar==($nBiayaPasien-$nBayarPasien)) || ($nilaiBayar==$nBiayaPasien)){
					...
					$sql="SELECT kunjungan_id FROM $dbbilling.b_bayar WHERE id='$idBayar' AND posting=0";
					//echo $sql.";<br>";
					$rsPost=mysql_query($sql);
					if (mysql_num_rows($rsPost)>0){
						$rwPost=mysql_fetch_array($rsPost);
...
IF(t.status_out=0,(IF(DATEDIFF(IFNULL(t.tgl_out,b.tgl),t.tgl_in)=0,1,DATEDIFF(IFNULL(t.tgl_out,b.tgl),t.tgl_in)+1))*((t.retribusi)), (IF(DATEDIFF(IFNULL(t.tgl_out,b.tgl),t.tgl_in)=0,0,DATEDIFF(IFNULL(t.tgl_out,b.tgl),t.tgl_in)+1))*((t.retribusi))) retribusi,										  
										  0               AS utip,
										  0               AS ppn,
										  bt.tipe,
							...
							if (mysql_errno()==0){
								while ($rwPost=mysql_fetch_array($rsPost)){
									...
									$sql="INSERT INTO k_transaksi(fk_id,fk_bayar_posting_id,id_trans,tgl,tgl_klaim,no_bukti,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,isFarmasi,isPavilyun,verifikasi,posting,user_act,tgl_act) VALUES('$idBayar','$idpost','$IdTrans','".$tglP."','".tglSQL($tanggalSlip)."','$no_slip','$unitId','$ksoId','$kasirId','$biayaRS','$nilai','$tipe','$is_paviliun',1,0,'$idUser',NOW())";
									//echo $sql.";<br>";
									...
									if (mysql_errno()>0)
									{
										...
										if ($ppn>0){
											...
											$sql="INSERT INTO k_transaksi(fk_id,fk_bayar_posting_id,id_trans,tgl,tgl_klaim,no_bukti,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,isFarmasi,isPavilyun,verifikasi,posting,user_act,tgl_act) VALUES('$idBayar','$idpost','$IdTrans','".$tglP."','".tglSQL($tanggalSlip)."','$no_slip','0','$ksoId','$kasirId','$ppn','$ppn','3','$is_paviliun',1,0,'$idUser',NOW())";
											//echo $sql.";<br>";
											$rsIns=mysql_query($sql);
										}
										...
										if ($retribusi>0){
											...
											$sql="INSERT INTO k_transaksi(fk_id,fk_bayar_posting_id,id_trans,tgl,tgl_klaim,no_bukti,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,isFarmasi,isPavilyun,verifikasi,posting,user_act,tgl_act) VALUES('$idBayar','$idpost','$IdTrans','".$tglP."','".tglSQL($tanggalSlip)."','$no_slip','12','$ksoId','$kasirId','$retribusi','$retribusi','4','$is_paviliun',1,0,'$idUser',NOW())";
											//echo $sql.";<br>";
											$rsIns=mysql_query($sql);
										}
										...
										if ($utip>0){
											...
											$sql="INSERT INTO k_transaksi(fk_id,fk_bayar_posting_id,id_trans,tgl,tgl_klaim,no_bukti,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,isFarmasi,isPavilyun,verifikasi,posting,user_act,tgl_act) VALUES('$idBayar','$idpost','$IdTrans','".$tglP."','".tglSQL($tanggalSlip)."','$no_slip','0','$ksoId','$kasirId','$utip','$utip','5','$is_paviliun',1,0,'$idUser',NOW())";
											//echo $sql.";<br>";
											$rsIns=mysql_query($sql);
										}
									}
			...
			if ($statusProses==''){
				...
				{						
					...
					$id_sak_utip=4134;	//Utip - Lainnya
					$jenistrans=9;	//1112a - Terjadinya Transaksi Pendapatan (Tunai) - Tipe: Normal
					
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING,VERIFIKASI) 
		VALUES('$id_k_trans','$user_kasir','0','1',$id_sak,'$tglP','$nokw','$uraian',$ntotal,0,now(),'0','D',0,$jenistrans,$jenistrans,1,'$no_bukti','0','0',1,0)";
					//echo $sql.";<br>";
					$rsPost=mysql_query($sql);
					...
					while ($rwData=mysql_fetch_array($rsData)){
						...
						if ($unit_id<>0){
							...
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,KSO_ID,POSTING,VERIFIKASI) 
			VALUES('$id_k_trans','$user_kasir','0','1',$id_pendapatan,'$tglP','$nokw','$uraian',0,$nilai_slip,now(),0,'K',0,$jenistrans,$jenistrans,1,'$no_bukti',$kso,$idunit,$kso,1,0)";
							//echo $sql.";<br>";
							$rsPost=mysql_query($sql);
						}else{
							//$nppn +=$nilai_slip;
					...
					if ($ntotal_ppn>0){
						$uraian="Penerimaan Kasir : ".$ckasir.", Slip : $nslip, ".$tanggalSlipAk;
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,KSO_ID,POSTING,VERIFIKASI) 
		VALUES('$id_k_trans','$user_kasir','0','1',$id_sak_ppn,'$tglP','$nokw','$uraian',0,$ntotal_ppn,now(),0,'K',0,$jenistrans,$jenistrans,1,'$no_bukti','$kso','0','$kso',1,0)";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
					}
					...
					if ($ntotal_utip>0){
						$uraian="Penerimaan Kasir : ".$ckasir.", Slip : $nslip, ".$tanggalSlipAk;
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,KSO_ID,POSTING,VERIFIKASI) 
		VALUES('$id_k_trans','$user_kasir','0','1',$id_sak_utip,'$tglP','$nokw','$uraian',0,$ntotal_utip,now(),0,'K',0,$jenistrans,$jenistrans,1,'$no_bukti','$kso','0','$kso',1,0)";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
					}
				}
		...
		/*$fBtind="LEFT JOIN (SELECT bt.id,bt.bayar_id,bt.nilai
							...
							WHERE bt.tipe = 0
								...
								$fTipeTrans
							  	$fFilterTgl
							UNION
							SELECT bt.id,bt.bayar_id,bt.nilai
							FROM $dbbilling.b_bayar b
							  INNER JOIN $dbbilling.b_bayar_tindakan bt
								ON b.id = bt.bayar_id
							  INNER JOIN $dbbilling.b_tindakan_kamar t
		...
		$fBtind="LEFT JOIN (SELECT bt.id,bt.bayar_id,bt.nilai
							...
							WHERE bt.tipe = 0
								...
								$fTipeTrans
							  	$fFilterTgl
							UNION
							SELECT bt.id,bt.bayar_id,bt.nilai+(DATEDIFF(IFNULL(IF(t.tgl_out IS NOT NULL && DATEDIFF(t.tgl_out, b.tgl_act) < 0, t.tgl_out, b.tgl_act), b.tgl_act),t.tgl_in)+1)*t.retribusi AS nilai
							FROM $dbbilling.b_bayar b
							  INNER JOIN $dbbilling.b_bayar_tindakan bt
								ON b.id = bt.bayar_id
							  INNER JOIN $dbbilling.b_tindakan_kamar t
								ON bt.tindakan_id = t.id
							WHERE (bt.tipe = 1/* OR bt.tipe = 4*/)
								$fkasir 
								$fSlip_ke 
								$fposting 
								$fTipeTrans
