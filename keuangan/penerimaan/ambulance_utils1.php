...
if($statusProses=='Error') {
    ...
    if($grid == 'grid1') {
		while ($rows=mysql_fetch_array($rs)) {
			$i++;
			$dt.=$rows["id"]."|".$rows['ambulan_tipe']."|".$rows['nilai'].chr(3).number_format($i,0,",",".").chr(3).tglSQL($rows["tgl"]).chr(3).$rows["tglSetor"].chr(3).$rows["jamSetor"].chr(3).$rows["no_bukti"].chr(3).$rows["nama"].chr(3).number_format($rows["nilai"],0,",",".").chr(3).$rows["ket"].chr(3).$rows["username"].chr(6);
		}
    }
