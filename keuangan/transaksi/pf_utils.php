...
if($statusProses=='Error') {
    ...
    if($grid == 1) {
        $sql="select t1.* from(
                select ap.id,no_penjualan,no_pasien,nama_pasien,dokter,ap.HARGA_TOTAL as sub_tot,u.unit_name as unit
    ...
    if($grid == 1) {
        while ($rows=mysql_fetch_array($rs)) {
            $i++;
            $dt.=$rows["id"]."|".$rows["no_penjualan"]."|".$rows["unit_id"].chr(3).number_format($i,0,",","").chr(3).$rows["unit_name"].chr(3).$rows["no_penjualan"].chr(3).$rows["no_pasien"].chr(3).$rows["nama_pasien"].chr(3).$rows["unit"].chr(3).$rows["dokter"].chr(3).number_format($rows["sub_tot"],2,",",".").chr(6);
        }
    }
    else if($grid == 2) {
        while ($rows=mysql_fetch_array($rs)) {
