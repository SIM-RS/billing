...
$id=$_REQUEST['id'];
$jum=substr_count($id,'.');
$sql="SELECT ma.ma_kode
FROM k_ms_transaksi transaksi
  INNER JOIN $dbanggaran.ms_ma ma
    ON ma.ma_id = transaksi.id_ma_dpa
WHERE transaksi.id = ".$id;
$q=mysql_query($sql);
$ro=mysql_fetch_array($q);
$jum=substr_count($ro[0],'.');
?>
<html>
...
<?php	
  ...
  $maxlevel=0;
  $cnt=0;
  $strSQL = "SELECT * FROM $dbanggaran.ms_ma msak WHERE msak.ma_kode like '$ro[0]%' ORDER BY msak.ma_rek,msak.ma_kode,msak.ma_level";
  ...
  while ($rows=mysql_fetch_array($rs)){
		 ...
		 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['ma_id']."*|*".$par[1]."*-*".$rows['ma_nama']."');window.close();";
		 $tree[$cnt][3]= "";
		 $tree[$cnt][4]= 0;
