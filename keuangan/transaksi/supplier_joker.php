...
<form id="form1">
    ...
    <table width="500" border="0" cellpadding="0" cellspacing="0" align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;" id="tbl">
        <tr>
            <td colspan="4">
                <fieldset style="width: 300px;background-color: silver">
                    <!--legend>
        ...
        <tr class="headtable" style="text-align:center; font-weight:bold;" height="30">
            ...
            <td id="no_spk" class="tblheader" onClick="ifPop.CallFr(this);">No SPK</td>
...
INNER JOIN ".$dbapotek.".a_pbf ON a_p.PBF_ID=a_pbf.PBF_ID) AS t1 GROUP BY t1.BATCH) as tbl ".$filter." order by ".$sorting;
		    ...
		    $sql="select * from (SELECT m.po_id,p.no_po,p.no_po no_spk,DATE_FORMAT(p.tgl_po,'%d-%m-%Y') as tgl,sum(m.jml_msk*harga_unit) as tagihan
			...
			group by p.no_po
			order by p.tgl_po) as tbl ".$filter." order by ".$sorting;
		}
		//echo $sql."<br>";		
                $rs = mysql_query($sql);
		//echo mysql_error();
                ...
                while($rw = mysql_fetch_array($rs)) {
                    $arfvalue = $rw['no_po'].'|'.number_format($rw['tagihan'],2,",",".").'|'.$bln.'|'.$thn.'|'.$rw['no_spk'];
                    ?>
