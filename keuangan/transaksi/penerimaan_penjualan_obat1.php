    ...
    <body bgcolor="#808080" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"><!-- onload="viewTime()"-->
        ...
        <div align="center">
            <table width="1000" border="0" cellpadding="0" cellspacing="0" bgcolor="#EAEFF3">
                ...
                <tr>
                    <td height="30" style="padding-left:40px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="kirim();">
...
var fromHome=<?php echo $fromHome ?>;
	...
	a1.setColHeader("NO,KSO,CARA BAYAR,UNIT PELAYANAN,SHIFT,NILAI,NILAI SLIP,NILAI HPP,VERIFIKASI<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	...
	a1.setIDColHeader(",KSO_NAMA,cara_bayar,nama,SHIFT,,,,");
	a1.setColWidth("50,150,90,160,,100,100,100,50");
	a1.setCellAlign("center,left,center,left,center,right,right,right,center");
	a1.setSubTotalAlign("center,center,center,right,center,right,right,right,left");
	a1.setCellHeight(20);
	a1.setImgPath("../icon");
	a1.setIDPaging("paging");
	a1.onLoaded(konfirmasi);
	//alert("../transaksi/utils.php?grd=penerimaanPenjualanObat&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value);
	a1.baseURL("../transaksi/utils.php?grd=penerimaanPenjualanObat&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value+"&shift="+document.getElementById('cmbShift').value);
	a1.Init();
	...
	var tmp;
		...
		if (val!=undefined){
			...
			a1.cellSubTotalSetValue(6,tmp[1]);
			a1.cellSubTotalSetValue(7,tmp[2]);
			a1.cellSubTotalSetValue(8,tmp[3]);
			if(key=='Error'){
	...
	function kirim(){
		var url;
		url="../transaksi/utils.php?grd=penerimaanPenjualanObat&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value+"&shift="+document.getElementById('cmbShift').value;
		//alert(url);
		a1.loadURL(url,"","GET"); 
	}
	//=========================================
	...
	var url2;
		...
		for (var i=0;i<a1.getMaxRow();i++)
		{
			if (a1.obj.childNodes[0].childNodes[i].childNodes[8].childNodes[0].checked)
			{
				...
				tmp+=tglx+String.fromCharCode(5)+idata[0]+String.fromCharCode(5)+idata[1]+String.fromCharCode(5)+idata[2]+String.fromCharCode(5)+nilai+String.fromCharCode(5)+idata[3]+String.fromCharCode(5)+no_slip+String.fromCharCode(5)+<?=$userId;?>+String.fromCharCode(5)+idata[4]+String.fromCharCode(5)+idata[5]+String.fromCharCode(5)+idata[7]+String.fromCharCode(6);
			}
		...
		if (tmp!="")
		{
			...
			{
				url="../transaksi/utils.php?grd=penerimaanPenjualanObat&act=verifikasiPenerimaanPenjualanObat&idUser=24&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value+"&shift="+document.getElementById('cmbShift').value+"&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}
