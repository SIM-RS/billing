...
switch(strtolower($_REQUEST['act'])) {
	case "verif_penerimaan_kasir_farmasi":
		...
		if ($posting==0)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$Kso_Id=$cdata[9];
				$sql="UPDATE $dbapotek.a_penjualan SET VERIFIKASI=1,VERIFIKASI_ACT='$userId' WHERE UNIT_ID='$IdUnit' AND USER_ID='$UserEntryResep' AND TGL='$tanggal' AND NO_PENJUALAN='$NoPenjualan' AND NO_PASIEN='$NoRm' AND CARA_BAYAR=1 AND SHIFT='$ShiftResep' AND VERIFIKASI=0";
				echo $sql."<br>";
				$rsPost=mysql_query($sql);
				if (mysql_errno()==0 && mysql_affected_rows()>0)
				{
					$sql="INSERT INTO k_transaksi(id_trans,tgl,tgl_klaim,no_bukti,no_faktur,no_terima,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,nilai_hpp,verifikasi,posting,user_act,tgl_act,cara_bayar,unit_id,ket) VALUES('$idTrans_Penerimaan_Kasir_Farmasi','$tanggal','$tanggalSlip','$no_slip','$NoPenjualan','$NoRm','$IdUnitBilling','$Kso_Id','$UserEntryResep','$cNilai','$cNilai','$cNetto',1,0,'$userId',NOW(),'1','$IdUnit','Penerimaan Kasir Farmasi')";
					echo $sql."<br>";
					$rsPost=mysql_query($sql);
					if (mysql_errno()>0){
						$sql="UPDATE $dbapotek.a_penjualan SET VERIFIKASI=0,VERIFIKASI_ACT='0' WHERE UNIT_ID='$IdUnit' AND USER_ID='$UserEntryResep' AND TGL='$tanggal' AND NO_PENJUALAN='$NoPenjualan' AND NO_PASIEN='$NoRm' AND CARA_BAYAR=1 AND SHIFT='$ShiftResep' AND VERIFIKASI=1";
						echo $sql."<br>";
						$rsPost=mysql_query($sql);
					}
				}
			...
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$IdUnitBilling=$cdata[8];
				$Kso_Id=$cdata[9];
				$sql="UPDATE $dbapotek.a_penjualan SET VERIFIKASI=0,VERIFIKASI_ACT=0 WHERE UNIT_ID='$IdUnit' AND USER_ID='$UserEntryResep' AND TGL='$tanggal' AND NO_PENJUALAN='$NoPenjualan' AND NO_PASIEN='$NoRm' AND CARA_BAYAR=1 AND SHIFT='$ShiftResep' AND VERIFIKASI=1 AND POSTING=0";
				echo $sql."<br>";
				$rsPost=mysql_query($sql);
				if (mysql_errno()==0 && mysql_affected_rows()>0)
				{
					$sql="DELETE FROM k_transaksi WHERE id_trans='$idTrans_Penerimaan_Kasir_Farmasi' AND tgl='$tanggal' AND no_faktur='$NoPenjualan' AND no_terima='$NoRm' AND unit_id_billing='$IdUnitBilling' AND kasir_id='$UserEntryResep' AND unit_id='$IdUnit'";
					echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}
			}
...
if($statusProses=='Error') {
    ...
    if ($grd == "penerimaanPenjualanObat") {
			...
			if ($shift!=0){
				$fShift=" AND ap.SHIFT='$shift'";
			}
			
			$fposting=" AND ap.VERIFIKASI = $posting";
			...
			$sql = "SELECT
					  ...
					  (ap.HARGA_TOTAL) AS HARGA_TOTAL2,
					  SUM((ap.QTY_JUAL) * ap.HARGA_SATUAN) AS HARGA_TOTAL,
					...
					WHERE ap.TGL = '$tanggal' AND ap.CARA_BAYAR = 1
						$fposting
    ...
    if ($grd == "penerimaanPenjualanObat") {
		if($posting==0){
			...
			while ($rows=mysql_fetch_array($rs)) 
			{
				...
				$sisip=$rows["UNIT_ID"]."|".$rows["NO_PENJUALAN"]."|".$rows["NO_PASIEN"]."|".$rows["NO_RESEP"]."|".$rows["USER_ID"]."|".$rows["SHIFT"]."|".$nilai."|".$netto."|".$rows["unit_billing"]."|".$rows["KSO_ID"];
				$dt.=$sisip.chr(3).$i.chr(3).$rows["NO_PENJUALAN"].chr(3).$rows["NO_RESEP"].chr(3).$rows["NO_PASIEN"].chr(3).$rows["NAMA_PASIEN"].chr(3).$rows["cbayar"].chr(3).$rows["KSO"].chr(3).$rows["UNIT_NAME"].chr(3).$rows["SHIFT"].chr(3).number_format($nilai,0,",",".").chr(3).number_format($netto,0,",",".").chr(3)."0".chr(6);
			}
		}
		else{
			$sqlTot="SELECT IFNULL(SUM(tot.HARGA_TOTAL),0) totJual,IFNULL(SUM(tot.HARGA_NETTO),0) totBahan FROM (".$sql.") AS tot";
    ...
    if ($dt!=$totpage.chr(5)) {
        $dt=substr($dt,0,strlen($dt)-1).chr(5).$_REQUEST['act']."|".$grd."|".number_format($ntot,0,",",".")."|".number_format($ntot2,0,",",".");
        $dt=str_replace('"','\"',$dt);
    }
