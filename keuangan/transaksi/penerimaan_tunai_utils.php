...
WHERE bt.tipe=0 AND bt.nilai>0 $waktu) AS t1 INNER JOIN $dbbilling.b_tindakan_ak t ON t1.tindakan_id=t.b_tindakan_id
INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id WHERE p.unit_id='$unit_id' $kso AND t.tipe_pendapatan=0) AS t2
...
INNER JOIN $dbbilling.b_tindakan_ak t ON bt.tindakan_id=t.b_tindakan_id INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id 
WHERE bt.tipe=0 $waktu $kso AND t.tipe_pendapatan=0 AND p.unit_id='$unit_id'
UNION
...
INNER JOIN $dbbilling.b_tindakan_kamar_ak t ON bt.tindakan_id=t.b_tindakan_kamar_id INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id
WHERE bt.tipe=1 $waktu $kso AND t.tipe_pendapatan=0 AND p.unit_id='$unit_id') AS t1 GROUP BY t1.kunjungan_id) AS t2 
