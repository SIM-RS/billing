...
switch ($act){
	case "verifikasiPenerimaanPenjualanObat":
		...
		if ($posting==0)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$sql="INSERT INTO k_transaksi(id_trans,tgl,tgl_klaim,no_bukti,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,nilai_hpp,verifikasi,posting,user_act,tgl_act,cara_bayar,unit_id,ket) VALUES('39','$cdata[0]','$tanggalSlip','','$cdata[1]','$cdata[2]','$cdata[10]','$cdata[3]','$cdata[4]','$cdata[5]',1,0,'$cdata[7]',NOW(),'$cdata[8]','$cdata[9]','$cdata[6]')";
				//echo $sql."<br>";
			...
			{
				$cdata=$arfdata[$i];
				$sql="SELECT * FROM k_transaksi WHERE id='$cdata'";
				//echo $sql;
				$cek=mysql_query($sql);
	...
	case "verifikasiPenerimaanBillingKasir":
		...
		if ($posting==0)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				$cdata=explode(chr(5),$arfdata[$i]);
				$kso=$cdata[2];
				$sql="INSERT INTO k_transaksi(id_trans,tgl,tgl_klaim,no_bukti,unit_id_billing,kso_id,kasir_id,nilai_sim,nilai,verifikasi,posting,user_act,tgl_act) VALUES('38','$cdata[4]','$tanggalSlip','$cdata[5]','$cdata[1]',$kso,'$cdata[0]','$cdata[3]','$cdata[6]',1,0,'$cdata[7]',NOW())";
				//echo $sql."<br>";
				$rsPost=mysql_query($sql);
				if (mysql_errno()<=0)
				{
	...
	VALUES($notrans,$id_piutang,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
		...
		else
		{
			...
			{
				$cdata=explode(chr(5),$arfdata[$i]);
				$sql="DELETE FROM k_transaksi WHERE id='".$cdata[0]."' AND posting=0";
				//echo $sql."<br>";
				$rsPost=mysql_query($sql);
				if (mysql_errno()<=0)
				{
	...
	else 
	{
		...
		case "penerimaanBillingKasir":			
			...
			if ($posting==0)
			{
				...
				UNION
				SELECT bbt.id,bbt.bayar_id,bbt.tindakan_id,IF((bbt.kso_id=0 OR ISNULL(bbt.kso_id)),p.kso_id,bbt.kso_id) kso_id,bbt.nilai,bbt.tipe,bbt.tgl,bbt.user_act,p.unit_id FROM (SELECT bt.*,b.tgl,b.user_act FROM ".$dbbilling.".b_bayar b INNER JOIN ".$dbbilling.".b_bayar_tindakan bt ON b.id=bt.bayar_id 
...
$dt=$totpage.chr(5);
	...
	if($posting==0){
		...
		{
			$i++;
			$dt.=$rows["id"].chr(3).$i.chr(3).$rows["KSO_NAMA"].chr(3).$rows["cara_bayar"].chr(3).$rows["nama"].chr(3).$rows["SHIFT"].chr(3).number_format($rows["nJual"],0,",",".").chr(3).number_format($rows["nSlip"],0,",",".").chr(3).number_format($rows["nBahan"],0,",",".").chr(3)."0".chr(6);
		}
	}
	break;
	case "penerimaanBillingKasir":
	...
	while ($rows=mysql_fetch_array($rs)) 
	{
		...
		$i++;
			...
			$dt.=$rows["id"]."|".$rows["user_act"]."|".$rows["unit_id"]."|".$rows["kso_id"]."|".$rows["nilai_slip"]."|".$rows["nilai_sim"].chr(3).$i.chr(3).$rows["unit"].chr(3).$rows["kso"].chr(3).$rows["nama"].chr(3).number_format($rows["nilai_sim"],0,",",".").chr(3).number_format($nilai,0,",",".").chr(3)."0".chr(6);
