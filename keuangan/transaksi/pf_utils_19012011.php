...
if($statusProses=='Error') {
    ...
    if($grid == 1) {
        $sql="select t1.*,if(kso_id=2,sub_tot-retur,nilai) as bayar from(
                select ap.id,no_penjualan,no_pasien,nama_pasien,dokter,sum(ap.qty_jual*ap.harga_satuan) as sub_tot,u.unit_name as unit
                ,au.unit_name,ap.unit_id,ap.kso_id,ifnull(sum(bo.nilai),0) as nilai
                ,sum(qty_retur*harga_satuan-((qty_retur*harga_satuan)*biaya_retur/100)) as retur
                 from $dbapotek.a_penjualan ap
                left join $dbapotek.a_unit u on u.unit_id = ap.ruangan
                left join k_bayar_obat bo on ap.id = bo.id_penjualan
                inner join $dbapotek.a_mitra am on ap.kso_id = am.idmitra
                left join $dbapotek.a_unit au on au.unit_id = ap.unit_id
                where $waktu and am.idmitra = '$kso' group by no_penjualan,ap.unit_id) t1
        ...
        $sql = "select * from(
                ...
                ,if(ap.kso_id=2,sum(ap.qty_jual*ap.harga_satuan),ifnull(bo.nilai,0)) bayar
                 from $dbapotek.a_penjualan ap
    ...
    if($grid == 1) {
        while ($rows=mysql_fetch_array($rs)) {
            $i++;
            $dt.=$rows["id"]."|".$rows["no_penjualan"]."|".$rows["unit_id"].chr(3).number_format($i,0,",","").chr(3).$rows["unit_name"].chr(3).$rows["no_penjualan"].chr(3).$rows["no_pasien"].chr(3).$rows["nama_pasien"].chr(3).$rows["unit"].chr(3).$rows["dokter"].chr(3).$rows["sub_tot"].chr(3).$rows["retur"].chr(3).$rows["bayar"].chr(6);
        }
    }
    else if($grid == 2) {
        while ($rows=mysql_fetch_array($rs)) {
            $i++;
            $dt .= $rows["id"].chr(3).number_format($i,0,",","").chr(3).$rows["obat_nama"].chr(3).$rows["qty_jual"].chr(3).$rows["harga_satuan"].chr(3).$rows["sub_tot"].chr(3).$rows["bayar"].chr(6);        }
    }
