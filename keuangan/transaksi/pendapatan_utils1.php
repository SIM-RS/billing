...
if($statusProses=='Error') {
    ...
    if($grid == 1 || $tipe=='lain2') {
        if($tipe=='lain2') {
		  ...
		  if($inap == 0){
			 //non-inap
			 $sql = "select * from (select k.id,u.nama as unit,date_format(k.tgl,'%d-%m-%Y') as tgl,ps.no_rm,ps.nama as pasien, ua.nama as unit_asal,t1.* from (SELECT gab.pelayanan_id,SUM(gab.biayaRS) AS biayaRS,SUM(gab.biaya_kso) AS biaya_kso,SUM(gab.biaya_pasien) AS biaya_pasien,
				    SUM(gab.bayar_kso) AS bayar_kso,SUM(gab.bayar_pasien) AS bayar_pasien FROM
				    (SELECT DISTINCT t.id,t.pelayanan_id,(qty*biaya) AS biayaRS,(qty*biaya_kso) AS biaya_kso,(qty*biaya_pasien) AS biaya_pasien,
				    bayar_kso,bayar-bayar_kso AS bayar_pasien
				    FROM $dbbilling.b_tindakan t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id
		  ...
		  $sql = "SELECT *
				    FROM (SELECT
						...
						FROM (SELECT
						  ...
						  INNER JOIN $dbbilling.b_ms_unit u
						    ON p.unit_id = u.id
						  INNER JOIN $dbbilling.b_ms_unit ua
						    ON p.unit_id_asal = ua.id
					 INNER JOIN $dbbilling.b_kunjungan k
					   ON p.kunjungan_id = k.id
					 INNER JOIN $dbbilling.b_ms_pasien ps
					   ON ps.id = k.pasien_id
						...
						FROM (SELECT
							 ...
							 WHERE p.kunjungan_id = $kunj_id and (mu.inap = 1
								    OR p.unit_id IN(SELECT
												...
												FROM $dbbilling.b_ms_unit u
												WHERE u.inap = 1))
								AND t.tgl = '$tgl' AND t.kso_id='$kso' UNION SELECT
                                                0 AS ms_tindakan_kelas_id,
                                                pelayanan_id,
                                                '$tgl' AS tgl,
                                                1 AS qty,
                                              ...
                                              WHERE p.kunjungan_id = $kunj_id AND p.kso_id='$kso'
                                                  AND date(tk.tgl_in) <= '$tgl'
                                                  AND (date(tk.tgl_out) >= '$tgl'
        ...
        if($tipe == 'lain2') {
            ...
            while ($rows=mysql_fetch_array($rs)) {
			 ...
			 if($tipe != 4){
                ...
                $dt.=$rows["id"].'|'.$rows['pelayanan_id'].chr(3).number_format($i,0,",","").chr(3).$rows["tgl"].chr(3).$rows["no_rm"].chr(3).$rows["pasien"].chr(3).$tmpLay.$unit_asal.chr(3).$rows["biayaRS"].chr(3).$rows["biaya_kso"].chr(3).$rows["biaya_pasien"].chr(3).($rows["biayaRS"]-($rows["biaya_kso"]+$rows["biaya_pasien"])).chr(3).$rows["bayar_kso"].chr(3).$rows["bayar_pasien"].chr(6);
                $tmpLay = '';
            }
        }
    }
