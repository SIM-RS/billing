...
switch($act){
    case 'add':
		...
		if (mysql_errno()==0){
			...
			for($i=1;$i<=count($temp)-1;$i++){
				$tmp = explode("*",$temp[$i]);
				$sql2 = "insert into ".$dbkeuangan.".k_transaksi_detail (transaksi_id,id_ma_sak,beban_jenis_id,jenis_layanan_id,unit_id,nilai,nilai_sim) values ('$transaksi_id','$tmp[2]','$tmp[3]','$tmp[0]','$tmp[1]','$tmp[4]','$tmp[4]')";
				//echo $sql2."<br>";
				mysql_query($sql2);
    ...
    case 'edit':
	   ...
	   $rs = mysql_query($sql);
		  ...
		  for($i=1;$i<=count($temp)-1;$i++){
				$tmp = explode("*",$temp[$i]);
				$sql2 = "insert into ".$dbkeuangan.".k_transaksi_detail (transaksi_id,beban_jenis_id,jenis_layanan_id,unit_id,nilai,nilai_sim) values ('$transaksi_id','$tmp[3]','$tmp[0]','$tmp[1]','$tmp[4]','$tmp[4]')";
				mysql_query($sql2);
				//$up = "update ".$dbkeuangan.".k_transaksi set id_ma_sak='$tmp[2]',unit_id='0' where id=$transaksi_id";
				//mysql_query($up);
		  }
    ...
    case 'pengeluaranlain2':
		...
		for ($j=0;$j<count($dataTrans);$j++){
			...
			if(mysql_num_rows($rs) > 0){
				...
				$sql = "update ".$dbkeuangan.".k_transaksi set no_post='$idpost', no_bukti='$noBukti',tgl='$TglBayar'".$ftglByrPajak.",verifikasi=1 where id = '$id'";
				//echo $sql.";<br>";
				$rs1 = mysql_query($sql);
				if (mysql_errno()==0){
...
WHERE t.id='$id' AND (td.pajak_id=0 OR td.pajak_id IS NULL)";
					...
					while ($rw2=mysql_fetch_array($rs2)){
						...
						$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,FK_TRANS,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) values('$idpost',$no_trans,$no_psg,$ma_d,'$tgl','$nokw','$uraian',$nilai,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,'$fk_beban_jenis',$cc_pbf_umumId,$cc_pbf_umumId,1)";
						//echo $sql.";<br>";
						$rs1 = mysql_query($sql);
					...
					//if ($jenis_suplier==0){
						$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,POSTING) values('$idpost',$no_trans,$no_psg,$ma_k,'$tgl','$nokw','$uraian',0,$nilai2,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1)";
						//echo $sql.";<br>";
						$rs1 = mysql_query($sql);
					//}
					...
					if ($arfdata[0]!=""){
						...
						for ($i=0;$i<count($arpajak);$i++){
							$dataPajak=explode("|",$arpajak[$i]);
							$nilai1=$dataPajak[1];
							$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING) values('$idpost',$no_trans,$no_psg,$dataPajak[0],'$tgl','$nokw','$uraian',0,$nilai1,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,1)";
							//echo $sql."<br>";
							$rs1 = mysql_query($sql);
							$sql="insert into ".$dbkeuangan.".k_transaksi_detail(transaksi_id,nilai,nilai_sim,pajak_id,jenis_pajak,ket) values('$id',$nilai1,$nilai1,$dataPajak[0],0,'$uraian')";
							//echo $sql."<br>";
						...
						$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING) values('$idpost',$no_trans,$no_psg,$ma_dPajak,'$tgl','$nokw','$uraian',$nilai2,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,0,1,1)";
						//echo $sql."<br>";
						$rs1 = mysql_query($sql);
						...
						if ($TglBayarPajak!=""){
							...
							for ($i=0;$i<count($arpajak);$i++){
								$dataPajak=explode("|",$arpajak[$i]);
								$nilai1=$dataPajak[1];
								$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING) values('$idpost',$no_trans,$no_psg,$dataPajak[0],'$TglBayarPajak','$nokw','$uraian',$nilai1,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,0,1,1)";
								//echo $sql."<br>";
								$rs1 = mysql_query($sql);
								$sql="insert into ".$dbkeuangan.".k_transaksi_detail(transaksi_id,nilai,nilai_sim,pajak_id,jenis_pajak,ket) values('$id',$nilai1,$nilai1,$dataPajak[0],1,'$uraian')";
								//echo $sql."<br>";
							...
							$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING) values('$idpost',$no_trans,$no_psg,$ma_kPajak,'$TglBayarPajak','$nokw','$uraian',0,$nilai2,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,1)";
							//echo $sql."<br>";
							$rs1 = mysql_query($sql);
						}
					}
