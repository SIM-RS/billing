...
switch($act){
    case 'add':
		...
		if (mysql_errno()==0){
			...
			for($i=1;$i<=count($temp)-1;$i++){
				$tmp = explode("*",$temp[$i]);
				$sql2 = "insert into ".$dbkeuangan.".k_transaksi_detail (transaksi_id,id_ma_sak,beban_jenis_id,jenis_layanan_id,unit_id,nilai,nilai_sim, flag) values ('$transaksi_id','$tmp[2]','$tmp[3]','$tmp[0]','$tmp[1]','$tmp[4]','$tmp[4]','$flag')";
				//echo $sql2."<br>";
				mysql_query($sql2);
				...
				if ($jenis_supplier=='1'){
					...
					if ($nterima==""){
						...
						/*$sql = "update ".$dbaset.".as_masuk m set bayar = 2, no_bukti_bayar='$nobukti'
						   WHERE m.po_id in (SELECT id FROM ".$dbaset.".as_po where no_spk='$nofaktur') and bayar = 0 and posted=1 AND tgl_terima BETWEEN '".tglSQL($tgl_filter1)."' AND '".tglSQL($tgl_filter2)."'";*/
						$sql = "update ".$dbaset.".as_masuk m set bayar = 2, bayar_id='$transaksi_id', flag = '$flag'
						   WHERE m.po_id in (SELECT id FROM ".$dbaset.".as_po where no_spk='$nofaktur') and bayar = 0 and posted=1 AND tgl_terima BETWEEN '".tglSQL($tgl_filter1)."' AND '".tglSQL($tgl_filter2)."'";
						//echo $sql."<br>";
						$rs = mysql_query($sql);
					}else{
    ...
    case 'edit':
	   ...
	   $sql = "update ".$dbkeuangan.".k_transaksi set id_trans = '$id_trans', tgl = '$tgl', no_bukti = '$nobukti', no_faktur = '$nofaktur'
		  ...
		  , ket = '$ket', id_ma_sak = '$id_ma_sak', id_ma_dpa = '$idma', flag = '$flag'
		  where id = '$id'";
	   $rs = mysql_query($sql);
		  ...
		  for($i=1;$i<=count($temp)-1;$i++){
				$tmp = explode("*",$temp[$i]);
				$sql2 = "insert into ".$dbkeuangan.".k_transaksi_detail (transaksi_id,beban_jenis_id,jenis_layanan_id,unit_id,nilai,nilai_sim,flag) values ('$transaksi_id','$tmp[3]','$tmp[0]','$tmp[1]','$tmp[4]','$tmp[4]','$flag')";
				mysql_query($sql2);
				//$up = "update ".$dbkeuangan.".k_transaksi set id_ma_sak='$tmp[2]',unit_id='0' where id=$transaksi_id";
				//mysql_query($up);
		  }
	   ...
	   if(mysql_num_rows($rs) > 0){
		  ...
		  if($jenis_supplier=='1'){
		  	...
		  	if ($nterima==""){
				...
				$nterima=explode("|",$nterima);
				for ($i=0;$i<count($nterima);$i++){
					$sql = "update ".$dbaset.".as_masuk m set bayar = 0, flag = '$flag'
		...
		for ($j=0;$j<count($dataTrans);$j++){
			...
			if ($arfdata[0]!="" && mysql_num_rows($qCek)<=0){
						...
						for ($i=0;$i<count($arpajak);$i++){
							...
							$nilai1=$dataPajak[1];
							$sql="insert into ".$dbakuntansi.".JURNAL(no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values($no_trans,$no_psg,$dataPajak[0],'$tgl','$nokw','$uraian',0,$nilai1,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,1,'$flag')";
							//echo $sql."<br>";
							$rs1 = mysql_query($sql);
							$sql="insert into ".$dbkeuangan.".k_transaksi_detail(transaksi_id,nilai,nilai_sim,pajak_id,jenis_pajak,ket,flag) values('$id',$nilai1,$nilai1,$dataPajak[0],0,'$uraian','$flag')";
							//echo $sql."<br>";
							$rs1 = mysql_query($sql);
						...
						$sql="insert into ".$dbakuntansi.".JURNAL(no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values($no_trans,$no_psg,$ma_dPajak,'$tgl','$nokw','$uraian',$nilai2,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,0,1,1,'$flag')";
						//echo $sql."<br>";
						$rs1 = mysql_query($sql);
			}
			...
			$arpajak=explode(chr(6),$arfdata[0]);
						...
						if ($TglBayarPajak!=""){
							...
							for ($i=0;$i<count($arpajak);$i++){
								$dataPajak=explode("|",$arpajak[$i]);
								$nilai1=$dataPajak[1];
								$sql="insert into ".$dbakuntansi.".JURNAL(no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values($no_trans,$no_psg,$dataPajak[0],'$TglBayarPajak','$nokw','$uraian',$nilai1,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,0,1,1,'$flag')";
								//echo $sql."<br>";
								$rs1 = mysql_query($sql);
								$sql="insert into ".$dbkeuangan.".k_transaksi_detail(transaksi_id,nilai,nilai_sim,pajak_id,jenis_pajak,ket,flag) values('$id',$nilai1,$nilai1,$dataPajak[0],1,'$uraian','$flag')";
								//echo $sql."<br>";
								$rs1 = mysql_query($sql);
								$nilai2 +=$nilai1;
							}
							
							$sql="insert into ".$dbakuntansi.".JURNAL(no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values($no_trans,$no_psg,$ma_kPajak,'$TglBayarPajak','$nokw','$uraian',0,$nilai2,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,1,'$flag')";
							//echo $sql."<br>";
							$rs1 = mysql_query($sql);
						}
		}
    ...
    case 'pengeluaranlain2':
		//$iduser=24;//user dr posting keuangan
		$iduser=0;//user dr posting keuangan
		$noBukti = $_REQUEST['noBukti'];
		$TglBayar = tglSQL($_REQUEST['TglBayar']);
		$TglBayarPajak = $_REQUEST['TglBayarPajak'];
		$ftglByrPajak="";
		//if ($TglBayarPajak!="") {
			...
			$ftglByrPajak=",tgl_klaim='$TglBayar'";
		//}
		$tcaraBayar = $_REQUEST['tcaraBayar'];
		...
		for ($j=0;$j<count($dataTrans);$j++){
			$id = $dataTrans[$j];
			if ($stBayar == 0){
				$sql = "select * from ".$dbkeuangan.".k_transaksi where id = '$id' and verifikasi = 0";
				//echo $sql.";<br>";
				$rs = mysql_query($sql);
				if(mysql_num_rows($rs) > 0){
					$row = mysql_fetch_array($rs);
					$id_trans = $row['id_trans'];
					$sql = "update ".$dbkeuangan.".k_transaksi set no_post='$idpost', no_bukti='$noBukti',tgl='$TglBayar'".$ftglByrPajak.",verifikasi=1, flag = '$flag' where id = '$id'";
					//echo $sql.";<br>";
					$rs1 = mysql_query($sql);
					if (mysql_errno()==0){
	
						$sql="SELECT t.*,td.id_ma_sak AS fk_ma_sak,td.beban_jenis_id AS fk_beban_jenis,IFNULL(td.unit_id,0) td_unit_id,IFNULL(td.nilai_sim,0) td_nilai_sim,IFNULL(td.nilai,0) td_nilai 
	...
	WHERE t.id='$id' AND (td.pajak_id=0 OR td.pajak_id IS NULL)";
						...
						while ($rw2=mysql_fetch_array($rs2)){
							...
							$fk_beban_jenis=$rw2["fk_beban_jenis"];
							//$ma_k=7;
							/*if ($tcaraBayar==2){
								$ma_k=12;
							}elseif ($tcaraBayar==3){
							...
							/*$sql="SELECT
									...
									WHERE dt.fk_jenis_trans = '$jtrans' AND LEFT(ma.MA_KODE,1)<>'1' 
							...
							if (mysql_num_rows($rs1) > 0){
								...
								$ma_d=$rw1["fk_ma_sak"];
								$fk_beban_jenis=$rw1["fk_ms_beban_jenis"];
							}*/
							
							$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,FK_TRANS,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,PBF_ID,POSTING,FLAG) values('$idpost',$no_trans,$no_psg,$ma_d,'$tgl','$nokw','$uraian',$nilai,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,'$fk_beban_jenis',$cc_pbf_umumId,$cc_pbf_umumId,'$cc_pbf_umumId',1,'$flag')";
							//echo $sql.";<br>";
							$rs1 = mysql_query($sql);
							
							/*if ($rw2["jenis_supplier"]>0){
								$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,POSTING) values('$idpost',$no_trans,$no_psg,$ma_k,'$tgl','$nokw','$uraian',0,$nilai2,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,'$cc_pbf_umumId',1)";
								echo $sql.";<br>";
								$rs1 = mysql_query($sql);
							}*/
							...
							/*if ($selisih<>0){
								...
								$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,SELISIH,POSTING) values('$idpost',$no_transSel,$no_psgSel,$ma_d,'$tgl','$nokw','$uraian_selisih',$selisih,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,$cc_pbf_umumId_SelD,1,1)";
								//echo $sql."<br>";
								//$rs1 = mysql_query($sql);
								
								$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,SELISIH,POSTING) values('$idpost',$no_transSel,$no_psgSel,$ma_k_sel,'$tgl','$nokw','$uraian_selisih',0,$selisih,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,$cc_pbf_umumId_SelK,1,1)";
								//echo $sql."<br>";
								//$rs1 = mysql_query($sql);
							}*/
						}
						...
						//if ($jenis_suplier==0){
							$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,POSTING,flag) values('$idpost',$no_trans,$no_psg,$ma_k,'$tgl','$nokw','$uraian',0,$nilai2,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,'$flag')";
							//echo $sql.";<br>";
							$rs1 = mysql_query($sql);
						//}
						...
						if ($arfdata[0]!=""){
							...
							for ($i=0;$i<count($arpajak);$i++){
								$dataPajak=explode("|",$arpajak[$i]);
								$nilai1=$dataPajak[1];
								$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values('$idpost',$no_trans,$no_psg,$dataPajak[0],'$tgl','$nokw','$uraian',0,$nilai1,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,1,'$flag')";
								//echo $sql."<br>";
								$rs1 = mysql_query($sql);
								$sql="insert into ".$dbkeuangan.".k_transaksi_detail(transaksi_id,nilai,nilai_sim,pajak_id,jenis_pajak,ket,flag) values('$id',$nilai1,$nilai1,$dataPajak[0],0,'$uraian','$flag')";
								//echo $sql."<br>";
							...
							$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values('$idpost',$no_trans,$no_psg,$ma_dPajak,'$tgl','$nokw','$uraian',$nilai2,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,0,1,1,'$flag')";
							//echo $sql."<br>";
							$rs1 = mysql_query($sql);
							...
							if ($TglBayarPajak!=""){
								...
								for ($i=0;$i<count($arpajak);$i++){
									$dataPajak=explode("|",$arpajak[$i]);
									$nilai1=$dataPajak[1];
									$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,FLAG) values('$idpost',$no_trans,$no_psg,$dataPajak[0],'$TglBayarPajak','$nokw','$uraian',$nilai1,0,CURDATE(),$iduser,'D',0,$jtrans,$jtrans,0,1,1,'$flag')";
									//echo $sql."<br>";
									$rs1 = mysql_query($sql);
									$sql="insert into ".$dbkeuangan.".k_transaksi_detail(transaksi_id,nilai,nilai_sim,pajak_id,jenis_pajak,ket,flag) values('$id',$nilai1,$nilai1,$dataPajak[0],1,'$uraian','$flag')";
									//echo $sql."<br>";
								...
								$sql="insert into ".$dbakuntansi.".JURNAL(FK_ID_POSTING,no_trans,NO_PASANGAN,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID,PAJAK,POSTING,flag) values('$idpost',$no_trans,$no_psg,$ma_kPajak,'$TglBayarPajak','$nokw','$uraian',0,$nilai2,CURDATE(),$iduser,'K',0,$jtrans,$jtrans,0,1,1,'$flag')";
								//echo $sql."<br>";
								$rs1 = mysql_query($sql);
							}
						}
				...
				if(mysql_num_rows($rs) > 0){
					...
					$no_post = $row['no_post'];
					$tgl = $row['tgl'];
					$sql = "DELETE FROM ".$dbakuntansi.".jurnal where FK_ID_POSTING = '$no_post' AND TGL='$tgl'";
					//echo $sql.";<br>";
					$rs1 = mysql_query($sql);
					if (mysql_errno()==0){
						$sql = "update ".$dbkeuangan.".k_transaksi set no_post='', no_bukti='',verifikasi=0, flag = '$flag' where id = '$id'";
						//echo $sql.";<br>";
						$rs1 = mysql_query($sql);
					}
				}
...
if ($stBayar<>2){
	...
	$strBayar=" AND t.verifikasi='$stBayar'";
}
...
if($pilihan == "pengeluaran") {
	...
	$sql = "SELECT 
			...
			FROM
			  ...
			  WHERE MONTH(tgl) = '$bln' 
				AND YEAR(tgl) = '$thn' AND t.flag = '$flag' AND t.tipe_trans = 2 
				".$strBayar."
			 order by $sorting) a1 $filter";
}
...
if($pilihan == "pengeluaran"){
	while ($rows=mysql_fetch_array($rs)) {
		...
		$sisip = $rows['id'].'|'.$rows['id_trans'].'|'.$rows['jenis_layanan'].'|'.$rows['unit_id'].'|'.''.'|'.$rows['no_faktur'].'|'.$rows['supplier_id'].'|'.$rows['nilai'].'|'.$rows['jenis_supplier'].'|'.$rows['verifikasiId'].'|'.$rows['nilai_sim'].'|'.$rows['ma_id'].'|'.$temp.'|'.$rows['kode_trans'].'|'.$rows['id_ma_sak'].'|'.$rows['posting'].'|'.$cc.'|'.$status_pajak.'|'.$rows['no_post'];
		$i++;
		$cket="";
		if ($rows['unit_id']!=0) $cket=" - ".$rows['nama_tl'];
		$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows['tgl'].chr(3).$rows["no_bukti"].chr(3).$rows["nama_trans"].$cket.chr(3).number_format($rows['nilai'],0,",",".").chr(3).$rows['ket1'].chr(3)."0".chr(6);
