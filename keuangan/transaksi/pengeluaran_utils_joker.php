...
switch($act){
    case 'add':
	   ...
	   /*}else{
		   $sql = "update ".$dbkeuangan.".k_transaksi set id_trans = '$id_trans', tgl = '$tgl', no_bukti = '$nobukti', no_faktur = '$nofaktur'
			  ...
			  , ket = '$ket', tgl_act = $tgl_act, id_ma_sak = '$id_ma_sak', id_ma_dpa = '$idma'
			  where id = '$id'";
		   //echo $sql."<br>";
		   $rs = mysql_query($sql);
    ...
    case 'edit':
	   ...
	   $rs = mysql_query($sql);
        ...
        break;
    case 'hapus':
	   $sql = "select no_faktur, jenis_supplier from k_transaksi where id = '$id' and jenis_supplier <> 0";
	   //echo $sql."<br>";
	   $rs = mysql_query($sql);
	   if(mysql_num_rows($rs) > 0){
		  $row = mysql_fetch_array($rs);
...
if($pilihan == "pengeluaran"){
	while ($rows=mysql_fetch_array($rs)) {
		...
		$sisip = $rows['id'].'|'.$rows['id_trans'].'|'.$rows['jenis_layanan'].'|'.$rows['unit_id'].'|'.$rows['ket1'].'|'.$rows['no_faktur'].'|'.$rows['supplier_id'].'|'.$rows['nilai'].'|'.$rows['jenis_supplier'].'|'.$rows['verifikasiId'].'|'.$rows['nilai_sim'].'|'.$rows['ma_id'].'|'.$temp.'|'.$rows['kode_trans'].'|'.$rows['id_ma_sak'].'|'.$rows['posting'].'|'.$cc.'|'.$status_pajak;
		$i++;
		$cket="";
		if ($rows['unit_id']!=0) $cket=" - ".$rows['nama_tl'];
		$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows['tgl'].chr(3).$rows["no_bukti"].chr(3).$rows["nama_trans"].$cket.chr(3).$rows["ma_nama"].$rows["nama_jenis_supplier"].chr(3).number_format($rows['nilai'],0,",",".").chr(3).$rows['ket'].chr(3).$rows['verifikasi'].chr(3)."0".chr(6);
