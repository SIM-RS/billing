...
switch (strtolower($_REQUEST['act']))
{
      case 'cari':
         ...
         <table id="pasien_table">	   
			...
			<th align="right">
		  <img alt="close" src="http://<?php echo $_SERVER['HTTP_HOST']?><?php echo $base_addr_billing; ?>/icon/x.png" style="cursor: pointer;" width="24" onclick="tutupDivPasien()"/>
	       </th>
           </tr>
            <?php           
			$sql="SELECT DISTINCT p.id,p.nama,p.alamat,p.rt,p.rw,w.nama AS desa,i.nama AS kec, l.nama AS kab,p.tgl_lahir,p.sex,
			k.tgl,k.umur_thn,n.nama as unit,k.id as kunjungan_id,k.verifikasi,k.note_verifikasi FROM $dbbilling.b_ms_pasien p
        INNER JOIN $dbbilling.b_kunjungan k ON k.pasien_id=p.id	
        LEFT JOIN $dbbilling.b_ms_wilayah w ON w.id=p.desa_id
        LEFT JOIN $dbbilling.b_ms_wilayah i ON i.id=p.kec_id
        LEFT JOIN $dbbilling.b_ms_wilayah l ON l.id=p.kab_id
        LEFT JOIN $dbbilling.b_ms_unit n ON n.id=k.unit_id	
        WHERE no_rm = '$norm' AND k.tgl between '".tglSQL($tglAwal)."' AND '".tglSQL($tglAkhir)."'";
		//echo $sql;
			$rs=mysql_query($sql);
            $i=1;
            while($rw=mysql_fetch_array($rs)){
				
		  $qPel="select id from $dbbilling.b_pelayanan where kunjungan_id='".$rw['kunjungan_id']."' limit 1";
