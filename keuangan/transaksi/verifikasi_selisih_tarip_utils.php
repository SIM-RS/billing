		...
		if ($posting==0)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				/* $sql="SELECT
						...
						FROM k_piutang kp
						WHERE kp.fk_id = '$idKunj' AND kp.kso_id='$ksoid'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				...
				$tglP=$rw["tglP"];
				$biayaRS=$rw["biayaRS"];
				$biayaPasien=$rw["biayaPasien"];
				$biayaKSO=$rw["biayaKSO"];
				$bayarPasien=$rw["bayarPasien"];
				$piutangPasien=$rw["piutangPasien"]; */
