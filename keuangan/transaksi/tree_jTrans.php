		 ...
		 if ($c_level>0){
			if ($c_islast==1){
				...
				if((mysql_num_rows($kueri1)==1 && mysql_num_rows($kueri2)>1)){
					...
					$kodedpa='';
					$ma_dpa='';
					$tree[$cnt][2]= "javascript:set_value('0','$rows[JTRANS_KODE]','$rows[JTRANS_NAMA]','0','$rows[JTRANS_ID]','$data1[MA_ID]','$rows[JTRANS_NAMA]','$iddpa','$ma_dpa');window.close();";
				}
			}
 		 }else{
		 	 $tree[$cnt][2] = null;
			 ...
			 if((mysql_num_rows($kueri1)==1 && mysql_num_rows($kueri2)>1)){
				 ...
				 while($data2 = mysql_fetch_array($kueri2)){
					 ...
					 if($data2['CC_RV_KSO_PBF_UMUM']!=1 && $c_islast==1){
						...
						$kodedpa='';
						$ma_dpa='';
						$tree[$cnt][2]= "javascript:set_value('0','$data2[MA_KODE]','$data2[MA_NAMA]','1','$rows[JTRANS_ID]','$data2[MA_ID]','$rows[JTRANS_NAMA]','$iddpa','$ma_dpa');window.close();";
					 }else
						$tree[$cnt][2] = null;
					 $tree[$cnt][3]= "";
					 $tree[$cnt][4]= 0;
						...
						if (substr($data2["MA_KODE"],0,1)=="4"){
						 ...
						 while ($rows1=mysql_fetch_array($rs1)){
							 ...
							 if ($c_islast==1){
								...
								$kodedpa='';
								$ma_dpa='';
								$tree[$cnt][2]= "javascript:set_value('$rows1[id]','$rows1[kode]','$rows1[nama]','1','$rows[JTRANS_ID]','$data2[MA_ID]','$rows[JTRANS_NAMA]','$iddpa','$ma_dpa');window.close();";
							 }else
								 $tree[$cnt][2] = null;
...
function set_value(a,b,c,satu,id_trans,id_ma_sak,parent,h,i){
	//alert('xx');
	...
	window.opener.BacaMA_DPA(id_trans,id_ma_sak);
