...
<form id="form1">
    ...
    <table width="500" border="0" cellpadding="0" cellspacing="0" align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;" id="tbl">
        <tr>
            <td colspan="4">
                <fieldset style="width: 280px;background-color: silver">
                    <!--legend>
                    ...
                    <input id="txtTglAkhir" name="txtTglAkhir" readonly size="11" class="txtcenter" type="text" value="<?php echo $txtTglAkhir; ?>" />&nbsp;
                                    <input type="button" name="btnTglAkhir" value="&nbsp;V&nbsp;" class="tblBtn" onclick="gfPop.fPopCalendar(document.getElementById('txtTglAkhir'),depRange,filterSPK);"/>
                </fieldset>            </td>
        </tr>
        <tr class="headtable" style="text-align:center; font-weight:bold;" height="30">
            <td class="tblheaderkiri" width="40">No</td>
            <td class="tblheader" width="90">Tgl PO</td>
            <!--td class="tblheader" width="200">No PO</td-->            
            <td class="tblheader">No SPK</td>
            <td class="tblheader" width="150">Tagihan</td>
        </tr>
        <?php
		if($jenis_supplier=='1'){
			$sql="SELECT t1.PBF_ID,t1.TANGGAL,t1.tgl, t1.noterima, 
t1.nobukti, t1.NO_PO no_po, t1.BATCH no_spk, t1.pbf_nama, SUM(t1.dpp) AS dpp, 
SUM(t1.ppn) AS ppn, SUM(t1.dpp_ppn) AS tagihan 
...
WHERE a.PBF_ID='$supplier' AND a.TIPE_TRANS=0 AND a.BATCH!='' AND a.BATCH!='-' AND a.BATCH IS NOT NULL AND a.BAYAR=0 AND a.TANGGAL BETWEEN '".tglSQL($txtTglAwal)."' AND '".tglSQL($txtTglAkhir)."') a_p 
INNER JOIN ".$dbapotek.".a_obat o ON a_p.OBAT_ID=o.OBAT_ID INNER JOIN ".$dbapotek.".a_kepemilikan k ON a_p.KEPEMILIKAN_ID=k.ID 
INNER JOIN ".$dbapotek.".a_pbf ON a_p.PBF_ID=a_pbf.PBF_ID) AS t1 GROUP BY t1.BATCH";
		}else{
		   /* $sql="SELECT m.po_id,p.no_po,p.no_po no_spk,DATE_FORMAT(p.tgl_po,'%d-%m-%Y') as tgl,sum(m.jml_msk*harga_unit) as tagihan
			FROM ".$dbaset.".as_masuk m
			INNER JOIN ".$dbaset.".as_po p ON p.id=m.po_id			
			WHERE p.vendor_id='$supplier'
			AND p.tgl_po BETWEEN '".tglSQL($txtTglAwal)."' AND '".tglSQL($txtTglAkhir)."' and bayar = 0
			group by p.no_po
			order by p.tgl_po";*/
		  ...
		  $sql="SELECT m.po_id,p.no_po,p.no_po no_spk,DATE_FORMAT(p.tgl_po,'%d-%m-%Y') AS tgl,SUM(m.jml_msk*harga_unit) AS tagihan
			FROM $dbaset.as_masuk m
			INNER JOIN $dbaset.as_po p ON p.id=m.po_id			
			WHERE p.tgl_po BETWEEN '2012-01-01' AND '2012-09-30' AND bayar = 0
			GROUP BY p.no_po
			ORDER BY p.tgl_po";
		}
		//echo $sql."<br>";		
		...
		//echo mysql_error();
                ...
                while($rw = mysql_fetch_array($rs)) {
                    $arfvalue = $rw['no_po'].'|'.number_format($rw['tagihan'],0,",",".").'|'.$bln.'|'.$thn.'|'.$rw['no_spk'];
                    ?>
        <tr id="lstDiag<?php echo $no; ?>" lang="<?php echo $arfvalue; ?>" height="20" class="itemtableReq" onMouseOver="this.className='itemtableMOverReq'" onMouseOut="this.className='itemtableReq'" onClick="fsetFaktur(this.lang);">
            <td class="tdisikiri" style="text-align:center"><?php echo $no;?></td>
            <td class="tdisi" style="text-align:center"><?php echo $rw['tgl']?></td>
            <!--td class="tdisi" style="text-align:center">&nbsp;<?php echo $rw['no_po']?></td-->            
            <td class="tdisi" style="text-align:center">&nbsp;<?php echo $rw['no_spk']?></td>
            <td class="tdisi" style="text-align:right"><?php echo number_format($rw['tagihan'],2,",",".")?>&nbsp;</td>
        </tr>
            <?php
            $no++;
                }
