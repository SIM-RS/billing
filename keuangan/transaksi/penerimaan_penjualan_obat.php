    ...
    <body bgcolor="#808080" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"><!-- onload="viewTime()"-->
        ...
        <div align="center">
            <table width="1000" border="0" cellpadding="0" cellspacing="0" bgcolor="#EAEFF3">
                ...
                <tr>
                    <td width="40%" style="padding-right:20px; text-align:right;" height="25">Nama KSO</td>
                ...
                <tr>
                    <td style="padding-right:20px; text-align:right;" height="25">Tanggal Slip</td>
                ...
                <tr>
                    <td height="30" style="padding-left:40px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="if (this.value=='1'){document.getElementById('spnVerif').innerHTML='UnVerifikasi';}else{document.getElementById('spnVerif').innerHTML='Verifikasi';};kirim();">
                        <option value="0">BELUM VERIFIKASI</option>
                        <option value="1">SUDAH VERIFIKASI</option>
                    </select>                    </td>
                    <td height="30" align="right" style="padding-right:40px;"><BUTTON type="button" id="btnVerifikasi" onClick="VerifikasiJurnal();"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;<span id="spnVerif">Verifikasi</span></BUTTON></td>
                </tr>
                <tr>
...
<script>
	...
	a1.setColHeader("NO,NO KW,NO RESEP,NO RM,NAMA PASIEN,CARA BAYAR,PENJAMIN,RUANGAN,SHIFT,NILAI,NILAI HPP,VERIFIKASI<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	...
	a1.setIDColHeader(",ap.NO_PENJUALAN,ap.NO_RESEP,ap.NO_PASIEN,ap.NAMA_PASIEN,ac.nama,am.NAMA,aunit.UNIT_NAME,ap.SHIFT,,,");
	a1.setColWidth("30,50,40,70,170,60,90,160,40,80,80,40");
	a1.setCellAlign("center,center,center,center,left,center,center,center,center,right,right,center");
	a1.setSubTotalAlign("center,center,center,center,center,center,right,right,right,right,right,left");
	a1.setCellHeight(20);
	a1.setImgPath("../icon");
	a1.setIDPaging("paging");
	a1.onLoaded(konfirmasi);
	//alert("../transaksi/penerimaan_penjualan_obat_utils.php?grd=penerimaanPenjualanObat&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value);
	a1.baseURL("../transaksi/penerimaan_penjualan_obat_utils.php?grd=penerimaanPenjualanObat&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value+"&shift="+document.getElementById('cmbShift').value);
	a1.Init();
	...
	var tmp;
		...
		if (val!=undefined){
			...
			a1.cellSubTotalSetValue(11,tmp[3]);
			if(key=='Error'){
				if(tmp[0]=='verifikasiPenerimaanPenjualanObat'){
					alert('Terjadi Error dlm Proses Verifikasi !');
				}
			}else{
				if(tmp[0]=='verifikasiPenerimaanPenjualanObat' && document.getElementById('cmbPost').value=='0'){
					alert('Proses Verifikasi Berhasil !');
				}
				else if(tmp[0]=='verifikasiPenerimaanPenjualanObat' && document.getElementById('cmbPost').value=='1' && tmp[4]=='sukses'){
					alert('Proses unVerifikasi Berhasil !');
				}
				else if(tmp[0]=='verifikasiPenerimaanPenjualanObat' && document.getElementById('cmbPost').value=='1' && tmp[4]=='gagal'){
	...
	var url2;
		...
		if (tmp!="")
		{
			...
			{
				url="../transaksi/penerimaan_penjualan_obat_utils.php?grd=penerimaanPenjualanObat&act=Verif_Penerimaan_Kasir_Farmasi&idUser=24&tanggal="+document.getElementById('tgl').value+"&kso="+document.getElementById('cmbKSO').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value+"&posting="+document.getElementById('cmbPost').value+"&shift="+document.getElementById('cmbShift').value+"&tanggalSlip="+document.getElementById('tglSlip').value+"&no_slip="+no_slip+"&userId=<?php echo $userId; ?>"+"&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}
