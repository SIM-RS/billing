...
if($statusProses=='Error') {
    ...
    if($grid == 1 || $tipe=='lain2') {
        if($tipe=='lain2') {
            ...
            //rawat jalan
            $sql = "select * from (select k.id,u.nama as unit,date_format(k.tgl,'%d-%m-%Y') as tgl,ps.nama as pasien,t1.* from (SELECT gab.pelayanan_id,SUM(gab.biayaRS) AS biayaRS,SUM(gab.biaya_kso) AS biaya_kso,SUM(gab.biaya_pasien) AS biaya_pasien,
		  ...
		  ,t1.pelayanan_id,sum(t1.biayaRS) as biayaRS,sum(t1.biaya_kso) as biaya_kso,sum(t1.biaya_pasien) as biaya_pasien,
		  sum(t1.bayar_kso) as bayar_kso,sum(t1.bayar_pasien) as bayar_pasien
			 from (SELECT gab.pelayanan_id,SUM(gab.biayaRS) AS biayaRS,SUM(gab.biaya_kso) AS biaya_kso,SUM(gab.biaya_pasien) AS biaya_pasien,
                    SUM(gab.bayar_kso) AS bayar_kso,SUM(gab.bayar_pasien) AS bayar_pasien FROM
                    (SELECT DISTINCT t.id,t.pelayanan_id,(qty*biaya) AS biayaRS,(qty*biaya_kso) AS biaya_kso,(qty*biaya_pasien) AS biaya_pasien,
                    bayar_kso,bayar-bayar_kso AS bayar_pasien
                    FROM $dbbilling.b_tindakan t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id
                    INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id_asal=mu.id
                    WHERE (mu.inap=1 OR p.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=1)) AND t.kso_id='$kso' AND t.tgl='$tgl'
                    UNION
                    SELECT tk.id,tk.pelayanan_id,tarip AS biayaRS,tk.beban_kso AS biaya_kso,tk.beban_pasien AS biaya_pasien,
                    tk.bayar_kso,IF((DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in) * tk.beban_pasien > tk.bayar_pasien),
                    IF (DATEDIFF('$tgl',tk.tgl_in) * tk.beban_pasien <= tk.bayar_pasien,tk.beban_pasien,0),tk.beban_pasien) AS bayar_pasien
                    FROM $dbbilling.b_tindakan_kamar tk INNER JOIN $dbbilling.b_pelayanan p ON tk.pelayanan_id=p.id
                    WHERE p.kso_id='$kso' AND tk.tgl_in<='$tgl' AND (tk.tgl_out>='$tgl' OR tk.tgl_out IS NULL)) AS gab GROUP BY gab.pelayanan_id
                    ) t1
                    inner join $dbbilling.b_pelayanan p on p.id = t1.pelayanan_id
                    inner join $dbbilling.b_kunjungan k on p.kunjungan_id = k.id
                    inner join $dbbilling.b_ms_pasien ps on ps.id = k.pasien_id
                    inner join $dbbilling.b_ms_unit u on u.id = p.unit_id) t2 $filter
				group by t1.pelayanan_id
				order by $sorting";*/
            ...
            /*$sql = "SELECT
                        ...
                        tgl,
                        nama,
                        SUM(biaya+qty_*tarip_k) AS biayaRS,
                        SUM(biaya_kso+qty_*beban_kso_k) AS biaya_kso,
                        SUM(biaya_pasien+qty_*beban_pasien_k) AS biaya_pasien,
                   ...
                   FROM (SELECT
                                 ...
                                 IF(tk.tgl_in IS NULL,0,if(tk.tgl_out is null,1,$tgl_k)) AS qty_
                            FROM $dbbilling.b_kunjungan k
                                 INNER JOIN $dbbilling.b_pelayanan p
                                   ON k.id = p.kunjungan_id
                                 INNER JOIN $dbbilling.b_tindakan t
                            ...
                            WHERE p.id >= (SELECT
                                                    ...
                                                    LIMIT 1)
                                   AND k.kso_id = '$kso'
                    $waktu) t1 $filter GROUP BY id order by $sorting";
            /*$sql = "SELECT
                        id,
                        kso_id,
                        pasien,
                        tgl,
                        nama,
                        SUM(qty*biaya+qty_hari*tarip_k) AS biayaRS,
                   ...
                   FROM (SELECT
                                 ...
                                 IFNULL(tk.bayar_kso,0) AS bayar_kso_k,
                                 IFNULL(tk.bayar_pasien,0) AS bayar_pasien_k,
                                 IF(tk.tgl_in IS NULL,0,DATEDIFF(IFNULL(tk.tgl_out,$tgl_k),tk.tgl_in)) AS qty_hari
                            FROM $dbbilling.b_kunjungan k
                                 INNER JOIN $dbbilling.b_pelayanan p
                                   ON k.id = p.kunjungan_id
                                 INNER JOIN $dbbilling.b_tindakan t
    ...
    if($grid == 1 || $tipe == 'lain2') {
        if($tipe == 'lain2') {
            ...
            while ($rows=mysql_fetch_array($rs)) {
                $i++;
                $tmpLay = $rows['unit'];
                $dt.=$rows["id"].'|'.$rows['pelayanan_id'].chr(3).number_format($i,0,",","").chr(3).$rows["tgl"].chr(3).$rows["pasien"].chr(3).$tmpLay.chr(3).$rows["biayaRS"].chr(3).$rows["biaya_kso"].chr(3).$rows["biaya_pasien"].chr(3).$rows["bayar_kso"].chr(3).$rows["bayar_pasien"].chr(6);
                $tmpLay = '';
            }
        }
    }
        ...
        while ($rows=mysql_fetch_array($rs)) {
            $i++;
            $dt .= $rows["id"].chr(3).number_format($i,0,",","").chr(3).$rows["tgl"].chr(3).$rows["unit"].chr(3).$rows["tindakan"].chr(3).$rows["dokter"].chr(3).$rows["biaya"].chr(3).$rows["biaya_kso"].chr(3).$rows["biaya_pasien"].chr(3).$rows["bayar_kso"].chr(3).$rows["bayar_pasien"].chr(6);
        }
    }
