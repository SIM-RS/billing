...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body bgcolor="#808080" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"><!-- onload="viewTime()"-->
        ...
        <div align="center">
        	...
        	<div id="divPopPx" class="popup" style="width:1010px;height:600px;display:none;">
            <fieldset>
                    ...
                    <tr id="trPen">
                        <td align="center" >
                            <table width="1000px" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td style="padding-left:10px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="if(this.value==0){document.getElementById('spnBtnVer').innerHTML='&nbsp;Masukkan --> Proses Pengajuan';}else{document.getElementById('spnBtnVer').innerHTML='&nbsp;Proses Pengajuan --> Keluarkan';}kirim();">
                                        <option value="0">BELUM PENGAJUAN</option>
                                        <option value="1">PROSES PENGAJUAN</option>
                                    </select></td>
                                    <td align="right" style="padding-right:10px;"><BUTTON type="button" id="btnVerifikasi" onClick="VerifikasiJurnal();"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle"><span id="spnBtnVer">&nbsp;Masukkan --> Proses Pengajuan</span></BUTTON></td>
                                </tr>
                            </table>
    ...
    <script type="text/JavaScript" language="JavaScript">
		...
		function fSimpanKlaim(){
			...
			url="pengajuan_Klaim_t_utils.php?grid=klaim&thn="+thn+"&act=buat_klaim&tglKlaim="+tglKlaim+"&kso="+kso+"&no_bukti="+noBukti+"&userId=<?php echo $userId; ?>";
			//alert(url);
			...
			if (sisip[2]=="0"){
				if (sisip[3]=="0"){
					if (confirm('Yakin Ingin Menghapus Data ?')){
						url="pengajuan_Klaim_t_utils.php?grid=klaim&thn="+thn+"&act=hapus_klaim&klaim_id="+sisip[0]+"&userId=<?php echo $userId; ?>";
						//alert(url);
						grdKlaim.loadURL(url,"","GET");
					}
				}else{
		...
		function fVerifKlaim(a,b){
			var url;
			url="pengajuan_Klaim_t_utils.php?grid=klaim&thn="+thn+"&act=verif_klaim&klaim_id="+a+"&tipeVerif="+b+"&userId=<?php echo $userId; ?>";
			if (b=="0"){
		...
		function fShowMainKlaim(){
			...
			grdKlaim.loadURL("pengajuan_Klaim_t_utils.php?grid=klaim&thn="+thn,"","GET");
			//fBatalKlaim();
		}
        ...
        function goFilterAndSort(grd){
            ...
            var url = "pengajuan_Klaim_t_utils.php?grid=1&tipe=<?php echo $tipe;?>&tipePend=0&kso="+kso+"&filter="+a.getFilter()+"&sorting="+a.getSorting()+"&page="+a.getPage();
            var tglA,tglZ;
            switch(timeFil){
                case 'harian':
        ...
        function filter(par){
            ...
            timeFil = document.getElementById('cmbTime').value;
            kso = document.getElementById('cmbKso').value;
            var url = "pengajuan_Klaim_t_utils.php?grid="+par;
            if(par == 1){
                url += "&kso="+kso;
            }
            else{
		...
		function kirim(){
			...
			document.getElementById('btnVerifikasi').disabled=true;
			url="pengajuan_Klaim_t_utils.php?grid=1&klaim_id="+klaim_id+"&txtTgl="+TglA+"&txtTgl2="+TglZ+"&kso="+kso+"&posting="+cmbPost;
			//alert(url);
			a.loadURL(url,"","GET");
		}
		//=========================================
		...
		var nilai='';
			...
			if (tmp!="")
			{
				...
				{
					url="pengajuan_Klaim_t_utils.php?grid=1&act=verifikasi&klaim_id="+klaim_id+"&userId=<?php echo $userId; ?>&txtTgl="+txtTglA+"&kso="+kso+"&posting="+document.getElementById('cmbPost').value+"&fdata="+tmp;
					//alert(url);
					a.loadURL(url,"","GET");
				}
			}
		...
		grdKlaim.baseURL("pengajuan_Klaim_t_utils.php?grid=klaim&thn="+thn);
		//alert("pengajuan_Klaim_t_utils.php?grid=klaim&thn="+thn);
        grdKlaim.Init();
		...
		a.baseURL("pengajuan_Klaim_t_utils.php?grid=1&tipe=<?php echo $tipe;?>&tipePend=0&kso="+kso+"&waktu="+timeFil+"&bln="+bln+"&thn="+thn);
		//alert("pengajuan_Klaim_t_utils.php?grid=1&tipe=<?php echo $tipe;?>&kso="+kso+"&waktu="+timeFil+"&bln="+bln+"&thn="+thn);
        a.Init();
        ...
        b.baseURL("pengajuan_Klaim_t_utils.php?grid=2&tipe=<?php echo $tipe;?>&tipePend=0&kunjungan_id=0&waktu=");
        b.Init();
