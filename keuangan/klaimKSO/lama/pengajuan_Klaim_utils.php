    ...
    case 'verifikasi':
		...
		else if ($posting==1)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$sqlCek="SELECT * FROM k_piutang WHERE kunjungan_id='$idKunj' AND kso_id='$ksoid' AND status=2";
				//echo $sqlCek."<br>";
				...
				if (mysql_num_rows($rsCek)>0){
					...
					if (mysql_affected_rows()>0){
						...
						if (mysql_affected_rows()==0){
							$sqlVer="UPDATE k_piutang
									SET biayaKSO_Klaim = '$selisihTarip', status=2
									WHERE kunjungan_id = '$idKunj' AND kso_id='$ksoid'";
							//echo $sqlVer."<br>";
							$rsVer=mysql_query($sqlVer);
							//=======unposting gagal======
		...
		if ($posting==0){
			$sql = "SELECT
					...
					FROM k_piutang kp
					  ...
					  INNER JOIN $dbbilling.b_ms_kso kso
						ON kp.kso_id = kso.id
					WHERE kp.tglP BETWEEN '$tgl' AND '$tgl2' AND kp.status=0
						$fkso $filter
