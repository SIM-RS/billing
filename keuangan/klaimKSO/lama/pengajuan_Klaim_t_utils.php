...
/* if($statusProses=='Error') {
	...
	if($grid == "klaim") {
		...
		while ($rows=mysql_fetch_array($rs)){
			$i++;
			$tedit="<img src='../icon/edit.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Mengubah' onClick='fEditKlaim()'>";
			...
			$tkso="<table width='100%' border='0' cellpadding='0' cellspacing='0'><tr><td height='25' align='center' style='border-bottom:solid 1px;'>UMUM</td></tr><tr><td height='25' align='center' style='border-bottom:solid 1px;'>BPJS</td></tr></table>";
			$tproses=$tedit."      |     ".$thapus."      |     ".$tverif;
			$sisip=$rows["id"]."|".$rows["kso_id"]."|".$rows["verifikasi"]."|".$rows["nilai"];
			$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["tgl_klaim"].chr(3).$rows["no_bukti"].chr(3).$tkso.chr(3).number_format($rows["nilai"],0,",",".").chr(3).$tstatus.chr(3).$tproses.chr(6);
		}
    }
	elseif($grid == 1) {
		while ($rows=mysql_fetch_array($rs)){
