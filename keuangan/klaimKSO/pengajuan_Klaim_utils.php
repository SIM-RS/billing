...
switch(strtolower($_REQUEST['act'])) {
	case 'buat_klaim':
		$klaim_id=0;
		$tglKlaim=tglSQL($_REQUEST['tglKlaim']);
		$no_bukti=$_REQUEST['no_bukti'];
		$sql="INSERT INTO k_klaim
            (tgl,
             no_bukti,
             kso_id,
             nilai,
	...
	case 'verif_klaim':
		...
		$sql="UPDATE k_klaim SET verifikasi='$tipeVerif',tgl_verifikasi=NOW(),user_verifikasi='$userId' WHERE id='".$klaim_id."'";
		$rs=mysql_query($sql);
		break;
	case 'hapus_klaim':
		$sql="DELETE FROM k_klaim WHERE id='".$klaim_id."'";
		$rs=mysql_query($sql);
		break;
    case 'verifikasi':
		...
		if ($posting==0)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				if (mysql_num_rows($rsVer)>0){
					...
					$sqlVer="UPDATE k_piutang
							SET biayaKSO_Klaim = '$selisihTarip', flag = '$flag'
							WHERE kunjungan_id = '$idKunj' AND kso_id='$ksoid' AND tipe=0";
					//echo $sqlVer."<br>";
				}else{
					$tipeUpdt=1;
				...
				if (mysql_affected_rows()>0){
					$sqlVer="UPDATE k_piutang
							SET status = 2,user_posting_bKlaim='$userId',tgl_posting_bKlaim=NOW(), flag = '$flag'
					...
					if (mysql_affected_rows()>0){
						...
						if (mysql_affected_rows()==0){
							$sqlVer="UPDATE k_piutang
									SET status = 0,user_posting_bKlaim='0',tgl_posting_bKlaim=NULL,biayaKSO_Klaim = '0', flag = '$flag'
						...
						$sqlVer="UPDATE k_piutang
								SET biayaKSO_Klaim = '0', flag = '$flag'
								WHERE kunjungan_id = '$idKunj' AND kso_id='$ksoid' AND tipe='$tipeUpdt'";
						//echo $sqlVer."<br>";
						$rsVer=mysql_query($sqlVer);
			...
			$sqlVer="SELECT IFNULL(SUM(nilai_klaim),0) ntot FROM k_klaim_detail WHERE klaim_id='$klaim_id'";
			//echo $sqlVer."<br>";
			$rsVer=mysql_query($sqlVer);
			$rwVer=mysql_fetch_array($rsVer);
			$ntot=$rwVer["ntot"];
			$sqlVer="UPDATE k_klaim SET nilai = '$ntot' WHERE id = '$klaim_id'";
			//echo $sqlVer."<br>";
			$rsVer=mysql_query($sqlVer);
		}
		else if ($posting==1)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$sqlCek="SELECT * FROM k_piutang WHERE kunjungan_id='$idKunj' AND , flag = '$flag' AND kso_id='$ksoid' AND status=2";
				//echo $sqlCek."<br>";
				$rsCek=mysql_query($sqlCek);
				if (mysql_num_rows($rsCek)>0){
					//========uposting selisih tarip=========
					$rwCek=mysql_fetch_array($rsCek);
					$k_piutang_id=$rwCek["id"];
					$selisihTarip=$rwCek["biayaKSO_Klaim"];
					$sqlVer="UPDATE k_piutang
							SET biayaKSO_Klaim = 0, status=0, flag = '$flag'
							WHERE kunjungan_id = '$idKunj' AND kso_id='$ksoid'";
					//echo $sqlVer."<br>";
					$rsVer=mysql_query($sqlVer);
					if (mysql_affected_rows()>0){
						$sqlVer="DELETE FROM k_klaim_detail WHERE fk_id = $k_piutang_id";
						//echo $sqlVer."<br>";
						$rsVer=mysql_query($sqlVer);
						if (mysql_affected_rows()==0){
							$sqlVer="UPDATE k_piutang
									SET biayaKSO_Klaim = '$selisihTarip', status=2, flag = '$flag'
									WHERE kunjungan_id = '$idKunj' AND kso_id='$ksoid'";
							//echo $sqlVer."<br>";
							$rsVer=mysql_query($sqlVer);
							//=======unposting gagal======
			...
			$sqlVer="SELECT IFNULL(SUM(nilai_klaim),0) ntot FROM k_klaim_detail WHERE klaim_id='$klaim_id' AND tstatus = 2";
			//echo $sqlVer."<br>";
			$rsVer=mysql_query($sqlVer);
			$rwVer=mysql_fetch_array($rsVer);
			$ntot=$rwVer["ntot"];
        ...
        break;
	case "setkunjungan":
		$klaim_detail_id = $_REQUEST['klaim_detail_id'];
		$kunjungan_id = $_REQUEST['kunjungan_id'];
		$kso_id = $_REQUEST['kso_id'];
		
		$sqlVer = "SELECT id FROM k_klaim_detail WHERE id = {$klaim_detail_id}";
		$rsVer=mysql_query($sqlVer);
		if (mysql_num_rows($rsVer)>0){
			$sqlVer = "UPDATE k_klaim_detail
	...
	case "hapusimport":
		...
		if(count($arfdata) > 0){
			...
			$shapus = "DELETE FROM k_klaim_detail WHERE tstatus = 0 {$whapus} ";
...
/* if($statusProses=='Error') {
		...
		$sql = "SELECT
				...
				FROM k_klaim k
				  INNER JOIN $dbbilling.b_ms_kso kso
					ON k.kso_id = kso.id
				WHERE k.flag = '$flag' AND YEAR(k.tgl) = '$thn'";		
    }
	elseif($grid == 1) {
		if ($posting==0){
			$sql = "SELECT
					...
					FROM k_piutang kp
					  ...
					  INNER JOIN $dbbilling.b_ms_kso kso
						ON kp.kso_id = kso.id
					WHERE kp.tglP BETWEEN '$tgl' AND pas.flag = '$flag' AND '$tgl2' AND kp.status=0
						$fkso $filter
			...
			$sql = "SELECT
					...
					FROM k_klaim_detail kkd
					  ...
					  INNER JOIN $dbbilling.b_ms_kso kso
						ON kp.kso_id = kso.id
					WHERE kp.status=2 AND pas.flag = '$flag' AND kkd.klaim_id='$klaim_id'
						$fkso $filter
					GROUP BY kp.kunjungan_id,kp.kso_id
					ORDER BY $sorting";
		} else {
			$sql = "SELECT 
					  ...
					  IFNULL(SUM(IFNULL(kp.biayaKSO, kkd.tarif)),0) AS biayaKSO,
					  IFNULL(SUM(IFNULL(kp.biayaKSO_Klaim, kkd.totaltarif)),0) AS biayaKSO_Klaim,
					  IFNULL(SUM(kp.biayaPasien),0) AS biayaPasien,
					  IFNULL(SUM(kp.bayarKSO),0) AS bayarKSO,
					  IFNULL(SUM(kp.bayarPasien),0) AS bayarPasien,
					  IFNULL(SUM(kp.piutangPasien),0) AS piutangPasien,
					  kkd.kunjungan_id, kkd.sep
					FROM k_klaim_detail kkd
					...
					WHERE kkd.tstatus < 2
					AND pas.flag = '$flag' AND kkd.sep IS NOT NULL
					  AND kkd.klaim_id = '{$klaim_id}'
						{$filter}
					GROUP BY kp.kunjungan_id, kp.kso_id, kkd.norm, kkd.sep
					ORDER BY {$sorting}";
		}
    }
    else if($grid == 2) {
            ...
            $sql = "select * from (select k.id,u.nama as unit,ps.no_rm,ps.nama as pasien,t1.*
				...
				AND t.kso_id='$kso' AND t.tgl='$tgl' AND p.flag = '$flag' and p.kunjungan_id = '$kunj_id') AS gab GROUP BY gab.id) t1
		...
		$sql = "SELECT *
				...
				WHERE pas.no_rm = '{$norm}'
				  {$ftgl}
				  {$fkso}) t1
				WHERE 0 = 0 AND pas.flag = '$flag' {$filter}
				ORDER BY {$sorting}";
	}
	...
	if($grid == "klaim") {
		...
		while ($rows=mysql_fetch_array($rs)){
			$i++;
			$baris++;
			$tedit="<img src='../icon/edit.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Mengubah' onClick='fEditKlaim({$baris})'>";
			$thapus="<img src='../icon/del.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Menghapus' onClick='fHapusKlaim({$baris});'>"; //$rows["id"]
			//$tverif="<input type='checkbox' name='' value='' onClick=''>";
			$tstatus="";
			$tverif="<img src='../icon/go.png' border='0' width='16' height='16' align='absmiddle' class='proses' title='Data Pengajuan Klaim Sudah Dibayar'>";
			...
			$sisip=$rows["id"]."|".$rows["kso_id"]."|".$rows["verifikasi"]."|".$rows["nilai"];
			$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["tgl_klaim"].chr(3).$rows["no_bukti"].chr(3).$rows["nama"].chr(3).number_format($rows["nilai"],0,",",".").chr(3).$tstatus.chr(3).$tproses.chr(6);
		}
    }
	elseif($grid == 1) {
		while ($rows=mysql_fetch_array($rs)){
			...
			if ($posting==0 || $posting==2){
				...
				$tKSO_Klaim="<input type='text' id='nilai_$i' class='txtinput' {$readonly} value='".number_format($nilai,0,",",".")."' style='width:70px; font:12px tahoma; padding:2px; text-align:right' onkeyup='zxc(this);' />";
			}else{
			...
			if($posting != 2){
				...
				$sisip=$rows["id"]."|".$rows["no_rm"]."|".tglSQL($rows["tgl"])."|".$rows["sep"]."|".$rows["kso_id"]; //."|".$rows["pasien"];
			}
		...
		while ($rows=mysql_fetch_array($rs)) {
            $i++;
			$baris++;
			$sisip = $rows['id']."|".$rows['unit_id']."|".$_REQUEST['kso_id']."|".$klaim_detail_id;
