...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
		...
		*/ 
		...
		function progresBar(total, progress){
			var percent = 0;
			percent = (progress / total) * 100;
			jQuery("#progress-bar").css('width', Math.round(percent) + '%');
			jQuery("#progress-status").html(Math.round(percent)+ "% Imported...");
			if(Math.round(percent) >= 70){
				jQuery("#progress-status").css('color','#fff');
			}
		...
		function readText(that){
			...
			jQuery("#progress-status").html(" ");
			jQuery("#progress-status").css('color','#000');
			if(that.files && that.files[0]){
				...
				reader.onload = function (e) {
					...
					var tdata = []; var tstr; var count = looping = 1; var totalDone = 0;
					var user_id = "<?php echo $userId; ?>";
					var kso_id = document.getElementById('cmbKsoBuatKlaimTerima').value;
					var dataFile = [];
					for (var i=0; i < output1.length; i++){
						// var tmpData = output1[i].replace(/\ /g,'');
						var tmpData = output1[i].split(" ").join('');
						tmpData = output1[i].split("\t").join('');
						
						if(tmpData.length > 10){
							dataFile.push(output1[i]);
						} else {
							// alert('kosong');
						}
					}
					
					var n = dataFile.length-1;
					var maxloop = Math.ceil(dataFile.length/10);
					for (var i=1; i < dataFile.length; i++){
						tdata.push(dataFile[i]);
						if(count == 10){
							var sent = tdata;
							var tmpcount = count;
							looping+=1; count = 1; tdata = [];
							...
							jQuery.ajaxQueue({
								...
								data: {	data: sent, 
										...
										'kso_id': kso_id, 
										'klaim_id': klaim_id, 
										'klaim_terima_id': klaim_terima_id
									  },
								type: 'POST',
								success: function(data) {
									totalDone += (data*1);
									progresBar(n, totalDone);
								}
							});
						} else if(looping == maxloop){
							var sent = tdata;
							var tmpcount = count;
							looping = 1; count = 1; tdata = [];
						...
						/* tdata = output1[i].split("\t");
						for (var j=0;j<tdata.length;j++){
							// alert(tdata[3]);
							tstr = tdata[j];
							document.getElementById('main').innerHTML = i + " - " + j;
							document.getElementById('main').innerHTML = tdata[3]+"<br /><br />";
							//document.getElementById('main').innerHTML += tdata[j]+"<BR>";
						} */
					}
				};//end onload()
				reader.readAsText(that.files[0]);
				// _("fileU").value = "";
				jQuery('#impo').prop('checked', false);
				...
				alert('Masukkan file import txt terlebih dahulu!');
    ...
    <body bgcolor="#808080" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"><!-- onload="viewTime()"-->
        ...
        <div align="center">
        	...
        	<div id="divPopPx" class="popup" style="width:1210px;height:auto;display:none;">
            <fieldset>
                ...
                <table id="tblPopBuatKlaim" width="1200" border="0" cellpadding="0" cellspacing="0" bgcolor="#EAEFF3" style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold"><span id="spnBuatKlaim" style="display:none"></span>
                    <tr height="30">
                      <td colspan="2" style="font-size:16px" align="center">Penerimaan Klaim KSO</td>
                    </tr>
                    <tr height="30">
                ...
                <table id="tblPopPx" style="display:none" width="1200" border="0" cellpadding="0" cellspacing="0" bgcolor="#EAEFF3">
                    <tr>
                        <td valign="top" align="center">
                            <table border="0" cellpadding="2" cellspacing="2" align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold">
                                <tr height="30">
									...
									<td>
										...
										<div id="div-form" style="text-align:center;">
											<!--form name="uploadForm" id="uploadForm" enctype="multipart/form-data" method="post"-->
												...
												<input type="button" name="upload" class="btnSubmit" value="Import" id="upload" onclick='readText(document.getElementById("fileU"))'/>
											<!--/form-->
											<div id="progress-div">
                    ...
                    <tr id="trPen">
                        <td align="center" >
                            ...
                            <fieldset style="max-width:100%;">
                                <div id="gridbox" style="width:1200px; height:440px; background-color:white; overflow:hidden;"></div>
                                <div id="paging" style="width:1200px;"></div>
                            </fieldset>
                        </td>
                    </tr>
                </table>
    ...
    <script type="text/JavaScript" language="JavaScript">
        ...
        function goFilterAndSort(grd){
			...
			var cmbpost=document.getElementById('cmbPost').value;
			switch(grd){
				case 'gridKlaimTerima':
					//grdKlaim
					var thn = document.getElementById('thn').value;
					var url = "penerimaan_Klaim_utils.php?grid=klaimTerima&thn="+thn+"&filter="+grdKlaim.getFilter()+"&sorting="+grdKlaim.getSorting()+"&page="+grdKlaim.getPage();
					grdKlaim.loadURL(url,"","GET");
					break;
				case 'gridbox':
				...
				case 'gridbox_pop':
					...
					//var namapas = sisip[4];
					//document.getElementById('namaPasien').value = namapas;
					url = "penerimaan_Klaim_utils.php?grid=3&norm="+norm+"&tglmsk="+tglmsk+"&sep="+sep+"&klaim_terima_detail_id="+klaim_terima_detail_id+"&kso="+kso_id+"&load=0&filter="+b.getFilter()+"&sorting="+b.getSorting()+"&page="+b.getPage();
					//alert(url);
					b.loadURL(url,"","GET");
					break;
			}
        ...
        function konfirmasi(key,val){
            ...
            if (val!=undefined){
                var tmp36 = val.split("|");
				//alert(tmp36[1]);
				if(tmp36[5] == "klaimTerima"){
					grdKlaim.cellSubTotalSetValue(7,tmp36[0]);
					grdKlaim.cellSubTotalSetValue(8,tmp36[1]);
				} else {
					a.cellSubTotalSetValue(10,tmp36[0]);
					a.cellSubTotalSetValue(11,tmp36[1]);
					a.cellSubTotalSetValue(12,tmp36[2]);
					a.cellSubTotalSetValue(13,tmp36[3]);
		...
		function cetakView(cmbKlaim){
			var param = cmbKlaim.split('|');
			var cmbJReport = 1;
			var cmbKsoRep = param[4];
			var url = '../laporan/penerimaanKlaim.php?cmbJReport='+cmbJReport+'&cmbKlaim='+cmbKlaim+'&cmbKsoRep='+cmbKsoRep;
			window.open(url);
		}
		
		function fcmbPostChange(p){
			if(p.value=="0" || p.value=="2" || p == "2"){
		...
		function chkKlik(p){
			...
			for (var i=0;i<a.getMaxRow();i++){
				...
				if (cekbox==1){
					if(document.getElementById('nilai_'+(i+1)).value){
			...
			a.cellSubTotalSetValue(13,FormatNumberFloor(parseInt(sTotal),"."));
		}
		...
		function setNoTerima(param){
			var thn = document.getElementById('thn').value;
			var noTerimaM = prompt('Masukkan No Bukti Penerimaan!');
			if(noTerimaM && noTerimaM != ''){
				//alert(noTerimaM);
				var url = "penerimaan_Klaim_utils.php?grid=klaimTerima&thn="+thn+"&noBuktiTerima="+noTerimaM+"&idKlaimTerima="+param+"&act=setnoterima";
				//alert(url);
				grdKlaim.loadURL(url,'','GET');
			}
		}
		...
		grdKlaim.setSubTotal(",,,,,SubTotal :&nbsp;,0,0,");
		grdKlaim.setIDColHeader(",,no_terima,,no_bukti,nama,,");
		...
		grdKlaim.setSubTotalAlign("center,center,center,center,center,center,right,right,center");
		...
		a.setColHeader("NO,TANGGAL KUNJUNGAN,TANGGAL PULANG, NO SEP, NO RM,NAMA,KSO,KUNJUNGAN AWAL,TARIF REGULER,BIAYA PASIEN,NILAI PENGAJUAN,NILAI PENERIMAAN,VERIFIKASI<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
		a.setCellType("txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,chk");
		a.setSubTotal(",,,,,,,,SubTotal :&nbsp;,0,0,0,,");
		a.setIDColHeader(",,,sep,no_rm,pasien,kso,unit,,,,,");
		a.setColWidth("30,70,70,140,60,130,100,110,70,70,70,80,50");
		a.setCellAlign("center,center,center,center,center,left,center,left,right,right,right,right,center");
		a.setSubTotalAlign("center,center,center,center,center,center,center,right,right,right,right,right,center");
        a.setCellHeight(20);
        a.setImgPath("../icon");
