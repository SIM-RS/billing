...
$tglACT=gmdate('Y-m-d',mktime(date('H')+7));
//===============================
$IdTrans=36;
...
switch(strtolower($_REQUEST['act'])) {
	...
	case 'verif_klaim_terima':
		...
		if ($tipeVerif=="1"){
			$sql="SELECT kt.*,k.tgl tglKlaim,k.kso_id,k.no_bukti no_buktiKlaim,k.nilai nilaiKlaim 
					FROM k_klaim_terima kt INNER JOIN k_klaim k ON kt.klaim_id=k.id 
					WHERE kt.id='$klaim_terima_id'";
			$rs1=mysql_query($sql);
			$rw1=mysql_fetch_array($rs1);
			$tglKlaim=$rw1["tglKlaim"];
			$tglKlaimTerima=$rw1["tgl"];
			$no_buktiKlaim=$rw1["no_buktiKlaim"];
			$no_buktiTerima=$rw1["no_bukti"];
			$kso_id=$rw1["kso_id"];
			$nilai_klaim=$rw1["nilaiKlaim"];
			$nilai_terima=$rw1["nilai"];
			
			$sql="INSERT INTO k_transaksi
							(fk_id,
							 id_trans,
							 tipe_trans,
							 tgl,
			...
			$sql="DELETE FROM k_transaksi WHERE fk_id='$klaim_terima_id' AND id_trans='1' AND tipe_trans='1'";
			$rs1=mysql_query($sql);
		}
    ...
    case 'verifikasi':
			...
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				if (mysql_num_rows($rsVer)>0){
					...
					if (mysql_errno()>0){
						...
						$sqlPi = "select kunjungan_id from k_klaim_detail where id = '{$klaim_detail_id}'";
						$qPi = mysql_query($sqlPi);
						$dPi = mysql_fetch_array($qPi);
						
						$sUpi = "update k_piutang
								 set bayarKSO = (bayarKSO+{$nilai_terima}),
									 tgl_lunasKSO = IF(bayarKSO >= biayaKSO_klaim, CURRENT_DATE(), NULL), flag = '$flag'
								 where kunjungan_id = ".$dPi['kunjungan_id'];
						$qUpi = mysql_query($sUpi);
					}
				}
			}
		...
		else if ($posting==1)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$sqlVer="SELECT id, klaim_detail_id, nilai_terima FROM k_klaim_terima_detail
						WHERE id = '$id' AND flag = '$flag' AND tstatus=0";
				//echo $sqlCek."<br>";
				$rsVer=mysql_query($sqlVer);
				if (mysql_num_rows($rsVer)>0){
					$rwVer=mysql_fetch_array($rsVer);
					$klaim_detail_id=$rwVer["klaim_detail_id"];
					$nilai_terima=$rwVer["nilai_terima"];
					...
					if (mysql_affected_rows()>0){
						...
						if (mysql_affected_rows()==0){
							...
							$sUpi = "update k_piutang
									 set bayarKSO = IF((bayarKSO-{$nilai_terima}) < 0, 0, (bayarKSO-{$nilai_terima})),
										 tgl_lunasKSO = IF(bayarKSO >= biayaKSO_klaim, CURRENT_DATE(), NULL), flag = '$flag'
									 where kunjungan_id = ".$dPi['kunjungan_id'];
							$qUpi = mysql_query($sUpi);
						}
			...
			if($statusProses!='Error'){
				...
				$sqlVer="SELECT IFNULL(SUM(nilai_terima),0) ntot FROM k_klaim_terima_detail WHERE klaim_terima_id='$klaim_terima_id' and tstatus <> 0";
				//echo $sqlVer."<br>";
				$rsVer=mysql_query($sqlVer);
				$rwVer=mysql_fetch_array($rsVer);
				$ntot=$rwVer["ntot"];
	...
	case "setkunjungan":
		...
		$sqlVer = "SELECT ktd.id, kt.klaim_id, IFNULL(kt.kso_id,0) kso_id
					...
					WHERE ktd.id = {$klaim_terima_detail_id} AND flag = '$flag'";
		$rsVer=mysql_query($sqlVer);
		if (mysql_num_rows($rsVer)>0){
	...
	case "hapusimport":
		...
		/* for ($i=0;$i<count($arfdbuat_klaim_terimaata);$i++){
		} */
		break;
	case "setnoterima":
		$noBuktiTerima = $_REQUEST['noBuktiTerima'];
		$idKlaimTerima = $_REQUEST['idKlaimTerima'];
		$sqlUp = "UPDATE k_klaim_terima 
					SET no_bukti = '{$noBuktiTerima}'
					WHERE id = '{$idKlaimTerima}'";
		$queryUp = mysql_query($sqlUp);
		if (mysql_affected_rows()==0){
			$statusProses='Error';
			$alasan='Pembaruan No Bukti Terima Gagal!';
		}
		break;
}
...
} else if (($statusProses=='') && (strtolower($_REQUEST['act'])=='setnoterima')){
	//echo mysql_error();
	$alasan = "Pembaruan No Bukti Terima Berhasil!";
} */
...
/* if($statusProses=='Error') {
	...
	if($grid == "klaimTerima") {
		$sql = "SELECT *
				FROM (SELECT
						...
						WHERE kt.flag = '$flag' AND YEAR(kt.tgl) = '{$thn}') t1
				WHERE 0 = 0 {$filter}
				ORDER BY {$sorting}";
						
		$sqlSum = "SELECT ifnull(SUM(tsum.nilai),0) as total, ifnull(SUM(tsum.nilai_terima),0) as terima FROM ({$sql}) as tsum";
		$rsSum = mysql_query($sqlSum);
		$rwSum = mysql_fetch_array($rsSum);
		$stot = $rwSum["total"];
		$stotterima = $rwSum["terima"];
    }
	elseif($grid == 1) {
		if ($posting=="0"){
			$sql = "SELECT *
					FROM (SELECT
							...
							FROM k_klaim_detail kd
							  ...
							  INNER JOIN $dbbilling.b_ms_kso kso
								ON kp.kso_id = kso.id
							WHERE kd.klaim_id='$klaim_id' AND kd.flag = '$flag' AND kd.tstatus<2
			...
			$sql = "SELECT *
					FROM (SELECT
							  ...
							  IFNULL(SUM(IFNULL(kp.biayaPasien, ktd.tarifPasien)),0) AS biayaPasien,
							  IFNULL(SUM(IFNULL(kd.nilai_klaim, IFNULL(ktd.nilai_pengajuan, ktd.tarif))),0) AS nilai_klaim,
							...
							WHERE ktd.klaim_terima_id='$klaim_terima_id'
								AND kt.flag = '$flag' AND ktd.tstatus <> 0
			...
			$sql = "SElECT *
					FROM (SELECT ktd.id,
							...
							WHERE ktd.klaim_terima_id = '$klaim_terima_id'
							  AND kt.flag = '$flag' AND ktd.tstatus < 2 AND ktd.sep IS NOT NULL
							GROUP BY kp.kunjungan_id, kp.kso_id, ktd.norm, ktd.sep) t1
					WHERE 0 = 0 $filter
					ORDER BY $sorting";
		}
            ...
            $sql = "select * from (select k.id,u.nama as unit,ps.no_rm,ps.nama as pasien,t1.*
				...
				INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id_asal = mu.id
                    WHERE p.flag = '$flag' AND (mu.parent_id <> 44 and mu.inap = 0 and p.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=0 and u.parent_id <> 44))
				AND t.kso_id='$kso' AND t.tgl='$tgl' and p.kunjungan_id = '$kunj_id') AS gab GROUP BY gab.id) t1
                    ...
                    inner join $dbbilling.b_ms_unit u on u.id = p.unit_id) t1 
		...
		if($tglmsk == '-'){
			...
			$ftgl = "AND k.tgl BETWEEN '{$tglACT} 00:00:00' AND '{$tglACT} 23:59:59'";
		}
		...
		$sql = "SELECT *
				...
				INNER JOIN $dbbilling.b_ms_kso kso
				   ON kso.id = k.kso_id
				WHERE pa.flag = '$flag' AND/* pas.no_rm = '{$norm}'  */
					sep like '%{$sep}%'
				  {$ftgl}
				  {$fkso}) t1
				WHERE 0 = 0 {$filter}
	...
	if($grid == "klaimTerima") {
		while ($rows=mysql_fetch_array($rs)){
			...
			$paramView = "'".$rows['id'].'|'.$rows['tgl_terima'].'|'.$rows['no_terima'].'|'.$rows['nilai_terima'].'|'.$rows['kso_id']."'";
			$cetakView = 'cetakView('.$paramView.')';
			$tview = "<img src='../icon/lihat.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Menghapus' onClick={$cetakView}>";
			...
			$noTerima = ($rows["no_terima"]!="")?$rows["no_terima"]:'Set No Terima';
			$noTerima = "<span style='text-decoration:underline; color:blue;' onclick='setNoTerima(".$rows["id"].")'>{$noTerima}</span>";
			$tproses=$tedit."      |     ".$tview."      |     ".$thapus."      |     ".$tverif;
			...
			$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["tgl_terima"].chr(3).$noTerima.chr(3).$rows["tgl_klaim"].chr(3).$rows["no_bukti"].chr(3).$rows["nama"].chr(3).number_format($rows["nilai"],0,",",".").chr(3).number_format($rows["nilai_terima"],0,",",".").chr(3).$tproses.chr(6);
		}
    }
	elseif($grid == 1) {
		while ($rows=mysql_fetch_array($rs)){
			...
			if ($posting==0|| $posting==2){
				...
				$tKSO_Terima = number_format($rows["nilai_terima"],0,",",".")."<input type='hidden' id='nilai_$i' value='".number_format($tKSO_Klaim,0,",",".")."' />";
			}
			if($posting == 0){
				$sisip=$rows["id"];
			} else {
			...
			$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["tgl"].chr(3).$rows["tgl_p"].chr(3).$rows["sep"].chr(3).$rows["no_rm"].chr(3).$rows["pasien"].chr(3).$kso.chr(3).$tmpLay.chr(3).number_format($tPerda,0,",",".").chr(3).number_format($tPx,0,",",".").chr(3).number_format($tKSO_Klaim,0,",",".").chr(3).$tKSO_Terima.chr(3)."0".chr(6);
			$tmpLay = '';
		}
    }
    else if($grid == 2) {
    ...
    if ($dt!=$totpage.chr(5)) {
		...
		} else if($grid == 'klaimTerima'){
			$dt = $dt.number_format($stot,0,",",".")."|".number_format($stotterima,0,",",".")."|||".$alasan."|".$grid;
		}
    } else {
		if($grid == 1){
			$dt = $dt.number_format($totPer,0,",",".")."|".number_format($totPas,0,",",".")."|".number_format($totKsoKlaim,0,",",".")."|".number_format($totKlaimTerima,0,",",".")."|".$alasan."|".$grid;
