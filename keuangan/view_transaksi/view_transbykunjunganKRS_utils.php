...
switch($waktu) {
    case 'harian':
        $waktu = " and DATE(k.tgl_pulang) = '$tgl' ";
        break;
    case 'periode':
        $tglAwal = tglSQL($_REQUEST['tglAwal']);
        $tglAkhir = tglSQL($_REQUEST['tglAkhir']);
        $waktu = " and DATE(k.tgl_pulang) between '$tglAwal' and '$tglAkhir' ";
        break;
    case 'bulan':
...
if($statusProses=='Error') {
    ...
    if($grid == 1){
		...
		$sql="SELECT 
			  ...
			  nmkso,
			  SUM(biaya) biaya,
			  SUM(biaya_kso) biaya_kso,
			  SUM(biaya_pasien) biaya_pasien,
			  SUM(bayar) bayar,
			  SUM(bayar_kso) bayar_kso,
			  SUM(bayar_pasien) bayar_pasien
			FROM
			(SELECT
			  k.id,
			  k.pasien_id,
			...
			SELECT
			  ...
			  AND UNIT_ID=t.UNIT_ID)	  bayar,  
			  0       bayarKso,
			  (SELECT IFNULL(SUM(TOTAL_HARGA),0) FROM $dbapotek.a_kredit_utang 
			  WHERE NORM=mp.no_rm AND FK_NO_PENJUALAN=t.NO_PENJUALAN 
			  AND UNIT_ID=t.UNIT_ID)	  bayar_pasien
			FROM $dbbilling.b_kunjungan k
			  INNER JOIN $dbbilling.b_ms_pasien mp
				ON k.pasien_id = mp.id
			  INNER JOIN $dbbilling.b_ms_unit mu
    ...
    if($grid == 1) {
    	...
    	while ($rows=mysql_fetch_array($rs)) {
			...
			$dt.=$rows["id"].'|'.$rows['kso_id'].'|'.$rows['pasien_id'].chr(3).number_format($i,0,",","").chr(3).$rows["tgl"].chr(3).$rows["no_rm"].chr(3).$rows["nama"].chr(3).$rows["nmkso"].chr(3).$unit_asal.chr(3).number_format($tPerda,0,",",".").chr(3).number_format($tPx,0,",",".").chr(3).number_format($tKSO,0,",",".").chr(3).number_format($tIur,0,",",".").chr(3).$stSelisih.chr(6);
			$tmpLay = '';
		}
    }
    else if($grid == 2) {
        ...
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $dt .= $rows["id"].chr(3).number_format($i,0,",",".").chr(3).$rows["tgl"].chr(3).$rows["unit"].chr(3).$rows["nmTind"].chr(3).$rows["dokter"].chr(3).number_format($tPerda,0,",",".").chr(3).number_format($tPx,0,",",".").chr(3).number_format($tKSO,0,",",".").chr(3).number_format($tIur,0,",",".").chr(3).$stSelisih.chr(6);
        }
    }
