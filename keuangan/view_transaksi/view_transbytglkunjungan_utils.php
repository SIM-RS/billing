...
switch($waktu) {
    ...
    case 'periode':
        ...
        $waktu = " and k.tgl between '$tglAwal' and '$tglAkhir' ";
        break;
    case 'bulan':
...
if($grid == 1){
	...
	$sql="SELECT * FROM (SELECT id,pasien_id,tgl,tgl1,no_rm,nama, unit_awal,
			...
			FROM $dbbilling.b_kunjungan k INNER JOIN $dbbilling.b_ms_pasien mp ON k.pasien_id = mp.id 
			...
			WHERE 0 = 0 AND k.flag = '$flag' {$waktu} /* k.tgl = '$tgl'  */
			UNION 
			SELECT k.id, k.pasien_id, k.tgl, mp.no_rm, mp.nama, mu.nama AS unit_awal, 
			t.id idTind, t.kso_id, kso.nama AS nmkso, 
			IF(t.status_out=0,(IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,1,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.tarip+t.retribusi), (IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,0,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.tarip+t.retribusi)) biaya, 
			IF(t.kso_id=1,0,IF(t.status_out=0,(IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,1,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.beban_kso+t.retribusi), (IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,0,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.beban_kso+t.retribusi))) biaya_kso, 
			IF(t.kso_id=1,IF(t.status_out=0,(IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,1,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.beban_pasien+t.retribusi), (IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,0,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.beban_pasien+t.retribusi)),0) biaya_pasien, 
			IF(t.kso_id=1,0,IF(t.status_out=0,(IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,1,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.beban_pasien), (IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,0,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.beban_pasien))) iur_pasien, 
			t.bayar, t.bayar_kso, IF(t.kso_id=1,t.bayar_pasien+IF(t.status_out=0,(IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,1,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.retribusi), (IF(DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)=0,0,DATEDIFF(IFNULL(t.tgl_out,NOW()),t.tgl_in)+1))*(t.retribusi)),t.bayar_pasien) bayar_pasien 
			FROM $dbbilling.b_kunjungan k INNER JOIN $dbbilling.b_ms_pasien mp ON k.pasien_id = mp.id 
			INNER JOIN $dbbilling.b_ms_unit mu ON k.unit_id = mu.id 
			INNER JOIN $dbbilling.b_pelayanan p ON k.id = p.kunjungan_id 
			...
			SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_SATUAN)+
  			FLOOR((t.PPN/100) *  SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_SATUAN)) biaya, 
			SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_KSO)+
  			FLOOR((t.PPN/100) *  SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_KSO)) biaya_kso, 
			IF(m.kso_id_billing=1,SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_PX)+
  			FLOOR((t.PPN/100) *  SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_PX)),0) biaya_pasien, 
			IF(m.kso_id_billing=1,0,SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_PX)+
  			FLOOR((t.PPN/100) *  SUM((t.QTY_JUAL-t.QTY_RETUR) * t.HARGA_PX))) iur_pasien, 
