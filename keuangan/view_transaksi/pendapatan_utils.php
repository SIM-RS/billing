    ...
    if($grid == 1 || $tipe=='lain2') {
        if($tipe=='lain2') {
            $sql="SELECT t.id,t.tgl,t.no_bukti,t.id_trans,mt.nama AS jenis_pendapatan,t.nilai,t.ket FROM k_transaksi t
				 ...
				 WHERE mt.tipe='1' and k.flag = '$flag' and MONTH(t.tgl)='$bln' AND YEAR(t.tgl)='$thn' and t.id_trans <> 1 ".$filter." order by ".$sorting;
            ...
            $sql = "select * from (select k.id,u.nama as unit,date_format(k.tgl,'%d-%m-%Y') as tgl,DATE_FORMAT(k.tgl_pulang,'%d-%m-%Y') AS tgl_p,ps.no_rm,ps.nama as pasien,t1.* from (SELECT gab.pelayanan_id,SUM(gab.biayaRS) AS biayaRS,SUM(gab.biaya_kso) AS biaya_kso,SUM(gab.biaya_pasien) AS biaya_pasien,
                    ...
                    WHERE (mu.parent_id <> 44 and mu.inap = 0 and p.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=0 and u.parent_id <> 44)) AND t.kso_id='$kso' AND t.tgl='$tgl' AND t.flag = '$flag'
...
FROM $dbbilling.b_tindakan t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id 
WHERE t.kso_id='$kso' AND t.jenis_kunjungan='3' AND t.tgl='$tgl' AND t.flag = '$flag' GROUP BY t.kunjungan_id
UNION
...
GROUP BY t2.kunjungan_id) AS t3 $filter ORDER BY $sorting";
            ...
            $sql = "select * from (select k.id,u.nama as unit,date_format(k.tgl,'%d-%m-%Y') as tgl,DATE_FORMAT(k.tgl_pulang,'%d-%m-%Y') AS tgl_p,ps.no_rm,ps.nama as pasien,t1.* from (SELECT gab.pelayanan_id,SUM(gab.biayaRS) AS biayaRS,SUM(gab.biaya_kso) AS biaya_kso,SUM(gab.biaya_pasien) AS biaya_pasien,
                    ...
                    FROM $dbbilling.b_tindakan t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id
                    INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id_asal=mu.id
                    WHERE (mu.parent_id = 44 and mu.inap = 0 and p.unit_id not IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=1) or p.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=0 and u.parent_id = 44)) AND t.kso_id='$kso' AND t.tgl='$tgl' AND t.flag = '$flag'
                    ) AS gab GROUP BY gab.kunjungan_id
                    ) t1
                    inner join $dbbilling.b_pelayanan p on p.id = t1.pelayanan_id
                    inner join $dbbilling.b_kunjungan k on p.kunjungan_id = k.id
		  ...
		  if($inap == 0){
			 ...
			 $sql = "select * from (select k.id,k.pasien_id,u.nama as unit,date_format(k.tgl,'%d-%m-%Y') as tgl,DATE_FORMAT(k.tgl_pulang,'%d-%m-%Y') AS tgl_p,ps.no_rm,ps.nama as pasien, ua.nama as unit_asal,t1.* from (SELECT gab.pelayanan_id,SUM(gab.biayaRS) AS biayaRS,SUM(gab.biaya_kso) AS biaya_kso,SUM(gab.biaya_pasien) AS biaya_pasien,
				    ...
				    WHERE mu.id='$unit_id' AND t.flag = '$flag' $kso $waktu /* AND t.tgl='$tgl' */ $fbayar
...
WHERE pl.unit_id='$unit_id' $waktu /* AND t.tgl = '$tgl' */ $kso $fbayar GROUP BY pl.id
UNION
...
WHERE p.unit_id='$unit_id' $fbayar AND p.flag = '$flag' AND DATE(t.tgl_in)<='$tgl' AND (DATE(t.tgl_out) >='$tgl' OR t.tgl_out IS NULL) 
...
INNER JOIN $dbbilling.b_kunjungan k ON p.kunjungan_id=k.id
INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id=mu.id
WHERE k.id='$kunj_id' AND DATE(tk.tgl_in)<='$tgl' AND (DATE(tk.tgl_out) >='$tgl' OR tk.tgl_out IS NULL) 
AND (DATE(k.tgl_pulang) >='$tgl' OR k.tgl_pulang IS NULL) AND p.flag = '$flag' AND p.kso_id='$kso' AND tk.aktif=1) AS t1) AS t2 WHERE t2.qty>0";
        }
	   else if($tipe == 3){
...
LEFT JOIN $dbbilling.b_ms_pegawai mp ON t.user_id=mp.id
WHERE t.pelayanan_id='$pel_id' $fbayar AND t.kso_id='$kso' AND t.tgl='$tgl'
UNION
...
INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id=mu.id
INNER JOIN $dbbilling.b_ms_unit u ON p.unit_id_asal = u.id
WHERE p.id='$pel_id' $fbayar2 AND p.flag = '$flag' AND DATE(tk.tgl_in)<='$tgl' AND (DATE(tk.tgl_out) >='$tgl' OR tk.tgl_out IS NULL) 
    ...
    if($grid == 1 || $tipe == 'lain2') {
        if($tipe == 'lain2') {
            while ($rows=mysql_fetch_array($rs)) {
                $i++;
                $dt.=$rows["id"]."|".$rows["id_trans"].chr(3).number_format($i,0,",",".").chr(3).tglSQL($rows["tgl"]).chr(3).$rows["no_bukti"].chr(3).$rows["jenis_pendapatan"].chr(3).number_format($rows["nilai"],0,",",".").chr(3).$rows["ket"].chr(6);
            }
        }
        else {
            while ($rows=mysql_fetch_array($rs)) {
			 ...
			 if($tipe != 4){
				...
				INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id = mu.id
                    WHERE (mu.parent_id <> 44 and mu.inap = 0 and p.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=0 and u.parent_id <> 44))
				AND t.kso_id='$kso' AND t.tgl='$tgl' AND b.flag = '$flag' and p.kunjungan_id = '".$rows["id"]."'";
				}
				else if($tipe == 2){
				    $sqlX = "SELECT *
				    FROM (SELECT
						...
						FROM (SELECT distinct
							 ...
							 WHERE p.kunjungan_id = '".$rows["id"]."'
								AND t.tgl = '$tgl' AND b.flag = '$flag' and (mu.inap = 1
				    ...
				    $sqlX = "SELECT DISTINCT u.nama
					   ...
					   and k.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=0 and u.parent_id = 44))
					    AND t.tgl='$tgl' AND b.flag = '$flag' and p.kunjungan_id = '".$rows["id"]."'";
					    //AND t.kso_id='$kso'
				}
