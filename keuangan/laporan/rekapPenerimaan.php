...
WHERE $waktu $qKso ORDER BY kso.id";
				...
				<tr>
					...
					$TotPerUnit=0;
					for ($i=1;$i<=count($arIdxKso);$i++){
						$sql="SELECT IFNULL(SUM(t1.nilai),0) AS jml FROM (SELECT DISTINCT bt.id,bt.nilai FROM $dbbilling.b_bayar b INNER JOIN $dbbilling.b_bayar_tindakan bt ON b.id=bt.bayar_id 
...
WHERE $waktu AND t.kso_id='".$arIdxKso[$i]."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND p.unit_id='".$rwUnit["id"]."') as t1";
						//echo $sql."<br>";
...
WHERE $waktu AND bt.tipe=1 AND t.kso_id='".$arIdxKso[$i]."' AND t.tipe_pendapatan=0 AND p.unit_id='".$rwUnit["id"]."') as t1";
