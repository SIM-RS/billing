...
?>
<title>.: Rekap Pendapatan Farmasi :.</title>
...
WHERE u.kdunitfar='".$rwUnit["kode"]."' $qKso AND $waktu GROUP BY SHIFT,NO_PENJUALAN,TGL,p.KSO_ID,p.UNIT_ID,p.JENIS_PASIEN_ID,p.CARA_BAYAR) AS t1";
							//echo $sqlJual."<br>";
...
WHERE u.kdunitfar='".$rwUnit["kode"]."' $qKso $waktu2";
				...
				<tr style="font-weight:bold;">
                    ...
                    ?>
				<tr>
				 	<td colspan="2" bgcolor="#FF7777" style="border-left:1px solid #000000; border-right:1px solid #000000; padding-left:50px; font-weight:bold; text-decoration:underline;">&nbsp;INSTALASI FARMASI</td>
					<td width="100" style="border-right:1px solid #000000;">&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000;">&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000;">&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000;">&nbsp;</td>
...
FROM $dbapotek.a_penjualan p 
LEFT JOIN $dbapotek.a_unit u ON p.RUANGAN=u.UNIT_ID INNER JOIN $dbapotek.a_penerimaan ap ON p.PENERIMAAN_ID=ap.ID 
WHERE $qKsoF AND $waktu GROUP BY SHIFT,NO_PENJUALAN,TGL,p.KSO_ID,p.UNIT_ID,p.JENIS_PASIEN_ID,p.CARA_BAYAR) AS t1";
							//echo $sqlJual."<br>";
							$rsJual=mysql_query($sqlJual);
							$rwJual=mysql_fetch_array($rsJual);
							$nJual=(($rwJual["nJual"]-$SubTot1)<0)?0:($rwJual["nJual"]-$SubTot1);
							$bJual=(($rwJual["nBahan"]-$SubTot2)<0)?0:($rwJual["nBahan"]-$SubTot2);

							$sqlReturn="SELECT IFNULL(SUM(ar.qty_retur * ar.nilai),0) AS nReturn,IFNULL(SUM(ar.qty_retur * IF(ap.NILAI_PAJAK=0,(ap.HARGA_BELI_SATUAN * (1-(ap.DISKON/100))),1.1 * (ap.HARGA_BELI_SATUAN * (1-(ap.DISKON/100))))),0) nBahan 
FROM $dbapotek.a_penjualan p LEFT JOIN $dbapotek.a_unit u ON p.RUANGAN=u.UNIT_ID 
INNER JOIN $dbapotek.a_penerimaan ap ON p.PENERIMAAN_ID=ap.ID INNER JOIN $dbapotek.a_return_penjualan ar ON p.ID=ar.idpenjualan
WHERE $qKsoF $waktu2";
							$rsReturn=mysql_query($sqlReturn);
							$rwReturn=mysql_fetch_array($rsReturn);
							$nReturn=(($rwReturn["nReturn"]-$SubTot3)<0)?0:($rwReturn["nReturn"]-$SubTot3);
							$bReturn=(($rwReturn["nBahan"]-$SubTot4)<0)?0:($rwReturn["nBahan"]-$SubTot4);
				...
				?>
				<tr>
				  	<td style="border-left:1px solid #000000; border-right:1px solid #000000;; text-align:center">1</td>
					<td style="border-right:1px solid #000000; text-transform:uppercase">&nbsp;INSTALASI FARMASI</td>
					<td width="100" style="border-right:1px solid #000000; text-align:right;"><?php echo number_format($nJual,0,",","."); ?>&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000; text-align:right;"><?php echo number_format($bJual,0,",","."); ?>&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000; text-align:right;"><?php echo number_format($nReturn,0,",","."); ?>&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000; text-align:right;"><?php echo number_format($bReturn,0,",","."); ?>&nbsp;</td>
