<?php
    ...
    if($waktu == 'Harian'){
		...
		$waktu2 = "rspelindo_keuangan.k_transaksi.tgl = '$tglAwal2' ";
		...
		$waktu2 = "month(rspelindo_keuangan.k_transaksi.tgl) = '$bln' AND year(rspelindo_keuangan.k_transaksi.tgl) = '$thn' ";
		...
		$waktu2 = "rspelindo_keuangan.k_transaksi.tgl between '$tglAwal2' and '$tglAkhir2' ";
	...
	if ($bayar==1){
		...
		$captionByr="SUDAH TERBAYAR";
	}else{
		$fbayar=" AND t.bayar_pasien=0";
		$fbayar2=" AND tk.bayar_pasien=0";
		$captionByr="BELUM TERBAYAR";
	}
	
	$qKso = "";
	if($kso==0){
		$fKso = "SEMUA";
		$qKso = "SELECT DISTINCT kso.id,kso.nama FROM (SELECT t.kso_id FROM $dbbilling.b_tindakan_ak t WHERE t.tgl BETWEEN '$tglAwal2' AND '$tglAkhir2' AND t.tipe_pendapatan='1' $fbayar 
UNION SELECT p.kso_id FROM $dbbilling.b_tindakan_kamar_ak tk INNER JOIN $dbbilling.b_pelayanan p ON tk.pelayanan_id=p.id 
WHERE DATE(tk.tgl_in)<='$tglAkhir2' AND tk.tipe_pendapatan='1' $fbayar2 AND (DATE(tk.tgl_out) >='$tglAwal2' OR tk.tgl_out IS NULL)) AS t1
INNER JOIN $dbbilling.b_ms_kso kso ON t1.kso_id=kso.id ORDER BY kso_id";
...
WHERE $dbbilling.b_ms_kso.id = '".$kso."'";
	...
	$totalF="";
	/*if ($tipe=="all"){
		$ptitle="Rekap Pengurang Pendapatan";
		$totalF=" + FARMASI";
	}else{*/
		$ptitle="Rekap Pengurang Pendapatan Billing";
	//}
?>
...
<table border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
    ...
    <tr>
        <td style="font-size:14px; height:70px; text-transform:uppercase; text-align:center; font-weight:bold;" valign="top">rekapitulasi pengurang pendapatan<?php if ($tipe!="all") echo " Billing ".$captionByr;?><br /><?php echo $Periode;?></td>
    </tr>
	<tr>
		<td>
			<table border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
    ...
    FROM $dbbilling.b_tindakan_ak t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id WHERE t.tipe_pendapatan='1' AND $waktu $qKso $fbayar AND p.unit_id='".$rwUnit["id"]."'";
							//echo $sql."<br>";
                            ...
                            if ($rwUnit["inap"]==1){
								//echo $sql."<br>";
								if($cwaktu == 'Harian'){
									$sql="SELECT IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.tarip),0) biayaRS,IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_kso),0) biayaKSO,
IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_pasien),0) biayaPx 
FROM (SELECT t1.id,t1.kunjungan_id,t1.unit_id_asal,IF(DATE(t1.tgl_in)<'$tglAwal2','$tglAwal2',t1.tgl_in) tgl_in, 
IF((DATE(t1.tgl_out)>'$tglAwal2' OR t1.tgl_out IS NULL),DATE_ADD('$tglAwal2',INTERVAL 1 DAY), 
...
WHERE p.unit_id='".$rwUnit["id"]."' AND p.flag = '$flag' AND t.tipe_pendapatan='1' $fbayar AND DATE(t.tgl_in)<='$tglAwal2' AND (DATE(t.tgl_out) >='$tglAwal2' OR t.tgl_out IS NULL) AND (DATE(k.tgl_pulang) >='$tglAwal2' OR k.tgl_pulang IS NULL) 
AND t.kso_id = '".$wKso["id"]."' AND t.aktif=1) AS t1) AS t2";
								}else if($waktu == 'Bulanan'){
									$sql="SELECT IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.tarip),0) biayaRS,IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_kso),0) biayaKSO,
IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_pasien),0) biayaPx 
FROM (SELECT t1.id,t1.kunjungan_id,t1.unit_id_asal,IF(DATE(t1.tgl_in)<'$tglAwal2','$tglAwal2',t1.tgl_in) tgl_in, 
IF((DATE(t1.tgl_out)>'$tglAkhir2' OR t1.tgl_out IS NULL),DATE_ADD('$tglAkhir2',INTERVAL 1 DAY), 
IF((DATEDIFF(t1.tgl_out,t1.tgl_in)=0 AND t1.status_out=0),DATE_ADD(t1.tgl_out,INTERVAL 1 DAY),t1.tgl_out)) AS tgl_out, 
t1.tarip,t1.beban_kso,t1.beban_pasien,t1.bayar,t1.bayar_kso,t1.bayar_pasien,t1.status_out 
FROM (SELECT p.id,p.kunjungan_id,p.unit_id_asal,t.tgl_in,IFNULL(t.tgl_out,k.tgl_pulang) AS tgl_out, t.tarip,t.beban_kso,
t.beban_pasien,t.bayar,t.bayar_kso,t.bayar_pasien,t.status_out 
FROM $dbbilling.b_tindakan_kamar_ak t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id 
INNER JOIN $dbbilling.b_kunjungan k ON k.id=p.kunjungan_id 
WHERE p.unit_id='".$rwUnit["id"]."' AND p.flag = '$flag' AND t.tipe_pendapatan='1' $fbayar AND DATE(t.tgl_in)<='$tglAkhir2' AND (DATE(t.tgl_out) >='$tglAwal2' OR t.tgl_out IS NULL) AND (DATE(k.tgl_pulang) >='$tglAwal2' OR k.tgl_pulang IS NULL) 
AND t.kso_id = '".$wKso["id"]."' AND t.aktif=1) AS t1) AS t2";
								}else{
									$sql="SELECT IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.tarip),0) biayaRS,IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_kso),0) biayaKSO,
IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_pasien),0) biayaPx 
...
AND t.kso_id = '".$wKso["id"]."' AND t.aktif=1) AS t1) AS t2";
							...
							if ($tipe=="all"){
								//========Farmasi=========
								$fKso = "SELECT DISTINCT m.IDMITRA id,mkso.nama FROM $dbapotek.a_mitra m INNER JOIN $dbbilling.b_ms_kso mkso ON m.KODE_MITRA=mkso.kode WHERE mkso.id='".$wKso["id"]."'";
								$rsKsoF=mysql_query($fKso);
								$rwKsoF=mysql_fetch_array($rsKsoF);
								$sqlJual="SELECT SUM(p.SUB_TOTAL) AS nJual FROM $dbapotek.a_penjualan p INNER JOIN $dbapotek.a_unit u ON p.RUANGAN=u.UNIT_ID INNER JOIN $dbapotek.a_penerimaan ap ON p.PENERIMAAN_ID=ap.ID WHERE u.kdunitfar='".$rwUnit["kode"]."' AND p.KSO_ID='".$rwKsoF["id"]."' AND $waktuF";
								//echo $sqlJual."<br>";
				...
				<tr style="font-weight:bold;">
                  <td style="border-left:1px solid #000000; border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000;">&nbsp;</td>
				  <td style="border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000;">&nbsp;TOTAL PENGURANG PENDAPATAN BILLING<?php echo $totalF; ?></td>
			  ...
			  ?>
				...
				<tr style="font-weight:bold;">
				  	<td style="border-left:1px solid #000000; border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000;">&nbsp;</td>
					<td style="border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000;">&nbsp;TOTAL PENGURANG PENDAPATAN LAIN-LAIN</td>
					<td width="100" style="border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000; text-align:right;"><?php echo number_format($sTotLain2,0,",","."); ?>&nbsp;</td>
				    <td width="100" style="border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000; text-align:right;"><?php echo number_format($sTotLain2,0,",","."); ?>&nbsp;</td>
