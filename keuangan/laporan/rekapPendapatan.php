<?php
	...
	if($kso==0){
		$fKso = "SEMUA";
		$qKso = "SELECT DISTINCT kso.id,kso.nama FROM (SELECT t.kso_id FROM $dbbilling.b_tindakan t WHERE t.tgl BETWEEN '$tglAwal2' AND '$tglAkhir2'
UNION SELECT p.kso_id FROM $dbbilling.b_tindakan_kamar tk INNER JOIN $dbbilling.b_pelayanan p ON tk.pelayanan_id=p.id 
WHERE DATE(tk.tgl_in)<='$tglAkhir2' AND (DATE(tk.tgl_out) >='$tglAwal2' OR tk.tgl_out IS NULL)) AS t1
...
WHERE $dbbilling.b_ms_kso.id = '".$kso."'";
	...
	$totalF="";
	if ($tipe=="all"){
		$ptitle="Rekap Pendapatan";
		$totalF=" + FARMASI";
	}else{
		$ptitle="Rekap Pendapatan Billing";
	}
?>
...
<?=$tlpRS;?>
    ...
    <tr>
        <td style="font-size:14px; height:70px; text-transform:uppercase; text-align:center; font-weight:bold;" valign="top">rekapitulasi penDAPATan<?php if ($tipe!="all") echo " Billing";?><br /><?php echo $Periode;?></td>
    </tr>
	<tr>
		<td>
			<table border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
    ...
    FROM $dbbilling.b_tindakan_ak t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id WHERE $waktu $qKso AND p.unit_id='".$rwUnit["id"]."' AND t.tipe_pendapatan=0";
							/*if ($rwUnit["id"]==39){
								echo $sql."<br>";
							}*/
...
WHERE p.unit_id='".$rwUnit["id"]."' AND t.tipe_pendapatan=0 AND DATE(t.tgl_in)<='$tglAwal2' AND (DATE(t.tgl_out) >='$tglAwal2' OR t.tgl_out IS NULL) AND (DATE(k.tgl_pulang) >='$tglAwal2' OR k.tgl_pulang IS NULL) 
...
FROM $dbbilling.b_tindakan_kamar_ak t INNER JOIN $dbbilling.b_pelayanan p ON t.pelayanan_id=p.id 
INNER JOIN $dbbilling.b_kunjungan k ON k.id=p.kunjungan_id 
WHERE p.unit_id='".$rwUnit["id"]."' AND t.tipe_pendapatan=0 AND DATE(t.tgl_in)<='$tglAkhir2' AND (DATE(t.tgl_out) >='$tglAwal2' OR t.tgl_out IS NULL) AND (DATE(k.tgl_pulang) >='$tglAwal2' OR k.tgl_pulang IS NULL) 
AND t.kso_id = '".$wKso["id"]."' AND t.aktif=1) AS t1) AS t2";
								}else{
									$sql="SELECT IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.tarip),0) biayaRS,IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_kso),0) biayaKSO,
IFNULL(SUM(IF(DATEDIFF(t2.tgl_out,t2.tgl_in)<0,0,DATEDIFF(t2.tgl_out,t2.tgl_in))*t2.beban_pasien),0) biayaPx 
...
AND t.kso_id = '".$wKso["id"]."' AND t.aktif=1) AS t1) AS t2";
							...
							if ($tipe=="all"){
								...
								$sqlJual="SELECT SUM(t1.nJual) AS nJual,SUM(t1.nBahan) AS nBahan FROM
(SELECT p.HARGA_TOTAL AS nJual,
SUM(p.QTY_JUAL * IF(ap.NILAI_PAJAK=0,(ap.HARGA_BELI_SATUAN * (1-(ap.DISKON/100))),1.1 * (ap.HARGA_BELI_SATUAN * (1-(ap.DISKON/100))))) nBahan 
FROM $dbapotek.a_penjualan p 
LEFT JOIN $dbapotek.a_unit u ON p.RUANGAN=u.UNIT_ID INNER JOIN $dbapotek.a_penerimaan ap ON p.PENERIMAAN_ID=ap.ID 
WHERE u.kdunitfar='".$rwUnit["kode"]."' AND p.KSO_ID='".$rwKsoF["id"]."' AND $waktuF GROUP BY SHIFT,NO_PENJUALAN,TGL,p.KSO_ID,p.UNIT_ID,p.JENIS_PASIEN_ID,p.CARA_BAYAR) AS t1";
								//echo $sqlJual."<br>";
				...
				<tr style="font-weight:bold;">
                  <td style="border-left:1px solid #000000; border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000;">&nbsp;</td>
				  <td style="border-right:1px solid #000000; border-top:1px solid #000000; border-bottom:1px solid #000000;">&nbsp;TOTAL PENDAPATAN BILLING<?php echo $totalF; ?></td>
