...
<body>
	...
	<div id="container">
		<?php
			...
			$sql = "SELECT j.*, s.MA_ID, s.MA_KODE, s.MA_KODE_KP, s.MA_PARENT_KODE, s.MA_PARENT, s.MA_NAMA, bj.id, bj.kode, bj.parent_kode, bj.nama, 
						CONCAT(s.MA_KODE, '.',bj.kode) fullkode
					FROM jurnal j
					INNER JOIN ma_sak s ON s.MA_ID = j.FK_SAK
					INNER JOIN ak_ms_beban_jenis bj ON bj.id = j.MS_BEBAN_JENIS_ID
					WHERE j.FK_ID_POSTING = '{$id_posting}' 
					  AND j.TGL_ACT = '{$tanggal_posting}'
					  AND j.POSTING = 1
					ORDER BY j.D_K ASC, j.MS_BEBAN_JENIS_ID, s.MA_KODE";
			...
			if($query && mysql_num_rows($query) > 0){
				while($data = mysql_fetch_object($query)){
					$arrData[$data->D_K][$data->fullkode ."|".$data->MS_BEBAN_JENIS_ID] += ($data->D_K == 'D' ? $data->DEBIT : $data->KREDIT);
		...
		<table id="dataNilai" align="center">
			<tr>
				<td colspan="8" >PT. PELABUHAN INDONESIA I (PERSERO)</td>
			</tr>
			<tr>
				<td colspan="6"><?php echo $namaRS; ?></td>
				<td colspan="2" align="center">NO. 12/VII</td>
			</tr>
			...
			<tr>
				...
				<td colspan="3">Harap dibukukan mutasi atas transaksi sebagai berikut &nbsp; : </td>
			</tr>
			<tr>
			...
			<?php
				...
				if($dataKosong == false){
					...
					echo "<td align='left' colspan='3'></td>";
					echo "<td width='20' class='borderbottom bordertop'>Rp</td>";
					echo "<td align='right' class='borderright borderbottom bordertop'>".($DK == 'D' ? $subtotal : '-')."</td>";
					echo "<td width='20' class='borderbottom bordertop'>Rp</td>";
					echo "<td align='right' class='borderbottom bordertop'>".($DK == 'K' ? $subtotal : '-')."</td>";
					echo "</tr>"; */
				}
			?>
			<tr>
