...
<head>
	<meta charset="UTF-8">
	<title>Laporan Biaya Perawatan dan Piutang</title>
	<style type="text/css">
...
<body>
	<center>
		<span id="cetakID" style="float:left; position:fixed; width:100%; display:block; left:0px; bottom:0px; background:#0084FF; padding:5px;"><button id="btCetak" type="button" name="btCetak" onclick="cetak()">Cetak Laporan Biaya Perawatan dan Piutang</button></span>
	</center>
	<div id="container">	
		<section style="margin-bottom:20px;">
		<b><?=$pemkabRS;?><br>  <?=$namaRS;?><br> <?=$alamatRS;?><br><?=$tlpRS;?></b>
				</section>
		<header id="title">
			<center style="text-transform:uppercase; line-height:1.25em;">
				Laporan Biaya Perawatan dan Piutang<br />
				<?php echo $nKso; ?>
				<?php echo $Periode; ?><br />
				<br />
			</center>
			...
			<table class="isian" width="100%">
				...
				<?php
					$sql = "SELECT *
							FROM (SELECT
									...
									WHERE 0 = 0 {$waktu} {$fkso}
									  AND kp.tipe=0 AND pas.flag = '$flag') tx
					...
					$sql="SELECT 
							...
							WHERE k.pulang = 1 AND mp.flag = '$flag' {$waktu} /* AND DATE(k.tgl_pulang) = '$tgl' */
							UNION
							SELECT
					...
					if($query && mysql_affected_rows() > 0){
						...
						while ($rows=mysql_fetch_array($query)) {
							...
							$sbKSO = "SELECT SUM(ktd.nilai_terima) total
										FROM k_klaim_terima kt
										INNER JOIN k_klaim_terima_detail ktd ON ktd.klaim_terima_id = kt.id
										WHERE ktd.kunjungan_id <> 0 AND ktd.kunjungan_id = ".$rows["id"]."
										GROUP BY ktd.kunjungan_id";
							$qbKSO = mysql_query($sbKSO);
							$bayarKSO = 0;
							if(mysql_num_rows($qbKSO) > 0){
								$dbKSO = mysql_fetch_array($qbKSO);
								$bayarKSO = $dbKSO['total'];
							}
