...
if ($_REQUEST['act'] == "excell") {
    ...
    header('Content-Disposition: attachment; filename="REKAP ASESMEN PASIEN DIAGNOSA ' . $bulanData[$_REQUEST['bulan']] . ' ' . $_REQUEST['tahun'] . '.xlsx"');
}
...
<table border="1" class="table table-bordered table-sm">
    <tr>
        <th class="tengah" colspan="17">FORMULIR LAPORAN SC DAN PERSALINAN NORMAL</th>
    </tr>
    <tr>
        <?php getTd(17) ?>
    </tr>
    ...
    <tr>
        <th class="tengah" rowspan="2" colspan="2">TINDAKAN</th>
...
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_presentase_sc WHERE tanggal_pemantauan = '{$date}' and pilihan = 'sc'")->fetch_assoc();
                echo '<td>'.$banyak['banyak'].'</td>';
            }   
        ?>
    </tr>
    <tr>
        <td colspan="2">POST PARTUM</td>
        <?php 
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_presentase_sc WHERE tanggal_pemantauan = '{$date}' and pilihan = 'normal'")->fetch_assoc();
                echo '<td>'.$banyak['banyak'].'</td>';
            } 
        ?>
    </tr>
    <tr>
        ...
        <?php
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak1 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_presentase_sc WHERE tanggal_pemantauan = '{$date}' and pilihan = 'sc'")->fetch_assoc();
                $banyak2 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_presentase_sc WHERE tanggal_pemantauan = '{$date}' and pilihan = 'normal'")->fetch_assoc();
...
<table border="1" class="table table-bordered table-sm">
    ...
    <tr>
        <th colspan="<?= $inc == 31 ? 18 : 17 ?>">Bulan : <?= $bulan ?></th>
    </tr>
    <tr>
        <th class="tengah" rowspan="2" colspan="2">TINDAKAN</th>
        <th class="tengah" colspan="<?= $inc == 31 ? 16 : 15 ?>">Tanggal</th>
    </tr>
    <tr>
