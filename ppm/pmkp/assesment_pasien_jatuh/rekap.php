...
if ($_REQUEST['act'] == "excell") {
    ...
    header('Content-Disposition: attachment; filename="PENANGAPAN OBAT YG HARUS DI WASPADAI ' . $bulanData[$_REQUEST['bulan']] . ' ' . $_REQUEST['tahun'] . '.xlsx"');
}
...
<CENTER><H4>ASSESMENT AWAL PASIEN RESIKO JATUH</H4></CENTER>
...
BULAN : <?= $bulan ?>
    ...
    <tr>
        <th class="tengah" rowspan="2" colspan="2">JATUH</th>
...
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_assesment_pasien_jatuh WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'ya'")->fetch_assoc();
    ...
    <tr>
        ...
        <?php 
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_assesment_pasien_jatuh WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'tidak'")->fetch_assoc();
                echo '<td>'.$banyak['banyak'].'</td>';
            } 
        ?>
    </tr>
    <tr>
        ...
        <?php
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak1 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_assesment_pasien_jatuh WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'ya'")->fetch_assoc();
                $banyak2 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_assesment_pasien_jatuh  WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'tidak'")->fetch_assoc();
