...
<table border="1" class="table table-bordered table-sm">
    <tr>
        <th class="tengah" colspan="32">KEPATUHAN JAM VISITE DPJP RSU PRIMA</th>
    </tr>
    <tr>
        <th class="tengah" colspan="32">TAHUN <?= $tahun ?></th>
    </tr>
    <tr><?php getTd(32) ?></tr>
    <tr>
    ...
    <tr>
        <?php getTd(15, 2, false, 'SESUAI JAM VISITE') ?>
    </tr>
    <tr>
    ...
    FROM
        ppi_kepatuhan_jam_visite_dokter pa 
        LEFT JOIN rspelindo_billing.b_ms_pegawai pe ON pe.id = pa.id_dokter
    WHERE
        pa.id_ruangan = {$ruangan} AND pa.tanggal_pemantauan BETWEEN '{$dateAwal}' AND '{$dateAkhir}' GROUP BY pa.id_dokter,pa.id_ruangan");
    $no = 0;
...
$date = setDate($i,$bulanFilter,$tahun);
                $dokter = $konek->rawQuery("SELECT * FROM ppi_kepatuhan_jam_visite_dokter WHERE id_ruangan = {$ruangan} AND id_dokter = {$rows['id_dokter']} AND tanggal_pemantauan = '{$date}'")->fetch_assoc();
                if($dokter['tanggal_pemantauan'] == $date){
                    if($dokter['pilihan'] == 'ya'){
                        echo "<td>✓</td>";
                        echo "<td></td>";
                    }else{
                        echo "<td></td>";
