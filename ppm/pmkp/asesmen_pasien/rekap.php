...
<table border="1" class="table table-bordered table-sm">
    <tr>
        <th class="tengah" colspan="17">ANGKA PASIEN RAWAT INAP RSU ANDHIKA  YANG BELUM DI TEGAKAN DIAGNOSA SETELAH 3 HARI</th>
    </tr>
    <tr>
        <?php getTd(17) ?>
    </tr>
    ...
    <tr>
        <th class="tengah" rowspan="2" colspan="2">Sudah Terdiagnosa</th>
...
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_pasien_asesmen WHERE tanggal_pemantauan = '{$date}' AND pilihan = '>3h'")->fetch_assoc();
    ...
    <tr>
        ...
        <?php
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak1 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_pasien_asesmen WHERE tanggal_pemantauan = '{$date}' AND pilihan = '>3h'")->fetch_assoc();
                $banyak2 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_pasien_asesmen WHERE tanggal_pemantauan = '{$date}' AND pilihan = '<3h'")->fetch_assoc();
