...
if ($_REQUEST['act'] == "excell") {
    ...
    header('Content-Disposition: attachment; filename="REKAP KETEPATAN WAKTU ' . $bulanData[$_REQUEST['bulan']] . ' ' . $_REQUEST['tahun'] . '.xlsx"');
}
...
<table border="1" class="table table-bordered table-sm">
    <tr>
        <th class="tengah" colspan="17">KETEPATAN WAKTU LAPOR NILAI KRITIS LABORATORIUM RSU PRIMA</th>
    </tr>
    <tr>
        <?php getTd(17) ?>
    </tr>
    ...
    <tr>
        <th class="tengah" rowspan="2" colspan="2">KETEPATAN WAKTU</th>
...
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_ketepatan_waktu_lapor WHERE tanggal_pemantauan = '{$date}' AND pilihan = '<10'")->fetch_assoc();
    ...
    <tr>
        ...
        <?php
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak1 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_ketepatan_waktu_lapor WHERE tanggal_pemantauan = '{$date}' AND pilihan = '<10'")->fetch_assoc();
                $banyak2 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_ketepatan_waktu_lapor WHERE tanggal_pemantauan = '{$date}' AND pilihan = '>10'")->fetch_assoc();
