...
if ($_REQUEST['act'] == "excell") {
    ...
    header('Content-Disposition: attachment; filename="REKAP KELENGKAPAN RESEP OBAT ' . $bulanData[$_REQUEST['bulan']] . ' ' . $_REQUEST['tahun'] . '.xlsx"');
}
...
<table border="1" class="table table-bordered table-sm">
    <tr>
        <th class="tengah" colspan="17">KELENGKAPAN PENULISAN RESEP OBAT</th>
    </tr>
    <tr>
        <?php getTd(17) ?>
    </tr>
...
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_kelengkapan_resep_obat WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'ya'")->fetch_assoc();
    ...
    <tr>
        ...
        <?php 
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_kelengkapan_resep_obat WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'tidak'")->fetch_assoc();
                echo '<td>'.$banyak['banyak'].'</td>';
            } 
        ?>
    </tr>
    <tr>
        ...
        <?php
            for($i = 1; $i <= 15; $i++){
                $bulanFilter = sprintf("%02d", $_REQUEST['bulan']);
$date = setDate($i,$bulanFilter,$tahun);
                $banyak1 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_kelengkapan_resep_obat WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'tidak'")->fetch_assoc();
                $banyak2 = $konek->rawQuery("SELECT COUNT(id_pasien) AS banyak FROM ppi_kelengkapan_resep_obat WHERE tanggal_pemantauan = '{$date}' AND pilihan = 'ya'")->fetch_assoc();
