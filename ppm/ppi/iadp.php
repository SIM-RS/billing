...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        <div class="btn-group mr-2">
            <button type="button" class="btn btn-sm btn-outline-secondary" id="modalPemantauan" data-toggle="modal" data-target="#tambahModal">Tambah Pemantauan IADP</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="modal" data-target="#tambahIrsModal" onclick="isiTable('iadp-data-table','tampil_jumlah_iadp',$('#bulan-irs').val(),$('#tahun-irs').val());">Tambah Pasien IADP</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="modal" data-target="#exampleModal">Laporan Pemantauan iadp</button>
            <button type="button" id="lihat-grafik2" class="btn btn-sm btn-outline-secondary" data-toggle="modal" data-target="#graphicModal" value="utils.php|graphIadp">Grafik</button>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tambahModalLabel">Tambah Data Pemantauan iadp</h5>
                ...
                <form id="infus-form">
                    ...
                    <div class="row">
                        <div class="col-6">
                            ...
                            <div class="form-group">
                                <label for="">JH</label>
                                <select name="jh" id="jh" class="form-control">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-success" name="actInpus" id="actInpus" type="button" value="simpan-iadp">Save changes</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="iadp/Lapiadp.php" method="post" target="_blank">
                    <div class="form-group">
                        <label for="">Ruangan</label>
...
<div class="modal fade" id="tambahIrsModal" tabindex="-1" aria-labelledby="tambahIrsModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tambahIrsModalLabel">Tambah Pasien iadp</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="infeksi-operasi">
                    ...
                    <div class="form-group">
                        <label for="">Jumlah Pasien iadp</label>
                        <input type="text" name="irs" id="irs" class="form-control">
                    </div>
                </form>
                <div id="iadp-data-table"></div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.status == 200){
                    ...
                    $('#irs').val('');
                    isiTable('iadp-data-table','tampil_jumlah_iadp',$('#bulan-irs').val(),$('#tahun-irs').val(),$('#ruangan-irs').val());
                }else{
    ...
    $('#bulan-irs').change(function(){
        isiTable('iadp-data-table','tampil_jumlah_iadp',$('#bulan-irs').val(),$('#tahun-irs').val(),$('#ruangan-irs').val());
    });

    $('#tahun-irs').change(function(){
        isiTable('iadp-data-table','tampil_jumlah_iadp',$('#bulan-irs').val(),$('#tahun-irs').val(),$('#ruangan-irs').val());
    });

    $('#ruangan-irs').change(function(){
        isiTable('iadp-data-table','tampil_jumlah_iadp',$('#bulan-irs').val(),$('#tahun-irs').val(),$('#ruangan-irs').val());
    });
    ...
    $('#actInpus').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status == 200) {
                    alert('Berhasil '+(act == 'simpan-iadp' ? 'memasukan' : 'update')+' data');
                    document.getElementById('infus-form').reset();
                    $('#lihat-data').trigger('click');
                } else {
                    alert('Gagal '+(act == 'simpan-iadp' ? 'memasukan' : 'update')+' data');
                    document.getElementById('infus-form').reset();
                }
            }
        ...
        if (confirm("Yakin ingin menghapus data ini ?")) {
            $.ajax({
                ...
                success: function(data) {
                    if (data.status == 200) {
                        alert('Data berhasil dihapus');
                        isiTable('iadp-data-table','tampil_jumlah_iadp',$('#bulan-irs').val(),$('#tahun-irs').val());
                    } else {
                        alert('Data gagal dihapus');
                    }
                }
