...
$milik=$_REQUEST["milik"];if(($milik=="")||($milik=="0")) $kep=""; else $kep=" and a_p.KEPEMILIKAN_ID=$milik ";
$nmilik=$_REQUEST["nmilik"];
$jenis=$_REQUEST['jenis']; if($jenis=="") $jenis="0";
...
$worksheet->setColumn (5, 5, 25);//nama obat
$worksheet->setColumn (6, 6, 10);//expired
$worksheet->setColumn (7, 7, 5);//qty
$worksheet->setColumn (8, 9, 10);//sat - hrg_sat
$worksheet->setColumn (10, 10, 13);//subtotal
$worksheet->setColumn (11, 11, 6);//disk(%)
$worksheet->setColumn (12, 13, 13);//DPP - PPN
$worksheet->setColumn (14, 14, 14);//DPP+PPN
...
$worksheet->write($row, 6, "LAPORAN PENERIMAAN OBAT", $sheetTitleFormat);
$row += 1;
$worksheet->write($row, 6, "JENIS PENERIMAAN : ".$njenis, $sheetTitleFormat);
$row += 1;
$worksheet->write($row, 6, "KEPEMILIKAN : ".$nmilik, $sheetTitleFormat);
$row += 1;
...
$worksheet->write($row, $column+5, "Nama Obat", $columnTitleFormat);
$worksheet->write($row, $column+6, "Exp Date", $columnTitleFormat);
$worksheet->write($row, $column+7, "Qty", $columnTitleFormat);
$worksheet->write($row, $column+8, "Satuan", $columnTitleFormat);
$worksheet->write($row, $column+9, "Hrg Satuan", $columnTitleFormat);
$worksheet->write($row, $column+10, "Sub Total", $columnTitleFormat);
$worksheet->write($row, $column+11, "Disk (%)", $columnTitleFormat);
$worksheet->write($row, $column+12, "DPP", $columnTitleFormat);
$worksheet->write($row, $column+13, "PPN", $columnTitleFormat);
$worksheet->write($row, $column+14, "DPP+PPN", $columnTitleFormat);
...
$row++;
  ...
  $sql="SELECT a_p.*,DATE_FORMAT(a_p.TANGGAL,'%d/%m/%Y') AS tgl1,DATE_FORMAT(a_p.EXPIRED,'%d/%m/%Y') AS tgl2,
		...
		INNER JOIN a_pbf ON a_p.PBF_ID=a_pbf.PBF_ID 
		WHERE a_p.TIPE_TRANS=0 AND a_p.TANGGAL BETWEEN '$tgl_1' AND '$tgl_2'".$kep.$jns.$filter." 
		ORDER BY ".$sorting;
//echo $sql."<br>";
...
while ($rows=mysqli_fetch_array($rs)){
	if ($npbf!=$rows['PBF_NAMA']){
		if ($npbf!=""){
			$worksheet->write($row, 10, $tsubtot_perfaktur, $regularFormatRB);
			$worksheet->write($row, 12, $tdpp_perfaktur, $regularFormatRB);
			$worksheet->write($row, 13, $tppn_perfaktur, $regularFormatRB);
			$worksheet->write($row, 14, $tdpp_ppn_perfaktur, $regularFormatRB);
			$row++;
			...
			$worksheet->write($row, 9, "Total ".$npbf." : ", $sTotFormatR);
			$worksheet->write($row, 10, $tsubtot, $sTotFormatR);
			$worksheet->write($row, 11, "", $sTotFormatR);
			$worksheet->write($row, 12, $tdpp, $sTotFormatR);
			$worksheet->write($row, 13, $tppn, $sTotFormatR);
			$worksheet->write($row, 14, $tdpp_ppn, $sTotFormatR);
			$gtsubtot +=$tsubtot;
			$gtdpp +=$tdpp;
			$gtppn +=$tppn;
			$gtdpp_ppn +=$tdpp_ppn;
	...
	$worksheet->write($row, 5, $rows['OBAT_NAMA'], $regularFormatL);
	$worksheet->write($row, 6, $rows['tgl2'], $regularFormatC);
	$worksheet->write($row, 7, $rows['QTY_SATUAN'], $regularFormatC);
	$worksheet->write($row, 8, $rows['SATUAN'], $regularFormatC);
	$worksheet->write($row, 9, $rows['HARGA_BELI_SATUAN'], $regularFormatR);
	$worksheet->write($row, 10, $rows['subtotal'], $regularFormatR);
	$worksheet->write($row, 11, $rows['DISKON'], $regularFormatC);
	//$dpp=$rows['subtotal']-(($rows['subtotal']*$rows['DISKON'])/100);
	$dpp=$rows['dpp'];
	$worksheet->write($row, 12, $dpp, $regularFormatR);
	//$ppn=($rows['NILAI_PAJAK']>0)?($rows['subtotal']*10/100):0;
	$ppn=$rows['ppn'];
	$worksheet->write($row, 13, $ppn, $regularFormatR);
	$dpp_ppn=$dpp+$ppn;
	$worksheet->write($row, 14, $dpp_ppn, $regularFormatR);
...
$worksheet->write($row, 9, "GRAND TOTAL : ", $gsTotFormatR);
$worksheet->write($row, 10, $gtsubtot, $gsTotFormatR);
$worksheet->write($row, 11, "", $gsTotFormatR);
$worksheet->write($row, 12, $gtdpp, $gsTotFormatR);
$worksheet->write($row, 13, $gtppn, $gsTotFormatR);
$worksheet->write($row, 14, $gtdpp_ppn, $gsTotFormatR);
