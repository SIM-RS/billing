...
switch ($act){
 	...
 	case "save":
		...
		if ($cekEdit!=""){
			...
			$sql="update a_penerimaan set TGL_J_TEMPO='$tgl_j_tempo',HARI_J_TEMPO=$h_j_tempo,HARGA_BELI_TOTAL=$h_tot,DISKON_TOTAL=$disk_tot,NILAI_PAJAK=$ppn,MATERAI=$materai where NOTERIMA='$no_gdg' AND NOBUKTI='$no_faktur' AND TIPE_TRANS=0";
			//echo $sql."<br>";
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$NewdiskonRP=$arfvalue[13];
				$NewdiskonType=$arfvalue[17];
				$Newppn=$ppn;
				...
				/*tambahan untuk update average stok*/
				
				
				
				/*END tambahan untuk update average stok */
				...
				$sql="update a_penerimaan set TGL_J_TEMPO='$tgl_j_tempo',HARI_J_TEMPO=$h_j_tempo,EXPIRED='$tgl_exp',QTY_KEMASAN=$NewqtyKemasan,KEMASAN='$arfvalue[5]',HARGA_KEMASAN=$arfvalue[6],QTY_PER_KEMASAN=$NewqtyPerKemasan,QTY_SATUAN=$NewqtySatuan,QTY_STOK=$NewqtySatuan,SATUAN='$arfvalue[9]',HARGA_BELI_TOTAL=$h_tot,HARGA_BELI_SATUAN=$NewhargaSatuan,DISKON=$Newdiskon,DISKON_RP=$NewdiskonRP,DISKON_TYPE=$NewdiskonType,DISKON_TOTAL=$disk_tot,NILAI_PAJAK=$ppn,UPDT_H_NETTO=$updt_harga,JENIS=$arfvalue[14] where ID=$arfvalue[0]";
				//echo $sql."<br>";
				$rs1=mysqli_query($konek,$sql);
				
				
				
				$sql="update a_stok set nilai = nilai - $NilaiTransSelisih, qty_stok = qty_stok-$qtySatuanSelisih, rata2=nilai/qty_stok WHERE obat_id = $idOBAT and unit_id=$idunit and kepemilikan_id= $idKP";
				$rs1=mysqli_query($konek,$sql);
				
				$hrg = "select IFNULL(qty_stok,0) qty_stok ,IFNULL(nilai,0) nilai ,IFNULL(rata2,0) rata2 from a_stok where unit_id=$idunit AND obat_id=$idOBAT and kepemilikan_id= $idKP ";
				...
				$sql="UPDATE a_kartustok SET DEBET=DEBET-$qtySatuanSelisih,STOK_AFTER=STOK_AFTER-$qtySatuanSelisih,NILAI_TOTAL=NILAI_TOTAL-$NilaiTransSelisih, HARGA_SATUAN = $a_rata2 WHERE OBAT_ID=$idOBAT AND KEPEMILIKAN_ID=$idKP AND UNIT_ID=$idunit AND ID=$idKS";
				//echo $sql."<br>";
				$rs1=mysqli_query($konek,$sql);
				...
				$sql="update a_penerimaan set RATA2_UNIT_TERIMA = $a_rata2 where ID=$arfvalue[0]";
				//echo $sql."<br>";
				$rs1=mysqli_query($konek,$sql);
...
$sql="select distinct NOTERIMA,ap.NOBUKTI,ap.USER_ID_TERIMA,ap.TANGGAL,ap.KEPEMILIKAN_ID,date_format(ap.TANGGAL,'%d/%m/%Y') as tgl1,ap.HARGA_BELI_TOTAL,ap.DISKON_TOTAL,(ap.HARGA_BELI_TOTAL-ap.DISKON_TOTAL) as H_DISKON,(ap.HARGA_BELI_TOTAL-ap.DISKON_TOTAL)+ap.NILAI_PAJAK as TOTAL,ap.NILAI_PAJAK,ap.MATERAI,ap.UPDT_H_NETTO,ap.HARI_J_TEMPO,ap.JENIS,IF(ap.PBF_ID=0,ap.KET,a_pbf.PBF_NAMA) as PBF_NAMA,a_pbf.PBF_ID,no_po,k.NAMA,TERMASUK_PPN from a_penerimaan ap left join a_po on ap.FK_MINTA_ID=a_po.ID left join a_pbf on ap.PBF_ID=a_pbf.PBF_ID inner join a_kepemilikan k on ap.KEPEMILIKAN_ID=k.ID where ap.NOTERIMA='$no_gdg' and ap.NOBUKTI='$no_faktur'";
//echo $sql."<br>";
$rs=mysqli_query($konek,$sql);
...
function PPN_Checked_Unchecked(ob){
	...
	if (document.form1.chkItem.length){
		for (var i=0;i<document.form1.obat_id.length;i++){
			//if (document.form1.chkItem[i].checked){
				document.form1.chkItem[i].checked=true;
				HitungDiskon(i,2);
		...
		document.form1.chkItem.checked=true;
		HitungDiskon(0,2);
		//if (document.form1.chkItem.checked){
...
function HitungDiskon(i,j,k){
var tmp;
	//alert('diskon '+document.form1.plus_ppn.value);
	if (document.form1.obat_id.length){
		if (j==1){
			if (k==1){
	...
	<div id="listma" style="display:block">
	  ...
	  <table width="1150" border="0" cellpadding="1" cellspacing="0" class="txtinput">
        <tr> 
          ...
          <td width="110"><label title="Checked / Unchecked Untuk Mengubah Harga +PPN / Sudah Termasuk PPN">PPN (10%)&nbsp;<input type="checkbox" id="chkppn" name="chkppn" <?php if ($inc_ppn==0) echo " checked"; ?> onClick="PPN_Checked_Unchecked(this);" /></label></td>
          <td width="115">: 
        ...
        <tr> 
          ...
          <td>: <select name="materai" id="materai" onChange="Hitung_gTot();">
          		<option value="0" <?php if ($materai==0) echo " selected"; ?> class="txtinput">-</option>
              	<option value="3000" <?php if ($materai==3000) echo " selected"; ?> class="txtinput">3000</option>
              	<option value="6000" <?php if ($materai==6000) echo " selected"; ?> class="txtinput">6000</option>
            </select></td>
        </tr>
        <tr> 
          <td>No Faktur</td>
	  ...
	  ?>
        <tr class="itemtableA" onMouseOver="this.className='itemtableAMOver'" onMouseOut="this.className='itemtableA'"> 
          ...
          <td class="tdisi" align="center"><input name="diskon_type" type="hidden" value="<?php echo $rows['DISKON_TYPE']; ?>"><input name="diskon" type="text" readonly class="txtcenter" id="diskon" onKeyUp="HitungDiskon(<?php echo ($i-1); ?>,1,1);" lang="<?php echo $rows['DISKON']; ?>" value="<?php echo $rows['DISKON']; ?>" size="3" autocomplete="off"></td>
          <td class="tdisi" align="right"><input name="diskon_rp" type="text" readonly class="txtright" id="diskon_rp" onKeyUp="HitungDiskon(<?php echo ($i-1); ?>,2,1);" lang="<?php echo $rows['DISKON_RP']; ?>" value="<?php echo $rows['DISKON_RP']; ?>" size="7" autocomplete="off"></td>
        ...
        <script>
		HitungQtySatuan(<?php echo ($i-1); ?>);HitungSubTotal(<?php echo ($i-1); ?>);HitungDiskon(<?php echo ($i-1); ?>,2);
		</script>
        <?php 
	  }
	  mysqli_free_result($rs);
