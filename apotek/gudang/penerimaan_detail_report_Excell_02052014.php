...
$worksheet->setColumn (8, 8, 5);//qty
$worksheet->setColumn (9, 9, 10);//sat
$worksheet->setColumn (10, 10, 15);//hrg_sat
$worksheet->setColumn (11, 11, 13);//subtotal
$worksheet->setColumn (12, 12, 6);//disk(%)
$worksheet->setColumn (13, 14, 13);//DPP - PPN
$worksheet->setColumn (15, 15, 14);//DPP+PPN
...
$worksheet->write($row, $column+7, "Exp Date", $columnTitleFormat);
$worksheet->write($row, $column+8, "Qty", $columnTitleFormat);
$worksheet->write($row, $column+9, "Satuan", $columnTitleFormat);
$worksheet->write($row, $column+10, "Hrg Satuan", $columnTitleFormat);
$worksheet->write($row, $column+11, "Sub Total", $columnTitleFormat);
$worksheet->write($row, $column+12, "Disk (%)", $columnTitleFormat);
$worksheet->write($row, $column+13, "DPP", $columnTitleFormat);
$worksheet->write($row, $column+14, "PPN", $columnTitleFormat);
$worksheet->write($row, $column+15, "DPP+PPN", $columnTitleFormat);
...
$row++;
  ...
  $sql="SELECT a_p.*,DATE_FORMAT(a_p.TANGGAL,'%d/%m/%Y') AS tgl1,DATE_FORMAT(a_p.EXPIRED,'%d/%m/%Y') AS tgl2,
		...
		INNER JOIN a_pbf ON a_p.PBF_ID=a_pbf.PBF_ID LEFT JOIN (SELECT id, nama FROM a_sumber_dana) sd ON sd.id = a_p.SUMBER_DANA
...
while ($rows=mysqli_fetch_array($rs)){
	if ($npbf!=$rows['PBF_NAMA']){
		if ($npbf!=""){
			$worksheet->write($row, 11, $tsubtot_perfaktur, $regularFormatRB);
			$worksheet->write($row, 13, $tdpp_perfaktur, $regularFormatRB);
			$worksheet->write($row, 14, $tppn_perfaktur, $regularFormatRB);
			$worksheet->write($row, 15, $tdpp_ppn_perfaktur, $regularFormatRB);
			$row++;
			...
			$worksheet->write($row, 10, "Total ".$npbf." : ", $sTotFormatR);
			$worksheet->write($row, 11, $tsubtot, $sTotFormatR);
			$worksheet->write($row, 12, "", $sTotFormatR);
			$worksheet->write($row, 13, $tdpp, $sTotFormatR);
			$worksheet->write($row, 14, $tppn, $sTotFormatR);
			$worksheet->write($row, 15, $tdpp_ppn, $sTotFormatR);
			$gtsubtot +=$tsubtot;
			$gtdpp +=$tdpp;
			$gtppn +=$tppn;
			$gtdpp_ppn +=$tdpp_ppn;
	...
	$worksheet->write($row, 7, $rows['tgl2'], $regularFormatC);
	$worksheet->write($row, 8, $rows['QTY_SATUAN'], $regularFormatC);
	$worksheet->write($row, 9, $rows['SATUAN'], $regularFormatC);
	$worksheet->write($row, 10, $rows['HARGA_BELI_SATUAN'], $regularFormatR);
	$worksheet->write($row, 11, $rows['subtotal'], $regularFormatR);
	$worksheet->write($row, 12, $rows['DISKON'], $regularFormatC);
	//$dpp=$rows['subtotal']-(($rows['subtotal']*$rows['DISKON'])/100);
	$dpp=$rows['dpp'];
	$worksheet->write($row, 13, $dpp, $regularFormatR);
	//$ppn=($rows['NILAI_PAJAK']>0)?($rows['subtotal']*10/100):0;
	$ppn=$rows['ppn'];
	$worksheet->write($row, 14, $ppn, $regularFormatR);
	$dpp_ppn=$dpp+$ppn;
	$worksheet->write($row, 15, $dpp_ppn, $regularFormatR);
...
$worksheet->write($row, 10, "GRAND TOTAL : ", $gsTotFormatR);
$worksheet->write($row, 11, $gtsubtot, $gsTotFormatR);
$worksheet->write($row, 12, "", $gsTotFormatR);
$worksheet->write($row, 13, $gtdpp, $gsTotFormatR);
$worksheet->write($row, 14, $gtppn, $gsTotFormatR);
$worksheet->write($row, 15, $gtdpp_ppn, $gsTotFormatR);
