...
$bln_po = mysqli_real_escape_string($konek,$_REQUEST["bln_po"]);
if ($bln_po=="") $bln_po=(substr($tgl,3,1)=="0")?substr($tgl,4,1):substr($tgl,3,2);
//$th_po=$_REQUEST['th_po'];
$th_po = mysqli_real_escape_string($konek,$_REQUEST["th_po"]);
if ($th_po=="") $th_po=substr($tgl,6,4);
$th_skrg=substr($tgl,6,4);
//$no_gdg=$_REQUEST['no_gdg'];
$no_gdg = mysqli_real_escape_string($konek,$_REQUEST["no_gdg"]);
//$no_po=$_REQUEST['no_po'];
$no_po = mysqli_real_escape_string($konek,$_REQUEST["no_po"]);
//$kp_nama=$_REQUEST['kp_nama'];
$kp_nama = mysqli_real_escape_string($konek,$_REQUEST["kp_nama"]);
//$kp_id=$_REQUEST['kepemilikan_id'];
$kp_id = mysqli_real_escape_string($konek,$_REQUEST["kepemilikan_id"]);
//$no_faktur=$_REQUEST['no_faktur'];
$no_faktur = mysqli_real_escape_string($konek,$_REQUEST["no_faktur"]);
//$h_j_tempo=$_REQUEST['h_j_tempo'];
$h_j_tempo = mysqli_real_escape_string($konek,$_REQUEST["h_j_tempo"]);
//$tgl_gdg=$_REQUEST['tgl_gdg'];
$tgl_gdg = mysqli_real_escape_string($konek,$_REQUEST["tgl_gdg"]);
...
$h_tot = mysqli_real_escape_string($konek,$_REQUEST["h_tot"]);
//$disk_tot=$_REQUEST['disk_tot'];
$disk_tot = mysqli_real_escape_string($konek,$_REQUEST["disk_tot"]);
//$h_diskon=$_REQUEST['h_diskon'];
$h_diskon = mysqli_real_escape_string($konek,$_REQUEST["h_diskon"]);
//$ppn=$_REQUEST['ppn'];
$ppn = mysqli_real_escape_string($konek,$_REQUEST["ppn"]);
//$materai=$_REQUEST['materai'];
$materai = mysqli_real_escape_string($konek,$_REQUEST["materai"]);
//$total=$_REQUEST['total'];
$total = mysqli_real_escape_string($konek,$_REQUEST["total"]);
//$g_total=$_REQUEST['g_total'];
$g_total = mysqli_real_escape_string($konek,$_REQUEST["g_total"]);
//$pbf_id=$_REQUEST['pbf_id'];
$pbf_id = mysqli_real_escape_string($konek,$_REQUEST["pbf_id"]);
//$updt_harga=$_REQUEST['updt_harga'];
$updt_harga = mysqli_real_escape_string($konek,$_REQUEST["updt_harga"]);
//$sumber_dana=$_REQUEST['sumber_dana'];
$sumber_dana = mysqli_real_escape_string($konek,$_REQUEST["sumber_dana"]);
...
$sUP = "UPDATE a_sumber_dana sd
		...
		WHERE admin = 0 AND berlaku = 1 AND aktif = 1";
...
switch ($act){
 	case "save":
		...
		$aharga=mysqli_real_escape_string($konek,$_REQUEST['aharga']);
		//$idPemeriksa=$_REQUEST['idPemeriksa'];
		$idPemeriksa=mysqli_real_escape_string($konek,$_REQUEST['idPemeriksa']);
		//$fdata=$_REQUEST['fdata'];
		...
		if ($tmpuser!=$iduser){
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$hrg = "select IFNULL(qty_stok,0) qty_stok ,IFNULL(nilai,0) nilai ,IFNULL(rata2,0) rata2 from a_stok where unit_id=$idunit AND obat_id=".$arfvalue[1]." ";
				$hrg1 = mysqli_query($konek,$hrg);
				$hrg2 = mysqli_fetch_array($hrg1);
				$a_rata2 = $hrg2['rata2'];
				$a_qtystok = $hrg2['qty_stok'];
				$a_nilai = $hrg2['nilai'];
				
				$nilai = ($arfvalue[8]*$arfvalue[10]) * (1-($arfvalue[12]/100)); 
				$qtystok = $arfvalue[8];
				
				$rata2 = ($nilai + $a_nilai) / ($a_qtystok + $qtystok);
				//QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100))
	
				$sql="insert into ".$dbapotek.".a_penerimaan(OBAT_ID,FK_MINTA_ID,PBF_ID,UNIT_ID_TERIMA,KEPEMILIKAN_ID,USER_ID_TERIMA,NOKIRIM,NOTERIMA,NOBUKTI,TANGGAL_ACT,TANGGAL,TGL_J_TEMPO,HARI_J_TEMPO,BATCH,EXPIRED,QTY_KEMASAN,KEMASAN,HARGA_KEMASAN,QTY_PER_KEMASAN,QTY_SATUAN,SATUAN,QTY_STOK,HARGA_BELI_TOTAL,HARGA_BELI_SATUAN,DISKON,DISKON_RP,DISKON_TYPE,DISKON_TOTAL,NILAI_PAJAK,MATERAI,UPDT_H_NETTO,JENIS,TIPE_TRANS,STATUS,SUMBER_DANA,RATA2_UNIT_KIRIM,RATA2_UNIT_TERIMA) values($arfvalue[1],$arfvalue[0],$pbf_id,$idunit,$arfvalue[2],$iduser,'$no_faktur','$no_gdg','$no_faktur',NOW(),'$tgl2','$tgl_j_tempo',$h_j_tempo,'','$tgl_exp',$arfvalue[4],'$arfvalue[5]',$arfvalue[6],$arfvalue[7],$arfvalue[8],'$arfvalue[9]',$arfvalue[8],$h_tot,$arfvalue[10],$arfvalue[12],$arfvalue[13],$arfvalue[16],$disk_tot,$ppn,$materai,$updt_harga,$arfvalue[14],0,1,$sumber_dana,$arfvalue[10],$rata2)";
				//echo $sql."<br>";		
				...
				$hrg_baru=($arfvalue[11])/$arfvalue[8];
				if ($ppn!="0"){
				...
				if (mysqli_num_rows($rs1)==0){					
					...
					$dataO = mysqli_fetch_array($rs1);
						if(  (($dataO['HARGA_BELI_SATUAN']) < $hrg_baru) && ($dataO['lock_harga']==0) ){
						
							$sql="UPDATE ".$dbapotek.".a_harga SET HARGA_BELI_SATUAN=$hrg_baru,PROFIT=0,HARGA_JUAL_SATUAN=$hrg_baru WHERE OBAT_ID=$arfvalue[1] AND KEPEMILIKAN_ID=$arfvalue[2]";
							$rs1=mysqli_query($konek,$sql);
...
if ($act!="save"){
	...
	$sql="select distinct NO_PO,date_format(TANGGAL,'%d/%m/%Y') as tgl1,HARGA_BELI_TOTAL,DISKON_TOTAL,(HARGA_BELI_TOTAL-DISKON_TOTAL) as H_DISKON,(HARGA_BELI_TOTAL-DISKON_TOTAL)+NILAI_PAJAK as TOTAL,NILAI_PAJAK,UPDT_H_NETTO,a_pbf.PBF_ID,PBF_NAMA,no_spph,k.NAMA,k.ID,TERMASUK_PPN from a_po left join a_spph on a_po.FK_SPPH_ID=a_spph.spph_id 
	...
	if ($rows=mysqli_fetch_array($rs)){
		...
		$sql="SELECT * FROM a_pemeriksa WHERE /* tipe='$pemeriksa_tipe' and */ aktif = 1";
		$rs1=mysqli_query($konek,$sql);
		$pemeriksa="";
		while ($rw1=mysqli_fetch_array($rs1)){
			$pemeriksa .='<label style="cursor:pointer;"><input type="checkbox" name="pemeriksa" style="margin-right:10px;" lang="'.$rw1["pemeriksa"].'" value="'.$rw1["id"].'" />'.$rw1["pemeriksa"]."</label><br>";
		}
	}
}else{
...
function Hitung_gTot(){
	document.form1.g_total.value=document.form1.total.value*1+document.form1.materai.value*1;
}

function PPN_Checked_Unchecked(ob){
	//alert(ob.checked);
	if (ob.checked){
		document.form1.plus_ppn.value="0";
	}else{
		document.form1.plus_ppn.value="1";
	}
	...
	if (document.form1.chkItem.length){
		for (var i=0;i<document.form1.obat_id.length;i++){
			//if (document.form1.chkItem[i].checked){
				HitungDiskon(i,2);
				/*if (document.form1.diskon_type[i].value=="1"){
					HitungDiskon(i,2);
				}else{
					HitungDiskon(i,1);
				}*/
		...
		if (document.form1.chkItem.checked){
			HitungDiskon(0,2);
			/*if (document.form1.diskon_type.value=="1"){
				HitungDiskon(0,2);
			}else{
				HitungDiskon(0,1);
			}*/
...
var tmp;
	...
	if (document.form1.chkItem.length){
		if (j==1){
			document.form1.diskon_type[i].value="0";
			tmp=((document.form1.sub_tot[i].value*1)*(document.form1.diskon[i].value*1))/100;
			document.form1.diskon_rp[i].value=tmp.toFixed(2)*1;
		}else{
			document.form1.diskon_type[i].value="1";
			if (document.form1.diskon_rp[i].value*1 == 0 && document.form1.diskon[i].value*1 >0){
				tmp=((document.form1.sub_tot[i].value*1)*(document.form1.diskon[i].value*1))/100;
				document.form1.diskon_rp[i].value=tmp.toFixed(2)*1;
			}else{
		...
		if (document.form1.plus_ppn.value=="1"){
			...
			document.form1.dpp_ppn[i].value=tmp;
			tmp=tmp*100/110;
		...
		if (j==1){
			document.form1.diskon_type.value="0";
			tmp=((document.form1.sub_tot.value*1)*(document.form1.diskon.value*1))/100;
			document.form1.diskon_rp.value=tmp.toFixed(2)*1;
		}else{
			document.form1.diskon_type.value="1";
			if (document.form1.diskon_rp.value*1 == 0 && document.form1.diskon.value*1 >0){
				tmp=((document.form1.sub_tot.value*1)*(document.form1.diskon.value*1))/100;
				document.form1.diskon_rp.value=tmp.toFixed(2)*1;
			}else{
		...
		if (document.form1.plus_ppn.value=="1"){
			...
			document.form1.dpp_ppn.value=tmp;
			tmp=tmp*100/110;
...
<!-- Print Out -->
	...
	<link rel="stylesheet" href="../theme/print.css" type="text/css" />
	  ...
	  <table width="99%" border="0" cellpadding="1" cellspacing="0" class="txtinput">
        ...
        <td>: <?php echo number_format($materai,2,",","."); ?></td>
        </tr>
        <tr> 
          <td>No Faktur</td>
    ...
    <table id="tblpenerimaan" width="99%" border="0" cellpadding="1" cellspacing="0">
	  ...
	  $i=0;
		...
		if ($ppn>0){
			$dpp=$rows['subtotal']-$rows['DISKON_RP'];
			$dpp_ppn=$dpp+($dpp/10);
		}else{
			$dpp_ppn=$rows['subtotal']-$rows['DISKON_RP'];
			$dpp=$dpp_ppn*100/110;
		}
	  ?>
	...
	<div id="listma" style="display:block">
	  ...
	  <table width="1150" border="0" cellpadding="1" cellspacing="0" class="txtinput">
        <tr> 
          ...
          <td width="85"><label title="Checked / Unchecked Untuk Mengubah Harga +PPN / Sudah Termasuk PPN">PPN (10%)&nbsp;<input type="checkbox" id="chkppn" name="chkppn" checked onClick="PPN_Checked_Unchecked(this);" /></label></td>
          <td width="115">: 
        ...
        <tr> 
          ...
          <td>Materai</td>
          <td>: <select name="materai" id="materai" onChange="Hitung_gTot();">
          		<option value="0" class="txtinput">-</option>
              	<option value="3000" class="txtinput">3000</option>
              	<option value="6000" class="txtinput">6000</option>
            </select></td>
        </tr>
        <tr>
          <td colspan="2">
		  <?php
			$qry="SELECT DISTINCT no_po,PBF_NAMA,k.NAMA FROM a_po 
			INNER JOIN a_pbf ON a_po.PBF_ID=a_pbf.PBF_ID 
			LEFT JOIN a_spph sp ON a_po.FK_SPPH_ID=sp.spph_id 
			INNER JOIN a_kepemilikan k ON a_po.KEPEMILIKAN_ID=k.ID 
			WHERE YEAR(TANGGAL)=$th_po AND MONTH(TANGGAL)=$bln_po AND a_po.STATUS=0 ORDER BY no_po";
		  ?>
          ...
          <td>GRAND TOTAL</td>
          <td>: 
            <input name="g_total" type="text" class="txtright" id="g_total" value="<?php echo $g_total; ?>" size="15" readonly="true"></td>
        </tr>
        <tr> 
          <td>No Faktur</td>
          <td>: 
        ...
        <tr>
          ...
          <input type="button" name="plhPemeriksa" value=" ... " class="txtcenter" style="cursor:pointer" title="Klik Untuk Memilih Petugas Pemeriksa" onClick="ShowPop();" />          </td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td>Sumber Dana</td>
          <td>: <select name="sumber_dana" id="sumber_dana">
          		<option value="0" class="txtinput">-</option>
          		<?php 
				$sql="SELECT * FROM a_sumber_dana WHERE aktif=1 AND berlaku = 1";
				$rsdana=mysqli_query($konek,$sql);
				while ($rwdana=mysqli_fetch_array($rsdana)){
				?>
          		<option value="<?php echo $rwdana["id"]; ?>" class="txtinput"><?php echo $rwdana["nama"]." ".$rwdana["tahun"]; ?></option>
                <?php 
				}
				?>
            </select></td>
	  ...
	  ?>
        <tr class="itemtableA" onMouseOver="this.className='itemtableAMOver'" onMouseOut="this.className='itemtableA'"> 
          ...
          <td class="tdisi" align="center"><input name="diskon_type" type="hidden" value="<?php echo $rows['DISKON_TYPE']; ?>"><input name="diskon" type="text" class="txtcenter" id="diskon" onKeyUp="HitungDiskon(<?php echo ($i-1); ?>,1);" value="<?php echo $rows['DISKON']; ?>" size="3" autocomplete="off"></td>
        ...
        <script>
		HitungQtySatuan(<?php echo ($i-1); ?>);HitungSubTotal(<?php echo ($i-1); ?>);HitungDiskon(<?php echo ($i-1); ?>,<?php echo $rows['DISKON_TYPE']+1; ?>);
		</script>
        <?php 
	  }
	  mysqli_free_result($rs);
