 	...
 	case "save":
		...
		if ($tmpuser!=$iduser){
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$sql="insert into ".$dbapotek.".a_penerimaan(OBAT_ID,FK_MINTA_ID,PBF_ID,UNIT_ID_TERIMA,KEPEMILIKAN_ID,USER_ID_TERIMA,NOKIRIM,NOTERIMA,NOBUKTI,TANGGAL_ACT,TANGGAL,TGL_J_TEMPO,HARI_J_TEMPO,BATCH,EXPIRED,QTY_KEMASAN,KEMASAN,HARGA_KEMASAN,QTY_PER_KEMASAN,QTY_SATUAN,SATUAN,QTY_STOK,HARGA_BELI_TOTAL,HARGA_BELI_SATUAN,DISKON,DISKON_RP,DISKON_TYPE,DISKON_TOTAL,NILAI_PAJAK,MATERAI,UPDT_H_NETTO,JENIS,TIPE_TRANS,STATUS,SUMBER_DANA) values($arfvalue[1],$arfvalue[0],$pbf_id,$idunit,$arfvalue[2],$iduser,'$no_faktur','$no_gdg','$no_faktur',NOW(),'$tgl2','$tgl_j_tempo',$h_j_tempo,'','$tgl_exp',$arfvalue[4],'$arfvalue[5]',$arfvalue[6],$arfvalue[7],$arfvalue[8],'$arfvalue[9]',$arfvalue[8],$h_tot,$arfvalue[10],$arfvalue[12],$arfvalue[13],$arfvalue[16],$disk_tot,$ppn,$materai,$updt_harga,$arfvalue[14],0,1,$sumber_dana)";
				//echo $sql."<br>";
				$rs1=mysqli_query($konek,$sql);
				$status=0;
