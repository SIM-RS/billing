 	...
 	case "save":
		...
		if ($tmpuser!=$iduser){
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$sql="insert into a_penerimaan(OBAT_ID,FK_MINTA_ID,PBF_ID,UNIT_ID_TERIMA,KEPEMILIKAN_ID,USER_ID_TERIMA,NOKIRIM,NOTERIMA,NOBUKTI,TANGGAL_ACT,TANGGAL,TGL_J_TEMPO,HARI_J_TEMPO,BATCH,EXPIRED,QTY_KEMASAN,KEMASAN,HARGA_KEMASAN,QTY_PER_KEMASAN,QTY_SATUAN,SATUAN,QTY_STOK,HARGA_BELI_TOTAL,HARGA_BELI_SATUAN,DISKON,DISKON_TOTAL,KET,NILAI_PAJAK,UPDT_H_NETTO,JENIS,TIPE_TRANS,STATUS) values($arfvalue[0],0,$pbf_id,$idunit,$kp_id,$iduser,'$no_faktur','$no_gdg','$no_faktur',NOW(),'$tgl2','$tgl_j_tempo',$h_j_tempo,'','$tgl_exp',$arfvalue[3],'$arfvalue[4]',$arfvalue[5],$arfvalue[6],$arfvalue[7],'$arfvalue[8]',$arfvalue[7],$h_tot,$arfvalue[9],$arfvalue[11],$disk_tot,'$asal_perolehan',$ppn,$updt_harga,$jenis,0,1)";
				//echo $sql."<br>";
				$rs1=mysqli_query($konek,$sql);
				$status=0;
			}
...
$qry="SELECT SATUAN  FROM a_obat INNER JOIN a_satuan ON a_obat.obat_satuan_kecil = a_satuan.SATUAN GROUP BY SATUAN";
