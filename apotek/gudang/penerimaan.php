...
//$tipe=$_REQUEST['tipe'];
$tipe = mysqli_real_escape_string($konek,$_REQUEST["tipe"]);
if ($tipe=="") $tipe=1;
if ($tipe==2 || $tipe==4){
	 ...
	 //echo $sql."<br>";
	  ...
	  ?>
        <tr class="itemtable" onMouseOver="this.className='itemtableMOver'" onMouseOut="this.className='itemtable'"> 
          ...
          <td class="tdisi" align="center"><span id="nobukti_<?php echo $i; ?>" lang="<?php echo $rows['NOTERIMA'].'|'.$rows['NO_PO']; ?>" style="cursor:pointer;text-decoration:underline;color:#CC3300" title="Klik Untuk Mengubah No Faktur" onClick="NoFakturKlik(this);"><?php echo $rows['NOBUKTI']; ?></span></td>
          <td class="tdisi"><?php echo $rows['PBF_NAMA']; ?></td>
          <td class="tdisi"><?php echo $rows['NAMA']; ?></td>
          <td width="30" class="tdisi"><img src="../icon/lihat.gif" border="0" width="16" height="16" align="absmiddle" class="proses" title="Klik Untuk Melihat Detail Penerimaan" onClick="location='?f=penerimaan_detail&no_gdg=<?php echo $rows['NOTERIMA']; ?>&no_faktur='+document.getElementById('nobukti_<?php echo $i; ?>').innerHTML+'&tipe=<?php echo $tipe; ?>&bulan=<?php echo $bulan; ?>&ta=<?php echo $ta; ?>'"></td>
          <td width="30" class="tdisi"><img src="../icon/edit.gif" border="0" width="16" height="16" align="absmiddle" class="proses" title="Klik Untuk Mengubah Data Penerimaan PBF" onClick="location='?f=<?php echo $lnkedit; ?>&no_gdg=<?php echo $rows['NOTERIMA']; ?>&no_faktur=<?php echo $rows['NOBUKTI']; ?>&tipe=<?php echo $tipe; ?>&bulan=<?php echo $bulan; ?>&ta=<?php echo $ta; ?>'"></td> <!--  location='?f=<?php echo $lnkedit; ?>&no_gdg=<?php echo $rows['NOTERIMA']; ?>&no_faktur=<?php echo $rows['NOBUKTI']; ?>&tipe=<?php echo $tipe; ?>&bulan=<?php echo $bulan; ?>&ta=<?php echo $ta; ?>'  -->
        </tr>
        <?php 
	  }
...
function test(){
    alert('Maaf system masih dalam proses perbaikan, silahkan hubungi operator untuk edit penerimaan');
}

function NoFakturKlik(p){
	//alert(p.id);
	var tmp=p.lang.split("|");
	var nobukti=p.innerHTML;
	var nofaktur = prompt('Masukkan No Faktur Yang Benar', nobukti);
	if (nofaktur!=null && trim(nofaktur)!=nobukti && trim(nofaktur)!=''){
		//alert(nofaktur);
		Request("utils.php?act=UpdateFaktur&noFaktur="+trim(nofaktur)+"&noFakturLama="+nobukti+"&noGD="+tmp[0]+"&noPO="+tmp[1],p.id,"","GET","NOLOAD");
	}
}
</script>
</html>
