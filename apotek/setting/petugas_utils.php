...
convert_var($page,$filter,$filter,$userId);
...
switch(strtolower($act)){
	case 'tambah':
		...
		$rsPeg = mysqli_query($konek,$sqlPegId);
		$rwPeg = mysqli_fetch_array($rsPeg);
		$idPeg = $rwPeg['id'];
		$sqlGrp = "INSERT INTO b_ms_group_petugas (ms_group_id,ms_pegawai_id) values('".$_REQUEST['idgroup']."','".$idPeg."')";
		$rsGrp=mysqli_query($konek,$sqlGrp);
		break;
	...
	case 'simpan':
		...
		$rs = mysqli_query($konek,$sqlGrpUp);
		break;
