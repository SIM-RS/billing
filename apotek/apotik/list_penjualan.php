...
convert_var($minta_id,$jns_pasien,$statuspasien);
...
convert_var($jns_p,$statusp,$tgl_d,$tgl_s);
...
<iframe height="72" width="130" name="sort"
	...
	<div id="view" style="display:block"> 
      <p><span class="jdltable">DAFTAR PENJUALAN 
        <select name="idunit" id="idunit" class="txtinput" onchange="location='?f=../apotik/list_penjualan.php&idunit='+this.value+'&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value+'&jns_pasien='+jns_pasien.value">
      ...
      <table width="48%" border="0" align="center" cellpadding="1" cellspacing="0" class="txtinput">
        ...
        <tr>
          ...
          <td>:
			...
			?>
              <option value="0" class="txtinput">All Pasien</option>
              <option value="1" class="txtinput"<?php if ($statuspasien=="1") {echo "selected";$sp="RAWAT JALAN";}?>>Rawat Jalan</option>
              <option value="2" class="txtinput"<?php if ($statuspasien=="2") {echo "selected";$sp="Inst.IGD";}?>>Inst. Gawat Darurat</option>
              <option value="3" class="txtinput"<?php if ($statuspasien=="3") {echo "selected";$sp="RAWAT INAP";}?>>Rawat Inap</option>
            </select></td>
          <td >&nbsp;</td>
        </tr>
      </table>
      <table width="100%" align="center" border="0" cellpadding="1" cellspacing="0">
        <tr class="headtable"> 
          ...
          <td id="TGL_ACT" width="65" class="tblheader" onclick="ifPop.CallFr(this);">Tgl 
            Trans</td>
          <td id="NO_PENJUALAN" width="50" class="tblheader" onclick="ifPop.CallFr(this);">No 
            Penj</td>
          <td id="NO_RESEP" width="53" class="tblheader" onclick="ifPop.CallFr(this);">No 
            Resep </td>
          ...
          <td id="SUM_SUB_TOTAL" width="80" class="tblheader" onclick="ifPop.CallFr(this);">H. Total </td>
          <td id="SUM_SUB_TOTAL" width="80" class="tblheader" onclick="ifPop.CallFr(this);">PPN</td>
          <td id="SUM_SUB_TOTAL" width="80" class="tblheader" onclick="ifPop.CallFr(this);">H. Total+PPN </td>
	 ...
	 if ($idunit=="0"){
		...
		a_penjualan.UTANG,SUM(FLOOR((a_penjualan.QTY_JUAL)*a_penjualan.HARGA_NETTO)) AS HARGA_NETTO,
		...
		ROUND((a_penjualan.PPN/100) * SUM(a_penjualan.QTY_JUAL* a_penjualan.HARGA_SATUAN)) AS PPN_NILAI,
	...
	//echo $sql."<br>";
	  ...
	  ?>
        <tr class="itemtable" onmouseover="this.className='itemtableMOver'" onmouseout="this.className='itemtable'"> 
          ...
          <td class="tdisi" align="center">&nbsp;<?php echo $rows['KSO']; ?></td>
          <td class="tdisi" align="center">&nbsp;<?php echo $rows['UNIT_NAME']; ?></td>
          <td class="tdisi" align="center"><?php if($rows['SHIFT']==1){echo 'Pagi';}elseif($rows['SHIFT']==2){echo 'Siang';}else{echo 'Malam';} ?></td>
          ...
          <td class="tdisi" align="right"><?php echo number_format($rows['HARGA_TOTAL']+$rows['PPN_NILAI'],0,",","."); ?></td>
          <td<?php if ($tCaraBayar<>4) echo ' colspan="2" width="36"'; else echo ' width="18"';?> class="tdisi" align="center"><a href="../newreport/kwi_jual.php?no_penjualan=<?php echo $rows['NO_PENJUALAN'];?>&sunit=<?php echo $rows['UNIT_ID']; ?>&no_pasien=<?php echo $rows['NO_PASIEN']; ?>&tgl=<?php echo $rows['TGL']; ?>&iduser_jual=<?php echo $rows['USER_ID'];?><?php if ($jRetur==0) echo '&viewdel=true';?>" onclick="NewWindow(this.href,'name','560','500','yes');return false"><img src="../icon/lihat.gif" border="0" width="16" height="16" align="absmiddle" title="Klik Untuk Melihat Detail Penjualan"></a></td>
          <?php if ($tCaraBayar==4){?>
          <td class="tdisi" align="center" width="18"><a href="../transaksi/utang_kredit.php?no_penjualan=<?php echo $rows['NO_PENJUALAN'];?>&sunit=<?php echo $idunit; ?>&no_pasien=<?php echo $rows['NO_PASIEN']; ?>&tgl=<?php echo $rows['TGL']; ?>&iuser=<?php echo $iduser; ?>&ishift=<?php echo $shift; ?>" onclick="NewWindow(this.href,'bayar','1000','500','yes');return false"><img src="../icon/edit.gif" border="0" width="16" height="16" align="absmiddle" title="Klik Untuk Mencatat Pembayaran Resep"></a></td>
          <?php }?>
        </tr>
        <?php
	 //$hartot=$hartot+$rows['HARGA_TOTAL'];
	  ...
	  $sql2="select sum(t1.HARGA_TOTAL+t1.PPN_NILAI) as HARTOT from (".$sql.")as t1";
	  ...
	  $sql3="select sum(t1.HARGA_NETTO) as TOT_NETTO from (".$sql.")as t1";
	  $hs3=mysqli_query($konek,$sql3);
	  $totnetto=0;
	  $show3=mysqli_fetch_array($hs3);
	  $totnetto=$show3['TOT_NETTO'];
	  
	  $sql4="select sum(t1.HARGA_TOTAL) as HARGA_TOTAL from (".$sql.")as t1";
	  $hs4=mysqli_query($konek,$sql4);
	  $totharga=0;
	  $show4=mysqli_fetch_array($hs4);
	  $totharga=$show4['HARGA_TOTAL'];
	  
	  $sql5="select sum(t1.PPN_NILAI) as TOT_PPN from (".$sql.")as t1";
	  $hs5=mysqli_query($konek,$sql5);
	  $totppn=0;
	  $show5=mysqli_fetch_array($hs5);
	  $totppn=$show5['TOT_PPN'];
	  ...
	  ?>
        <tr class="itemtable"> 
          ...
          <td colspan="6" align="right" class="tdisi">Jumlah Total &nbsp; </td>
          <td class="tdisi" align="right"><?php echo number_format($totnetto,0,",","."); ?></td>
          <td class="tdisi" align="right"><?php echo number_format($totharga,0,",","."); ?></td>
          <td class="tdisi" align="right"><?php echo number_format($totppn,0,",","."); ?></td>
          <td class="tdisi" align="right">&nbsp;<? echo number_format($hartot,0,",","."); ?></td>
          <td colspan="2" width="36" class="tdisi">&nbsp;</td>
        </tr>
      </table>
	</div>
	...
	<p align="center"><span class="jdltable">DAFTAR PENJUALAN <? echo strtoupper($unit) ?></span>  
      ...
      <table width="100%" align="center" border="0" cellpadding="1" cellspacing="0">
        <tr class="headtable"> 
          ...
          <td id="SUM_SUB_TOTAL" width="80" class="tblheader">H. total </td>
          <td id="SUM_SUB_TOTAL" width="80" class="tblheader">PPN</td>
	  ...
	  ?>
        <tr class="itemtable" onmouseover="this.className='itemtableMOver'" onmouseout="this.className='itemtable'"> 
          ...
          <td class="tdisi" align="center" style="font-size:12px;"><?php if($rows['SHIFT']==1){echo 'Pagi';}elseif($rows['SHIFT']==2){echo 'Siang';}else{echo 'Malam';} ?></td>
          <td class="tdisi" align="right" style="font-size:12px;"><?php echo number_format($rows['HARGA_TOTAL'],0,",","."); ?></td>
          <td class="tdisi" align="right" style="font-size:12px;"><?php echo number_format($rows['PPN_NILAI'],0,",","."); ?></td>
          <td class="tdisi" align="right" style="font-size:12px;"><?php echo number_format($rows['HARGA_TOTAL']+$rows['PPN_NILAI'],0,",","."); ?></td>
        </tr>
        <?php 
	  }
	  mysqli_free_result($rs);
	  ...
	  <td width="2" class=""></td>
        ...
        <tr> 
          ...
          <td align="right" class="txtright"><?php echo number_format($totharga,0,",","."); ?></td>
          <td align="right" class="txtright"><?php echo number_format($totppn,0,",","."); ?></td>
