...
$worksheet->write($row, $column+7, "Harga", $columnTitleFormat);
$worksheet->write($row, $column+8, "Poli (Ruangan)", $columnTitleFormat);
$worksheet->write($row, $column+9, "Dokter", $columnTitleFormat);
...
while ($rows=mysqli_fetch_array($rs)){
	...
	$worksheet->write($row, 6, $rows['QTY_JUAL'], $regularFormatC);
	$worksheet->write($row, 7, $rows['H_JUAL'], $regularFormatRF);
	$worksheet->write($row, 8, $rows['UNIT_NAME'], $regularFormatC);
	$worksheet->write($row, 9, $rows['DOKTER'], $regularFormatC);
	$row++;
	$i++;
}
