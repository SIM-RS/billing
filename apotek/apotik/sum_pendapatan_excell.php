...
$worksheet->write($row, 6, "PPN", $columnTitleFormat);
$worksheet->write($row, 7, "Tot Jual + PPN", $columnTitleFormat);
$worksheet->write($row, 8, "Tot Return", $columnTitleFormat);
$worksheet->write($row, 9, "Setoran", $columnTitleFormat);
...
$row++;
  ...
  if ($sorting=="" or $sorting==$defaultsort1) $sorting=$defaultsort;
	$sql="SELECT DATE_FORMAT(t2.TGL,'%d/%m/%Y') AS TGL,t2.SHIFT,t2.NAMA,t2.KSO_ID,t2.KPID,t2.KPNAMA,t2.CARABAYAR_ID,t2.cara_bayar,SUM(t2.tot_jual) AS tot_jual,SUM(t2.tot_ppn) AS tot_ppn,
		...
		SUM(t1.tot_jual) AS tot_jual,SUM(t1.tot_ppn) AS tot_ppn,tot_retur FROM 
		...
		GROUP BY DATE(ar.tgl_retur),ar.shift_retur,ap.KSO_ID,ap.UNIT_ID,ap.JENIS_PASIEN_ID,ap.CARA_BAYAR,ap.NO_PASIEN,ap.USER_ID) AS t2".$filter."
...
while ($rows=mysqli_fetch_array($rs)){
	...
	$worksheet->write($row, 6, $ttot_ppn, $regularFormatRF);
	$worksheet->write($row, 7, $ttot_jual+$ttot_ppn, $regularFormatRF);
	$worksheet->write($row, 8, $t_ret, $regularFormatRF);
	$worksheet->write($row, 9, $tsetoran, $regularFormatRF);
	$row++;
	$i++;
}
