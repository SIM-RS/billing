...
if($_REQUEST['tipe_lap']==2){
	...
	$fperiode = "AND DATE_FORMAT(p.TGL,'%Y-%m') BETWEEN '$thn1-$bln1' AND '$thn2-$bln2'";
	$fTGL = " WHERE DATE_FORMAT(TGL,'%Y-%m') BETWEEN '$thn1-$bln1' AND '$thn2-$bln2' ";
}
...
$ct = "SUM(IF(DATE_FORMAT(p.TGL,'%m-%Y')='$bulan-$tahun',p.QTY_JUAL,'0')) AS ".$alias;
...
for($i=1;$i<$jum_bln;$i++){
	...
	$b = strtoupper(getBulan($bulan));
	$alias = substr("$b",0,3)."_".$tahun;
	$ct = $ct.",SUM(IF(DATE_FORMAT(p.TGL,'%m-%Y')='$bulan-$tahun',p.QTY_JUAL,'0')) AS ".$alias;
	array_push($arrBln,$alias); 
}
//echo $ct;
//print_r($arrBln);
...
function goExcell()
{
	window.open('rpt_per_bulan_pemakaian_obat_XLS.php?bln1=<?=$bln1;?>&thn1=<?=$thn1;?>&bln2=<?=$bln2;?>&thn2=<?=$thn2;?>&idunit=<?=$unit_id;?>&tipe_lap=<?=$_REQUEST['tipe_lap'];?>&kepemilikan_id=<?php echo $_REQUEST['kepemilikan_id']; ?>&golongan_id=<?php echo $_REQUEST['golongan_id']; ?>&unit_farmasi=<?php echo $unit_farmasi; ?>');
}
</script>