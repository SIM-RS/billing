...
<iframe height="72" width="130" name="sort"
	...
	<div id="view" style="display:block"> 
      ...
      <table width="48%" border="0" align="center" cellpadding="1" cellspacing="0" class="txtinput">
        ...
        <tr> 
          ...
          <td width="177">: 
            <select name="jns_pasien" id="jns_pasien" class="txtinput" onchange="location='?f=list_penjualan.php&idunit='+idunit.value+'&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value+'&jns_pasien='+jns_pasien.value">
        ...
        <tr>
          <td>Jenis Pasien</td>
          <td>:
            <select name="statuspasien" id="statuspasien" class="txtinput" onchange="location='?f=list_penjualan.php&idunit='+idunit.value+'&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value+'&jns_pasien='+jns_pasien.value+'&statuspasien='+statuspasien.value">
            <?php 
			$sp="ALL PASIEN";
			?>
              <option value="0" class="txtinput">All Pasien</option>
      ...
      <table width="100%" align="center" border="0" cellpadding="1" cellspacing="0">
        <tr class="headtable"> 
          ...
          <td id="UNIT_NAME" width="135" class="tblheader" onclick="ifPop.CallFr(this);">Ruangan<br>
            (Poli)</td>
	...
	$sql="SELECT date_format(a_penjualan.TGL,'%d/%m/%Y') as tgl1,a_penjualan.TGL, a_penjualan.NO_PENJUALAN,am.NAMA AS KSO, a_penjualan.NO_RESEP, a_penjualan.NO_PASIEN, a_penjualan.NAMA_PASIEN, 
		a_kepemilikan.NAMA, a_unit.UNIT_NAME, a_penjualan.DOKTER, a_penjualan.SHIFT, a_penjualan.CARA_BAYAR,ac.nama as cbayar, a_penjualan.UTANG,SUM(a_penjualan.QTY_JUAL*a_penjualan.HARGA_NETTO) AS HARGA_NETTO, HARGA_TOTAL-SUM(FLOOR(a_penjualan.QTY_RETUR * a_penjualan.HARGA_SATUAN * (100-a_penjualan.BIAYA_RETUR)/100)) HARGA_TOTAL, 
		...
		WHERE (a_penjualan.UNIT_ID =$idunit) AND (a_penjualan.TGL BETWEEN '$tgl_1' AND '$tgl_2')".$jns_p.$statusp.$filter." 
		GROUP BY a_penjualan.NO_PENJUALAN,a_penjualan.JENIS_PASIEN_ID ORDER BY ".$sorting;
	//echo $sql."<br>";
		$rs=mysqli_query($konek,$sql);
	  ...
	  ?>
        <tr class="itemtable" onmouseover="this.className='itemtableMOver'" onmouseout="this.className='itemtable'"> 
          ...
          <td<?php if ($tCaraBayar<>4) echo ' colspan="2" width="36"'; else echo ' width="18"';?> class="tdisi" align="center"><a href="../report/kwi_retur.php?no_penjualan=<?php echo $rows['NO_PENJUALAN'];?>&sunit=<?php echo $idunit; ?>&no_pasien=<?php echo $rows['NO_PASIEN']; ?>&tgl=<?php echo $rows['TGL']; ?><?php if ($jRetur==0) echo '&viewdel=true';?>" onclick="NewWindow(this.href,'name','560','500','yes');return false"><img src="../icon/lihat.gif" border="0" width="16" height="16" align="absmiddle" title="Klik Untuk Melihat Detail Penjualan"></a></td>
          <?php if ($tCaraBayar==4){?>
...
var spasien=statuspasien.value;
	...
	OpenWnd('../apotik/list_resep_pasien_excell.php?tgl_d='+tgld+'&tgl_s='+tgls+'&idunit1='+idunit1+'&jns_pasien='+jpasien+'&statuspasien='+spasien+'&jp=<?php echo $jp; ?>&sp=<?php echo $sp; ?>&filter=<?php echo $jfilter; ?>&sorting=<?php echo $sorting; ?>',600,450,'childwnd',true);
}
