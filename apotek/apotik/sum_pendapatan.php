...
convert_var($bulan,$ta,$jns_pasien,$minta_id);
...
convert_var($bulan,$ta,$jns_p,$tgl_d,$tgl_s);
$d=explode("-",$tgl_d);
$tgl_de="$d[2]-$d[1]-$d[0]";
...
<input name="tgl_s" type="text" id="tgl_s" size="11" maxlength="10" readonly="true" class="txtcenter" value="<?php if ($_GET['tgl_s']<>'') echo $_GET['tgl_s']; else echo $tgl;?>" > 
      ...
      <table width="90%" align="center" border="0" cellpadding="1" cellspacing="0">
        <tr class="headtable"> 
          ...
          <td id="t2.tot_retur" width="80" class="tblheader" onclick="ifPop.CallFr(this);">PPN </td>
          <td id="t2.tot_retur" width="80" class="tblheader" onclick="ifPop.CallFr(this);">Tatal Jual + PPN </td>
	  ...
	  if ($sorting=="") $sorting=$defaultsort;
		...
		$sql="SELECT DATE_FORMAT(t2.TGL,'%d/%m/%Y') AS TGL,t2.SHIFT,t2.NAMA,t2.KSO_ID,t2.KPID,t2.KPNAMA,t2.CARABAYAR_ID,t2.cara_bayar,SUM(t2.tot_jual) AS tot_jual,SUM(t2.tot_ppn) AS tot_ppn,
			...
			SUM(t1.tot_jual) AS tot_jual,
			SUM(t1.tot_ppn) AS tot_ppn,
			tot_retur FROM 
			(SELECT TGL,TGL_ACT,NO_PENJUALAN,SHIFT,ap.KSO_ID,am.NAMA,ak.ID AS KPID,ak.NAMA AS KPNAMA,
			ac.id AS CARABAYAR_ID,ac.nama AS cara_bayar,
			SUM((QTY_JUAL) * HARGA_SATUAN) AS tot_jual,
		   (PPN/100) * SUM(QTY_JUAL* HARGA_SATUAN) AS tot_ppn,
			...
			GROUP BY SHIFT,NO_PENJUALAN,TGL,ap.KSO_ID,ap.UNIT_ID,ap.JENIS_PASIEN_ID,ap.CARA_BAYAR,ap.NO_PASIEN,ap.USER_ID ORDER BY TGL_ACT DESC) AS t1 
			...
			ak1.NAMA AS KPNAMA,ap.CARA_BAYAR AS CARABAYAR_ID,ac1.nama AS cara_bayar,0 AS tot_jual, 0 AS tot_ppn, SUM(ar.nilai) AS tot_retur 
			...
			GROUP BY DATE(ar.tgl_retur),ar.shift_retur,ap.KSO_ID,ap.JENIS_PASIEN_ID,ap.UNIT_ID,ap.CARA_BAYAR,ap.USER_ID) AS t2".$filter."
	...
	//	echo $sql."<br>";
	  ...
	  while ($rows=mysqli_fetch_array($rs)){
		...
		$ttot_ppn=$rows["tot_ppn"];
		$t_ret=$rows["tot_retur"];
		$tsetoran=($ttot_jual+$ttot_ppn)-$t_ret;
	  ?>
        <tr class="itemtable" onmouseover="this.className='itemtableMOver'" onmouseout="this.className='itemtable'"> 
          ...
          <td class="tdisi" align="right">&nbsp;<?php echo number_format($ttot_ppn,0,",","."); ?></td>
          <td class="tdisi" align="right">&nbsp;<?php echo number_format($ttot_jual+$ttot_ppn,0,",","."); ?></td>
	  ...
	  $sql2="select sum(t4.tot_jual) as tot_jual,sum(t4.tot_ppn) as tot_ppn,sum(t4.tot_retur) as tot_retur from (".$sql.") as t4";
	 ...
	 if ($show=mysqli_fetch_array($hs)){
		 ...
		 $tot_ppn=$show['tot_ppn'];
		 //$tot_ret=$show['tot_ret'];
		 $setoran=($tot_jual+$tot_ppn)-$tot_retur;
	 }
...
/*	$sql2="select UNIT_NAME from a_unit where UNIT_ID=$idunit1";
	...
	$unit=$u['UNIT_NAME'];*/
	  ?>
        <tr class="itemtable"> 
          <td class="tdisikiri" align="right" colspan="6">Jumlah Total&nbsp;          </td>
          <td class="tdisi" align="right">&nbsp;<? echo number_format($tot_jual,0,",","."); ?></td>
          <td class="tdisi" align="right">&nbsp;<? echo number_format($tot_ppn,0,",","."); ?></td>
          <td class="tdisi" align="right">&nbsp;<? echo number_format($tot_jual+$tot_ppn,0,",","."); ?></td>
	...
	<p align="center"><span class="jdltable">LAPORAN SUMMARY PENDAPATAN <? echo strtoupper($unit) ?></span>  
      ...
      <table width="90%" align="center" border="0" cellpadding="1" cellspacing="0">
        <tr class="headtable"> 
          ...
          <td width="80" class="tblheader">PPN</td>
          <td width="80" class="tblheader">Tatal Jual + PPN </td>
	  ...
	  ?>
        <tr class="itemtable" onmouseover="this.className='itemtableMOver'" onmouseout="this.className='itemtable'"> 
          ...
          <td class="tdisi" align="right" style="font-size:12px;">&nbsp;<?php echo number_format($ttot_ppn,0,",","."); ?></td>
          <td class="tdisi" align="right" style="font-size:12px;">&nbsp;<?php echo number_format($ttot_ppn+$ttot_jual,0,",","."); ?></td>
	  ...
	  ?>
        <tr class="itemtable"> 
          ...
          <td class="tdisi" align="right" style="font-size:12px;">&nbsp;<? echo number_format($tot_ppn,0,",","."); ?></td>
          <td class="tdisi" align="right" style="font-size:12px;">&nbsp;<? echo number_format($tot_jual+$tot_ppn,0,",","."); ?></td>
