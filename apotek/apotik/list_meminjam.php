...
<input name="act" id="act" type="hidden" value="save">
	...
	<div id="idArea" style="display:block">
	  <p><span class="jdltable">LAPORAN PENERIMAAN OBAT<br>UNIT : <?php echo $namaunit; ?></span></p>
...
<td width="120">Unit Pengirim </td>
            <td colspan="2">:
              <select name="unit_tujuan" id="unit_tujuan" onchange="location='?f=../apotik/list_meminjam.php&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value+'&unit_tujuan='+unit_tujuan.value">
			  ...
			  <td width="193">
			  <button type="button" onclick="location='?f=../apotik/list_meminjam.php&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value+'&unit_tujuan='+unit_tujuan.value">
	  ...
	  if ($sorting=="") $sorting=$defaultsort;
		...
		$sql="Select date_format(ap.TGL_TERIMA,'%d/%m/%Y') as tgl1,ap.NOTERIMA,sum(ap.QTY_SATUAN) as QTY_SATUAN,sum(FLOOR(ap.QTY_SATUAN*ap.HARGA_BELI_SATUAN)) as HARGA_BELI_TOTAL,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,ak.NAMA,au.UNIT_NAME From a_penerimaan ap inner Join a_unit au ON au.UNIT_ID = ap.UNIT_ID_KIRIM inner Join a_obat ao ON ap.OBAT_ID = ao.OBAT_ID inner Join a_kepemilikan ak ON ak.ID = ap.KEPEMILIKAN_ID where ap.UNIT_ID_TERIMA=$idunit AND ap.TIPE_TRANS=1 and ap.QTY_SATUAN>0 and ap.STATUS=1 and ap.TANGGAL between '$tgl_1' and '$tgl_2'".$unit_tj.$filter." group by ap.NOTERIMA,ao.OBAT_NAMA,ak.NAMA order by ".$sorting;
		//echo $sql."<br>";
