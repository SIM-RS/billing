...
$workbook->send('List_Pengeluaran_Obat_'.$tgl_s.'.xls');
...
$worksheet->write($row, 4, "DAFTAR PENGIRIMAN OBAT KE RUANGAN/POLI", $sheetTitleFormat);
$row += 1;
$worksheet->write($row, 4, "UNIT : $unitn", $sheetTitleFormat);
$row += 1;
$worksheet->write($row, 4, "TGL : ".$tgl_d." s/d ".$tgl_s, $sheetTitleFormat);
...
$worksheet->write($row, $column+2, "No Kirim", $columnTitleFormat);
$worksheet->write($row, $column+3, "Ruangan", $columnTitleFormat);
$worksheet->write($row, $column+4, "Nama Obat", $columnTitleFormat);
$worksheet->write($row, $column+5, "Kepemilikan", $columnTitleFormat);
$worksheet->write($row, $column+6, "Qty", $columnTitleFormat);
$worksheet->write($row, $column+7, "Sub Total Beli", $columnTitleFormat);
$worksheet->write($row, $column+8, "Harga Jual", $columnTitleFormat);
$worksheet->write($row, $column+9, "Sub Total Jual", $columnTitleFormat);
...
$sql="SELECT TANGGAL,DATE_FORMAT(TANGGAL,'%d-%m-%Y') tgl1,NOKIRIM,NAMA,OBAT_NAMA,SUM(QTY_JUAL) AS QTY_SATUAN,
HARGA_BELI_SATUAN,
/*
			FLOOR(IF (NILAI_PAJAK>0,SUM(HARGA_BELI_TOTAL-(DISKON/100*HARGA_BELI_TOTAL)+(10/100*(HARGA_BELI_TOTAL-(DISKON/100*HARGA_BELI_TOTAL)))),SUM(HARGA_BELI_TOTAL-(DISKON/100*HARGA_BELI_TOTAL))))*/ QTY_JUAL*RATA2_UNIT_KIRIM AS HARGA_BELI_TOTAL,
			
			FLOOR(HARGA_SATUAN) AS HARGA_SATUAN,SUM(FLOOR(HARGA_SATUAN)*QTY_JUAL) AS SUB_TOTAL,UNIT_NAME 
			...
			GROUP BY NOKIRIM, a_penerimaan.OBAT_ID,TANGGAL ORDER BY ".$sorting;
$i=1;
$rowStart=$row+1;
//echo $sql."<br>";
$rs=mysqli_query($konek,$sql);
...
while ($rows=mysqli_fetch_array($rs)){
	...
	if($nokirim != $rows['NOKIRIM']){
		$worksheet->write($row, 1, $rows['tgl1'], $regularFormatC);
		$worksheet->write($row, 2, $rows['NOKIRIM'], $regularFormatC);
		$worksheet->write($row, 3, $rows['UNIT_NAME'], $regularFormatC);
	} else {
	...
	$worksheet->write($row, 5, $rows['NAMA'], $regularFormatC);
	$worksheet->write($row, 6, $rows['QTY_SATUAN'], $regularFormatC);
	$worksheet->write($row, 7, $rows['HARGA_BELI_TOTAL'], $regularFormatRF);
	$worksheet->write($row, 8, $rows['HARGA_SATUAN'], $regularFormatRF);
	$worksheet->write($row, 9, $rows['SUB_TOTAL'], $regularFormatRF); // "=(G".($row+1)."*I".($row+1).")"
	$row++;
	$i++;
	$nokirim = $rows['NOKIRIM'];
}
$worksheet->write($row, 5, "Total", $regularFormatR);
$worksheet->write($row, 6, '=Sum(G'.$rowStart.':G'.$row.')', $regularFormatRF);
$worksheet->write($row, 7, '=Sum(H'.$rowStart.':H'.$row.')', $regularFormatRF);
$worksheet->write($row, 9, '=Sum(J'.$rowStart.':J'.$row.')', $regularFormatRF);
