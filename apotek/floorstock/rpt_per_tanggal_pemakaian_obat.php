...
$fperiode = "AND ap.TANGGAL BETWEEN '$tgl1' AND '$tgl2'";
...
$ct = "SUM(IF(ap.TANGGAL='$tgl1',ap.QTY_SATUAN,'0')) AS ".tgl2As($tgl1);
...
for($i=1;$i<$jum_hari;$i++){
	...
	$ct = $ct.",SUM(IF(ap.TANGGAL='$tgl1',ap.QTY_SATUAN,'0')) AS ".tgl2As($tgl1);
	array_push($arrHari,tgl2As($tgl1));
}
...
function goExcell()
{
	window.open('rpt_per_tanggal_pemakaian_obat_XLS.php?tgl_d=<?=$tgl_1;?>&tgl_s=<?=$tgl_2;?>&idunit=<?=$unit_id;?>&tipe_lap=<?=$_REQUEST['tipe_lap'];?>&kepemilikan_id=<?php echo $_REQUEST['kepemilikan_id']; ?>&golongan_id=<?php echo $_REQUEST['golongan_id']; ?>');
}
</script>