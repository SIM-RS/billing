...
convert_var($tgl2,$tgl_ctk,$idunit);
...
convert_var($bulan,$ta,$minta_id,$unit_tujuan);
 

if ($bulan=="") $bulan=(substr($th[1],0,1)=="0")?substr($th[1],1,1):$th[1];
if ($ta=="") $ta=$th[2];
if($unit_tujuan=="0" OR $unit_tujuan=="") $unit_tj=""; else $unit_tj="and a_penerimaan.UNIT_ID_TERIMA=$unit_tujuan";

$tgl_d=$_REQUEST['tgl_d'];
$tgl_s=$_REQUEST['tgl_s'];
convert_var($bulan,$ta,$unit_tj,$tgl_d,$tgl_s);


if ($tgl_d=="") $tgl_d=$tgl;
$d=explode("-",$tgl_d);
$tgl_de="$d[2]-$d[1]-$d[0]";if ($tgl_s=="") $tgl_s=$tgl;
$s=explode("-",$tgl_s);
$tgl_se="$s[2]-$s[1]-$s[0]";
...
<div align="center">
      ...
      <table width="49%" align="center" border="0" cellpadding="1" cellspacing="0" class="txtinput">
        ...
        <tr>
          <td>Jenis Laporan</td>
          <td>:&nbsp;<select id="cmbjlap" name="cmbjlap" onchange="if (this.value==1){document.getElementById('trcmbData').style.visibility='visible'}else{document.getElementById('trcmbData').style.visibility='collapse'}">
