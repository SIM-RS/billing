...
$nunit1="FLOORSTOCK";
convert_var($tglctk,$tgl,$tglact,$idunit,$nunit1);

$cmbdata=$_REQUEST['cmbdata'];
$cmbjlap=$_REQUEST['cmbjlap'];
$tgl_d=$_REQUEST['tgl_d'];
$tgl_s=$_REQUEST['tgl_s'];

convert_var($cmbdata,$cmbjlap,$tgl_d,$tgl_s);
...
//convert_var($tgl_s1,$tgl_d1,$sorting,$filter);
//===============================
...
$columnTitleFormatR =& $workbook->addFormat(array('bold'=>1,'size'=>11,'align'=>'right','fgcolor'=>'grey','pattern'=>1));
...
if ($cmbjlap==0) $jdl="LAPORAN REKAP PEMAKAIAN OBAT OLEH RUANGAN/POLI";
$cUnit="ap.UNIT_ID=20";
$cUnit1="UNIT_ID=20";
if ($idunit==""){
	$cUnit="ap.UNIT_ID<>20";
	$cUnit1="UNIT_ID<>20";
	$nunit1="ALL FARMASI";
	$jdl="LAPORAN RESEP OBAT OLEH RUANGAN/POLI";
	if ($cmbjlap==0) $jdl="LAPORAN REKAP RESEP OBAT OLEH RUANGAN/POLI";
}
...
if ($cmbjlap==1){
	...
	$worksheet->write($row, 2, $jdl, $sheetTitleFormat);
	$row += 1;
	$worksheet->write($row, 2, "UNIT : $nunit1", $sheetTitleFormat);
	$row += 1;
	$worksheet->write($row, 2, "TGL : ".$tgl_d." s/d ".$tgl_s, $sheetTitleFormat);
	$row += 1;
	$worksheet->write($row, 0, "Unit Pelayanan", $columnTitleFormat);
	$worksheet->write($row, 1, "Qty Obat", $columnTitleFormat);
	$worksheet->write($row, 2, "Nilai Bahan Jual", $columnTitleFormat);
	$worksheet->write($row, 3, "Nilai Jual", $columnTitleFormat);
	$worksheet->write($row, 4, "Nilai Bahan Return", $columnTitleFormat);
	$worksheet->write($row, 5, "Nilai Return", $columnTitleFormat);
	$row += 1;
	...
	$rs=mysqli_query($konek,$sql);
	while ($rows=mysqli_fetch_array($rs)){
		$sql="SELECT IFNULL(SUM(t.QTY_JUAL),0) QTY_JUAL,IFNULL(SUM(t.NILAI_BAHAN),0) NILAI_BAHAN_JUAL,IFNULL(SUM(t.NILAI_JUAL),0) NILAI_JUAL,
IFNULL(SUM(t.NILAI_BAHAN_RETUR),0) NILAI_BAHAN_RETUR,IFNULL(SUM(t.NILAI_RETUR),0) NILAI_RETUR
FROM (SELECT ap.*,IF(apn.NILAI_PAJAK>0,ap.QTY_JUAL*apn.HARGA_BELI_SATUAN*(1-(apn.DISKON/100))*1.1,ap.QTY_JUAL*apn.HARGA_BELI_SATUAN) AS NILAI_BAHAN,
ap.QTY_JUAL*ap.HARGA_SATUAN NILAI_JUAL,
IF(apn.NILAI_PAJAK>0,ap.QTY_RETUR*apn.HARGA_BELI_SATUAN*(1-(apn.DISKON/100))*1.1,ap.QTY_RETUR*apn.HARGA_BELI_SATUAN) AS NILAI_BAHAN_RETUR,
ap.QTY_RETUR*ap.HARGA_SATUAN*(100-ap.BIAYA_RETUR)/100 NILAI_RETUR
FROM (SELECT PENERIMAAN_ID,QTY_JUAL,QTY_RETUR,BIAYA_RETUR,HARGA_SATUAN,RUANGAN FROM a_penjualan 
WHERE $cUnit1 AND RUANGAN=".$rows['UNIT_ID']." AND TGL BETWEEN '$tgl_d1' AND '$tgl_s1') AS ap 
INNER JOIN a_penerimaan apn ON ap.PENERIMAAN_ID=apn.ID) AS t";
		$rs1=mysqli_query($konek,$sql);
		$rw1=mysqli_fetch_array($rs1);
		
		$worksheet->write($row, 0, $rows['UNIT_NAME'], $regularFormat);
		$worksheet->write($row, 1, $rw1['QTY_JUAL'], $regularFormatR);
		$worksheet->write($row, 2, $rw1['NILAI_BAHAN_JUAL'], $regularFormatR);
		$worksheet->write($row, 3, $rw1['NILAI_JUAL'], $regularFormatR);
		$worksheet->write($row, 4, $rw1['NILAI_BAHAN_RETUR'], $regularFormatR);
		$worksheet->write($row, 5, $rw1['NILAI_RETUR'], $regularFormatR);
		$row++;
	}
	$maxRow=$row;
	$row++;
	$worksheet->write($row, 0, "TOTAL : ", $columnTitleFormatR);
	$worksheet->write($row, 1, '=Sum(B5:B'.$maxRow.')', $sTotFormatR);
	$worksheet->write($row, 2, '=Sum(C5:C'.$maxRow.')', $sTotFormatR);
	$worksheet->write($row, 3, '=Sum(D5:D'.$maxRow.')', $sTotFormatR);
	$worksheet->write($row, 4, '=Sum(E5:E'.$maxRow.')', $sTotFormatR);
	$worksheet->write($row, 5, '=Sum(F5:F'.$maxRow.')', $sTotFormatR);
}
