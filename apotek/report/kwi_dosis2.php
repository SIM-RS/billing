...
DATE_FORMAT(mp.tgl_lahir, '%d-%m-%Y') tgl_lahir,mp.no_ktp,mp.telp,mp.sex,
		mp.no_rm,r.alamat,r.dokter_nama dokter,mu.nama poli,
		CONCAT((CONCAT((CONCAT((CONCAT(mp.alamat,' RT.',mp.rt)),' RW.',mp.rw)),', Desa ',w.nama)),', Kecamatan ',wi.nama,', ',wii.nama) alamat_, r.iter, bmk.nama AS nm_kso
FROM $dbbilling.b_resep r INNER JOIN $dbbilling.b_pelayanan p ON r.id_pelayanan=p.id
INNER JOIN $dbbilling.b_kunjungan k ON p.kunjungan_id=k.id INNER JOIN $dbbilling.b_ms_pasien mp ON k.pasien_id=mp.id
	LEFT JOIN $dbbilling.b_ms_wilayah w
		ON mp.desa_id = w.id
	...
	LEFT JOIN $dbbilling.b_ms_wilayah wii
		ON mp.kab_id = wii.id
INNER JOIN $dbbilling.b_ms_unit mu ON p.unit_id=mu.id
INNER JOIN $dbbilling.b_ms_kso bmk ON bmk.id = p.kso_id
WHERE r.apotek_id='$idunit' AND r.no_resep='$no_resep' 
AND r.id_pelayanan='$idpel' AND r.no_rm='$no_pasien'";
	//}
  ...
  <table width="1000" height="113" border="0" align="left" cellpadding="0" cellspacing="0" class="style1">
    <tr class="style1"> 
      <td height="45" colspan="3" class="style1" style="font-size:16px"> <b><?= "FARMASI PRIMA HUSADA CIPTA MEDAN" ?><br>
        <?= $alamatRS; ?>&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;<?php echo $apname; ?><br>
        RESEP ELEKTRONIK</b> 
    ...
    <tr> 
      ...
      <td>: <?php echo $showSingle['no_resep']; ?></td>
      <td align="right">Tgl Cetak : <?php echo date("d-m-Y H:i:s"); ?></td>
    </tr>
    <tr > 
    ...
    <tr> 
      <td >Penjamin</td>
      <td colspan="2" >: <?php echo $showSingle['nm_kso']; ?></td>
    </tr>
    <tr> 
      <td >Iter</td>
    ...
    <tr> 
      <td colspan="3" ><table width="1000" border="0" cellpadding="0" cellspacing="0" align="left" style="line-height:1.2em;">
          <?php 
		  	/*if ($iduser_jual!=""){
	...
	IF($dbbilling.b_resep.status = 2, 'nedet', 'det') nedet
...
FROM $dbbilling.b_resep INNER JOIN $dbapotek.a_obat ON $dbapotek.a_obat.OBAT_ID=$dbbilling.b_resep.obat_id
LEFT JOIN $dbapotek.a_penjualan ON $dbapotek.a_penjualan.ID =  $dbbilling.b_resep.penjualan_id 
WHERE $dbapotek.a_penjualan.tgl ='$tgl' AND $dbbilling.b_resep.apotek_id='$idunit' AND $dbbilling.b_resep.no_resep='$no_resep' 
AND $dbbilling.b_resep.id_pelayanan='$idpel' AND $dbbilling.b_resep.no_rm='$no_pasien' AND $dbbilling.b_resep.racikan = 0";
			}
...
AND $dbbilling.b_resep.id_pelayanan='$idpel' AND $dbbilling.b_resep.no_rm='$no_pasien' AND $dbbilling.b_resep.racikan = 0";
          ...
          <tr> 
            ...
            <td align="left" valign="top" class="style1">[ <?php echo $showPrint['isi']; ?> ]</td>
          </tr>
          <? $z++;} ?>
...
AND $dbbilling.b_resep.id_pelayanan='$idpel' AND $dbbilling.b_resep.no_rm='$no_pasien' AND $dbbilling.b_resep.obat_manual <> ''";
          ...
          <?php 
		  	/*if ($iduser_jual!=""){
				...
				$sqlPrint2="SELECT $dbbilling.b_resep.racikan, IF($dbbilling.b_resep.status = 2, 'nedet', 'det') nedet
FROM $dbbilling.b_resep 
WHERE $dbbilling.b_resep.tgl='$tgl' AND $dbbilling.b_resep.apotek_id='$idunit' AND $dbbilling.b_resep.no_resep='$no_resep' 
...
AND $dbbilling.b_resep.id_pelayanan='$idpel' AND $dbbilling.b_resep.no_rm='$no_pasien' AND $dbbilling.b_resep.racikan='".$showPrint2['racikan']."'";
          ...
          <tr> 
            ...
            <td align="left" class="style1">&nbsp;</td>
			<td align="left" class="style1">[ <?php echo $showPrint3['isi']; ?> ]</td>
          </tr>
          <?php } ?>
          
         <?
		 	$qBaru = "SELECT IF(textH = '' OR textH = 'undefined','-',textH) AS textH, IF(textT = '' OR textT = 'undefined','-',textT) AS textT, IF(textK = '' OR textK = 'undefined','-',textK) AS textK, IF(textP = '' OR textP = 'undefined','-',textP) AS textP FROM a_penjualan WHERE NO_PENJUALAN = '".$_REQUEST['no_penjualan']."' AND unit_id = '".$_REQUEST['sunit']."' AND NO_KUNJUNGAN = '".$_REQUEST['idpel']."'";
			$exeQBaru=mysqli_query($konek,$qBaru);
			$showPrint3=mysqli_fetch_array($exePrint3)
         ?>
		  <tr> 
            <td width="286" colspan="2" align="left" class="style1">&nbsp;</td>
            <td width="350" align="left" class="style1">&nbsp;</td>
            <td align="left" class="style1">&nbsp;</td>
            <td align="right" class="style1">H : <?=$showPrint3['textH'];?> <br> T : <?=$showPrint3['textT']; ?> <br> K : <?=$showPrint3['textT']; ?> <br> P : <?=$showPrint3['textP']; ?></td>
          </tr>		  
		  <?php } ?>
           ...
           <?
		 	$qBaru = "SELECT IF(textH = '' OR textH = 'undefined','-',textH) AS textH, IF(textT = '' OR textT = 'undefined','-',textT) AS textT, IF(textK = '' OR textK = 'undefined','-',textK) AS textK, IF(textP = '' OR textP = 'undefined','-',textP) AS textP FROM a_penjualan WHERE NO_PENJUALAN = '".$_REQUEST['no_penjualan']."' AND unit_id = '".$_REQUEST['sunit']."' AND NO_KUNJUNGAN = '".$_REQUEST['idpel']."'";
			$exeQBaru=mysqli_query($konek,$qBaru);
			$showPrint3=mysqli_fetch_array($exeQBaru)
         ?>
		  <tr> 
            <td width="286" colspan="2" align="left" class="style1">&nbsp;</td>
            <td width="350" align="left" class="style1">&nbsp;</td>
