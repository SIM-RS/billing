...
convert_var($iduser,$iduser_jual,$idunit,$no_penjualan,$no_pasien);
    ...
    <tr> 
      <td colspan="3" ><table width="550" border="0" cellpadding="0" cellspacing="0" align="left">
          <?php 
			...
			<?php if ($ada_retur>0){
				$ppn = $showPrint['PPN'];
				$ppn_harga = $showPrint['HARGA_SATUAN']*($ppn/100);
				?>
          <tr> 
            ...
            <!--td align="right" class="style1"><?php echo number_format($showPrint['HARGA_SATUAN']+$ppn_harga,0,',','.'); ?></td-->
            <?php
				$jTot = floor($showPrint['QTY_JUAL'] * ($showPrint['HARGA_SATUAN'] + $ppn_harga));
				//echo "<br>".$jTot."<br>";
				$htot += $jTot;
			?>
		  ...
		  <?php 
					//$htot += $showPrint['SUB_TOTAL'];
				}?>
          <? } ?>
		 <?php if($showPrint['PPN_NILAI'] > 0){ ?>
          <tr align="right">
            <td colspan="3" class="style2" style="padding-right:5px;">PPN</td>
            <td class="style2" style="padding-right:10px;"><span class="style2" style="padding-right:10px;"><?php echo number_format($showPrint['PPN_NILAI'],0,",","."); ?></span></td>
          </tr>
		  <? } ?>
          <tr align="right"> 
      ...
      <?php
	  	if ($ada_retur>0){
			if ($iduser_jual!=""){
				$sql_ret= "SELECT SUM(FLOOR(QTY_RETUR*(HARGA_SATUAN+(HARGA_SATUAN*(PPN/100)))*(100-BIAYA_RETUR)/100)) as nretur from a_penjualan where NO_PENJUALAN='$no_penjualan' and UNIT_ID=$idunit and NO_PASIEN='$no_pasien' AND a_penjualan.TGL='$tgl' AND a_penjualan.USER_ID=$iduser_jual";
			}else{
				$sql_ret= "SELECT SUM(FLOOR(QTY_RETUR*(HARGA_SATUAN+(HARGA_SATUAN*(PPN/100)))*(100-BIAYA_RETUR)/100)) as nretur from a_penjualan where NO_PENJUALAN='$no_penjualan' and UNIT_ID=$idunit and NO_PASIEN='$no_pasien' AND a_penjualan.TGL='$tgl'";
			}
			//echo $sql_ret;
