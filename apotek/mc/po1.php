			...
			<div id="listma" style="display:block">
			  ...
			  <table width="98%" border="0" cellpadding="1" cellspacing="0">
                <tr class="headtable"> 
                  ...
                  <td id="no_po" width="120" class="tblheader" onClick="ifPop.CallFr(this);">No 
                    PO </td>
			  ...
			  $sql="select distinct date_format(TANGGAL,'%d/%m/%Y') as tgl1,date_format(TANGGAL,'%d-%m-%Y') as tgl2,no_po,a_po.KET,UPDT_H_NETTO,TERMASUK_PPN,a_po.pbf_id,PBF_NAMA,no_spph,k.ID,k.NAMA from a_po inner join a_pbf on a_po.pbf_id=a_pbf.pbf_id inner join a_spph on a_spph.spph_id=a_po.FK_SPPH_ID inner join a_kepemilikan k on a_po.KEPEMILIKAN_ID=k.ID where month(TANGGAL)=$bulan and year(TANGGAL)=$ta".$filter." group by no_po order by ".$sorting;
			  //echo $sql."<br>";
				$rs=mysqli_query($konek,$sql);
				$jmldata=mysqli_num_rows($rs);
