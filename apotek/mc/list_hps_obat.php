		...
		<div align="center">
		  <form name="form1" method="post" action="">
			...
			<link rel="stylesheet" href="../theme/print.css" type="text/css" />
		      ...
		      <table width="96%" border="0" cellpadding="1" cellspacing="0" align="center">
                <tr class="headtable"> 
                  ...
                  <td id="NO_HAPUS" width="160" class="tblheader" onClick="ifPop.CallFr(this);">No. 
                    Hapus </td>
                  <td id="UNIT_NAME" width="90" class="tblheader" onClick="ifPop.CallFr(this);">Unit 
                  </td>
			  ...
			  //$sql="Select a_obat_hapus.*,a_unit.UNIT_NAME,a_penerimaan.NOTERIMA,a_obat.OBAT_NAMA From a_obat_hapus Inner Join a_penerimaan ON a_obat_hapus.PENERIMAAN_ID = a_penerimaan.ID Inner Join a_obat ON a_obat_hapus.OBAT_ID = a_obat.OBAT_ID Inner Join a_unit ON a_obat_hapus.UNIT_ID = a_unit.UNIT_ID where month(TGL_HAPUS)=$bulan and year(TGL_HAPUS)=$ta".$filter." order by ".$sorting;
			  $sql="SELECT a_obat_hapus.TGL_HAPUS,a_obat_hapus.NO_HAPUS,a_unit.UNIT_NAME,a_obat.OBAT_NAMA,a_kepemilikan.NAMA,SUM(a_obat_hapus.QTY) AS QTY,a_obat_hapus.ALASAN
					FROM dbapotik.a_obat INNER JOIN dbapotik.a_obat_hapus ON (a_obat.OBAT_ID = a_obat_hapus.OBAT_ID) INNER JOIN dbapotik.a_kepemilikan 
					ON (a_kepemilikan.ID = a_obat_hapus.KEPEMILIKAN_ID) INNER JOIN dbapotik.a_unit ON (a_unit.UNIT_ID = a_obat_hapus.UNIT_ID)
					WHERE (MONTH(a_obat_hapus.TGL_HAPUS)=$bulan AND YEAR(a_obat_hapus.TGL_HAPUS)=$ta)
			...
			<link rel="stylesheet" href="../theme/print.css" type="text/css" />
			  ...
			  <table width="96%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td colspan="6"><span class="txtinput">Bulan : </span>
                      <select name="bulan" id="bulan" class="txtinput" onChange="location='list_hps_obat.php?bulan='+bulan.value+'&ta='+ta.value">
                      ...
                      <select name="ta" id="ta" class="txtinput" onChange="location='list_hps_obat.php?bulan='+bulan.value+'&ta='+ta.value">
                        <?php for ($i=($th[2]-5);$i<($th[2]+1);$i++){ ?>
                        <option value="<?php echo $i; ?>" class="txtinput"<?php if ($i==$ta) echo "selected";?>><?php echo $i;?></option>
                        <? }?>
                    </select></td>
                  <td width="530" align="right" colspan="6"><BUTTON type="button" onClick="location='hapus_obat.php'"><IMG SRC="../icon/add.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Tambah Penghapusan Obat </BUTTON></td>
                </tr>
              </table>
		      <table width="96%" border="0" cellpadding="1" cellspacing="0">
                <tr class="headtable"> 
                  ...
                  <td id="NO_HAPUS" width="140" class="tblheader" onClick="ifPop.CallFr(this);">No. 
                    Hapus </td>
                  <td id="UNIT_NAME" width="90" class="tblheader" onClick="ifPop.CallFr(this);">Unit 
                  </td>
                  <td id="OBAT_NAMA" class="tblheader"  onClick="ifPop.CallFr(this);">Nama 
