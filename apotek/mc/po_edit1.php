 	...
 	case "save":
		...
		for ($j=0;$j<count($arfvalue);$j++){
			...
			if ($arfdata[13]!="0"){
				...
				$sql="update a_po set QTY_KEMASAN=$arfdata[3],QTY_PAKAI=QTY_PAKAI - $qty_kemasan,HARGA_KEMASAN=$arfdata[5],QTY_PER_KEMASAN=$arfdata[6],QTY_SATUAN=$arfdata[7],HARGA_BELI_TOTAL=$h_tot,HARGA_BELI_SATUAN=$arfdata[9],DISKON=$arfdata[11],DISKON_TOTAL=$disk_tot,NILAI_PAJAK=$ppn,UPDT_H_NETTO=$updt_harga,TERMASUK_PPN=$chk_ppn where ID=$arfdata[13]";
				//echo $sql."<br>";
			}else{
...
*/
$sql1="select distinct HARGA_BELI_TOTAL,DISKON_TOTAL,(HARGA_BELI_TOTAL-DISKON_TOTAL) as H_DISKON,NILAI_PAJAK,(HARGA_BELI_TOTAL-DISKON_TOTAL)+NILAI_PAJAK as TOTAL,TERMASUK_PPN from a_po where NO_PO='$no_po'";
//echo $sql1;
...
<input name="sub_tot" type="text" class="txtright" id="sub_tot" onKeyUp="" value="<?php echo $rows['subtotal']; ?>" size="8" readonly="true">
                  ...
                  <td width="50" align="center" class="tdisi"> 
                    <input name="diskon_rp" type="text" class="txtright" id="diskon_rp" onKeyUp="HitungDiskon(<?php echo ($i-1); ?>,2);" value="<?php echo (($rows['subtotal']*$rows['DISKON'])/100); ?>" size="7" autocomplete="off"></td>
                </tr>
                <?php 
			  }
