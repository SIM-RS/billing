...
$rquery1 = mysqli_query($konek,$query1);
$dquery1 = mysqli_fetch_array($rquery1);
...
switch($_REQUEST['id']) {
	...
	case 'cmbTmpLayStokView':
		...
		if($val[1]=='far'){
        	...
        	$sql="SELECT distinct au.UNIT_ID as id,u.nama FROM (SELECT distinct b.unit_id FROM b_ms_pegawai_unit b
               where ms_pegawai_id='".$val[0]."') as t1
               inner join b_ms_unit u on t1.unit_id=u.id
			   INNER JOIN dbapotek_tangerang.a_unit au ON au.unit_billing=u.id
               where u.parent_id='".$val[1]."' and u.aktif=1 order by nama";
		}
        break;
	case 'JnsLayanan15':
    ...
    case 'JnsLayanan_sjp':
        $wil='';
        $sql="select id,nama from b_ms_unit where id in (1,44,57,60,64,66) and kategori=2 and aktif=1 and level=1";
        break;
	...
	case 'JnsLayananKonsul':
        $wil='';
        $sql="SELECT id,nama FROM b_ms_unit WHERE (kategori=2 OR kategori=5) AND aktif=1 AND LEVEL=1 AND id IN (SELECT DISTINCT parent_id FROM b_ms_unit WHERE inap=0 AND LEVEL=2)";
		break;
    case 'JnsLayananJln':
		$wil='';
    ...
    case 'JnsLayananDenganInap':
        $wil='';
        $lang=true;
        $sql="select id,nama,inap as lang from b_ms_unit where kategori=2 and inap=1 and aktif=1 and level=1";
        break;
...
INNER JOIN b_ms_unit mu1 ON mu.parent_id=mu1.id WHERE mu1.id<>'27' AND mul.id_loket=$val[0] AND mu.aktif=1 and mu.kategori = 2";
        break;
    case 'JnsLayananFull':
        $wil='';
...
INNER JOIN b_ms_unit mu1 ON mu.parent_id=mu1.id WHERE mul.id_loket=$val[0] AND mu.aktif=1 and mu.kategori = 2";
    ...
    case 'TmpLayMedik':
		$wil = '';
		$sql = "SELECT id, nama FROM b_ms_unit WHERE aktif=1 AND id IN (58,59,61) ORDER BY nama ";
		break;
    ...
    case 'cmbKelasKamar':
		...
		$q_def_kls=mysqli_query($konek,$def_kls);
		$rw_def_kls=mysqli_fetch_array($q_def_kls);
...
INNER JOIN b_ms_kamar k ON kt.kamar_id=k.id WHERE k.unit_id = $val[0] AND (mk.id=4 OR mk.id=37) AND k.aktif=1";
		$rsJmlKls=mysqli_query($konek,$sql);
		if (mysqli_num_rows($rsJmlKls)<=0){
			$sql="SELECT DISTINCT mk.id,mk.nama,kt.tarip FROM b_ms_kamar_tarip kt INNER JOIN b_ms_kelas mk ON kt.kelas_id = mk.id
...
WHERE kamar.unit_id = $val[0] AND kamar.aktif=1";
    ...
    case 'Retribusi':
		...
		$idKlsTind = " inner join b_ms_tindakan_kelas tk on tk.ms_tindakan_id = b.id
			    ...
			    where klasifikasi_id = 11 and tu.ms_unit_id = '$val[1]' and b.id='2362'";
	    ...
	    $wil = '-';
            $idKlsTind = " inner join b_ms_tindakan_kelas tk on tk.ms_tindakan_id = b.id
			    inner join b_ms_tindakan_unit tu on tu.ms_tindakan_kelas_id = tk.id
                            where klasifikasi_id=11 and (b.id='2364' or b.id='2365')"; //IGD
        }
        else if($val[0]=='50' || $val[0]=='132') {
            $idKlsTind = " inner join b_ms_tindakan_kelas tk on tk.ms_tindakan_id = b.id
			    inner join b_ms_tindakan_unit tu on tu.ms_tindakan_kelas_id = tk.id
                            where klasifikasi_id=11 and tu.ms_unit_id = '$val[1]' and b.id='2366'"; //PAVILYUN
        }
        $sql="SELECT distinct b.id,b.nama,tk.tarip as lang FROM b_ms_tindakan b $idKlsTind";
		$rs = mysqli_query($konek,$sql);
		if(mysqli_num_rows($rs) == 0){
			$sql = "select '0' as id, '-' as nama, 0 as lang";
		}
    ...
    case 'cmbGroup':
        $wil='';
        $sql="SELECT id,nama FROM a_group WHERE aktif = 1 ORDER BY nama";
        break;
    case 'cmbKlasi':
        $wil='';
    ...
    case 'cmbTmpLay':
        $wil='';
        $lang=true;
        $sql="SELECT distinct u.id,u.nama,inap as lang FROM (SELECT distinct b.unit_id FROM b_ms_pegawai_unit b
               where ms_pegawai_id='".$val[0]."') as t1
               inner join b_ms_unit u on t1.unit_id=u.id
               where u.parent_id='".$val[1]."' and u.aktif=1 order by nama";
        break;
    case 'cmbDokPengganti':
	 	$wil='';
		$sql="SELECT id,nama FROM b_ms_pegawai WHERE spesialisasi_id=62 ORDER BY nama";
	break;
    case 'cmbDok':
        $wil='';
        $sql="SELECT b.id,b.nama
...
if($_REQUEST['id']=="StatusPas") {
    ?>
<option value="1" label="UMUM WARGA"<?php if ($_REQUEST['defaultId']=="1") echo " selected";?>>UMUM WARGA</option>
<option value="2" label="UMUM NON WARGA"<?php if ($_REQUEST['defaultId']=="1") echo " selected";?>>UMUM NON WARGA</option>
    <?php
}
