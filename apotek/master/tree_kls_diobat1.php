...
<tr>
  <td align=center>
  <button value="TOP PARENT" onClick="fSetValue(window.opener,'<? echo $par[2];?>*-*0*|*<? echo $par[0];?>*-*0*|*<? echo $par[1];?>*-*0*|*<? echo $par[3];?>*-*0');window.close();" id="top" style=" width:100px; height:30px;">Top Parent</button>
  </td>
</tr>
<tr>
...
<?php	
  ...
  while ($rows=mysqli_fetch_array($rs)){
		 ...
		 if ($c_level>0){
			 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[2]."*-*".$rows['KLS_ID']."*|*".$par[0]."*-*".$rows['KLS_NAMA']."*|*".$par[1]."*-*".$rows['KLS_KODE']."*|*".$par[3]."*-*".$rows['KLS_LEVEL']."');window.close();";
 		 }else
		 	 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[2]."*-*".$rows['KLS_ID']."*|*".$par[0]."*-*".$rows['KLS_NAMA']."*|*".$par[1]."*-*".$rows['KLS_KODE']."*|*".$par[3]."*-*".$rows['KLS_LEVEL']."');window.close();";
		 $tree[$cnt][3]= "";
		 $tree[$cnt][4]= 0;
