...
convert_var($tglctk,$tgl,$tgl_d1,$tgl_s1);
...
convert_var($sorting,$filter,$kelas,$golongan,$jenis);
...
$worksheet->write($row, $column+3, "AP-RS", $columnTitleFormat);
// $worksheet->write($row, $column+4, "AP-Krakatau", $columnTitleFormat);
// $worksheet->write($row, $column+5, "AP-BICT", $columnTitleFormat);
...
$worksheet->write($row, $column+4, "FLOORSTOCK", $columnTitleFormat);
$worksheet->write($row, $column+5, "Total", $columnTitleFormat);
...
$row++;
  ...
  $sql="SELECT ao.OBAT_KODE,ao.OBAT_NAMA,ak.NAMA,t1.*,t1.AP1+/* t1.AP2+t1.AP3*/t1.FS /* +t1.AP4+t1.AP5+t1.AP6+t1.AP7+t1.AP8+t1.AP10+t1.AP11 */ AS QTY_TOTAL FROM a_obat ao INNER JOIN 
		(SELECT `a_penerimaan`.`OBAT_ID` AS `OBAT_ID`,`a_penerimaan`.`KEPEMILIKAN_ID` AS `KEPEMILIKAN_ID`,
		SUM(IF((`a_penjualan`.`UNIT_ID` = 7),(`a_penjualan`.`QTY_JUAL` - `a_penjualan`.`QTY_RETUR`),0)) AS `AP1`,
		-- SUM(IF((`a_penjualan`.`UNIT_ID` = 8),(`a_penjualan`.`QTY_JUAL` - `a_penjualan`.`QTY_RETUR`),0)) AS `AP2`,
		-- SUM(IF((`a_penjualan`.`UNIT_ID` = 9),(`a_penjualan`.`QTY_JUAL` - `a_penjualan`.`QTY_RETUR`),0)) AS `AP3`,
		SUM(IF((`a_penjualan`.`UNIT_ID` = 20),(`a_penjualan`.`QTY_JUAL` - `a_penjualan`.`QTY_RETUR`),0)) AS `FS`,
		SUM(FLOOR((`a_penjualan`.`QTY_JUAL` - `a_penjualan`.`QTY_RETUR`) * `a_penjualan`.`HARGA_NETTO`)) AS `TOTAL` 
...
while ($rows=mysqli_fetch_array($rs)){
	...
	$worksheet->write($row, 4, $rows['FS'], $regularFormatR);
	$worksheet->write($row, 5, $rows['QTY_TOTAL'], $regularFormatR);
	$worksheet->write($row, 6, $rows['TOTAL'], $regularFormatR);
	$row++;
}
$worksheet->write($row, 7, "TOTAL", $regularFormatR);
$worksheet->write($row, 8, '=Sum(I8:I'.$row.')', $regularFormatR);
