...
/*						$sql="select STOK_AFTER from a_kartustok where OBAT_ID=$obat_id and KEPEMILIKAN_ID=$kepemilikan_id and UNIT_ID=$idunit order by TGL_TRANS desc,ID desc limit 1";
						...
						$sql="select if (sum(QTY_STOK) is null,0,sum(QTY_STOK)) as jml,IF (SUM(QTY_STOK*HARGA_BELI_SATUAN-(DISKON*QTY_STOK*HARGA_BELI_SATUAN/100)+(QTY_STOK*HARGA_BELI_SATUAN/10)) IS NULL,0,SUM((QTY_STOK*HARGA_BELI_SATUAN-(DISKON*QTY_STOK*HARGA_BELI_SATUAN/100)) * 1.1) AS ntot from a_penerimaan where OBAT_ID=$obat_id and UNIT_ID_TERIMA=$idunit and KEPEMILIKAN_ID=$kepemilikan_id and STATUS=1 and QTY_STOK>0";
						$rs=mysqli_query($konek,$sql);
						$cstk_after=0;
						$cnilai=0;
						if ($rows=mysqli_fetch_array($rs)){
						...
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,'$tgl2','$tgl_act','',$cstk_bfr,$stok,0,$stok,'Stok Opname',$iduser,$cidp,5,$cnilai)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
					}
				}
...
/*					$sql="select STOK_AFTER from a_kartustok where OBAT_ID=$obat_id and KEPEMILIKAN_ID=$kepemilikan_id and UNIT_ID=$idunit order by TGL_TRANS desc,ID desc limit 1";
					...
					$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,'$tgl2','$tgl_act','',$cstk_bfr,-$selisih,0,$stok,'Stok Opname',$iduser,0,5,$cnilai)";
					//echo $sql;
					$rs=mysqli_query($konek,$sql);
				}else{
					$selisih=$stok-$cstok;
...
/*						$sql="select STOK_AFTER from a_kartustok where OBAT_ID=$obat_id and KEPEMILIKAN_ID=$kepemilikan_id and UNIT_ID=$idunit order by TGL_TRANS desc,ID desc limit 1";
					...
					$sql="select if (sum(QTY_STOK) is null,0,sum(QTY_STOK)) as jml,IF (SUM(QTY_STOK*HARGA_BELI_SATUAN-(DISKON*QTY_STOK*HARGA_BELI_SATUAN/100)+(QTY_STOK*HARGA_BELI_SATUAN/10)) IS NULL,0,SUM((QTY_STOK*HARGA_BELI_SATUAN-(DISKON*QTY_STOK*HARGA_BELI_SATUAN/100)) * 1.1) AS ntot from a_penerimaan where OBAT_ID=$obat_id and UNIT_ID_TERIMA=$idunit and KEPEMILIKAN_ID=$kepemilikan_id and STATUS=1 and QTY_STOK>0";
						...
						$cstk_bfr=$cstk_after-$selisih;
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,'$tgl2','$tgl_act','',$cstk_bfr,$selisih,0,$stok,'Stok Opname',$iduser,$cidp,5,$cnilai)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
					}
				}
...
/*						$sql="select STOK_AFTER from a_kartustok where OBAT_ID=$obat_id and KEPEMILIKAN_ID=$kepemilikan_id and UNIT_ID=$idunit order by TGL_TRANS desc,ID desc limit 1";
				...
				if ($cstok>$stok){
					...
					$selisih=$cstok-$stok;
/*					$sql="select STOK_AFTER from a_kartustok where OBAT_ID=$obat_id and KEPEMILIKAN_ID=$kepemilikan_id and UNIT_ID=$idunit order by ID desc limit 1";
					$rs=mysqli_query($konek,$sql);
					$cstk_bfr=0;
					if ($rows=mysqli_fetch_array($rs)){
						$cstk_bfr=$rows['STOK_AFTER'];
...
/*								$sql="select STOK_AFTER from a_kartustok where OBAT_ID=$obat_id and KEPEMILIKAN_ID=$kepemilikan_id and UNIT_ID=$idunit order by TGL_TRANS desc,ID desc limit 1";
								...
								$cstk_bfr=$cstk_after-$selisih;
								$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,'$tgl2','$tgl_act','',$cstk_bfr,$selisih,0,$stok,'Stok Opname',$iduser,$cid,5,$cnilai)";
								//echo $sql;
								$rs=mysqli_query($konek,$sql);
							}
						}
