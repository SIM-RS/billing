				...
				<?php 					
					...
					function printHeader(){					
						...
						if ($iduser_jual!=""){
							...
							$qSingle = "SELECT DATE_FORMAT(pu.tgl_act,'%d-%m-%Y %H:%i:%s') tgl_balik, 
											   DATE_FORMAT( rp.tgl_retur, '%d-%m-%Y %H:%i:%s') tgl_retur, 
											   rp.no_retur, pu.no_pengembalian,
											   ap.NO_PENJUALAN, ap.NAMA_PASIEN, ap.NO_PASIEN, u.username, ap.ALAMAT, ap.DOKTER,
											   un.UNIT_NAME, m.NAMA kso_nama, ap.NO_KUNJUNGAN
										FROM a_penjualan ap
										...
										WHERE ap.NO_PENJUALAN = '{$noPenjualan}'
										  AND pu.no_pengembalian = '{$no_balik}'
										  AND ap.UNIT_ID = '{$unit}'
										  AND ap.TGL = '{$tanggal}'
					...
					function printFooter($hasil) {						
						...
						$sudahB = "SELECT IF(IFNULL(ku.TOTAL_HARGA,0) <> 0, ku.TOTAL_HARGA, ku.BAYAR_UTANG) TOTAL_HARGA, ku.TGL_BAYAR, 
						...
						if(mysqli_num_rows($qsudahB1) > 0){
							...
							$sudahB = "SELECT IF(IFNULL(ku.TOTAL_HARGA,0) <> 0, ku.TOTAL_HARGA, IF(ku.BAYAR_UTANG <> 0, ku.BAYAR_UTANG, ap.HARGA_TOTAL)) TOTAL_HARGA,
						...
						$sBNilai = "select nilai from a_pengembalian_uang pu where pu.no_pengembalian = '{$no_balik}'";
						$qBNilai = mysqli_query($konek,$sBNilai);
						$dBNilai = mysqli_fetch_array($qBNilai);

						echo "applet.append(\"             Telah Dikembalikan       : "; 
						//echo number_format($balik,0,",",".");
						$tBalik = number_format($dBNilai['nilai'],0,",","."); 
						$spasiNilaiTotal = 12 - strlen((string)$tBalik);	
						...
						echo "applet.append(\"No. Pengembalian : $no_balik\\n\")"; echo chr(59); echo "\n\t\t\t";
						if($balik > 0){
					...
					function printDataObat() {						
						...
						$sqlPrint = "SELECT DATE_FORMAT(rp.tgl_retur,'%d-%m-%Y %H:%i:%s') tgl_retur, rp.userid_retur, rp.no_retur, 
										ap.NO_PENJUALAN, o.OBAT_ID, o.OBAT_NAMA, rp.qty_retur, rp.nilai, u.username, rp.balik_uang
									FROM a_return_penjualan rp
									INNER JOIN a_penjualan ap
									   ON ap.ID = rp.idpenjualan
						...
						//echo $sqlPrint;
						$resultObat=mysqli_query($konek,$sqlPrint);
					...
					/* while($showPrint=mysqli_fetch_array($exePrint)){
						echo"
							<tr>
								...
								<td align='center'>".$showPrint['qty_retur']."</td>
								<td align='right'>".number_format($showPrint['nilai'],0,",",".")."&nbsp;&nbsp;</td>
							</tr>
						";
...
<body onLoad="deteksiPrinter()">
	...
	<div id="idArea" style="display:block;">
		<?php	
			...
			if ($iduser_jual!=""){
				$qrySingle = "SELECT DATE_FORMAT(pu.tgl_act,'%d-%m-%Y %H:%i:%s') tgl_balik, DATE_FORMAT( rp.tgl_retur, '%d-%m-%Y %H:%i:%s') tgl_retur, 
									   rp.no_retur, pu.no_pengembalian,
									   ap.NO_PENJUALAN, ap.NAMA_PASIEN, ap.NO_PASIEN, u.username, ap.ALAMAT, ap.DOKTER,
									   un.UNIT_NAME, m.NAMA kso_nama, ap.NO_KUNJUNGAN
								FROM a_penjualan ap
				...
				$qrySingle = "SELECT DATE_FORMAT(pu.tgl_act,'%d-%m-%Y %H:%i:%s') tgl_balik, rp.no_retur, pu.no_pengembalian,
									   ap.NO_PENJUALAN, ap.NAMA_PASIEN, ap.NO_PASIEN, u.username, ap.ALAMAT, ap.DOKTER,
									   un.UNIT_NAME, m.NAMA kso_nama, ap.NO_KUNJUNGAN
								FROM a_penjualan ap
								INNER JOIN a_return_penjualan rp
	...
	<div id="content">
		<div id="head-content">
			...
			<div id="head-content-right">NOTA RETUR<br /><?php echo $apname; ?></div>
			<div class="clear"></div-->
			<table id="head-table">
		...
		<div id="head-content-detil">
			<table id="head-table">
				<tr>
					<td width="168">No. Retur</td>
					<td>:</td>
					<td><?php echo $showSingle['no_retur']; ?></td>
				</tr>
				<tr>
					<td width="168">Tgl. Retur</td>
					<td>:</td>
			...
			<table id="head-table">
				...
				<tr align="right" class="border-bottom">
					<td colspan="3">Telah Dikembalikan :</td>
					<td><?php echo number_format($dBNilai['nilai'],0,",","."); ?>&nbsp;&nbsp;</td>
				</tr>
			</table>
			<table id="head-table">
				...
				<tr style="font-size:12px;">
					<td width="168">No. Pengembalian</td>
					<td>:</td>
					<td><?php 
						echo $no_balik;
					?></td>
