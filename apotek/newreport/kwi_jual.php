				...
				<div id="idArea" style="display:block;">
					...
					<tr>
						...
						if(($showSingle['cb_id'] == '2' || $showSingle['cb_id'] == '4') && (int)$ada_retur <= 0 && (int)$dBayar['bayar_'] <= 0 /*&& date("d-m-Y",strtotime($showSingle['TGL'])) == $tglNow*/ ){
							// 16-10-2019 #hafiz - hilangkan tombol delete jika lewat dari bulan berjalan
							$tglPenjualan = strtotime($tgl);
							$bulanPenjualan = date('m', $tglPenjualan);
							$bulanBerjalan = date('m');
							// jika bulan penjualan sama dengan bulan berjalan, tampilkan tombol delete
							// if ($bulanPenjualan == $bulanBerjalan) {
							if($iduser == 732) {
						?>
