...
<iframe height="72" width="130" name="sort"
	...
	<div id="view" style="display:block"> 
      <p><span class="jdltable">REKAP PENERIMAAN 
        <select name="idunit" id="idunit" class="txtinput" onchange="location='?f=../newreport/rekap_penerimaan.php&idunit='+this.value+'&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value">
      ...
      <table width="53%" border="0" align="center" cellpadding="1" cellspacing="0" class="txtinput">
        <tr> 
          ...
          <td colspan="2">: 
            <input name="tgl_d" type="text" id="tgl_d" size="11" maxlength="10" readonly="true" class="txtcenter" value="<?php if ($_GET['tgl_d']<>'') echo $_GET['tgl_d']; else echo $tgl;?>" onchange="location='?f=../apotik/rekap_penerimaan.php&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value" /></input> 
            <input type="button" name="Buttontgl_d" value=" V " class="txtcenter" onClick="gfPop.fPopCalendar(this.form.tgl_d,depRange);"/>
            </input>&nbsp;&nbsp;s/d&nbsp;&nbsp;
            <input name="tgl_s" type="text" id="tgl_s" size="11" maxlength="10" readonly="true" class="txtcenter" onchange="location='?f=../apotik/rekap_penerimaan.php&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value" value="<?php if ($_GET['tgl_s']<>'') echo $_GET['tgl_s']; else echo $tgl;?>" >
            </input> 
		...
		<tr>
			<td></td>
			<td>
				<button style="margin-left:8px;" type="button" onclick="location='?f=../newreport/rekap_penerimaan.php&idunit='+idunit.value+'&tgl_d='+tgl_d.value+'&tgl_s='+tgl_s.value"> 
					<!-- +'&jns_pasien='+jns_pasien.value+'&statuspasien='+statuspasien.value -->
      ...
      <table width="500px" align="center" border="0" cellpadding="1" cellspacing="0">
        <tr class="headtable"> 
          ...
          <td id="username" width="150" class="tblheader" onclick="ifPop.CallFr(this);">Petugas</td>
          <td id="unit" width="150" class="tblheader" onclick="ifPop.CallFr(this);">Kasir</td>
          <td id="pembayaran" width="100" class="tblheader" onclick="ifPop.CallFr(this);">Penerimaan</td>
          <td id="pengembalian" width="100" class="tblheader" onclick="ifPop.CallFr(this);">Pengembalian</td>
    ...
    <?php 
		...
		$sql = "SELECT *
				...
				LEFT JOIN (SELECT SUM(pu.nilai) total, pu.user_act, pu.unit_id
						...
						WHERE pu.status = 0
						  AND pu.tgl_act BETWEEN '{$tgl_1} 00:00:00' AND '{$tgl_2} 23:59:59'
						GROUP BY pu.user_act, pu.unit_id) pu2
				   ON pu2.user_act = ku.USER_ID
				  AND pu2.unit_id = ku.UNIT_ID
				WHERE ku.TGL_BAYAR BETWEEN '{$tgl_1} 00:00:00' AND '{$tgl_2} 23:59:59'
	...
	?>
        <tr class="itemtable" onmouseover="this.className='itemtableMOver'" onmouseout="this.className='itemtable'"> 
          ...
          <td class="tdisi" align="right"><?php echo number_format($rows['pembayaran'],0,",","."); ?></td>
          <td class="tdisi" align="right"><?php echo number_format($rows['pengembalian'],0,",","."); ?></td>
	 ...
	 // if ($rows=mysqli_fetch_array($rs))	$totresep=$rows['jml_resep'];  
	  ?>
        <tr class="itemtable"> 
          <td class="tdisikiri" align="right" colspan="5">Grand Total &nbsp; </td>
	...
	<tr>
	 ...
	 <td align="center" colspan="2">
		<BUTTON onclick="cetakNow()" type="button" <?php  if($jmldata==0) echo "disabled='disabled'"; ?>><IMG SRC="../icon/printer.png" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Cetak Rekap Penerimaan</BUTTON>
		&nbsp;
...
var req;
function cetakNow(){
	NewWindow("../newreport/laporan_rekap_penerimaan_view.php?idunit=<?php echo $idunit; ?>&tgl_d=<?php echo $tgl_1; ?>&tgl_s=<?php echo $tgl_2; ?>&filter=<?php echo $jfilter; ?>&sorting=<?php echo $sorting; ?>",'Laporan Pendapatan Farmasi',1280,600,'yes');
}
