...
function DSGridObject(id){
	...
	this.box.onscroll=function(){
		var posScroll=this.scrollHeight-(this.scrollTop/(this.scrollHeight - this.clientHeight)) * this.scrollHeight;
		document.getElementById('hdrbox_'+id).scrollLeft=this.scrollLeft;
		document.getElementById('subTotbox_'+id).scrollLeft=this.scrollLeft;
		//alert(this.style.height);
	}
	this.box.style.height=(parseInt(this.grid.gridboxh)-parseInt(this.grid.hdrcolh)-parseInt(this.grid.hdrcaptionh)-1-25) + 'px';
	this.box.appendChild(this.obj);
	this.grid.appendChild(this.box);
	//============== tambahan ==================//
	this.objsubTot = document.createElement("TABLE"); this.objsubTot.cellSpacing = 0; this.objsubTot.cellPadding = 0; this.objsubTot.style.width = "100%"; this.objsubTot.style.tableLayout = "fixed"; this.objsubTot.style.borderCollapse = 'collapse'; this.objsubTot.border = 0;
    r = this.objsubTot.insertRow(0);r.id="subTotcol_"+id;r.className = "GridSubTotal"; r.style.height = "20px";
	this.grid.subTotcolh=r.style.height.substr(0,r.style.height.length-2);
	this.boxSubTot = document.createElement("DIV");
	//alert(this.box.scrollTop);
	this.boxSubTot.id='subTotbox_'+id;this.boxSubTot.style.width='100%';this.boxSubTot.style.overflow='hidden';
	this.boxSubTot.appendChild(this.objsubTot);
	this.grid.appendChild(this.boxSubTot);
	//============== tambahan ==================//
	...
	this.setColHeader=function(str){
		...
		//var cRow = tbl.rows.length;
		var cRow = document.getElementById('hdrcol_'+id);
		//alert(cRow + cRow.childNodes.length);
		if (cRow.childNodes.length>0){
			//alert(cRow.childNodes.length);
			for (var i=cRow.childNodes.length-1;i>=0;i--){
				document.getElementById('hdrcol_'+id).deleteCell(i);
			}
			for (var i=0;i<a.length;i++){
				document.getElementById('hdrcol_'+id).insertCell(i).innerHTML = a[i];
				if (this.cellAlignIsEmpty==true) this.cellAlignArr[i]='center';
	...
	this.setSubTotal=function(str){
		var a=str.split(",");
		for (var i=0;i<a.length;i++){
			document.getElementById('subTotcol_'+id).insertCell(i).innerHTML = a[i];
		}
		document.getElementById('subTotcol_'+id).insertCell(i).innerHTML = '&nbsp;';
	}
	this.setSubTotalAlign=function(str){
		var a=str.split(",");
		for (var i=0;i<a.length;i++){
			document.getElementById('subTotcol_'+id).childNodes[i].style.textAlign = a[i];
		}
		/*document.getElementById('subTotcol_'+id).style.backgroundColor='#EAEFF3';
		document.getElementById('subTotcol_'+id).style.color='#000066';
		document.getElementById('subTotcol_'+id).style.fontSize = '11px';*/
	}
	this.cellSubTotalSetValue=function(col,p){
		var crow=document.getElementById('subTotcol_'+id);
		crow.childNodes[col-1].innerHTML=p;
	}
	this.cellSubTotalGetValue=function(col){
		var crow=document.getElementById('subTotcol_'+id);
		return crow.childNodes[col-1].innerHTML;
	}
	/*this.setSubTotalColWidth=function(str){
		var a=str.split(",");
		for (var i=0;i<a.length;i++){
			document.getElementById('subTotcol_'+id).childNodes[i].style.width = a[i]+'px';
		}
		document.getElementById('subTotcol_'+id).childNodes[a.length-1].style.width = parseInt(a[a.length-1])+20+'px';
	}*/
	this.setColWidth=function(str){
		this.ColWidthArr=str.split(",");
		for (var i=0;i<this.ColWidthArr.length;i++){
			document.getElementById('hdrcol_'+id).childNodes[i].style.width = this.ColWidthArr[i]+'px';
			if (document.getElementById('subTotcol_'+id).childNodes.length>0){
				document.getElementById('subTotcol_'+id).childNodes[i].style.width = this.ColWidthArr[i]+'px';
			}			
		}
		...
		if (document.getElementById('subTotcol_'+id).childNodes.length>0){
			//document.getElementById('subTotcol_'+id).childNodes[this.ColWidthArr.length-1].style.width = parseInt(this.ColWidthArr[this.ColWidthArr.length-1])+20+'px';
			document.getElementById('subTotcol_'+id).childNodes[this.ColWidthArr.length].style.width = '12px';
		}else{
	...
	this.cellsGetChecked=function(row,col){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows[row-1];
		
		return crow.childNodes[col-1].childNodes[0].checked;
	}
	this.cellsSetValue=function(row,col,p){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows[row-1];
		if (this.cellTypeArr){
			//alert(this.cellTypeArr[col-1]);
			if (this.cellTypeArr[col-1]=="txt"){
				crow.childNodes[col-1].innerHTML=p;
			}else if (this.cellTypeArr[col-1]=="chk"){
				crow.childNodes[col-1].childNodes[0].value=p;
				if (parseInt(p)==1){
					crow.childNodes[col-1].childNodes[0].checked=true;
				}else{
					crow.childNodes[col-1].childNodes[0].checked=false;
				}
			}else{
				crow.childNodes[col-1].childNodes[0].value=p;
			}
	...
	this.cellsGetLang=function(row,col){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows[row-1];
		var tmp1;
		
		if (this.cellTypeArr){
		  tmp1 = (this.cellTypeArr[col-1]=="txt")?crow.childNodes[col-1].lang:crow.childNodes[col-1].childNodes[0].lang;
		  while (tmp1.indexOf('&amp;')>0){
		      tmp1=tmp1.replace('&amp;','&');
		  }
		  while (tmp1.indexOf('&gt;')>0){
		  ...
		  tmp1 = crow.childNodes[col-1].lang;
		  while (tmp1.indexOf('&amp;')>0){
		      tmp1=tmp1.replace('&amp;','&');
		  }
		  while (tmp1.indexOf('&gt;')>0){
	...
	this.cellsSetLang=function(row,col,p){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows[row-1];
		if (this.cellTypeArr){
			if (this.cellTypeArr[col-1]=="txt"){
				crow.childNodes[col-1].lang=p;
			}else{
				crow.childNodes[col-1].childNodes[0].lang=p;
			}
		}else{
			crow.childNodes[col-1].lang=p;
		}
	...
	this.setRowId=function(row,p){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows[row-1];
		if(crow!=undefined){
		  crow.id=crow.id.substr(0,id.length+4)+p;
		}
	}
	...
	this.setSorting=function(par1){
		document.getElementById(id).sorting = par1;
	}
	this.getSorting=function(){
		return document.getElementById(id).sorting;
	}
	this.setFilter=function(par2){
		document.getElementById(id).filter = par2;
	}
	...
	this.AddRowItem=function(str){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows.length;
		var tmpcData=tbl.innerHTML;
	...
	//alert(this.cellHeight);
		...
		cell=str.split(String.fromCharCode(3));
		//alert(document.getElementById('tblgrid_'+id).innerHTML);
		if (crow%2) clsname="GridItemEven"; else clsname="GridItemOdd";
		dt="<tr id=\""+id+"item"+cell[0]+"\" height=\""+q+"\" lang=\""+(crow+1)+"\" class=\""+clsname+"\" onclick=\"ItemTblMClick(this,'"+id+"');"+onclicke+"\" ondblclick=\""+ondoubleclicke+"\" onmouseover=\"ItemTblMOver(this);\" onmouseout=\"ItemTblMOut(this,'"+id+"');\">";
		for (var j=1;j<cell.length;j++){
			if (typecell[j-1]=="txt"){
				itemCol=cell[j];
			}else if (typecell[j-1]=="chk"){
				itemCol="<input type=\"checkbox\" lang=\"\" value=\""+cell[j]+"\""+((parseInt(cell[j])==1)?" checked='checked'":"")+" />";
			}else if (typecell[j-1]=="txtbox"){
				itemCol="<input type=\"text\" class=\"txtinput\" id=\""+id+"item"+cell[0]+(j-1)+"\" style=\"text-align:"+alg[j-1]+"\" lang=\"\" value=\""+cell[j]+"\" size=\"8\" />";
			}else{
				itemCol=cell[j];
			}
			dt+="<td width=\""+r[j-1]+"\" align=\""+alg[j-1]+"\" lang=\"\">"+itemCol+"</td>";
		}
		dt+="</tr>";
		tmpcData=tmpcData.replace("<tbody>","");
		tmpcData=tmpcData.replace("</tbody>","");
		tmpcData += dt;
		document.getElementById('tblgrid_'+id).innerHTML=tmpcData;
		//alert(document.getElementById('tblgrid_'+id).innerHTML);
	}
	this.DeleteRowItem=function(row){
		var tbl=document.getElementById('tblgrid_'+id);
		var crow=tbl.rows[row-1];
		if(crow==undefined){
		  return '';
			...
			for (var i=1;i<=lastRow;i++){
				var tds = tbl.rows[i-1].getElementsByTagName('td');
				tds[0].innerHTML=i;
			}
		}		
	}
						    ...
						    if(lstData != this.pdata[1]){
								...
								if (this.rows.length>0){
									...
									for (var i=1;i<cell.length;i++){
										if (typecell[i-1]=="txt"){
											itemCol=cell[i];
										}else if (typecell[i-1]=="chk"){
											itemCol="<input type=\"checkbox\" lang=\"\" value=\""+cell[i]+"\""+((parseInt(cell[i])==1)?" checked='checked'":"")+" />";
										}else if (typecell[i-1]=="txtbox"){
											itemCol="<input type=\"text\" class=\"txtinput\" id=\""+id+"item"+cell[0]+(i-1)+"\" style=\"text-align:"+alg[i-1]+"\" lang=\"\" value=\""+cell[i]+"\" size=\"8\" />";
										}else{
											itemCol=cell[i];
										}
										dt+="<td width=\""+r[i-1]+"\" align=\""+alg[i-1]+"\" lang=\"\">"+itemCol+"</td>";
									}
									dt+="</tr>";
									for (var i=1;i<this.rows.length;i++){
										...
										for (var j=1;j<cell.length;j++){
											...
											dt+="<td align=\""+alg[j-1]+"\" lang=\"\">"+itemCol+"</td>";
										}
										dt+="</tr>";
									}
								}else{
								...
								if (idpage!=undefined){
								  ...
								  else if(document.getElementById(id).page>document.getElementById(id).maxpage){
								    document.getElementById(id).page=1;
								  }
