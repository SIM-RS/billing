	...
	if($_REQUEST['status'] == "0"){
		/* $sql = "SELECT * 
				FROM
				  ...
				  FROM
					$dbbilling.b_resep 
					LEFT JOIN $dbapotek.a_penjualan ap
		...
		/*$sql = "SELECT *
				FROM
				(SELECT 
				  NULL id, p.DOKTER dokter_nama, p.NO_KUNJUNGAN id_pelayanan, p.NO_RESEP no_resep, p.NO_PASIEN no_rm, p.NAMA_PASIEN nama_pasien,
				  t1.nama unit, k.NAMA kepemilikan, t1.ksoNm kso, p.CARA_BAYAR cara_bayar_id, cb.nama cara_bayar, k.ID kepemilikanId, p.NO_PENJUALAN n_penj,
				  p.TGL tgltrans, DATE_FORMAT(p.TGL, '%d-%m-%Y') tglll, t1.umur, t1.tgl_lahir, t1.no_ktp, t1.telp, t1.sex, p.ALAMAT alamat, t1.pasien_id, t1.alamat_, NULL iter, p.NO_KUNJUNGAN NOKUNJUNGAN
				FROM
				  ...
				  INNER JOIN 
					(SELECT 
					  pel.id, pel.unit_id, u.nama, kso.nama ksoNm, pel.pasien_id,
					  DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(mp.tgl_lahir, '%Y') - (
						DATE_FORMAT(NOW(), '00-%m-%d') < DATE_FORMAT(mp.tgl_lahir, '00-%m-%d')
					  ) umur,
					...
					FROM
					  ...
					  LEFT JOIN $dbbilling.b_ms_unit u ON u.id = pel.unit_id 
					  LEFT JOIN $dbbilling.b_ms_kso kso ON kso.id = pel.kso_id 
					  INNER JOIN $dbbilling.b_ms_pasien mp ON mp.id = pel.pasien_id 
					  LEFT JOIN $dbbilling.b_ms_wilayah w ON mp.desa_id = w.id 
					  LEFT JOIN $dbbilling.b_ms_wilayah wi ON mp.kec_id = wi.id 
					  LEFT JOIN $dbbilling.b_ms_wilayah wii ON mp.kab_id = wii.id) t1 
					ON t1.id = p.NO_KUNJUNGAN 
				  INNER JOIN a_cara_bayar cb ON cb.id = p.CARA_BAYAR 
				WHERE {$ftgl1} AND p.UNIT_ID = '$idunit' 
				GROUP BY p.NO_RESEP, p.NO_PASIEN, p.NO_PENJUALAN, p.NO_KUNJUNGAN) AS gab {$filter} 
				ORDER BY ".$sorting;*/
		$sql="SELECT *
				FROM
				(SELECT 
