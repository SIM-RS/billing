...
switch(strtolower($_REQUEST['act']))
{
  ...
  case 'simpan':
	...
	$sql="SELECT DISTINCT r.no_resep,r.no_rm,IFNULL(m.IDMITRA,2) kso_id,r.cara_bayar,r.kepemilikan_id,r.dokter_nama,
			r.nama_pasien,r.alamat,u.unit_billing,IFNULL(u.UNIT_ID,0) UNIT_ID,CURDATE() tgl,NOW() tgl_act,r.kronis kronis,r.jamin_kso, u.status_inap
	...
	if($statusInap == 0){
		$ppn = 10;
		$ppn_nilai = ROUND($tot_harga * 0.1);
		$hargaTotalPPN = $ppn_nilai + $tot_harga;
	}
	...
	//$nokw="000001";
	$cekNo = "select MAX(nomor) NO_PENJUALAN from a_no_penjualan_log where unit_id=$idunit and tahun=$artgl_act[0]"; //order by id DESC limit 1
	$qcekNo = mysqli_query($konek,$cekNo);
	$dcekNo = mysqli_fetch_array($qcekNo);

	$sql="select  MAX(CAST(NO_PENJUALAN AS UNSIGNED)) AS NO_PENJUALAN from $dbapotek.a_penjualan where UNIT_ID=$idunit and YEAR(TGL)=$artgl_act[0] ORDER BY ID DESC LIMIT 1";
...
;
	...
	/* if ($rows=mysqli_fetch_array($rs)){
		...
		$updateNO = "insert into a_no_penjualan_log(nomor,unit_id,tahun) value('{$nokw}','$idunit','$artgl_act[0]')";
...
//	$no_penjualan=$nokw;
	...
	for ($j=0;$j<count($arfdata);$j++){
			...
			if($kso_id != 1) $dijamin = 1;

			$sql="select * from $dbbilling.b_resep WHERE id = '".$data[0]."'";
			//echo $sql."<br>";
			$rs=mysqli_query($konek,$sql) or die(mysqli_error($konek." - 227"));;
			$rw=mysqli_fetch_array($rs);
			...
			if($kp_id!=$data[4]){
				...
				//echo $sql."<br>";
				$rs=mysqli_query($konek,$sql) or die(mysqli_error($konek." - 235"));;
			}
			...
			// echo $sql."<br>";
			$rs=mysqli_query($konek,$sql) or die(mysqli_error($konek." - 240"));
			$done="false";
			$jml=$qty;
			//$success=1;
			$cid=0;
			...
			while (($rows=mysqli_fetch_array($rs))&&($done=="false"))
			{
				...
				if ($cstok>=$jml){
					...
					$rs1=mysqli_query($konek,$sql) or die(mysqli_error($konek." - 262"));
					$sql="insert into $dbapotek.a_penjualan(PENERIMAAN_ID,UNIT_ID,USER_ID,JENIS_PASIEN_ID,KSO,KSO_ID,TGL,TGL_ACT,NO_PENJUALAN,NO_KUNJUNGAN,NO_PASIEN,NO_RESEP,DOKTER,RUANGAN,SHIFT,CARA_BAYAR,QTY_JUAL,QTY,BIAYA_RETUR,HARGA_NETTO,HARGA_SATUAN,SUB_TOTAL,SUM_SUB_TOTAL,EMBALAGE,JASA_RESEP,HARGA_TOTAL,KET_DOSIS,STATUS,NAMA_PASIEN,ALAMAT,UTANG,RACIKAN,textH,textT,textK,textP,KRONIS,DIJAMIN,PPN,PPN_NILAI,HARGA_TOTAL_PPN,HARGA_KSO) values
					($cid,$idunit,$iduser,$kp_id,$kso,$kso_id,'$tgltrans',$tglJualFix,'$no_penjualan','$no_kunj','$NoRM','$NoResep','$dokter','$ruangan','$shft',$cara_bayar,$jml,$jml,$biaya_retur,$hNetto,$hSatuan,$jml*$hSatuan,$subtotal,$embalage,$jasa_resep,$tot_harga,'$ketDosis',1,'$nm_pasien','$txtalamat',$nutang,$isRacikan,'$txtH','$txtT','$txtK','$txtP','$kronis','$dijamin',$ppn,$ppn_nilai,$hargaTotalPPN,$harga_kso)";
					// echo $sql."<br>";
					$rs1=mysqli_query($konek,$sql) or die(mysqli_error($konek." - 266"));
					if($rs1 == FALSE){
						echo mysql_error();
					}
					$idInsert[]=mysqli_insert_id($konek);
					if (mysqli_errno($konek)>0){
						$success=0;
						$sql="update $dbapotek.a_penerimaan set QTY_STOK=QTY_STOK+$jml where ID=$cid";
						//echo $sql."<br>";
						$rs1=mysqli_query($konek,$sql) or die(mysqli_error($konek." - 272"));
					} else {
						$success=1;
					}
				}else{
					...
					$rs1=mysqli_query($konek,$sql);
					$sql="insert into $dbapotek.a_penjualan(PENERIMAAN_ID,UNIT_ID,USER_ID,JENIS_PASIEN_ID,KSO,KSO_ID,TGL,TGL_ACT,NO_PENJUALAN,NO_KUNJUNGAN,NO_PASIEN,NO_RESEP,DOKTER,RUANGAN,SHIFT,CARA_BAYAR,QTY_JUAL,QTY,BIAYA_RETUR,HARGA_NETTO,HARGA_SATUAN,SUB_TOTAL,SUM_SUB_TOTAL,EMBALAGE,JASA_RESEP,HARGA_TOTAL,KET_DOSIS,STATUS,NAMA_PASIEN,ALAMAT,UTANG,RACIKAN,textH,textT,textK,textP,KRONIS,HARGA_KSO,PPN,PPN_NILAI,HARGA_TOTAL_PPN) values
					($cid,$idunit,$iduser,$kp_id,$kso,$kso_id,'$tgltrans',$tglJualFix,'$no_penjualan','$no_kunj','$NoRM','$NoResep','$dokter','$ruangan',$shft,$cara_bayar,$cstok,$cstok,$biaya_retur,$hNetto,$hSatuan,$cstok*$hSatuan,$subtotal,$embalage,$jasa_resep,$tot_harga,'$ketDosis',1,'$nm_pasien','$txtalamat',$nutang,$isRacikan,'$txtH','$txtT','$txtK','$txtP','$kronis',$harga_kso,$ppn,$ppn_nilai,$hargaTotalPPN)";
					//echo $sql."<br>";
					$rs1=mysqli_query($konek,$sql);
					$idInsert[]=mysqli_insert_id($konek);
					if (mysqli_errno($konek)>0){
	...
	if($_REQUEST['status'] == "0"){
		...
		$sql = "SELECT * 
				FROM
				  (SELECT 
					...
					$dbbilling.b_resep.kso_id,
					$dbbilling.b_resep.diagnosa_id,
					$dbbilling.b_ms_unit.nama AS unit,
					$dbapotek.a_kepemilikan.NAMA AS kepemilikan,
					$dbbilling.b_ms_kso.nama AS kso,
					$dbapotek.a_cara_bayar.id cara_bayar_id,
					$dbapotek.a_cara_bayar.nama cara_bayar,
					$dbapotek.a_kepemilikan.ID AS kepemilikanId,
					0 n_penj, NULL tgltrans,
					...
					mp.no_ktp, mp.telp, mp.sex,
					$dbbilling.b_resep.alamat, k.pasien_id,
					CONCAT( ( CONCAT( ( CONCAT( (CONCAT(mp.alamat, ' RT.', mp.rt)), ' RW.', mp.rw ) ), ', Desa ', w.nama ) ), ', Kecamatan ', wi.nama, ', ', wii.nama ) alamat_,
				  ...
				  FROM
					...
					INNER JOIN $dbapotek.a_kepemilikan 
					  ON $dbbilling.b_resep.kepemilikan_id = $dbapotek.a_kepemilikan.ID 
					INNER JOIN $dbapotek.a_cara_bayar 
					  ON $dbbilling.b_resep.cara_bayar = $dbapotek.a_cara_bayar.id 
					INNER JOIN $dbbilling.b_ms_unit 
					  ON $dbbilling.b_ms_unit.id = p.unit_id 
				  ...
				  GROUP BY $dbbilling.b_resep.no_resep,
					...
					$dbbilling.b_resep.id_pelayanan{$fgroup}) AS gab $filter ORDER BY ".$sorting;
				//	echo $sql;
	} else {
		...
		$sql="SELECT *
				...
				(SELECT 
				  ...
				  (SELECT nama FROM $dbbilling.b_ms_unit WHERE id=pel.unit_id) unit,
				  k.NAMA kepemilikan,
				  (SELECT nama FROM $dbbilling.b_ms_kso WHERE id=pel.kso_id) kso,
				  ...
				  DATE_FORMAT(mp.tgl_lahir, '%d-%m-%Y') tgl_lahir,
				  mp.no_ktp, mp.telp, mp.sex, p.ALAMAT alamat, pel.pasien_id,
				  CONCAT((CONCAT((CONCAT((CONCAT(mp.alamat, ' RT.', mp.rt)),' RW.',mp.rw)),', Desa ',
				  (SELECT nama FROM $dbbilling.b_ms_wilayah WHERE id=mp.desa_id))),
				  ', Kecamatan ',(SELECT nama FROM $dbbilling.b_ms_wilayah WHERE id=mp.kec_id),', ',
				  (SELECT nama FROM $dbbilling.b_ms_wilayah WHERE id=mp.kab_id)) alamat_,
				  NULL iter,
				...
				FROM
				  ...
				  INNER JOIN $dbbilling.b_pelayanan pel
					ON p.NO_KUNJUNGAN=pel.id
				  INNER JOIN $dbbilling.b_ms_pasien mp
					ON pel.pasien_id=mp.id
				WHERE {$ftgl1}
...
IF($dbbilling.b_resep.racikan=0,$dbbilling.b_resep.qty,IF($dbbilling.b_resep.obat_id=0,$dbbilling.b_resep.qty,0)) qty,$dbbilling.b_resep.racikan as racikan2,$dbbilling.b_resep.kepemilikan_id,$dbbilling.b_resep.status,$dbbilling.b_resep.id_pelayanan,$dbbilling.b_resep.no_rm,$dbbilling.b_resep.no_resep,$dbbilling.b_resep.ket_dosis, $dbbilling.b_resep.kso_id kso_id_billing, b_resep.kronis kronis,b_resep.jamin_kso jamin_kso, concat('(',$dbbilling.b_ms_bobat.nama,')') as nm_bentuk, IF(sdtd = 1,'dtd','') AS dtd
...
if($grd == "true")
{
	...
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$sisip=$rows["id"].'|'.$rows["kepemilikanId"]."|".$rows["no_resep"]."|".$rows["no_rm"]."|".$rows["id_pelayanan"]."|".$rows["n_penj"]."|".$rows["tgltrans"]."|".$rows["cara_bayar_id"]."|".$rows["nama_pasien"]."|".$rows["sex"]."|".$rows["umur"]."|".$rows["tgl_lahir"]."|".$rows["alamat_"]."|".$rows["telp"]."|".$rows["no_ktp"]."|".$rows["no_rm"]."|".$rows["tglll"]."|".$diagnosa."|".$rows["kso_id"]; //."|".$rows["txtH"]."|".$rows["txtT"]."|".$rows["txtK"]."|".$rows["txtP"]
...
{	
	...
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$sql="SELECT * FROM a_mitra WHERE kso_id_billing=".$rows["kso_id_billing"]."";
		$rs1=mysqli_query($konek,$sql);
		$rw1=mysqli_fetch_array($rs1);
		$profit=$rw1["MARGIN_HJUAL"];
		
		$i++;
		$j++;
		if($_REQUEST['status'] == "0"){
			$sqlHarga="SELECT FLOOR(max(HARGA_BELI_SATUAN)+($profit*max(HARGA_BELI_SATUAN)/100)) HARGA_JUAL_SATUAN, FLOOR(HARGA_JUAL_SATUAN)  hapus,FLOOR(HARGA_BELI_SATUAN) HARGA_BELI_SATUAN FROM a_harga WHERE OBAT_ID=".$rows["OBAT_ID"]." AND KEPEMILIKAN_ID=".$rows["kepemilikan_id"]." ORDER BY HARGA_JUAL_SATUAN DESC LIMIT 1";
		...
		$sisip=$rows["id"]."|".$hSatuan."|".$hNetto."|".$rows["qty"]."|".$rows["kepemilikan_id"]."|".$rows["OBAT_ID"]."|".$rows["isRacikan"]."|".$rows["textH"]."|".$rows["textT"]."|".$rows["textK"]."|".$rows["textP"]."|".$rows["kronis"]."|".$rows["jamin_kso"];
