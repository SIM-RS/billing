	...
	for ($i = 0; $i < count($qty_satuan); $i++)
	 {
		...
		$sql="SELECT a.* FROM a_penjualan a INNER JOIN a_penerimaan b ON a.PENERIMAAN_ID=b.ID WHERE a.UNIT_ID=$idunit AND a.NO_PENJUALAN='$no_penjualan[$i]' AND b.OBAT_ID=$obat_id[$i] AND a.QTY_JUAL>a.QTY_RETUR AND a.TGL BETWEEN '$tgl_se' AND '$tgl_de' ORDER BY ID DESC";
		//echo $sql."<br>";
		$rs=mysqli_query($konek,$sql);
		$cjml=$qty_satuan[$i];
		$ok="false";
		while (($rows=mysqli_fetch_array($rs))&&($ok=="false")){
			$cqty=$rows["QTY_JUAL"]-$rows["QTY_RETUR"];
			$cidp=$rows["ID"];
	...
	if (document.forms[0].pilihobat.length){
		for (i=0;i<document.forms[0].pilihobat.length;i++){
			if (document.forms[0].pilihobat[i].checked){
				...
				//p=p.replace(".","");
				//p=p.replace(",",".");
				if (parseFloat(document.getElementById('qty_satuan'+i.toString()).value)>parseFloat(p)){
		...
		if (document.forms[0].pilihobat.checked){
			...
			p=tds[9].innerHTML;
			if (parseFloat(document.getElementById('qty_satuan0').value)>parseFloat(p)){
				alert('Jumlah Item Obat Yang diReturn Lebih Besar dr Sisa Stock');
				return false;
			}
		}
...
?>
<div id="divobat" align="left" style="position:absolute; z-index:1; left: 700px; width:325px; top: 25px; height: 230px; overflow: scroll; border:1px solid; display:none; background-color: #CCCCCC; layer-background-color: #CCCCCC;"></div>
<div align="center"> 
  <div id="input" style="display:block">
	...
	<input type="hidden" name="filter" id="filter" value="<?php echo $filter; ?>">
      <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="txtinput">
        <tr>
            <td width="151">Nama Pasien</td>
            <td width="12">:</td>
            <td colspan="2"><input name="nama_pasien" size="50" id="nama_pasien"<?php if (strpos($nama_pasien1,'"')>0) echo "value='".$nama_pasien1."'"; else echo 'value="'.$nama_pasien1.'"'; ?> class="txtinput" onKeyUp="suggest(event,this,1);" autocomplete="off"></td>
        </tr>
		  <tr>
      ...
      <table id="tblRetur" width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <?php
		   ...
		   $sql="SELECT a.TGL,a.NO_PENJUALAN,b.OBAT_ID,a.JENIS_PASIEN_ID,a.BIAYA_RETUR,a.HARGA_SATUAN,SUM(a.QTY_JUAL) AS QTY_JUAL,SUM(a.QTY_RETUR) AS QTY_RETUR,
				...
				ON b.OBAT_ID = c.OBAT_ID WHERE a.NAMA_PASIEN ='$nama_pasien' AND a.UNIT_ID=$idunit 
				AND a.TGL BETWEEN '$tgl_se' AND '$tgl_de'".$filter." GROUP BY c.OBAT_ID,a.NO_PENJUALAN,c.OBAT_NAMA ORDER BY ".$sorting;
		   //echo $sql."<br>";
      ...
      <p align="center">
        ...
        <BUTTON type="button" onClick="if(ValidateForm('nama_pasien','ind')){fSubmit();}"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Simpan&nbsp;</BUTTON>
