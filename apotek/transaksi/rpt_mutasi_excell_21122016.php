...
if ($bentuk=="0") $fbentuk=""; else $fbentuk=" WHERE ao.OBAT_BENTUK='$bentuk'";
...
if ($idunit1=="0"){
	...
	$worksheet->mergeCells($row,$column+11,$row,$column+16);
	$worksheet->write($row, $column+17, "Adj", $columnTitleFormat);
	$worksheet->write($row, $column+18, "Saldo Akhir", $columnTitleFormat);
	$worksheet->writeBlank($row, $column+19, $columnTitleFormat);
	$worksheet->mergeCells($row,$column+18,$row,$column+19);
}else{
	$worksheet->write($row, $column, "No", $columnTitleFormat);
	$worksheet->write($row, $column+1, "Nama Obat", $columnTitleFormat);
	$worksheet->write($row, $column+2, "Kategori", $columnTitleFormat);
	...
	$worksheet->writeBlank($row, $column+8, $columnTitleFormat);
	$worksheet->writeBlank($row, $column+9, $columnTitleFormat);
	$worksheet->mergeCells($row,$column+6,$row,$column+9);
	$worksheet->write($row, $column+10, "Keluar", $columnTitleFormat);
	$worksheet->writeBlank($row, $column+11, $columnTitleFormat);
	$worksheet->writeBlank($row, $column+12, $columnTitleFormat);
	$worksheet->writeBlank($row, $column+13, $columnTitleFormat);
	$worksheet->writeBlank($row, $column+14, $columnTitleFormat);
	$worksheet->mergeCells($row,$column+10,$row,$column+14);
	$worksheet->write($row, $column+15, "Adj", $columnTitleFormat);
	$worksheet->write($row, $column+16, "Saldo Akhir", $columnTitleFormat);
	$worksheet->writeBlank($row, $column+17, $columnTitleFormat);
	$worksheet->mergeCells($row,$column+16,$row,$column+17);
}
$row += 1;
...
if ($idunit1=="0"){
	...
	$worksheet->write($row, $column+12, "Prod", $columnTitleFormat);
	$worksheet->write($row, $column+13, "Unit", $columnTitleFormat);
	$worksheet->write($row, $column+14, "Ret", $columnTitleFormat);
	$worksheet->write($row, $column+15, "Milik", $columnTitleFormat);
	$worksheet->write($row, $column+16, "Hapus", $columnTitleFormat);
	$worksheet->writeBlank($row, $column+17, $columnTitleFormat);
	$worksheet->mergeCells($row-1,$column+17,$row,$column+17);
	$worksheet->write($row, $column+18, "Qty", $columnTitleFormat);
	...
	if ($idunit1=="17")
		$worksheet->write($row, $column+10, "Prod", $columnTitleFormat);
	else
		$worksheet->write($row, $column+10, "Jual", $columnTitleFormat);
		
	$worksheet->write($row, $column+11, "Unit", $columnTitleFormat);
	$worksheet->write($row, $column+12, "Ret", $columnTitleFormat);
	$worksheet->write($row, $column+13, "Milik", $columnTitleFormat);
	$worksheet->write($row, $column+14, "Hapus", $columnTitleFormat);
	$worksheet->writeBlank($row, $column+15, $columnTitleFormat);
	$worksheet->mergeCells($row-1,$column+15,$row,$column+15);
	$worksheet->write($row, $column+16, "Qty", $columnTitleFormat);
...
if ($idunit1=="0"){
	while ($rows=mysqli_fetch_array($rs)){
		...
		if ($qty_trans<>0){
			...
			$worksheet->write($row, 12, $rows['PRODUKSI_OUT'], $regularFormatR);
			$worksheet->write($row, 13, $rows['UNIT_OUT'], $regularFormatR);
			$worksheet->write($row, 14, $rows['RETURN_OUT'], $regularFormatR);
			$worksheet->write($row, 15, $rows['MILIK_OUT'], $regularFormatR);
			$worksheet->write($row, 16, $rows['HAPUS'], $regularFormatR);
			$worksheet->write($row, 17, $rows['ADJUST'], $regularFormatR);
			$worksheet->write($row, 18, $qty_akhir, $regularFormatR);
			$worksheet->write($row, 19, $nilai_akhir, $regularFormatRF);
			$row++;
		}
	}
	...
	$worksheet->write($row, 15, "Total Saldo Akhir", $regularFormatR);
	...
	$worksheet->mergeCells($row,15,$row,18);
	$worksheet->write($row, 19, '=Sum(T10:T'.$row.')', $regularFormatRF);
}else{
	while ($rows=mysqli_fetch_array($rs)){
		...
		if ($qty_trans<>0){
			...
			if ($idunit1=="17")
				$worksheet->write($row, 10, $rows['PRODUKSI_OUT'], $regularFormatR);
			else
				$worksheet->write($row, 10, $rows['JUAL'], $regularFormatR);
				
			$worksheet->write($row, 11, $rows['UNIT_OUT'], $regularFormatR);
			$worksheet->write($row, 12, $rows['RETURN_OUT'], $regularFormatR);
			$worksheet->write($row, 13, $rows['MILIK_OUT'], $regularFormatR);
			$worksheet->write($row, 14, $rows['HAPUS'], $regularFormatR);
			$worksheet->write($row, 15, $rows['ADJUST'], $regularFormatR);
			$worksheet->write($row, 16, $qty_akhir, $regularFormatR);
			$worksheet->write($row, 17, $nilai_akhir, $regularFormatRF);
			$row++;
		}
	}
	$worksheet->write($row, 3, "Total Saldo Awal", $regularFormatR);
	...
	$worksheet->writeBlank($row, 16, $regularFormatR);
	$worksheet->mergeCells($row,13,$row,16);
	$worksheet->write($row, 17, '=Sum(R10:R'.$row.')', $regularFormatRF);
}
