...
$bentuk=$_REQUEST['bentuk'];if ($bentuk=="") $fbentuk=""; else $fbentuk=" AND ao.OBAT_BENTUK='$bentuk'";
...
if($bentuk == "")
{
	...
	$query = "SELECT * FROM a_bentuk WHERE BENTUK = '$bentuk'";
	$exec = mysqli_query($konek,$query);
	$data = mysqli_fetch_array($exec);
	$nmbentuk = $data['BENTUK'];
}
...
$row += 1;
$worksheet->write($row, 1, "BENTUK : $nmbentuk", $sheetTitleFormat);
...
while ($rows=mysqli_fetch_array($rs)){
	...
	$worksheet->write($row, 4, floor($rows['nilai']), $regularFormatR);
	$row++;
}
