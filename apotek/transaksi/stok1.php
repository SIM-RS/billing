	...
	case "save":
		...
		if ($rows=mysqli_fetch_array($rs)){
			...
			if ($rows1=mysqli_fetch_array($rs1)){
				...
				if ($cjml>0){
					...
					if ($hargab==0){
						...
						$sql="update a_stok_opname set idpenerimaan=$cidp,opname=$stok,adjust=$stok,qty=$stok,harga_satuan=$hargab,tgl='$tgl2',tgl_act='$tgl_act' where idunit=$idunit and idobat=$obat_id and kepemilikan_id=$kepemilikan_id";
						//echo $sql."<br>";
						$rs=mysqli_query($konek,$sql);
						/*$sql="select SQL_NO_CACHE ifnull(sum(QTY_STOK),0) as jml,IFNULL(SUM(QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)) * 1.1),0) AS ntot from a_penerimaan where OBAT_ID=$obat_id and UNIT_ID_TERIMA=$idunit and KEPEMILIKAN_ID=$kepemilikan_id and STATUS=1 and QTY_STOK>0";*/
						...
						$cstk_bfr=$cstk_after-$stok;
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$stok,0,$stok,'Stok Opname',$iduser,$cidp,5,$cnilai)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
					}
				}
			...
			if ($hargab==0){
				...
				/*$sql="select SQL_NO_CACHE if (sum(QTY_STOK) is null,0,sum(QTY_STOK)) as qty_stok from a_penerimaan where OBAT_ID=$obat_id and UNIT_ID_TERIMA=$idunit and KEPEMILIKAN_ID=$kepemilikan_id and QTY_STOK>0 and STATUS=1";*/
				$sql="SELECT SQL_NO_CACHE IFNULL(SUM(t1.stok_after),0) AS qty_stok FROM (SELECT SQL_NO_CACHE UUID() AS cuid,IFNULL(QTY_STOK,0) AS stok_after FROM a_penerimaan WHERE OBAT_ID=$obat_id AND KEPEMILIKAN_ID=$kepemilikan_id AND UNIT_ID_TERIMA=$idunit AND QTY_STOK>0 AND STATUS=1) AS t1";
				//echo $sql;
				$rs=mysqli_query($konek,$sql);
				$cstok=0;
				if ($rows=mysqli_fetch_array($rs)){
				...
				if ($cstok>$stok){
					...
					$selisih=$cstok-$stok;
					/*$sql="select SQL_NO_CACHE if (sum(QTY_STOK) is null,0,sum(QTY_STOK)) as jml,IF (SUM(QTY_STOK*HARGA_BELI_SATUAN-(DISKON*QTY_STOK*HARGA_BELI_SATUAN/100)+(QTY_STOK*HARGA_BELI_SATUAN/10)) IS NULL,0,SUM((QTY_STOK*HARGA_BELI_SATUAN-(DISKON*QTY_STOK*HARGA_BELI_SATUAN/100)) * 1.1)) AS ntot from a_penerimaan where OBAT_ID=$obat_id and UNIT_ID_TERIMA=$idunit and KEPEMILIKAN_ID=$kepemilikan_id and STATUS=1 and QTY_STOK>0";*/
					$sql="SELECT SQL_NO_CACHE IFNULL(SUM(t1.stok_after),0) AS jml,IFNULL(SUM(t1.ntot),0) AS ntot FROM (SELECT SQL_NO_CACHE UUID() AS cuid,IFNULL(QTY_STOK,0) AS stok_after,IFNULL((IF(((NILAI_PAJAK<=0) OR (TIPE_TRANS=4) OR (TIPE_TRANS=5) OR (TIPE_TRANS=100)),QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)),QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)) * 1.1)),0) AS ntot FROM a_penerimaan WHERE OBAT_ID=$obat_id AND KEPEMILIKAN_ID=$kepemilikan_id AND UNIT_ID_TERIMA=$idunit AND QTY_STOK>0 AND STATUS=1) AS t1";
					$rs=mysqli_query($konek,$sql);
					$cstk_after=0;
					$cnilai=0;
					...
					$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,-$selisih,0,$stok,'Stok Opname',$iduser,0,5,$cnilai)";
					//echo $sql;
					$rs=mysqli_query($konek,$sql);
				}else{
					$selisih=$stok-$cstok;
					if ($selisih>0){
						...
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
						$sql="select UUID() AS cuid,ID from a_penerimaan where UNIT_ID_TERIMA=$idunit and USER_ID_KIRIM=$iduser and TANGGAL='$tgl2' and TANGGAL_ACT='$tgl_act' order by ID desc limit 1";
						//echo $sql."<br>";
						$rs1=mysqli_query($konek,$sql);
						$cidp=0;
						if ($rows1=mysqli_fetch_array($rs1)){
						...
						$cstk_bfr=$cstk_after-$selisih;
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$selisih,0,$stok,'Stok Opname',$iduser,$cidp,5,$cnilai)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
					}
				}
						...
						while (($rows=mysqli_fetch_array($rs)) && ($ok=="false")){
							...
							if ($cqty>=0){
								if ($cidp==0){
									...
									//echo $sql."<br>";
									$rs1=mysqli_query($konek,$sql);
									$sql="select ID,UUID() AS cuid from a_penerimaan where UNIT_ID_TERIMA=$idunit and USER_ID_KIRIM=$iduser and TANGGAL='$tgl2' and TANGGAL_ACT='$tgl_act' order by ID desc limit 1";
									//echo $sql."<br>";
									$rs1=mysqli_query($konek,$sql);
									if ($rows1=mysqli_fetch_array($rs1)){
										$cidp=$rows1['ID'];
					...
					if ($selisih>0){
						if ($hargab==0){
							...
							if ($rows=mysqli_fetch_array($rs)){
								...
								$cstk_bfr=$cstk_after-$selisih;
								$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,NILAI_TOTAL) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$selisih,0,$stok,'Stok Opname',$iduser,$cid,5,$cnilai)";
								//echo $sql;
								$rs=mysqli_query($konek,$sql);
							}
						}
					}else{
						$sql="insert into a_stok_opname(idunit,iduser,idpenerimaan,idobat,kepemilikan_id,stok,opname,adjust,qty,harga_satuan,tgl,tgl_act) values($idunit,$iduser,0,$obat_id,$kepemilikan_id,$cstok,$stok,$selisih,$selisih,$hargab,'$tgl2','$tgl_act')";
						//echo $sql."<br>";
						$rs1=mysqli_query($konek,$sql);				
