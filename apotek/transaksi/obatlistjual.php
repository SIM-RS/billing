...
?>
    ...
    <table id="tblObat" border="0" cellpadding="1" cellspacing="0">
	  ...
	  $no=$_REQUEST["no"];
	  $aKsoId=$_REQUEST["ksoid"];
	  $sql="SELECT * FROM a_mitra WHERE idmitra=$aKsoId";
	  //  echo $sql;
	  ...
	  while ($rows=mysqli_fetch_array($rs)){
		...
		$sqlBook = "select obat_id, sum(qty) jml 
					from a_booking_obat 
					where obat_id = '{$cidobat}' 
					  and unit_id = '{$idunit}'
					  and kepemilikan_id = '".$rows['KEPEMILIKAN_ID']."'
					group by obat_id";
		$QBook = mysqli_query($konek,$sqlBook);
		$dBook = mysqli_fetch_array($QBook);
		
		//echo $sqlBook;
		
		$cstok=$rows['stok']-$dBook['jml'];
