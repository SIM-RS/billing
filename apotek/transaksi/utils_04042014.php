...
switch(strtolower($_REQUEST['act']))
{
  ...
  case 'simpan':
	...
	$nokw="000001";
	$cekNo = "select MAX(CAST(NO_PENJUALAN AS SIGNED)) NO_PENJUALAN from a_no_penjualan_log"; //order by id DESC limit 1
	...
	/* if ($rows=mysqli_fetch_array($rs)){
		...
		$updateNO = "insert into a_no_penjualan_log(NO_PENJUALAN) value('{$nokw}')";
...
WHERE $dbbilling.b_resep.tgl='$tgl2' AND $dbbilling.b_resep.apotek_id='$idunit' ".$fSta." GROUP BY $dbbilling.b_resep.no_resep,$dbbilling.b_resep.no_rm,$dbbilling.b_resep.cara_bayar,$dbbilling.b_resep.id_pelayanan) AS gab $filter ORDER BY ".$sorting; */
	...
	if($_REQUEST['status'] == "0"){
		...
		$sql = "SELECT *
				...
				WHERE DATE_FORMAT(p.TGL_ACT, '%Y-%m-%d') = '$tgl2'
