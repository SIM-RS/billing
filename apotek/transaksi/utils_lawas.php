		...
		if (mysqli_errno($konek)==0){*/
			...
			while (($rows=mysqli_fetch_array($rs))&&($done=="false"))
			{
				...
				if ($cstok>=$jml)
				{
					...
					($cid,$idunit,$iduser,$kp_id,$kso,$kso_id,'$tgltrans',NOW(),'$no_penjualan','$no_kunj','$NoRM','$NoResep','$dokter','$ruangan',$shft,$cara_bayar,$jml,$jml,$biaya_retur,$hNetto,$hSatuan,$jml*$hSatuan,$subtotal,$embalage,$jasa_resep,$tot_harga,'$ketDosis',1,'$nm_pasien','$txtalamat',$nutang,$isRacikan,'$txtH','$txtT','$txtK','$txtP')";
					//echo $sql."<br>";
					$rs1=mysqli_query($konek,$sql);
					if (mysqli_errno($konek)>0){
						$success=0;
					...
					$rs1=mysqli_query($konek,$sql);
					$sql="insert into $dbapotek.a_penjualan(PENERIMAAN_ID,UNIT_ID,USER_ID,JENIS_PASIEN_ID,KSO,KSO_ID,TGL,TGL_ACT,NO_PENJUALAN,NO_KUNJUNGAN,NO_PASIEN,NO_RESEP,DOKTER,RUANGAN,SHIFT,CARA_BAYAR,QTY_JUAL,QTY,BIAYA_RETUR,HARGA_NETTO,HARGA_SATUAN,SUB_TOTAL,SUM_SUB_TOTAL,EMBALAGE,JASA_RESEP,HARGA_TOTAL,KET_DOSIS,STATUS,NAMA_PASIEN,ALAMAT,UTANG,RACIKAN,textH,textT,textK,textP) values
					($cid,$idunit,$iduser,$kp_id,$kso,$kso_id,'$tgltrans',NOW(),'$no_penjualan','$no_kunj','$NoRM','$NoResep','$dokter','$ruangan',$shft,$cara_bayar,$cstok,$cstok,$biaya_retur,$hNetto,$hSatuan,$cstok*$hSatuan,$subtotal,$embalage,$jasa_resep,$tot_harga,'$ketDosis',1,'$nm_pasien','$txtalamat',$nutang,$isRacikan,'$txtH','$txtT','$txtK','$txtP')";
					//echo $sql."<br>2";
					$rs1=mysqli_query($konek,$sql);
					if (mysqli_errno($konek)>0){
						$success=0;
...
CONCAT((CONCAT((CONCAT((CONCAT(mp.alamat,' RT.',mp.rt)),' RW.',mp.rw)),', Desa ',w.nama)),', Kecamatan ',wi.nama,', ',wii.nama) alamat_, $dbbilling.b_resep.iter, $dbbilling.b_resep.id_pelayanan NOKUNJUNGAN
FROM $dbbilling.b_resep
...
if($grd == "true")
{
	...
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$HTKP="<span id='pasiens' style='text-decoration:underline' title='Klik Untuk Melihat HTKP' onclick='detailHTKP(".$rows["NOKUNJUNGAN"].",".$rows["pasien_id"].",".$rows["no_resep"].");'>HTKP</span>";
		$sisip=$rows["id"].'|'.$rows["kepemilikanId"]."|".$rows["no_resep"]."|".$rows["no_rm"]."|".$rows["id_pelayanan"]."|".$rows["n_penj"]."|".$rows["tgltrans"]."|".$rows["cara_bayar_id"]."|".$rows["nama_pasien"]."|".$rows["sex"]."|".$rows["umur"]."|".$rows["tgl_lahir"]."|".$rows["alamat_"]."|".$rows["telp"]."|".$rows["no_ktp"]."|".$rows["no_rm"];
