...
IFNULL(t2.prod_in,0) AS prod_in,IFNULL(t2.prod_in_nilai,0) AS prod_in_nilai,
IFNULL(t2.sisa_in,0) AS sisa_in,IFNULL(t2.sisa_in_nilai,0) AS sisa_in_nilai,
IFNULL(t2.rsp,0) AS rsp,
...
SUM(IF (tipetrans=0,DEBET *tmp.HARGA_SATUAN,0)) AS pbf_nilai,
SUM(IF (tipetrans=1,IF (DEBET IS NULL,0,DEBET),0)) AS unit_in,
SUM(IF (tipetrans=1,DEBET * tmp.HARGA_SATUAN,0)) AS unit_in_nilai, 
SUM(IF (tipetrans=2,IF (DEBET IS NULL,0,DEBET),0)) AS milik_in, 
SUM(IF (tipetrans=2,DEBET * tmp.HARGA_SATUAN,0)) AS milik_in_nilai, 
SUM(IF ((tipetrans=6 OR tipetrans=7),IF (KREDIT IS NULL,0,IF(KREDIT=0,0,KREDIT * -1)),0)) AS rt_in, 
SUM(IF ((tipetrans=6 OR tipetrans=7),IF(KREDIT=0,0,(KREDIT * -1)) * tmp.HARGA_SATUAN,0)) AS rt_in_nilai, 
SUM(IF (tipetrans=4,IF (DEBET IS NULL,0,DEBET),0)) AS prod_in, 
SUM(IF (tipetrans=4,DEBET *tmp.HARGA_SATUAN,0)) AS prod_in_nilai,
SUM(IF (tipetrans=11,IF (DEBET IS NULL,0,DEBET),0)) AS sisa_in, 
SUM(IF (tipetrans=11,DEBET *tmp.HARGA_SATUAN,0)) AS sisa_in_nilai, 
SUM(IF (tipetrans=8,IF (KREDIT IS NULL,0,KREDIT),0)) AS rsp, 
SUM(IF (tipetrans=8,KREDIT * tmp.HARGA_SATUAN,0)) AS rsp_nilai, 
SUM(IF (tipetrans=1,IF (KREDIT IS NULL,0,KREDIT),0)) AS unit_out, 
SUM(IF (tipetrans=1,KREDIT * tmp.HARGA_SATUAN,0)) AS unit_out_nilai, 
SUM(IF ((tipetrans=9 OR tipetrans=7),IF (DEBET IS NULL,0,IF(DEBET=0,0,DEBET * -1)),0)) AS rt_out, 
SUM(IF ((tipetrans=9 OR tipetrans=7),IF(DEBET=0,0,(DEBET * -1)) * tmp.HARGA_SATUAN,0)) AS rt_out_nilai, 
SUM(IF (tipetrans=2,IF (KREDIT IS NULL,0,KREDIT),0)) AS milik_out, 
SUM(IF (tipetrans=2,KREDIT * tmp.HARGA_SATUAN,0)) AS milik_out_nilai, 
SUM(IF (tipetrans=10,IF (KREDIT IS NULL,0,KREDIT),0)) AS hapus, 
SUM(IF (tipetrans=10,KREDIT * tmp.HARGA_SATUAN,0)) AS hapus_nilai, 
SUM(IF (tipetrans=4,IF (KREDIT IS NULL,0,KREDIT),0)) AS prod_out, 
SUM(IF (tipetrans=4,KREDIT * tmp.HARGA_SATUAN,0)) AS prod_out_nilai, 
SUM(IF ((tipetrans=5 OR tipetrans=100),IF (DEBET-KREDIT IS NULL,0,DEBET-KREDIT),0)) AS adj, 
SUM(IF ((tipetrans=5 OR tipetrans=100),IF (fkid=0,(DEBET-KREDIT) * ah.HARGA_BELI_SATUAN,(DEBET-KREDIT) * ap.HARGA_BELI_SATUAN),0)) AS adj_nilai 
...
GROUP BY tmp.OBAT_ID,tmp.KEPEMILIKAN_ID) AS t2 ON (t1.OBAT_ID=t2.OBAT_ID AND t1.KEPEMILIKAN_ID=t2.KEPEMILIKAN_ID)";
		...
		t1.rt_in,t1.rt_in_nilai,
		t1.prod_in,t1.prod_in_nilai,
		t1.sisa_in,t1.sisa_in_nilai,
		t1.rsp,t1.rsp_nilai,t1.UNIT_OUT,t1.UNIT_OUT_NILAI,
		...
		SUM(IF (tipetrans=4,KREDIT * tmp.HARGA_SATUAN,0)) AS prod_out_nilai, 
		SUM(IF ((tipetrans=5 OR tipetrans=100),IF (DEBET-KREDIT IS NULL,0,DEBET-KREDIT),0)) AS adj, 
		SUM(IF ((tipetrans=5 OR tipetrans=100),IF (fkid=0,(DEBET-KREDIT) * tmp.HARGA_SATUAN,(DEBET-KREDIT) * ap.HARGA_BELI_SATUAN),0)) AS adj_nilai 
		FROM (SELECT * FROM a_kartustok WHERE UNIT_ID=$cunit AND TGL_ACT BETWEEN '$ta-$tbulan-01 00:00:00' AND '$ta-$tbulan-31 23:59:59' ORDER BY ".$corder."ID DESC) AS tmp
		LEFT JOIN a_penerimaan ap ON tmp.fkid=ap.ID LEFT JOIN (SELECT OBAT_ID,KEPEMILIKAN_ID,MAX(HARGA_BELI_SATUAN) AS HARGA_BELI_SATUAN FROM a_harga GROUP BY OBAT_ID,KEPEMILIKAN_ID) ah ON (tmp.OBAT_ID=ah.OBAT_ID AND tmp.KEPEMILIKAN_ID=ah.KEPEMILIKAN_ID)
...
SUM(IF (tipetrans=4,KREDIT * tmp.HARGA_SATUAN,0)) AS prod_out_nilai, 
SUM(IF ((tipetrans=5 OR tipetrans=100),IF (DEBET-KREDIT IS NULL,0,DEBET-KREDIT),0)) AS adj, 
SUM(IF ((tipetrans=5 OR tipetrans=100),IF (fkid=0,(DEBET-KREDIT) * tmp.HARGA_SATUAN,(DEBET-KREDIT) * tmp.HARGA_SATUAN),0)) AS adj_nilai 
FROM (SELECT * FROM a_kartustok WHERE UNIT_ID=$cunit AND TGL_ACT BETWEEN '$ta-$tbulan-01 00:00:00' AND '$ta-$tbulan-31 23:59:59' ORDER BY ".$corder."ID DESC) AS tmp
LEFT JOIN a_penerimaan ap ON tmp.fkid=ap.ID LEFT JOIN (SELECT OBAT_ID,KEPEMILIKAN_ID,MAX(HARGA_BELI_SATUAN) AS HARGA_BELI_SATUAN FROM a_harga GROUP BY OBAT_ID,KEPEMILIKAN_ID) ah ON (tmp.OBAT_ID=ah.OBAT_ID AND tmp.KEPEMILIKAN_ID=ah.KEPEMILIKAN_ID)
GROUP BY tmp.OBAT_ID,tmp.KEPEMILIKAN_ID) AS t2 ON (t1.OBAT_ID=t2.OBAT_ID AND t1.KEPEMILIKAN_ID=t2.KEPEMILIKAN_ID)";
		//echo $sql."<br>";
	...
	if (($bulan[0]<8 && $ta==2010) || ($ta<2010)){
		...
		IFNULL(prod_in,0) AS prod_in,IFNULL(prod_in_nilai,0) AS prod_in_nilai,
		IFNULL(sisa_in,0) AS sisa_in,IFNULL(sisa_in_nilai,0) AS sisa_in_nilai,
		IFNULL(rsp,0) AS rsp,IFNULL(rsp_nilai,0) AS rsp_nilai,
