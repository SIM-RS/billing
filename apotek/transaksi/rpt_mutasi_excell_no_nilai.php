...
if ($idunit1=="0"){
	...
	$worksheet->mergeCells($row,$column+5,$row,$column+10);
...
if ($idunit1=="0"){
	...
	$worksheet->write($row, $column+9, "Sisa", $columnTitleFormat);
	if ($idunit1=="16"){
...
if ($idunit1=="0"){
	...
	$worksheet->write($row, 4, '=Sum(E11:E'.$row.')', $regularFormatRF);
	...
	$worksheet->write($row, 18, '=Sum(S11:S'.$row.')', $regularFormatRF);
	...
	while ($rows=mysqli_fetch_array($rs)){
		...
		if ($qty_trans<>0){
			...
			$worksheet->write($row, 9, $rows['SISA_IN'], $regularFormatR);
			if ($idunit1=="16"){
	...
	$worksheet->write($row, 16, '=Sum(Q11:Q'.$row.')', $regularFormatRF);
