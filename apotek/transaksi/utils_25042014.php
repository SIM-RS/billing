...
if($grd == "true")
{
	...
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$sisip=$rows["id"].'|'.$rows["kepemilikanId"]."|".$rows["no_resep"]."|".$rows["no_rm"]."|".$rows["id_pelayanan"]."|".$rows["n_penj"]."|".$rows["tgltrans"]."|".$rows["cara_bayar_id"]."|".$rows["nama_pasien"]."|".$rows["sex"]."|".$rows["umur"]."|".$rows["tgl_lahir"]."|".$rows["alamat_"]."|".$rows["telp"]."|".$rows["no_ktp"]."|".$rows["no_rm"]."|".$rows["tglll"]; //."|".$rows["txtH"]."|".$rows["txtT"]."|".$rows["txtK"]."|".$rows["txtP"]
