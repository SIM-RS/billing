...
}elseif($grdDet == "true")
{
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["OBAT_NAMA"].chr(3).$rows["ket_dosis"].chr(3).$rows["racikan"]." ".$rows["nm_bentuk"].chr(3).number_format($hStok,0,",",".").chr(3).number_format($hNetto,0,",",".").chr(3).$cqtyRsp.chr(3).number_format($hSatuan,0,",",".").chr(3).number_format(($hSatuan*$rows["qty"]),0,",",".").chr(3).$d.chr(6);
	}
}
