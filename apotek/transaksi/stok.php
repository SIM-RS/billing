...
$hppr=mysqli_real_escape_string($konek,$_REQUEST["hppr"]);
			...
			if ($rows1=mysqli_fetch_array($rs1)){
				...
				if ($cjml>0){
					...
					if ($ada_harga==0){
						...
						$sql="insert into a_penerimaan(OBAT_ID,UNIT_ID_TERIMA,KEPEMILIKAN_ID,USER_ID_KIRIM,TANGGAL_ACT,TANGGAL,QTY_SATUAN,QTY_STOK,HARGA_BELI_SATUAN,TIPE_TRANS,STATUS) values($obat_id,$idunit,$kepemilikan_id,$iduser,NOW(),CURDATE(),$stok,$stok,$hargabp,5,1)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
						...
						$sql="update a_stok_opname set idpenerimaan=$cidp,opname=$stok,adjust=$stok,qty=$stok,harga_satuan=$hargab,harga_satuan_persediaan=$hargabp,tgl=CURDATE(),tgl_act=NOW() where idunit=$idunit and idobat=$obat_id and kepemilikan_id=$kepemilikan_id";
						//echo $sql."<br>";
						$rs=mysqli_query($konek,$sql);
						...
						$cstk_bfr=0;
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$stok,0,$stok,'Stok Opname',$iduser,$cidp,5,$hargabp,$hppr*$stok,$hppr)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
						//Update Stok
						...
						if ($rw1["cAda"]>0){
							$sql="UPDATE a_stok SET qty_stok=$stok,nilai=$stok*$hppr,rata2=$hppr,tgl_act=NOW() WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
						}else{
							$sql="INSERT INTO a_stok(unit_id,obat_id,kepemilikan_id,qty_stok,nilai,rata2,tgl_act) 
							VALUES($idunit,$obat_id,$kepemilikan_id,$stok,$hppr*$stok,$hppr,NOW())";
						}
						$rs1=mysqli_query($konek,$sql);
					}
				}
				...
				if ($cstok>$stok){
					...
					while (($rows=mysqli_fetch_array($rs)) && ($ok=="false")){
						...
						if ($qty>=$selisih){
							...
							//echo $sql."<br>";
							$rs1=mysqli_query($konek,$sql);					
							$sql="insert into a_stok_opname(idunit,iduser,idpenerimaan,idobat,kepemilikan_id,stok,opname,adjust,qty,harga_satuan,harga_satuan_persediaan,tgl,tgl_act) values($idunit,$iduser,$cid,$obat_id,$kepemilikan_id,$cstok,$stok,-$selisih,-$selisih,$hargab,$hargabp,'$tgl2','$tgl_act')";
							//echo $sql."<br>";
							$rs1=mysqli_query($konek,$sql);
							$sql="SELECT SQL_NO_CACHE IFNULL(SUM(t1.stok_after),0) AS jml,IFNULL(SUM(t1.ntot),0) AS ntot FROM (SELECT UUID() AS cuid,IFNULL(QTY_STOK,0) AS stok_after,IFNULL((IF(((NILAI_PAJAK<=0) OR (TIPE_TRANS=4) OR (TIPE_TRANS=5) OR (TIPE_TRANS=100)),QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)),QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)) /* 1.1*/)),0) AS ntot FROM a_penerimaan WHERE OBAT_ID=$obat_id AND KEPEMILIKAN_ID=$kepemilikan_id AND UNIT_ID_TERIMA=$idunit AND QTY_STOK>0 AND STATUS=1) AS t1";
							$rs1=mysqli_query($konek,$sql);
							...
							$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,-$selisih,0,$cstk_after,'Stok Opname',$iduser,$cid,5,$ch_satuan,$cstk_after*$hppr,$hppr)";
							//echo $sql;
							$rs1=mysqli_query($konek,$sql);
							//Update Stok
							$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
							...
							$rw1=mysqli_fetch_array($rs1);
							if ($rw1["cAda"]>0){
								$sql="UPDATE a_stok SET qty_stok=$cstk_after,nilai=$cstk_after*$hppr,rata2=$hppr,tgl_act=NOW() WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
							}else{
								$sql="INSERT INTO a_stok(unit_id,obat_id,kepemilikan_id,qty_stok,nilai,raat2,tgl_act) 
								VALUES($idunit,$obat_id,$kepemilikan_id,$cstk_after,$cstk_after*$hppr,$hppr,NOW())";
							}
							$rs1=mysqli_query($konek,$sql);
						}else{
							$selisih=$selisih-$qty;
							...
							$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN,NILAI_TOTAL,HARGA_SATUAN_AWAL) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,-$qty,0,$cstk_after,'Stok Opname',$iduser,$cid,5,$ch_satuan,$cstk_after*$hppr,$hppr)";
							//echo $sql;
							$rs1=mysqli_query($konek,$sql);
							//Update Stok
							$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
					...
					$selisih=$stok-$cstok;
					if ($selisih>0){
						$sql="insert into a_penerimaan(OBAT_ID,UNIT_ID_TERIMA,KEPEMILIKAN_ID,USER_ID_KIRIM,TANGGAL_ACT,TANGGAL,QTY_SATUAN,QTY_STOK,HARGA_BELI_SATUAN,TIPE_TRANS,STATUS) values($obat_id,$idunit,$kepemilikan_id,$iduser,NOW(),CURDATE(),$selisih,$selisih,$hargabp,5,1)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
						...
						$sql="insert into a_stok_opname(idunit,iduser,idpenerimaan,idobat,kepemilikan_id,stok,opname,adjust,qty,harga_satuan,harga_satuan_persediaan,tgl,tgl_act) values($idunit,$iduser,$cidp,$obat_id,$kepemilikan_id,$cstok,$stok,$selisih,$selisih,$hargab,$hargabp,'$tgl2','$tgl_act')";
						//echo $sql."<br>";
						$rs=mysqli_query($konek,$sql);
						$sql="SELECT SQL_NO_CACHE IFNULL(SUM(t1.stok_after),0) AS jml,IFNULL(SUM(t1.ntot),0) AS ntot FROM (SELECT UUID() AS cuid,IFNULL(QTY_STOK,0) AS stok_after,IFNULL((IF(((NILAI_PAJAK<=0) OR (TIPE_TRANS=4) OR (TIPE_TRANS=5) OR (TIPE_TRANS=100)),QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)),QTY_STOK*HARGA_BELI_SATUAN * (1-(DISKON/100)) /* 1.1*/)),0) AS ntot FROM a_penerimaan WHERE OBAT_ID=$obat_id AND KEPEMILIKAN_ID=$kepemilikan_id AND UNIT_ID_TERIMA=$idunit AND QTY_STOK>0 AND STATUS=1) AS t1";
						$rs=mysqli_query($konek,$sql);
						...
						$cstk_bfr=$cstk_after-$selisih;
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$selisih,0,$cstk_after,'Stok Opname',$iduser,$cidp,5,$hargabp,$cstk_after*$hppr,$hppr)";
						//echo $sql;
						$rs=mysqli_query($konek,$sql);
						//Update Stok
						$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
				...
				if ($cstok>$stok){
					...
					while (($rows=mysqli_fetch_array($rs)) && ($ok=="false")){
						...
						if ($qty>=$selisih){
							...
							$cstk_bfr=$cstk_after+$qty;
							$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,-$qty,0,$cstk_after,'Stok Opname',$iduser,$cid,5,$ch_satuan,$cstk_after*$hppr,$hppr)";
							//echo $sql;
							$rs1=mysqli_query($konek,$sql);
							//Update Stok
							$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
							...
							$rw1=mysqli_fetch_array($rs1);
							if ($rw1["cAda"]>0){
								$sql="UPDATE a_stok SET qty_stok=$cstk_after,nilai=$cstk_after*$hppr,nilai=$hppr,tgl_act=NOW() WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
							}else{
								$sql="INSERT INTO a_stok(unit_id,obat_id,kepemilikan_id,qty_stok,nilai,rata2,tgl_act) 
								VALUES($idunit,$obat_id,$kepemilikan_id,$cstk_after,$cstk_after*$hppr,$hppr,NOW())";
							}
						...
						while (($rows=mysqli_fetch_array($rs)) && ($ok=="false")){
							...
							if ($cqty>=0){
								if ($cidp==0){
									...
									$rw1=mysqli_fetch_array($rs1);
									if ($rw1["cAda"]>0){
										$sql="UPDATE a_stok SET qty_stok=$cstk_after,nilai=$cstk_after*$hppr,$hppr,tgl_act=NOW() WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
									}else{
										$sql="INSERT INTO a_stok(unit_id,obat_id,kepemilikan_id,qty_stok,nilai,rata2,tgl_act) 
										VALUES($idunit,$obat_id,$kepemilikan_id,$cstk_after,$cstk_after*$hppr,$hppr,NOW())";
									}
									...
									$cstk_bfr=$cstk_after-$selisih;
									$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$selisih,0,$cstk_after,'Stok Opname',$iduser,$cidp,5,'$ch_satuan',$cstk_after*$hppr,$hppr)";
									//echo $sql;
									$rs1=mysqli_query($konek,$sql);
									//Update Stok
									$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
								...
								$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$jml,0,$cstk_after,'Stok Opname',$iduser,$cidp,5,'$ch_satuan',$cstk_after*$hppr,$hppr)";
								//echo $sql;
								$rs1=mysqli_query($konek,$sql);
								//Update Stok
								$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
				...
				if ($cstok>$stok){
					...
					if ($selisih>0){
						...
						if ($ada_harga==0){
							...
							if ($rows=mysqli_fetch_array($rs)){
								$cid=$rows['ID'];
								$sql="insert into a_stok_opname(idunit,iduser,idpenerimaan,idobat,kepemilikan_id,stok,opname,adjust,qty,harga_satuan,tgl,tgl_act) values($idunit,$iduser,$cid,$obat_id,$kepemilikan_id,$cstok,$stok,$stok-$cstok,$selisih,$hargabp,CURDATE(),NOW())";
								//echo $sql."<br>";
								$rs1=mysqli_query($konek,$sql);
								...
								$cstk_bfr=$cstk_after-$selisih;
								$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN_AWAL,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$selisih,0,$cstk_after,'Stok Opname',$iduser,$cid,5,$hargabp,$cstk_after*$hppr,$hppr)";
								//echo $sql;
								$rs1=mysqli_query($konek,$sql);
								//Update Stok
								$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
						...
						$sql="insert into a_stok_opname(idunit,iduser,idpenerimaan,idobat,kepemilikan_id,stok,opname,adjust,qty,harga_satuan,tgl,tgl_act) values($idunit,$iduser,0,$obat_id,$kepemilikan_id,$cstok,$stok,$selisih,$selisih,$hargabp,CURDATE(),NOW())";
						//echo $sql."<br>";
						$rs1=mysqli_query($konek,$sql);				
						...
						$cstk_bfr=$cstk_after-$selisih;
						$sql="insert into a_kartustok(OBAT_ID,KEPEMILIKAN_ID,UNIT_ID,TGL_TRANS,TGL_ACT,NO_BUKTI,STOK_BEFOR,DEBET,KREDIT,STOK_AFTER,KET,USER_ID,fkid,tipetrans,HARGA_SATUAN,NILAI_TOTAL,HARGA_SATUAN) values($obat_id,$kepemilikan_id,$idunit,CURDATE(),NOW(),'',$cstk_bfr,$selisih,0,$stok,'Stok Opname',$iduser,0,5,0,$stok*$hppr,$hppr)";
						//echo $sql;
						$rs1=mysqli_query($konek,$sql);
						//Update Stok
						$sql="SELECT IFNULL(COUNT(id),0) cAda FROM a_stok WHERE unit_id=$idunit AND obat_id=$obat_id AND kepemilikan_id=$kepemilikan_id";
...
var keywords=par.value;//alert(keywords);
	if(keywords==""){
		...
		if (key==38 || key==40){
		...
		}else if (key!=27 && key!=37 && key!=39 && key!=123){
			RowIdx=0;
			fKeyEnt=false;
			Request('../transaksi/obatlist_opn.php?aKepemilikan=0&idunit=<?php echo $idunit; ?>&aKeyword='+keywords , 'divobat', '', 'GET' );
			...
			RowIdx=0;
			fKeyEnt=false;
			Request('../transaksi/obatlist_opn.php?aKepemilikan=0&idunit=0&aKeyword='+keywords , 'divobat', '', 'GET' );
			if (document.getElementById('divobat').style.display=='none') fSetPosisi(document.getElementById('divobat'),par);
			document.getElementById('divobat').style.display='block';
		}
...
var tds;
	...
	document.forms[0].hpp.value=cdata[9];
	document.forms[0].hppr.value=cdata[10];
	document.getElementById('divobat').style.display='none';
}
</script>
</head>
...
<div align="center">
  ...
  <div id="input" style="display:block">
	  ...
	  <table width="65%" border="0" cellpadding="1" cellspacing="1" class="txtinput">
        ...
        <tr> 
          <td><input type="checkbox" name="chkupdt" value="checkbox" onClick="if (this.checked){hpp.disabled='';hppr.disabled='';profit.disabled='';}else{hpp.disabled='true';hppr.disabled='true';profit.disabled='true';}">
            Update Master Harga</td>
          <td>&nbsp;</td>
          <td >&nbsp;</td>
        </tr>
		  <tr> 
          <td>Harga Netto Jual</td>
          <td>:</td>
          <td ><input name="hpp" type="text" class="txtright" id="hpp" size="15" maxlength="11" disabled="true" ></td>
        </tr>
		<tr> 
          <td>Harga Netto Persediaan</td>
          <td>:</td>
          <td ><input name="hppr" type="text" class="txtright" id="hppr" size="15" maxlength="11" ></td>
        </tr>
      
        <tr style="display:none"> 
          <td>Keuntungan</td>
          <td>:</td>
          <td ><input name="profit" type="text" class="txtcenter" id="profit" size="5" maxlength="5" disabled="true" value="0">
            %</td>
        </tr>
        <!--tr> 
		  <td>Keterangan</td>
  ...
  <div id="listma" style="display:block">
	  ...
	  if (($bulan!=$bln)||($ta!=$th[2]))
	  		...
	  		$sql="select ao.OBAT_ID,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,ak.ID,ak.NAMA,so.stok,so.adjust,1 as sdh_so,sum(so.qty*so.harga_satuan) as nilai,ah.harga_beli_satuan harga, s.rata2 rata2
			from a_stok_opname so 
			inner join a_obat ao on so.idobat=ao.OBAT_ID
		    inner join a_kepemilikan ak on so.kepemilikan_id=ak.ID 
			inner join a_harga ah on so.idobat=obat_id and so.kepemilikan_id=ah.kepemilikan_id
			left join a_stok s on so.idobat=s.obat_id and so.kepemilikan_id=s.kepemilikan_id AND so.idunit=s.unit_id
			where so.idunit=$idunit and month(so.tgl)=$bulan and year(so.tgl)=$ta".$filter." group by so.idobat,so.kepemilikan_id,so.stok,so.opname order by ".$sorting;
	  else

			$sql="SELECT t1.*,IF (t2.adjust IS NULL,0,t2.adjust) AS adjust,IF (t2.adjust IS NULL,0,1) AS sdh_so,t2.harga,t2.rata2 rata2
...
INNER JOIN a_kepemilikan ak ON t.KEPEMILIKAN_ID=ak.ID WHERE ao.OBAT_ISAKTIF=1".$filter.") AS t1 
LEFT JOIN 
(SELECT DISTINCT 
      so.idobat,
      so.kepemilikan_id,
      so.adjust,
      ah.`HARGA_BELI_SATUAN` AS harga,
	  s.rata2 as rata2
    FROM
      a_stok_opname so
     INNER JOIN a_harga ah ON so.`idobat`=`ah`.`OBAT_ID` AND so.`kepemilikan_id`=`ah`.`KEPEMILIKAN_ID`
	 left join a_stok s on so.idobat=s.obat_id and so.kepemilikan_id=s.kepemilikan_id AND so.idunit=s.unit_id
WHERE so.idunit=$idunit AND MONTH(so.tgl)=$bulan AND YEAR(so.tgl)=$ta) AS t2 ON CONCAT(t1.OBAT_ID,t1.ID)=CONCAT(t2.idobat,t2.kepemilikan_id) 
ORDER BY ".$sorting;
	//  echo $sql;
		$rs=mysqli_query($konek,$sql);
	  ...
	  while ($rows=mysqli_fetch_array($rs)){
		...
		$arfvalue="act*-*edit*|*stok1*-*".$rows['stok']."*|*obat_id*-*".$rows['OBAT_ID']."*|*obat_nama*-*".$rows['OBAT_NAMA']."*|*kepemilikan_id*-*".$rows['ID']."*|*stok*-*".$rows['stok']."*|*sdh_so*-*".$rows['sdh_so']."*|*hpp*-*".$rows['harga']."*|*hppr*-*".$rows['rata2'];
