		...
		if (mysqli_errno($konek)==0){*/
			...
			while (($rows=mysqli_fetch_array($rs))&&($done=="false"))
			{
				...
				if ($cstok>=$jml)
				{
					...
					($cid,$idunit,$iduser,$kp_id,$kso,$kso_id,'$tgltrans',NOW(),'$no_penjualan','$no_kunj','$NoRM','$NoResep','$dokter','$ruangan',$shft,$cara_bayar,$jml,$jml,$biaya_retur,$hNetto,$hSatuan,$jml*$hSatuan,$subtotal,$embalage,$jasa_resep,$tot_harga,'$ketDosis',1,'$nm_pasien','$txtalamat',$nutang,$isRacikan,'$textH','$textT','$textK','$textP')";
					//echo $sql."<br>";
					$rs1=mysqli_query($konek,$sql);
					if (mysqli_errno($konek)>0){
						$success=0;
					...
					$rs1=mysqli_query($konek,$sql);
					$sql="insert into $dbapotek.a_penjualan(PENERIMAAN_ID,UNIT_ID,USER_ID,JENIS_PASIEN_ID,KSO,KSO_ID,TGL,TGL_ACT,NO_PENJUALAN,NO_KUNJUNGAN,NO_PASIEN,NO_RESEP,DOKTER,RUANGAN,SHIFT,CARA_BAYAR,QTY_JUAL,QTY,BIAYA_RETUR,HARGA_NETTO,HARGA_SATUAN,SUB_TOTAL,SUM_SUB_TOTAL,EMBALAGE,JASA_RESEP,HARGA_TOTAL,KET_DOSIS,STATUS,NAMA_PASIEN,ALAMAT,UTANG,RACIKAN,textH,textT,textK,textP) values
					($cid,$idunit,$iduser,$kp_id,$kso,$kso_id,'$tgltrans',NOW(),'$no_penjualan','$no_kunj','$NoRM','$NoResep','$dokter','$ruangan',$shft,$cara_bayar,$cstok,$cstok,$biaya_retur,$hNetto,$hSatuan,$cstok*$hSatuan,$subtotal,$embalage,$jasa_resep,$tot_harga,'$ketDosis',1,'$nm_pasien','$txtalamat',$nutang,$isRacikan,'$textH','$textT','$textK','$textP')";
					//echo $sql."<br>";
					$rs1=mysqli_query($konek,$sql);
					if (mysqli_errno($konek)>0){
						$success=0;
...
FROM
  ...
  WHERE $dbbilling.b_resep.tgl='$tgl2' AND $dbbilling.b_resep.apotek_id='$idunit' ".$fSta."
...
if($grd == "true")
{
	...
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$sisip=$rows["id"].'|'.$rows["kepemilikanId"]."|".$rows["no_resep"]."|".$rows["no_rm"]."|".$rows["id_pelayanan"]."|".$rows["n_penj"]."|".$rows["tgltrans"]."|".$rows["cara_bayar_id"]."|".$rows["nama_pasien"]."|".$rows["sex"]."|".$rows["umur"]."|".$rows["tgl_lahir"]."|".$rows["alamat_"]."|".$rows["telp"]."|".$rows["no_ktp"]."|".$rows["no_rm"]."|".$rows["txtH"]."|".$rows["txtT"]."|".$rows["txtK"]."|".$rows["txtP"];
		$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["no_resep"].chr(3).$rows["no_rm"].chr(3).$rows["nama_pasien"].chr(3).$rows["unit"].chr(3).$rows["dokter_nama"].chr(3).$cCaraBayar.chr(3).$rows["kepemilikan"].chr(3).$rows["kso"].chr(3).$infoPasien.chr(3).$rows["iter"].chr(6);
	}
}elseif($grdDet == "true")
{
	while ($rows=mysqli_fetch_array($rs))
	{
		...
		$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["OBAT_NAMA"].chr(3).$rows["ket_dosis"].chr(3).$ket." ".$rows["nm_bentuk"]." ".$rows["dtd"].chr(3).number_format($hStok,0,",",".").chr(3).number_format($hNetto,0,",",".").chr(3).$jml.chr(3).number_format($hSatuan,0,",",".").chr(3).number_format(($hSatuan*$rows["qty"]),0,",",".").chr(3).$d.chr(6);
	}
}
