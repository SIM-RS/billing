...
WHERE t.pelayanan_id = '".$pelayanan_id."') AS gab $filter order by $sorting";
    ...
    elseif($grdAnamnesa == "true") {	//Anamnese
         $sql = "SELECT a.*, DATE_FORMAT(a.TGL,'%d-%m-%Y %H:%i') AS tgltok, p.nama AS dokter
				...
				WHERE a.PASIEN_ID = '".$_REQUEST['pasien_id']."' and snurs = 1";
    }
	elseif($grdResep == "true") {	//resep --joker--
        $sql = "select r.id,r.apotek_id,r.dokter_id,IF(r.racikan=0,'Bukan',IF(r.obat_id=0,'Racikan',CONCAT('Racikan',' ',r.racikan,' (',r.qty_bahan,' ',r.satuan,')'))) as racikan,r.racikan as racikanfix,r.qty_bahan,r.qty,r.ket_dosis,r.obat_id,r.dokter_nama,IFNULL(o.OBAT_NAMA,r.obat_manual) OBAT_NAMA,r.type_dokter,r.tgl, r.diagnosa_id, r.obat_manual, r.id_bentuk
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    elseif($grdAnamnesa == "true") {	//Anamnese
        ...
        while($rows=mysql_fetch_array($rs)) {
            ...
            $sisipan=$rows["id"]."|".$rows['qty']."|".$rows['obat_id']."|".$rows['harga_jual']."|".$rows['stok']."|".$rows['apotek_id']."|".$rows["dokter_id"]."|".$rows['type_dokter']."|".$rows['tgl']."|".$rows['diagnosa_id']."|".$rows["racikanfix"]."|".$rows["qty_bahan"]."|".$rows["obat_manual"]."|".$rows["id_bentuk"];
            $i++;
