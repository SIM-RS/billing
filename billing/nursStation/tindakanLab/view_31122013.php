...
<?php
	...
	if($param == 'paren'){
		$celuk = "k.parent_id AS paren";
		$group = "k.parent_id";
		$tambah = "SUBSTR(t.pemeriksaan_id, -1) = 1";
		$join = "LEFT JOIN b_ms_kelompok_lab k ON k.id = SUBSTRING_INDEX(t.pemeriksaan_id,'|',1)";
	} elseif($param == 'paren2'){
		$celuk = "p.kelompok_lab_id  AS paren";
		$group = "p.kelompok_lab_id";
		$tambah = "SUBSTR(t.pemeriksaan_id, -1) = 2";
		$join = "LEFT JOIN b_ms_pemeriksaan_lab p
		...
		$sqlPar = "SELECT $celuk FROM b_tindakan_lab t
					$join
					WHERE $tambah AND t.pelayanan_id = '$id' AND t.kunjungan_id = '".$_REQUEST['idKunj']."'
					GROUP BY $group";
	...
	function treeSub($id){
		...
		if($jum != 0){
			...
			while($dataSub = mysql_fetch_array($querySub)){
				...
				$status = 1;
				$color = '"'.$test.$dataSub['idx']." - ".$dataSub['nama'].'"';
				if($dataSub['nm']=='2'){
					$cekColor = colorCek($dataSub['idx']);
					if($cekColor != 0){
						$color = '"'.$test.$dataSub['idx']." -".$dataSub['nama'].'"';
					} else {
						$color ='"'.$test.$dataSub['idx'].' - <span style=\'color:red;\'>'.$dataSub['nama'].'</span>"';
						$status = 0;
					}
				}
	...
	while($data = mysql_fetch_array($query)){
		...
		echo '{title: "'.$test.$data['id']." - ".$data['nama'].'", key: "'.$data['id'].'|1||1"'.$expand;
		echo $children;
