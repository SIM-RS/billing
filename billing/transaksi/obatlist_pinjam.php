...
?>
    <table id="tblObat" border="0" cellpadding="1" cellspacing="0">
	  ...
	  if ($aKepemilikan=="0"){
	  	$sql="select t1.*,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,ak.NAMA from (select ap.OBAT_ID,ap.KEPEMILIKAN_ID,sum(ap.QTY_STOK) as stok from $dbapotek.a_penerimaan ap where ap.UNIT_ID_TERIMA=$idunit and ap.QTY_STOK>0 and ap.STATUS=1 group by ap.OBAT_ID,ap.KEPEMILIKAN_ID) as t1 inner join $dbapotek.a_obat ao on t1.OBAT_ID=ao.OBAT_ID inner join $dbapotek.a_kepemilikan ak on t1.KEPEMILIKAN_ID=ak.ID where ao.OBAT_NAMA like '$aKeyword%' order by ao.OBAT_NAMA";
	  }else{
	  	$sql="select t1.*,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,ak.NAMA from (select ap.OBAT_ID,ap.KEPEMILIKAN_ID,sum(ap.QTY_STOK) as stok from $dbapotek.a_penerimaan ap where ap.UNIT_ID_TERIMA=$idunit and ap.QTY_STOK>0 and ap.KEPEMILIKAN_ID=1 and ap.STATUS=1 group by ap.OBAT_ID,ap.KEPEMILIKAN_ID) as t1 inner join $dbapotek.a_obat ao on t1.OBAT_ID=ao.OBAT_ID inner join $dbapotek.a_kepemilikan ak on t1.KEPEMILIKAN_ID=ak.ID where ao.OBAT_NAMA like '$aKeyword%' order by ao.OBAT_NAMA";
	  }
	  //echo $sql;
	  $rs=mysql_query($sql);
	  $arfvalue="";
