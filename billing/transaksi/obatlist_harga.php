...
?>
    <table id="tblObat" border="0" cellpadding="1" cellspacing="0">
	  ...
	  //echo $no."<br>";
	  if ($aKepemilikan=="0")
	  	$sql="select distinct ao.OBAT_ID,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,ak.ID,ak.NAMA,ah.HARGA_BELI_SATUAN from $dbapotek.a_obat ao inner join $dbapotek.a_harga ah on ao.OBAT_ID=ah.OBAT_ID inner join $dbapotek.a_kepemilikan ak on ah.KEPEMILIKAN_ID=ak.ID where ao.OBAT_ISAKTIF=1 and ak.AKTIF=1 and trim(ao.OBAT_NAMA) like '%$aKeyword%' ORDER BY ao.OBAT_NAMA,ak.NAMA";
	  else
	  	$sql="select distinct ao.OBAT_ID,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,ak.ID,ak.NAMA,ah.HARGA_BELI_SATUAN from $dbapotek.a_obat ao inner join $dbapotek.a_harga ah on ao.OBAT_ID=ah.OBAT_ID inner join $dbapotek.a_kepemilikan ak on ah.KEPEMILIKAN_ID=ak.ID where ak.ID=$aKepemilikan and ao.OBAT_ISAKTIF=1 and ak.AKTIF=1 and trim(ao.OBAT_NAMA) like '%$aKeyword%' ORDER BY ao.OBAT_NAMA,ak.NAMA";
	  //echo $sql;
	  $rs=mysql_query($sql);
	  $arfvalue="";
	  $i=0;
