...
<html>
    ...
    <?php
                                ...
                                <td class="tblJdlKn" align="center" style="text-transform:uppercase;"><?php echo $arrBln[$bln]; ?></td>
                                    ...
                                    <?php
                                $sqlCr = "SELECT b_ms_kso.id, b_ms_kso.nama FROM b_kunjungan INNER JOIN b_pelayanan ON b_kunjungan.id = b_pelayanan.kunjungan_id INNER JOIN b_ms_kso ON b_ms_kso.id = b_kunjungan.kso_id WHERE $fTmp AND YEAR(b_pelayanan.tgl)='$thn' GROUP BY b_ms_kso.id";
        ...
        <?php
                                    ...
                                    $j = 0;
                                        while($rwBln = mysql_fetch_array($rsBln)) {
                                            $qJml = "SELECT COUNT(b_pelayanan.id) AS jml FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id WHERE $fTmp AND MONTH(b_pelayanan.tgl)='".$rwBln['bulan']."' AND YEAR(b_pelayanan.tgl)='$thn' AND b_pelayanan.kso_id='".$rwCr['id']."'";
