...
INNER JOIN b_pelayanan ON b_pelayanan.id = b_pasien_keluar.pelayanan_id WHERE b_pasien_keluar.keadaan_keluar LIKE 'Meninggal > 48 Jam' AND b_pelayanan.unit_id = '".$wUnit1['id']."' AND YEAR(b_pasien_keluar.tgl_act) = ($thn-1) ";
    ...
    <?php
    echo $qBln = "SELECT MONTH(b_pasien_keluar.tgl_act) AS bulan FROM b_pelayanan INNER JOIN b_pasien_keluar ON b_pasien_keluar.pelayanan_id=b_pelayanan.id WHERE $fTmp AND YEAR(b_pasien_keluar.tgl_act)='$thn' AND b_pasien_keluar.keadaan_keluar LIKE '%meninggal' GROUP BY MONTH(b_pasien_keluar.tgl_act)";
