...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_persetujuan_tind_icu b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["age"].'|'.$rows["jenis_kelamin"].'|'.$rows["adress"].'|'.$rows["tindakan"].'|'.$rows["terhadap"].'|'.$rows["alternatif1"].'|'.$rows["alternatif2"].'|'.$rows["alternatif3"];
		$dt.=$rows["id"].'|'.$rows['name'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["name"].chr(3).$rows["age"].chr(3).$rows["adress"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
