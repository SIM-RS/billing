...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_fom_cath_form b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=$rows['penyakit'].'|'.$rows['TB'].'|'.$rows['BB'].'|'.$rows['list'].'|'.$rows['obat'];
		$dt.=$rows["id"].'|'.$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).tglSQL($rows["tgl_list"]).chr(3).$rows["tindakan"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
