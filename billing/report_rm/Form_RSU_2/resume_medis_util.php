...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bp.nama as nm_usr FROM b_fom_neonatus_discharge b
	LEFT JOIN b_ms_pegawai bp ON bp.id=b.user_act
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["gpa"].'|'.$rows["apgar"].'|'.$rows["kuning"].'|'.$rows["data_lab"].'|'.$rows["gol_darah"].'|'.$rows["hg"].'|'.$rows["g6pd"].'|'.$rows["t4"].'|'.$rows["xray"].'|'.$rows["terapi"].'|'.$rows["susu"].'|'.$rows["berat_pulang"].'|'.$rows["perawatan"].'|'.$rows["terapi_lanjut"].'|'.$rows["diagnosa"].'|'.tglSQL($rows["tgl_kembali"]);
		$dt.=$rows["id"].'|'.$rows['sex'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$rows["sex"].chr(3).$rows["ditolong"].chr(3).
		tglSQL($rows["tgl_ditolong"]).chr(3).$rows["berat"].chr(3).$rows["panjang"].chr(3).$rows["lingkaran_kpl"].chr(3).$rows["lingkar_dada"].chr(3).$rows["tgl_act"].chr(3).$rows["nm_usr"].chr(6);
	}
