...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT * FROM b_fom_med_chk_bdh_wnt b
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$dt.=$rows["id"].'|'.$rows['grade'].chr(3).number_format($i,0,",","").chr(3).$rows["keluhan"].chr(3).$rows["umbilical"].chr(3).$rows["inguinal"].chr(3).$rows["incisional"].chr(3).$rows["ampulla_recti"].chr(3).$rows["hemoroid"].chr(3).$rows["grade"].chr(3).$rows["polip"].chr(3).$rows["sphiner_ani"].chr(3).$rows["anjuran"].chr(6);
	}
