...
switch(strtolower($_REQUEST['act'])){
	...
	if($isRekam==0)
	{
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr, DATE_FORMAT(b.tgl_lanjut, '%d-%m-%Y') as tgl_lanjut1 FROM b_fom_resum_medis_new b
...
WHERE b.pelayanan_id='$idPel' ".$filter." and isRekam = '$isRekam' order by ".$sorting;
	}else{
		$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr, DATE_FORMAT(b.tgl_lanjut, '%d-%m-%Y') as tgl_lanjut1 FROM b_fom_resum_medis_new b
LEFT JOIN b_pelayanan bp ON bp.id=b.pelayanan_id
LEFT JOIN b_ms_pasien bmp ON bmp.id=bp.pasien_id
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisip = $rows["id"].'|'.urlencode($rows['terhadap']).'|'.urlencode($rows['jns']).'|'.urlencode($rows['ppembayaran']).'|'.urlencode($rows['dmasuk']).'|'.urlencode($rows['rpenyakit']).'|'.urlencode($rows['pemeriksaanf']).'|'.urlencode($rows['ppenunjang']).'|'.urlencode($rows['terapip']).'|'.urlencode($rows['hkonsultasi']).'|'.urlencode($rows['alergi']).'|'.urlencode($rows['hasillab']).'|'.urlencode($rows['diet']).'|'.urlencode($rows['dilanjutkan']).'|'.urlencode($rows['tgl_lanjut1']);
		$dt.= $sisip.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["anjuran"].chr(3).
		tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
