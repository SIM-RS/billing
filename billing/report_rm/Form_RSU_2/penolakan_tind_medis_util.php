...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_fom_tolak_tind_medis b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["resiko"];
		$dt.=$rows["id"].'|'.$rows['tindakan'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
