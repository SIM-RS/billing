...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_fom_resum_medis b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$dt.=$rows["id"].'|'.$rows['terhadap'].'|'.$rows['jns'].chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["anjuran"].chr(3).
