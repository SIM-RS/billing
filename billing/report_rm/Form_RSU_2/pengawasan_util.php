...
$sqlP="SELECT p.*, ku.no_reg,
  ...
  DATE_FORMAT(p.tgl_lahir,'%d-%m-%Y') AS tgl_lhr,p.nama_ortu as ortu,u.nama as unit2,TIMESTAMPDIFF(YEAR,p.tgl_lahir,CURDATE()) AS usia,mk.nama AS nm_kls,u.nama AS nm_unit,peg.nama AS dokter,
  CONCAT((CONCAT((CONCAT((CONCAT(p.alamat,' RT.',p.rt)),' RW.',p.rw)),', Desa ',w.nama)),', Kecamatan ',wi.nama,', ',mw2.nama) alamat_,
