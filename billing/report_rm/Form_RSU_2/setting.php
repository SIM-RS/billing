...
/*$sql = "SELECT DISTINCT p.nama,
		...
		YEAR(CURDATE()) - YEAR(p.tgl_lahir) AS umur, GROUP_CONCAT(md.nama) AS diag, peg.nama AS dokter, 
  ...
  DATE_FORMAT(bpk.tgl_act, '%d-%m-%Y') tgl_keluar
	...
	$sql = "SELECT DISTINCT p.nama,
		...
		YEAR(CURDATE()) - YEAR(p.tgl_lahir) AS umur, md.nama AS diag, peg.nama AS dokter,p.nama_ortu as ortu2, 
		p.`no_rm`, u.nama AS ruang_rawat, kel.nama AS kelas, u.nama AS unit,
...
$cara_keluar = $identitas["cara_keluar"];
$tgl_keluar = $identitas["tgl_keluar"];
$ortu = $identitas["ortu2"];
?>
