...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_ms_check_kardiologi b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=$rows["tekanan_darah"].'|'.$rows['nadi'].'|'.$rows['tumor'].'|'.$rows['jvp'].'|'.$rows['paru_paru'].'|'.$rows['jantung'].'|'.$rows['hati'].'|'.$rows['limpa'].'|'.$rows['lain_lain'].'|'.$rows['edema'].'|'.$rows['ekg'].'|'.$rows['treadmil'].'|'.$rows['echokardiografi'].'|'.$rows['anjuran'];
