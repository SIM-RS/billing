...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_ms_check_neurologi b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=$rows["kesadaran"].'|'.$rows['selaput_otak'].'|'.$rows['intrakranial'].'|'.$rows['syaraf_otak'].'|'.$rows['motorik'].'|'.$rows['patologis'].'|'.$rows['sensorik'].'|'.$rows['otonom'].'|'.$rows['luhur'].'|'.$rows['lain_lain'].'|'.$rows['eeg'].'|'.$rows['anjuran'];
