...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_ms_check_tht b
LEFT JOIN b_pelayanan bp ON bp.id=b.pelayanan_id
LEFT JOIN b_ms_pasien bmp ON bmp.id=bp.pasien_id
LEFT JOIN b_ms_pegawai pp ON pp.id=bp.user_act
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$daun=explode('|',$rows["daun"]);
		$liang=explode('|',$rows["liang"]);
		$membran=explode('|',$rows["membran"]);
		$audio=explode('|',$rows["audiogram"]);
		
		$sisanya=$daun[0].'|'.$daun[1].'|'.$liang[0].'|'.$liang[1].'|'.$membran[0].'|'.$membran[1].'|'.$audio[0].'|'.$audio[1].'|'.$rows['hidung'].'|'.$rows['rhinopharynx'].'|'.$rows['larynx'].'|'.$rows['anjuran'];
