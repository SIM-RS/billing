...
$sql = "SELECT DISTINCT p.nama,
		...
		YEAR(CURDATE()) - YEAR(p.tgl_lahir) AS umur, GROUP_CONCAT(distinct md.nama) AS diag, peg.nama AS dokter, 
		p.`no_rm`, u.nama AS ruang_rawat, kel.nama AS kelas, u.nama AS unit,
  DATE_FORMAT(kun.tgl_act, '%d-%m-%Y') tglawal, DATE_FORMAT(kun.tgl_act, '%H:%i:%s') jamawal, btt.nama tindakan, bmk.nama klasifikasi,kk.no_reg as no_reg2
	FROM b_pelayanan k
