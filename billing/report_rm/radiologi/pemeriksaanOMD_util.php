...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_ms_periksa_omd b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=tglSQL($rows["tgl_datang"]).'|'.$rows['jam_datang'].'|'.$rows['jam_siap1'].'|'.$rows['jam_siap2'];
