...
switch($_REQUEST['id']) {
	...
	case 'JnsLayananKonsul':
        $wil='';
        $sql="SELECT id,nama FROM b_ms_unit WHERE (kategori=2 OR kategori=5) AND aktif=1 AND LEVEL=1 AND id IN (SELECT DISTINCT parent_id FROM b_ms_unit WHERE inap=0 AND LEVEL=2 AND aktif=1)";
		break;
    case 'JnsLayananJln':
		$wil='';
