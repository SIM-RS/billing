...
$idUser=$_REQUEST['idUsr']; //untuk laporan
...
$sql = "SELECT DISTINCT p.id, p.nama,
		...
		YEAR(CURDATE()) - YEAR(p.tgl_lahir) AS umur, md.nama AS diag, peg.nama AS dokter, peg.id AS dokter_id,
		p.`no_rm`, u.nama AS ruang_rawat, kel.nama AS kelas, u.nama AS unit,
  DATE_FORMAT(kun.tgl_act, '%d-%m-%Y') tglawal, DATE_FORMAT(kun.tgl_act, '%H:%i:%s') jamawal, kun.no_reg
	FROM b_pelayanan k
...
$noreg1 = $identitas["no_reg"];
$id_pasien = $identitas["id"];
?>
