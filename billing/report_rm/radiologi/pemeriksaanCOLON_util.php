...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_ms_periksa_colon b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=tglSQL($rows["tgl_datang"]).'|'.$rows['jam_datang'].'|'.$rows['jam_siap1'].'|'.$rows['jam_siap2'].'|'.$rows['jam_siap3'].'|'.$rows['jam_siap4'];
		$dt.=$rows["id"].'|'.$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).tglSQL($rows["tgl_datang"]).chr(3).$rows["jam_datang"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
