...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_laporan_obstetri_ginekologi b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['lama'].'|'.$rows['cerai'].'|'.$rows['keluhan'].'|'.$rows['menarche'].'|'.$rows['lamahaid'].'|'.$rows['dysmenorrhoe'].'|'.$rows['dyspareuni'].'|'.$rows['kontrasepsi'].'|'.$rows['flour'].'|'.$rows['fluxus'].'|'.$rows['gangguan_m'].'|'.$rows['gangguan_d'].'|'.$rows['lain'].'|'.$rows['riwayat_o'].'|'.$rows['riwayat_g'].'|'.$rows['abdomen'].'|'.$rows['vulva'].'|'.$rows['uretra'].'|'.$rows['vagina'].'|'.$rows['cervix'].'|'.$rows['adnexa'].'|'.$rows['cul'].'|'.$rows['pap'].'|'.$rows['anjuran'];
		$dt.=$rows["id"].'|'.$rows['nikah'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["nikah"].chr(3).$rows["keluhan"].chr(3).$rows["anjuran"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
