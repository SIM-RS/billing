...
$queryD = "SELECT b.nama FROM lap_srt_balasan a INNER JOIN b_ms_pegawai b ON a.user_act = b.id WHERE a.pelayanan_id = '$idPel' AND a.kunjungan_id = '$idKunj'";
$dqueryD = mysql_fetch_array(mysql_query($queryD));

?>
<script>
<?
	if($identitas["dr_rujuk"]!="")
	{
?>
...
<?
	...
	{
?>
jQuery("#dr").text("<?=$dqueryD["nama"]?>");
jQuery(".gb1").text("<?=$dqueryD["nama"]?>");
<?
	}else{
		?>
jQuery("#dr").text("<?=$_REQUEST['nmUser'];?>");
jQuery(".gb1").text("<?=$_REQUEST['nmUser'];?>");		
		<?
	}
?>
</script>
