...
switch(strtolower($_REQUEST['act'])){
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["alamat"].'|'.$rows["no_ktp"].'|'.tglSQL($rows["tgl_r"]).'|'.tglSQL($rows["tgl_l"]).'|'.tglSQL($rows["tgl_ra"]).'|'.$rows["hasil_lain"].'|'.$rows["hubungan"];
		$dt.=$rows["id"].'|'.$rows['penerima_kuasa'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["penerima_kuasa"].chr(3).$rows["alamat"].chr(3).$rows["no_ktp"].chr(3).tglSQL($rows["tgl_r"]).chr(3).tglSQL($rows["tgl_l"]).chr(3).tglSQL($rows["tgl_ra"]).chr(3).$rows["hasil_lain"].chr(3).$rows["hubungan"].chr(3).$rows["nama2"].chr(6);
	}
