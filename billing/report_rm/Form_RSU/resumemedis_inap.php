...
INNER JOIN b_ms_satuan_lab msl ON b.id_satuan=msl.id ORDER BY a.id_pelayanan,c.kode_urut";
	...
	WHERE ((t.pelayanan_id = '".$_REQUEST['idPel']."') OR (pl.kunjungan_id = '".$_REQUEST['idKunj']."' AND pl.unit_id IN ('63','72'))) AND tin.id NOT IN (74,3076,3090,2333) AND tin.kel_tindakan_id NOT IN (61,304)) AS gab";
					...
					*/
					$qDiagAkhir = "SELECT 
					  GROUP_CONCAT(IFNULL(b_diagnosa.diagnosa_manual, b_ms_diagnosa.nama) SEPARATOR '<br>&nbsp;') AS diagnosa
					FROM
					   b_diagnosa
					  LEFT JOIN b_ms_diagnosa
						ON b_diagnosa.ms_diagnosa_id = b_ms_diagnosa.id 
					  INNER JOIN b_pelayanan 
						ON b_pelayanan.id = b_diagnosa.pelayanan_id 
					  INNER JOIN b_ms_pegawai 
						ON b_diagnosa.user_id = b_ms_pegawai.id 
					WHERE b_diagnosa.kunjungan_id = '".$_REQUEST['idKunj']."'
					  AND b_pelayanan.jenis_kunjungan = 3
					  AND b_diagnosa.tgl='".$tgl_diag."'
					GROUP BY b_diagnosa.kunjungan_id";
