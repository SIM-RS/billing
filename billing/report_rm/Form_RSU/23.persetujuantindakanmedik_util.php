...
switch(strtolower($_REQUEST['act'])){
	...
	$idPel=$_REQUEST['idPel'];
	$id=$_REQUEST['id'];
	$sql="SELECT a.*,b.nama as nama2,c.nama as nama3 
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["id_tindakan2"].'|'.$rows["resiko"].'|'.$rows["alasan"].'|'.$rows["saksi1"].'|'.$rows["saksi2"].'|'.$rows["saksirs"];
		$dt.=$rows["id"].'|'.$rows['id_tindakan1'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama2"].chr(3).$rows["nama3"].chr(3).$rows["resiko"].chr(3).$rows["alasan"].chr(3).$rows["saksi1"].chr(3).$rows["saksi2"].chr(3).$rows["saksirs"].chr(6);
	}
