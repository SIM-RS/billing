...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["taggal_lahir"].'|'.$rows["umur"].'|'.$rows["jenis_kelamin"].'|'.$rows["alamat"].'|'.$rows["no_telp"].'|'.$rows["no_ktp_sim"].'|'.$rows["hubungan"].'|'.$rows["pemahaman"].'|'.$rows["tanggal"].'|'.$rows["jam"].'|'.$rows["saksi1"].'|'.$rows["saksi2"];
