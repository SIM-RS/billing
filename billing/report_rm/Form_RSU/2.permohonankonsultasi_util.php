...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr,pp2.nama as dokter2 FROM b_konsultasi b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['keluhan'].'|'.$rows['hasil'].'|'.$rows['pilihan'];
		$dt.=$rows["id"].'|'.$rows['dokter'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["dokter2"].chr(3).$rows["keluhan"].chr(3).$rows["hasil"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
