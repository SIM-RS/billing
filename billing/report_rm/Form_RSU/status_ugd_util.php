...
$alasan='';
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_ms_status_ugd b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		...
		$sisanya='|'.$rows["jam"]
				...
				.'|'.$rows["trauma"]
				.'|'.$rows["pasien_datang"]
				.'|'.$rows["ket_pasien_datang"]
				.'|'.$rows["kasus"]
				.'|'.$rows["riwayat_alergi"]
				.'|'.$rows["ket_riwayat_alergi"]
				.'|'.$rows["pemeriksaan_fisik"]
				.'|'.$rows["ket_pemeriksaan_fisik"]
				.'|'.$rows["keluhan_utama"]
				.'|'.$rows["keluhan_tambahan"]
				.'|'.$rows["penyakit_sekarang"]
				.'|'.$rows["penyakit_dahulu"]
				.'|'.$rows["keadaan_umum"]
				.'|'.$rows["e"]
				.'|'.$rows["m"]
				.'|'.$rows["v"]
