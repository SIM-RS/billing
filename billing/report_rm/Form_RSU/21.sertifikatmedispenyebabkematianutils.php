...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr, peg.nama AS dokter FROM sertifikat_kematian b
LEFT JOIN b_pelayanan bp ON bp.id=b.pelayanan_id
LEFT JOIN b_ms_pasien bmp ON bmp.id=bp.pasien_id
LEFT JOIN b_tindakan bmt ON bp.id = bmt.pelayanan_id
LEFT JOIN b_ms_pegawai peg ON peg.id = bmt.user_act
LEFT JOIN b_ms_pegawai pp ON pp.id=b.user_act
WHERE b.pelayanan_id='$idPel' ".$filter." group by s_mati_id order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=$rows["statuspenduduk"].'|'.$rows["hubungan"].'|'.$rows["kepala"].'|'.$rows["tempat"].'|'.$rows["jenazah"].'|'.$rows["kualifikasi"].'|'.$rows["diagnosis"].'|'.$rows["penyakit"].'|'.$rows["gangguan"].'|'.$rows["cedera"].'|'.$rows["lain"].'|'.$rows["tgl_stat_jnzh"].'|'.$rows["namapemeriksa"].'|'.$rows["tgl_priksa_jnzh"].'|'.$rows["lainnya"].'|'.$rows["kematianA"].'|'.$rows["kematianB"].'|'.$rows["kematianC"].'|'.$rows["kematianD"].'|'.$rows["kematian2A"].'|'.$rows["kematian2B"].'|'.$rows["kematian2C"].'|'.$rows["kematian2D"].'|'.$rows["kondisi"];
		$dt.=$rows["s_mati_id"].'|'.$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$rows["nama_usr"].chr(6);
	}
