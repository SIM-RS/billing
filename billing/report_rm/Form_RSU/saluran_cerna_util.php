...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["identitas"].'|'.$rows["gastroskopi"].'|'.$rows["klonoskopi"].'|'.$rows["ligasi"].'|'.$rows["polipectomy_gastroskopi"].'|'.$rows["kauterisasi_gastroskopi"].'|'.$rows["st_hemoroid"].'|'.$rows["safary_bouginage"].'|'.$rows["kolonoskopi_sth"].'|'.$rows["polipectomi_hautensasi_kolon"].'|'.$rows["heat_probe_vikusgaster"];
		$dt.=$rows["id"].'|'.tglSQL($rows['tgl']).$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
