...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr,pp2.nama as dokter2 FROM b_ms_pemakain_obat b
...
LEFT JOIN b_ms_pegawai pp2 ON pp2.id=b.dokter
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['dokter'].'|'.$rows['jenis_operasi'].'|'.$rows['a'].'|'.$rows['b'].'|'.$rows['c'].'|'.$rows['d'].'|'.$rows['e'].'|'.$rows['f'].'|'.$rows['g'].'|'.$rows['h'].'|'.$rows['i'].'|'.$rows['j'].'|'.$rows['k'].'|'.$rows['l'].'|'.$rows['m'].'|'.$rows['n'].'|'.$rows['o'].'|'.$rows['p'].'|'.$rows['q'].'|'.$rows['r'].'|'.$rows['s'].'|'.$rows['t'].'|'.$rows['u'].'|'.$rows['v'].'|'.$rows['w'].'|'.$rows['x'].'|'.$rows['y'].'|'.$rows['z'].'|'.$rows['aa'].'|'.$rows['bb'].'|'.$rows['cc'].'|'.$rows['dd'].'|'.$rows['ee'].'|'.$rows['ff'].'|'.$rows['gg'].'|'.$rows['hh'].'|'.$rows['ii'].'|'.$rows['jj'];
		$dt.=$rows["id"].'|'.tglSQL($rows['tgl']).$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["dokter2"].chr(3).$rows["jenis_operasi"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
