...
/*switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_fom_asuhan_gizi b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=$rows['antropometri'].'|'.$rows['antropometri_eval'].'|'.$rows['biokimia'].'|'.$rows['biokimia_eval'].'|'.$rows['fisik'].'|'.$rows['fisik_eval'].'|'.$rows['riwayat_gizi'].'|'.$rows['riwayat_gizi_eval'].'|'.$rows['riwayat_personal'].'|'.$rows['riwayat_personal_eval'];
		$dt.=$rows["id"].'|'.$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["diagnosa_gizi"].chr(3).$rows["intervensi"].chr(3).$rows["rencana_monev"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
