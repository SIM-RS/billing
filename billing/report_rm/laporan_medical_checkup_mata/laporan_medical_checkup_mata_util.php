...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_laporan_mata b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['od'].'|'.$rows['os'].'|'.$rows['presbyopia'].'|'.$rows['visus1'].'|'.$rows['visus2'].'|'.$rows['koreksi1'].'|'.$rows['koreksi2'].'|'.$rows['adisi1'].'|'.$rows['adisi2'].'|'.$rows['kedudukan1'].'|'.$rows['kedudukan2'].'|'.$rows['palpebra1'].'|'.$rows['palpebra2'].'|'.$rows['conjunctiva1'].'|'.$rows['conjunctiva2'].'|'.$rows['cornea1'].'|'.$rows['cornea2'].'|'.$rows['coa1'].'|'.$rows['coa2'].'|'.$rows['pupil1'].'|'.$rows['pupil2'].'|'.$rows['iris1'].'|'.$rows['iris2'].'|'.$rows['lensa1'].'|'.$rows['lensa2'].'|'.$rows['vitreous1'].'|'.$rows['vitreous2'].'|'.$rows['fundus1'].'|'.$rows['fundus2'].'|'.$rows['tio1'].'|'.$rows['tio2'].'|'.$rows['campus1'].'|'.$rows['campus2'].'|'.$rows['test'].'|'.$rows['anjuran'];
		$dt.=$rows["id"].'|'.$rows['keluhan'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["keluhan"].chr(3).$rows["presbyopia"].chr(3).$rows["anjuran"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
