...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_form_persetujuan_pemberian_darah b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.tglSQL($rows["taggal_lahir"]).'|'.$rows["umur"].'|'.$rows["jenis_kelamin"].'|'.$rows["alamat"].'|'.$rows["no_telp"].'|'.$rows["no_ktp_sim"].'|'.$rows["hubungan"].'|'.$rows["pemahaman"].'|'.tglSQL($rows["tanggal"]).'|'.$rows["jam"].'|'.$rows["saksi1"].'|'.$rows["saksi2"];
		
		$dt.=$rows["id"].'|'.$rows['name'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
