...
switch(strtolower($_REQUEST['act'])){
	...
	$id=$_REQUEST['id'];
	$sql="SELECT * FROM b_ms_laporan_kejadian a where a.pelayanan_id='$idPel'";
	/*$sql="SELECT a.*,DATE_FORMAT(a.tgl_jam, '%d-%m-%Y') tgl, DATE_FORMAT(a.tgl_jam, '%H:%i:%s') jam 
	FROM b_ms_pengawasan_khusus_bayi a where a.pelayanan_id='$idPel'";*/
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["kronologis"].'|'.$rows["jenis_kejadian"].'|'.$rows["pelapor"].'|'.$rows["kejadian_mengenai"].'|'.tglSQL($rows["tgl"]).'|'.$rows["jam"].'|'.$rows["unit_kerja"].'|'.$rows["lokasi_kejadian"].'|'.$rows["akibat_kejadian"].'|'.$rows["kejadian_sama"].'|'.$rows["jumlah_kejadian"].'|'.$rows["tindakan"].'|'.$rows["grading_resiko"];
		$dt.=$rows["id"].'|'.$rows['pokok_masalah'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["pokok_masalah"].chr(3).$rows["kronologis"].chr(3).$rows["jenis_kejadian"].chr(3).$rows["pelapor"].chr(3).$rows["kejadian_mengenai"].chr(3).tglSQL($rows["tgl"]).'/<br>'.$rows["jam"].chr(3).$rows["unit_kerja"].chr(3).$rows["lokasi_kejadian"].chr(3).$rows["akibat_kejadian"].chr(3).$rows["kejadian_sama"].chr(3).$rows["jumlah_kejadian"].chr(3).$rows["tindakan"].chr(3).$rows["grading_resiko"].chr(6);
	}
