...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,pp.nama AS nama_usr FROM b_fom_monitoring_eso b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya=$rows["status_perempuan"].'|'.$rows["kesudahan"].'|'.$rows["penyakit"].'|'.$rows["saat"].'|'.$rows["riwayat_eso"].'|'.$rows["kesudahan_eso"].'|'.$rows["ket_tambahan"].'|'.$rows["data_lab"];
		$dt.=$rows["id"].'|'.$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["manifestasi"].chr(3).
