...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_ceklist_salurancerna b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["colonoscope"].'|'.$rows["duadenoscope"].'|'.$rows["brochoscope"].'|'.$rows["s_date"].'|'.$rows["s_teskebocoran"].'|'.$rows["s_tandakebocoran"].'|'.$rows["s_skope"].'|'.$rows["s_nobath"].'|'.$rows["s_mulaiperendaman"].'|'.$rows["s_selesaiperendaman"]
		.'|'.$rows["s_penanggungjawab"].'|'.$rows["s_spoel_airbersih"].'|'.$rows["s_spoel_alkohol"].'|'.$rows["s_penanggungjawab2"].'|'.$rows["a_date"].'|'.$rows["a_teskebocoran"].'|'.$rows[" a_tandakebocoran"].'|'.$rows["a_skope"].'|'.$rows["a_nobath"].'|'.$rows["a_mulaiperendaman"].'|'.$rows["a_selesaiperendaman"].'|'.$rows["a_penanggungjawab"].'|'.$rows[" a_spoel_airbersih"].'|'.$rows[" a_spoel_alkohol"].'|'.$rows["a_penanggungjawab2"];
