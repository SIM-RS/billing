...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_ms_pengkajian_awal_medis b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows["jam"].'|'.$rows["anamnesis"].'|'.$rows["hubungan"].'|'.$rows["keluhan_utama"].'|'.$rows["riwayat_ps"].'|'.$rows["alergi"].'|'.$rows["tahun"].'|'.$rows["riwayat_pd"].'|'.$rows["riwayat_pengobatan"].'|'.$rows["riwayat_pdk"].'|'.$rows["riwayat_pekerjaan"];
		$dt.=$rows["id"].'|'.tglSQL($rows['tanggal']).$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
