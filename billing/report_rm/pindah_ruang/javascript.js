...
/*              INPUT TABLE DINAMIS                 */
jQuery(function(){
	jQuery('.tambahRow').click(function(){
		var idx = jQuery('.catatanRow tr').length;
		jQuery('.catatanRow').append('<tr class="itemRow">' +
				'<input type="hidden" id="rawID" name="rawID[]" value="1" />' +
				'<td width=40 height=20 align=center>'+(parseInt(idx)+1)+'</td>' +
	...
	jQuery('.catatanRow').on('click', '.hapus',function(){
		if(confirm('Apakah anda yakin?')){
			...
			elm.remove();
			/*jQuery('.catatanRow tr').each(function(index, element) {
				jQuery(this).find('td:first').html(index+1);
			});	*/		
		}
	});
});
...
jQuery(function(){
	jQuery('.tambahRow2').click(function(){
		var idx2 = jQuery('.obatRow tr').length;
		jQuery('.obatRow').append('<tr class="itemRow2">' +
				'<input type="hidden" id="rawID2" name="rawID2[]" value="1" />' +
				'<td width=40 height=20 align=center>'+ (parseInt(idx2)+1) +'</td>' +
				'<td width=240 align=center><input type="text" class="catatan" id="nama_obat" name="nama_obat[]" /></td>' +
				'<td width=190 align=center><input type="text" class="catatan" id="dosis" name="dosis[]" /></td>' +
				'<td width=100 align=center><input type="text" class="catatan" id="jumlah_obat" name="jumlah_obat[]"  /></td>' +
				'<td width=150 align=center><input type="text" class="catatan" id="sisa_obat" name="sisa_obat[]" /></td>' +
				'<td width=50 align=center>' +
	...
	jQuery('.obatRow').on('click', '.hapus2',function(){
		if(confirm('Apakah anda yakin?')){
			var id = jQuery(this).parents('tr.itemRow2').find('td:eq(0) input:eq(0)').val();
			var elm = jQuery(this).parents('tr.itemRow2');
			//alert(id);
			var tmp = jQuery('#deleteObat').val();
			jQuery('#deleteObat').val(tmp+id+',');
			elm.remove();
			/*jQuery('.obatRow tr').each(function(index, element) {
				jQuery(this).find('td:first').html(index+1);
			});*/		
		}
	});
