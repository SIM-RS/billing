...
$sql = "SELECT DISTINCT p.nama,p.gol_darah,
		CONCAT((CONCAT((CONCAT((CONCAT(p.alamat,' RT.',p.rt)),' RW.',p.rw)),', Desa ',w.nama)),', Kecamatan ',wi.nama,', ',wii.nama) alamat_,
		p.sex, p.telp, DATE_FORMAT(p.`tgl_lahir`, '%d-%m-%Y') lahir, p.no_ktp,
		YEAR(CURDATE()) - YEAR(p.tgl_lahir) AS umur, md.nama AS diag, peg.nama AS dokter, 
		p.`no_rm`, u.nama AS ruang_rawat, kel.nama AS kelas, u.nama AS unit,
  DATE_FORMAT(kun.tgl_act, '%d-%m-%Y') tglawal, DATE_FORMAT(kun.tgl_act, '%H:%i:%s') jamawal,kk.no_reg AS no_reg2,
  CONCAT(DATE_FORMAT(CURDATE(),'%m-%Y'),'.',kk.no_reg,'.', p.`no_rm`,'.',k.id) AS no_formulir,kun.umur_bln,kun.umur_thn
  ,anam.*
	FROM b_pelayanan k
	INNER JOIN b_kunjungan kun
		ON k.kunjungan_id = kun.id
	LEFT JOIN (SELECT z.KUNJ_ID, z.KU, z.SOS, z.RPS, z.RPD, z.RPK, z.RA, z.KUM, z.GCS, z.KESADARAN, z.TENSI, z.TENSI_DIASTOLIK, z.RR, z.NADI, z.SUHU, z.TB, z.BB, z.GIZI, z.KL, z.LEHER, z.ABDOMEN, z.GENITALIA, z.COR, z.AMATA, z.PULMO, z.INSPEKSI, z.PALPASI, z.AUSKULTASI, z.PERKUSI, z.EXT, z.THORAX, z.UTINLANJUT FROM anamnese z ORDER BY z.ANAMNESE_ID DESC) anam
...
$umur = $identitas["umur_thn"]." Tahun ".$identitas["umur_bln"]." Bulan";
$sex = $identitas["sex"];
...
$TENSI = $identitas['TENSI'];
$TENSI_DIASTOLIK = $identitas['TENSI_DIASTOLIK'];
$nadi = $identitas['NADI'];
$suhu = $identitas['SUHU'];
$Respirasi = $identitas['RR'];
$bb = $identitas['BB'];
$tb = $identitas['TB'];
$cor = $identitas['COR'];
$pulma = $identitas['PULMO'];
$abdomen = $identitas['ABDOMEN'];
$eks = $identitas['EXT'];
$gol_darah=$identitas['gol_darah'];
$UTINLANJUT=$identitas['UTINLANJUT'];
$RPD=$identitas['RPD'];
$RPK=$identitas['RPK'];
$RPS=$identitas['RPS'];
$AMATA=$identitas['AMATA'];
$GCS=$identitas['GCS'];
?>
