...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_vaccination b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['sakit'].'|'.$rows['derita'].'|'.$rows['terapi'].'|'.$rows['sebut'].'|'.$rows['obat'].'|'.$rows['makanan'].'|'.$rows['reaksi'].'|'.$rows['wanita'].'|'.$rows['vaksinasi'].'|'.$rows['riwayat'];
		$dt.=$rows["id"].'|'.tglSQL($rows['tgl']).$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).tglSQL($rows["tgl"]).chr(3).$rows["derita"].chr(3).$rows["riwayat"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
