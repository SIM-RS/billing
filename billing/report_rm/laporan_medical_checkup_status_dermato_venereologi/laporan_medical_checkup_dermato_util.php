...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_laporan_dermato b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['alergi'].'|'.$rows['p_kulit'].'|'.$rows['k_rambut'].'|'.$rows['disuri'].'|'.$rows['discharge'].'|'.$rows['lain'].'|'.$rows['anak_h'].'|'.$rows['anak_m'].'|'.$rows['hamil'].'|'.$rows['hamil_ke'].'|'.$rows['pms'].'|'.$rows['cs'].'|'.$rows['kulit'].'|'.$rows['getah'].'|'.$rows['saraf'].'|'.$rows['rambut'].'|'.$rows['kuku'].'|'.$rows['gene'].'|'.$rows['anjuran'];
		$dt.=$rows["id"].'|'.$rows['keluhan'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["keluhan"].chr(3).$rows["alergi"].chr(3).$rows["anjuran"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
