...
switch(strtolower($_REQUEST['act'])){
	...
	$sql="SELECT b.*,bmp.nama,bmp.no_rm,bmp.nama,pp.nama AS nama_usr FROM b_ket_pem_ulang_transfusi b
...
WHERE b.pelayanan_id='$idPel' ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$sisanya='|'.$rows['nominal2'].'|'.$rows['name'].'|'.$rows['address'].'|'.$rows['hubungan'].'|'.$rows['menyatakan'];
		$dt.=$rows["id"].'|'.$rows['nominal'].$sisanya.chr(3).number_format($i,0,",","").chr(3).$rows["no_rm"].chr(3).$rows["name"].chr(3).$rows["address"].chr(3).$rows["hubungan"].chr(3).tglSQL($rows["tgl_act"]).chr(3).$rows["nama_usr"].chr(6);
	}
