...
switch($_REQUEST['id']) {
    ...
    case 'JnsLayanan':
        $wil='';
		//$all = ($_REQUEST['reg'] != '' && $_REQUEST['reg'] == 1) ? 'true' : 'false';
		//$all = 'true';
...
WHERE kamar.unit_id = $val[0] AND kamar.aktif=1";
    ...
    case 'Retribusi':
		...
		$idKlsTind = " inner join b_ms_tindakan_kelas tk on tk.ms_tindakan_id = b.id
			    inner join b_ms_tindakan_unit tu on tu.ms_tindakan_kelas_id = tk.id LEFT JOIN b_ms_kso ks ON ks.id = tk.kso_id where klasifikasi_id = 11 and tu.ms_unit_id = '$val[1]'";
			//	echo $idKlsTind;
	    ...
	    //$wil = '-';
            $idKlsTind = " inner join b_ms_tindakan_kelas tk on tk.ms_tindakan_id = b.id
				inner join b_ms_tindakan_unit tu on tu.ms_tindakan_kelas_id = tk.id LEFT JOIN b_ms_kso ks ON ks.id = tk.kso_id where klasifikasi_id=11 and tu.ms_unit_id = '$val[1]'"; //IGD
				//echo $idKlsTind;
        }
        else if($val[0]=='50' || $val[0]=='132') {
            $idKlsTind = " inner join b_ms_tindakan_kelas tk on tk.ms_tindakan_id = b.id
			    inner join b_ms_tindakan_unit tu on tu.ms_tindakan_kelas_id = tk.id LEFT JOIN b_ms_kso ks ON ks.id = tk.kso_id where klasifikasi_id=101 and tu.ms_unit_id = '$val[1]'"; //PAVILYUN
							//echo $idKlsTind;
        }
        $sql="SELECT tk.id,CONCAT(b.nama,' - ',ks.nama) as nama,tk.tarip as lang FROM b_ms_tindakan b $idKlsTind and b.id<>1918 ORDER BY nama asc";
		$rs = mysql_query($sql);
		if(mysql_num_rows($rs) == 0){
			$sql = "select '0' as id, '-' as nama, 0 as lang";
		}
        break;
    case 'RetribusiPcrKlinik' :
            $wil='';
            $lang = true;
            $sql = "SELECT DISTINCT b.id,b.nama,tk.tarip AS lang FROM b_ms_tindakan b INNER JOIN b_ms_tindakan_kelas tk ON tk.ms_tindakan_id = b.id INNER JOIN b_ms_tindakan_unit tu ON tu.ms_tindakan_kelas_id = tk.id WHERE klasifikasi_id = 11 AND tk.tarip = 0 AND tu.ms_unit_id = {$val[1]} AND b.id <> 1918 ORDER BY nama ASC";
            $rs = mysql_query($sql);
            if(mysql_num_rows($rs) == 0) $sql  = "SELECT '0' AS id, '-' AS nama, 0 AS lang";
        break;
    case 'cmbAgama':
        $wil='';
...
WHERE b.`nama`='".$val[0]."'";
	...
	case 'cmbIsiDataRM':
		...
		break;
	case 'retribusiRawatInap':
		$dataGet = explode('|',$_REQUEST['value']);
		$ksoPasienGet = $dataGet[1];
		$tempat_layanan = $dataGet[0];
		if($ksoPasienGet == 1) $tipe = 1;
		elseif($ksoPasienGet == 6) $tipe = 2;
		else $tipe = 3;

		$sql = "SELECT rri.nilai, rri.tempat_layanan,CONCAT(rri.nilai,' - ',u.nama) as nama FROM b_ms_retribusi_rawat_inap as rri LEFT JOIN b_ms_unit u ON u.id = rri.tempat_layanan WHERE tipe = {$tipe} AND rri.tempat_layanan = {$tempat_layanan}";

		$query = mysql_query($sql);
		while($rows = mysql_fetch_assoc($query)){
			$selected = $tempat_layanan == $rows['tempat_layanan'] ? "selected" : "";
			echo '<option value="'.$rows['nilai'].'" '.$selected.'>'.$rows['nama'].'</option>';
		}
		exit;
		break;
}
