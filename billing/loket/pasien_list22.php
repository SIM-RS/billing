...
WHERE p.nama LIKE '".$keyword[0]."%' AND p.alamat LIKE '%".$keyword[1]."%' ORDER BY p.nama, p.alamat LIMIT 250";
			...
			if($total==0 && $_REQUEST['txt']=='NoKTP') //jika tidak ada datanya
			{
				...
				$no_ktp = $keyword[2];
				$q = "SELECT a.*,IF(a.JENIS_KLMIN=1,'L','P')AS GENDER,b.NO_KK,b.ALAMAT,b.NO_RT,b.NO_RW,b.TELP,c.`id` AS PROP,d.`id` AS KAB,e.`id` AS KEC,f.`id` AS KEL FROM t_biodata_wni AS a 
INNER JOIN t_data_keluarga AS b ON b.NIK_KK=a.NIK 
INNER JOIN $db_billing.b_ms_wilayah AS c ON c.`kode_siak`=a.`NO_PROP`  
INNER JOIN $db_billing.b_ms_wilayah AS d ON d.`kode_siak`=CONCAT(a.`NO_PROP`,'.',a.`NO_KAB`)
INNER JOIN $db_billing.b_ms_wilayah AS e ON e.`kode_siak`=CONCAT(a.`NO_PROP`,'.',a.`NO_KAB`,'.',a.`NO_KEC`)
INNER JOIN $db_billing.b_ms_wilayah AS f ON f.`kode_siak`=CONCAT(a.`NO_PROP`,'.',a.`NO_KAB`,'.',a.`NO_KEC`,'.',a.`NO_KEL`) where a.NO_KTP like '%$no_ktp%'"; 
				$s = mysql_query($q); echo mysql_error();
			?>
			...
			<?
				...
				{
					/*$PROP = mysql_fetch_array(mysql_query("SELECT id from b_ms_wilayah where kode_siak='$d[NO_PROP]'"));
					$KAB = mysql_fetch_array(mysql_query("SELECT id from b_ms_wilayah where kode_siak='$d[NO_PROP].$d[NO_KAB]'"));
					$KEC = mysql_fetch_array(mysql_query("SELECT id from b_ms_wilayah where kode_siak='$d[NO_PROP].$d[NO_KAB].$d[NO_KEC]'"));
					$KEL = mysql_fetch_array(mysql_query("SELECT id from b_ms_wilayah where kode_siak='$d[NO_PROP].$d[NO_KAB].$d[NO_KEC].$d[NO_KEL]'"));*/
					
				 $val="||".$d['NAMA_LGKP']."|".tglSQL($d['TGL_LHR'])."|".$d['ALAMAT']."|".
				   $d['NO_RT']."|".$d['NO_RW']."|".$d['KEL']."|".$d['KEC']."|".$d['KAB']."|".$d['PROP']."|".$d['NAMA_LGKP_AYAH']."||".$d['GENDER']."|".$d['PDDK_AKH']."|".$d['JENIS_PKRJN']."|".$d['AGAMA']."|".$d['TELP']."|||||".$d['NO_KTP']; //echo $val;
			?>
