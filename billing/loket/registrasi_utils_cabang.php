...
switch(strtolower($_REQUEST['act'])) {
    ...
    //$rs=mysql_query("select * from b_ms_pasien where id='".$idPasien."' limit 0,10");
        if($idPasien=="") {
            ...
            $sqlTambah="insert into b_ms_pasien (no_rm,no_ktp,nama,sex,agama,tgl_lahir,alamat,rt,rw,desa_id,kec_id,kab_id,prop_id,telp,pendidikan_id,pekerjaan_id,nama_ortu,nama_suami_istri,user_act,tgl_act,gol_darah,cabang_id)
				values('$noRm','$noKTP','$nama','$gender','$agama','$tglLhr','$alamat','$rt','$rw','$desa','$kec','$kab','$prop','$telp','$pend','$pek','$namaOrtu','$namaSuTri','$userId','$tglact','$darah',$cabang)";
            ...
            $sqlUpdate="update b_ms_pasien set nama='$nama',sex='$gender',agama='$agama',tgl_lahir='$tglLhr',alamat='$alamat',rt='$rt',rw='$rw',desa_id='$desa',kec_id='$kec',kab_id='$kab',prop_id='$prop',telp='$telp',".(($pend=='0')?'':"pendidikan_id='$pend',").(($pek=='0')?'':"pekerjaan_id='$pek',")."nama_ortu='$namaOrtu',nama_suami_istri='$namaSuTri',no_ktp='$noKTP', gol_darah='$darah' 
        ...
        /*$sqlKunj="insert into b_kunjungan (no_billing,loket_id,pasien_id,asal_kunjungan,ket,jenis_layanan,unit_id,retribusi,tgl,umur_thn,umur_bln,kel_umur,kelas_id,kso_id,kso_kelas_id,tgl_sjp,no_sjp,no_anggota,status_penj,nama_peserta,diag_awal,pendidikan_id,pekerjaan_id,alamat,rt,rw,desa_id,kec_id,kab_id,prop_id,isbaru,tgl_act,user_act) values ('$noBil','$asal',$idPasien,$asalMsk,'$ket',$jnsLayanan,$tmpLayanan,'$retribusi','$tglKun','$thn','$bln','".$filterKelUmur."','$kelas','$statusPas','$hakKelas','$tglSJP','$noSJP','$noAnggota','$statusPenj','$namaPKSO','$diagAwal',$pend,$pek,'$alamat','$rt','$rw',$desa,$kec,$kab,$prop,$isBaru,'$tglact',$userId)";*/
       ...
       //echo $sqlKunj."<br>";
		...
		if (mysql_affected_rows()>0){
			$sqlGetKunj="select max(id) as id from b_kunjungan where pasien_id=$idPasien and user_act=$userId and cabang_id = '{$cabang}'";
			//echo $sqlGetKunj."<br>";
			...
			if (mysql_errno()==0){
				...
				//$tarif_akun = 0;
				if($inap == 0) {
					$sqlGetTin="select * from b_ms_tindakan_kelas where ms_tindakan_id='$retribusi' and ms_kelas_id=$kelas";
					...
					if($tarip != 0) {
						...
						$sqlRetribusi="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,jenis_kunjungan,kunjungan_id,pelayanan_id,kso_id,kso_kelas_id,tgl,biaya,biaya_pasien,biaya_kso,bayar,bayar_kso,bayar_pasien,lunas,user_id,type_dokter,tgl_act,user_act,unit_act) values('".$rwGetTin['id']."','".$tmpLayanan."','".$cJenisKunj."','".$rwGetKunj['id']."',".$rwGetPel['id'].",'".$statusPas."','".$hakKelas."','$tglKun',".$tarip.",'$beban_pasien','$beban_kso',".$bayar.",'$bayar_kso','$bayar_pasien','$lunas','".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."',NOW(),$userId,$asal)";
					...
					if ($rwCekIsInap["inap"]==1){
						$sqlKamar = "SELECT kode,nama,tarip,kelas_id FROM b_ms_kamar mk 
									INNER JOIN b_ms_kamar_tarip mkt ON mk.id=mkt.kamar_id 
									WHERE mk.id = '$kamar' AND mkt.kelas_id='$kelas'";
						//echo $sqlKamar."<br>";
						...
						if($rowCekStat['kso_id'] == 1) {
							...
							/*
							if ($statusPas == 4) {
								if($jnsLayanan != '94' && $tmpLayanan != '48' && $tmpLayanan != '112') {//Bukan IPIT & Bukan ICU IGD/ROI IGD
									$sqlHp = "SELECT * FROM b_ms_kso_paket_hp where b_ms_kso_id = 4 and b_ms_kelas_id = '$hakKelas'";
									$rsHp = mysql_query($sqlHp);
									$rowHp = mysql_fetch_array($rsHp);
									$beban_kso = $rowHp['jaminan'];
									if(($kelas != $hakKelas) && ($rowKamar['tarip']>$beban_kso)) {
										$beban_pasien=$rowKamar['tarip']-$beban_kso;
									}
								}else {
									$sqlHp = "SELECT * FROM b_ms_kso_paket_hp where b_ms_kso_id = 4 and b_ms_kelas_id = '10'";
									$rsHp = mysql_query($sqlHp);
									$rowHp = mysql_fetch_array($rsHp);
									$beban_kso = $rowHp['jaminan'];
								}
								...
								if($jnsLayanan != '94' && $tmpLayanan != '48' && $tmpLayanan != '112') {//Bukan IPIT & Bukan ICU IGD/ROI IGD
									$sqlHp = "SELECT DISTINCT tarip FROM b_ms_kamar_tarip mkt WHERE mkt.kelas_id='$hakKelas'";
									$rsHp = mysql_query($sqlHp);
									$rowHp = mysql_fetch_array($rsHp);
									$beban_kso = $rowHp['tarip'];
									if(($kelas != $hakKelas) && ($rowKamar['tarip']>$beban_kso)) {
										$beban_pasien=$rowKamar['tarip']-$beban_kso;
									}
								}
			...
			if(mysql_num_rows($rsGetPel)<=0) {
				...
				if (mysql_errno()==0){
					...
					if (mysql_errno()==0){
						...
						if($inap == '0') {
							$sqlGetRet ="select * from b_ms_tindakan_kelas where ms_tindakan_id='$retribusi' and ms_kelas_id=$kelas";
							// echo $sqlGetRet."<br>";
							...
							if($prev_inap == '0') {
								...
								if($retribusi != $prev_retribusi) {
									if($tarip == 0) {//=========Jika Tarip =0 --> data sebelumnya dihapus===========
										...
										$sqlRetribusi="update b_tindakan set jenis_kunjungan='".$cJenisKunj."',kso_id='$statusPas',kso_kelas_id='$hakKelas',ms_tindakan_kelas_id='".$rwGetRet['id']."',biaya=".$tarip.",biaya_kso='$beban_kso',biaya_pasien='$beban_pasien',
								...
								if($tarip != 0) {
									...
									$sqlRetribusi="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,jenis_kunjungan,kunjungan_id,pelayanan_id,kso_id,kso_kelas_id,tgl,biaya,biaya_pasien,biaya_kso,bayar,bayar_pasien,lunas,user_id,type_dokter,tgl_act,user_act,unit_act) values('".$rwGetRet['id']."','".$tmpLayanan."','".$cJenisKunj."','$idKunj',".$rwGetPel['id'].",'".$statusPas."','".$hakKelas."','$tglKun',".$tarip.",'$beban_pasien','$beban_kso',".$tarip.",'$bayar_pasien','$lunas','".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."',NOW(),$userId,$asal)";
									//echo $sqlRetribusi."<br>";
									mysql_query($sqlRetribusi);
									if (mysql_errno()==0){
							...
							if ($rowKamar = mysql_fetch_array($rsKamar)){
								...
								if ($inap == $prev_inap) {
									...
									if($rowCekStat['kso_id'] == 1) {
										...
										if ($statusPas=="4") {
											if($jnsLayanan=="94" || $tmpLayanan=="48" || $tmpLayanan=="112") {
												$sqlBKso = "SELECT * FROM b_ms_kso_paket_hp WHERE b_ms_kso_id=4 AND b_ms_kelas_id=10";
												$rsBKso = mysql_query($sqlBKso);
												$rowBKso = mysql_fetch_array($rsBKso);
												$beban_kso = $rowBKso['jaminan'];
											}else {
												$sqlBKso = "SELECT * FROM b_ms_kso_paket_hp WHERE b_ms_kso_id=4 AND b_ms_kelas_id='".$hakKelas."'";
												$rsBKso = mysql_query($sqlBKso);
												$rowBKso = mysql_fetch_array($rsBKso);
												$beban_kso = $rowBKso['jaminan'];
				
												if(($hakKelas != $rowKamar['kelas_id']) && ($rowKamar['tarip']>$beban_kso)) {
													$beban_pasien = $rowKamar['tarip']-$beban_kso;
												}
											}
										}else {
											if($jnsLayanan!="94" || $tmpLayanan!="48" || $tmpLayanan!="112") {
												$sqlBKso = "SELECT DISTINCT tarip FROM b_ms_kamar_tarip WHERE kelas_id='".$hakKelas."'";
												$rsBKso = mysql_query($sqlBKso);
												$rowBKso = mysql_fetch_array($rsBKso);
												$beban_kso = $rowBKso['tarip'];
...
(SELECT k.id,k.pasien_id,p.no_rm,p.no_ktp,k.no_billing,UPPER(p.nama) as nama,p.tgl_lahir,UPPER(p.alamat) as alamat,p.rt,p.rw,p.desa_id,p.kec_id,p.kab_id,p.prop_id,
...
WHERE k.loket_id = '$asal' AND pl.unit_id_asal='$asal' ".$saringan.$user.") k INNER JOIN b_ms_pasien p ON k.pasien_id=p.id INNER JOIN b_ms_unit u ON k.unit_id=u.id 
