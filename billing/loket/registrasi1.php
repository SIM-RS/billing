                ...
                <td width="9">&nbsp;</td>
					...
					//}
                     ?>
						...
						  <input type="button" value="Pasien Baru" id="BtnNoRM" onClick="getNoRM()" style="height:50px; float:left; margin-left:10px; cursor:pointer;" />					</td>
                </tr>
                <tr>
                <td width="9">&nbsp;</td>
        ...
        function listPasien(feel,how,txtId){
			...
			if(how=='show')
			{
				...
				if(feel.which==13 && keyword!=stuff)
				{
					...
					else
					{
						...
						if(txtId == 'NoRm')
						{
							...
							document.getElementById('div_pasien').style.display='none';	
							//document.getElementById('div_pasien').style.display='block';
							Request('pasien_list.php?act=cari&txt='+txtId+'&keyword='+norm+'&cabang='+cabang,'div_pasien','','GET',GetPasienByNorm);
						}
						else if(txtId == 'NoKTP')
						{
							stuff = document.getElementById('Nama').value+'<?php echo chr(3);?>'+document.getElementById('Alamat').value+'<?php echo chr(3);?>'+document.getElementById('NoKTP').value; //alert(stuff);
							document.getElementById('div_pasien').style.display='block';
							var url = 'pasien_list.php?act=cari&txt='+txtId+'&keyword='+stuff+'&cabang='+cabang; //alert(url);
							Request(url,'div_pasien','','GET');
				...
				else if ((feel.which==38 || feel.which==40) && document.getElementById('div_pasien').style.display=='block')
				{
					...
					var url = 'pasien_list.php?act=cari&txt='+txtId+'&keyword='+stuff+'&status=all'+'&cabang='+cabang; //alert(url);
					Request(url,'div_pasien','','GET');
				}
    ...
    defaultId => value yang akan diselected
        ...
        function simpan(action)
        {
			...
			if((tmpLayanan1==189 || prev_tmpLayanan1==189) && prev_tmpLayanan1 !=0){
				if(tmpLayanan1 != prev_tmpLayanan1 ){
					alert('Untuk Merubah Data Pasien MCU Silahkan Menghapus Registrasi Yang Salah dan Lakukan Registrasi Ulang Menggunakan No RM / Nama Pasien dari Registrasi Yang Telah Terhapus !!');
					return false;
				}
			
			}
			
			var Ret= document.getElementById("Retribusi").value;
		    var prev_Ret = document.getElementById("prev_retribusi").value;
			
			
			if((Ret==1316 || prev_Ret==1315 || Ret==1315 || prev_Ret==1316 ) && prev_Ret !=0){
				if(Ret != prev_Ret ){
					alert('Untuk Merubah Data Pasien SKD Silahkan Menghapus Registrasi Yang Salah dan Lakukan Registrasi Ulang Menggunakan No RM / Nama Pasien dari Registrasi Yang Telah Terhapus !!');
					return false;
				}
        ...
        function ambilData(multi)
        {
            ...
            var p="txtIdPasien*-*"+sisipan[1]+"*|*txtIdKunj*-*"+sisipan[0]+"*|*NoRm*-*"+sisipan[2]+"*|*NoBiling*-*"+sisipan[3]
                ...
                +"*|*prev_inap*-*"+sisipan[34]+"*|*StatusPenj*-**|*"+defSim+"*|*btnHapus*-*"+disableHapus+"*|*HakKelas*-*"+sisipan[33]+"*|*NoKTP*-*"+sisipan[37]+"*|*prop*-*"+sisipan[44]+"*|*darah*-*"+sisipan[46]+"*|*noReg1*-*"+sisipan[45]+"*|*kabx*-*"+sisipan[43]+"*|*kecx*-*"+sisipan[42]+"*|*desx*-*"+sisipan[41]+"*|*cmbKw*-*"+sisipan[49];
		...
		function batal(exc)
        {
		  ...
		  p += "*|*skpJamkesda*-*true*|*skpJamkesdaKmr*-*true*|*UpdStatusPx*-*true*|*prop*-**|*kabx*-**|*kecx*-**|*desx*-**|*prev_TmpLayanan*-**|*prev_retribusi*-*";
            fSetValue(window,p);
            if(exc == '1'){
