...
if($in == "cDokterG")
{
	...
	$dataQD = mysql_fetch_array($execQD);
	
	if(($dataQD['spesialisasi_id'] == 35 || $dataQD['spesialisasi_id'] == 36 || $dataQD['spesialisasi_id'] == 37) && ($idUnit1 == 156))
	{
		?>
        <script>
			jQuery("#Retribusi").val(2);
        </script>
        <?
	}elseif(($dataQD['spesialisasi_id'] != 35) && ($idUnit1 == 156)){
		?>
        <script>
			jQuery("#Retribusi").val(3);
        </script>
        <?
	}
