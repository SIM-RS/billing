...
if($_REQUEST['kamar'] == 'true'){
    $qPeserta="SELECT p.no_rm,p.nama,k.alamat,k.rt,k.rw,p.tgl_lahir,if(pl.jenis_layanan=94,'ICU','RAWAT INAP') AS jenis
    ...
    $qPeserta="SELECT p.no_rm,p.nama,k.alamat,k.rt,k.rw,p.tgl_lahir,u.nama AS jenis,IF(p.sex='L','Laki-laki','Perempuan') AS sex,no_sjp,
...
$jdl="SURAT KEABSAHAN PESERTA (SKP) JAMKESDA<br/>".$namaRS."<br/>
            NON DATABASE (BLOCK GRAND)<br/>
            Berlaku : 1 (satu) X Kunjungan";
if ($_REQUEST['jampersal'] == 'true'){
	$jdl="SURAT JAMINAN PERSALINAN (JAMPERSAL)<br/>".$namaRS."<br/>
            Berlaku : 1 (satu) X Kunjungan";
}
if($rwPeserta['id_kso']==38){
	$jud = "JAMKESMAS";
}else if($rwPeserta['id_kso']==53){
	$jud = "JAMPERSAL";
}else if($rwPeserta['id_kso']==46){
	$jud = "JAMKESDA";
}else if($rwPeserta['id_kso']==39){
	$jud = "SKTM";
}
?>
<table width="1250" border="0" cellpadding="0" cellspacing="0" align="left">
    <tr>
        <td colspan="6" align="center" style="padding-bottom:50px; font-weight:bold; font-size:18px;">
			SURAT KEABSAHAN PESERTA (SKP) <?php echo $jud?><br />
			<?=$namaRS?><br />
			Berlaku : 1 (satu) X Kunjungan
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td align="center">&nbsp;<?=$kotaRS?>, <?php echo $rwPeserta['tglkunj'];?> <br/> <?php if ($rwPeserta['id_kso'] == 53) echo "Mengetahui"; else echo "Petugas PPATRS";?></td>
    </tr>
    <tr>
