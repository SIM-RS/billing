       ...
       //echo $sqlKunj."<br>";
		...
		if (mysql_affected_rows()>0){
			...
			if (mysql_errno()==0){
				...
				if($mcu_id==189){
					$sqlPelayananKonsul="insert into b_pelayanan (no_antrian,jenis_kunjungan,pasien_id,unit_id_asal,kunjungan_id,jenis_layanan,unit_id,kso_id,kelas_id,tgl,dilayani,tgl_act,user_act,dokter_tujuan_id,type_dokter_tujuan) values 
					((fGetMaxNo_Antrian(2,'$tglKun')),'".$cJenisKunj."',$idPasien,'$mcu_id','".$idKunj."',$jnsLayanan,'2',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
					((fGetMaxNo_Antrian(156,'$tglKun')),'".$cJenisKunj."',$idPasien,'$mcu_id','".$idKunj."',$jnsLayanan,'156',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
					((fGetMaxNo_Antrian(17,'$tglKun')),'".$cJenisKunj."',$idPasien,'$mcu_id','".$idKunj."',$jnsLayanan,'17',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
					((fGetMaxNo_Antrian(61,'$tglKun')),'".$cJenisKunj."',$idPasien,'$mcu_id','".$idKunj."',$jnsLayanan,'61',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
					((fGetMaxNo_Antrian(58,'$tglKun')),'".$cJenisKunj."',$idPasien,'$mcu_id','".$idKunj."',$jnsLayanan,'58',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."')";
					mysql_query($sqlPelayananKonsul);
					//echo $sqlPelayananKonsul;
				}
				...
				if($inap == 0) {
					...
					if($tarip != 0) {
						...
						$sqlRetribusi="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,ak_ms_unit_id,jenis_kunjungan,kunjungan_id,pelayanan_id,kso_id,kso_kelas_id,kelas_id,tgl,biaya,biaya_pasien,biaya_kso,bayar,bayar_kso,bayar_pasien,lunas,user_id,type_dokter,tgl_act,user_act,unit_act) values('".$idTindAdm."','".$tmpLayanan."','".$ak_ms_unit_id."','".$cJenisKunj."','".$idKunj."',".$idPel.",'".$statusPas."','".$hakKelas."','$kelas','$tglKun',".$tarip.",'$beban_pasien','$beban_kso',".$bayar.",'$bayar_kso','$bayar_pasien','$lunas','".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."',NOW(),$userId,$asal)";
						//echo $sqlRetribusi."<br>";
						mysql_query($sqlRetribusi);
						if (mysql_errno()==0){
							//insert ke b_tindakan_komponen
							$idTindRet=mysql_insert_id();
							/*$getIdTind="select max(id) as id from b_tindakan where pelayanan_id='".$idPel."' and kunjungan_id='".$idKunj."' and user_act=$userId";
							//echo $getIdTind."<br/>";
					...
					if($taripKonsul != 0 && $retribusi != '1315' && $retribusi != '1316' ){
						if($statusPas==1){
						...
						$sqlKOnsultasi="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,ak_ms_unit_id,jenis_kunjungan,kunjungan_id,pelayanan_id,kso_id,kso_kelas_id,kelas_id,tgl,biaya,biaya_pasien,biaya_kso,bayar,bayar_kso,bayar_pasien,lunas,user_id,type_dokter,tgl_act,user_act,unit_act) values('".$KlsIdKonsul."','".$tmpLayanan."','".$sqlKonsul2['ak_ms_unit_id']."','".$cJenisKunj."','".$idKunj."',".$idPel.",'".$statusPas."','".$hakKelas."','$kelas','$tglKun',".$taripKonsul.",'$BiayaKonsulP','$BiayaKonsulK','0','0','0','0','".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."',NOW(),$userId,$asal)";
					//	echo $sqlKOnsultasi."<br>";
						mysql_query($sqlKOnsultasi);
						if (mysql_errno()==0){
							$idTindKonsul=mysql_insert_id();
							/*$getIdTind1="select max(id) as id from b_tindakan where pelayanan_id='".$idPel."' and kunjungan_id='".$idKunj."' and user_act=$userId";
							//echo $getIdTind."<br/>";
							$rsIdTind1=mysql_query($getIdTind1);
							$rwIdTind1=mysql_fetch_array($rsIdTind1);*/
			
							$sqlTambah="insert into b_tindakan_komponen (tindakan_id,ms_komponen_id,nama,tarip,tarip_prosen) SELECT '".$idTindKonsul."',k.id,k.nama ,$taripKonsul,b.tarip_prosen FROM b_ms_tindakan_komponen b 
							inner join b_ms_komponen k on b.ms_komponen_id=k.id
					 ...
					 if($retribusi=="1315" || $retribusi=="1316"){
						...
						/*$sqlR = "SELECT b.`tarip` tarip_ret ,b.id as ms_tindakan_kelas_id, a.* FROM b_ms_tindakan a
							...
							WHERE c.`ms_unit_id`='$tmpLayanan' AND a.`kel_tindakan_id`=66";
						//echo $sqlR;*/
						$sqlGetTinSKD="SELECT a.*,b.`ak_ms_unit_id` 
									FROM b_ms_tindakan_kelas a
										INNER JOIN b_ms_tindakan b ON a.`ms_tindakan_id`=b.`id`		
									WHERE a.ms_tindakan_id='$retribusi' and a.ms_kelas_id=$kelas";
						...
						$sqlRet="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,ak_ms_unit_id,jenis_kunjungan,kunjungan_id,pelayanan_id,kso_id,kso_kelas_id,kelas_id,tgl,biaya,biaya_pasien,biaya_kso,bayar,bayar_kso,bayar_pasien,lunas,user_id,type_dokter,tgl_act,user_act,unit_act) values('".$KlsIdR."','".$tmpLayanan."','".$ak_ms_unit_id."','".$cJenisKunj."','".$idKunj."',".$idPel.",'".$statusPas."','".$hakKelas."','$kelas','$tglKun',".$taripR.",'$BiayaRP','$BiayaRK','0','0','0','0','".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."',NOW(),$userId,$asal)";
						//echo $sqlRetribusi."<br>";
						mysql_query($sqlRet);
					...
					if ($rwCekIsInap["inap"]==1){
						...
						$sqlKamar = "insert into b_tindakan_kamar (pelayanan_id, unit_id_asal, kso_id, tgl_in, kamar_id, kode, nama, tarip, beban_pasien, beban_kso, kelas_id,aktif)
								values ('".$idPel."','".$asal."','".$statusPas."', CONCAT('$tglKun',' ',CURRENT_TIME()), '$kamar', '".$rowKamar['kode']."', '".$rowKamar['nama']."', '".$rowKamar['tarip']."', '$beban_pasien', '$beban_kso', '".$rowKamar['kelas_id']."','0')";
						//echo $sqlKamar."<br>";
						$rsKamar = mysql_query($sqlKamar);
						if (mysql_errno()>0){
							$statusProses='Error';
    ...
    case 'simpan':
		...
		if (mysql_errno()==0){
			...
			if(mysql_num_rows($rsGetPel)<=0) {
				...
				$sqlSimpan="update b_kunjungan set pasien_id=$idPasien,asal_kunjungan=$asalMsk,ket='$ket',jenis_layanan=$jnsLayanan,unit_id=$tmpLayanan,retribusi='$retribusi',tgl='$tglKun',umur_thn='$thn',umur_bln='$bln',umur_hr='$hari',kelas_id='$kelas',kso_id=$statusPas,kso_kelas_id='$hakKelas',tgl_sjp='$tglSJP',no_sjp=$strNoSJP,no_anggota='$noAnggota',status_penj='$statusPenj',nama_peserta='$namaPKSO',pendidikan_id=$pend,pekerjaan_id=$pek,alamat='$alamat',rt='$rt',rw='$rw',desa_id='$desa',kec_id='$kec',kab_id='$kab',prop_id='$prop',kel_umur='$filterKelUmur',no_reg='$noreg1' where id=$idKunj";
				// echo $sqlSimpan."<br>";
				...
				if (mysql_errno()==0){
					...
					if ($rwGetPel['unit_id']!=$tmpLayanan){
						...
						$sqlPel = "update b_pelayanan set no_antrian=(fGetMaxNo_Antrian($tmpLayanan,'$tglKun')),jenis_kunjungan='".$cJenisKunj."',jenis_layanan = $jnsLayanan, unit_id = $tmpLayanan,kso_id='$statusPas',kelas_id = $kelas, tgl = '$tglKun', dokter_tujuan_id = '".$_REQUEST['dokter_id']."', type_dokter_tujuan = '".$_REQUEST['dokterPengganti']."' where id = '".$idPel."'";
					}else{
						$sqlPel = "update b_pelayanan set kso_id='$statusPas',kelas_id = $kelas, tgl = '$tglKun', dokter_tujuan_id = '".$_REQUEST['dokter_id']."', type_dokter_tujuan = '".$_REQUEST['dokterPengganti']."' where id = '".$idPel."'";
					}
					//echo $sqlPel."<br>";
					mysql_query($sqlPel);
					if (mysql_errno()==0){
						...
						//============ Jika MCU, konsul ke 5 unit ===============//
						if($prev_tmpLayanan!=$tmpLayanan && $tmpLayanan==189){
							$sqlPelayananKonsul="insert into b_pelayanan (no_antrian,jenis_kunjungan,pasien_id,unit_id_asal,kunjungan_id,pelayanan_id_asal,jenis_layanan,unit_id,kso_id,kelas_id,tgl,dilayani,tgl_act,user_act,dokter_tujuan_id,type_dokter_tujuan) values 
							((fGetMaxNo_Antrian(2,'$tglKun')),'".$cJenisKunj."',$idPasien,'$tmpLayanan','".$idKunj."','$idPel',$jnsLayanan,'2',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
							((fGetMaxNo_Antrian(156,'$tglKun')),'".$cJenisKunj."',$idPasien,'$tmpLayanan','".$idKunj."','$idPel',$jnsLayanan,'156',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
							((fGetMaxNo_Antrian(17,'$tglKun')),'".$cJenisKunj."',$idPasien,'$tmpLayanan','".$idKunj."','$idPel',$jnsLayanan,'17',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
							((fGetMaxNo_Antrian(61,'$tglKun')),'".$cJenisKunj."',$idPasien,'$tmpLayanan','".$idKunj."','$idPel',$jnsLayanan,'61',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."'),
							((fGetMaxNo_Antrian(58,'$tglKun')),'".$cJenisKunj."',$idPasien,'$tmpLayanan','".$idKunj."','$idPel',$jnsLayanan,'58',$statusPas,$kelas,'$tglKun',0,NOW(),$userId,'".$_REQUEST['dokter_id']."','".$_REQUEST['dokterPengganti']."')";
							mysql_query($sqlPelayananKonsul);
							//echo $sqlPelayananKonsul;
						}
...
INNER JOIN b_ms_tindakan_kelas tk ON k.retribusi = tk.`ms_tindakan_id`) AS t1 
INNER JOIN b_ms_unit n ON t1.id_jenis_layanan=n.id) AS gab ".$filter." ORDER BY ".$sorting;
    ...
    if($grd == "true") {
		...
		$sqlAR = "select * from b_asal_masuk_kunjungan where idKunj = ".$rows['id'];
			...
			if($rows['dilayani']==1)
			{
				$dt.=$sisipan.chr(3)."<span style='color:#F00'>".number_format($i,0,",","")."</span>".chr(3)."<span style='color:#F00'>".$rows['no_rm']."</span>".chr(3)."<span style='color:#F00'>".$rows["nama"]."</span>".chr(3)."<span style='color:#F00'>".$rows["nama_kso"]."</span>".chr(3)."<span style='color:#F00'>".$rows["tempat_layanan"]."</span>".chr(3)."<span style='color:#F00'>".number_format($rows["tarip_r"],0,",",".")."</span>".chr(3)."<span style='color:#F00'>".$Dokter["dokter"]."</span>".chr(3)."<span style='color:#F00'>".$rows["kelas"]."</span>".chr(3)."<span style='color:#F00'>".$rows["alamat"]." RT. ".$rows["rt"]." RW. ".$rows['rw']."</span>".chr(6);
			}else{
				$dt.=$sisipan.chr(3).$i.chr(3).$rows['no_rm'].chr(3).$rows["nama"].chr(3).$rows["nama_kso"].chr(3).$rows["tempat_layanan"].chr(3).number_format($rows["tarip_r"],0,",",".").chr(3).$Dokter["dokter"].chr(3).$rows["kelas"].chr(3).$rows["alamat"]." RT. ".$rows["rt"]." RW. ".$rows['rw'].chr(6);
			}
            //$rows["id"]
        }
    }
