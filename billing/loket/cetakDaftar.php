...
<table border="0" cellpadding="2" cellspacing="2" align="left" class="kwi">
    ...
    <?php
    $sql1 = "SELECT p.no_rm, k.no_billing, p.nama, p.alamat, k.unit_id, u.nama AS namaunit, date_format(k.tgl,'%d-%m-%Y') as tgl, k.kso_id, kso.nama AS status, k.id, p.rt, p.rw,  bp.no_antrian, k.tgl, DATE_FORMAT(k.tgl_act, '%d-%m-%Y') AS tgl_new, DATE_FORMAT(k.tgl_act, '%k:%i:%s') AS jam_new, bmp.nama AS nm_input
                FROM b_ms_pasien p
