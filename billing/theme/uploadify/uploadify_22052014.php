...
if (!empty($_FILES)) {
	...
	if (in_array($fileParts['extension'], $fileTypes)) {
		...
		if($_POST['action']=='Tambah'){
			$query="insert into b_hasil_rad (tgl,pelayanan_id,nama_file,fcontent,hasil,user_id,tgl_act,user_act) values (now(),'$pelayanan_id','$nama_file','$fcontent','$hasil','$dokter',now(),'$user_id')";
			$result = mysql_query ($query);
			mysql_free_result($result);
		}
		else if($_POST['action']=='Simpan'){
			$queri="update b_hasil_rad set hasil='$hasil',user_id='$dokter',nama_file='$nama_file',fcontent='$fcontent',tgl_act=now(),user_act='$user_id' where id='$id'";
