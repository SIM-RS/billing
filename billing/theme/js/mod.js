	...
	for (var i=0;i<arpar.length;i++){
		if (document.getElementById(arpar[i]).value==""){
			if (lng=="eng"){
				...
				alert("Pengisian Form Belum Lengkap, "+arpar[i]+" belum diisi");
			}
...
function fSetPosisi_apotek(obj,pc){
  var p=fGetXYObj(pc);
  with (obj.style) {
  	left=p[0]-1;
	top=p[1]+pc.offsetHeight+1;
...
function fSetPosisi_darah(obj,pc){
  var p=fGetXYObj(pc);
  //obj.style.left=(parseInt(p[0])-1)+'px';
  obj.style.top=(parseInt(p[1])+parseInt(pc.offsetHeight))+'px';
}

function fSetPosisi_asterix(obj,pc){
  var p=fGetXYObj(pc);
  //alert(obj.id);
  //obj.style.left=(parseInt(p[0])-1)+'px';
  //alert((parseInt(p[1])+" | "+parseInt(pc.offsetHeight)));
  obj.style.top=(parseInt(p[1])-parseInt(500)+parseInt(pc.offsetHeight))+'px';
  //alert(obj.style.top);
}
