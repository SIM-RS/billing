...
Request = function( vUrl , vTarget, vForm, vMethod,evl,noload) {
  ...
  // alert(statusRequest);
  var arrID = vTarget.split(',');
  console.log(arrID);
  var arrLong = arrID.length;
  if(noload==undefined){ 
	if(arrLong > 1)
		for(var nr=0; nr < arrLong; nr++ )
			GetId(arrID[nr]).innerHTML = statusRequest;
	else
		  ...
		  if (typeof(req[pos]) != 'undefined' && 
			...
			req[pos].xmlhttp.readyState == 4) {
			  if (req[pos].xmlhttp.status == 200 || req[pos].xmlhttp.status == 304) {
					if(arrLong > 1)
						for(var nr=0; nr < arrLong; nr++ )
							GetId(arrID[nr]).innerHTML = req[pos].xmlhttp.responseText;
					else
