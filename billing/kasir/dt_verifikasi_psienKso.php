...
$sql="SELECT id FROM b_ms_group_petugas mgp WHERE mgp.ms_pegawai_id='".$_SESSION['userId']."' AND ms_group_id='78'";
$rsGrp=mysql_query($sql);
$isVerifKasir=0;
$attReadOnly=" readonly";
if (mysql_num_rows($rsGrp)>0 || $_SESSION['userId']=='732'){
	$isVerifKasir=1;
	$attReadOnly="";
}
?>
        ...
        <tr>
            <td>
                <table width="100%">
                    ...
                    <img alt="tglAkhir" src="http://<?php echo $_SERVER['HTTP_HOST'];?>/simrs-pelindo/billing/icon/archive1.gif" onClick="gfPop.fPopCalendar(document.getElementById('txtTglAkhir'),depRange,getChange);" style="cursor: pointer"/>                							</td>
                                            <td width="100">&nbsp;</td>
                                            <td>Tgl Setor</td>
                    ...
                    <img alt="tglSetor" src="http://<?php echo $_SERVER['HTTP_HOST'];?>/simrs-pelindo/billing/icon/archive1.gif" onClick="gfPop.fPopCalendar(document.getElementById('txtTglSetor'),depRange,getChange);" style="cursor: pointer"/></td>
                						...
                						<tr>
                						  <td>Nama Kasir</td>
                						  <td>:
                						    <input type="hidden" value="<?=$_SESSION['userId'];?>" id="id_penyetor" name="id_penyetor" />
                                              <input type="text" id="nm_penyetor" <?php echo $attReadOnly; ?> name="nm_penyetor" value="<?=$nama_user;?>" size="35" onKeyUp="listKasir(event,'show',this.value)" />
                                              <div id="div_kasir" align="center" class="div_pasien"></div></td>
                                        	<td width="100">&nbsp;</td>
											<td>No. Slip Setor</td>
										...
										<tr>
										  ...
										  <td>&nbsp;</td>
											...
											<button type="submit" id="btnCetak" name="btnCetak" style="cursor:pointer" onClick="cetak()">Cetak</button>
												<button type="submit" id="btnCetak" name="btnCetak" style="cursor:pointer" onClick="kwitansi()">Cetak Kwitansi</button>											</td>
										</tr>
                					</table>
                					</fieldset>
                          		</td>
...
function cetak(){
	window.open('cetak_verifikasi_psienKSO.php?tgl='+document.getElementById('txtTglSetor').value+'&kasir=<?=$_SESSION['userId'];?>'+"&filter="+b_kanan.getFilter());	
}
function kwitansi(){
	window.open('kwitansi_verifikasi_psienKSO.php?tgl='+document.getElementById('txtTglSetor').value+'&kasir=<?=$_SESSION['userId'];?>'+"&filter="+b_kanan.getFilter());	
}
function getChange(){
	...
	//alert(utils);
	b_kiri.loadURL("verifikasi_psienKso_util.php?grd=kiri"+utils,"","GET");
	b_kanan.loadURL("verifikasi_psienKso_util.php?grd=kanan"+utils,"","GET");
}
...
function firstData(){
	...
	b_kiri.setColHeader("No,<input type='checkbox' id='check_kiri' name='check_kanan' onclick='selectallb_kiri(this.checked)' />,Tgl Pulang,No Kwi,No RM,Pasien,Nilai");
	...
	b_kiri.baseURL("verifikasi_psienKso_util.php?grd=kiri"+utils);
	b_kiri.Init();
	
	b_kanan = new DSGridObject("gridkanan");
	b_kanan.setHeader(" ",false);
	b_kanan.setColHeader("No,<input type='checkbox' id='check_kanan' name='check_kanan' onclick='selectallb_kirids(this.checked)' />,Tgl Pulang,No Kwi,Tgl Setor,No RM,Pasien,Slip ke,Nilai,Disetor Oleh");
	...
	b_kanan.baseURL("verifikasi_psienKso_util.php?grd=kanan"+utils);
	b_kanan.Init();	
}		
...
function goFilterAndSort(abc){
	...
	if (abc=="gridkiri"){
		b_kiri.loadURL("verifikasi_psienKso_util.php?grd=kiri"+utils+"&filter="+b_kiri.getFilter()+"&sorting="+b_kiri.getSorting()+"&page="+b_kiri.getPage(),"","GET");
	}
	else if (abc=="gridkanan"){
		b_kanan.loadURL("verifikasi_psienKso_util.php?grd=kanan"+utils+"&filter="+b_kanan.getFilter()+"&sorting="+b_kanan.getSorting()+"&page="+b_kanan.getPage(),"","GET");
	}
} 
...
function getKirib_kiri(){
	...
	//alert(nss);
	Request("verifikasi_psienKso_util.php?grd=kiri"+utils+"&act=kiri&data="+data+"&nss="+nss,"ajax","","GET",function(){
...
function getKananb_kiri(){
	...
	//alert(nss);
	//alert("verifikasi_psienKso_util.php?grd=kiri"+utils+"&act=kanan&data="+data);
	Request("verifikasi_psienKso_util.php?grd=kiri"+utils+"&act=kanan&data="+data+"&nss="+nss,"ajax","","GET",function(){
		document.getElementById('check_kiri').checked=false;
		document.getElementById('check_kanan').checked=false;
		b_kiri.loadURL("verifikasi_psienKso_util.php?grd=kiri"+utils,"","GET");
		b_kanan.loadURL("verifikasi_psienKso_util.php?grd=kanan"+utils,"","GET");
