...
// echo $p."<br>";
$PATH_INFO = 'http://'.$_SERVER['HTTP_HOST'].$base_addr.'/billing/master/kelLab_tree_ajax.php';
$target='tree';
$PATH_INFO .= "?&type=".$type;
$susun = $PATH_INFO."?*-*".$target."*-*&p=".$p."*-*".$_REQUEST['cnt']."#".$_REQUEST['cnt'];
// $PATH_INFO = $susun."&type=".$type;
?>
<html>
                    ...
                    <td nowrap>
                        ...
                        while ($rows=mysql_fetch_array($rs)) {
							...
							if($type != 2)
								...
								$str = "select kode from b_ms_kelompok_tindakan_pemeriksaan_lab where id=$rows[parent_id]";
							...
							$arfvalue="idKel*-*".$rows['id']."*|*kode*-*".$kode."*|*kode_kel*-*".$rows['kode']."*|*kel*-*".$rows['nama_kelompok']."*|*level*-*".$rows['level']."*|*parent*-*".$rows['parent_id']."*|*btnHapusKel*-*false";
                            if ($c_level>0)
