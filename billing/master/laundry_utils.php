 ...
 switch(strtolower($_REQUEST['act'])){
	case 'kanan':
		...
		for($i=0;$i<count($dt); $i++){
			if($dt[$i]!=''){
				$sql1="INSERT INTO $dbcssd.cssd_ms_alat (idbarang,tipe) VALUES ('".$dt[$i]."','1') ";
				mysql_query($sql1);
			}
		}
		break;
...
FROM dbaset.as_ms_barang mb";*/
	$sql="SELECT * FROM $dbaset.as_ms_barang mb WHERE mb.level=2 AND tipebarang='HP' AND (mb.kodebarang LIKE '04%' OR mb.kodebarang LIKE '06%') AND mb.idbarang NOT IN (SELECT idbarang FROM $dbcssd.cssd_ms_alat) $filter ORDER BY $sorting";
	break;
	case "2":
...
LEFT JOIN $dbaset.as_ms_barang mb
ON mb.idbarang = ma.idbarang where ma.tipe=1 $filter ORDER BY $sorting";
	break;
}
//echo $sql."<br>";
//$perpage=100;
