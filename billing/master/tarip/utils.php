    ...
    foreach($ret[$i]['tarif_kelas'] as $val){
        ...
        if(mysql_num_rows($query) > 0){
            ...
            if ($fetch['tarip'] != $val['tarip']) {
                ...
                $queryCekAdaAtauTidak = mysql_query("SELECT * FROM b_ms_tindakan_unit WHERE ms_tindakan_kelas_id = {$fetch['id']} AND ms_unit_id = {$ret[$i]['id_tmpt_layanan']} AND flag = 1");
                if (mysql_num_rows($queryCekAdaAtauTidak) == 0) {
                    mysql_query("INSERT INTO b_ms_tindakan_unit(ms_tindakan_kelas_id,ms_unit_id,kode_unit,flag)VALUES({$fetch['id']},{$ret[$i]['id_tmpt_layanan']},'{$getUnit['kode']}',1)");
                }
                continue;
            }
        }
        if(mysql_query("INSERT INTO b_ms_tindakan_kelas(ms_tindakan_id,ms_kelas_id,tarip,kso_id,user_act)VALUES({$ret[$i]['id_tindakan']},{$val['id_kelas']},{$val['tarip']},{$ret[$i]['id_kso']},{$user})")){
            ...
            mysql_query("INSERT INTO b_ms_tindakan_unit(ms_tindakan_kelas_id,ms_unit_id,kode_unit,flag)VALUES({$idTindakanKelas},{$ret[$i]['id_tmpt_layanan']},'{$getUnit['kode']}',1)");
        }
    }
}
