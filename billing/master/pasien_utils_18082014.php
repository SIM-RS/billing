...
switch(strtolower($_REQUEST['act'])) {
    ...
    case 'tambah':
            ...
            if($cek1==0){
			$sqlTambah="insert into $dbbilling.b_ms_pasien (no_rm,nama,sex,agama,tgl_lahir,alamat,rt,rw,desa_id,kec_id,kab_id,prop_id,telp,pendidikan_id,pekerjaan_id,nama_ortu,nama_suami_istri,user_act,tgl_act,no_ktp,gol_darah) values('$noRm','$nama','$gender','$agama','$tglLhr','$alamat','$rt','$rw','$desa','$kec','$kab','$prop','$telp','$pend','$pek','$namaOrtu','$namaSuTri','$userId','$tglact','$ktp','$darah')";
            $rs=mysql_query($sqlTambah);
           	if (mysql_errno()<=0){
				...
				$sqlTambah="insert into $dbbilling.b_ms_kso_pasien (kso_id,pasien_id,kelas_id,no_anggota,aktif,st_anggota,nama_peserta,tgl_act)
					values('$statusPas','$idpas','$hakKelas','$noAnggota','1','$statusPenj','$namaPeserta','$tglact')";
				$rs=mysql_query($sqlTambah);
           		if (mysql_errno()>0){
					 $statusProses="Error";
    ...
    case 'simpan':
	    ...
	    kelas_id='$hakKelas',no_anggota='$noAnggota',aktif='1',st_anggota='$statusPenj',nama_peserta='$namaPeserta',tgl_act='$tglact' where id='$idKsoPasien'";
...
".$saringan.") as gab ".$filter." order by ".$sorting;
    }
