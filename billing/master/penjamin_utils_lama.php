...
switch(strtolower($grd)){
	case '1':
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':		
				$sqlTambah="insert into b_ms_kso (kode,nama,kepemilikan_id,alamat,telp,fax,kontak,aktif)
					values('".$_REQUEST['kode']."','".$_REQUEST['nama']."','".$_REQUEST['kpid']."','".$_REQUEST['alamat']."','".$_REQUEST['telp']."','".$_REQUEST['fax']."','".$_REQUEST['kontak']."','".$_REQUEST['caktif']."')";
			...
			case 'hapus':
				...
				break;
			case 'simpan':
				$sqlSimpan = "UPDATE b_ms_kso SET  kode='".$_REQUEST['kode']."',nama='".$_REQUEST['nama']."',kepemilikan_id='".$_REQUEST['kpid']."',alamat='".$_REQUEST['alamat']."',telp='".$_REQUEST['telp']."',fax='".$_REQUEST['fax']."',kontak='".$_REQUEST['kontak']."',aktif='".$_REQUEST['caktif']."' WHERE id='".$_REQUEST['id']."'";	
	...
	case '12':
		...
		switch(strtolower($_REQUEST['act'])){
			case 'update':
				$sqlSimpan = "UPDATE b_ms_kso_pasien SET instansi_id='".$_REQUEST['instansi_id']."',no_anggota='".$_REQUEST['no_anggota']."',st_anggota='".$_REQUEST['st_anggota']."',nama_peserta='".$_REQUEST['nama_peserta']."',tgl_act=now() WHERE id='".$_REQUEST['id']."'";	
				$rs=mysql_query($sqlSimpan);
				break;
		}
		break;
...
switch($grd){
	case "1":
		$sql="SELECT * FROM (SELECT kso.id,kso.kepemilikan_id,k.NAMA kepemilikan,kso.kode,kso.nama,kso.alamat,kso.telp,kso.fax,kso.kontak,kso.aktif caktif,IF(kso.aktif=1,'Ya','Tidak') aktif 
FROM b_ms_kso kso LEFT JOIN $dbapotek.a_kepemilikan k ON kso.kepemilikan_id=k.ID) gab ".$filter." order by ".$sorting;
		break;
	case "2":
...
switch($grd){
	...
	{
		$i++;
		$dt.=$rows["id"]."|".$rows["caktif"]."|".$rows["kepemilikan_id"].chr(3).$rows["kode"].chr(3).$rows["nama"].chr(3).$rows["kepemilikan"].chr(3).$rows["alamat"].chr(3).$rows["telp"].chr(3).$rows["fax"].chr(3).$rows["kontak"].chr(3).$rows["aktif"].chr(6);
	}
	break;
