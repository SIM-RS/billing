...
switch(strtolower($_REQUEST['act'])){
	...
	case 'simpan':
		if($id == ''){
			$sql = "insert into b_ms_jadwal_dokter (unit_id, dokter_id, hari, mulai, selesai,flag)
				values({$unit_id}, {$dokter_id}, {$hari}, '{$mulai}', '{$selesai}', '{$flag}')";	
			mysql_query($sql);					
		}else{
			$sql = "update b_ms_jadwal_dokter set dokter_id = {$dokter_id}, hari = {$hari}, mulai = '{$mulai}', selesai = '{$selesai}', flag = '{$flag}'
				where id = {$id}";
			mysql_query($sql);					
		}
		break;
	case 'hapus':
		$sql = "delete from b_ms_jadwal_dokter where id = {$id}";
		mysql_query($sql);
		break;
}
//=======================================
...
if ($sorting=="")
	$sorting="p.nama, jd.hari, jd.mulai";
...
while ($rows=mysql_fetch_array($rs)){
	$i++;
	$dt.=($rows["id"].'||'.$rows['dokter_id'].'||'.$rows['hari'].'||'.$rows['mulai'].'||'.$rows['selesai']).chr(3).$i.chr(3).$rows["nama"].chr(3).$rows["nip"].chr(3).$arr_hari[$rows["hari"]].chr(3).$rows["mulai"].' - '.$rows['selesai'].chr(6);
}
