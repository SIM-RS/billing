...
if($_REQUEST['par']!=''){
	$cek="SELECT
		...
		FROM
		  icd.mrconso m 
		  INNER JOIN icd.mrhier mh 
			ON m.AUI = mh.AUI 
		WHERE m.SAB LIKE 'ICD9CM%'
		  AND m.CODE = '".$_REQUEST['par']."'";
	$qCek=mysql_query($cek,$koneksi_inacbg);
	...
	$cek2="SELECT
		...
		WHERE m.SAB LIKE 'ICD9CM%'
		  AND mh.AUI = '".$rw['PAUI']."'";
	$qCek2=mysql_query($cek2,$koneksi_inacbg);
...
<tr bgcolor="whitesmoke" style="background-color:#663333; color:#FDFFFA; font-weight:bold; font-size:14px">
    <td align="left" colspan="2">&nbsp;<img onClick="<?php echo "window.location='inacbg_icd9cm.php'" ?>" style="cursor:pointer; width:25" src="../icon/folder_up.png">&nbsp;<img onClick="<?php echo "window.location='inacbg_icd9cm.php?PAUI=".$fb[0]."&b=".$fb[1]."'" ?>" style="cursor:pointer; width:25" src="../icon/folder_root.png"></td>
</tr>
<?php
...
while($rows=mysql_fetch_array($kueri)){
	...
	if($rows['TTY']=='HT'){
		$islast=0;
		$onKlik="window.location='inacbg_icd9cm.php?PAUI=".$rows['AUI']."&b=".$rows['PTR']."'";
	}
	else{
		$islast=1;
		$onKlik="window.opener.document.getElementById('txtICD9cm').value='".$rows['CODE']."';window.close()";
	}
