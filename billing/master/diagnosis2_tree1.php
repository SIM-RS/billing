...
//	echo "<script>alert(zxczxczczxczxczxczx);
	...
	//	$a_levelx = $_REQUEST['level'];
		...
		$sqlTambahx="insert into b_ms_diagnosa (kode,nama,level,parent_id,parent_kode,surveilance,aktif,dg_kode)
				values('$a_kode','$a_nama','$a_level','$a_parentId','$a_parentKode','$a_sur','$a_aktif','$a_dg_kode')";
	...
	$a_kode = $_REQUEST['kode'];
		...
		$sql="select * from b_ms_diagnosa where kode='$kode' and nama='".$_REQUEST['nama']."' and level='$a_level' and parent_id='$a_parentId' and parent_kode='$a_parentKode' and surveilance='$a_sur' and dg_kode='$a_dg_kode' and aktif='".$a_aktif."'";
		//echo $sql."<br>";
		$rs1=mysql_query($sql);
		if (mysql_num_rows($rs1)>0){
			...
			if ($cparent!=$parentId){
				...
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				$sql="update b_ms_diagnosa set kode='$a_kode',nama='$a_nama',level='$a_level',parent_id='$a_parentId',parent_kode='$a_parentKode',surveilance='".$a_sur."',dg_kode='".$a_dg_kode."',aktif='".$a_aktif."' where id='$a_id'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				if ($cislast==0){
...
//alert(document.getElementById('act1').value);
	...
	//alert (sbx);
		...
		//		alert(parentId);
			//	alert(parentKode);
		 window.location.href="diagnosis2_tree.php?act1="+sbx+"&kode="+kode+"&nama="+nama+"&level="+level+"&parentId="+parentId+"&parentKode="+parentKode+"&sur="+sur+"&aktif="+aktif+"&kdgol="+dg_kode;
