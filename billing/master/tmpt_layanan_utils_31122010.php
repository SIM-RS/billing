...
switch(strtolower($_REQUEST['act'])){
	case 'tambah':
		...
		if(mysql_affected_rows()==0){
			$sqlTambah="insert into b_ms_unit (kode,nama,level,parent_id,parent_kode,ket,kategori,jenis_layanan,inap,aktif)
				values('".$_REQUEST['parentKode'].$_REQUEST['kode']."','$nama','".$level."','".$_REQUEST['parentId']."','".$_REQUEST['parentKode']."','".$_REQUEST['ket']."','".$_REQUEST['ktgr']."','$cakupan','".$_REQUEST['inap']."','".$_REQUEST['aktif']."')";
			//echo $sqlTambah."<br/>";
	...
	case 'simpan':
		...
		if($rwParent['parent_id']==$_REQUEST['parentId']){
			$sqlSimpan="update b_ms_unit set kode='".$_REQUEST['parentKode'].$_REQUEST['kode']."',nama='$nama',level='".$level."',
...
if($statusProses=='Error'){	
	...
	$sql="SELECT id,kode,nama,level,jenis_layanan,parent_id,parent_kode,ket,if(kategori=1,'Loket',if(kategori=2,'Pelayanan',if(kategori=3,'Administrasi',if(kategori=4,'Kasir',null)))) as kategori,islast,if(inap=1,'Ya','Tidak') as inap,if(aktif=1,'Aktif','Tidak') as aktif FROM b_ms_unit ".$filter." order by ".$sorting;
	...
	while ($rows=mysql_fetch_array($rs)){
		$i++;
		$dt.=$rows["id"].'|'.$rows['jenis_layanan'].chr(3).number_format($i,0,",","").chr(3).$rows["kode"].chr(3).$rows["nama"].chr(3).$rows["level"].chr(3).$rows["parent_kode"].chr(3).$rows["ket"].chr(3).$rows["kategori"].chr(3).$rows["inap"].chr(3).$rows["aktif"].chr(6);
	}
