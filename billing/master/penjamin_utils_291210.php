...
if ($sorting==""){
	if($grd==2 ||$grd==3|| $grd==4|| $grd==5|| $grd==6|| $grd==7|| $grd==8|| $grd==9|| $grd==10|| $grd==11|| $grd==12){
		$defaultsort="nama";
	}
	$sorting=$defaultsort;
}
...
switch($grd){
	...
	{
		$i++;
		$dt.=$rows["id"].chr(3)."0".chr(3).$rows["nama"].chr(3).$rows["kelompok"].chr(3).$rows["klasifikasi"].chr(3).$rows["nilai"].chr(6);
	}
	case "11":
	while ($rows=mysql_fetch_array($rs))
	{
