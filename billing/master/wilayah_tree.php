...
<table width="1000" border="0" cellspacing="0" cellpadding="2" align="center" class="tabel">  
  ...
  <tr>
	...
	<td><button type="button" id="btnTree" name="btnTree" value="Tampilan Tabel" onclick="window.location='ref_wilayah_prof.php'" class="tblViewTable">Tampilan Tabel</button></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
	...
	<?php	
	  ...
	  $maxlevel=0;
	  $cnt=0;
	  $strSQL = "select * from b_ms_wilayah order by kode";
	  ...
	  while ($rows=mysql_fetch_array($rs)){
			 ...
			 $sql="select * from b_ms_wilayah where id=".$rows['parent_id'];
			 $rs1=mysql_query($sql);
...
switch(strtolower($_REQUEST['act'])){
	case 'tambah':
		mysql_query("select * from b_ms_wilayah where kode='".$_REQUEST['txtKode']."'");
		if(mysql_affected_rows()>0){
			echo "<script>alert('Data Sudah Ada!');</script>";
		}
		else{
			$sqlTambah="insert into b_ms_wilayah (kode,nama,level,parent_id,parent_kode,aktif)
				values('".$kode."','".$_REQUEST['txtNama']."','".$_REQUEST['txtLevel']."','".$_REQUEST['txtParentId']."','".$_REQUEST['txtParentKode']."','".$cekAktif."')";
			$rs=mysql_query($sqlTambah);
			echo "<script>window.location='wilayah_tree.php'</script>";
		}
		break;
	case 'hapus':
		mysql_query("select * from b_ms_wilayah where parent_id='".$_REQUEST['txtId']."'");
		if(mysql_affected_rows()>0){
			echo "<script>alert('Data merupakan induk! ".$_REQUEST['txtId']."');</script>";
		}
		else{
			$sql="delete from b_ms_wilayah where id='".$_REQUEST['txtId']."'";
			//echo $sql."<br>";
			$rs=mysql_query($sql);
		...
		break;
	case 'simpan':
		$sql="select * from b_ms_wilayah where kode='$kode' and nama='".$_REQUEST['txtNama']."' and level=$lvl and parent_id=$parentId and parent_kode='$ParentKode' and aktif='".$cekAktif."'";
		//echo $sql."<br>";
		$rs1=mysql_query($sql);
		if (mysql_num_rows($rs1)>0){
			echo "<script>alert('Data Tersebut Sudah Ada');</script>";
		}else{
			$sql="select * from b_ms_wilayah where id='".$_REQUEST['txtId']."'";
			//echo $sql."<br>";
			$rs=mysql_query($sql);
			...
			if ($cparent!=$parentId){
				...
				$sql="update b_ms_wilayah set islast=0 where id=".$parentId;
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				$sql="update b_ms_wilayah set kode='$kode',nama='".$_REQUEST['txtNama']."',level=$lvl,parent_id=$parentId,parent_kode='$ParentKode',aktif='".$cekAktif."' where id='".$_REQUEST['txtId']."'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				if ($cislast==0){
					$sql="update b_ms_wilayah set kode=CONCAT('$kode',right(kode,length(kode)-length('$cmkode'))),parent_kode=CONCAT('$kode',right(parent_kode,length(parent_kode)-length('$cmkode'))),level=".$cur_lvl." where left(kode,length('$cmkode'))='$cmkode'";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
				}
				$sql="select * from b_ms_wilayah where parent_id=$cparent";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				...
				if (($kode_ma!=$cmkode) && ($cislast==0)){
					$sql="update b_ms_wilayah set kode=CONCAT('$kode',right(kode,length(kode)-length('$cmkode'))),parent_kode=CONCAT('$kode',right(parent_kode,length(parent_kode)-length('$cmkode'))) where left(kode,length('$cmkode'))='$cmkode'";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
				}
			}
