...
$PATH_INFO="";
$_SESSION["PATH_INFO"]=$PATH_INFO.'&cabang='.$cabang;


$qstr_ma_sak="par=txtParentId*txtParentKode*txtParentLvl*txtLevel&cabang=".$cabang;
...
<table width="1000" border="0" cellspacing="0" cellpadding="2" align="center" class="tabel">  
  ...
  <tr>
	...
	<td width="45%">
		<select name="cabang" id="cabang" class="txtinput" onchange="window.location='tmpt_layanan_tree.php?cabang='+this.value">
			<?php 
  ...
  <tr>
	...
	<input id="txtParentKode"  name="txtParentKode" type="text" size="20" maxlength="20" style="text-align:center" value="<?php echo $ParentKode; ?>" readonly="true" class="txtcenter"> 
      <input type="button" class="btninput" id="btnParent" name="btnParent" value="..." title="Pilih Induk" onClick="OpenWnd('tmpt_layanan_tree_view.php?<?php echo $qstr_ma_sak; ?>&cabang=<?=$cabang?>',800,500,'msma',true)">             
  ...
  <tr>
	...
	<td><button type="button" id="btnTree" name="btnTree" value="Tampilan Tabel" onclick="window.location='tmpt_layanan.php?cabang=<?=$cabang?>'" class="tblViewTable">Tampilan Tabel</button></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  ...
  <tr>
	...
	<?php	
	  ...
	  if (isset($_REQUEST["p"])) {
		  $_SESSION['itemtree.filter'] = $_REQUEST["p"]."&cabang=".$cabang;
	  ...
	  $p .= '&cabang='.$cabang;
	  // $_SESSION['itemtree.filter'] .= '&cabang='.$cabang;
	  $canRead = true;
	  $include_del = 1;
	  $maxlevel=0;
	  $cnt=0;
	  $strSQL = "select * from b_ms_unit where cabang_id = '{$cabang}' order by kode";
	  ...
	  while ($rows=mysql_fetch_array($rs)){
			 ...
			 $sql="select * from b_ms_unit where id=".$rows['parent_id']." and cabang_id = '{$cabang}'";
			 $rs1=mysql_query($sql);
...
$level = ($_REQUEST['txtLevel']=='')?1:$_REQUEST['txtLevel'];

switch(strtolower($_REQUEST['act'])){
	case 'tambah':
		
		mysql_query("select * from b_ms_unit where kode='".$kode."' and cabang_id = '{$cabang}'");
		if(mysql_affected_rows()>0){
			echo "<script>alert('Data Sudah Ada!');</script>";
		}
		else{
			$sqlTambah="insert into b_ms_unit (kode,kode_ak,nama,level,parent_id,parent_kode,ket,kategori,jenis_layanan,islast,inap,aktif,cabang_id)
				values('".$kode."','$kodeAk','".$_REQUEST['txtNama']."','".$level."','".$_REQUEST['txtParentId']."','".$_REQUEST['txtParentKode']."','".$_REQUEST['txtKet']."','".$_REQUEST['txtKtgr']."','$cakupan','1','".$txtInap."','".$cekAktif."',{$cabang})";
			$rs=mysql_query($sqlTambah);
			if ($parentId>0){
				$sql="update b_ms_unit set islast=0 where id=$parentId and cabang_id = '{$cabang}'";
				//echo $sql;
				$rs=mysql_query($sql);
			}
			echo "<script>window.location='tmpt_layanan_tree.php?cabang={$cabang}'</script>";
		}
		break;
	case 'hapus':
		mysql_query("select * from b_ms_unit where parent_id='".$_REQUEST['txtId']."' and cabang_id = '{$cabang}'");
		if(mysql_affected_rows()>0){
			echo "<script>alert('Data merupakan induk! ".$_REQUEST['txtId']."');</script>";
		}
		else{
			$sql="delete from b_ms_unit where id='".$_REQUEST['txtId']."' and cabang_id = '{$cabang}'";
			//echo $sql."<br>";
			$rs=mysql_query($sql);
			$sql1="select * from b_ms_unit where parent_id=".$parentId." and cabang_id = '{$cabang}'";
			//echo $sql1."<br>";
			$rs2=mysql_query($sql1);
			if (mysql_num_rows($rs2)<=0){
				$sql2="update b_ms_unit set islast=1 where id=".$parentId." and cabang_id = '{$cabang}'";
				//echo $sql2."<br>";
		...
		break;
	case 'simpan':
		$sql="select * from b_ms_unit where kode='$kode' and nama='".$_REQUEST['txtNama']."' and level=$lvl and parent_id=$parentId and parent_kode='$ParentKode' and ket='".$_REQUEST['txtKet']."' and kategori='".$_REQUEST['txtKtgr']."' and inap='".$txtInap."' and aktif='".$cekAktif."' and cabang_id = '{$cabang}'";
		//echo $sql."<br>";
		$rs1=mysql_query($sql);
		if (mysql_num_rows($rs1)>0){
			echo "<script>alert('Data Tersebut Sudah Ada');</script>";
		}else{
			$sql="select * from b_ms_unit where id='".$_REQUEST['txtId']."' and cabang_id = '{$cabang}'";
			//echo $sql."<br>";
			$rs=mysql_query($sql);
			...
			if ($cparent!=$parentId){
				...
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				$sql="update b_ms_unit set kode='$kode',kode_ak='$kodeAk',nama='".$_REQUEST['txtNama']."',ket='".$_REQUEST['txtKet']."',kategori='".$_REQUEST['txtKtgr']."',jenis_layanan='$cakupan',level=$lvl,parent_id=$parentId,parent_kode='$ParentKode',inap='".$txtInap."',aktif='".$cekAktif."' , cabang_id = {$cabang} where id='".$_REQUEST['txtId']."' and cabang_id = '{$cabang}'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				if ($cislast==0){
					$sql="update b_ms_unit set kode=CONCAT('$kode',right(kode,length(kode)-length('$cmkode'))),parent_kode=CONCAT('$kode',right(parent_kode,length(parent_kode)-length('$cmkode'))),level=".$cur_lvl." where left(kode,length('$cmkode'))='$cmkode' and cabang_id = '{$cabang}'";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
				}
				$sql="select * from b_ms_unit where parent_id=$cparent and cabang_id = '{$cabang}'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				if (mysql_num_rows($rs)<=0){
					$sql="update b_ms_unit set islast=1 where id=".$cparent." and cabang_id = '{$cabang}'";
					//echo $sql."<br>";
					$rs1=mysql_query($sql);
				}				
			}else{	
				$sql="update b_ms_unit set kode='$kode',kode_ak='$kodeAk',nama='".$_REQUEST['txtNama']."',level=$lvl,parent_id=$parentId,parent_kode='$ParentKode',ket='".$_REQUEST['txtKet']."',kategori='".$_REQUEST['txtKtgr']."',jenis_layanan='$cakupan',inap='".$txtInap."',aktif='".$cekAktif."', cabang_id = {$cabang} where id='".$_REQUEST['txtId']."' and cabang_id = '{$cabang}'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				if (($kode!=$cmkode) && ($cislast==0)){
					$sql="update b_ms_unit set kode=CONCAT('$kode',right(kode,length(kode)-length('$cmkode'))),parent_kode=CONCAT('$kode',right(parent_kode,length(parent_kode)-length('$cmkode'))) where left(kode,length('$cmkode'))='$cmkode' and cabang_id = '{$cabang}'";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
				}
			}
