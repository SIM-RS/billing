	...
	case 'tambah':
		switch($_REQUEST["smpn"])
		{
			...
			case 'btnSimpanPem':
				...
				break;
			case 'btnSimpanKat':
				$sqlTambah="insert into b_ms_kategori_normal_lab (kode_kat,kategori,aktif_kat) values('".$_REQUEST['kode_kat']."','".$_REQUEST['kategori']."','".$_REQUEST['aktif_kat']."')";
				$rs=mysql_query($sqlTambah);
				break;
		}
		break;
	...
	case 'simpan':
		switch($_REQUEST["smpn"])
		{
			...
			case 'btnSimpanKat':
				...
				aktif_kat='".$_REQUEST['aktif_kat']."' where id_kat='".$_REQUEST['id_kat']."'";
				$rs=mysql_query($sqlUpdate);
				break;
		}
		break;
...
elseif($grd1 == "true")
{
	while ($rows=mysql_fetch_array($rs))
	{
		...
		$dt.=$arv.chr(3).$i.chr(3).$rows["nama"].chr(3).$dKat["kategori"].chr(3).$rows["normal1"]." - ".$rows["normal2"]." ".$rows["nama_satuan"].chr(3).$rows["metode"].chr(6);
	}
