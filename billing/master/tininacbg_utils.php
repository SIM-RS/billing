	...
	case 'tambah':
		switch($_REQUEST["smpn"])
		{
			...
			case 'btnSimpanTrf':
				...
				break;
			case 'inacbg':
				$sql1 = "SELECT CONCAT('C',SUBSTR(MAX(cui),2)+1) AS no_cui1, MAX(cui) AS no_cui FROM mrconso";
				$rsql1 = mysql_query($sql1);
				$dsql1 = mysql_fetch_array($rsql1);
				$id_cui = $dsql1['no_cui1'];
				
				$sql3 = "SELECT CONCAT('A',SUBSTR(MAX(aui),2)+1) AS no_aui FROM mrconso";
				$rsql3 = mysql_query($sql3);
				$dsql3 = mysql_fetch_array($rsql3);
				$id_aui = $dsql3['no_aui'];
				...
				if($parent=="-")
				{
					...
					$sqlTambah1="insert into mrconso(CUI,LAT,TS,LUI,STT,SUI,ISPREF,AUI,SAUI,SCUI,SDUI,SAB,TTY,CODE,STR,SRL,SUPPRESS,CVF) 
					values('".$id_cui."','ENG','P','Lxxxxxxx','PF','Sxxxxxxx','Y','$id_aui',' ',' ',' ','ICD9CM_2005','PT','$code','$nm',0,'N','256')";
					$rsqlTambah1=mysql_query($sqlTambah1);
					
					$sqlTambah2="insert into mrhier(CUI,AUI,CXN,PAUI,SAB,RELA,PTR,HCD,CVF)
					values('".$id_cui."','$id_aui',1,'$id_paui','ICD9CM_2005',' ','$ptr',' ',' ')";
					$rsqlTambah2=mysql_query($sqlTambah2);
					
				
				}else{
					$pui = "SELECT * FROM mrconso WHERE CODE = '$parent'";
					$rpui = mysql_query($pui);
					$dpui = mysql_fetch_array($rpui);
					$p_aui = $dpui['AUI'];
					
					$sql2 = "SELECT * FROM mrhier WHERE aui = '$p_aui'";
					$rsql2 = mysql_query($sql2);
					$dsql2 = mysql_fetch_array($rsql2);
					$ptr = $dsql2['PTR'];
					$ptr1 = $ptr.".".$p_aui;
					...
					$sqlTambah2="insert into mrhier(CUI,AUI,CXN,PAUI,SAB,RELA,PTR,HCD,CVF)
					values('".$id_cui."','$id_aui',1,'$p_aui','ICD9CM_2005',' ','$ptr1',' ',' ')";
					$rsqlTambah2=mysql_query($sqlTambah2);
					
					$sqlTambah3="update mrconso set TTY='HT', ISPREF='N', TS='S' where AUI = '$p_aui'";
					$rsqlTambah3=mysql_query($sqlTambah3);
					
				}
				
				//keterkaitan TS,ISPREF,TTY --> PT, Y, P (TTY, ISPREF, TS) create jadi PT parent jadi HT --- HT -> HT, N, S
			...
			break;
			case 'btnSimpanPem':
				$sqlTambah="insert into b_ms_pemeriksaan_lab(kelompok_lab_id,nama) values('".$_REQUEST['kelId']."','".$_REQUEST['txtTindPem']."')";
				$rs=mysql_query($sqlTambah);
				break;
		}
		break;
	...
	case 'simpan':
		switch($_REQUEST["smpn"])
		{
			...
			case 'btnSimpanKel':
				...
				break;
			case 'btnSimpanPem':
				$sqlUpdate="update b_ms_pemeriksaan_lab set kelompok_lab_id='".$_REQUEST['kelId']."',nama='".$_REQUEST['txtTindPem']."' where id=".$_REQUEST['id'];
				$rs=mysql_query($sqlUpdate);
				break;
			case 'inacbg':
				if($parent=="-")
				{
					...
					$sqlSimpan1 = "update mrconso set CODE='$code',STR='$nm' where AUI = '$aui'";
					$rsqlSimpan1 = mysql_query($sqlSimpan1);
					
					$sqlSimpan1 = "update mrhier set PTR='$ptr',PAUI='$id_paui' where AUI = '$aui'";
					$rsqlSimpan1 = mysql_query($sqlSimpan1);
					...
					$sqlSimpan1 = "update mrhier set PTR='$ptr1',PAUI='$p_aui' where AUI = '$aui'";
					$rsqlSimpan1 = mysql_query($sqlSimpan1);
	...
	case 'hapus':
		switch($_REQUEST["hps"])
		{
			...
			case 'btnHapusPem':
				...
				break;
			case 'inacbg':
				$sqlHapus="delete from mrconso WHERE aui = '$aui'";
				mysql_query($sqlHapus);
				$sqlHapus="delete from mrhier WHERE aui = '$aui'";
				mysql_query($sqlHapus);
				break;
		}
		break;
...
if($grd == "true")
{
	...
	$sql = "SELECT m.CUI, m.AUI, m.CODE, m.STR, mh.PAUI, mh.PTR, m.TTY, IFNULL((SELECT str FROM mrconso WHERE aui = mh.PAUI),'') AS induk, IFNULL(mm.code,'-') as p_code
	FROM mrconso m INNER JOIN mrhier mh ON m.AUI = mh.AUI 
	LEFT JOIN mrconso mm ON mh.PAUI = mm.AUI
...
elseif($grd3=="true")
{
	...
	$sql = "SELECT b.id,c.id AS kelid,b.nama 
...
if($grd == "true")
{
	...
	{
		$i++;
		$dt.=$rows["AUI"]."|".$rows["p_code"]."|".$rows["CODE"]."|".$rows["STR"].chr(3).$i.chr(3).$rows["CODE"].chr(3).$rows["STR"].chr(3).$rows["induk"].chr(6);
	}
}
elseif($grd1 == "true")
{
...
elseif($grd3 == "true")
{
	...
	{
		$i++;
		$arv = $rows["id"]."|".$rows["nama"]."|".$rows["kelid"];
