...
mTab.setTabCaption("MASTER JADWAL DOKTER,JADWAL DOKTER");
mTab.setTabCaptionWidth("500,500");
mTab.onLoaded(showgrid);
mTab.setTabPage("jadwal_dokter_master.php,jadwal_dokter_generate.php");
...
function showgrid(){
	...
	tab1.attachEvent("onRowClick","ubah2");
...
function simpan(){
	...
	if(mulai == '' || selesai == '')
		...
		if(parseInt(jQuery.ajax({type:'get', async:false, url:'jadwal_dokter_master_utils.php?act=cek&unit_id='+unit_id+'&dokter_id='+dokter_id+'&hari='+hari+'&mulai='+mulai+'&selesai='+selesai}).responseText) > 0)
			alert('Jadwal tersebut bertabrakan dengan jadwal yang sudah ada!');
		else{
			tab1.loadURL("jadwal_dokter_master_utils.php?act=simpan&id="+id+'&unit_id='+unit_id+'&dokter_id='+dokter_id+'&hari='+hari+'&mulai='+mulai+'&selesai='+selesai,"","GET");
			batal();
		}
	}
}
...
function ubah(){
	var ids = tab2.getRowId(tab1.getSelRow()).split('||');
