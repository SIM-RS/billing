...
<?php
	...
	if($waktu == 'Harian'){
		...
		$waktu = " h.`tgl_act` = '$tglAwal2' ";
		$Periode = "Tanggal : ".$tglAwal[0]." ".$arrBln[$tglAwal[1]]." ".$tglAwal[2];
	}
	else if($waktu == 'Bulanan'){
		$bln = $_POST['cmbBln'];
		$thn = $_POST['cmbThn'];
		$waktu = " month(h.`tgl_act`) = '$bln' and year(h.`tgl_act`) = '$thn' ";
		...
		$waktu = " h.`tgl_act` between '$tglAwal2' and '$tglAkhir2' ";
...
<table width="800" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="5" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase"><b>36. Laporan Laboratorium <?php echo $rwUnit1['nama'] ?> - <?php if($rwUnit2['nama']=="") echo 'Semua'; else echo $rwUnit2['nama'] ?><br /><?php echo $Periode;?></b></td>
  </tr>
  <tr>
    <td height="30" colspan="3"><b></b></td>
  ...
  <tr height="30">
    ...
    <td colspan="3" style="border-top:1px solid; border-bottom:1px solid;">&nbsp;Diagnosis</td>
    <td width="20%" style="border-top:1px solid; border-bottom:1px solid;">Jumlah&nbsp;&nbsp;&nbsp;</td>
  </tr>
  <?php
...
//		}
		...
		$sqlDiag = "SELECT l.`nama` AS hasil, COUNT(l.id) AS jml 
					FROM b_hasil_lab h 
					INNER JOIN b_ms_normal_lab p ON p.`id`=h.`id_normal` 
					INNER JOIN b_ms_pemeriksaan_lab l ON l.`id`=p.`id_pemeriksaan_lab` 
					INNER JOIN b_pelayanan pl ON pl.`id`=h.`id_pelayanan` 
					INNER JOIN b_ms_pasien ps ON ps.`id`=pl.`pasien_id` 
					INNER JOIN b_kunjungan k ON k.`id`=pl.`kunjungan_id` 
					WHERE $waktu $fKso GROUP BY l.`id`;";
  ...
  <tr>
  	<td>&nbsp;<?php echo $no; ?></td>
	<td colspan="3" style="text-transform:uppercase">&nbsp;<?php echo $rwDiag['hasil']; ?></td>
    <td style="padding-right:10px"><?php echo $rwDiag['jml']; ?>&nbsp;&nbsp;&nbsp;</td>
  </tr>
  <?php
  		 $no++;
