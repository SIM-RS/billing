...
<?php
    ...
    $sqlUnit1 = "SELECT id,nama,inap FROM b_ms_unit WHERE id = '".$jnsLay."'";
    $rsUnit1 = mysql_query($sqlUnit1);
    $rwUnit1 = mysql_fetch_array($rsUnit1);
	
	if($tmpLay != "0"){
		$fUnit = "p.unit_id='$tmpLay'";
		$sqlUnit2 = "SELECT id,nama,inap FROM b_ms_unit WHERE id = '".$tmpLay."'";
		$rsUnit2 = mysql_query($sqlUnit2);
		$rwUnit2 = mysql_fetch_array($rsUnit2);
		$tmpNama = $rwUnit2['nama'];
		$tmpInap = $rwUnit2['inap'];
	} else {
		$fUnit = "p.jenis_layanan='$jnsLay'";
		$tmpNama = "SEMUA";
		$tmpInap = $rwUnit1['inap'];
	}
...
<table width="800" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
    ...
    <tr>
        <td colspan="2" height="70" valign="top" style="text-align:center; text-transform:uppercase; font-weight:bold; font-size:14px;">laporan penerimaan (seluruh pasien) <?php echo $rwUnit1['nama']?> - <?php echo $tmpNama; ?><br><?php echo $Periode;?></td>
    </tr>
    <tr>
        <td height="30" width="50%" style="font-weight:bold">&nbsp;Tempat Layanan:&nbsp;<?php echo $tmpNama; ?></td>
...
WHERE $fUnit $waktu AND bt.tipe=0 AND bt.nilai>0 GROUP BY p.id";
					}else{
...
WHERE $fUnit $waktu AND t.kso_id='".$rwKso1['id']."' AND bt.tipe=0 AND bt.nilai>0 GROUP BY p.id
UNION 
...
INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
WHERE $fUnit $waktu AND p.kso_id='".$rwKso1['id']."' AND bt.tipe=1 AND bt.nilai>0 GROUP BY p.id) AS t1 GROUP BY t1.id) AS t2 
...
FROM b_bayar b INNER JOIN b_bayar_tindakan bt ON b.id=bt.bayar_id
INNER JOIN b_tindakan t ON bt.tindakan_id=t.id INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
WHERE $fUnit $waktu AND t.kso_id='".$rwKso1['id']."' AND bt.tipe=0 AND bt.nilai>0 GROUP BY p.id) AS t1 
