...
<title>Kasus Penyakit Pasien Berdasarkan Penjamin Pasien</title>
</head>
...
<table width="800" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px">
  ...
  <tr>
    <td colspan="5" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase"><b>Laporan Kasus Penyakit Pasien Berdasarkan Penjamin Pasien<br />Jenis Layanan <?php echo $rwUnit1['nama'];?>&nbsp;Tempat Layanan <?php echo $rwUnit2['nama'];?><br /><?php echo $Periode;?></b></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <tr>
    <td width="40%" height="28">&nbsp;STATUS PASIEN</td>
  ...
  <?php
		$qTmp = "SELECT k.kso_id, kso.nama FROM b_pasien_keluar pk
		...
		while($rwTmp = mysql_fetch_array($rsTmp))
		{
			$qBaru = "SELECT COUNT(k.pasien_id) AS jml
					...
					WHERE pk.kasus = '1' AND k.kso_id = '".$rwTmp['kso_id']."' AND pl.unit_id = '".$_REQUEST['TmpLayanan']."' $waktu ";
			...
			$qLama = "SELECT COUNT(k.pasien_id) AS jml
					...
					INNER JOIN b_pelayanan pl ON pl.kunjungan_id = k.id 
					INNER JOIN b_pasien_keluar pk ON pk.pelayanan_id = pl.id 
					WHERE pk.kasus = '0' AND k.kso_id = '".$rwTmp['kso_id']."' AND pl.unit_id = '".$_REQUEST['TmpLayanan']."' $waktu ";
			...
			$qK = "SELECT COUNT(k.pasien_id) AS jml
				FROM b_kunjungan k 
				INNER JOIN b_pelayanan pl ON pl.kunjungan_id = k.id 
				WHERE pl.unit_id = '".$_REQUEST['TmpLayanan']."' AND k.kso_id = '".$rwTmp['kso_id']."' $waktu ";
			$rsK = mysql_query($qK);
