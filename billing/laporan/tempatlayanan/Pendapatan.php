...
<table width="750" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
	...
	<tr>
		<td colspan="2" style="text-align:center; font-size:14px; text-transform:uppercase; font-weight:bold;" height="70" valign="top">laporan pendapatan <?php echo $rwUnit1['nama']?> - <?php echo $tmpNama; ?><br><?php echo $Periode;?></td>
	</tr>
	<tr height="30">
		<td width="50%">&nbsp;<b>Tempat Layanan: <?php echo $tmpNama; ?></b></td>
	...
	<tr>
		<td colspan="2">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				...
				<?php
						$qKso="SELECT t.kso_id, kso.nama FROM b_kunjungan k 
								...
								WHERE $fUnit $waktu $fKso AND pl.flag = '$flag' GROUP BY kso.id";
						//echo $qKso."<br>";
...
INNER JOIN b_tindakan t ON t.pelayanan_id = pl.id 
INNER JOIN b_ms_unit u ON u.id = pl.unit_id
WHERE $fUnit $waktu AND p.flag = '$flag' AND t.kso_id = '".$rwKso['kso_id']."' GROUP BY pl.id ORDER BY pl.tgl,p.nama";
						$rs = mysql_query($sql);
						$no = 1;
						$sub = 0;
						$jml = 0;
...
WHERE p.id = '".$rw['pelayanan_id']."' $waktu AND p.flag = '$flag' AND mt.klasifikasi_id = '11' AND t.kso_id='".$rwKso['kso_id']."'";
...
WHERE p.id = '".$rw['pelayanan_id']."' $waktu AND p.flag = '$flag' AND mt.klasifikasi_id <> '11' AND t.kso_id='".$rwKso['kso_id']."'";
				...
				<tr height="30" style="font-weight:bold">
					...
					<td valign="top" style=" text-align:right; border-top:1px solid;"><span style="text-align:right; padding-right:10px;"><?php echo number_format($sub_ret,0,",",".");?>&nbsp;</span></td>
					<td valign="top" style="text-align:right; border-top:1px solid;"><span style="text-align:right; padding-right:10px;"><?php echo number_format($sub_tind,0,",",".");?>&nbsp;</span></td>
					<td valign="top" style="text-align:right; padding-right:10px; border-top:1px solid;"><?php echo number_format($sub,0,",",".");?>&nbsp;</td>
				</tr>
				<?php 
							$tot = $tot + $sub;
							$tot_tind = $tot_tind + $sub_tind;
							$tot_ret = $tot_ret + $sub_ret;
						}
						mysql_free_result($rsKso);
				?>
				<tr height="30" style="font-weight:bold;">
					<td colspan="4" style="text-align:right; border-bottom:1px solid; border-top:1px solid">Total&nbsp;</td>
					<td style="border-bottom:1px solid; border-top:1px solid; text-align:right; padding-right:10px;"><?php echo number_format($tot_ret,0,",",".");?>&nbsp;</td>
					<td style="border-bottom:1px solid; border-top:1px solid; text-align:right; padding-right:10px;"><?php echo number_format($tot_tind,0,",",".");?>&nbsp;</td>
...
INNER JOIN b_ms_unit u ON u.id = pl.unit_id
WHERE $fUnit $waktu2 $fKso1 AND pl.flag = '$flag'
...
INNER JOIN b_ms_unit u ON u.id = pl.unit_id
WHERE $fUnit $waktu2 AND pl.kso_id='".$rwKso['kso_id']."' AND p.flag = '$flag'
...
INNER JOIN b_ms_tindakan mt ON mtk.ms_tindakan_id = mt.id
WHERE t.pelayanan_id = '".$rw['pelayanan_id']."' $waktu AND t.flag = '$flag' AND mt.klasifikasi_id = '11' AND t.kso_id='".$rwKso['kso_id']."'";
							$rsRet = mysql_query($qRet);
							$rwRet = mysql_fetch_array($rsRet);
...
WHERE t.pelayanan_id='".$rw['pelayanan_id']."' AND p.flag = '$flag' $waktu2) AS t";
...
FROM b_tindakan_kamar t WHERE t.pelayanan_id='".$rw['pelayanan_id']."' AND DATE(t.tgl_in) <= LAST_DAY('$waktu3') AND (DATE(t.tgl_out) >= '$waktu3' OR t.tgl_out IS NULL)) AS t1";
				...
				<?php
								...
								$sub_itind = $sub_itind + $itind;
								$sub_kmr = $sub_kmr + $kmr;
								$sub_iret = $sub_iret + $iret;
				...
				<tr height="30" style="font-weight:bold">
					...
					<td>&nbsp;</td>
					<td valign="top" style="text-align:right;">Subtotal</td>
					<td valign="top" style="text-align:right; padding-right:10px; border-top:1px solid;"><?php echo number_format($sub_iret,0,",",".");?>&nbsp;</td>
					<td valign="top" style="text-align:right; padding-right:10px; border-top:1px solid;"><?php echo number_format($sub_itind,0,",",".");?>&nbsp;</td>
					<td valign="top" style="text-align:right; padding-right:10px; border-top:1px solid;"><?php echo number_format($sub_kmr,0,",",".");?>&nbsp;</td>
					<td valign="top" style="text-align:right; padding-right:10px; border-top:1px solid;"><?php echo number_format($sub,0,",",".");?>&nbsp;</td>
				</tr>
				<?php 
						$tot = $tot + $sub;
						$tot_kmr = $tot_kmr + $sub_kmr;
						$tot_itind = $tot_itind + $sub_itind;
						$tot_iret = $tot_iret + $sub_iret;
						} mysql_free_result($rsKso);
				?>
				<tr height="30" style="font-weight:bold;">
					<td colspan="4" style="text-align:right; border-bottom:1px solid; border-top:1px solid">Total&nbsp;</td>
					<td style="border-bottom:1px solid; border-top:1px solid; text-align:right; padding-right:10px;"><?php echo number_format($tot_ret,0,",",".");?>&nbsp;</td>
					<td style="border-bottom:1px solid; border-top:1px solid; text-align:right; padding-right:10px;"><?php echo number_format($tot_itind,0,",",".");?>&nbsp;</td>
					<td style="border-bottom:1px solid; border-top:1px solid; text-align:right; padding-right:10px;"><?php echo number_format($tot_kmr,0,",",".");?>&nbsp;</td>
					<td style="border-bottom:1px solid; border-top:1px solid; text-align:right; padding-right:10px;"><?php echo number_format($tot,0,",",".");?>&nbsp;</td>
				</tr>
			</table>
			</div>
