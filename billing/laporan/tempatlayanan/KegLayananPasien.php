...
if($_REQUEST['TmpLayanan']!=0){
	...
	$fUnit = " mu.parent_id=".$_REQUEST['JnsLayanan'];
		...
		<td>
			<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				<tr style="font-size:12px; font-weight:bold;">
					...
					<td align="right" width="15%" style="border-top:1px solid; border-bottom:1px solid; padding-right:20px;">Tarif RS</td>
				...
				<?php
						$sql = "SELECT mt.id, mt.nama AS layanan, t.biaya, 
								...
								INNER JOIN b_ms_kelompok_tindakan kt ON kt.id = mt.kel_tindakan_id
								INNER JOIN b_ms_unit mu ON mu.id = pl.unit_id
								WHERE $fUnit AND kt.id = '".$rwLay['id']."' $waktu $fKso
								GROUP BY mt.nama";
						$rs = mysql_query($sql);
						$no=1;
						$jml = 0;
