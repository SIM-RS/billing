...
if($_REQUEST['TmpLayanan']!='0'){	
	...
	$fUnit=" WHERE b_pelayanan.unit_id =".$_REQUEST['TmpLayanan'];
}else
	$fUnit=" WHERE 0 = 0 ";
  	...
  	<td>
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			<tr style="font-weight:bold; text-align:center;">
				...
				<td width="23%" style="border-top:1px solid; border-bottom:1px solid;">&nbsp;Nama</td>
				<td width="2%" align="center" style="border-top:1px solid; border-bottom:1px solid;">&nbsp;JK</td>
				<td width="15%" align="center" style="border-top:1px solid; border-bottom:1px solid;">&nbsp;Umur</td>
		  </tr>
			  <?php
				$sqlRuj = "SELECT b_ms_unit.nama AS asal, b_ms_pasien.id, b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, b_ms_pasien.sex, b_kunjungan.umur_bln, b_kunjungan.umur_thn,b_kunjungan.umur_hr, b_ms_pegawai.nama AS dokter, DATE_FORMAT(b_pelayanan.tgl,'%d-%m-%Y') AS tgl FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id 
				INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id 
				INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_pelayanan.dokter_id 
				INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id_asal 
				{$fUnit} $waktu $fKso GROUP BY b_pelayanan.id ORDER BY b_pelayanan.tgl";	
			  ...
			  <tr>
				...
				<td align="center"><?php echo $rwRuj['umur_thn'].' thn '.$rwRuj['umur_bln'].' bln'.$rwRuj['umur_hr'].' hr';?></td>
			  </tr>
			  <?php
					$no++;
				}
