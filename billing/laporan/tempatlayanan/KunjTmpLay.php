...
<title>Kunjungan Tempat Layanan</title>
</head>
...
<?php
	...
	if($waktu == 'Harian'){
		...
		$bln = $_POST['cmbBln'];
		$thn = $_POST['cmbThn'];
		$waktu = " AND month(b_pelayanan.tgl) = '$bln' and year(b_kunjungan.tgl) = '$thn' ";
...
<table width="750" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td colspan="2" align="right" height="30">Yang Mencetak :&nbsp;<b><?php echo $rwPeg['nama'];?></b>&nbsp;</td>
  </tr>
  <tr height="30" style="font-size:12px; font-weight:bold;">
    ...
    <td width="15%" align="right" style="border-top:1px solid;">Kunjungan Ulang</td>
  ...
  <tr>
	...
	<?php
		if($_REQUEST['TmpLayanan']!=0){
			$fUnit = "b_ms_unit.id = '".$_REQUEST['TmpLayanan']."'";
		}else{
			$fUnit = "b_ms_unit.parent_id = '".$_REQUEST['JnsLayanan']."'";
		}
			
			$sqlKunj = "SELECT b_ms_unit.id, b_ms_unit.nama FROM b_kunjungan
				...
				WHERE $fUnit $waktu $fKso GROUP BY b_ms_unit.nama";
  ...
  <tr height='21'>
    <td>&nbsp;</td>
    <td>&nbsp;<strong><?php echo '* '.$rwKunj['nama']; ?></strong></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <?php
  $sqlKso = "SELECT b_ms_kso.id, b_ms_kso.nama FROM b_kunjungan
			INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id
			INNER JOIN b_ms_kso ON b_ms_kso.id = b_pelayanan.kso_id
			WHERE b_pelayanan.unit_id = '".$rwKunj['id']."' $waktu $fKso
			GROUP BY b_ms_kso.id";
  $rsKso = mysql_query($sqlKso);
  while($rowKso = mysql_fetch_array($rsKso)){
	?>
  <tr height='25'>
    <td>&nbsp;</td>
    <td colspan='2'>&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<b><u><?php echo $rowKso['nama'] ?></u></b></td>
    <?php
		$qBr = "SELECT COUNT(t.id) AS jml, t.kso_id FROM (SELECT b_pelayanan.id, b_pelayanan.kso_id FROM b_kunjungan 
INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id WHERE b_kunjungan.isbaru = '1' AND b_pelayanan.unit_id = '".$rwKunj['id']."' $waktu AND b_pelayanan.kso_id = '".$rowKso['id']."' GROUP BY b_pelayanan.id) AS t GROUP BY t.kso_id ";
		$sBr = mysql_query($qBr);
		$wBr = mysql_fetch_array($sBr);
	?>
    <td style="text-align:right; padding-right:20px;"><?php if($wBr['jml']==0) echo 0; else echo $wBr['jml']; ?></td>
	<?php
		$qLm = "SELECT COUNT(t.id) AS jml, t.kso_id FROM (SELECT b_pelayanan.id, b_pelayanan.kso_id FROM b_kunjungan 
INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id WHERE b_kunjungan.isbaru = '0' AND b_pelayanan.unit_id = '".$rwKunj['id']."' $waktu AND b_pelayanan.kso_id = '".$rowKso['id']."' GROUP BY b_pelayanan.id) AS t GROUP BY t.kso_id";
		$sLm = mysql_query($qLm);
		$wLm = mysql_fetch_array($sLm);
	?>
    <td style="text-align:right; padding-right:20px;"><?php if($wLm['jml']==0) echo 0; else echo $wLm['jml']; ?></td>
  ...
  <tr>
	...
	<td>&nbsp;</td>
	<td>&nbsp;</td>
	<td colspan="3" align="right">Tgl Cetak: <?php echo $date_now;?>&nbsp;Jam: <?php echo $jam;?></td>
  </tr>
  <tr>
  	<td>&nbsp;</td>
	<td>&nbsp;</td>
  ...
  <tr>
	...
	<td>&nbsp;</td>
	<td>&nbsp;</td>
	<td colspan="2" style="text-align:right; text-transform:uppercase;"><b><?php echo $rwPeg['nama'];?></b>&nbsp;</td>
  </tr>
  <tr><td colspan="6">&nbsp;</td></tr>
  <tr id="trTombol">
