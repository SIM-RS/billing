		...
		<tr>
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td width="25%" align="center" style="border:#000000 1px solid">Status Pasien</td>
						<td width="25%" align="center" style="border-bottom:#000000 1px solid; border-right:#000000 1px solid; border-top:#000000 solid 1px">Tempat Layanan Asal</td>
					...
					<?php
					$qKso = "SELECT kso.id, kso.nama FROM b_kunjungan k
					...
					?>
					<tr>
						<td colspan="4">&nbsp;<?php echo $rwKso['nama']?></td>
					</tr>
					<?php
					$qUnitAsal = "SELECT u.id, u.nama AS unit
								FROM b_pelayanan pl
								...
								WHERE pl.kso_id = '".$rwKso['id']."' $waktu 
								AND pl.unit_id = '".$_REQUEST['TmpLayanan']."'
								GROUP BY u.nama";
					$rsUnitAsal = mysql_query($qUnitAsal);
					while($rwUnitAsal = mysql_fetch_array($rsUnitAsal)){
					?>
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;<?php echo $rwUnitAsal['unit']?></td>
					    <td>&nbsp;</td>
					    <td>&nbsp;</td>
					</tr>
					<?php
						$qT = "SELECT mt.nama AS tindakan, SUM(t.qty) AS jml FROM b_ms_tindakan mt
								...
								INNER JOIN b_kunjungan k ON k.id = pl.kunjungan_id
								WHERE pl.unit_id_asal = '".$rwUnitAsal['id']."' AND pl.unit_id = '".$_REQUEST['TmpLayanan']."'
								$waktu AND pl.kso_id = '".$rwKso['id']."' GROUP BY mt.nama";
						$rsT = mysql_query($qT);
					...
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td>&nbsp;<?php echo $rwT['tindakan'];?></td>
						<td align="center"><?php echo $rwT['jml'];?></td>
					</tr>
					<?php
						}
						mysql_free_result($rsT);
						}
						mysql_free_result($rsUnitAsal);
					}
