...
WHERE b_pelayanan.id = '".$rwLop['pelayanan_id']."' GROUP BY b_tindakan_kamar.id ) AS cH";
							 ...
							 $qOp = "SELECT SUM(t.total) AS jml FROM (SELECT b_tindakan.qty,
								...
								AND b_pelayanan.unit_id_asal = '".$rwLop['unit_id']."' AND b_pelayanan.unit_id IN (47,63)) AS t";
							 $rsOp = mysql_query($qOp);
							 $rwOp = mysql_fetch_array($rsOp);
							 
							 $qPk = "SELECT SUM(t.total) AS jml FROM (SELECT b_tindakan.qty, b_tindakan.biaya_kso, b_tindakan.qty*b_tindakan.biaya_kso AS total  FROM b_pelayanan 
INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id 
WHERE b_pelayanan.kunjungan_id='".$rw['kunjungan_id']."' AND b_pelayanan.jenis_kunjungan='3'
AND b_pelayanan.id>'".$rwLop['pelayanan_id']."' AND b_pelayanan.unit_id='58' 
AND (b_pelayanan.unit_id_asal='".$rwLop['unit_id']."' 
OR b_pelayanan.unit_id_asal IN 
(SELECT unit_id FROM b_pelayanan 
...
AND b_pelayanan.id>'".$rwLop['pelayanan_id']."' AND b_pelayanan.unit_id_asal='".$rwLop['unit_id']."' AND b_ms_unit.inap=0  ))) AS t";
							...
							/*$qPa = "SELECT SUM(t.total) AS jml FROM (SELECT b_tindakan.qty, b_tindakan.biaya_kso, b_tindakan.qty*b_tindakan.biaya_kso AS total FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id WHERE b_pelayanan.kunjungan_id = '".$rw['kunjungan_id']."'  AND b_pelayanan.id > '".$rwLop['pelayanan_id']."'  AND b_pelayanan.unit_id = '59'  AND b_pelayanan.unit_id_asal = '".$rwLop['unit_id']."') AS t";
							 ...
							 $qPa = "SELECT SUM(t.total) AS jml FROM (SELECT b_tindakan.qty, b_tindakan.biaya_kso, b_tindakan.qty*b_tindakan.biaya_kso AS total  FROM b_pelayanan 
INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id 
WHERE b_pelayanan.kunjungan_id='".$rw['kunjungan_id']."' AND b_pelayanan.jenis_kunjungan='3'
AND b_pelayanan.id>'".$rwLop['pelayanan_id']."' AND b_pelayanan.unit_id='59' 
AND (b_pelayanan.unit_id_asal='".$rwLop['unit_id']."' 
OR b_pelayanan.unit_id_asal IN 
(SELECT unit_id FROM b_pelayanan 
INNER JOIN b_ms_unit ON b_ms_unit.id=b_pelayanan.unit_id
...
AND b_pelayanan.id>'".$rwLop['pelayanan_id']."' AND b_pelayanan.unit_id_asal='".$rwLop['unit_id']."' AND b_ms_unit.inap=0  ))) AS t";
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						<tr>
							...
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><?php if($rwOp['jml']!="") echo number_format($rwOp['jml'],0,",","."); else echo "-";?></td>
