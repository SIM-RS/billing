...
switch(strtolower($grd)){
	case '1':
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':		
				$sqlTambah="insert into b_ms_kso (kode,nama,alamat,telp,fax,kontak)
					values('".$kodebr."','".$_REQUEST['nama']."','".$_REQUEST['alamat']."','".$_REQUEST['telp']."','".$_REQUEST['fax']."','".$_REQUEST['kontak']."')";
			...
			case 'hapus':
				...
				break;
			case 'simpan':
				$sqlSimpan = "UPDATE b_ms_kso SET nama='".$_REQUEST['nama']."',alamat='".$_REQUEST['alamat']."',telp='".$_REQUEST['telp']."',fax='".$_REQUEST['fax']."',kontak='".$_REQUEST['kontak']."' WHERE id='".$_REQUEST['id']."'";	
			...
			case 'tambah':
				for($i=0;$i<(sizeof($id)-1);$i++){
					...
					if(mysql_num_rows($rsCek)==0){
						$sqlTambah="insert into b_ms_tindakan_tdk_jamin (b_ms_tindakan_id,b_ms_kso_id)
							values('".$id[$i]."','".$_REQUEST['ksoId']."')";
						$rs=mysql_query($sqlTambah);
					}
				}
				break;
	...
	case '5':		
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':
				$sqlTambah="insert into b_ms_kso_paket_hp (b_ms_kso_id,b_ms_kelas_id,jaminan) values ('".$_REQUEST['ksoId']."','".$_REQUEST['kelas']."','".$_REQUEST['nilai']."')";
				mysql_query($sqlTambah);
				break;
			case 'simpan':
				$sqlSimpan="update b_ms_kso_paket_hp set b_ms_kso_id='".$_REQUEST['ksoId']."'
				,b_ms_kelas_id='".$_REQUEST['kelas']."',jaminan='".$_REQUEST['nilai']."' where id='".$_REQUEST['id']."'";
				//echo $sqlSimpan."</br>";
	...
	case '6':
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':
				$sqlTambah="insert into b_ms_kso_paket (kso_id,nama,nilai,paket,instalasi,frekuensi) values ('".$_REQUEST['ksoId']."','".$_REQUEST['nama']."','".$_REQUEST['nilai']."','".$_REQUEST['status']."','".$_REQUEST['instal']."','".$_REQUEST['frekuensi']."')";
				mysql_query($sqlTambah);
				break;
			case 'simpan':
				...
				,frekuensi='".$_REQUEST['frekuensi']."' where id='".$_REQUEST['id']."'";
				mysql_query($sqlSimpan);
				break;
			case 'hapus':
		...
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':
				for($i=0;$i<(sizeof($id)-1);$i++){
					...
					$rsCek=mysql_query($sqlCek);
					if(mysql_num_rows($rsCek)==0){
						$sqlTambah="insert into b_ms_kso_paket_detail (ms_tindakan_id,kso_paket_id)
							values('".$id[$i]."','".$_REQUEST['paketId']."')";
						$rs=mysql_query($sqlTambah);
					}
				}
				break;
		...
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':
				for($i=0;$i<(sizeof($id)-1);$i++){
					...
					$rsCek=mysql_query($sqlCek);
					if(mysql_num_rows($rsCek)==0){
						$sqlTambah="insert into b_ms_kso_luar_paket (ms_tindakan_id,kso_id)
							values('".$id[$i]."','".$_REQUEST['ksoId']."')";
						$rs=mysql_query($sqlTambah);
					}
				}
			...
			case 'hapus':
				...
				break;
			case 'editbiaya':
				$sqlEditBiaya="update b_ms_kso_luar_paket set nilai='".$_REQUEST['biaya']."' where id='".$_REQUEST['id']."'";
	...
	case '12':
		...
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':
				for($i=0;$i<(sizeof($id)-1);$i++){
					...
					$rsCek=mysql_query($sqlCek);
					if(mysql_num_rows($rsCek)==0){
						$sqlTambah="insert into b_ms_kso_pakomodasi (ms_tindakan_id,ms_kso_id)
							values('".$id[$i]."','".$_REQUEST['ksoId']."')";
						$rs=mysql_query($sqlTambah);
					}
				}
		...
		switch(strtolower($_REQUEST['act'])){
			case 'update':
				$sqlSimpan = "UPDATE b_ms_kso_pasien SET instansi_id='".$_REQUEST['instansi_id']."',no_anggota='".$_REQUEST['no_anggota']."',st_anggota='".$_REQUEST['st_anggota']."',nama_peserta='".$_REQUEST['nama_peserta']."',tgl_act=now(),kelas_id='".$_REQUEST['hak_kelas']."' WHERE id='".$_REQUEST['id']."'";	
				$rs=mysql_query($sqlSimpan);
				//nama_pasien=(AN) M FAUZI WAHYU S&gender=L&tgllahir=21-07-2003&alamat=JL BRIGJEN KATAMSO , BANCILAN&rt=17&rw=3&prop=1&kab=1182&kec=1245&desa=1258&ortu=-&suami=1&pendidikan=1&telp=
				$datapas=mysql_fetch_array(mysql_query("select pasien_id from b_ms_kso_pasien where id='".$_REQUEST['id']."'"));
				$sqlSimpanPas= "update b_ms_pasien set nama='".$_REQUEST['nama_pasien']."',sex='".$_REQUEST['gender']."',agama='".$_REQUEST['agama']."',tgl_lahir='".tglSQL($_REQUEST['tgllahir'])."',alamat='".$_REQUEST['alamat']."',rt='".$_REQUEST['rt']."',rw='".$_REQUEST['rw']."',desa_id='".$_REQUEST['desa']."',kec_id='".$_REQUEST['kec']."',kab_id='".$_REQUEST['kab']."',prop_id='".$_REQUEST['prop']."',telp='".$_REQUEST['telp']."',pendidikan_id='".$_REQUEST['pendidikan']."',pekerjaan_id='".$_REQUEST['pekerjaan']."',nama_ortu='".$_REQUEST['ortu']."',nama_suami_istri='".$_REQUEST['suami']."' where id='".$datapas['pasien_id']."'";
				$rs=mysql_query($sqlSimpanPas);
				break;
		}
		break;
		case '21':
		switch(strtolower($_REQUEST['act'])){
			case 'tambah':		
				$sqlTambah="insert into b_ms_instansi (kode,nama,alamat,kota,telp,aktif)
					values('".$_REQUEST['kode']."','".$_REQUEST['nama']."','".$_REQUEST['alamat']."','".$_REQUEST['kota']."','".$_REQUEST['telp']."','".$_REQUEST['aktif']."')";
				$rs=mysql_query($sqlTambah);
				break;
			case 'hapus':
				...
				break;
			case 'simpan':
				$sqlSimpan = "UPDATE b_ms_instansi SET kode='".$_REQUEST['kode']."',nama='".$_REQUEST['nama']."',alamat='".$_REQUEST['alamat']."',kota='".$_REQUEST['kota']."',telp='".$_REQUEST['telp']."',aktif='".$_REQUEST['aktif']."' WHERE id='".$_REQUEST['instansiId']."'";	
				$rs=mysql_query($sqlSimpan);
				break;
		}
		break;
...
switch($grd){
	case "1":
		$sql="SELECT id,kepemilikan_id,kode,nama,alamat,telp,fax,kontak FROM b_ms_kso ".$filter." order by ".$sorting;
		break;
	case "2":
...
WHERE b_ms_kso_pasien.kso_id='".$_REQUEST['ksoId']."') as gab ".$filter." order by ".$sorting;*/
		$sql="SELECT * FROM (SELECT kso_p.id, b_ms_pasien.no_rm, b_ms_pasien.nama, b_ms_pasien.alamat, 
			b_ms_pasien.nama_ortu, b_ms_pasien.nama_suami_istri, b_ms_pasien.sex, b_ms_pasien.agama,
			DATE_FORMAT(b_ms_pasien.tgl_lahir,'%d-%m-%Y') AS tgl_lahir, b_ms_pasien.alamat AS alamatpasien, b_ms_pasien.rt, b_ms_pasien.rw, b_ms_pasien.desa_id, b_ms_pasien.kec_id,
			b_ms_pasien.kab_id, b_ms_pasien.prop_id, b_ms_pasien.telp, b_ms_pasien.pekerjaan_id, b_ms_pasien.pendidikan_id,
			IF(b_ms_instansi.nama IS NULL,'-',b_ms_instansi.nama) AS instansi, kso_p.nama_peserta, ifnull(b_ms_instansi.id,0) AS insId,
			kso_p.no_anggota, kso_p.st_anggota,kso_p.kelas_id
		...
		break;
	case "21":
		$sql="select * from (SELECT id, kode, nama, alamat, kota, telp, if(aktif=1,'Aktif','Tdk Aktif') as aktif, b_ms_instansi.aktif as cmbAktif FROM b_ms_instansi) as gab ".$filter." order by ".$sorting;
		break;
}
...
switch($grd){
	...
	{
		$i++;
		$dt.=$rows["id"].chr(3).$rows["kode"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["telp"].chr(3).$rows["fax"].chr(3).$rows["kontak"].chr(6);
	}
	break;
	...
	while ($rows=mysql_fetch_array($rs))
	{
		$sisip = $rows['id']."|".$rows['nama_ortu']."|".$rows['nama_suami_istri']."|".$rows['sex']."|".$rows['agama']."|".$rows['tgl_lahir']."|".$rows['alamatpasien']."|".$rows['rt']."|".$rows['rw']."|".$rows['desa_id']."|".$rows['kec_id']."|".$rows['kab_id'].'|'.$rows['prop_id'].'|'.$rows['telp'].'|'.$rows['pendidikan_id'].'|'.$rows['pekerjaan_id'].'|'.$rows['kelas_id'].'|'.$rows['insId'];
		$i++;
		$dt.=$sisip.chr(3).$i.chr(3).$rows["no_rm"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["instansi"].chr(3).$rows["nama_peserta"].chr(3).$rows["no_anggota"].chr(3).$rows["st_anggota"].chr(6);
	}
	case "21":
	while ($rows=mysql_fetch_array($rs))
	{
		$sisip2 = $rows['id']."|".$rows['cmbAktif'];
		$i++;
		$dt.=$sisip2.chr(3).$i.chr(3).$rows["kode"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["kota"].chr(3).$rows["telp"].chr(3).$rows["aktif"].chr(6);
	}
}
