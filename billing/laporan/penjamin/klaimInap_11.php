		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size:11px; font-family:Arial, Helvetica, sans-serif">
				...
				<td width="80%" class="jdl">			 
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						<tr style="text-align:center; font-weight:bold;">
							<td width="6%" class="right">Diagnosa</td>
							...
							<td width="4%" class="right">Ends</td>
			...
			<?php
				/* $igd = $kamar = $makan = $tindakanNO = $visite = $gizi = $tindakanO = $lab = $rad = $pa = $jmlTindakan = $obat
				  ...
				  if($stsPas!=0) $fKso = "AND b_kunjungan.kso_id = '".$stsPas."'";
				  $qry = "SELECT b_kunjungan.id AS kunjungan_id, b_pelayanan.id AS pelayanan_id,
						...
						WHERE b_pelayanan.unit_id = '".$tmpLay."' $waktu $fKso GROUP BY b_pelayanan.id ORDER BY b_tindakan_kamar.tgl_out";
				  $rs = mysql_query($qry);
			...
			<tr valign="top">
				...
				<?php
						$qLop = "SELECT DATE_FORMAT(b_pelayanan.tgl,'%d-%m-%Y') AS masuk,
								DATE_FORMAT(b_pelayanan.tgl_krs,'%d-%m-%Y') AS keluar, SUM(DATEDIFF(b_tindakan_kamar.tgl_out,b_tindakan_kamar.tgl_in)) AS jHr,
								...
								WHERE b_pelayanan.kunjungan_id = '".$rw['kunjungan_id']."' GROUP BY b_pelayanan.id";
						$rsLop = mysql_query($qLop);
						while($rwLop = mysql_fetch_array($rsLop))
						{							 
							 $qKamar = "SELECT SUM(t.biaya) AS jml FROM (SELECT b_tindakan_kamar.id, DATEDIFF(b_tindakan_kamar.tgl_out,b_tindakan_kamar.tgl_in)*b_tindakan_kamar.beban_kso AS biaya FROM b_pelayanan INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id WHERE b_pelayanan.id='".$rwLop['pelayanan_id']."' GROUP BY b_tindakan_kamar.id) AS t";
							 $rsKamar = mysql_query($qKamar);
							 $rwKamar = mysql_fetch_array($rsKamar);
							 
							 $qMakan = "SELECT SUM(b_tindakan.qty*b_tindakan.biaya_kso) AS jml FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan_kelas.ms_tindakan_id = b_ms_tindakan.id WHERE b_pelayanan.id='".$rwLop['pelayanan_id']."' AND (b_ms_tindakan.id = '742'   OR b_ms_tindakan.id = '746' OR b_ms_tindakan.id = '747' OR b_ms_tindakan.id = '748' OR b_ms_tindakan.id = '749')";
							 $rsMakan = mysql_query($qMakan);
							 $rwMakan = mysql_fetch_array($rsMakan);
							 
							 $qTin = "SELECT SUM(t.total) AS jml FROM (SELECT b_tindakan.id, b_tindakan.biaya_kso, b_tindakan.qty, b_ms_tindakan.nama, b_ms_tindakan.klasifikasi_id, b_tindakan.qty*b_tindakan.biaya_kso AS total, b_pelayanan.unit_id FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id WHERE b_pelayanan.id = '".$rwLop['pelayanan_id']."' AND (b_ms_tindakan.id <> '742' AND b_ms_tindakan.id <> '746' AND b_ms_tindakan.id <> '747' AND b_ms_tindakan.id <> '748' AND b_ms_tindakan.id <> '749' AND b_ms_tindakan.id <> '2387') AND (b_ms_tindakan.klasifikasi_id<>13 AND b_ms_tindakan.klasifikasi_id<>14)) AS t";
							 $rsTin = mysql_query($qTin);
							 $rwTin = mysql_fetch_array($rsTin);
							 
							 $qVisite = "SELECT SUM(t.biaya) AS jml FROM (SELECT SUM(b_tindakan.qty)*b_tindakan.biaya_kso AS biaya FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id WHERE (b_ms_tindakan.klasifikasi_id = '13' OR b_ms_tindakan.klasifikasi_id = '14' AND b_ms_tindakan.id<>'2378') AND b_pelayanan.id = '".$rwLop['pelayanan_id']."' GROUP BY b_tindakan.id) AS t";
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						<tr>
							<td width="6%" style="border-right:1px solid #000000; border-bottom:1px solid #000000; text-align:left; font-size:9px; padding-left:5px;">
							<?php								
							...
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><?php echo number_format($rwRad['jml'],0,",",".");?></td>
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><?php echo number_format($rwHd['jml'],0,",",".");?></td>
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><?php echo number_format($rwEnd['jml'],0,",",".");?></td>
				...
				<td width="80%" class="tbl">		 
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						<tr>
							...
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><span id="span_rad" style="text-align:right"><?php echo number_format($rad,0,",",".");?></span></td>
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><span id="span_rad" style="text-align:right"><?php echo number_format($hd,0,",",".");?></span></td>
							<td width="4%" style="text-align:right; padding-right:5px; border-bottom:1px solid #000000; border-right:1px solid #000000;"><span id="span_rad" style="text-align:right"><?php echo number_format($end,0,",",".");?></span></td>
