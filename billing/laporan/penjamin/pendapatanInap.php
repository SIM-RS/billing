...
<table align="left" width="1200" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td align="center" height="70" valign="top" style="font-weight:bold; font-size:14px; text-transform:uppercase">LAPORAN PENDAPATAN TEMPAT LAYANAN PASIEN INAP ( 
      <?php if($tmpLayanan=='0') echo 'SEMUA'; else echo $rwUnit2['nama']?> )<br/>
      <?php echo $Periode;?>&nbsp;</td>
  </tr>
  <tr>
    <td style="font-weight:bold; height:30px;">Penjamin Pasien&nbsp;:&nbsp;<?php echo $hsPenjamin['nama'];?></td>
  </tr>
  <tr>
    <td>
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<?php
				...
				$qUn = "SELECT b_ms_unit.id, b_ms_unit.nama FROM b_kunjungan
					...
					WHERE b_tindakan.kso_id = '".$stsPas."' $fUnit $waktu
					AND b_ms_unit.parent_id <> '76' GROUP BY b_ms_unit.nama ";
				//echo $qUn."<br>";
...
b_pelayanan.id AS pelayanan_id, b_pelayanan.unit_id_asal FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id 
INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id 
WHERE b_tindakan.kso_id = '".$stsPas."' AND b_pelayanan.unit_id = '".$rwUn['id']."' $waktu 
GROUP BY b_ms_pasien.nama ORDER BY b_pelayanan.tgl,b_ms_pasien.nama";
			//echo $Pas."<br>";
			...
			$sqlRet ="SELECT SUM(b_tindakan.biaya_kso) AS retribusi FROM b_tindakan INNER JOIN b_ms_tindakan_kelas 
...
WHERE b_ms_tindakan.klasifikasi_id = '11' AND b_tindakan.pelayanan_id = '".$rwPas['pelayanan_id']."'";
			//echo $sqlRet."<br>";
			...
			$sqlLabA = "SELECT IFNULL(SUM(b_tindakan.biaya_kso),0) AS PA FROM b_tindakan INNER JOIN b_pelayanan ON b_tindakan.pelayanan_id = b_pelayanan.id 
WHERE b_pelayanan.unit_id = '59' AND b_pelayanan.unit_id_asal = '".$rwUn['id']."' AND b_tindakan.kunjungan_id = '".$rwPas['kunjungan_id']."'";
			//echo $sqlLabA."<br>";
			$rsLabA = mysql_query($sqlLabA);
			$rowLabA = mysql_fetch_array($rsLabA);
			
			$sqlLabK = "SELECT IFNULL(SUM(b_tindakan.biaya_kso),0) AS PK FROM b_tindakan INNER JOIN b_pelayanan ON b_tindakan.pelayanan_id = b_pelayanan.id 
WHERE b_pelayanan.unit_id = '58' AND b_pelayanan.unit_id_asal = '".$rwUn['id']."' AND b_tindakan.kunjungan_id = '".$rwPas['kunjungan_id']."'";
			//echo $sqlLabA."<br>";
...
WHERE b_pelayanan.unit_id = '61' AND b_pelayanan.unit_id_asal = '".$rwUn['id']."' AND b_tindakan.kunjungan_id = '".$rwPas['kunjungan_id']."'";
			...
			$sqlHD = "SELECT IFNULL(SUM(b_tindakan.biaya_kso),0) AS hd FROM b_tindakan INNER JOIN b_pelayanan ON b_tindakan.pelayanan_id = b_pelayanan.id 
WHERE b_pelayanan.unit_id = '65' AND b_pelayanan.unit_id_asal = '".$rwUn['id']."' AND b_tindakan.kunjungan_id = '".$rwPas['kunjungan_id']."'";
		...
		$rowEnd = mysql_fetch_array($rsEnd);
			...
			$sqlKon = "SELECT SUM(b_tindakan.biaya_kso) AS konsul
			FROM b_pelayanan
INNER JOIN b_tindakan ON b_pelayanan.id = b_tindakan.pelayanan_id
			INNER JOIN b_ms_tindakan_kelas 
ON b_tindakan.ms_tindakan_kelas_id = b_ms_tindakan_kelas.id
INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id  
WHERE b_ms_tindakan.id = '2363' AND b_tindakan.kunjungan_id = '".$rwPas['kunjungan_id']."'
AND b_pelayanan.unit_id_asal = '".$rwUn['id']."'";
			...
			<td align="right"><?php echo number_format($rowEnd['end'],0,",",".")?>&nbsp;</td>
				<td align="right"><?php echo number_format($rowKon['konsul'],0,",",".")?>&nbsp;</td>
				<td align="right">0&nbsp;</td>
				<?php
					$biayaTotal = $rowRet['retribusi']+$rowTin['tindakan']+$rowLabA['PA']+$rowLabK['PK']+$rowRad['rad']+$rowHD['hd']+$rowOk['ok']+$rowEnd['end']+$rowKon['konsul'];
				?>
