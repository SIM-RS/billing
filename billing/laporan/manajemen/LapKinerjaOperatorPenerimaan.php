...
<title>Kinerja Operator Penerimaan Retribusi Pemeriksaan</title>
</head>
...
<table width="750" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
    <td colspan="4" align="center" height="50" style="font-size:15px;"><b>Laporan Kinerja Operator Penerimaan Retribusi Pemeriksaan<br />Jenis Layanan <?php echo $rwUnit1['nama']?>  Jenis Layanan <?php if($_REQUEST['TmpLayanan']=='0') echo 'Semua'; else echo $rwUnit2['nama']?> Status Pasien <?php if($_REQUEST['StatusPas']=='0') echo 'Semua'; else echo $rwKso['nama']?><br/>
  ...
  <?php
  		$sql = "SELECT mp.id,mp.nama,COUNT(t.id) jml FROM b_tindakan t INNER JOIN b_ms_tindakan_kelas mtk ON t.ms_tindakan_kelas_id=mtk.id INNER JOIN b_ms_tindakan mt ON mtk.ms_tindakan_id=mt.id INNER JOIN b_ms_pegawai mp ON t.user_act=mp.id INNER JOIN b_ms_unit mu ON t.ms_tindakan_unit_id=mu.id WHERE t.tgl BETWEEN '2010-10-01' AND '2010-10-29' AND mt.klasifikasi_id=11  $fKso $fUnit GROUP BY t.user_act ORDER BY mp.nama";
		$rs = mysql_query($sql);
