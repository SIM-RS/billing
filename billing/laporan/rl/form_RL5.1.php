...
<table width="700" border="0" style="border-collapse:collapse; font:12px arial">
  <tr>
    <td><table width="100%" border="0" style="border-collapse:collapse">
      <tr>
        <td width="9%" rowspan="3" style="border-bottom:2px solid #000000"><center><img src="logo-bakti-husada.jpg" width="49" height="60" /></center></td>
        <td width="55%" height="18"><span class="style1">Formulir RL 5.1</span></td>
        <td width="36%" rowspan="3" style="border-bottom:2px solid #000000"><img src="pojok.png" /></td>
      </tr>
      <tr>
        <td height="20" ><span class="style1">PENGUNJUNG RUMAH SAKIT </span></td>
      </tr>
      <tr>
        <td height="20" style="border-bottom:2px solid #000000"><span class="style1"><?php echo $txtJenis.' '.$txtTempat; ?></span></td>
        </tr>
    </table></td>
  </tr>
  <tr>
  ...
  <tr>
    <td><table width="60%" border="1" style="border-collapse:collapse">
      ...
      <?php
	    ...
	    $baru = "SELECT COUNT(DISTINCT b_pelayanan.pasien_id) AS jml 
		FROM b_kunjungan 
		...
		WHERE b_kunjungan.isbaru = 1 $fUnit $waktu  "; //echo $baru;
		$hasilB = mysql_query($baru);
...
<?php					    
		$lama = "SELECT COUNT(DISTINCT b_pelayanan.pasien_id) AS jml 
		FROM b_kunjungan 
		INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id
		INNER JOIN b_ms_unit ON b_pelayanan.unit_id = b_ms_unit.id
		WHERE b_kunjungan.isbaru <> 1 $fUnit $waktu ";// echo $lama;
		$hasilL = mysql_query($lama);
		$dataL = mysql_fetch_array($hasilL);
	  ?>
      <tr>
        <td><div align="center"><strong>2</strong></div></td>
