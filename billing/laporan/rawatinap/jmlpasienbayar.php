...
<title>.: Jumlah Pasien Menurut Cara Pembayaran :.</title>
</head>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			<?php
					...
					{
						$qTot = "SELECT COUNT(b_pelayanan.id) AS jml FROM b_pelayanan INNER JOIN b_ms_kso ON b_ms_kso.id=b_pelayanan.kso_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id WHERE b_pelayanan.unit_id='$tempat' $waktu";
						$sTot = mysql_query($qTot);
						$wTot = mysql_fetch_array($sTot);
						
						$qJml = "SELECT COUNT(b_pelayanan.id) AS jml FROM b_pelayanan INNER JOIN b_ms_kso ON b_ms_kso.id=b_pelayanan.kso_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id WHERE b_pelayanan.unit_id='$tempat' AND b_pelayanan.kso_id='".$rw['id']."' $waktu";
						$sJml = mysql_query($qJml);
						$wJml = mysql_fetch_array($sJml);
			?>
			<tr>
			  <td align="center" class="isi"><?php echo $no;?></td>
				<td class="isi" style="padding-left:10px; text-transform:uppercase;" height="25"><?php echo $rw['nama']?></td>
