...
<table width="700" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td height="100" valign="top" style="font-size:14px; text-transform:uppercase; font-weight:bold; text-align:center;">laporan instalasi rawat inap<br />asal pasien<br />ruang <?php echo $ruang['nama'];?><br /><?php echo $Periode;?></td>
  </tr>
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			<?php
					$sql = "SELECT b_ms_asal_rujukan.id, b_ms_asal_rujukan.nama, COUNT(b_pelayanan.id) AS jml FROM b_ms_asal_rujukan INNER JOIN b_kunjungan ON b_kunjungan.asal_kunjungan=b_ms_asal_rujukan.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id WHERE b_pelayanan.unit_id='$tempat' $waktu GROUP BY b_ms_asal_rujukan.id";
					$rs = mysql_query($sql);
					$no=1;
					$tot = 0;
					while($rw = mysql_fetch_array($rs))
					{
						...
						$qJml = "SELECT COUNT(b_pelayanan.id) AS jml FROM b_ms_asal_rujukan INNER JOIN b_kunjungan ON b_kunjungan.asal_kunjungan=b_ms_asal_rujukan.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id WHERE b_pelayanan.unit_id='$tempat' $waktu AND b_ms_asal_rujukan.id='".$rw['id']."'
GROUP BY b_ms_asal_rujukan.id";
			...
			<tr>
			  <td align="center" class="isi"><?php echo $no;?></td>
				...
				<td class="isiKn" style="text-align:right; padding-right:60px;"><?php echo number_format($wJml['jml']/$wTot['jml']*100,2,",",".")?>&nbsp;%</td>
			</tr>
			<?php
					$no++;
					$tot = $tot + $wJml['jml'];
