...
<title>.: Data Pola Kematian :.</title>
</head>
...
<table width="700" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td height="100" valign="top" style="font-size:14px; text-transform:uppercase; font-weight:bold; text-align:center;">laporan instalasi rawat inap<br />data pola kematian <?php if($stsPas!=0) echo "pasien ".$wK['nama']; else echo "";?><br />ruang <?php echo $ruang['nama'];?><br /><?php echo $Periode;?></td>
  </tr>
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			<tr>
			  <td width="5%" class="jdl">no</td>
				<td width="50%" height="30" class="jdl">diagnosis</td>
				<td width="20%" class="jdl">&sum;</td>
				<td width="25%" class="jdlKn">Prosentase (%)</td>
			</tr>
			<?php
					$sql = "SELECT t.id, t.nama, t.jml FROM (SELECT b_ms_diagnosa.id, b_ms_diagnosa.nama, COUNT(b_pelayanan.id) AS jml FROM b_pelayanan INNER JOIN b_diagnosa_rm ON b_diagnosa_rm.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_diagnosa ON b_ms_diagnosa.id=b_diagnosa_rm.ms_diagnosa_id INNER JOIN b_pasien_keluar ON b_pasien_keluar.pelayanan_id=b_pelayanan.id WHERE b_pelayanan.unit_id='$tempat' AND b_pasien_keluar.cara_keluar LIKE '%meninggal' $waktu $fKso GROUP BY b_ms_diagnosa.id ) AS t ORDER BY t.jml DESC LIMIT 10";
					$rs = mysql_query($sql);
					$no=1;
					$tot = 0;
					while($rw = mysql_fetch_array($rs))
					{
						$qTot = "SELECT COUNT(b_diagnosa_rm.pelayanan_id) AS jml FROM b_pelayanan INNER JOIN b_diagnosa_rm ON b_diagnosa_rm.pelayanan_id=b_pelayanan.id INNER JOIN b_pasien_keluar ON b_pasien_keluar.pelayanan_id=b_pelayanan.id WHERE b_pelayanan.unit_id='$tempat' AND b_pasien_keluar.cara_keluar LIKE '%meninggal' $fKso $waktu";
