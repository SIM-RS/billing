...
if($_POST['isExcel']=='yes'){
	...
	header('Content-Disposition: attachment; filename="CaraKeluarTmptLay.xls"');
}
...
<table width="800" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="3" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase"><b>Laporan Cara Keluar Pasien Berdasarkan Tempat Layanan<br />Status Pasien <?php if($stsPas==0) echo "SEMUA"; else echo $rwKso['nama'];?><br /><?php echo $Periode;?></b></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
				...
				<?php
					...
					while($rwck=mysql_fetch_array($rsck))
					{
						$qJml = "SELECT COUNT(b_pelayanan.pasien_id) AS jml
									...
									INNER JOIN b_pelayanan ON b_pelayanan.id = b_pasien_keluar.pelayanan_id
									INNER JOIN b_kunjungan ON b_kunjungan.id = b_pelayanan.kunjungan_id
									WHERE b_pelayanan.unit_id = '".$rwSt['id']."' $fKso
