...
<table width="700" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
	...
	<tr>
		<td height="30">&nbsp;<b>Asal Tempat Layanan Rawat Inap</b></td>
	</tr>
	<tr>
		<td>
...
WHERE b_ms_unit.inap = '1' AND b_pelayanan.unit_id = '".$tmpLay."' $waktu AND (b_ms_unit.id<>'72' AND b_ms_unit.id<>'112')
...
INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id_asal
WHERE b_pelayanan.unit_id = '".$tmpLay."' $waktu AND b_ms_unit.inap = '1' $fKso
...
ORDER BY b_ms_kso.nama";
	...
	GROUP BY b_pelayanan.pasien_id) AS t";
					...
					?>
					<td style="border-bottom:1px solid #000000; border-right:1px solid #000000; text-align:center" width="20"><?php if($rwQ['px']=="") echo 0; else echo $rwQ['px']; ?></td>
					<td style="border-bottom:1px solid #000000; border-right:1px solid #000000; text-align:right" width="50"><?php if($rwQ['rp']=="") echo 0; else echo number_format($rwQ['rp'],0,",",".");?>&nbsp;</td>
					<?php
							} mysql_free_result($rs);
							$totalpx += $totpx;
							$totalrp += $totrp;
					?>
					<td style="border-bottom:1px solid #000000; border-right:1px solid #000000; text-align:center" width="20"><?php echo $totpx;?></td>
					<td style="border-bottom:1px solid #000000; border-right:1px solid #000000; text-align:right" width="50"><?php echo number_format($totrp,0,",",".");?>&nbsp;</td>
				</tr>
				<?php
						$no++;
