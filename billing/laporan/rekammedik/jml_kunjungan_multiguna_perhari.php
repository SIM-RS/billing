<?php
	if($_POST['isExcel']=='yes'){
		...
		header('Content-Disposition: attachment; filename="Jumlah Kunjungan Pasien per-Hari.xls"');
	}
...
return $hasil;
		  ...
		  /*echo $sId="SELECT spesialisasi_id, spesialisasi, COUNT(*) jmlId
				FROM b_ms_pegawai
				WHERE spesialisasi_id <> 0 AND spesialisasi <> '' and aktif = 1
				GROUP BY spesialisasi_id
				ORDER BY spesialisasi";
		  $qId=mysql_query($sId);*/
		 $sId = "SELECT 
  b_ms_pegawai.spesialisasi_id,
  b_ms_reference.nama,
  COUNT(b_ms_pegawai.id) jmlId 
FROM
  b_ms_pegawai
  INNER JOIN b_ms_reference ON b_ms_pegawai.spesialisasi_id = b_ms_reference.id
WHERE spesialisasi_id <> 0 
  AND b_ms_pegawai.aktif = 1 
  AND b_ms_reference.id NOT IN (42,55,56)
GROUP BY spesialisasi_id 
ORDER BY b_ms_reference.nama";
		  $qId=mysql_query($sId);
		  $n=1;
		...
		<tr>
		  ...
		  <td colspan="<?=$jml+3?>">&nbsp;<b><?=$rwId['nama']?></b></td>		  
		</tr>
		  
		  <?php
		    $sDokter="SELECT nama, id
					FROM b_ms_pegawai
					WHERE spesialisasi_id = '".$rwId['spesialisasi_id']."' and aktif = 1";
			//echo $sDokter."<br>";
...
//echo $que1;
  ...
  $queIsi = $que['jml'];
	...
	$tot = $quew['jml'];
				  ?>
				<td><?=$tot; $totBN[$i]+=$tot;?></td>
			  <?php
			  $totLah += $tot;
			   }?>
			  <td><?=$totLah; $gSTotB+=$totLah;?></td>
			  <td><?=number_format($totLah/$tglb,2); $gSTotB1+=number_format($totLah/$tglb,2);?></td>
			  </tr>	
		  <?php 
		    }
		  ?>			
		<tr>
		  ...
		  <?php for($i=$tgla;$i<=$tglb;$i++){?>
		    <td>&nbsp;<?=$totBN[$i];$totBN[$i]=0;?></td>
		  <?php }?>
		  <td>&nbsp;<?=$gSTotB; $gSTotB=0;?></td>
		  <td>&nbsp;<?=$gSTotB1; $gSTotB1=0;?></td>
		</tr>
		<?php
		   $n++;
		  }
  ...
  <tr id="trTombol">
        <td align="center" colspan="2" class="noline">
            <input type="button" onClick="cetak(document.getElementById('trTombol'));" value="Print/Cetak" id="btnPrint">
            <input type="button" onClick="window.close();" value="Tutup" id="btnTutup">        </td>
    </tr>
</table>
