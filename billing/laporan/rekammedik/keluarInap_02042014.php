				...
				<div id="lama">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						...
						<?php
								...
								$qK = "SELECT b_ms_kso.id, b_ms_kso.nama, b_tindakan_kamar.tgl_out FROM b_kunjungan  INNER JOIN b_pelayanan ON b_kunjungan.id = b_pelayanan.kunjungan_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_kso ON b_ms_kso.id = b_pelayanan.kso_id LEFT JOIN b_pasien_keluar ON b_pasien_keluar.pelayanan_id = b_pelayanan.id WHERE $fUnit $waktu $fCara $fKeadaanKeluar $fKso
GROUP BY b_ms_kso.id ORDER BY b_ms_kso.nama";
						...
						<?php
								...
								b_kunjungan.umur_bln, b_kunjungan.umur_thn, 
								IF(b_kunjungan.umur_thn=0 AND b_kunjungan.umur_bln=0,
  								CONCAT(DATEDIFF(b_kunjungan.tgl,b_ms_pasien.tgl_lahir),' hr'),
  								CONCAT(b_kunjungan.umur_thn,'thn ',b_kunjungan.umur_bln,' bln')
  								) AS usia,
								...
								WHERE $fUnit AND b_pelayanan.kso_id = '".$rwK['id']."' $fCara $fKeadaanKeluar $fKso $waktu GROUP BY b_pelayanan.id ";
								//echo $sql."<br><br>";
								$rs = mysql_query($sql);
								$no = 1;
		...
		WHERE d.pelayanan_id='".$rw["pelayanan_id"]."'";
						...
						<tr valign="top">
							...
							<td style="text-align:center"><?php $rw['cara_keluar']."/".$rw['keadaan_keluar'];  echo $rw['keadaan_keluar']; ?></td>
				...
				<div id="pindahan">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
					    ...
					    <td style="text-align:center"><?php echo $no;?></td>
							...
							<td style="text-align:center"><?php echo $rw['kelas'];?></td>
                            <?php
							$sMRS="SELECT DATE_FORMAT(tgl,'%d-%m-%Y') mrs FROM b_pelayanan WHERE kunjungan_id='".$rw['kunjungan_id']."' AND jenis_kunjungan=3 ORDER BY tgl ASC LIMIT 1";
