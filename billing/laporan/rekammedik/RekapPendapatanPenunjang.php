...
if($_POST['isExcel']=='yes'){
	...
	header('Content-Disposition: attachment; filename="RekapPendapatanPenunjang.xls"');
}
...
<?php
	...
	if($asal==0){
		$jns = "(b_pelayanan.jenis_kunjungan <> '3' OR b_ms_unit.id IN (72,112,47))";
	}else{
	...
	if($unit==1){ /*poli*/
		...
		$fUnit = "AND b_ms_unit.parent_id<>'50' AND b_ms_unit.parent_id<>'1'";
	}else{ /*semua*/
		$fUnit = "AND b_ms_unit.parent_id<>'50'";
	}
					...
					<?php
							if($asal=='1'){
								$sql = "SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.parent_id FROM b_pelayanan 
									...
									WHERE $jns AND b_pelayanan.unit_id = '".$tmpLay."' 
									AND b_ms_unit.id NOT IN (47,72,112) $fKso $waktu 
