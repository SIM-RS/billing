...
if($_POST['isExcel']=='yes'){
	...
	header('Content-Disposition: attachment; filename="10DiagnosisTerbanyak.xls"');
}
...
<head>
	...
	<title>10 Diagnosis Terbanyak Pasien Meninggal</title>
</head>
...
<?php
	...
	if($waktu == 'Harian'){
		...
		$waktu = " and ((b_diagnosa_rm.tgl = '$tglAwal2') or (b_diagnosa_rm.tgl_act = '$tglAwal2')) ";
		$Periode = "Tanggal : ".$tglAwal[0]." ".$arrBln[$tglAwal[1]]." ".$tglAwal[2];
	}else if($waktu == 'Bulanan'){
		$bln = $_POST['cmbBln'];
		$thn = $_POST['cmbThn'];
		$waktu = " and ((month(b_diagnosa_rm.tgl) = '$bln' and year(b_diagnosa_rm.tgl) = '$thn') or (month(b_diagnosa_rm.tgl_act) = '$bln' and year(b_diagnosa_rm.tgl_act) = '$thn')) ";
		...
		$waktu = " and ((b_diagnosa_rm.tgl between '$tglAwal2' and '$tglAkhir2') OR (b_diagnosa_rm.tgl_act between '$tglAwal2' and '$tglAkhir2')) ";
	
	$Periode = "Periode : ".$tglAwal[0]." ".$arrBln[$tglAwal[1]]." ".$tglAwal[2].' s/d '.$tglAkhir[0]." ".$arrBln[$tglAkhir[1]]." ".$tglAkhir[2];
	}

	$sqlUnit1 = "SELECT id,nama FROM b_ms_unit WHERE id = '".$_REQUEST['JnsLayanan15']."'";
	$rsUnit1 = mysql_query($sqlUnit1);
	$rwUnit1 = mysql_fetch_array($rsUnit1);
	
	$sqlUnit2 = "SELECT id,nama FROM b_ms_unit WHERE id = '".$_REQUEST['TmpLayanan15']."'";
...
<table width="800" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
	...
	<tr>
		<td colspan="2" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase"><b>Laporan 10 Diagnosis Terbanyak Pasien meninggal <?php echo $rwUnit1['nama'] ?> - <?php if($rwUnit2['nama']=="") echo 'Semua'; else echo $rwUnit2['nama'] ?><br />
		...
		<td colspan="2">
			<table width="100%"  border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				...
				<?php
					...
					if($_REQUEST['TmpLayanan15']==0){
						$fUnit = " b_pelayanan.jenis_layanan = '".$_REQUEST['JnsLayanan15']."'";
					}else{
						$fUnit = " b_pelayanan.unit_id = '".$_REQUEST['TmpLayanan15']."' ";
					}
				
				$sqlDiag = "SELECT b_diagnosa_rm.diagnosa_id, b_ms_diagnosa.nama, b_ms_diagnosa.kode, COUNT(b_diagnosa_rm.pelayanan_id) AS jml 
					FROM b_ms_diagnosa INNER JOIN b_diagnosa_rm ON b_ms_diagnosa.id = b_diagnosa_rm.ms_diagnosa_id 
					...
					WHERE $fUnit $fKso $waktu AND b_pasien_keluar.cara_keluar LIKE '%Meninggal' AND b_diagnosa_rm.primer=1 
					GROUP BY b_ms_diagnosa.id ORDER BY jml DESC, nama LIMIT 10";
