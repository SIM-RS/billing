...
if($_REQUEST['TmpLayanan']!=0){
	...
	$fUnit = " AND u.id=".$_REQUEST['TmpLayanan'];
}else
...
<table width="800" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="7" align="center" valign="top" height="70" style="font-size:14px; text-transform:uppercase"><b>Laporan Pengiriman Konsul <?php echo $rwTmpLay['nama']."<br/>".$Periode;?></b></td>
  </tr>
  <tr height="30">
  ...
  <tr height="30">
    ...
    <td width="20%" align="center" style="border-top:1px solid; border-bottom:1px solid; font-weight:bold;">&nbsp;Tujuan Tempat Layanan</td>
    ...
    <td width="12%" align="center" style="border-top:1px solid; border-bottom:1px solid; font-weight:bold;">&nbsp;Umur</td>
  </tr>
  <?php
  		$qUnit = "SELECT u.id, u.nama FROM b_kunjungan 
			...
			WHERE $waktu $fUnit AND u.parent_id <> '76'
  ...
  <?php
  $qPas = "SELECT DATE_FORMAT(b_pelayanan.tgl_act,'%d-%m-%Y %H:%i:%s') tgl,b_ms_unit.nama,b_ms_pasien.no_rm,b_ms_pasien.nama nmPasien,b_ms_pasien.sex,b_kunjungan.umur_thn,b_kunjungan.umur_bln,b_kunjungan.umur_hr FROM b_kunjungan INNER JOIN b_pelayanan ON b_kunjungan.id=b_pelayanan.kunjungan_id INNER JOIN b_ms_pasien ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_ms_unit ON b_pelayanan.unit_id=b_ms_unit.id WHERE $waktu AND b_pelayanan.unit_id_asal = '".$rwUnit['id']."' $fKso AND b_ms_unit.inap=0 ORDER BY b_ms_unit.nama,b_pelayanan.tgl_act";
  ...
  <tr>
	...
	<td style="text-transform:uppercase">&nbsp;<?php echo $rwPas['nmPasien']?></td>
	<td align="center"><?php echo $aSex[$rwPas['sex']]?>&nbsp;</td>
