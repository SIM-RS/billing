...
<table width="1000" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td><?php if($_REQUEST['cmbJenisLayanan']==0){?>
	<div id="jalan">
	  ...
	  <?php
			...
			while($rw = mysql_fetch_array($rs))
			{
				...
				while($rw2 = mysql_fetch_array($rs2))
				{
					...
					else if($rw2['umur_thn']<1 && $rw2['isbaru']==1)
					{
						$u2 = $u2 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
					...
					else if($rw2['umur_thn']>=1 && $rw2['umur_thn']<=4 && $rw2['isbaru']==1)
					{
						$u3 = $u3 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
					...
					else if($rw2['umur_thn']>=5 && $rw2['umur_thn']<=14 && $rw2['isbaru']==1)
					{
						$u4 = $u4 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
					...
					else if($rw2['umur_thn']>=15 && $rw2['umur_thn']<=24 && $rw2['isbaru']==1)
					{
						$u5 = $u5 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
					...
					else if($rw2['umur_thn']>=25 && $rw2['umur_thn']<=44 && $rw2['isbaru']==1)
					{
						$u6 = $u6 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
					...
					else if($rw2['umur_thn']>=45 && $rw2['umur_thn']<=64 && $rw2['isbaru']==1)
					{
						$u7 = $u7 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
					...
					else if($rw2['umur_thn']>=65 && $rw2['isbaru']==1)
					{
						$u8 = $u8 + $rw2['jml'];
						if($rw2['jml'] == "")
						{
				...
				$qSex = "SELECT COUNT(b_ms_pasien.id) AS jml, b_kunjungan.umur_bln, b_kunjungan.umur_thn, b_kunjungan.isbaru, b_ms_pasien.sex, b_ms_diagnosa.id, b_ms_diagnosa.nama FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id INNER JOIN b_diagnosa_rm ON b_diagnosa_rm.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_diagnosa ON b_ms_diagnosa.id = b_diagnosa_rm.ms_diagnosa_id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fUnit $fKso $waktu AND b_kunjungan.isbaru = '1' AND b_ms_diagnosa.id = '".$rw['id']."' GROUP BY b_ms_pasien.sex";
					...
					{				
						...
						else if($rwSex['sex']=='P' && $rwSex['isbaru']==1)
						{
							$p = $p + $rwSex['jml'];
							if($rwSex['jml'] == "")
							{
					...
					$qBaru = "SELECT COUNT(b_ms_pasien.id) AS jml, b_kunjungan.umur_bln, b_kunjungan.umur_thn, b_kunjungan.isbaru, b_ms_pasien.sex, b_ms_diagnosa.id, b_ms_diagnosa.nama FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id INNER JOIN b_diagnosa_rm ON b_diagnosa_rm.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_diagnosa ON b_ms_diagnosa.id = b_diagnosa_rm.ms_diagnosa_id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fUnit $fKso $waktu AND b_ms_diagnosa.id = '".$rw['id']."' AND b_kunjungan.isbaru = '1' GROUP BY b_kunjungan.isbaru";
	  ...
	  <?php
			...
			while($rw = mysql_fetch_array($rs))
			{
				...
				$qSex = "SELECT COUNT(b_ms_pasien.id) AS jml, b_kunjungan.umur_bln, b_kunjungan.umur_thn, b_kunjungan.isbaru, b_ms_pasien.sex, b_ms_diagnosa.id, b_ms_diagnosa.nama FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id INNER JOIN b_diagnosa_rm ON b_diagnosa_rm.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_diagnosa ON b_ms_diagnosa.id = b_diagnosa_rm.ms_diagnosa_id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fUnit $fKso $waktu AND b_kunjungan.isbaru = '1' AND b_ms_diagnosa.id = '".$rw['id']."' GROUP BY b_ms_pasien.sex";
					...
					$qKeluar = "SELECT COUNT(b_ms_pasien.id) AS jml FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id INNER JOIN b_diagnosa_rm ON b_diagnosa_rm.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_diagnosa ON b_ms_diagnosa.id = b_diagnosa_rm.ms_diagnosa_id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_pasien_keluar ON b_pasien_keluar.pelayanan_id = b_pelayanan.id WHERE $fUnit $fKso $waktu2 AND b_ms_diagnosa.id = '".$rw['id']."'";
