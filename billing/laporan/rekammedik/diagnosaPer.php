...
<table width="800" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
   ...
   <?php
		...
		$sqlDiag = "SELECT 
					  t1.kode_rm kode,
					  GROUP_CONCAT(t1.nama SEPARATOR '<br/>- ') nama,
					  SUM(jml) jml 
					FROM
					  (SELECT 
					  ...
					  WHERE b_pelayanan.unit_id = '".$wU['id']."' 
						$fKso $fDiag $waktu 
						AND dr.primer = 1 
  ...
  ?>
  <tr height="20">
  	<td style="font-size:11px; padding-left:10px;" valign="top"><?php echo $no; ?></td>
	<td style="font-size:11px; padding-left:10px;" valign="top"><?php echo $rwDiag['kode']; ?></td>
	<td style="font-size:11px; text-transform:uppercase; padding-left:10px;" valign="top"><?php echo "- ".$rwDiag['nama']; ?></td>
	<td style="font-size:11px; text-align:right; padding-right:30px;" valign="top"><?php echo $rwDiag['jml']; ?></td>
  </tr>
  <?php
  		 $no++;
