...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
	...
	<tr>
		<td height="70" valign="top" style="font-size:14px; font-weight:bold; text-align:center; text-transform:uppercase;">rekapitulasi data pasien mASIH DIRAWAT- RAWAT INAP<br>tempat layanan - <?php echo $txtUnit; ?><br><?php echo $Periode;?></td>
	</tr>
	<tr>
				...
				<div id="lama">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						<tr style="font-weight:bold; text-align:center">
							...
							<td width="7%" style="border-top:1px solid; border-bottom:1px solid;">Kunjungan</td>
							...
							<td width="8%" style="border-top:1px solid; border-bottom:1px solid;">Umur</td>
							<td width="3%" style="border-top:1px solid; border-bottom:1px solid;">L/P</td>
							<td width="9%" style="border-top:1px solid; border-bottom:1px solid;">Diagnosa Masuk</td>
							<td width="9%" style="border-top:1px solid; border-bottom:1px solid;">Diagnosa Akhir</td>
							<td width="9%" style="border-top:1px solid; border-bottom:1px solid;">Dokter Yang Merawat</td>
							...
							<td width="6%" style="border-top:1px solid; border-bottom:1px solid;">Penjamin</td>
							<td width="7%" style="border-top:1px solid; border-bottom:1px solid;">Asal Kunjungan</td>
							<td width="11%" style="border-top:1px solid; border-bottom:1px solid;">DPJP</td>
						</tr>
						<?php
								if($stsKunj=='0'){ 
									...
									$qK = "SELECT b_ms_unit.id, b_ms_unit.nama FROM b_kunjungan INNER JOIN b_pelayanan ON b_kunjungan.id = b_pelayanan.kunjungan_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fUnit $fKso AND b_tindakan_kamar.tgl_out IS NULL GROUP BY b_ms_unit.id ORDER BY b_ms_unit.nama";
								}else if($stsKunj=='1'){
									/*$qK = "SELECT b_ms_kso.id, b_ms_kso.nama FROM b_kunjungan INNER JOIN b_pelayanan ON b_kunjungan.id = b_pelayanan.kunjungan_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_kso ON b_ms_kso.id = b_pelayanan.kso_id WHERE $fUnit $fKso $waktu GROUP BY b_ms_kso.id ORDER BY b_ms_kso.nama";*/
						...
						<?php
								if($stsKunj=='0'){ 
									...
									INNER JOIN b_ms_kso as mu ON mu.id = b_pelayanan.kso_id 
									...
									LEFT JOIN b_ms_pegawai mp ON mp.id = b_pelayanan.dokter_tujuan_id
									WHERE b_pelayanan.unit_id = '".$rwK['id']."' AND b_ms_unit.inap <> '1' $fKso 
									AND b_tindakan_kamar.tgl_out IS NULL AND b_pelayanan.dilayani = 1
									...
									INNER JOIN b_ms_unit ON b_ms_unit.id = b_tindakan_kamar.unit_id_asal
									LEFT JOIN b_ms_pegawai mp ON mp.id = b_pelayanan.dokter_tujuan_id
									WHERE b_pelayanan.unit_id = '".$rwK['id']."' $fKso
									AND b_tindakan_kamar.tgl_out IS NULL AND b_pelayanan.dilayani = 1
									AND b_kunjungan.pulang = 0
									GROUP BY b_pelayanan.id";
								}
						...
						<tr>
							...
							*/
							...
							<td style="padding-left:5px; " valign="top"><?php echo $rw['dokterPJ'];?></td>
						</tr>
						<?php
								$no++;
								}
				...
				<div id="pindahan">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
						<tr style="font-weight:bold; text-align:center">
							...
							<td width="7%" style="border-top:1px solid; border-bottom:1px solid;">DPJP</td>
						</tr>
						<?php
								$qK = "SELECT b_ms_unit.id, b_ms_unit.nama FROM b_kunjungan INNER JOIN b_pelayanan ON b_kunjungan.id = b_pelayanan.kunjungan_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fUnit $fKso AND b_tindakan_kamar.tgl_out IS NULL GROUP BY b_ms_unit.id ORDER BY b_ms_unit.nama";
						...
						<?php
								...
								LEFT JOIN b_ms_pegawai mp ON mp.id = b_pelayanan.dokter_tujuan_id
								WHERE b_pelayanan.unit_id = '".$rwK['id']."' AND b_ms_unit.inap = '1' $fKso
								 AND b_tindakan_kamar.tgl_out IS NULL AND b_kunjungan.pulang = 0 AND b_pelayanan.dilayani = 1 
								GROUP BY b_pelayanan.id";
								//echo $sql."<br>";
								$rs = mysql_query($sql);
								$no = 1;
