...
if($_POST['isExcel']=='yes'){
	...
	header('Content-Disposition: attachment; filename="RekapTindakanPenunjang.xls"');
}
...
<table width="1000" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
    ...
    <tr>
        <td height="70" valign="top" style="text-align:center; font-size:14px; text-transform:uppercase; font-weight:bold;">rekapitulasi pemeriksaan <?php echo $rwUnit1['nama'];?> - <?php echo $rwUnit2['nama'];?><br>status pasien <?php echo $rwKso['nama'];?><br><?php echo $Periode;?></td>
    </tr>
	<tr>
		<td height="30" style="font-weight:bold; padding-left:10px; text-transform:uppercase">&nbsp;<b>Asal Tempat Layanan <?php if($asal==0) echo "RAWAT JALAN"; else echo "RAWAT INAP";?></b></td>
	</tr>
    <tr>
        <td>
		    ...
		    <?php
				...
				if($asal==1){
					//
					$qInap = "SELECT DISTINCT IF(b_ms_unit.parent_id=94,'IPIT',
						IF(b_ms_unit.parent_id=50,'PAV',b_ms_unit.nama)) AS nama,
						IF(b_ms_unit.parent_id=94,b_ms_unit.parent_id,
						IF(b_ms_unit.parent_id=50,b_ms_unit.parent_id,b_ms_unit.id)) AS id
						FROM b_kunjungan
						...
						WHERE $fUnit $waktu $fKso AND b_ms_unit.inap = '1'
						AND b_ms_unit.islast = '1' AND (b_ms_unit.id<>'72' 
						AND b_ms_unit.id<>'112')
						GROUP BY b_ms_unit.id";
						/*//
					$qInap = "SELECT b_ms_unit.id,b_ms_unit.nama
						FROM b_kunjungan
						INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id
						INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id
						INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id_asal
					...
					$qInap = "SELECT b_ms_unit.id, b_ms_unit.nama FROM b_kunjungan
						...
						INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id_asal
						INNER JOIN b_ms_unit u ON u.id = b_pelayanan.unit_id
						WHERE $fUnit $waktu $fKso AND (b_ms_unit.inap = '0' OR b_ms_unit.id = '72' 
						OR b_ms_unit.id = '112') 
		    ...
		    <td width="50" style="text-align:center; font-size:14px; border-bottom:1px solid; border-top:1px solid; border-right:1px solid;">&sum;</td>
                ...
                <?php
                        if($asal==1){
							$fAsal = "AND b_ms_unit.inap = '1'";
						}else{
							$fAsal = "AND b_ms_unit.inap = '0'";
						}
						$sql = "SELECT b_kunjungan.id AS kunjungan_id, 
                                ...
                                INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id_asal
                                WHERE $fUnit $fKso $waktu $fAsal GROUP BY b_ms_tindakan.id ORDER BY b_ms_tindakan.nama";
                        $rs = mysql_query($sql);
                        $no = 1;
                        $ttlTot = 0;
		...
		?>
			...
			while($wInap = mysql_fetch_array($sInap)){
				...
				if($wInap['id']==50 || $wInap['id']==94){
					$unitnya=" AND b_ms_unit.parent_id = '".$wInap['id']."' ";
				}
				else{
					$unitnya=" AND b_pelayanan.unit_id_asal = '".$wInap['id']."' ";
				}
			    $q1 = "SELECT COUNT(b_tindakan.pelayanan_id) AS jml
				    ...
				    INNER JOIN b_ms_tindakan ON b_ms_tindakan_kelas.ms_tindakan_id = b_ms_tindakan.id
					INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id_asal
				    WHERE $fUnit AND b_ms_tindakan.id = '".$rw['tindakan_id']."'
					$unitnya $waktu $fKso";
				    $s1 = mysql_query($q1);
