...
if($_POST['isExcel']=='yes'){
	...
	header('Content-Disposition: attachment; filename="KasusPenyakitTmptLay.xls"');
}
...
<?php
	...
	if($tmpLayanan != 0) {
    	$fUnit = " un.id = '".$tmpLayanan."' ";
	}else{
		$fUnit = " un.parent_id = '".$jnsLayanan."'";
	}
	if($stsPas != 0){
...
<table width="750" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="5" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase"><b>Laporan Kasus Penyakit Pasien Berdasarkan Tempat Layanan<br />Status Pasien <?php if($stsPas==0) echo "SEMUA"; else echo $rwKso['nama'];?></b><br /><b><?php echo $Periode;?></b></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <?php
		...
		while($rwTmp = mysql_fetch_array($rsTmp))
		{
			$qBaru = "SELECT COUNT(k.pasien_id) AS jml
					...
					INNER JOIN b_pelayanan pl ON pl.kunjungan_id = k.id 
					INNER JOIN b_pasien_keluar pk ON pk.pelayanan_id = pl.id 
					WHERE pk.kasus = '1' AND pl.unit_id = '".$rwTmp[id]."' $waktu $fKso ";
			...
			$qLama = "SELECT COUNT(k.pasien_id) AS jml
					...
					INNER JOIN b_pelayanan pl ON pl.kunjungan_id = k.id 
					INNER JOIN b_pasien_keluar pk ON pk.pelayanan_id = pl.id 
					WHERE pk.kasus = '0' AND pl.unit_id = '".$rwTmp[id]."' $waktu $fKso ";
