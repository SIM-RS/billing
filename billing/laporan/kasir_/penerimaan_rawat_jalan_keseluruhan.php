...
$jdlLayanan="Rawat Inap";
$tipe = $_REQUEST['tipe'];
switch ($tipe){
	case "1":
		$jdlLayanan="Rawat Jalan";
		break;
	case "2":
		$jdlLayanan="Rawat Darurat";
		break;
}
...
<table id="tblPrint" width="1000" border="0" cellspacing="0" cellpadding="0" align="left" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="2" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase;"><b>Laporan Penerimaan <?php echo $jdlLayanan; ?><br />Secara Keseluruhan<br />Tanggal : <?php echo $periode; ?></b></td>
  </tr>
  <tr>
  	...
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
            <tr style="text-align:center; font-weight:bold;">
                ...
                <td width="7%" style="border-top:1px solid; border-bottom:1px solid; border-right:1px solid;">&nbsp;HD&nbsp;</td>
                <td width="7%" style="border-top:1px solid; border-bottom:1px solid; border-right:1px solid;">&nbsp;Endoscopy&nbsp;</td>
...
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id $fTmpLay ORDER BY mu.id";
			...
			?>
            <tr>
              <td colspan="17">&nbsp;<b><?php echo strtoupper($rwUnit['nama']); ?></b></td>
            </tr>
            <?php
...
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
INNER JOIN b_ms_kso kso ON bt.kso_id=kso.id WHERE p.unit_id='".$rwUnit['unit_id']."' AND bt.nilai>0 AND bt.tipe=0
...
ORDER BY kso.id";
			...
			?>
            <tr>
            	<td colspan="17">&nbsp;<b><?php echo $data['nama']; ?></b></td>
            </tr>
            <?php
					$no=1;
					$tot_p=0;
...
WHERE p.unit_id='".$rwUnit["unit_id"]."' AND bt.kso_id='".$data['id']."' AND bt.tipe=0
AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))
GROUP BY mp.id,p.id,bt.kso_id ORDER BY p.id";
...
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=59 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND bt.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
...
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=58 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND bt.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
						$sPk = mysql_query($qPk);
...
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=61 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND bt.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
						$sRad = mysql_query($qRad);
						$wRad = mysql_fetch_array($sRad);

						$qHd = "SELECT IFNULL(SUM(bt.nilai),0) nilai FROM (SELECT * FROM b_bayar WHERE tgl $period) b
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=65 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND bt.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
						$sHd = mysql_query($qHd);
						$wHd = mysql_fetch_array($sHd);

						$qEnd = "SELECT IFNULL(SUM(bt.nilai),0) nilai FROM (SELECT * FROM b_bayar WHERE tgl $period) b
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=67 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND bt.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
						$sEnd = mysql_query($qEnd);
						$wEnd = mysql_fetch_array($sEnd);
						
						$qFar = "SELECT SUM(ap.QTY_JUAL*ap.HARGA_SATUAN-
FLOOR(ap.QTY_RETUR*ap.HARGA_SATUAN*((100-ap.BIAYA_RETUR)/100))) AS nilai
FROM $dbbilling.b_pelayanan p INNER JOIN $dbapotek.a_penjualan ap ON p.id=ap.NO_KUNJUNGAN 
WHERE p.kunjungan_id='".$data2['kunjungan_id']."' AND p.jenis_kunjungan=1 AND ap.CARA_BAYAR=1";
						...
						$t = $p+$k+$tind+$rm+$kk4+$iur+$pa+$pk+$rad+$hd+$end+$farm;
						$tot=$tot_p+$tot_k+$tot_t+$tot_rm+$tot_kk4+$tot_i+$tot_pa+$tot_pk+$tot_r+$tot_hd+$tot_end+$tot_f;
			?>
            <tr style="text-align:center;">
                ...
                <td width="10%" style="" align="right"><?php echo number_format($hd,'0',',','.'); ?>&nbsp;</td>
                <td width="10%" style="" align="right"><?php echo number_format($end,'0',',','.'); ?>&nbsp;</td>
            ...
            <tr style="text-align:center; font-weight:bold;">
                ...
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($tot_hd,'0',',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($tot_end,'0',',','.'); ?>&nbsp;</td>
            ...
            <?php
				...
				$total_hd=$total_hd+$tot_hd;
				$total_end=$total_end+$tot_end;
            ...
            <tr style="text-align:center; font-weight:bold;">
                ...
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($total_hd,'0',',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($total_end,'0',',','.'); ?>&nbsp;</td>
