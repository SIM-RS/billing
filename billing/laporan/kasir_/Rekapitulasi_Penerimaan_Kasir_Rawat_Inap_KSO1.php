...
WHERE b.kasir_id = '$kasir' $fkasir AND b.tgl $period ORDER BY kso.id";
$rsKSO=mysql_query($sqlKSO);
  	...
  	<td colspan="2">
		<table border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			<tr style="text-align:center; font-weight:bold;background-color:#CCC;">
				...
				<td style="border-right:1px solid;border-top:1px solid; border-bottom:1px solid;" colspan="3">Jumlah</td>
			</tr>
            <tr style="text-align:center; font-weight:bold;background-color:#CCC;">
				<?php
				$i=0;
                ...
                <td width="100" style="border-right:1px solid;border-bottom:1px solid;">TOTAL UMUM + IUR</td>
			</tr>
			<?php
			...
			<tr style="background-color:#EEE;font-weight:bold;">
				<td height="25" colspan="2" style="border-bottom:1px solid;border-right:1px solid;border-left:1px solid;">&nbsp;&nbsp;Kasir : <?php echo $kasir_rw["nama"]; ?></td>
				<?php
				for($i=0;$i<$jml_kso;$i++){
					$subTotuang[$i]=0;
					$subTotiur[$i]=0;
            ...
            <?php
			/*$sql = "SELECT DISTINCT mu.id,mu.nama FROM (SELECT * FROM b_bayar b WHERE b.kasir_id = '$kasir' AND b.user_act='".$kasir_rw["id"]."' AND b.tgl $period) b
			...
			$sql = "SELECT DISTINCT * FROM (SELECT mu.id,mu.nama FROM (SELECT * FROM b_bayar b WHERE b.kasir_id = '$kasir' AND b.user_act='".$kasir_rw["id"]."' 
AND b.tgl $period) b INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id 
...
UNION
SELECT mu.id,mu.nama FROM (SELECT * FROM b_bayar b WHERE b.kasir_id = '$kasir' AND b.user_act='".$kasir_rw["id"]."' 
AND b.tgl $period) b INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id 
INNER JOIN b_tindakan_kamar t ON bt.tindakan_id = t.id INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id 
INNER JOIN b_ms_unit mu ON p.unit_id = mu.id WHERE bt.tipe=1 AND bt.nilai>0) gab ORDER BY gab.nama";
			//echo $sql."<br>";
            ...
            <tr>
				...
				for($i=0;$i<$jml_kso;$i++){
					//bayar tindakan
					$sql = "SELECT IFNULL(SUM(bt.nilai),0) AS nilai FROM (SELECT * FROM b_bayar WHERE kasir_id = '$kasir' AND user_act = '".$kasir_rw["id"]."' AND tgl $period) b
					INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id
					INNER JOIN b_tindakan t ON bt.tindakan_id = t.id
					INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
				...
				FROM (SELECT *
					  FROM b_bayar b
					  WHERE kasir_id = '$kasir' AND b.user_act='".$kasir_rw["id"]."'
						  AND tgl $period) b
            ...
            <tr style="font-weight:bold" height="25">
			  ...
			  ?>
              <td style="border-right:1px solid;border-bottom:1px solid;" align="right"><?php echo number_format($SubtotalPx,0,',','.'); ?>&nbsp;</td>
            ...
            <tr style="font-weight:bold" height="25">
			  ...
			  ?>
              <td style="border-right:1px solid;border-bottom:1px solid;" align="right"><?php echo number_format($gTotalPx,0,',','.'); ?>&nbsp;</td>
              <td style="border-right:1px solid;border-bottom:1px solid;" align="right"><?php echo number_format($gtotIur,0,',','.'); ?>&nbsp;</td>
