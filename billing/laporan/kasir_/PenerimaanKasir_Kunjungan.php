		...
		<td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				<tr>
					...
					<td width="4%" class="tblatas">Unit Asal</td>
				...
				?>
				<tr>
					<td colspan="16" style="padding-left:20px; text-transform:uppercase; text-decoration:underline; font-weight:bold;" height="25" valign="bottom"><?php echo $rwNm['nama'];?></td>
				</tr>
				<?php
						$sqlPas = "SELECT t.no_rm, t.pasien, t.kunjungan_id FROM (SELECT b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, b_kunjungan.id AS kunjungan_id FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id WHERE b_bayar.user_act='".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'
UNION
SELECT b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, b_kunjungan.id AS kunjungan_id FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id WHERE b_bayar.user_act='".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."') AS t GROUP BY t.kunjungan_id";
						//echo $sqlPas."<br>";
				...
				<?php
						...
						/*if($wcek['penunjang']==1){
							...
							$sql = "SELECT t.id, t.nama, t.inap, t.penunjang, t.pelayanan_id, t.status FROM (SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap, b_ms_unit.penunjang, b_pelayanan.id AS pelayanan_id, b_ms_kso.nama AS status FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id=b_pelayanan.unit_id INNER JOIN b_ms_kso ON b_ms_kso.id=b_pelayanan.kso_id WHERE b_pelayanan.kunjungan_id='".$rwPas['kunjungan_id']."' AND b_ms_unit.penunjang=0 AND b_ms_unit.kategori<>5 UNION SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap, b_ms_unit.penunjang, IFNULL(b_pelayanan.id,0) AS pelayanan_id, b_ms_kso.nama AS STATUS FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id=b_pelayanan.unit_id_asal INNER JOIN b_ms_unit mu ON mu.id=b_pelayanan.unit_id INNER JOIN b_ms_kso ON b_ms_kso.id=b_pelayanan.kso_id WHERE b_pelayanan.kunjungan_id='".$rwPas['kunjungan_id']."' AND b_ms_unit.penunjang=1 AND mu.penunjang=1 AND mu.kategori<>5 UNION SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap, b_ms_unit.penunjang, 
IFNULL(b_pelayanan.id,0) AS pelayanan_id, b_ms_kso.nama AS STATUS 
FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id=b_pelayanan.unit_id_asal 
INNER JOIN b_ms_unit mu ON mu.id=b_pelayanan.unit_id INNER JOIN b_ms_kso ON b_ms_kso.id=b_pelayanan.kso_id 
WHERE b_pelayanan.kunjungan_id='".$rwPas['kunjungan_id']."' AND b_ms_unit.penunjang=0 AND mu.penunjang=1 AND mu.kategori<>5 AND (b_ms_unit.kategori=2 OR b_ms_unit.kategori=1) UNION SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap, b_ms_unit.penunjang, b_pelayanan.id AS pelayanan_id, b_ms_kso.nama AS status FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_unit ON b_ms_unit.id=b_pelayanan.unit_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id INNER JOIN b_ms_kso ON b_ms_kso.id=b_tindakan.kso_id WHERE b_pelayanan.kunjungan_id='".$rwPas['kunjungan_id']."' AND b_ms_unit.penunjang=0 AND b_bayar_tindakan.tipe=1 ) AS t GROUP BY t.id ORDER BY t.pelayanan_id";
						//}
						//echo $sql.";<br>";
						//if ($no==2) echo $sql."<br>";
						$rs = mysql_query($sql);
						...
						if (mysql_num_rows($rs)>0){
							...
							{
								$sqlStatus="SELECT DISTINCT kso.nama FROM b_bayar b INNER JOIN b_bayar_tindakan bt ON b.id=bt.bayar_id
INNER JOIN b_ms_kso kso ON bt.kso_id=kso.id
WHERE b.kunjungan_id='".$rwPas['kunjungan_id']."' AND b.user_act='".$rwNm['id']."' AND b.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								$rsStatus=mysql_query($sqlStatus);
								$cstatus="";
								while ($rwStatus=mysql_fetch_array($rsStatus)){
									$cstatus .=$rwStatus["nama"].", ";
								}
								if ($cstatus!=""){
									$cstatus=substr($cstatus,0,strlen($cstatus)-2);
								}
								...
								if ($rw['penunjang']==1){
									$qTin = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_pelayanan INNER JOIN b_ms_unit ON b_pelayanan.unit_id_asal=b_ms_unit.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id=b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id WHERE b_pelayanan.kunjungan_id='".$rwPas['kunjungan_id']."' AND b_ms_unit.kategori=1 AND b_bayar_tindakan.tipe=0 AND b_ms_tindakan.klasifikasi_id<>11 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id='".$rw['id']."' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								}else{
									$qTin = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id=b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id WHERE b_pelayanan.kunjungan_id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_ms_tindakan.klasifikasi_id<>11 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id='".$rw['id']."' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								}
								$sTin = mysql_query($qTin);
								$wTin = mysql_fetch_array($sTin);
								
								$qPk = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id=b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id  INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id  WHERE b_kunjungan.id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id_asal='".$rw['id']."' AND b_pelayanan.unit_id='58' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								$sPk = mysql_query($qPk);
								$wPk = mysql_fetch_array($sPk);
								
								$qPa = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id=b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id  INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id  WHERE b_kunjungan.id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id_asal='".$rw['id']."' AND b_pelayanan.unit_id='59' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								$sPa = mysql_query($qPa);
								$wPa = mysql_fetch_array($sPa);
								
								$qRad = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id=b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id  INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id  WHERE b_kunjungan.id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id_asal='".$rw['id']."' AND b_pelayanan.unit_id='61' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								//echo $qRad."<br>";
								$sRad = mysql_query($qRad);
								$wRad = mysql_fetch_array($sRad);
								
								$qHd = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id=b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id  INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id  WHERE b_kunjungan.id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id_asal='".$rw['id']."' AND b_pelayanan.unit_id='65' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								$sHd = mysql_query($qHd);
								$wHd = mysql_fetch_array($sHd);
								
								$qEnd = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id=b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id  INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id  WHERE b_kunjungan.id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id_asal='".$rw['id']."' AND b_pelayanan.unit_id='67' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								$sEnd = mysql_query($qEnd);
								$wEnd = mysql_fetch_array($sEnd);
								
								$qOk = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id=b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id  INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id  WHERE b_kunjungan.id='".$rwPas['kunjungan_id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.user_act='".$rwNm['id']."' AND b_pelayanan.unit_id_asal='".$rw['id']."' AND (b_pelayanan.unit_id='63' OR b_pelayanan.unit_id='47') AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
								$sOk = mysql_query($qOk);
				...
				<tr>
					...
					<?php 
							$total = $wRet['nilai']+$wTin['nilai']+$wPk['nilai']+$wPa['nilai']+$wRad['nilai']+$wHd['nilai']+$wEnd['nilai']+$wOk['nilai'];
						
					?>
					<td style="text-align:right; padding-right:5px;"><?php echo number_format($total,0,",",".")?></td>
					<td style="text-align:center;"><?php echo $cstatus;?></td>
				</tr>
				<?php
							$ret = $ret + $wRet['nilai'];
							$tin = $tin + $wTin['nilai'];
