...
<table id="tblPrint" width="800" border="0" cellspacing="0" cellpadding="0" align="left" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="2" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase;"><b>Laporan Setoran Penerimaan <?php echo $rwKsr['nama'];?><br />Periode <?php echo $tglAwal[0]." ".$arrBln[$tglAwal[1]]." ".$tglAwal[2].' s/d '.$tglAkhir[0]." ".$arrBln[$tglAkhir[1]]." ".$tglAkhir[2]?></b></td>
  </tr>
  <tr>
  ...
  <tr>
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			<?php
					$sqlRuang = "SELECT t.id, t.nama, t.inap FROM (SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id  INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND DATE(b_bayar.disetor_tgl) BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.disetor = 1
					UNION
					SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND DATE(b_bayar.disetor_tgl) BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.disetor = 1) AS t GROUP BY t.id";
			...
			<?php
					$sqlPas = "SELECT t.pelayanan_id, t.kso, t.kwi, t.no_rm, t.pasien, t.jam, t.tgl FROM (SELECT b_pelayanan.id AS pelayanan_id, b_ms_kso.nama AS kso, b_bayar.nobukti AS kwi, b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl FROM b_pelayanan INNER JOIN b_ms_pasien ON b_pelayanan.pasien_id = b_ms_pasien.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_kso ON b_ms_kso.id = b_tindakan.kso_id  INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND DATE(b_bayar.disetor_tgl) BETWEEN  '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.disetor=1 AND b_pelayanan.unit_id = '".$rwRuang['id']."' UNION
				SELECT b_pelayanan.id AS pelayanan_id, b_ms_kso.nama AS kso, b_bayar.nobukti AS kwi, b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl FROM b_pelayanan INNER JOIN b_ms_pasien ON b_pelayanan.pasien_id = b_ms_pasien.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_kso ON b_ms_kso.id = b_tindakan.kso_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND DATE(b_bayar.disetor_tgl) BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.disetor=1 AND b_pelayanan.unit_id = '".$rwRuang['id']."') AS t GROUP BY t.pelayanan_id";
					//echo $sqlPas."<br>";
					...
					{
						$qTind = "SELECT SUM(b_bayar_tindakan.nilai) AS jml FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_pelayanan.id='".$rwPas['pelayanan_id']."' AND b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND DATE(b_bayar.disetor_tgl) BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.disetor=1";
						//echo $qTind."<br>";
						//if ($rwRuang['id']==39) echo $qTind."<br>";
						$sTind = mysql_query($qTind);
						$wTind = mysql_fetch_array($sTind);
						
						$qInap = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_pelayanan INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_pelayanan.id = '".$rwPas['pelayanan_id']."' AND b_bayar_tindakan.tipe = 1 AND b_bayar.user_act = '".$rwNm['id']."' AND DATE(b_bayar.disetor_tgl) BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.disetor=1";
						//echo $qInap."<br>";
