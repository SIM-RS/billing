...
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=65 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND t.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
						$sHd = mysql_query($qHd);
...
INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id INNER JOIN b_tindakan_ak t ON bt.tindakan_id = t.b_tindakan_id 
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
WHERE p.unit_id=67 AND p.unit_id_asal='".$rwUnit["unit_id"]."' AND t.kso_id='".$data['id']."' AND p.kunjungan_id='".$data2['kunjungan_id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))";
