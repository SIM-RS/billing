  	...
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			<?php
					...
					t.jam, 
					t.tgl 
					FROM (SELECT b_pelayanan.id AS pelayanan_id, b_ms_kso.nama AS kso, b_bayar.nobukti AS kwi, b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl 
					FROM b_pelayanan 
					INNER JOIN b_ms_pasien ON b_pelayanan.pasien_id = b_ms_pasien.id 
					...
					INNER JOIN b_ms_kso ON b_ms_kso.id = b_bayar_tindakan.kso_id  
					
					WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN  '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2' AND b_pelayanan.unit_id = '".$rwRuang['id']."'
					UNION
					SELECT 
					...
					FROM b_pelayanan 
					INNER JOIN b_ms_pasien ON b_pelayanan.pasien_id = b_ms_pasien.id 
					LEFT JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id 
					...
					INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id 
					INNER JOIN b_ms_kso ON b_ms_kso.id = b_bayar_tindakan.kso_id 
					WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2' AND b_pelayanan.unit_id = '".$rwRuang['id']."') AS t GROUP BY t.pelayanan_id";
					//echo $sqlPas."<br>";
