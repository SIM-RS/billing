...
<table id="tblPrint" width="800" border="0" cellspacing="0" cellpadding="0" align="left" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="2" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase;"><b>Laporan Penerimaan Kasir Rawat Jalan<br />Periode <?php echo $periode; ?></b></td>
  </tr>
  <tr>
  	...
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
            <tr style="text-align:center; font-weight:bold;">
				...
				<td width="8%" style="border-top:1px solid; border-bottom:1px solid; border-left:1px solid; border-right:1px solid;">Jam</td>
				<!--td width="10%" style="border-top:1px solid; border-bottom:1px solid; border-right:1px solid;">No. Kwitansi</td-->
                <td width="10%" style="border-top:1px solid; border-bottom:1px solid; border-right:1px solid;">No. RM</td>
                <td width="20%" style="border-top:1px solid; border-bottom:1px solid; border-right:1px solid;">Nama Pasien</td>
                <td width="15%" style="border-top:1px solid; border-bottom:1px solid; border-right:1px solid;">Status</td>
...
INNER JOIN b_ms_pegawai mp ON b.user_act=mp.id";
			...
			?>
            <tr>
              <td colspan="12" style="text-decoration:underline">&nbsp;<b><?php echo $rwKasir['nama']; ?></b></td>
            </tr>
            <?php
...
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id ORDER BY mu.id";
			...
			?>
            <tr>
            	<td colspan="12">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><?php echo $poli['nama']; ?></b></td>
            </tr>
            <?php
				$sql2="SELECT gab.tanggal,gab.jam,gab.nobukti,gab.no_rm,gab.pasien,gab.kso_id,gab.b_tindakan_id,gab.kso,gab.pelayanan_id,
SUM(ret) ret,SUM(konsul) konsul,SUM(resum) resum,SUM(gab.kk4) kk4,SUM(gab.tind) tind
FROM 
(SELECT DISTINCT mp.id,b.tgl tanggal,DATE_FORMAT(b.tgl_act,'%H:%i') jam,b.nobukti,mp.no_rm,mp.nama pasien,kso.id kso_id,kso.nama kso,t.b_tindakan_id,p.id pelayanan_id,
IF((mt.klasifikasi_id=11 AND mt.id<>2363),bt.nilai,0) ret,IF((mt.id=2363),bt.nilai,0) konsul,
IF((mt.id=10),bt.nilai,0) resum,IF((mt.id=12),bt.nilai,0) kk4,
IF((mt.id<>2363 AND mt.id<>10 AND mt.id<>12),bt.nilai,0) tind
FROM (SELECT * FROM b_bayar WHERE user_act='".$rwKasir['id']."' AND tgl $period) b 
...
INNER JOIN b_ms_pasien mp ON k.pasien_id=mp.id INNER JOIN b_ms_kso kso ON bt.kso_id=kso.id
WHERE p.unit_id='".$poli['id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))) AS gab 
GROUP BY gab.id,gab.pelayanan_id,gab.kso_id ORDER BY gab.pelayanan_id";
				//echo $sql2.";<br>";
				$no = 0;
				$tot_konsul=0;
				$tot_tindakan=0;
				...
				while($data2=mysql_fetch_array($kueri2)){
					...
					if ($data2['kso_id']==1){
						...
						$kk4=0;
						$tind=0;
						$iur=$data2['konsul']+$data2['resum']+$data2['kk4']+$data2['tind'];
					}
					$tot_konsul=$tot_konsul+$konsul;
					$tot_tindakan=$tot_tindakan+$tind;
					$tot_iur=$tot_iur+$iur;
					$tot_kk4=$tot_kk4+$kk4;
					$tot_rm=$tot_rm+$rm;
					
					$t = $konsul+$rm+$kk4+$tind+$iur;
					$tot=$tot_konsul+$tot_tindakan+$tot_iur+$tot_kk4+$tot_rm;
			?>
            <tr style="text-align:center;">
            	<td width="5%" style="" align="center"><?php echo $no; ?></td>
				<td width="10%" style="" align="center"><?php echo tglSQL($data2['tanggal']); ?></td>
				<td width="8%" style="" align="center"><?php echo $data2['jam']; ?></td>
				<!--td width="10%" style="" align="center"><?php echo $data2['nobukti']; ?></td-->
                <td width="10%" style="" align="center">&nbsp;<?php echo $data2['no_rm']; ?></td>
                <td width="10%" style="" align="left">&nbsp;<?php echo $data2['pasien']; ?></td>
                <td width="10%" style="" align="center"><?php echo $data2['kso']; ?></td>
            ...
            <tr style="text-align:center; font-weight:bold;">
                ...
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($tot_konsul,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($tot_tindakan,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($tot_iur,0,',','.'); ?>&nbsp;</td>
            ...
            <?php
				$total_konsul=$total_konsul+$tot_konsul;
				$total_tindakan=$total_tindakan+$tot_tindakan;
				$total_iur=$total_iur+$tot_iur;
            ...
            <?php 
				$gtotal_konsul=$gtotal_konsul+$total_konsul;
				$gtotal_tindakan=$gtotal_tindakan+$total_tindakan;
				$gtotal_iur=$gtotal_iur+$total_iur;
				$gtotal_kk4=$gtotal_kk4+$total_kk4;
				$gtotal_rm=$gtotal_rm+$total_rm;
				$gtotal=$gtotal+$total;
			}
			?>
            <tr>
            ...
            <tr style="text-align:center; font-weight:bold;">
                ...
                <td width="10%" style="">&nbsp;</td>
                <td width="10%" style="">&nbsp;</td>
                <td width="10%" style="" align="right">Grand Total :&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($gtotal_konsul,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($gtotal_tindakan,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($gtotal_iur,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($gtotal_kk4,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($gtotal_rm,0,',','.'); ?>&nbsp;</td>
                <td width="10%" style="border-top:1px solid;" align="right"><?php echo number_format($gtotal,0,',','.'); ?>&nbsp;</td>
			</tr>
		</table>
	</td>
  </tr>
