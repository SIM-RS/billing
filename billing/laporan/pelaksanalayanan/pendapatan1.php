...
<?php
	...
	$sqlDok = "SELECT id, nama FROM b_ms_pegawai WHERE id = '".$_REQUEST['cmbDokJns']."'";
	...
	<tr>
		<td height="30"><table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			<tr style="font-weight:bold;">
				...
				<td width="50%" align="right">Jenis Layanan:&nbsp;<?php echo $rwUnit1['nama'] ?>&nbsp;</td>
			</tr>
		</table></td>
	</tr>
	<tr>
		<td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				...
				<?php
						if($tmpLayanan==0){
							$fUnit = "AND b_pelayanan.jenis_layanan = '".$jnsLayanan."'";
						}else{
							$fUnit = "AND b_pelayanan.unit_id = '".$tmpLayanan."'";
						}
						if($stsPas!=0) $fKso = " AND b_tindakan.kso_id = '".$stsPas."'";
						
						$qTmp = "SELECT b_ms_unit.id, b_ms_unit.nama FROM b_kunjungan INNER JOIN b_pelayanan ON b_kunjungan.id = b_pelayanan.kunjungan_id
...
WHERE $waktu AND b_ms_pegawai.id = '".$_REQUEST['cmbDokJns']."' $fKso $fUnit GROUP BY b_ms_unit.id ORDER BY b_ms_unit.nama";
				...
				<?php
						$qSts = "SELECT b_ms_kso.id, b_ms_kso.nama FROM b_kunjungan
							...
							AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' $fKso
				...
				<?php
						$qTgl = "SELECT DATE_FORMAT(b_tindakan.tgl,'%d-%m-%Y') AS tgl FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id LEFT JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id WHERE $waktu AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_pelayanan.unit_id = '".$rwTmp['id']."' AND b_tindakan.kso_id = '".$rwSts['id']."' GROUP BY b_tindakan.tgl";
				...
				<?php
					...
					$sql = "SELECT b_ms_pasien.id, DATE_FORMAT(b_tindakan.tgl,'%d/%m/%Y') AS tgl, b_ms_pasien.no_rm, b_ms_pasien.nama, b_pelayanan.id AS pelayanan_id, b_kunjungan.id AS kunjungan_id FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id = b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id LEFT JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id WHERE b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.ms_tindakan_unit_id= '".$rwTmp['id']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_tindakan.tgl = '".$tgl."' GROUP BY b_ms_pasien.id";
					$rs = mysql_query($sql);
					$no = 1;
					$sub = 0;
					$subRet = 0;
					...
					{
						$qRet = "SELECT SUM(b_tindakan.biaya) AS jml FROM b_tindakan INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id WHERE b_ms_tindakan.klasifikasi_id = '11' AND b_tindakan.pelayanan_id = '".$rw['pelayanan_id']."' AND b_tindakan.tgl = '".$tgl."' AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_tindakan.ms_tindakan_unit_id = '".$rwTmp['id']."'";
						$rsRet = mysql_query($qRet);
						$rwRet = mysql_fetch_array($rsRet);
						
						$qVis = "SELECT SUM(b_tindakan.biaya) AS jml FROM b_tindakan INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id = b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan_kelas.ms_tindakan_id = b_ms_tindakan.id WHERE (b_ms_tindakan.klasifikasi_id = '13' OR b_ms_tindakan.klasifikasi_id = '14') AND b_tindakan.pelayanan_id = '".$rw['pelayanan_id']."' AND b_tindakan.tgl = '".$tgl."' AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_tindakan.ms_tindakan_unit_id = '".$rwTmp['id']."'";
						$rsVis = mysql_query($qVis);
						$rwVis = mysql_fetch_array($rsVis);
						
						$qTin = "SELECT SUM(b_tindakan.biaya) AS jml FROM b_tindakan INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id
INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id WHERE b_ms_tindakan.klasifikasi_id <> '11' AND b_ms_tindakan.klasifikasi_id <> '13' AND b_ms_tindakan.klasifikasi_id <> '14' AND b_tindakan.pelayanan_id = '".$rw['pelayanan_id']."' AND b_tindakan.tgl = '".$tgl."' AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_tindakan.ms_tindakan_unit_id = '".$rwTmp['id']."'";
				...
				<tr>
				  <td style="padding-right:20px; text-align:right"><?php echo $no;?></td>
					...
					<td style="padding-right:10px; text-align:right"><?php echo number_format($rwRet['jml'],0,",",".")?></td>
					<td style="padding-right:10px; text-align:right"><?php echo number_format($rwVis['jml'],0,",",".")?></td>
					<td style="padding-right:10px; text-align:right"><?php echo number_format($rwTin['jml'],0,",",".")?></td>
					<?php
						$tot = $rwRet['jml'] + $rwVis['jml'] + $rwTin['jml'];
					?>
				...
				<?php
					$no++;
					$sub = $sub + $tot;
					$subRet = $subRet + $rwRet['jml'];
					$subVis = $subVis + $rwVis['jml'];
					$subTin = $subTin + $rwTin['jml'];
