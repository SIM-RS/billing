...
if($_POST['export']=='Export Excell'){
    ...
    header('Content-Disposition: attachment; filename="Lap Pendapatan Dokter rs Tindakan.xls"');
}
...
<body>
    <?php
    $queryDokter = mysql_query("SELECT td.id_dokter,p.nama,td.id_tindakan_kelas FROM b_ms_tarif_dokter td LEFT JOIN b_ms_pegawai p ON p.id = td.id_dokter WHERE id_dokter = {$_REQUEST['cmbDokSemua']} GROUP BY td.id_dokter,p.nama");
    ...
    <div class="container p-2">
        ...
        <table class="table table-borderless table-sm">
            <tr>
                <td colspan="5" align="center" style="font-weight: bold;text-transform: uppercase;border: none;">REKAPITULASI PENDAPATAN TINDAKAN RS DAN DOKTER<br><?= $cetak?></td>
            </tr>
            <tr>
        ...
        <table border="1" class="table table-bordered table-sm">
            ...
            <tbody>
                ...
                while ($rows = mysql_fetch_assoc($queryDokter)) {
                    echo "<tr>";
                    echo "<td colspan='5' class='font-weight-bold'>" . $rows['nama'] . "</td>";
                    echo "</tr>";
                    $sql = "SELECT
                                q.*,
                                td.tarip_dokter_nominal AS nominal,
                                td.tarip_dokter_presentase AS presentase,
                                sum(q.tarip) as total,
                                count(*) as jumlah,
                                (sum(q.tarip) * td.tarip_dokter_presentase / 100) pendapatan_dokter,
                                (sum(q.tarip) - (sum(q.tarip) * td.tarip_dokter_presentase / 100)) pendapatan_rs,
                                (count(*) * td.tarip_dokter_nominal) pendapatan_dokter_nominal,
                                (sum(q.tarip) - (count(*) * td.tarip_dokter_nominal)) pendapatan_rs_nominal
                            FROM
                                (
                                SELECT
                                    t.kunjungan_id,
                                ...
                                INNER JOIN ( SELECT * FROM b_ms_tarif_dokter WHERE id_dokter = {$rows['id_dokter']} ) td ON td.id_tindakan_kelas = q.ms_tindakan_kelas_id
                                GROUP BY
                                    q.ms_tindakan_kelas_id,
                                    q.tgl
                                    ";
                    // echo $sql;
                    $totalPendapatanDokter = 0;
                    $totalPendapatanRs = 0;
                    $queryPendapatan = mysql_query($sql);
                    $no = 1;
                    while ($data = mysql_fetch_assoc($queryPendapatan)) {
                        $pendapatanRs = 0;
                        $dokterPendapatan = 0;
                        echo "<tr>";
                        echo "<td align=''>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" .$data['nama'] . "</td>";
                        echo "<td align='center'>" . tanggalC($data['tgl']) . "</td>";
                        echo "<td align='right'>" . number_format($data['tarip'], 0, ",", ",") . "</td>";
                        if ($data['pendapatan_dokter_nominal'] != '') {
                            $dokterPendapatan = $data['pendapatan_dokter_nominal'];
                            $totalPendapatanDokter += $data['pendapatan_dokter_nominal'];
                        } else if ($data['pendapatan_dokter'] != '') {
                            $dokterPendapatan = $data['pendapatan_dokter'];
                            $totalPendapatanDokter += ($data['pendapatan_dokter']);
                        }

                        if($data['pendapatan_rs'] != ''){
                            $pendapatanRs = $data['pendapatan_rs'];
                        }else if($data['pendapatan_rs_nominal'] != ''){
                            $pendapatanRs = $data['pendapatan_rs_nominal'];
                        }
                        echo "<td align='right'>" . number_format($dokterPendapatan, 0, ",", ",") . "</td>";
                        echo "<td align='right'>" . number_format($pendapatanRs, 0, ",", ",") . "</td>";
                        echo "</tr>";
                        $totalPendapatanRs += $pendapatanRs;
                        $no++;
                    }
                    echo "<tr><td colspan='3' class='font-weight-bold text-center'>Total</td>";
                    echo "<td class='text-right font-weight-bold'>" . number_format($totalPendapatanDokter, 0, ",", ",") . "</td>";
                    echo "<td class='text-right font-weight-bold'>" . number_format($totalPendapatanRs, 0, ",", ",") . "</td></tr>";
                }
                ?>
            </tbody>
        </table>
