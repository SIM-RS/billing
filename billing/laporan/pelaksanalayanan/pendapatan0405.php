...
WHERE $waktu AND b_ms_pegawai.id = '".$_REQUEST['cmbDokJns']."' $fKso $fUnit GROUP BY b_ms_unit.id ORDER BY b_ms_unit.nama";
				...
				<?php
						$qSts = "SELECT b_ms_kso.id, b_ms_kso.nama FROM b_kunjungan
							...
							INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id
							INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id 
				...
				<?php
						$qTgl = "SELECT DISTINCT DATE_FORMAT(b_tindakan.tgl,'%d-%m-%Y') AS tgl FROM b_kunjungan INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id = b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id  WHERE $waktu AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_pelayanan.unit_id = '".$rwTmp['id']."' AND b_tindakan.kso_id = '".$rwSts['id']."'";
				...
				<?php
					...
					{
						$qRet = "SELECT SUM(b_tindakan.qty*b_tindakan.biaya) AS jml FROM b_tindakan INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id = b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id = b_ms_tindakan_kelas.ms_tindakan_id INNER JOIN b_pelayanan ON b_tindakan.pelayanan_id=b_pelayanan.id WHERE b_ms_tindakan.klasifikasi_id = '11' AND b_tindakan.kunjungan_id = '".$rw['kunjungan_id']."' AND b_tindakan.tgl = '".$tgl."' AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_pelayanan.unit_id = '".$rwTmp['id']."'";
						$rsRet = mysql_query($qRet);
						$rwRet = mysql_fetch_array($rsRet);
						
						$qVis = "SELECT SUM(b_tindakan.qty*b_tindakan.biaya) AS jml FROM b_tindakan INNER JOIN b_ms_tindakan_kelas ON b_tindakan.ms_tindakan_kelas_id = b_ms_tindakan_kelas.id INNER JOIN b_ms_tindakan ON b_ms_tindakan_kelas.ms_tindakan_id = b_ms_tindakan.id INNER JOIN b_pelayanan ON b_tindakan.pelayanan_id=b_pelayanan.id WHERE (b_ms_tindakan.klasifikasi_id = '13' OR b_ms_tindakan.klasifikasi_id = '14') AND b_tindakan.kunjungan_id = '".$rw['kunjungan_id']."' AND b_tindakan.tgl = '".$tgl."' AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_pelayanan.unit_id = '".$rwTmp['id']."'";
						...
						WHERE (b_ms_tindakan.klasifikasi_id <> 13 AND b_ms_tindakan.klasifikasi_id <> 14 AND b_ms_tindakan.klasifikasi_id <> 11) AND b_tindakan.kunjungan_id = '".$rw['kunjungan_id']."' AND b_tindakan.tgl = '".$tgl."' AND b_tindakan.user_id = '".$_REQUEST['cmbDokJns']."' AND b_tindakan.kso_id = '".$rwSts['id']."' AND b_pelayanan.unit_id = '".$rwTmp['id']."'";
