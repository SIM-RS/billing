...
<body>
    ...
    <div class="container p-2">
        ...
        <table class="table table-borderless table-sm">
            <tr>
                <td colspan="2" align="center" style="font-weight: bold;text-transform: uppercase;border: none;">LAPORAN PENDAPATAN DOKTER TERBANYAK<br><?= $cetak?></td>
            </tr>
        </table>
    ...
    // while($row = mysql_fetch_assoc($query)){
        $sql = "SELECT * FROM(
                    ...
                    FROM
                        b_ms_pegawai 
                WHERE
                    pegawai_jenis = 8) p ON p.id = t.user_id
        ...
        // echo '<tr><td colspan="2" style="font-weight:bold">'.$row['nama'].'</td>';
        while ($rows = mysql_fetch_assoc($queryDokter)) {
                    echo '<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$rows['nama'].'</td>';
                    echo '<td align="right">'.number_format($rows['pendapatan'],0,',',',').'</td><tr>';
        }
    // }
                ?>
            </tbody>
