...
$worksheet->setColumn (9, 20, 10);//KELAS
...
$worksheet->setColumn (15, 15, 8);//T OP
$worksheet->setColumn (16, 16, 8);//PK
$worksheet->setColumn (17, 17, 8);//PA
$worksheet->setColumn (18, 18, 8);//RAD
$worksheet->setColumn (19, 19, 8);//HD
$worksheet->setColumn (20, 20, 8);//END*/
$worksheet->setColumn (21, 21, 10);//JML TIND
$worksheet->setColumn (22, 22, 8);//OBAT
$worksheet->setColumn (23, 23, 10);//JML TIND + OBAT
$worksheet->setColumn (24, 24, 8);//PMI
$worksheet->setColumn (25, 28, 12);//JUMLAH
/*$worksheet->setColumn (26, 26, 10);//BYR TUNAI
$worksheet->setColumn (27, 27, 10);//TOT BIAYA PX
$worksheet->setColumn (28, 28, 10);//STATUS VERF*/
$worksheet->setColumn (29, 29, 15);//PENJAMIN PX
...
$worksheet->write($row, 15, "TIND OPERATIF", $columnTitleFormat);
$worksheet->write($row, 16, "PK", $columnTitleFormat);
$worksheet->write($row, 17, "PA", $columnTitleFormat);
$worksheet->write($row, 18, "RAD", $columnTitleFormat);
$worksheet->write($row, 19, "HD", $columnTitleFormat);
$worksheet->write($row, 20, "ENDS", $columnTitleFormat);
$worksheet->write($row, 21, "JML TINDAKAN", $columnTitleFormat);
$worksheet->write($row, 22, "OBAT", $columnTitleFormat);
$worksheet->write($row, 23, "JML TINDAKAN + OBAT", $columnTitleFormat);
$worksheet->write($row, 24, "PMI", $columnTitleFormat);
$worksheet->write($row, 25, "JUMLAH", $columnTitleFormat);
$worksheet->write($row, 26, "BAYAR TUNAI", $columnTitleFormat);
$worksheet->write($row, 27, "TOTAL BIAYA PX", $columnTitleFormat);
$worksheet->write($row, 28, "STATUS VERIFIKASI", $columnTitleFormat);
$worksheet->write($row, 29, "PENJAMIN PX", $columnTitleFormat);
...
/*	$qKamarByr = "SELECT SUM(cH.hari*cH.bayar_pasien) AS jml FROM (SELECT IF(b_tindakan_kamar.status_out=0, 
	...
	$worksheet->write($row, 12, $bTin, $regularFormatR);
	$worksheet->write($row, 13, $bVisite, $regularFormatR);
	$worksheet->write($row, 14, $bGizi, $regularFormatR);
	$worksheet->write($row, 15, $bOp, $regularFormatR);
	$worksheet->write($row, 16, (($bPk=="")?"0":$bPk), $regularFormatR);
	$worksheet->write($row, 17, (($bPa=="")?"0":$bPa), $regularFormatR);
	$worksheet->write($row, 18, (($bRad=="")?"0":$bRad), $regularFormatR);
	$worksheet->write($row, 19, (($bHd=="")?"0":$bHd), $regularFormatR);
	$worksheet->write($row, 20, (($bEnd=="")?"0":$bEnd), $regularFormatR);
	$worksheet->write($row, 21, $jmlTind, $regularFormatR);
	$worksheet->write($row, 22, $bObat, $regularFormatR);
	$worksheet->write($row, 23, $jumlahTindakan, $regularFormatR);
	$worksheet->write($row, 24, "0", $regularFormatR);
	$worksheet->write($row, 25, $jumlahTindakan, $regularFormatR);
	$worksheet->write($row, 26, $Bayar, $regularFormatR);
	$worksheet->write($row, 27, $jumlahTindakan+$Bayar, $regularFormatR);
	$worksheet->write($row, 28, $verf, $regularFormatC);
	$worksheet->write($row, 29, $rwLop['penjamin'], $regularFormatC);
