...
<table id="tblPrint" width="100%" border="0" cellspacing="0" cellpadding="0" align="left" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="2" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase;"><b>Laporan PENDAPATAN <?=$_SESSION['namaP'] ?><br />
  	...
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			<tr style="text-align:center; font-weight:bold;">
			  <td colspan="11" align="left">Tunai</td>
		  </tr>
			<tr style="text-align:center; font-weight:bold;">
				<td width="30" style="border-top:1px solid; border-bottom:1px solid;">No</td>
				<td width="150" style="border-top:1px solid; border-bottom:1px solid;">Tgl Bayar</td>
				<td width="80" style="border-top:1px solid; border-bottom:1px solid;">Jam</td>
				<td width="150" style="border-top:1px solid; border-bottom:1px solid;">No.<br />Kwitansi</td>
                <td width="150" style="border-top:1px solid; border-bottom:1px solid;">Nama Kasir</td>
				<td width="150" style="border-top:1px solid; border-bottom:1px solid;">Tgl Kunjungan</td>
                <td width="120" style="border-top:1px solid; border-bottom:1px solid;">No.RM</td>
				<td width="250" style="border-top:1px solid; border-bottom:1px solid;" align="left">Nama Pasien</td>
				<td width="180" style="border-top:1px solid; border-bottom:1px solid;">Status</td>
				<?php
					$total = array();
					$total2 = array();
					...
					$total['diskon_rs'] = 0;
					$total['diskon_dokter'] = 0; 
					$total2['diskon_rs'] = 0;
					$total2['diskon_dokter'] = 0;
					$total['PPN']=$total2['PPN']=0;
					$total['subtotal']=$total2['subtotal']=0;
				?>
				<td width="130" style="border-top:1px solid; border-bottom:1px solid; padding-right:20px;" align="right">Diskon RS</td>
				<td width="130" style="border-top:1px solid; border-bottom:1px solid; padding-right:20px;" align="right">Diskon Dokter</td>
				<td width="130" style="border-top:1px solid; border-bottom:1px solid; padding-right:20px;" align="right">PPN Nilai</td>
				<td width="130" style="border-top:1px solid; border-bottom:1px solid; padding-right:20px;" align="right">Total</td>
			</tr>
			...
			<?php
				...
				$sql = "SELECT gb.id, gb.no_kwitansi, gb.nm_kasir, SUM(gb.nilai) nilai, gb.nilai nilaiBayar,
							...
							SUM(gb.PPN_NILAI) PPN_NILAI, gb.pegawai_id, gb.pegawai_nama,gb.nilai_diskon,gb.nilai_diskon_dokter
						FROM (SELECT b.id, b.no_kwitansi, 
							bt.tindakan_id, bt.tipe, 
							SUM(bt.nilai) nilai, b.nilai nilaiBayar,
							b.nilai_diskon,b.nilai_diskon_dokter,
						...
						WHERE bt.tipe = 0 AND b.flag = '$flag'
						...
						WHERE bt.tipe = 1 AND b.flag = '$flag'
						  AND DATE(b.disetor_tgl) BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' 
						  /*AND TIME_FORMAT(b.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2'*/
						  {$fKasir} $fKso
						GROUP BY b.id
					...
					while($data = mysql_fetch_array($query)){
						...
						echo "<td align='right' >".number_format($data['nilai_diskon'],0,",",".")."</td>";
						echo "<td align='right' >".number_format($data['nilai_diskon_dokter'],0,",",".")."</td>";
						$total['diskon_rs'] = $total['diskon_rs'] + $data['nilai_diskon'];
						$total['diskon_dokter'] = $total['diskon_dokter'] + $data['nilai_diskon_dokter'];

						echo "<td align='right' >".number_format($data['PPN_NILAI'],0,",",".")."</td>";
						$total['PPN'] = $total['PPN'] + $data['PPN_NILAI'];
						
						$subtotal += $data['PPN_NILAI'] - $data['nilai_diskon'] - $data['nilai_diskon_dokter'];
			...
			<tr>
			  <td height="25" colspan="25" style="text-align:right; font-weight:bold; border-top:1px solid; ">&nbsp;</td>
		  	</tr>
			<tr style="text-align:center; font-weight:bold;">
			  <td colspan="11" align="left">Non Tunai</td>
		  </tr>
			<tr style="text-align:center; font-weight:bold;">
				<td width="30" style="border-top:1px solid; border-bottom:1px solid;">No</td>
                <td width="150" style="border-top:1px solid; border-bottom:1px solid;">Tgl Pulang</td>
				<td width="80" style="border-top:1px solid; border-bottom:1px solid;">Jam</td>
				<td width="150" style="border-top:1px solid; border-bottom:1px solid;">No.<br />Kunjungan</td>
				 <td width="150" style="border-top:1px solid; border-bottom:1px solid;">Nama Kasir</td>
				<td width="150" style="border-top:1px solid; border-bottom:1px solid;">Tgl Kunjungan</td>
				<td width="120" style="border-top:1px solid; border-bottom:1px solid;">No.RM</td>
				<td width="250" style="border-top:1px solid; border-bottom:1px solid;" align="left">Nama Pasien</td>
			...
			<?php
					...
					while($data = mysql_fetch_array($query)){
						...
						foreach($kol as $key => $val){
							echo "<td align='right'>".number_format($data[$key],0,",",".")."</td>";
							$total2[$key] = $total2[$key] + $data[$key];
							//$total3[$key] = ($total2[$key] + $data[$key]) + ($total[$key] + $data[$key]);
							$subtotal += $data[$key];
						}
						
						//$total3= ($total2[$key] + $data[$key]) + ($total[$key] + $data[$key]);
						
						echo "<td align='right' >".number_format(0,0,",",".")."</td>";
						echo "<td align='right' >".number_format(0,0,",",".")."</td>";
						echo "<td align='right' >".number_format($data['PPN_NILAI'],0,",",".")."</td>";
						$total2['PPN'] = $total2['PPN'] + $data['PPN_NILAI'];
						
						$subtotal += $data['PPN_NILAI'];
						echo "<td align='right' style='font-weight:bold;'>".number_format($subtotal,0,",",".")."</td>";
						
						$total2['subtotal'] += $subtotal;
						echo "</tr>";
						$no++;
					}
					/* while($data = mysql_fetch_array($query)){
			...
			<tr style="background:#efefef;">
				...
				<?php
					foreach($total2 as $nilai2){
					//$total_semua+=$nilai+;
						echo "<td align='right' style='border-top:1px solid; font-weight:bold;'>".number_format($nilai2,0,",",".")."</td>";
					}
				?>
			</tr>
			<tr>
			...
			<!--<tr style="text-align:center; font-weight:bold;">
			  <td colspan="11" align="left">Tunai + Non Tunai</td>
		  </tr> -->
			
			<tr style="background:#efefef;">
				<td height="25" colspan="9" style="text-align:right; font-weight:bold; border-top:1px solid; ">TOTAL TUNAI + NON TUNAI&nbsp;</td>
			
				<?php
			
					/*foreach(array_combine($total,$total2) as $nilai => $nilai2) {
				
					
					echo "<td align='right' style='border-top:1px solid; font-weight:bold;'>".number_format($nilai+$nilai2,0,",",".")."</td>";
					}*/
					
					
					 foreach($total as $key => $value) {

						 echo "<td align='right' style='border-top:1px solid; font-weight:bold;'>".number_format($total[$key]+$total2[$key],0,",",".")."</td>";
