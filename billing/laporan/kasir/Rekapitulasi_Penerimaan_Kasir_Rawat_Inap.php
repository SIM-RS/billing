		...
		<table border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
            ...
            <tr style="text-align:center; font-weight:bold;background-color:#CCC;">
				...
				while($kasir_rw=mysql_fetch_array($kueri_kasir)){
					$i++;
					$idKasir[$i]=$kasir_rw['id'];
				?>
                <td colspan="2" style="border-bottom:1px solid; border-right:1px solid;"><?php if($kasir_rw['nama']==''){echo $kasir_rw['id'];} else{echo $kasir_rw['nama'];} ?></td>
                <?php
				}
				?>
            ...
            <?php
			$sql = "SELECT DISTINCT mu.id,mu.nama FROM (SELECT * FROM b_bayar b WHERE b.kasir_id = '$kasir' $fkasir AND b.tgl $period) b
            ...
            <tr>
                ...
                //while($kas=mysql_fetch_array($kueri_kasir2)){
					//bayar tindakan
					$sql = "SELECT IFNULL(SUM(IF (bt.kso_id=1,bt.nilai,0)),0) AS nilai,IFNULL(SUM(IF (bt.kso_id=1,0,bt.nilai)),0) AS iur FROM (SELECT * FROM b_bayar WHERE kasir_id = '$kasir' AND user_act = '".$idKasir[$i]."' AND tgl $period) b
					INNER JOIN b_bayar_tindakan bt ON b.id = bt.bayar_id
					INNER JOIN b_tindakan t ON bt.tindakan_id = t.id
					INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id
