  	...
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			<tr style="text-align:center; font-weight:bold;">
				...
				<td width="12%" style="border-top:1px solid; border-bottom:1px solid;">Nilai</td>
			</tr>
			<?php
					if($nmKasir!=0){
					...
					$sqlNm = "SELECT b_ms_pegawai.id, b_ms_pegawai.nama FROM b_bayar INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_bayar.user_act INNER JOIN b_ms_pegawai_unit ON b_ms_pegawai_unit.ms_pegawai_id = b_ms_pegawai.id WHERE $fKasir AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2' GROUP BY b_ms_pegawai.id ORDER BY b_ms_pegawai.nama";
					//echo $sqlNm."<br>";
			...
			?>
			<tr>
				<td colspan="8" style="padding-left:20px; text-transform:uppercase; text-decoration:underline; font-weight:bold;" height="25"><?php echo $rwNm['nama'];?></td>
			</tr>
			<?php
					$sqlRuang = "SELECT t.id, t.nama, t.inap FROM (SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id  INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2'
					UNION
					SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap FROM b_pelayanan INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2') AS t GROUP BY t.id";
					//echo $sqlRuang."<br>";
			...
			<tr>
				<td>&nbsp;</td>
				<td colspan="7" style="text-transform:uppercase; font-weight:bold;"><?php echo $rwRuang['nama']?></td>
			</tr>
			<?php
					$sqlPas = "SELECT 
					t.pelayanan_id, 
					...
					t.jam, 
					t.tgl 
					FROM (SELECT b_pelayanan.id AS pelayanan_id, b_ms_kso.nama AS kso, b_bayar.nobukti AS kwi, b_ms_pasien.no_rm, b_ms_pasien.nama AS pasien, TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl FROM b_pelayanan INNER JOIN b_ms_pasien ON b_pelayanan.pasien_id = b_ms_pasien.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_kso ON b_ms_kso.id = b_tindakan.kso_id  INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN  '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2' AND b_pelayanan.unit_id = '".$rwRuang['id']."'
					UNION
					SELECT 
					b_pelayanan.id AS pelayanan_id, 
					b_ms_kso.nama AS kso, 
					b_bayar.nobukti AS kwi, 
					b_ms_pasien.no_rm, 
					b_ms_pasien.nama AS pasien, 
					TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, 
					DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl 
					FROM b_pelayanan INNER JOIN b_ms_pasien ON b_pelayanan.pasien_id = b_ms_pasien.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_ms_kso ON b_ms_kso.id = b_tindakan.kso_id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2' AND b_pelayanan.unit_id = '".$rwRuang['id']."') AS t GROUP BY t.pelayanan_id";
					//echo $sqlPas."<br>";
					...
					{
						$qTind = "SELECT SUM(b_bayar_tindakan.nilai) AS jml FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_pelayanan.id='".$rwPas['pelayanan_id']."' AND b_bayar.user_act = '".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2'";
						//echo $qTind."<br>";
						//if ($rwRuang['id']==39) echo $qTind."<br>";
						$sTind = mysql_query($qTind);
						$wTind = mysql_fetch_array($sTind);
						
						$qInap = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_pelayanan INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_pelayanan.id = '".$rwPas['pelayanan_id']."' AND b_bayar_tindakan.tipe = 1 AND b_bayar.user_act = '".$rwNm['id']."' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND TIME_FORMAT(b_bayar.tgl_act,'%H:%i') BETWEEN '$txtJam1' AND '$txtJam2'";
						//echo $qInap."<br>";
			...
			?>
			<tr>
			  <td height="25" colspan="7" style="text-align:right; font-weight:bold; border-top:1px solid; border-bottom:1px solid;">TOTAL&nbsp;</td>
