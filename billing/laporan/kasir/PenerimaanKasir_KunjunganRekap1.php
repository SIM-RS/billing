...
b_bayar_tindakan.kso_id,kso.nama,b_bayar_tindakan.nilai,b_bayar_tindakan.id btinId FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id INNER JOIN b_ms_kso kso ON b_bayar_tindakan.kso_id=kso.id WHERE b_bayar.user_act='".$rwNm['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'
UNION
...
IF((b_bayar_tindakan.kso_id=0 OR ISNULL(b_bayar_tindakan.kso_id)),b_pelayanan.kso_id,b_bayar_tindakan.kso_id) kso_id,IFNULL(kso.nama,b_ms_kso.nama) nama,b_bayar_tindakan.nilai,b_bayar_tindakan.id btinId FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_ms_kso ON b_pelayanan.kso_id=b_ms_kso.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id LEFT JOIN b_ms_kso kso ON b_bayar_tindakan.kso_id=kso.id WHERE b_bayar.user_act='".$rwNm['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."') AS t GROUP BY t.id,t.kso_id ORDER BY t.tgl,t.pasien";
						//echo $sqlPas."<br>";
