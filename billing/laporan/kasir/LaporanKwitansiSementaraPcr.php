...
header('Content-Disposition: attachment; filename="lap_kwitansi_sementara_pcr_umum.xls"');
}
?>
...
<title>.: Laporan Kwitansi Pcr Sementara :.</title>
</head>
...
<table id="tblPrint" width="800" border="0" cellspacing="0" cellpadding="0" align="left" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    <td colspan="2" align="center" height="70" valign="top" style="font-size:14px; text-transform:uppercase;"><b>Laporan Kwitansi Pcr Sementara <br />Periode <?php echo $tglAwal[0]." ".$arrBln[$tglAwal[1]]." ".$tglAwal[2].' s/d '.$tglAkhir[0]." ".$arrBln[$tglAkhir[1]]." ".$tglAkhir[2]?></b></td>
  </tr>
  <tr>
  	...
  	<td colspan="2">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			<?php
					...
					b_ms_pegawai.id, 
					b_ms_pegawai.nama 
					FROM b_total_biaya_pcr bt
					INNER JOIN b_ms_unit ON b_ms_unit.id = 232
					INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = bt.user_id 
					INNER JOIN b_ms_pegawai_unit ON b_ms_pegawai_unit.ms_pegawai_id = b_ms_pegawai.id 
					WHERE $fKasir AND bt.tanggal_bayar BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' GROUP BY b_ms_pegawai.id ORDER BY b_ms_pegawai.nama";
            ...
            <?php
				$sUnit="SELECT 
						...
						WHERE 
						bt.user_id = '".$rwNm['id']."' 
						AND bt.tanggal_bayar BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'
			...
			<?php
					$sqlPas = "SELECT
					TIME_FORMAT(b.tanggal_act,'%H:%i') AS jam, 
					DATE_FORMAT(b.tanggal_bayar,'%d-%m-%Y') AS tgl,
					b.no_kwitansi as kwi,
					ps.no_rm,
					ps.nama as pasien,
					kso.nama as kso,
					b.total_pcr as nilai
					FROM
					b_total_biaya_pcr b
					INNER JOIN b_kunjungan k ON k.id= b.id_pelayanan
					INNER JOIN b_ms_pasien ps ON ps.id=k.pasien_id
					INNER JOIN b_ms_kso kso ON kso.id= 1
					WHERE
					b.user_id = '".$rwNm['id']."'
					AND b.tanggal_bayar BETWEEN  '".$tglAwal2."' AND '".$tglAkhir2."' 
					ORDER BY b.id";
