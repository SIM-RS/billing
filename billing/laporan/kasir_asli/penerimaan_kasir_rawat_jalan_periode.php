...
INNER JOIN b_pelayanan p ON t.pelayanan_id = p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id ORDER BY mu.id";
            ...
            <?php
				/*$sql2="SELECT b.tgl tanggal,DATE_FORMAT(b.tgl_act,'%H:%i') jam,b.nobukti,mp.no_rm,mp.nama pasien,kso.id kso_id,kso.nama kso,p.id pelayanan_id,
...
SUM(IF((mt.id<>2363 AND mt.id<>10 AND mt.id<>12),bt.nilai,0)) tind
FROM (SELECT * FROM b_bayar WHERE user_act='".$rwKasir['id']."' AND tgl $period) b 
INNER JOIN b_bayar_tindakan bt ON b.id=bt.bayar_id INNER JOIN b_tindakan t ON bt.tindakan_id=t.id
...
WHERE p.unit_id='".$poli['id']."' AND bt.tipe=0
...
INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_kunjungan k ON p.kunjungan_id=k.id
INNER JOIN b_ms_pasien mp ON k.pasien_id=mp.id INNER JOIN b_ms_kso kso ON t.kso_id=kso.id
WHERE p.unit_id='".$poli['id']."' AND bt.tipe=0 AND t.tipe_pendapatan=0 AND (t.bayar_pasien>0 OR (t.bayar_pasien=0 AND p.jenis_kunjungan<>3 AND t.tgl<=b.tgl))
GROUP BY mp.id,p.id,t.kso_id ORDER BY p.id";
				$no = 0;
				$tot_konsul=0;
				$tot_tindakan=0;
