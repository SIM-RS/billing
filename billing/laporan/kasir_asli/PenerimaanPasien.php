    ...
    <td>
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			<tr style="font-weight:bold; text-align:center;">
				...
				<td width="10%" style="border-bottom:1px solid; border-top:1px solid;">Obat</td>
			...
			<?php
					...
					$sqlUn = "SELECT t.id, t.nama, t.inap FROM (SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_bayar.user_act INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fTmp $fKso AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'
					UNION
					SELECT b_ms_unit.id, b_ms_unit.nama, b_ms_unit.inap FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_bayar.user_act INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE $fTmp $fKso AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."') AS t GROUP BY t.id";
			...
			?>
			<tr>
			  <td colspan="10" style="text-transform:uppercase; text-decoration:underline; font-weight:bold; padding-left:10px"><?php echo $rwUn['nama']?></td>
		  	</tr>
		  	<?php
					$sqlTmp = "SELECT t.id, t.nama FROM (SELECT b_ms_pegawai.id, b_ms_pegawai.nama FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_bayar.user_act INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE b_pelayanan.unit_id = '".$rwUn['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 $fKso AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'
					UNION 
					SELECT b_ms_pegawai.id, b_ms_pegawai.nama FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_bayar.user_act INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id WHERE b_pelayanan.unit_id = '".$rwUn['id']."' AND b_bayar_tindakan.nilai<>0 AND b_bayar_tindakan.tipe=1 $fKso AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."') AS t GROUP BY t.id";
			...
			?>
			<tr>
				<td colspan="10" style="text-transform:uppercase; text-decoration:underline; font-weight:bold; padding-left:20px"><?php echo $rwTmp['nama']?></td>
			</tr>
			<?php
					$sqlKso = "SELECT t.id, t.nama FROM (SELECT b_ms_kso.id, b_ms_kso.nama FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_ms_kso ON b_ms_kso.id = b_tindakan.kso_id WHERE b_pelayanan.unit_id = '".$rwUn['id']."' AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.user_act = '".$rwTmp['id']."' $fKso
					UNION 
					SELECT b_ms_kso.id, b_ms_kso.nama FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_unit ON b_ms_unit.id = b_pelayanan.unit_id INNER JOIN b_ms_kso ON b_ms_kso.id = b_tindakan.kso_id WHERE b_pelayanan.unit_id = '".$rwUn['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.user_act = '".$rwTmp['id']."' $fKso) AS t GROUP BY t.id ";
			...
			?>
			<tr>
				<td colspan="10" style="text-transform:uppercase; font-weight:bold; padding-left:30px"><?php echo $rwKso['nama']?></td>
			</tr>
			<?php
					$sql = "SELECT t.id,t.pelayanan_id, t.tgl, t.tglb, t.jam, t.kwi, t.no_rm, t.nama,  t.nilai, t.tindakan_id FROM (SELECT DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl,b_bayar.tgl tglb, TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, b_bayar.nobukti AS kwi, b_ms_pasien.id,b_ms_pasien.no_rm, b_ms_pasien.nama, b_pelayanan.id AS pelayanan_id, b_bayar.nilai, b_bayar_tindakan.tindakan_id FROM b_pelayanan INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_pasien ON b_ms_pasien.id = b_pelayanan.pasien_id WHERE b_pelayanan.unit_id = '".$rwUn['id']."'  AND b_bayar_tindakan.tipe=0 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.user_act = '".$rwTmp['id']."' AND b_tindakan.kso_id = '".$rwKso['id']."'
					UNION 
					SELECT DATE_FORMAT(b_bayar.tgl,'%d-%m-%Y') AS tgl,b_bayar.tgl tglb, TIME_FORMAT(b_bayar.tgl_act,'%H:%i') AS jam, b_bayar.nobukti AS kwi, b_ms_pasien.id,b_ms_pasien.no_rm, b_ms_pasien.nama, b_pelayanan.id AS pelayanan_id, b_bayar.nilai, b_bayar_tindakan.tindakan_id FROM b_pelayanan INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id = b_pelayanan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id INNER JOIN b_ms_pasien ON b_ms_pasien.id = b_pelayanan.pasien_id WHERE b_pelayanan.unit_id = '".$rwUn['id']."' AND b_bayar_tindakan.tipe=1 AND b_bayar_tindakan.nilai<>0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' AND b_bayar.user_act = '".$rwTmp['id']."' AND b_tindakan.kso_id = '".$rwKso['id']."') AS t GROUP BY t.pelayanan_id,t.tglb";
					//echo $sql."<br>";
					$rs = mysql_query($sql);
					$no = 1;
					$sub1 = 0;
					...
					{ 
						...
						$qObat = "SELECT SUM(b_bayar_tindakan.nilai) AS nilai FROM b_pelayanan INNER JOIN $dbapotek.a_penjualan ap ON ap.NO_KUNJUNGAN = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = ap.ID INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_pelayanan.id = '".$rw['pelayanan_id']."' AND b_bayar_tindakan.tipe = 2 AND b_bayar.user_act = '".$rwTmp['id']."' AND b_bayar.tgl = '".$rw['tglb']."'";
						//echo $qInap."<br>";
			...
			<tr>
				...
				<td style="text-align:right; padding-right:10px;"><?php echo number_format($jmlObat,0,",",".")?></td>
				<td style="text-align:right; padding-right:10px;"><?php echo number_format($rw1['nilai']+$jmlObat+$wTind['nilai'],0,",",".")?></td>
			</tr>
			<?php
					$no++;
					$sub1 += $rw1['nilai'];
					$sub2 += $wTind['nilai'];
					$subObat += $jmlObat;
					$sub3 += $rw1['nilai']+$jmlObat+$wTind['nilai'];
					}
			?>
			<tr>
				<td>&nbsp;</td>
				...
				<td style="text-align:right; padding-right:10px; border-top:1px solid;"><?php echo number_format($subObat,0,",",".")?></td>
			...
			<tr>
				...
				<td style="text-align:right; border-top:1px solid; font-weight:bold; padding-right:10px;"><?php echo number_format($tObat,0,",",".")?></td>
			...
			<tr>
				...
				<td style="text-align:right; border-top:1px solid; font-weight:bold; padding-right:10px;"><?php echo number_format($totObat,0,",",".")?></td>
			...
			<tr style="font-size:12px;">
			  <td height="25" colspan="6" style="text-align:right; border-top:1px solid; border-bottom:1px solid; font-weight:bold;">GRAND TOTAL&nbsp;</td>
			  ...
			  <td style="text-align:right; border-top:1px solid; border-bottom:1px solid; font-weight:bold; padding-right:10px;"><?php echo number_format($ttotObat,0,",",".")?></td>
