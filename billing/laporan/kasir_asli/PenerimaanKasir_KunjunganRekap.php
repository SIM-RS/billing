		...
		<td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				...
				<?php
						...
						$sqlNm = "SELECT b_ms_pegawai.id, b_ms_pegawai.nama FROM b_bayar INNER JOIN b_ms_pegawai ON b_ms_pegawai.id = b_bayar.user_act INNER JOIN b_ms_pegawai_unit ON b_ms_pegawai_unit.ms_pegawai_id = b_ms_pegawai.id WHERE $fKasir AND b_bayar.kasir_id = '".$kasir."' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."' GROUP BY b_ms_pegawai.id ORDER BY b_ms_pegawai.nama";
...
b_tindakan.kso_id,kso.nama,b_bayar_tindakan.nilai,b_bayar_tindakan.id btinId FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan ON b_tindakan.pelayanan_id=b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id INNER JOIN b_ms_kso kso ON b_tindakan.kso_id=kso.id WHERE b_bayar.user_act='".$rwNm['id']."' AND b_bayar.kasir_id = '".$kasir."' AND b_bayar_tindakan.tipe=0 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'
UNION
...
b_tindakan_kamar.kso_id,kso.nama,b_bayar_tindakan.nilai,b_bayar_tindakan.id btinId FROM b_ms_pasien INNER JOIN b_kunjungan ON b_kunjungan.pasien_id=b_ms_pasien.id INNER JOIN b_pelayanan ON b_pelayanan.kunjungan_id=b_kunjungan.id INNER JOIN b_tindakan_kamar ON b_tindakan_kamar.pelayanan_id=b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id=b_tindakan_kamar.id INNER JOIN b_bayar ON b_bayar.id=b_bayar_tindakan.bayar_id INNER JOIN b_ms_kso kso ON b_tindakan_kamar.kso_id=kso.id WHERE b_bayar.user_act='".$rwNm['id']."' AND b_bayar.kasir_id = '".$kasir."' AND b_bayar_tindakan.tipe=1 AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."') AS t GROUP BY t.id,t.kso_id ORDER BY t.tgl,t.pasien";
						//echo $sqlPas."<br>";
						...
						while($rwPas = mysql_fetch_array($rsPas))
						{
							...
							$qObat = "SELECT IFNULL(SUM(b_bayar_tindakan.nilai),0) AS nilai FROM b_pelayanan INNER JOIN $dbapotek.a_penjualan ap ON ap.NO_KUNJUNGAN = b_pelayanan.id INNER JOIN b_bayar_tindakan ON b_bayar_tindakan.tindakan_id = ap.ID INNER JOIN b_bayar ON b_bayar.id = b_bayar_tindakan.bayar_id WHERE b_pelayanan.kunjungan_id = '".$rwPas['kunjungan_id']."' AND b_bayar.id='".$rwPas["id"]."' AND b_bayar_tindakan.tipe = 2 AND b_bayar.user_act = '".$rwNm['id']."' AND b_bayar.kasir_id = '".$kasir."' AND b_bayar.tgl BETWEEN '".$tglAwal2."' AND '".$tglAkhir2."'";
				...
				<tr valign="bottom">
				    ...
				    <td align="right"><?php echo number_format($rwPas['nilai']+$wObat['nilai'],0,",",".");?></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				...
				<?php
						$no++;
						$stot = $stot + $rwPas['nilai']+$wObat['nilai'];
						}
				?>
				<tr>
