...
<table id="tblPrint" width="800" border="0" cellspacing="0" cellpadding="0" align="left" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
  ...
  <tr>
    ...
    Laporan Penerimaan Kasir Rawat Jalan<br />
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
			...
			FROM (SELECT *
			  ...
			  INNER JOIN b_ms_unit mu
				ON p.unit_id = mu.id
			ORDER BY mu.nama";*/
			/*echo $sql="SELECT * FROM (SELECT id,nama FROM b_ms_unit WHERE parent_id=1 AND aktif=1 ORDER BY nama) t
UNION
...
ORDER BY mu.nama) t2";*/
			...
			$sql="SELECT abc.nama, abe.nama AS nm__unit, SUM(b.nilai) AS tagihan, (SELECT COUNT(a1.pasien_id) FROM b_pelayanan a1
INNER JOIN b_kunjungan b1 ON a1.kunjungan_id = b1.id
INNER JOIN b_bayar ab1 ON ab1.kunjungan_id = b1.id
WHERE ab1.tgl $period AND ab1.user_act = a.user_act AND a1.unit_id = ab.unit_id) AS jml_pasien
FROM b_bayar a INNER JOIN b_bayar_tindakan b ON a.id = b.bayar_id
INNER JOIN b_tindakan c ON b.tindakan_id = c.id
INNER JOIN b_pelayanan ab ON ab.id = c.pelayanan_id
INNER JOIN b_ms_pegawai abc ON a.user_act = abc.id
INNER JOIN b_ms_unit abe ON ab.unit_id = abe.id
INNER JOIN b_kunjungan bk ON a.kunjungan_id = bk.id
WHERE a.tgl $period
GROUP BY a.user_act, ab.unit_id";
            ...
            <tr style="text-align:center;">
				...
				<td width="10%" style="border-right:1px solid; border-bottom:1px solid;" align="left">&nbsp;<?php echo $poli['nm__unit']; ?></td>
               	<td width="10%" style="border-right:1px solid; border-bottom:1px solid;" align="right">&nbsp;<?php echo $poli['nama']; ?></td>
                <td width="8%" style="border-right:1px solid; border-bottom:1px solid;" align="center"><?php echo $poli['jml_pasien'];$tpasien+=$poli['jml_pasien']; ?>&nbsp;</td>
              <td width="10%" style="border-right:1px solid; border-bottom:1px solid;" align="right"><?php echo number_format($poli['tagihan'],0,',','.');$tUang+=$poli['tagihan']; ?>&nbsp;</td>
			</tr>
            <?php
			$no++;
			}
            ...
            <tr style="text-align:center; font-weight:bold;">
              ...
              <td colspan="3" style="border-right:1px solid; border-bottom:1px solid;" align="right">Jumlah&nbsp;</td>
              <?php 
			  if ($jml_kasir==0){
			  	$totJmlPx=0;
			  ?>
             <!-- <td style="border-right:1px solid; border-bottom:1px solid;" align="right">0&nbsp;</td>-->
			  ...
			  ?>
              <td style="border-right:1px solid; border-bottom:1px solid;" align="center"><?php echo $tpasien; ?></td>
              <td style="border-right:1px solid; border-bottom:1px solid;" align="right"><?php echo number_format($tUang,0,',','.'); ?>&nbsp;</td>
            </tr>
		</table>
	</td>
  </tr>
