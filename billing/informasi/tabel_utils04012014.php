...
if ($grd1=="true") {
    ...
    $sql = "select * from(select pel.id AS id_pelayanan,k.id,date_format(pel.tgl, '%d-%m-%Y') as tgl, p.no_rm, p.nama, p.sex
		...
		IF(pel.dilayani=1,'Sudah','Belum') AS status_dilayani
...
if ($grd1=="true") {
    while ($rows=mysql_fetch_array($rs)) {
		...
		if($_REQUEST['inap']=='1'){
			...
			$dt.=$rows["id"].chr(3).$i.chr(3).$rows['jns_layanan'].chr(3).$rows["tmp_layanan"].chr(3).$rKmr["nama"]."&nbsp;".chr(3).$rows["tgl_masuk"].chr(3).$rows["tgl_pulang"].chr(3).$rows["no_rm"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["statuspas"].chr(3).$rows['status_pulang'].chr(6);
		}
		else{
        	$dt.=$rows["id"].chr(3).$i.chr(3).$rows['jns_layanan'].chr(3).$rows["tmp_layanan"].chr(3).$rows["tgl_masuk"].chr(3).$rows["tgl_pulang"].chr(3).$rows["no_rm"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["statuspas"].chr(3).$rows['status_dilayani'].chr(6);
		}
    }
}
elseif ($grdRiw1=="true") {
