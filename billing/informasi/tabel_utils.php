...
$grdKmr = $_GET['grdkmr'];
$grdKunjAktif=$_GET['grdKunjAktif'];
$is_switching = $_GET['is_switching'];
$jns = $_GET['jns'];
//====================================================================
...
if ($grd1=="true") {
	...
	if($jns <> 27)
	{
		...
		, date_format(k.tgl_pulang, '%d-%m-%Y') as tgl_pulang, if(pulang=1,'Sudah','Belum') as status_pulang,
		...
		where $stsPul $fUnit $fSts k.tgl between '$tglM' and '$tglS' AND pel.flag = '$flag') as tab1 $filter order by $sorting";	
		//penambahan Flag, untuk menampilkan data sesuai flag
	}else{
		$sql = "select * from(select concat(date_format(k.tgl, '%Y-%m-%d '),date_format(k.tgl_act, '%H:%i:%s')) tgldesc, pel.id AS id_pelayanan,k.id,date_format(pel.tgl, '%d-%m-%Y') as tgl, p.no_rm, p.nama, p.sex
		, p.alamat, if(k.isbaru = 0, 'Lama', 'Baru') as kunjungan, kso.nama as statuspas
		, uj.nama as jns_layanan, ut.nama as tmp_layanan, concat(date_format(k.tgl, '%d-%m-%Y '),date_format(k.tgl_act, '%H:%i')) as tgl_masuk
	...
	if($_REQUEST['sts']!=0 && $_REQUEST['sts']!="")
	{
		if($_REQUEST['sts'] == 4 || $_REQUEST['sts'] == 6)
		{
			//$nSts = "(6 OR k.kso_id=4)";
			$fSts = " and (k.kso_id= 6 OR k.kso_id=4)";
		}else{
			//$nSts = $_REQUEST['sts'];
			$fSts = " and k.kso_id=".$_REQUEST['sts'];
		}
		//$fSts = " and k.kso_id=".$nSts;
	}
	if($_REQUEST['tmp']!='0' && $_REQUEST['tmp']!=''){
	...
	if($jns == 27)
	{
		...
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.flag = '$flag' and k.tgl between '$tglM' and '$tglS' $fSts) as jml, 
		...
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.flag = '$flag' and k.tgl between '$tglM' and '$tglS' $fSts and pel.batal = 0 and k.pulang = 1) as sudah";
		
		$sql2 = "select count(k.id) as batal
		from b_pelayanan pel inner join b_ms_unit u on pel.unit_id_asal = u.id
		inner join b_kunjungan k on pel.kunjungan_id= k.id
		inner join b_ms_unit ut on ut.id = pel.unit_id
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.flag = '$flag' and k.tgl between '$tglM' and '$tglS' $fSts and pel.batal = 1";
	}else{
		$sql = "select (select count(k.id) as jml
		from b_pelayanan pel inner join b_ms_unit u on pel.unit_id_asal = u.id
		inner join b_kunjungan k on pel.kunjungan_id= k.id
		...
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.flag = '$flag' and k.tgl between '$tglM' and '$tglS' $fSts and pel.batal = 0 AND k.pulang = 1 /* AND pel.checkoutact IS NOT NULL */) as sudah";
    ...
    echo $total.'|'.$sudah.'|'.$belum."|".$batal;
    mysql_free_result($rs);
    mysql_close($konek);
    return;
}
else if($grdKunjAktif=="true") {

  $sql = "select *from (SELECT pel.id pel_id, DATE_FORMAT(k.tgl, '%d-%m-%Y') AS tgl, p.`no_rm`, p.`nama`,p.`alamat`, u2.nama unit_asal, u.`nama` unit_sekarang,kso.`nama` kso FROM (SELECT * FROM b_pelayanan AS a
	INNER JOIN (SELECT MAX(id)  AS ids FROM b_pelayanan  GROUP BY kunjungan_id) b
	ON a.id = b.ids ORDER BY a.id DESC) pel
	INNER JOIN b_kunjungan k ON pel.kunjungan_id=k.`id`
	INNER JOIN b_ms_unit u ON pel.unit_id = u.`id`
	INNER JOIN b_ms_unit u2 ON pel.unit_id_asal = u2.`id`
	INNER JOIN b_ms_kso kso ON k.kso_id = kso.id
	INNER JOIN b_ms_pasien p ON k.pasien_id = p.id
	WHERE k.`tgl_pulang` IS NULL and k.flag = '$flag') tab1 $filter /*order by $sorting*/";
    ...
    $sql = "select * from (SELECT k.id AS idkunj, k.pasien_id, k.unit_id, u.nama AS tmptlay, k.tgl, k.kso_id, kso.nama AS penjamin, date_format(k.tgl_pulang,'%Y-%m-%d') tgl_pulang
            ...
            WHERE k.pasien_id = '".$_REQUEST['idPas']."' AND k.hapus=0 and k.flag = '$flag') as gab $filter order by $sorting";
}
else if($grd3=="true") {
...
$filter and b_ms_tindakan.klasifikasi_id='".$_REQUEST['cmbKlasi']."' AND b_ms_tindakan_unit.ms_unit_id='".$_REQUEST['tmp']."' ORDER BY b_ms_tindakan.nama ";
   ...
   $sql = "select * from (SELECT pl.id AS pelId, pl.kunjungan_id, k.pulang, pl.pasien_id, pl.jenis_layanan, pl.unit_id, pl.dilayani, u.inap, u.nama AS tmptlay, pl.tgl, pl.dokter_id, p.nama AS dokter, pl.kelas_id, kl.nama, date_format(k.tgl_pulang,'%d-%m-%Y') as tgl_pulang
		  ...
		  inner join b_kunjungan k on pl.kunjungan_id = k.id
            WHERE pl.kunjungan_id = '".$_REQUEST['idKunj']."' AND pl.hapus=0 and k.flag = '$flag') as gab $filter order by $sorting";
}
else if($grdRiw3=="true") {
   $sql = "select * from (SELECT pl.kunjungan_id, t.id AS tindakan_id, pl.id AS pelayanan_id, mt.nama AS tind, mk.nama AS kelas, mp.nama AS dokter, t.qty, t.biaya*t.qty AS biaya, t.bayar, t.lunas AS STATUS, t.ket, pl.no_lab, t.tgl FROM b_pelayanan pl INNER JOIN b_tindakan t ON t.pelayanan_id = pl.id INNER JOIN b_ms_tindakan_kelas mtk ON mtk.id = t.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan mt ON mt.id = mtk.ms_tindakan_id INNER JOIN b_ms_kelas mk ON mtk.ms_kelas_id = mk.id LEFT JOIN b_ms_pegawai mp ON mp.id = t.user_id WHERE t.pelayanan_id = '".$_REQUEST['idPel']."' and pl.flag = '$flag') as gab $filter order by $sorting";
}
else if($grdPel=="true") {
...
if ($grd1=="true") {
    while ($rows=mysql_fetch_array($rs)) {
		...
		$warna = ($rows['status_pulang'] == "Sudah" ? "blue" : "#000");
		...
		if($_REQUEST['inap']=='1'){
			...
			$dt	.= $rows["id"].
					chr(3)."<font style='color:{$warna}'>".$i."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows['jns_layanan']."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["tmp_layanan"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rKmr["nama"]."&nbsp;"."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["tgl_masuk"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["tgl_pulang"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["no_rm"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["nama"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["alamat"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["statuspas"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows['status_pulang']."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows['kunjungan']."</font>".
					chr(6);
		}
		else{
        	$dt	.= $rows["id"].
					...
					chr(3)."<font style='color:{$warna}'>".$rows["alamat"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows["statuspas"]."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows['status_dilayani']."</font>".
					chr(3)."<font style='color:{$warna}'>".$rows['kunjungan']."</font>".
					chr(6);
		}
    }
...
elseif($grdKunjAktif=="true") {
    ...
    while ($rows=mysql_fetch_array($rs)) {
        $i++;
        $dt.=$rows["pel_id"].chr(3).$i.chr(3).$rows["tgl"].chr(3).$rows["no_rm"].chr(3).$rows["unit_asal"].chr(3).$rows["unit_sekarang"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["kso"].chr(6);
    }
}
...
elseif($grdKmr == 'true'){
    while($rows = mysql_fetch_array($rs)){
		$i++;
        $dt .= $rows['id'].'|'.$rows['unit_id'].'|'.$rows['kelas_id'].chr(3).$i.chr(3).$rows['kamar'].chr(3).$rows['kelas'].chr(3).number_format($rows['tarip'],0,',','.').chr(3).$rows['jumlah_tt'].chr(3).$rows['total_tt'].chr(3).$rows['terpakai'].chr(3).$rows['kosong'].chr(6);
    }
} elseif ($_REQUEST['getListKamar']) {
	$sql = "SELECT
		kmr.id,
		kmr.nama 
	FROM
		b_ms_kamar kmr
		INNER JOIN b_ms_kamar_tarip kt ON kmr.id = kt.kamar_id
		INNER JOIN b_ms_kelas kls ON kls.id = kt.kelas_id 
	WHERE
		kmr.aktif = 1 
		AND kt.unit_id = '".$_REQUEST['unit_id']."'
		AND kmr.id NOT IN ('".$_REQUEST['id_kamar_kecuali']."')
	";

	$isiOption = "";
	$rs=mysql_query($sql);
	while ($rows = mysql_fetch_array($rs)) {
		$isiOption .= '<option value="'.$rows['id'].'">'.$rows['nama'].'</option>';
	}
	echo ($isiOption);
}
