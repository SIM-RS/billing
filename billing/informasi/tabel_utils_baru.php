...
if ($grd1=="true") {
    ...
    if($_REQUEST['stsPul']=="0")
        $stsPul=" pulang=".$_REQUEST['stsPul']." AND k.tgl BETWEEN '$tglM' and '$tglS' AND";
	if($_REQUEST['stsPul']=="")
		$tglPlng = " AND k.tgl BETWEEN '$tglM' and '$tglS'";
	if($_REQUEST['stsPul']=="1")
        $stsPul=" (k.pulang=1 AND DATE(k.tgl_pulang) BETWEEN '$tglM' and '$tglS' OR (pel.dilayani=2 AND DATE(pel.tgl_krs) BETWEEN '$tglM' and '$tglS')) AND";
    if($_REQUEST['tmp']!="0")
        $fTmp="  b_kunjungan.kso_id=".$_REQUEST['sts']." AND ".$stsPul." b_pelayanan.jenis_layanan=".$_REQUEST['jns']." AND b_pelayanan.unit_id=".$_REQUEST['tmp']." AND ";
	if($_REQUEST['sts']!=0)
    ...
    $sql = "select * from(select k.id,date_format(pel.tgl, '%d-%m-%Y') as tgl, p.no_rm, p.nama, p.sex
		, p.alamat, if(k.isbaru = 0, 'Lama', 'Baru') as kunjungan, kso.nama as statuspas
		, uj.nama as jns_layanan, ut.nama as tmp_layanan, date_format(k.tgl, '%d-%m-%Y') as tgl_masuk
		, date_format(tgl_pulang, '%d-%m-%Y') as tgl_pulang, pulang
		...
		where $stsPul $fUnit $fSts $tglPlng) as tab1 $filter order by $sorting";
}
else if($grd1 == 'getVal'){
...
WHERE b_ms_tindakan.klasifikasi_id='".$_REQUEST['cmbKlasi']."' AND b_ms_tindakan_unit.ms_unit_id='".$_REQUEST['tmp']."' ORDER BY b_ms_tindakan.nama ";
}
...
if ($grd1=="true") {
    while ($rows=mysql_fetch_array($rs)) {
        ...
        $dt.=$rows["id"].chr(3).$i.chr(3).$rows['jns_layanan'].chr(3).$rows["tmp_layanan"].chr(3).$rows["tgl_masuk"].chr(3).$rows["tgl_pulang"].chr(3).$rows["no_rm"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(3).$rows["statuspas"].chr(3).$pul.chr(6);
    }
}
elseif ($grdRiw1=="true") {
    while ($rows=mysql_fetch_array($rs)) {
...
elseif($grdKmr == 'true'){
    while($rows = mysql_fetch_array($rs)){
        $dt .= $rows['id'].'|'.$rows['unit_id'].'|'.$rows['kelas_id'].chr(3).$rows['kode'].chr(3).$rows['namakamar'].chr(3).$rows['kodekelas'].chr(3).$rows['namakelas'].chr(3).$rows['jumlah_tt'].chr(3).$rows['jumlah_tt_b'].chr(3).$rows['jumlah_kosong'].chr(3).$rows['tarip'].chr(6);
    }
}
