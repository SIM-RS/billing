            ...
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
                 <tr>
                      ...
                      <td width="100%" align="center" valign="top" colspan="2" >
                          ...
                          <div id="gridboxRiw2" style="width:800px; height:150px; background-color:white; overflow:hidden;"></div>
                          <div id="pagingRiw2" style="width:800px;"></div>
                      </td>
                 </tr>
            </table>
          </td>
...
<script>
	...
	function rincian(){
		...
		if (sisip[2]=="1") url="../unit_pelayanan/RincianTindakan.php?idKunj="+sisip[1]+"&idPel="+document.getElementById('txtIdPel').value+"&idUser="+<?php echo $userId; ?>+"&inap=1";
        ...
        switch(item){
            case 'Kunjungan':
                ...
                if(confirm("Data kunjungan "+pasien+" ke "+kunjungan+" akan dihapus.\nAnda yakin?")){
                    ...
                    Request("tabel_utils_rad.php?act=cekTindakan&pelayanan_id="+hidden[2],'spanRes','','GET'
                            ,function(){
                                if(document.getElementById('spanRes').innerHTML <= 0){
                                    /*alert("tabel_utils_rad.php?act=hapusPel&idPel="+hidden[2]+"&grdRiw2=true&idKunj="+sisip[1]+"&idPas="+sisip[0]+"&filter="+riw2.getFilter()+"&sorting="+riw2.getSorting()+"&page="+riw2.getPage());*/
                                    riw2.loadURL("tabel_utils_rad.php?act=hapusPel&idPel="+hidden[2]+"&grdRiw2=true&idKunj="+sisip[1]+"&idPas="+sisip[0]+"&filter="+riw2.getFilter()+"&sorting="+riw2.getSorting()+"&page="+riw2.getPage(),'','GET');
                                }
                                else{
            ...
            case 'Tindakan':
                ...
                if(confirm("Data tindakan tindakan "+tindakan+" pada "+pasien+" akan dihapus.\nAnda yakin?")){
                    ...
                    Request("tabel_utils_rad.php?act=cekBayar&tindakan_id="+hidden[2],'spanRes','','GET'
                            ,function(){
                                if(document.getElementById('spanRes').innerHTML <= 0){
                                    riw3.loadURL("tabel_utils_rad.php?act=hapusTin&tindakan_id="+hidden[2]+"&grdRiw3=true&idPel="+sisipan[2]
	...
	function goFilterAndSort(grd){
		//alert(grd);
		if (grd=="gridboxRiw1"){			
			riw1.loadURL("tabel_utils_rad.php?grdRiw1=true&idKunj="+dataPasien[1]+"&idPas="+dataPasien[0]+"&filter="+riw1.getFilter()+"&sorting="+riw1.getSorting()+"&page="+riw1.getPage(),"","GET");
		}else if (grd=="gridboxRiw2"){
			var sisip = riw1.getRowId(riw1.getSelRow()).split("|");			
			riw2.loadURL("tabel_utils_rad.php?grdRiw2=true&idKunj="+sisip[1]+"&idPas="+sisip[0]+"&filter="+riw2.getFilter()+"&sorting="+riw2.getSorting()+"&page="+riw2.getPage(),"","GET");
		}else if (grd=="gridboxRiw3"){
			var sisipan = riw2.getRowId(riw2.getSelRow()).split("|");			
			riw3.loadURL("tabel_utils_rad.php?grdRiw3=true&idPel="+sisipan[2]+"&filter="+riw3.getFilter()+"&sorting="+riw3.getSorting()+"&page="+riw3.getPage(),"","GET");
		}
	}
	...
	riw1.baseURL("tabel_utils_rad.php?grdRiw1=true");
	riw1.Init(); */
	...
	riw2.baseURL("tabel_utils_rad.php?grdRiw2=true");
	riw2.Init();
	...
	riw3.baseURL("tabel_utils_rad.php?grdRiw3=true");
	riw3.Init();
     ...
     function gotoServe(){
        if(riw2.getRowId(riw2.getSelRow()) != ''){
            ...
            if(sisip[5] == 0 && sisip[6] == 1){
                ...
                Request("tabel_utils_rad.php?act=accessVerify&user_id=<?php echo $userId;?>&unit_id="+sisip[3],'spanRes','','GET'
                        ,function(){
                            if(document.getElementById('spanRes').innerHTML > 0){
                                ...
                                document.getElementById('sentPar').value = no_rm+'*|*'+sisip[4]+'*|*'+sisip[3]+'*|*'+tgl+'*|*'+sisip[5]+'*|*'+1;
	...
	//alert(how);
		if(how=='show'){
			if(feel.which==13  && keyword!=stuff){
				...
				Request('pasien_list_rad.php?act=cari&keyword='+stuff,'div_pasien','','GET');
				RowIdx=0;
			}
	...
	function setPasien(val)
	{
		...
		fSetValue(window,p);
		document.getElementById('div_pasien').style.display='none';
		riw2.loadURL("tabel_utils_rad.php?grdRiw2=true&idKunj="+dataPasien[1]+"&idPas="+dataPasien[0],"","GET");
		setDisable(1);
	}
	...
	function ambilDataRiw1()
	{
		...
		fSetValue(window,p);		
		//alert("tabel_utils_rad.php?grdRiw2=true&idKunj="+sisip[1]+"&idPas="+sisip[0]);
		riw2.loadURL("tabel_utils_rad.php?grdRiw2=true&idKunj="+sisip[1]+"&idPas="+sisip[0],"","GET");
	...
	function ambilDataRiw2()
	{
		if(actx == 'Kunjungan'){
		    var sisip = riw1.getRowId(riw1.getSelRow()).split('|');
		    Request("tabel_utils_rad.php?act=cekPelayanan&kunjungan_id="+sisip[1],'spanRes','','GET'
			,function(){
			    if(document.getElementById('spanRes').innerHTML <= 0){
                    riw1.loadURL("tabel_utils_rad.php?act=hapusKun&grdRiw1=true&idKunj="+sisip[1]
				    +"&idPas="+dataPasien[0]+"&filter="+riw1.getFilter()+"&sorting="+riw1.getSorting()+"&page="+riw1.getPage(),'','GET');
			    }
			    /*else{
		...
		fSetValue(window,p);
		//alert("tabel_utils_rad.php?grdRiw3=true&idPel="+sisipan[2]+"&unit="+sisipan[3]);
		riw3.loadURL("tabel_utils_rad.php?grdRiw3=true&idPel="+sisipan[2]+"&unit="+sisipan[3],"","GET");
	}
     ...
     function ambilAkses(){
        ...
        if("<?php echo $_POST['hidKunId'];?>" != ''){
            Request('pasien_list_rad.php?act=kiriman&kunjungan_id=<?php echo $_POST['hidKunId'];?>','div_pasien','','GET',setPasien);
            //document.getElementById('NoRm').value = "<?php echo $_POST['hidRM'];?>";
            //document.getElementById('NoRm').focus();
        }
     }
     Request("tabel_utils_rad.php?act=getAkses&user_id=<?php echo $userId;?>",'spanRes','','GET',ambilAkses);
</script>
</html>
