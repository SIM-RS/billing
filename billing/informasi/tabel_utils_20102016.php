...
if ($grd1=="true") {
	...
	if($jns == 27)
	{
		...
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.tgl between '$tglM' and '$tglS' $fSts and pel.batal = 0 and k.pulang = 1) as sudah";
		...
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.tgl between '$tglM' and '$tglS' $fSts and pel.batal = 1";
	}else{
		$sql = "select (select count(k.id) as jml
		from b_pelayanan pel inner join b_ms_unit u on pel.unit_id_asal = u.id
		inner join b_kunjungan k on pel.kunjungan_id= k.id
		...
		inner join b_ms_unit uj on uj.id = ut.parent_id
		inner join b_ms_kso kso on k.kso_id = kso.id
		where $fUnit and k.tgl between '$tglM' and '$tglS' $fSts and pel.batal = 0 AND pel.checkoutact IS NOT NULL) as sudah";
