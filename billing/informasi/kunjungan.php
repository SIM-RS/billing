...
<table width="1000" border="0" cellspacing="0" cellpadding="0" class="tabel" align="center">
  ...
  <td width="1%">&nbsp;</td>
    ...
    <td width="31%" colspan="2">&nbsp;<select id="JnsLayanan" name="JnsLayanan" onchange="showTmpLay();viewKamar();cekDOk();" class="txtinput"></select></td>
  ...
  <td>&nbsp;</td>
    ...
    <td colspan="2">&nbsp;<select style="max-width:250px;" id="TmpLayanan" name="TmpLayanan" onchange="showTabel();" class="txtinput"></select></td>
  ...
  <td>&nbsp;</td>
    ...
    <td width="10%" align="right" id="cb1" style="display:none;">DPJB :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
    <td colspan="2" align="left" id="cb2" style="display:none;"><!--input type="checkbox" name="byTgl" id="byTgl" value="0" onclick="if(this.checked==true) this.value=1; else this.value=0; goFilterAndSort('gridbox');" />&nbsp;Tampilkan semua data sesuai periode yang dipilih-->
    
    <select name="dok1" id="dok1" onchange="showTabel();" class="txtinput">
    <?php
		$query1 = "SELECT DISTINCT b.id, b.nama FROM b_pelayanan a 
INNER JOIN b_ms_pegawai b ON a.dokter_tujuan_id = b.id
INNER JOIN b_kunjungan ab ON a.kunjungan_id = ab.id
WHERE ab.pulang = 0 AND ab.tgl BETWEEN '2014-03-01' AND '2014-03-28' AND a.jenis_kunjungan = 3";
    ?>
    </select>
    </td>
  <td>&nbsp;</td>
  ...
  <tr>
  <td>&nbsp;</td>
    <td colspan="3"><b><span style="width:180px; display:inline-block;">Pasien Pulang</span>: <span id="spanPul">0</span> Orang<br />
	<span style="width:180px; display:inline-block;">Pasien Belum Pulang</span>: <span id="spanBel">0</span> Orang<br />
	<span style="width:180px; display:inline-block;">Pasien Batal Berkunjung</span>: <span id="spanBatal">0</span> Orang</b>
	</td>
    <td>&nbsp;</td>
...
<script>	
	...
	function loadGrid(){
		...
		if (isLoaded==0){
			...
			a.setColHeader("NO,JENIS LAYANAN,TEMPAT LAYANAN,TGL MULAI,TGL SELESAI,NO RM,NAMA PASIEN,ALAMAT,STATUS PASIEN,DILAYANI,PASIEN");
			a.setIDColHeader(",jns_layanan,tmp_layanan,,,no_rm,nama,alamat,statuspas,status_dilayani,kunjungan");
			a.setColWidth("30,150,150,100,100,75,120,150,100,100,80");
			a.setCellAlign("center,center,center,center,center,center,left,center,center,center,center");
			a.setCellHeight(20);
			a.setImgPath("../icon");
	...
	function att(){
		...
		document.getElementById('spanBatal').innerHTML = info[3];
	}
	
	function cekDOk(){
			var tgl1 = document.getElementById("tglMsk").value;
			var tgl2 = document.getElementById("tglSelesai").value;
			...
			//alert(unit_id);
			if(document.getElementById("JnsLayanan").value == 27)
			{
				jQuery("#cb1").show();
				jQuery("#cb2").show();
				jQuery("#load2").load("combo_load.php?tgl1="+tgl1+"&tgl2="+tgl2+"&unit_id="+unit_id);
			}else{
				jQuery("#cb1").hide();
				jQuery("#cb2").hide();
			}
	...
	function viewKamar(){
		...
		if(zxc=='27'){
			a.setHeader("DATA PASIEN");
			a.setColHeader("NO,JENIS LAYANAN,TEMPAT LAYANAN,KAMAR,TGL MULAI,TGL SELESAI,NO RM,NAMA PASIEN,ALAMAT,STATUS PASIEN,PULANG,PASIEN");
			a.setIDColHeader(",jns_layanan,tmp_layanan,,,,no_rm,nama,alamat,statuspas,status_pulang,kunjungan");
			a.setColWidth("30,150,150,100,100,100,75,120,150,100,100,80");
			a.setCellAlign("center,center,center,center,center,center,center,left,center,center,center,center");
			a.setCellHeight(20);
			a.setImgPath("../icon");
