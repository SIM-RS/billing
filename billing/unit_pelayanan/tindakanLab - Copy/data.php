...
<?php
	...
	function cekSub($idSub){
		...
		//$sqlCek = "select * from b_ms_kelompok_lab where parent_id = $idSub";
		$jumCek = mysql_num_rows(mysql_query($sqlCek));
		return $jumCek;
	}
	function treeSub($id){
		$sqlSub = "select * from b_ms_f_pemeriksaan_lab where parent_id = $id";
		//$sqlSub = "select * from b_ms_kelompok_lab where parent_id = $id";
		...
		if($jum != 0){
			...
			while($dataSub = mysql_fetch_array($querySub)){
				...
				echo '{title: "'.$dataSub['nama'].'", key: "'.$dataSub['id'].'"'.$expand;
				echo $children;
				treeSub($dataSub['id']);
				echo $ctup.'}'.$koma.'';
				$i++;
	...
	while($data = mysql_fetch_array($query)){
		...
		echo '{title: "'.$data['nama'].'", key: "'.$data['id'].'"'.$expand;
		echo $children;
