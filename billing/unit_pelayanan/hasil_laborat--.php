                ...
                <td height="30" colspan="2">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
						...
						<td align="center" style="border:#000000 solid 1px; font-weight:bold;font-size:12px">Jenis Pemeriksaan</td>
                            <td align="center" width="150" style="border:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px">Hasil</td>
                            <td align="center" width="100" style="border:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px">Satuan</td>
                            <td align="center" width="100" style="border:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px">Nilai Rujukan</td>
...
INNER JOIN b_ms_kelompok_lab d ON c.kelompok_lab_id=d.id";
			...
			?>
			<tr height="25">
                    <td align="left" colspan="7" style="padding-left:10px;font-size:12px;border-bottom:1px solid;border-top:1px solid" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i><b><?php echo strtoupper($dt1["nama_kelompok"]); ?></b></i></td>
            </tr>
			<?php
				$sql = "SELECT c.*,b.metode,DATE_FORMAT(a.tgl_act,'%d-%m-%Y') AS tgl_act,a.hasil as hasilc,
CONCAT(b.normal1,' - ',b.normal2) normal,a.ket,mp.nama dok , b.normal1, b.normal2, a.hasil, d.nama_satuan
FROM (SELECT * FROM b_hasil_lab WHERE id_pelayanan='$idPel') AS a
INNER JOIN b_ms_normal_lab b ON a.id_normal=b.id
INNER JOIN b_ms_pemeriksaan_lab c ON b.id_pemeriksaan_lab=c.id
INNER JOIN b_ms_satuan_lab d ON b.id_satuan=d.id
...
WHERE c.kelompok_lab_id='".$dt1['id']."'";
			...
			<tr height="25">
                    ...
                    <td align="left" style="padding-left:35px;font-size:12px" >&nbsp;
						<?php 
						echo $spasi.$dt["nama"];
						/*if(is_numeric($dt["hasil"]))
						{
							if(is_numeric($dt["normal1"]))
							{
								if(($dt["hasil"] > $dt["normal2"]) || ($dt["hasil"] < $dt["normal1"]))
								{
									?>
									<b><? echo $spasi.$dt["nama"]; ?> * </b>
                                    <?
								}else{
									echo $spasi.$dt["nama"];
								}	
							}else{
								if(is_numeric(substr($dt["normal1"],0,1)))
								{
									if(($dt["hasil"] > $dt["normal2"]) || ($dt["hasil"] < $dt["normal1"]))
									{
										?>
									...
									$pnj = -1 * (strlen($dt["normal1"])-1);
									if(($dt["hasil"] > substr($dt["normal1"],$pnj)))
									{
										?>
										<b><? echo $spasi.$dt["nama"]; ?> * </b>
										<?
									...
									//$pnj = -1 * (strlen($dt["normal1"])-1);
//									echo substr($dt["normal2"],$pnj)."-".$pnj;	
								}
							}
...
//									echo substr($dt["normal2"],$pnj)."-".$pnj;	
                    ...
                    <td align="center" style="font-size:12px"><?php echo $dt['nama_satuan']?></td>
                    <td align="center" style="font-size:12px"><?php echo $dt['normal']?></td>
                    <td align="center" style="font-size:12px"><?php echo $dt['metode'];?>&nbsp;</td>
