...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $i++;
            $dt.=$sisip.chr(3)."<span $css>".number_format($i,0,",","")."</span>".chr(3)."<span $css>".$rows["nama"]."</span>".chr(3)."<span $css>".$rows["dokter"]."</span>".chr(3)."<span $css>".$rows["utama"]."</span>".chr(3)."<span $css>".$rows['d_akhir']."</span>".chr(3)."<span $css>".$rows['d_klinis']."</span>".chr(3)."<span $css>".$rows['nm_unit']."</span>".chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
