...
switch(strtolower($_REQUEST['act'])){
	case 'ins':
		$sql1="INSERT INTO $dbaskep.ask_soap (pelayanan_id,tgl,ket_S,ket_O,ket_A,ket_P,ket_I,ket_E,ket_R,user_id,user_act,tgl_act,jenis,flag) VALUES ('$idPel','$t','$subs[0]','$subs[1]','$subs[2]','$subs[3]','$subs[4]','$subs[5]','$subs[6]','$userId','$userId',now(),'$jenis','$flag')";
		mysql_query($sql1);
		break;
	case 'up':
		...
		//echo mysql_error();
			if($upd){
					$sql2in="INSERT INTO $dbaskep.ask_soap (pelayanan_id,tgl,ket_S,ket_O,ket_A,ket_P,ket_I,ket_E,ket_R,user_id,user_act,tgl_act,jenis,id_lama,alasan,status_soap,flag) VALUES ('$idPel','$t','$subs[0]','$subs[1]','$subs[2]','$subs[3]','$subs[4]','$subs[5]','$subs[6]','$dok','$userId',now(),'$jenis','$id','$alasan',1,'$flag')";
					mysql_query($sql2in);
				}
		break;
	case 'del':
...
$sql="SELECT *, DATE_FORMAT(tgl, '%d-%m-%Y %k:%i:%s') AS tgl1 FROM $dbaskep.ask_soap WHERE pelayanan_id='$idPel' AND status_soap='1' $filter ORDER BY $sorting";
//$sql="SELECT * FROM $dbaskep.ask_soap WHERE pelayanan_id='$idPel' $filter ORDER BY $sorting";
...
while ($rows=mysql_fetch_array($rs)){
	...
	$i++;
	$dt.=$id.chr(3).$i.chr(3).$rows["tgl1"].chr(3).$data.chr(3).$rows["ket_S"].chr(3).$rows["ket_O"].chr(3).$rows["ket_A"].chr(3).$rows["ket_P"].chr(3).$rows["ket_I"].chr(3).$rows["ket_E"].chr(3).$rows["ket_R"].chr(6);
}
