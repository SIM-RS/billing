...
if($statusProses=='Error') {
    ...
    elseif($grd1 == "true") {
    $sql="select * from (SELECT d.diagnosa_id as id,d.ms_diagnosa_id,md.kode,md.nama,d.pelayanan_id,d.user_id,p.nama as dokter,d.primer,if(d.primer=1,'Utama','Sekunder') as utama,d.akhir,if(d.akhir=1,'Ya','Tidak') as d_akhir, d.type_dokter, md.dg_kode, bmu.nama AS nm_unit
	FROM b_diagnosa d 
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."' ORDER BY $dbapotek.a_obat.OBAT_ID";
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$rows["dokter"].chr(3).$rows["utama"].chr(3).$rows['d_akhir'].chr(3).$rows['nm_unit'].chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
