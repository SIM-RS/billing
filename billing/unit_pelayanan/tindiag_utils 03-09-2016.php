		...
		FROM b_tindakan_kamar WHERE pelayanan_id='".$pelayanan_id."'";
			...
			WHERE pd.kso_paket_id=$idpaket AND t1.tgl=CURRENT_DATE()";
						...
						if ($icd9cm=="true"){
							...
							$sqlTind="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,jenis_kunjungan,pelayanan_id,kunjungan_id,kunjungan_kelas_id,kso_id,kso_kelas_id,kelas_id,tgl,ket,qty,biaya,biaya_kso,biaya_pasien,tgl_act,user_act,user_id,type_dokter,unit_act,exprt,banyak_user_id) values('".$_REQUEST['idTind']."','".$_REQUEST['unitId']."','".$cJenisKunj."','".$pelayanan_id."','".$kunjungan_id."','".$_REQUEST['kunjungan_kelas_id']."','".$_REQUEST['ksoId']."','".$kelas_id."','".$_REQUEST['ksoKelasId']."','".$tgltindak."','".$_REQUEST['ket']."',".$_REQUEST['qty'].",'".$biayaRS."','$biaya_kso','$biaya_pasien',now(),$userId,".$_REQUEST['idDok'].",'".$isDokPengganti."','".$_GET['unit_pelaksana']."','".$_REQUEST['exprt']."','".$_REQUEST['b_dok']."')";
							//echo $sqlTind."<br/>";
							mysql_query($sqlTind);
...
t.type_dokter,t.tgl_act AS tanggal2, t.tgl AS tanggal3,CONCAT(DATE_FORMAT(t.tgl,'%d-%m-%Y '),DATE_FORMAT(t.tgl_act,'%T')) AS tanggal, tin.klasifikasi_id, peg.nama AS petugas, '0' ms_tindakan_id_icd9cm, '0' AS sampah, t.banyak_user_id
FROM b_tindakan t
