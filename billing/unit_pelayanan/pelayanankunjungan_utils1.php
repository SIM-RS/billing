...
if($statusProses=='Error') {
    ...
    if($grd == "true") {
		...
		, inap, penunjang,date_format(k.tgl_sjp,'%d-%m-%Y') as tgl_sjp,k.no_sjp,k.no_anggota,k.status_penj,k.nama_peserta, pl.checkout as sRujuk, pl.pelayanan_id_asal as pelayanan_id_asal, pl.dokter_tujuan_id, pl.batal, p.telp, pl.jenis_kunjungan
		FROM b_ms_pasien p 
		...
		(SELECT nama FROM b_ms_unit WHERE id=pl.unit_id) unit,pl.ket,
		k.umur_thn, k.umur_bln, k.umur_hr, pl.unit_id, pl.kelas_id,pl.verifikasi,pl.verifikator, k.tgl,k.pasien_id, m.KEPEMILIKAN_ID,concat(date_format(pl.tgl,'%d-%m-%Y'),' ',date_format(pl.tgl_act,'%H:%i')) as tgl_act,pl.dilayani,ifnull(kso.nama,'UMUM') as namakso
		, inap, penunjang,date_format(k.tgl_sjp,'%d-%m-%Y') as tgl_sjp,k.no_sjp,k.no_anggota,k.status_penj,k.nama_peserta, pl.cito, pl.checkout as tgl_keluar, pl.accLab, pl.checkout as sRujuk, pl.pelayanan_id_asal as pelayanan_id_asal, pl.dokter_tujuan_id, pl.batal, p.telp, pl.jenis_kunjungan
		FROM b_ms_pasien p 
		INNER JOIN b_kunjungan k ON k.pasien_id = p.id
		INNER JOIN b_pelayanan pl ON k.id = pl.kunjungan_id
		LEFT JOIN b_ms_pegawai mp ON pl.dokter_id = mp.id
	...
	if($act == 'cmbDokterUnit'){
		...
		LEFT JOIN b_ms_kso kso ON kso.id = k.kso_id
		LEFT JOIN b_ms_kamar mk ON mk.id = tk.kamar_id
		LEFT JOIN $dbapotek.a_mitra m ON m.kso_id_billing = kso.id ".$saringan." ".$dilayani." AND dok.id IS NOT NULL order by ".$sorting.") as gab ".$filter;
		}else{
			$sDok="select distinct * from (SELECT dok.id, dok.nama AS nama 
		FROM b_ms_pasien p 
		INNER JOIN b_kunjungan k ON k.pasien_id = p.id
