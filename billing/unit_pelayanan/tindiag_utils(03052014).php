            ...
            case 'btnSimpanDiag':
				...
				if(mysql_num_rows($qCekDiag)>0 && $_REQUEST['isManual']=='0'){
					...
					if($_REQUEST['isManual']=='1'){
						$sqlTambah="insert into b_diagnosa (ms_diagnosa_id,diagnosa_manual,kunjungan_id,pelayanan_id,tgl,primer,kasus_baru,kasus_sys,akhir,klinis,banding,tgl_act,user_act,user_id,type_dokter) values('0','".$_REQUEST['diagnosa']."','".$kunjungan_id."','".$pelayanan_id."',CURDATE(),'".$_REQUEST['isPrimer']."','".$kasusBaru."','".$kasus_sys."','".$_REQUEST['isAkhir']."','".$_REQUEST['isKlinis']."','0',now(),$userId,".$_REQUEST['idDok'].",$isDokPengganti)";
					}
					else{
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $sisip=$rows["id"]."|".$rows['user_id']."|".$rows['ms_diagnosa_id']."|".$rows['type_dokter']."|".$rows['primer']."|".$rows['akhir']."|".$gol_id."|".$gol."|".$isPK."|".$rows['klinis']."|".$rows['banding']."|".$rows['ismanual']."|".$rows['sampah']."|".$rows["nama"];
            $i++;
