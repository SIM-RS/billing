		...
		FROM b_tindakan_kamar WHERE pelayanan_id='".$pelayanan_id."'";
			...
			WHERE pd.kso_paket_id=$idpaket AND t1.tgl=CURRENT_DATE()";
						...
						if ($icd9cm=="true"){
							...
							$sqlTind="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,jenis_kunjungan,pelayanan_id,kunjungan_id,kunjungan_kelas_id,kso_id,kso_kelas_id,tgl,ket,qty,biaya,biaya_kso,biaya_pasien,tgl_act,user_act,user_id,type_dokter,unit_act,exprt) values('".$_REQUEST['idTind']."','".$_REQUEST['unitId']."','".$cJenisKunj."','".$pelayanan_id."','".$kunjungan_id."','".$_REQUEST['kunjungan_kelas_id']."','".$_REQUEST['ksoId']."','".$_REQUEST['ksoKelasId']."','".$tgltindak."','".$_REQUEST['ket']."',".$_REQUEST['qty'].",'".$biayaRS."','$biaya_kso','$biaya_pasien',now(),$userId,".$_REQUEST['idDok'].",'".$isDokPengganti."','".$_GET['unit_pelaksana']."','".$_REQUEST['exprt']."')";
							//echo $sqlTind."<br/>";
							mysql_query($sqlTind);
				...
				if(mysql_num_rows($qCekDiag)>0 && $_REQUEST['isManual']=='0'){
					...
					if ($fdiagBanding!="|" && $fdiagBanding!=""){
						...
						for ($i=0;$i<(count($arrfdiagBanding)-1);$i++){
							$sqlTambah="insert into b_diagnosa (ms_diagnosa_id,diagnosa_manual,diag_banding_id,kunjungan_id,pelayanan_id,tgl,primer,kasus_baru,akhir,klinis,banding,tgl_act,user_act,user_id,type_dokter) values('0','".$arrfdiagBanding[$i]."','".$diagBanding_id."','".$kunjungan_id."','".$pelayanan_id."',CURDATE(),'0','2','0','0','1',now(),$userId,".$_REQUEST['idDok'].",$isDokPengganti)";
							//echo $sqlTambah."<br/>";
							$rs=mysql_query($sqlTambah);
						}
					}
...
FROM b_tindakan_kamar WHERE pelayanan_id='".$pelayanan_id."'";
	...
	WHERE pd.kso_paket_id=$idpaket AND t1.tgl='".$rwcekTindAsli["tgl"]."'";
				...
				if ($icd9cm=="true"){
					...
					$sqlUpdate="UPDATE b_tindakan SET exprt = '".$_REQUEST['exprt']."', tgl='".$tgltindak."', ms_tindakan_kelas_id = '".$_REQUEST['idTind']."',ms_tindakan_unit_id='".$_REQUEST['unitId']."', ket = '".$_REQUEST['ket']."', qty=".$_REQUEST['qty'].", biaya = '".$biayaRS."', user_act = $userId, user_id=".$_REQUEST['idDok'].",type_dokter='".$isDokPengganti."', biaya_pasien = '$biaya_pasien', biaya_kso = '$biaya_kso' WHERE id = '".$_REQUEST['id']."'";
            ...
            case 'btnSimpanDiag':
				if($_REQUEST['isManual']=='1'){
					$sqlUpdate="update b_diagnosa set ms_diagnosa_id='0',diagnosa_manual='".$_REQUEST['diagnosa']."',primer='".$_REQUEST['isPrimer']."',akhir='".$_REQUEST['isAkhir']."',klinis='".$_REQUEST['isKlinis']."',banding='".$_REQUEST['isBanding']."',user_act=$userId, user_id=".$_REQUEST['idDok'].",type_dokter='".$isDokPengganti."' where diagnosa_id='".$_REQUEST['id']."'";
				}
				else{
					$sqlUpdate="update b_diagnosa set ms_diagnosa_id='".$_REQUEST['idDiag']."',diagnosa_manual=(NULL),primer='".$_REQUEST['isPrimer']."',akhir='".$_REQUEST['isAkhir']."',klinis='".$_REQUEST['isKlinis']."',banding='".$_REQUEST['isBanding']."',user_act=$userId, user_id=".$_REQUEST['idDok'].",type_dokter='".$isDokPengganti."' where diagnosa_id='".$_REQUEST['id']."'";	
				}
...
if($statusProses=='Error') {
	...
	//joker
		...
		/*
		$sql="SELECT * FROM
(SELECT t.id, t.qty, t.biaya, t.qty * t.biaya AS subtotal, t.kunjungan_id, t.ms_tindakan_kelas_id, 
...
t.type_dokter,t.tgl_act AS tanggal, tin.klasifikasi_id, peg.nama AS petugas,'0' ms_tindakan_id_icd9cm
FROM b_tindakan t
INNER JOIN b_pelayanan pl ON pl.id = t.pelayanan_id
INNER JOIN b_ms_tindakan_kelas tk ON tk.id = t.ms_tindakan_kelas_id
INNER JOIN b_ms_kelas k ON tk.ms_kelas_id=k.id
...
UNION
SELECT t.id, t.qty, t.biaya, t.qty * t.biaya AS subtotal, t.kunjungan_id, 0 ms_tindakan_kelas_id, 
...
t.type_dokter,t.tgl_act AS tanggal, '' klasifikasi_id, peg.nama AS petugas,t.ms_tindakan_id ms_tindakan_id_icd9cm
...
WHERE t.pelayanan_id = '".$pelayanan_id."'
    ...
    elseif($grd1 == "true") {
	/*	
    $sql="select * from (SELECT d.diagnosa_id as id,d.ms_diagnosa_id,md.kode,md.nama as nama1,IF(d.diagnosa_manual IS NULL,md.nama,d.diagnosa_manual) AS nama,IF(d.diagnosa_manual IS NULL,0,1) AS ismanual,d.pelayanan_id,d.user_id,p.nama as dokter,d.primer,if(d.primer=1,'Utama','Sekunder') as utama,d.akhir,if(d.akhir=1,'Ya','Tidak') as d_akhir,d.klinis,if(d.klinis=1,'Ya','Tidak') as d_klinis,d.banding,if(d.banding=1,'Ya','Tidak') as d_banding, d.type_dokter, md.dg_kode, bmu.nama AS nm_unit
	FROM b_diagnosa d 
	...
	WHERE d.kunjungan_id = '".$kunjungan_id."') as gab $filter order by $sorting ";
	*/
	...
	INNER JOIN b_ms_pegawai p ON p.id = d.user_id
	INNER JOIN b_ms_unit bmu ON pl.unit_id = bmu.id
	WHERE d.kunjungan_id = '".$kunjungan_id."'
	UNION
    ...
    elseif($grdAnamnesa == "true") {	//Anamnese
         $sql = "SELECT a.*, DATE_FORMAT(a.TGL,'%d-%m-%Y %H:%i') AS tgltok, p.nama AS dokter
				...
				WHERE a.PASIEN_ID = '".$_REQUEST['pasien_id']."' and snurs = 0";
    }
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $sisip=$rows["id"]."|".$rows['user_id']."|".$rows['ms_diagnosa_id']."|".$rows['type_dokter']."|".$rows['primer']."|".$rows['akhir']."|".$gol_id."|".$gol."|".$isPK."|".$rows['klinis']."|".$rows['banding']."|".$rows['ismanual']."|".$rows['sampah']."|".$rows["nama"]."|".$rows["mati"];
            $i++;
            $dt.=$sisip.chr(3)."<span $css>".number_format($i,0,",","")."</span>".chr(3)."<span $css>".$rows["tgldiag"]."</span>".chr(3)."<span $css>".$rows["nama"]."</span>".chr(3)."<span $css>".$rows["dokter"]."</span>".chr(3)."<span $css>".$rows["utama"]."</span>".chr(3)."<span $css>".$rows['d_akhir']."</span>".chr(3)."<span $css>".$rows['d_klinis']."</span>".chr(3)."<span $css>".$rows['nm_unit']."</span>".chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
