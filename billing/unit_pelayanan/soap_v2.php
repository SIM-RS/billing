...
<tr>
	<td width="671" height="30" colspan="9" align="center" valign="bottom">
		...
		<tr>
		  <td width="64%">
          <select id="asalSOAP" name="asalSOAP" class="txtinput" onchange="gantiJenis(this.value);" style="display:none;">
            ...
            <td width="27%" style="display:none">&nbsp;<input type="text" class="txtcenter" readonly="readonly" name="tanggalSOAP" id="tanggalSOAP" size="11" value="<?php echo date('d-m-Y'); ?>"/>&nbsp;<input type="button" class="btninput" id="btnTanggalSOAP" name="btnTanggalSOAP" value=" V " onClick="gfPop.fPopCalendar(document.getElementById('tanggalSOAP'),depRange,fungsikosong);" /></td>
	</tr>
    	<tr>
        	<td>
            <select id="cmbDokSOAPIER" style="display:none;" name="cmbDokSOAPIER" class="txtinput" onchange="ampek(this.value)" onkeypress="setDok('btnSimpanDiag',event,this.value);"></select>
        ...
        <tr>
        	...
        	<input type="text" id="tglSoap" class="txtcenter" name="tglSoap" size="10" value="<?php echo date("d-m-Y") ?>" />
				<input type="button" class="btninput" id="btnTanggal1" name="btnTanggal1" value=" V " onclick="gfPop.fPopCalendar(document.getElementById('tglSoap'),depRange,fungsikosong);" />
				&nbsp;&nbsp;&nbsp;
				<!--<input type="text" class="txtcenter" id="mnt" name="mnt" size="7" value="<?php echo gmdate("H:i:s",mktime(date('H')+7)) ?>" />-->
                <input type="text" class="txtcenter" id="mnt" name="mnt" size="7" />
                <input type="hidden" id="jnsLayxxx" name="jnsLayxxx" value="<?php echo $_REQUEST['jnsLay'] ?>" />			<input type="hidden" id="tglMas" name="tglMas" value="<?=$_REQUEST['tglMaster'] ?>" />
			...
			<td width="772" align="center">
				...
				<table width="100%" align="center">
				  ...
				  <tr style="display:none">
					...
					<tr>
					  <td align="center"><div class="divAlasan" style="display:none">&nbsp;Alasan ubah</div></td>
					...
					<tr>
						<td colspan="5" align="center"><div id="dridSub" align="center" style="width:700px; height: 200px; background-color:white;"></div><br/>
							<div id="pagingSub" style="width:700px; display:block;"></div></td>
					</tr>
				</table>
			</fieldset></td>
		</tr>
...
document.getElementById("mnt").value=t;
}
...
subj.onLoaded(konfirmasi_soab);
subj.Init();

function konfirmasi_soab(key,val)
{
...
//return false;
if(par=="up"){
	if(ValidateForm("txtAlasanx",'ind')){
	var subs=encodeURIComponent(document.getElementById('txtSxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtOxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtAxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtPxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtIxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtExxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtRxxx').value);
	var id=document.getElementById('txtIdSub').value;
	var cmbDok="<? echo $_SESSION['userId'];?>";
	var mnt=document.getElementById('mnt').value;
	var als=document.getElementById('txtAlasanx').value;
	var jnsSOAP1 = "<? echo $_SESSION['spesialis'];?>";
	if(jnsSOAP1 == 129)
	{
		jnsSOAP1 = 0;
	}else{
	...
	subj.loadURL("soap_utils.php?cmbDok="+cmbDok+"&grd=1&act="+par+"&subs="+subs+"&id="+id+"&idPel="+getIdPel+"&tglMaster="+document.getElementById('tglSoap').value+"&cmbDokter="+document.getElementById('cmbDokDiag').value+"&mnt="+mnt+"&jenis="+jnsSOAP1+"&alasan="+als,'','GET');
	//subj1.loadURL("soap_utils.php?cmbDok="+cmbDok+"&grd=1&act="+par+"&subs="+subs+"&id="+id+"&idPel="+getIdPel+"&tglMaster="+document.getElementById('tglSoap').value+"&cmbDokter="+document.getElementById('cmbDokDiag').value+"&mnt="+mnt+"&jenis="+jnsSOAP1+"&alasan="+als,'','GET');
	fResetSOP();
	}
}else{
	//alert("");
	//if(ValidateForm("txtSxxx,txtOxxx,txtAxxx,txtPxxx,txtIxxx,txtExxx,txtRxxx",'ind')){
	var subs=encodeURIComponent(document.getElementById('txtSxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtOxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtAxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtPxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtIxxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtExxx').value)+String.fromCharCode(5)+encodeURIComponent(document.getElementById('txtRxxx').value);
	var id=document.getElementById('txtIdSub').value;
...
function ambilIdS1(){
var sisip=subj1.getRowId(subj1.getSelRow()).split("|")
ssoap = 1;

	document.getElementById('txtIdSub').value=sisip[0];
	jQuery('#txtSxxx').val(subj1.cellsGetValue(subj1.getSelRow(),4));
	jQuery('#txtOxxx').val(subj1.cellsGetValue(subj1.getSelRow(),5));
	jQuery('#txtAxxx').val(subj1.cellsGetValue(subj1.getSelRow(),6));
	jQuery('#txtPxxx').val(subj1.cellsGetValue(subj1.getSelRow(),7));
	jQuery('#txtIxxx').val(subj1.cellsGetValue(subj1.getSelRow(),8));
	jQuery('#txtExxx').val(subj1.cellsGetValue(subj1.getSelRow(),9));
	jQuery('#txtRxxx').val(subj1.cellsGetValue(subj1.getSelRow(),10));
