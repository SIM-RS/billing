...
AND pl.jenis_kunjungan = 3";
//echo $sqlPas."<br>";
$rsPas = mysql_query($sqlPas);
$rwPas = mysql_fetch_array($rsPas);


$sDokterMerawat="SELECT
(SELECT nama FROM b_ms_pegawai WHERE id = p.dokter_tujuan_id) AS dokterTujuan,
p.dokter_tujuan_id
FROM b_pelayanan p 
INNER JOIN b_ms_unit mu ON p.unit_id=mu.id 
WHERE 
p.kunjungan_id='".$_REQUEST['idKunj']."' 
AND p.jenis_kunjungan=3 ORDER BY p.id LIMIT 1";
$rsDokterMerawat=mysql_query($sDokterMerawat);
$rwDokterMerawat=mysql_fetch_array($rsDokterMerawat);
$dokter=$rwDokterMerawat['dokterTujuan'];					
$dokter_id=$rwDokterMerawat['dokter_tujuan_id'];
            ...
            <td colspan="3">
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr style="font-size: 13px; ">
                        <td style="border-left:1px solid;">&nbsp;</td>
                        <td style="">&nbsp;Kelas</td>
                        <td style="">:&nbsp;<?php echo $rwPas['kelas'];?></td>
                        <td style="">&nbsp;Kunjungan</td>
                        <td style="">:&nbsp;RAWAT INAP</td>
                        <td style="border-right:1px solid;">&nbsp;</td>
                    </tr>
                    <tr style="font-size: 13px; ">
                        <td style="border-left:1px solid;border-bottom:1px solid;">&nbsp;</td>
                        <td style="border-bottom:1px solid;">&nbsp;Asal Kunjungan</td>
                        <?php
						$sAsal="SELECT unit_id FROM b_kunjungan WHERE id='".$_REQUEST['idKunj']."'";
						$qAsal=mysql_query($sAsal);
						$rwAsal=mysql_fetch_array($qAsal);
						
						$asal_kunjungan='';
						if($rwAsal['unit_id']=='45'){
							$asal_kunjungan="IGD";	
						}
						else{
							$asal_kunjungan="RAWAT JALAN";
						}
						?>
                        <td style="border-bottom:1px solid;">:&nbsp;<?php echo $asal_kunjungan;?></td>
            ...
            <td colspan="3">
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr style="font-size: 13px;">
                        ...
                        <td align="center">:</td>
                        <td style="border-right:1px solid;">&nbsp;<?php echo $rsqlSOAP['ket_O']; ?></td>
                    </tr>                    
                    <?php
...
WHERE id_kunjungan='".$_REQUEST['idKunj']."' AND bp.jenis_kunjungan=3) AS a 
...
INNER JOIN b_ms_satuan_lab msl ON b.id_satuan=msl.id ORDER BY a.id_pelayanan,c.kode_urut";
					$exL=mysql_query($sqlL);//echo $sqlL;
					...
					ambulan				17
					bank darah			19
					rawat inap			22,23,24,74
					sewa alat			29
					jasa				44
					jenazah				59
					labu darah			75
					lama hari perwatan	72
					paket persalinan	73
					ct scan				85
					akomodasi			86
					*/
					// bagus tutup (22-07-2014)
					/*$sqlT="select * from
                                        ...
                                        WHERE 
										t.kunjungan_id = '".$_REQUEST['idKunj']."' 
										AND pl.jenis_kunjungan = 3
										AND tin.klasifikasi_id NOT IN (1,20,82,6,5,11,17,19,22,23,24,74,29,44,59,75,72,73,85,86)
										) as gab";*/
					/*$sqlT="select * from
	...
	WHERE ((t.pelayanan_id = '".$_REQUEST['idPel']."') OR (pl.kunjungan_id = '".$_REQUEST['idKunj']."' AND pl.unit_id IN ('63','72'))) AND tin.id NOT IN (74,3076,3090,2333) AND tin.kel_tindakan_id NOT IN (61,304)) AS gab"; */
					$sqlT="select * from
        (SELECT DISTINCT tin.nama, tin.kode, icd.kode_icd_9cm 
	FROM b_tindakan t
	INNER JOIN b_pelayanan pl ON pl.id = t.pelayanan_id AND t.user_id = pl.dokter_tujuan_id
	...
	LEFT JOIN b_ms_pegawai peg ON peg.id=t.user_act
	LEFT JOIN b_tindakan_icd9cm icd ON t.id = icd.b_tindakan_id
	WHERE pl.kunjungan_id = '".$_REQUEST['idKunj']."' AND (pl.unit_id IN ('63','72') OR tin.id in (1003,2183)) AND tin.id NOT IN (74,3076,3090,2333) AND tin.kel_tindakan_id NOT IN (61,304)) AS gab";
	//echo $sqlT."<br>";
                    ...
                    <tr style="font-size: 13px;">
                        ...
                        <td style="border-right:1px solid;">
                            ...
                            ?>
          						<tr>
            						<td style="font-size: 13px; "><?=$dT['nama']." ".$dT['kode_icd_9cm'];?></td>
          						</tr>
        						<?php }?>	
        					</table>
                        </td>
					...
					WHERE b_resep.kunjungan_id='".$_REQUEST['idKunj']."' AND b_pelayanan.jenis_kunjungan=3 ORDER BY b_resep.id DESC LIMIT 1";
                    ...
                    <tr style="font-size: 13px;">
                        ...
                        <td style="border-right:1px solid;">
                            ...
                            ?>
          						<tr>
            						<td style="font-size: 13px; "><?=$rwO['OBAT_NAMA']." (".$rwO['ket_dosis'].")";?></td>
          						</tr>
        						<?php }?>	
        					</table>
                        </td>
                    ...
                    <?php
					$cekDiag="select d.tgl 
							from b_diagnosa d
							inner join b_pelayanan p on p.id=d.pelayanan_id
							where p.kunjungan_id = '".$_REQUEST['idKunj']."' and p.jenis_kunjungan=3 order by d.diagnosa_id desc limit 1";
					$qCekDiag=mysql_query($cekDiag);
					$rwCekDiag=mysql_fetch_array($qCekDiag);
					$tgl_diag=$rwCekDiag['tgl'];
					...
					$qDiagAkhir = "SELECT 
						...
						WHERE 
						b_diagnosa_rm.kunjungan_id = '".$_REQUEST['idKunj']."' AND b_diagnosa.akhir = 1 AND b_pelayanan.jenis_kunjungan=3
						GROUP BY b_diagnosa_rm.kunjungan_id";
					*/
					$qDiagAkhir = "SELECT 
					  GROUP_CONCAT(CONCAT(IFNULL(a.diagnosa_manual, b_ms_diagnosa.nama),' [',bm.kode,']',' ',IFNULL((SELECT IFNULL(CONCAT('/',diagnosa_manual),'') FROM b_diagnosa WHERE diag_banding_id = a.diagnosa_id),' ')) SEPARATOR '<br>&nbsp;') AS diagnosa
					FROM
					   b_diagnosa a
					  LEFT JOIN b_ms_diagnosa
						ON a.ms_diagnosa_id = b_ms_diagnosa.id 
					  LEFT JOIN b_diagnosa_rm bdrm ON a.diagnosa_id = bdrm.diagnosa_id
					  LEFT JOIN b_ms_diagnosa bm ON bdrm.ms_diagnosa_id = bm.id
					  INNER JOIN b_pelayanan 
						ON b_pelayanan.id = a.pelayanan_id 
					  INNER JOIN b_ms_pegawai 
					...
					if($nDiagAkhir==0){
						...
						left join b_ms_diagnosa as b_ms_diagnosa2 ON b_ms_diagnosa2.id = b_diagnosa.ms_diagnosa_id 
						WHERE 
						b_diagnosa_rm.kunjungan_id = '".$_REQUEST['idKunj']."'  AND b_diagnosa.primer = 1 AND b_pelayanan.jenis_kunjungan=3
					...
					*/
                    ...
                    <tr style="font-size: 13px; ">
                        ...
                        <td align="center" valign="top">:</td>
                        <td style="border-right:1px solid;" valign="top">&nbsp;<?php echo $rwDiagAkhir['diagnosa']; ?></td>
                    </tr>
                    <?php
					}
					?>
