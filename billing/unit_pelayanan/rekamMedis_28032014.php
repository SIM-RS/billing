...
ORDER BY unit,
  unit_name";
                    ...
                    <ul style="list-style-position:inside; list-style-type:square; padding-left:2px;">
                        ...
                        ?>
                        <li style="padding-left: 10px"><?php echo $rwObat['OBAT_NAMA'].' <br />(Jumlah : '.$rwObat['qty'].') <br> '.$rwObat['ket_dosis'].''; if($rwObat['obat_apotek']=='tru'){echo '';}else if($rwObat['obat_apotek']!='true'){echo ' [Obat Diubah]';}?></li>
                    ...
                    <?
						while($dtAnam = mysql_fetch_array($execAnam))
						{
                            ...
                            <tr>
                                ...
                                <td <?php echo $border; ?> align="center"><span onClick="viewResume('<?=$dtAnam['ANAMNESE_ID']?>','<?=$dtAnam['KUNJ_ID']?>','<?=$dtAnam['PEL_ID']?>')" style="color:#03F; cursor:pointer;">Detil</span></td>
                            </tr>
                            <?
						}
					?>
...
<script>
	...
	function viewResume(a,b,c)
	{
		window.open("resumemedis.php?idKunj="+b+"&idPel="+c+"&idPasien=<?=$_REQUEST['idPasien']?>&id_anamnesa="+a);
	}
