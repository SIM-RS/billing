...
function distribusiBiaya($ksoId,$kunjId,$jenisKunj,$plafon,$tipe){
	$nilai = $plafon;
	$jenisKunj="";
	if ($jenisKunj<>0) $jenisKunj=" AND p.jenis_kunjungan = '$jenisKunj'";
	$fKronis="";
	if ($ksoId==6) $fKronis="AND ap.KRONIS<>2";
	...
	if ($nilai<$rwObat["biaya"]){	
		...
		$sqlKamar="UPDATE b_tindakan_kamar, b_pelayanan SET b_tindakan_kamar.beban_kso=0
					WHERE b_tindakan_kamar.pelayanan_id = b_pelayanan.id
						AND b_pelayanan.kunjungan_id = '$kunjId'
						AND b_tindakan_kamar.kso_id = '$ksoId'";
		$rsKamar=mysql_query($sqlKamar);
		// Update Biaya Kso Tindakan = 0
		$sqlKamar="UPDATE b_tindakan, b_pelayanan SET b_tindakan.beban_kso=0
					WHERE b_tindakan.pelayanan_id = b_pelayanan.id
						AND b_pelayanan.kunjungan_id = '$kunjId'
						AND b_tindakan.kso_id = '$ksoId'";
		$rsKamar=mysql_query($sqlKamar);
	// Cukup
	}else{
...
FROM b_tindakan_kamar b INNER JOIN b_pelayanan p ON b.pelayanan_id=p.id WHERE p.kunjungan_id='$kunjId' 
...
AND b.aktif=1 AND b.kso_id='$ksoId'";
			...
			while ($rwKamar=mysql_fetch_array($rs)){
				...
				if ($nilai>=$biaya){
					...
					$sql="UPDATE b_tindakan_kamar SET beban_kso='$biaya' WHERE id='$idKamar'";
					$rsUpdt=mysql_query($sql);
					$nilaiSisa=$nilai%$qty;
					$nilai=0;
				}else{
					$sql="UPDATE b_tindakan_kamar SET beban_kso='0' WHERE id='$idKamar'";
					$rsUpdt=mysql_query($sql);
				}
			}
			...
			if ($nilai>0 || $nilaiSisa>0){
				...
				$sqlTind = "SELECT t.* FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
				...
				while($rwTind = mysql_fetch_array($rsTind))
				{
					...
					$sqlUp = "UPDATE b_tindakan SET biaya_kso=$biaya WHERE id = $tindId";
					//echo $sqlUp.";<br>";
					$rsUp = mysql_query($sqlUp);
				}
			}else{
				$sql="UPDATE b_tindakan SET biaya_kso='0' WHERE kunjungan_id='$kunjId' AND kso_id='$ksoId'";
				$rsUpdt=mysql_query($sql);
			}
		}
		// Cukup Biaya Kamar
...
function distribusiBiayaPx($ksoId,$kunjId,$jenisKunj,$plafon,$tipe){
	$nilai = $plafon;
	$jenisKunj="";
	if ($jenisKunj<>0) $jenisKunj=" AND p.jenis_kunjungan = '$jenisKunj'";
...
AND b.aktif=1 AND b.kso_id='$ksoId') AS gab";
	...
	if ($nilai>$rwKamar["biayaPx"]){
		...
		while($rwTind = mysql_fetch_array($rsTind))
		{
			...
			$sqlUp = "UPDATE b_tindakan SET biaya_pasien=$biaya WHERE id = $tindId";
			//echo $sqlUp.";<br>";
			$rsUp = mysql_query($sqlUp);
		}
	}else{
...
AND b.aktif=1 AND b.kso_id='$ksoId'";
			...
			while ($rwKamar=mysql_fetch_array($rs)){
				...
				if ($nilai>=$biaya){
					...
					$biaya=floor($nilai/$qty);
					$sql="UPDATE b_tindakan_kamar SET beban_pasien='$biaya' WHERE id='$idKamar'";
					$rsUpdt=mysql_query($sql);
					$nilaiSisa=$nilai%$qty;
					$nilai=0;
				}else{
					$sql="UPDATE b_tindakan_kamar SET beban_pasien='0' WHERE id='$idKamar'";
					$rsUpdt=mysql_query($sql);
				}
			}
			...
			if ($nilai>0 || $nilaiSisa>0){
				...
				$sql="UPDATE b_tindakan,b_pelayanan SET b_tindakan.biaya_pasien=0 
					WHERE b_tindakan.pelayanan_id = b_pelayanan.id 
					AND b_pelayanan.kunjungan_id='$kunjId' AND b_tindakan.kso_id='$ksoId'";
				$rsUpdt=mysql_query($sql);
			}
		}else{
			$sql="UPDATE b_tindakan_kamar,b_pelayanan SET b_tindakan_kamar.beban_pasien=0 
				WHERE b_tindakan_kamar.pelayanan_id = b_pelayanan.id 
				AND b_pelayanan.kunjungan_id='$kunjId' AND b_tindakan_kamar.kso_id='$ksoId'";
			$rsUpdt=mysql_query($sql);
		}
		$sqlUp = "UPDATE b_tindakan,b_pelayanan SET b_tindakan.biaya_pasien=0 
				WHERE b_tindakan.pelayanan_id = b_pelayanan.id 
				AND b_pelayanan.kunjungan_id='$kunjId' AND b_tindakan.kso_id='$ksoId'";
		//echo $sqlUp.";<br>";
		$rsUp = mysql_query($sqlUp);
...
function distribusiBiayaPx1($ksoId,$kunjId,$jenisKunj,$plafon,$tipe){
	$nilai = $plafon;
	$jenisKunj="";
	if ($jenisKunj<>0) $jenisKunj=" AND p.jenis_kunjungan = '$jenisKunj'";
	$sql="SELECT IFNULL(SUM((ap.QTY_JUAL-ap.QTY_RETUR)*ap.HARGA_SATUAN),0) AS biaya
...
AND b.aktif=1 AND b.kso_id='$ksoId'";
			...
			if ($nilai>0 || $nilaiSisa>0){
				...
				$sql="UPDATE b_tindakan SET biaya_pasien='0' WHERE kunjungan_id='$kunjId' AND kso_id='$ksoId'";
				$rsUpdt=mysql_query($sql);
			}
		}
		// Cukup Biaya Kamar
