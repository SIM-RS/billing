...
?>
    <table id="tblObat" border="0" cellpadding="1" cellspacing="0">
      <tr class="headtable">
        <td class="tdisikiri" width="80"> KODE </td>
        <td class="tdisi" align="left"> NAMA OBAT </td>
        <td class="tdisi" width="80"> SATUAN </td>
        <!--td class="tdisi" width="90"><?php //echo $rows['NAMA']; ?></td-->
        <td class="tdisi" width="60"> STOK </td>
      </tr>
      <?php 
	  ...
	  //if ($aKepemilikan=="0"){
/*	  	$sql="select ao.OBAT_ID,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL from $dbapotek.a_obat ao
		where ao.OBAT_NAMA like '%$aKeyword%' and OBAT_ISAKTIF=1 order by ao.OBAT_NAMA";
*/		
		//query sebelumnya
		/*$sql="select ao.OBAT_ID,ao.OBAT_KODE,ao.OBAT_NAMA,ao.OBAT_SATUAN_KECIL,so.qty_stok as stok
		from $dbapotek.a_obat ao inner join $dbapotek.a_stok so on so.obat_id=ao.OBAT_ID
		where ao.OBAT_NAMA like '%$aKeyword%' and OBAT_ISAKTIF=1 AND so.unit_id = 2 order by ao.OBAT_NAMA";*/
