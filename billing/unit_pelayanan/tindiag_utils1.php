									...
									if($rowCekStat['kso_id'] == 4) {
										...
										if(mysql_num_rows($rsCek) > 0) {
											...
											if(($_REQUEST['ksoKelasId'] != $_GET['kelas_id']) && ($_GET['kelas_id'] != 1) && ($idParent != 94) && ($unitId != 48) && ($unitId != 112)) {
											   ...
											   $sqlCekAkomodasi = "select ifnull(sum(biaya * qty),0) as jumlah,ifnull(sum(biaya_pasien * qty),0) as biaya_pasien from b_tindakan t inner join  b_ms_tindakan_kelas kt on t.ms_tindakan_kelas_id = kt.id where kt.ms_tindakan_id in (select ms_tindakan_id from b_ms_kso_pakomodasi where ms_kso_id = '4') and t.pelayanan_id = '$pelayanan_id'";
		...
		FROM b_tindakan_kamar WHERE pelayanan_id='".$pelayanan_id."'";
												...
												while ($rwBiayaKamar=mysql_fetch_array($rsBiayaKamar)){
													if ($rwBiayaKamar['tarip']==0 || $rwBiayaKamar['beban_kso']==0){
														if ($rwBiayaKamar['kamar_id']!=0){
															...
															$sql="UPDATE b_tindakan_kamar SET tarip='".$rwKmr['tarip']."',
															retribusi='".($rwKmr['tarip']*0.03)."',beban_kso='".$paket_hp."',beban_pasien='".$bPx."',kelas_id='".$_GET['kelas_id']."' WHERE id='".$rwBiayaKamar['id']."'";
															$rsKmr=mysql_query($sql);											
														}else{
															...
															$rowBiayaKamarPasien +=$bPx * $rwBiayaKamar['qtyHari'];
															$sql="UPDATE b_tindakan_kamar SET kamar_id='".$rwKmr['kamar_id']."',kode='".$rwKmr['kode']."',nama='".$rwKmr['nama']."',tarip='".$rwKmr['tarip']."',retribusi='".($rwKmr['tarip']*0.03)."',beban_kso='".$paket_hp."',beban_pasien='".$bPx."',kelas_id='".$_GET['kelas_id']."' WHERE id='".$rwBiayaKamar['id']."'";
															//echo $sql."<br>";
															$rsKmr=mysql_query($sql);
														}
													}else{
			...
			WHERE pd.kso_paket_id=$idpaket AND t1.tgl=CURRENT_DATE()";
						...
						if ($icd9cm=="true"){
							...
							$sqlTind="insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,jenis_kunjungan,pelayanan_id,kunjungan_id,kunjungan_kelas_id,kso_id,kso_kelas_id,kelas_id,tgl,ket,qty,biaya,biaya_kso,biaya_pasien,tgl_act,user_act,user_id,type_dokter,unit_act,exprt,banyak_user_id,ak_ms_unit_id) values('".$_REQUEST['idTind']."','".$_REQUEST['unitId']."','".$cJenisKunj."','".$pelayanan_id."','".$kunjungan_id."','".$_REQUEST['kunjungan_kelas_id']."','".$_REQUEST['ksoId']."','".$kelas_id."','".$_REQUEST['ksoKelasId']."','".$tgltindak."','".$_REQUEST['ket']."',".$_REQUEST['qty'].",'".$biayaRS."','$biaya_kso','$biaya_pasien',now(),$userId,".$_REQUEST['idDok'].",'".$isDokPengganti."','".$_GET['unit_pelaksana']."','".$_REQUEST['exprt']."','".$_REQUEST['b_dok']."','".$_REQUEST['akUnit']."')";
							//echo $sqlTind."<br/>";
							mysql_query($sqlTind);
							...
							if($_REQUEST['b_dok']==1){
								...
								for($i=0; $i<(count($dok)-1); $i++){
								   $sql_an = "insert into b_tindakan_dokter_anastesi (tindakan_id,dokter_id,user_act,is_anastesi)
										 values ('".$rwIdTind['id']."','".$dok[$i]."','$userId',0)";
            ...
            case 'btnSimpanDiag':
				...
				if(mysql_num_rows($qCekDiag)>0 && $_REQUEST['isManual']=='0'){
					...
					if($diagBanding_id > 0 && $_REQUEST['isManual']!='1'){
						$sIns="insert into b_diagnosa_rm(diagnosa_id, ms_diagnosa_id, kunjungan_id, pelayanan_id, tgl, user_id, type_dokter, primer, akhir, klinis, banding, mati, kasus_baru, tgl_act_diag, user_act_diag) value 
						({$diagBanding_id}, '".$_REQUEST['idDiag']."', '{$kunjungan_id}', '{$pelayanan_id}', '".tglSQL($_REQUEST['TglDiagB'])."','".$_REQUEST['idDok']."', {$isDokPengganti}, '".$_REQUEST['isPrimer']."', '".$_REQUEST['isAkhir']."', '".$_REQUEST['isKlinis']."', '0', '".$_REQUEST['isMati']."', '{$kasusBaru}', {$tgljamDiag}, '{$userId}' )";
						//echo $sIns."<br>";
						mysql_query($sIns);
					}
					$statusProses='Fine';
					...
					if ($fdiagBanding!="*|*" && $fdiagBanding!=""){
						...
						for ($i=0;$i<(count($arrfdiagBanding)-1);$i++){
						  ...
						  if($isiDiagBanding[1]!="")
						  {
							 $sqlTambah="insert into b_diagnosa (ms_diagnosa_id,diagnosa_manual,diag_banding_id,kunjungan_id,pelayanan_id,tgl,primer,kasus_baru,akhir,klinis,banding,tgl_act,user_act,user_id,type_dokter) values('0','".$isiDiagBanding[1]."','".$diagBanding_id."','".$kunjungan_id."','".$pelayanan_id."','".tglSQL($_REQUEST['TglDiagB'])."','0','2','0','0','1',$tgljamDiag,$userId,".$_REQUEST['idDok'].",$isDokPengganti)";
							//echo $sqlTambah."<br/>";
							$rs=mysql_query($sqlTambah);
						  }
						}
...
WHERE p.pasien_id='".$_GET['pasienId']."' AND p.unit_id='".$_GET['tmpLay']."' AND p.tgl=CURDATE() AND u.penunjang=0 AND u.kategori!=5 AND u.id<>130";
				...
				if($numCek == 0){
                    ...
                    $sqlTambah1="insert into b_pelayanan(no_antrian,jenis_kunjungan,pasien_id,kunjungan_id,jenis_layanan,unit_id,kso_id,unit_id_asal,kelas_id,ket,dokter_id,type_dokter,tgl,tgl_act,user_act,dokter_tujuan_id,type_dokter_tujuan, cito, pelayanan_id_asal,rujuk) values((fGetMaxNo_Antrian(".$_REQUEST['tmpLay'].",CURDATE())),'".$pl_id."','".$_REQUEST['pasienId']."','".$kunjungan_id."','".$_REQUEST['jnsLay']."','".$_REQUEST['tmpLay']."','$kso_id','".$_REQUEST['unitAsal']."','".$plKelas."','".$_REQUEST['ket']."','".$_REQUEST['idDok']."','$isDokPengganti','$tglRujuk',now(),$userId,'".$_REQUEST['idDokTujuan']."','".$_REQUEST['isDokTujuanPengganti']."', '".$_REQUEST['cito']."', '".$pelayanan_id."',".$_REQUEST['sRujuk'].")";
					//echo $sqlTambah1."<br>";
						...
						if($_REQUEST['tmpLay'] == '61'){
							foreach($keyTindLab as $val){
								...
								$sql = "INSERT INTO b_tindakan_rad (pemeriksaan_id, pelayanan_id, kunjungan_id, tgl_act, user_act, status) 
										VALUES ('".$data[0]."', '{$idPelKey}', '{$kunjungan_id}', NOW(), '{$userId}', '".$data[1]."')";
							...
							foreach($keyTindLab as $val){
								$data = explode('||',$val);
								//echo $val;
								$sql = "INSERT INTO b_tindakan_lab (pemeriksaan_id, pelayanan_id, kunjungan_id, tgl_act, tipe, user_act, status) 
										VALUES ('".$data[0]."', '{$idPelKey}', '{$kunjungan_id}', NOW(), '{$tipe_periksa_lab}', '{$userId}', '".$data[1]."')";
								$hasil = mysql_query($sql);
							}
						}
					}
                    ...
                    if($_REQUEST['isInap']=='1') {
						...
						if ($rwCekIsInap["inap"]==1){
							...
							$sqlTambah="insert into b_tindakan_kamar (pelayanan_id,unit_id_asal,kso_id,tgl_in,kamar_id,kode,nama,tarip,retribusi,beban_kso,beban_pasien,kelas_id,aktif)
						values('".$rwNewPel['idpel']."','".$_REQUEST['unitAsal']."','$cStatPas',".$tgljam.",'".$_REQUEST['kamar']."','".$rwKamar['kode']."','".$rwKamar['nama']."','".$biaya."','".$biaya*0.03."','$beban_kso','$beban_pasien','$kelasnya','0')";
                        ...
                        if($res != 0 && $res != '' && isset($res)) {
                            ...
                            $sqlBTin = "insert into b_tindakan (ms_tindakan_kelas_id,ms_tindakan_unit_id,jenis_kunjungan,pelayanan_id,kunjungan_id,kunjungan_kelas_id,kso_id,kso_kelas_id,tgl,ket,biaya,biaya_pasien,biaya_kso,tgl_act,user_act,user_id,unit_act,banyak_user_id) values('".$rwTindKonsul['ms_tindakan_kelas_id']."','".$_REQUEST['tmpLay']."','".$pl_id."','".$rwNewPel['idpel']."','".$kunjungan_id."','".$_REQUEST['kunjungan_kelas_id']."','".$_REQUEST['ksoId']."','".$_REQUEST['ksoKelasId']."','".$tgltindak."','".$_REQUEST['ket']."','".$rwTindKonsul['tarip']."','$biaya_pasien','".$biaya_kso."',now(),'$userId',0,'".$_REQUEST['unitAsal']."','".$_REQUEST['b_dok']."')";
							//echo $sqlBTin."<br/>";
                            mysql_query($sqlBTin);
                            //".$_REQUEST['idDok']."
			...
			if(($jmlKamarSama<=0) || ($_REQUEST['tmpLayKamar'] == $_REQUEST['unitAsal'])) {
				...
				if($_REQUEST['tmpLayKamar'] == $_REQUEST['unitAsal']){
					if ($_GET['kelas_lama'] == $kelasnya){
						...
						$rs = mysql_query($sql);
						if (mysql_num_rows($rs) > 0){
							$sql = "update b_tindakan_kamar set kamar_id = '".$_GET['kamar_id']."', nama = '".$rwKamar['nama']."',tarip='".$biaya."',retribusi='".$biaya*0.03."',beban_kso='".$beban_kso."',beban_pasien='".$beban_pasien."',kelas_id='".$kelasnya."' where pelayanan_id='".$pelayanan_id."' and tgl_out is null and aktif=1";
							mysql_query($sql);
						}else{
							$sql = "update b_tindakan_kamar set tgl_out=".$tgljam." where pelayanan_id='".$pelayanan_id."' and tgl_out is null and aktif=1";
							mysql_query($sql);
						
							$sql = "insert into b_tindakan_kamar (pelayanan_id,unit_id_asal,kso_id,tgl_in,kamar_id,kode,nama,tarip,retribusi,beban_kso,beban_pasien,kelas_id,aktif) values('".$pelayanan_id."','".$_REQUEST['unitAsal']."','$kso_id',".$tgljam.",'".$_REQUEST['kamar_id']."','".$rwKamar['kode']."','".$rwKamar['nama']."','".$biaya."','".$biaya*0.03."','".$beban_kso."','".$beban_pasien."','$kelasnya','1')";
							mysql_query($sql);
						}
					}
				}
					...
					$sqlTambah="insert into b_tindakan_kamar (pelayanan_id,unit_id_asal,kso_id,tgl_in,kamar_id,kode,nama,tarip,retribusi,beban_kso,beban_pasien,kelas_id,aktif) values('".$rwNewPel['idpel']."','".$_REQUEST['unitAsal']."','$kso_id',".$tgljam.",'".$_REQUEST['kamar_id']."','".$rwKamar['kode']."','".$rwKamar['nama']."','".$biaya."','".$biaya*0.03."','".$beban_kso."','".$beban_pasien."','$kelasnya','$caktif')";
					////////////////////////////////////////////////////////////////
					//echo $sqlTambah."<br>";
					$rs=mysql_query($sqlTambah);
					//mysql_free_result($rsCekStat);
										...
										while ($rwBiayaKamar=mysql_fetch_array($rsBiayaKamar)){
											if ($rwBiayaKamar['tarip']==0 || $rwBiayaKamar['beban_kso']==0){
												if ($rwBiayaKamar['kamar_id']!=0){
													...
													$rowBiayaKamarPasien +=$bPx * $rwBiayaKamar['qtyHari'];
													$sql="UPDATE b_tindakan_kamar SET tarip='".$rwKmr['tarip']."',retribusi='".($rwKmr['tarip']*0.03)."',beban_kso='".$paket_hp."',beban_pasien='".$bPx."',kelas_id='".$_GET['kelas_id']."' WHERE id='".$rwBiayaKamar['id']."'";
													$rsKmr=mysql_query($sql);											
												}else{
													$sql="SELECT mkt.*,mk.kode,mk.nama FROM b_ms_kamar_tarip mkt INNER JOIN b_ms_kamar mk ON mkt.kamar_id=mk.id WHERE mkt.unit_id='".$unitId."' AND mkt.kelas_id='".$_GET['kelas_id']."' LIMIT 1";
													//echo $sql."<br>";
	...
	WHERE pd.kso_paket_id=$idpaket AND t1.tgl='".$rwcekTindAsli["tgl"]."'";
				...
				if ($icd9cm=="true"){
					...
					if($_REQUEST['b_dok']==1){
						...
						for($i=0; $i<(count($dok)-1); $i++){
						   $sql_an = "insert into b_tindakan_dokter_anastesi (tindakan_id,dokter_id,user_act,is_anastesi)
								 values ('".$_REQUEST['id']."','".$dok[$i]."','$userId',0)";
						   mysql_query($sql_an) or die(mysql_error()." - line 860");
						}	
					}
					...
					if ($fdiagBanding!="*|*" && $fdiagBanding!=""){
						...
						if($isiDiagBanding[1]!=$isiDiagBanding[2]){
						 if($isiDiagBanding[0]!=''){
							...
							$sqlTambah="insert into b_diagnosa (ms_diagnosa_id,diagnosa_manual,diag_banding_id,kunjungan_id,pelayanan_id,tgl,primer,kasus_baru,akhir,klinis,banding,tgl_act,user_act,user_id,type_dokter) values
							('0','".$isiDiagBanding[1]."','".$_REQUEST['id']."','".$kunjungan_id."','".$pelayanan_id."','".tglSQL($_REQUEST['TglDiagB'])."','0','2','0','0','1',$tgljamDiag,$userId,".$_REQUEST['idDok'].",$isDokPengganti)";
							//echo $sqlTambah."<br/>";
							$rs=mysql_query($sqlTambah);
						 }
						}
    ...
    case 'hapus':
		...
		if (mysql_errno()==0){
			if ($_REQUEST["hps"]=='btnHapusTind' && $msg==""){
				...
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act) VALUES('$kethapus','$sqlDelete',now(),'$userId')";
				mysql_query($sql);
			}
		}
	//INSYA ALLAH
...
WHERE t.pelayanan_id = '".$pelayanan_id."'
	...
	elseif($grdResep == "true") {	//resep --joker--
		/*
        $sql = "select r.id,r.apotek_id,r.dokter_id,IF(r.racikan=0,'Bukan',IF(r.obat_id=0,'Racikan',CONCAT('Racikan',' ',r.racikan,' (',r.qty_bahan,' ',r.satuan,')'))) as racikan,r.racikan as racikanfix,r.qty_bahan,r.qty,r.ket_dosis,r.obat_id,r.dokter_nama,IFNULL(o.OBAT_NAMA,r.obat_manual) OBAT_NAMA,r.type_dokter,r.tgl, r.diagnosa_id, r.obat_manual, r.id_bentuk, r.jhobat
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
		  ...
		  if(mysql_num_rows($rs_an) > 0){
            ...
            //$sisip = $rows['id']."|".$rows['ms_tindakan_kelas_id']."|".$rows['ms_tindakan_unit_id']."|".$rwJns['parent_id']."|".$rows['user_id']."|".$rows['kunjungan_id']."|".$rows['unit_act']."|".$rows['type_dokter']."|".$rows['klasifikasi_id'].'|'.$dokter_an.'|'.$rows['ms_tindakan_id_icd9cm'].'|'.$rows["nama"].'|'.$rows["kelas"].'|'.$rows["biaya"].'|'.$rows["qty"].'|'.$rows["ket"].'|'.tglSQL($rows["tanggal3"]).'|'.$rows["sampah"];//joker;
