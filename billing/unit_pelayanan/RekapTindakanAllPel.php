...
IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*tk.retribusi, 
								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*tk.retribusi) retribusi,								 
                            ...
                            <td align="center" style="font-size:12px;">&nbsp;</td>
                                ...
                                <?php 
									...
									if($rwKSO['id']!=$idKsoUmum && (strpos(",,".$idKsoBPJS.",",",".$rwKSO['id'].",")<=0) && (strpos(",,".$idKsoJR.",",",".$rwKSO['id'].",")<=0)){
										...
										$subBebanPasien+=$nBebanPasienKamar+$nBiayaJaminanRetribusi;
									}
								}
							}
					...
					/* if($rwKSO['id']==1){
                        ...
                        <?php
							$sObat="SELECT * 
									  ...
									  am.NAMA KSO, 
									  SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN)+
									  FLOOR(
									  		(ap.PPN/100) *  SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN)
										  	)
									   AS SUBTOTAL,
									...
									IF(ap.CARA_BAYAR=2,   SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN)+
									  FLOOR(
									  		(ap.PPN/100) *  SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN)
										  	),0)
									...
									,'0')
									  ...
									  ap.KRONIS,
									  ap.DIJAMIN AS STAT_DIJAMIN, ap.id idpenjualan
									FROM
									  (SELECT 
										dba_ap.* 
									  FROM
						...
						while($rwObat=mysql_fetch_array($qObat)){
							...
							$sRetur="SELECT SUM(a.QTY_RETUR) QTY, SUM(a.QTY_RETUR * a.HARGA_SATUAN) AS SUBTOTAL, SUM(IF(a.CARA_BAYAR=2,(a.QTY_RETUR * a.HARGA_SATUAN),0)) AS DIJAMIN,
								...
								WHERE a.NO_PENJUALAN = '".$rwObat['NO_PENJUALAN']."' AND a.UNIT_ID = '".$rwObat['UNIT_ID']."' AND a.NO_KUNJUNGAN = '".$rwObat['NO_KUNJUNGAN']."' AND a.NO_PASIEN = '".$rwObat['NO_PASIEN']."'
