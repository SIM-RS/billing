...
if ($tipe=="2") {
	$skso="SELECT DISTINCT IF(t.kso_id IS NULL,p.kso_id,t.kso_id) kso_id,mk.nama FROM b_pelayanan p
INNER JOIN b_ms_unit mu ON p.unit_id=mu.id LEFT JOIN b_tindakan t ON p.id=t.pelayanan_id LEFT JOIN b_ms_kso mk ON t.kso_id=mk.id
WHERE p.kunjungan_id='$idKunj'";
}elseif ($tipe=="1") {
	$skso="SELECT DISTINCT IF(t.kso_id IS NULL,p.kso_id,t.kso_id) kso_id,mk.nama 
FROM b_pelayanan p INNER JOIN b_ms_unit mu ON p.unit_id=mu.id LEFT JOIN b_tindakan t ON p.id=t.pelayanan_id LEFT JOIN b_ms_kso mk ON t.kso_id=mk.id 
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
                        <tr height="30">
                          <td width="28" align="left"  style="border-top:#000000 solid 1px; border-bottom:#000000 solid 1px; font-weight:bold;font-size:12px">&nbsp;</td>
                            <td align="left" width="380" style="padding-left:0px;border-top:#000000 solid 1px; border-bottom:#000000 solid 1px; font-weight:bold;font-size:12px">Uraian Layanan</td>
                            <td align="center" width="101" style="border-top:#000000 solid 1px; border-bottom:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px">Hak Kelas</td>
						...
						if ($tipe=="2"){
                        	$sKSO="SELECT id,nama,urutan FROM (SELECT 
									DISTINCT 
									IF(kso.id IS NULL,p.kso_id,kso.id) id,
									kso.nama,
									t.id as urutan
									FROM b_pelayanan p
									INNER JOIN b_ms_unit mu ON p.unit_id=mu.id 
									...
									LEFT JOIN b_ms_kso kso ON kso.id=tk.kso_id
                        ...
                        while($rwUnit=mysql_fetch_array($qUnit)) {
							...
							// 		$tgl_selisih=$cekselisih2['selisih'];
							// 		echo $tgl_selisih;
							// 	$updateselisih = "UPDATE b_tindakan set qty = '7' where kunjungan_id = '$idKunj' AND pelayanan_id = '".$rwUnit['id']."' AND (ms_tindakan_kelas_id = '3602' OR ms_tindakan_kelas_id = '3599' OR ms_tindakan_kelas_id = '3600' OR ms_tindakan_kelas_id = '3601' OR ms_tindakan_kelas_id = '3597')";
                        ...
                        <tr>
							...
							if ($rwUnit["inap"]==1){
								...
								IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*(tk.tarip), 
								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*(tk.tarip)) biaya,
								
-- IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*(tk.retribusi), 
-- 								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*(tk.retribusi)) 
								tk.retribusi, -- biaya admin cman sekali decyber-->
				 
								IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*tk.beban_kso, 
								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*tk.beban_kso) biaya_kso,
								 
								IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*tk.beban_pasien, 
								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)*tk.beban_pasien) biaya_px,
								
								tk.bayar,tk.bayar_kso, 
								
								IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1),
								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)) cHari,
								...
								while($rwKmr=mysql_fetch_array($qKmr)) {
									...
									//else if($rwKSO['id']==9){
										...
										if($rwKSO['id']==$idKsoUmum){
											$nBiayaJaminanKamar=0;
											$nBiayaJaminanRetribusi=0;
											$totBiaya_umum+=$rwKmr['biaya']+$rwKmr['retribusi'];	
										}else if($rwKSO['id'] == 100){
											$nBiayaJaminanKamar = 0;
											$nBiayaJaminanRetribusi = 0;
										}
										else{
											$subBiayaKSO+=$rwKmr['biaya']+$rwKmr['retribusi'];
											$nBiayaJaminanRetribusi=$rwKmr['retribusi'];
										}
									}
                            ?>
                            <tr>
                              <td align="left" style="font-size:12px">&nbsp;</td>
                              <td align="left" style="padding-left:75px;font-size:12px"><?php echo "Administrasi"; ?></td>
                              ...
                              <td align="right" style="font-size:12px;"><?php echo number_format($rwKmr['retribusi'],0,",",".");?>&nbsp;</td>
                              <td align="right" style="font-size:12px;"><?php echo number_format($nBiayaJaminanRetribusi,0,",",".");?>&nbsp;</td>
                              <td align="center" style="font-size:12px;">&nbsp;</td>
                            </tr>
                        <tr>
                          <td align="left" style="font-size:12px">&nbsp;</td>
                            ...
                            <td align="center" style="font-size:12px;">&nbsp;</td>
                                ...
                                <?php 
									$subBiaya+=$rwKmr['biaya']+$rwKmr['retribusi'];
									$subNilaiJaminan+=$nBiayaJaminanKamar+$nBiayaJaminanRetribusi;
									
									$subTotal+=$rwKmr['biaya']+$rwKmr['retribusi'];
									$subTotalJaminan+=$nBiayaJaminanKamar+$nBiayaJaminanRetribusi;
									
									//if($rwKSO['id']!=1 && $rwKSO['id']!=6 && $rwKSO['id']!=9){
									if($rwKSO['id']!=$idKsoUmum && (strpos(",,".$idKsoBPJS.",",",".$rwKSO['id'].",")<=0) && (strpos(",,".$idKsoJR.",",",".$rwKSO['id'].",")<=0)){
										$nBebanPasienKamar=($rwKmr['biaya']-$nBiayaJaminanKamar)+($rwKmr['retribusi']-$nBiayaJaminanRetribusi);
					...
					/* if($rwKSO['id']==1){
						...
						$umum_stlh_krs2 = " AND t._krs<>1 ";
							...
							$sqlDiskonDokter = "SELECT dokter_diskon FROM b_bayar WHERE kunjungan_id = {$_REQUEST['idKunj']}";
							$queryDiskon = mysql_query($sqlDiskonDokter);
							while($rows = mysql_fetch_assoc($queryDiskon)){
								$pecah = explode("**",$rows['dokter_diskon']);
								for($i = 0; $i< sizeof($pecah);$i++){
									$keyVal = explode(':',$pecah[$i]);
...
// 								DATE_FORMAT(tk.tgl_in,'%d-%m-%Y') tgl_in,
// 								IF(tk.status_out=0,IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1), 
// 								IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)+1)) jmlh
// 								FROM b_tindakan_kamar tk 
// 								INNER JOIN b_pelayanan p ON tk.pelayanan_id=p.id 
// 								LEFT JOIN b_ms_kamar mk ON tk.kamar_id=mk.id 
// 								LEFT JOIN b_ms_kelas mKls ON tk.kelas_id=mKls.id 
...
// // 
// // echo $sKmr;
// $hehe=mysql_query($sKmr);
// while($hehe1=mysql_fetch_array($hehe)){
// echo $hehe1['jmlh'];
// if($hehe == FALSE){
// 	echo mysql_error();
// }
// }
                                ...
                                while($rwTind=mysql_fetch_array($qTind)) {
									...
									$diskonPerDokter = $arr[$rwKonsul['user_id']];
								?>
                        <tr>
						...
						$sObat="SELECT *, 
								  ...
								  FLOOR(
								   ...
								   FLOOR( 
									  ...
									  ap.KRONIS,
									  ap.DIJAMIN AS STAT_DIJAMIN, ap.id AS idpenjualan,
									  au.UNIT_NAME
									...
									FROM
									  ...
									  INNER JOIN $dbapotek.a_unit au ON ap.UNIT_ID=au.UNIT_ID
									  where kso.id='".$rwKSO['id']."'
									  $fCaraBayar
									GROUP BY ap.NO_PENJUALAN) AS tbl ORDER BY OBAT_NAMA";
						//echo $sObat."<br><br>"; //decyber
						...
						while($rwObat=mysql_fetch_array($qObat)){
							...
							$sRetur="SELECT SUM(a.QTY_RETUR) QTY, SUM(a.QTY_RETUR * a.HARGA_SATUAN) AS SUBTOTAL, SUM(IF(a.CARA_BAYAR=2,(a.QTY_RETUR * a.HARGA_SATUAN),0)) AS DIJAMIN,
								b.TGL_BAYAR, c.tgl_retur, DATEDIFF(c.tgl_retur,b.TGL_BAYAR) AS cek, IF(TIMEDIFF(c.tgl_retur,b.TGL_BAYAR)>0,'1','0') AS DiffDate
                        ...
                        <tr>
                          <td align="left">&nbsp;</td>
                            <td align="left" style="padding-left:75px"><?php echo "Pembelian Obat - NOTA PENJUALAN : ".$rwObat['NO_PENJUALAN'];//." - ".$rwObat['UNIT_NAME'];?></td>
                            ...
                            <td align="center" style="font-size:12px;"><?php //echo $rwObat['QTY'];//$qtyObt;?></td>
						...
						<tr id="trSubTotal" style="display:table-row; vertical-align:bottom" height="20">
                        	<td colspan="5">&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px; ">Diskon&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px;"><?php echo number_format($diskonPerDokter,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px;"><?php echo number_format($subTotalJaminan,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px;">&nbsp;</td>
                        </tr>
                            <?php
                        } // END UNIT
						...
						<?php
						 //get total diskon
							...
							$diskonRs = 0;
							$diskonDokter = 0;
							$sqlDiskon = "SELECT nilai_diskon,nilai_diskon_dokter FROM b_bayar WHERE kunjungan_id = {$_REQUEST['idKunj']}";
							$query = mysql_query($sqlDiskon);
							while($rows = mysql_fetch_assoc($query)){
								$diskonRs += $rows['nilai_diskon'];
								$diskonDokter += $rows['nilai_diskon_dokter'];
								$totalDiskon += ($rows['nilai_diskon'] + $rows['nilai_diskon_dokter']);
							}
						?>
						<tr style="display:table-row; vertical-align:bottom" height="20">
                        	<td>&nbsp;</td>
                        	<td colspan="5" align="right" style="font-weight:bold; font-size:12px; border-top:solid #000000 1px;">Diskon RS&nbsp;</td>
							<td align="right" style="font-weight:bold; border-top:solid #000000 1px;font-size:12px;"><?php echo number_format($diskonRs,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px;">&nbsp;</td>
						</tr>
						<tr style="display:table-row; vertical-align:bottom" height="20">
                        	<td>&nbsp;</td>
                        	<td colspan="5" align="right" style="font-weight:bold; font-size:12px;">Diskon Dokter&nbsp;</td>
							<td align="right" style="font-weight:bold;font-size:12px;"><?php echo number_format($diskonDokter,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px;">&nbsp;</td>
                        </tr>
						<tr style="display:table-row; vertical-align:bottom" height="20">
                        	<td>&nbsp;</td>
                        	<td colspan="5" align="right" style="font-weight:bold; font-size:12px; border-top:solid #000000 1px;">Total Diskon&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px;font-size:12px;"><?php echo number_format($totalDiskon,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px;">&nbsp;</td>
                        </tr>
						...
						*/
						...
						$qReturBayar = "SELECT
										...
										FROM $dbbilling.b_return_pembayaran r
										  INNER JOIN $dbbilling.b_bayar b
											ON r.bayar_id = b.id
										  INNER JOIN rspelindo_keuangan.k_ms_user peg
                        ...
                        <tr height="30">
                          <td align="left" style=" font-size:12px">&nbsp;</td>
                            ...
                            <td align="right" style="font-weight:bold; font-size:12px"><?php echo number_format($totBiaya-$totalDiskon,0,",",".");?>&nbsp;</td>
              ...
              <td width="607" style="vertical-align:top">
              		<table width="100%" border="0" cellpadding="0" cellspacing="0"> 
                        ...
                        <tr <?php echo $disp_UMUM; ?>>
                          ...
                          <td width="19%" align="right" style="font-weight:bold;font-size:12px;"><?php echo number_format($totBiaya_umum-$totalDiskon,0,",","."); ?>&nbsp;</td>
                        </tr>
                        <tr <?php echo $disp_BPJS_Pav_UMUM; ?>>
                        ...
                        <tr>
                            ...
                            <td align="right" style="font-weight:bold;font-size:12px; border-top:1px solid;"><?php echo number_format($total_tagihan_pasien-$totalDiskon,0,",","."); ?>&nbsp;</td>
                        </tr>
                        <tr>
                            <td height="19" align="right" style="font-weight:bold;font-size:12px"></td>
                            <td align="right" style="font-weight:bold;font-size:12px">&nbsp;</td>
                       ...
                       <tr>
                            ...
                            <td align="right" style="font-weight:bold;font-size:12px"><?php echo number_format($totBAYAR-$totalDiskon,0,",",".");?>&nbsp;</td>
                        </tr>
                        <?php
						if($nReturObat>0){
						?>
            ...
            <tr class="kwi">
            	<td colspan="2">
                	...
                	<table width="87%" border="0" cellpadding="0" cellspacing="0"> 
                        ...
                        <tr>
                            <td align="left" style="font-size:12px;font-size:10px;">&nbsp;</td>
                            <td width="8%" align="center" style="font-size:12px;font-size:10px;"><?php echo $no; ?>.</td>
                            <td width="18%" align="center" style="font-size:12px;font-size:10px;"><?php echo $rwReturBayar['tgl']; ?></td>
                            <td width="14%" align="center" style="font-size:12px;font-size:10px;"><?php echo $rwReturBayar['nokwi']; ?></td>
							<td width="14%" align="center" style="font-size:12px;font-size:10px;"><?php echo $rwReturBayar['status_penj']; ?></td>
                            <td align="right" style="font-size:12px;font-size:10px;"><?php echo "(".number_format($rwReturBayar['nominal'],0,",",".").")"; ?>&nbsp;</td>
                            <td width="21%" align="center" style="font-size:12px;font-size:10px;"><?php echo $rwReturBayar['kasir']; ?></td>
                            <td width="21%" align="right" style="font-size:12px;font-size:10px;">&nbsp;</td>
              ...
              <td width="444" align="center" style="font-weight:bold; vertical-align:top">Belawan, <?php echo gmdate('d-m-Y',mktime(date('H')+7));?><br/>
            ...
            <tr id="trTombol">
                <td colspan="2" class="noline" align="center">
					...
					&nbsp;<input type="button" id="btnExpExcl" style="cursor:pointer" onClick="zxc()" value="Export ke Excel">
