...
<?php 
	...
	$sql="select * from (select * from (SELECT mt.id, mt.unit_id, mt.kode, mt.nama, tk.ms_kelas_id, tk.tarip, tk.tarip_askes, tk.ms_tindakan_id, tk.id AS idtk,k.nama as kelas,kl.nama AS kelompok
...
WHERE mt.aktif = 1 AND tu.ms_unit_id='".$unitId."' ".$tambahan." AND (mt.nama LIKE '$aKeyword%' OR mt.kode LIKE '$aKeyword%') order by mt.nama) as t1
	...
	INNER JOIN b_ms_kelas k ON tk.ms_kelas_id=k.id
	INNER JOIN b_ms_kelompok_tindakan kl ON mt.kel_tindakan_id=kl.id
	WHERE mt.aktif = 1 AND tu.ms_unit_id='".$unitId."' AND (mt.nama LIKE '$aKeyword%' OR mt.kode LIKE '$aKeyword%') order by mt.nama) as t1
	) as gab";
		//echo $sql;
		$rs=mysql_query($sql);
	}
	...
	{
		$i++;
		$arfvalue=$rows['idtk']."*|*".$rows['nama']."*|*".$rows["kode"]."*|*".$rows["tarip"]."*|*".$rows["id"]."*|*".$rows['kelas']."*|*".$rows['tarip_askes'];
?>
