...
WHERE t.pelayanan_id = '".$pelayanan_id."') AS gab $filter order by $sorting";
    ...
    elseif($grdAnamnesa == "true") {	//Anamnese
         $sql = "SELECT a.*, DATE_FORMAT(a.TGL,'%d-%m-%Y %H:%i') AS tgltok, p.nama AS dokter
				...
				WHERE a.PASIEN_ID = '".$_REQUEST['pasien_id']."'";
    }
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
		  ...
		  if(mysql_num_rows($rs_an) > 0){
            ...
            $sisip = $rows['id']."|".$rows['ms_tindakan_kelas_id']."|".$rows['ms_tindakan_unit_id']."|".$rwJns['parent_id']."|".$rows['user_id']."|".$rows['kunjungan_id']."|".$rows['unit_act']."|".$rows['type_dokter']."|".$rows['klasifikasi_id'].'|'.$dokter_an.'|'.$rows['ms_tindakan_id_icd9cm'];//joker;
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).tglJamSQL($rows["tanggal"]).chr(3).$rows["nama"].chr(3).$rows["kelas"].chr(3).$rows["biaya"].chr(3).$rows["qty"].chr(3).$rows["subtotal"].chr(3).$dokter.chr(3).$rows["petugas"].chr(3).$rows["ket"].chr(6);
        }
    }
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $sisip=$rows["id"]."|".$rows['user_id']."|".$rows['ms_diagnosa_id']."|".$rows['type_dokter']."|".$rows['primer']."|".$rows['akhir']."|".$gol_id."|".$gol."|".$isPK."|".$rows['klinis']."|".$rows['banding']."|".$rows['ismanual'];
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$rows["dokter"].chr(3).$rows["utama"].chr(3).$rows['d_akhir'].chr(3).$rows['d_klinis'].chr(3).$rows['nm_unit'].chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
    ...
    elseif($grdAnamnesa == "true") {	//Anamnese
        ...
        while($rows=mysql_fetch_array($rs)) {
            ...
            $sisipan=$rows["id"]."|".$rows['qty']."|".$rows['obat_id']."|".$rows['harga_jual']."|".$rows['stok']."|".$rows['apotek_id']."|".$rows["dokter_id"]."|".$rows['type_dokter']."|".$rows['tgl']."|".$rows['diagnosa_id']."|".$rows["racikanfix"]."|".$rows["qty_bahan"]."|".$rows["obat_manual"]."|".$rows["id_bentuk"]."|".$rows["jhobat"];
            $i++;
            $dt.=$sisipan.chr(3).number_format($i,0,",","").chr(3).$obatNama.chr(3).$rows['qty'].chr(3).$rows["racikan"].chr(3).$rows['ket_dosis'].chr(3).$rows["dokter_nama"].chr(6);
        }
    }elseif($grdRsp1 == "true") {	//resep
        while($rows=mysql_fetch_array($rs)) {
            $i++;
