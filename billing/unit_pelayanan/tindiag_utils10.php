	...
	FROM b_tindakan_kamar WHERE pelayanan_id='".$pelayanan_id."'";
		...
		WHERE pd.kso_paket_id=$idpaket AND t1.tgl=CURRENT_DATE()";
            ...
            case 'btnSimpanResep':
				...
				billing.b_ms_pasien.no_rm, $dbbilling.b_ms_pasien.alamat,
				billing.b_pelayanan.kso_id,billing.b_ms_kso.kode, $dbbilling.b_ms_kso.nama AS ksonama, $dbapotek.a_mitra.KEPEMILIKAN_ID
				...
				LEFT JOIN $dbapotek.a_mitra ON $dbapotek.a_mitra.KODE_MITRA = $dbbilling.b_ms_kso.kode
...
$dbapotek.a_unit.UNIT_ID,$dbapotek.a_unit.UNIT_NAME, IF(b_resep.status=0,'Belum Dilayani','Sudah Dilayani') STATUS
FROM b_resep
INNER JOIN $dbapotek.a_unit ON $dbapotek.a_unit.UNIT_ID=billing.b_resep.apotek_id
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' GROUP BY no_resep,tgl,apotek_id ORDER BY tgl DESC,no_resep DESC";
	...
	elseif($grdRsp2 =="true"){	//resep
		$sql = "SELECT b_resep.id, $dbapotek.a_obat.OBAT_NAMA, b_resep.qty, b_resep.ket_dosis, IF(b_resep.racikan=0,'Bukan',CONCAT('Racikan',' ',b_resep.racikan,' (',b_resep.qty_bahan,' ',b_resep.satuan,')')) AS racikan, b_resep.dokter_nama 
FROM b_resep
INNER JOIN $dbapotek.a_obat ON $dbapotek.a_obat.OBAT_ID=billing.b_resep.obat_id
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."' ORDER BY $dbapotek.a_obat.OBAT_ID";
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["kode"].chr(3).$rows["nama"].chr(3).$rows["dokter"].chr(3).$rows["utama"].chr(3).$rows['d_akhir'].chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
    ...
    elseif($grdResep == "true") {	//resep
        ...
        while($rows=mysql_fetch_array($rs)) {
            $i++;
			$id=$rows["UNIT_ID"]."|".$rows['tgl']."|".$rows['no_resep'];
            $dt.=$id.chr(3).number_format($i,0,",","").chr(3).$rows["no_resep"].chr(3).$rows['tanggal'].chr(3).$rows["UNIT_NAME"].chr(3).$rows["STATUS"].chr(6);
        }
    }elseif($grdRsp2 == "true") {	//resep
        while($rows=mysql_fetch_array($rs)) {
            $i++;
            $dt.=$id.chr(3).number_format($i,0,",","").chr(3).$rows["OBAT_NAMA"].chr(3).$rows['qty'].chr(3).$rows["racikan"].chr(3).$rows['ket_dosis'].chr(3).$rows["dokter_nama"].chr(6);
        }
    }
