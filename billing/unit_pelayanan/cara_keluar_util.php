...
//===============================
$nama=$_REQUEST['txtCaraKeluar'];
$aktif=$_REQUEST['status'];
$id_cara_keluar=$_REQUEST['id_cara_keluar'];
...
switch(strtolower($_REQUEST['act'])) {
	case 'view_last':
		...
		break;
    case 'tambah':
		$sql="insert into b_ms_cara_keluar(nama,aktif,flag) values ('$nama','$aktif','$flag')";
		mysql_query($sql);
	   	break;
	case 'simpan':
		$sql="update b_ms_cara_keluar set nama='".$nama."', aktif='".$aktif."', flag='".$flag."' where id='".$id_cara_keluar."'";
		mysql_query($sql);
	   	break;
	case 'hapus':
		$sql="delete from b_ms_cara_keluar where id='".$id_cara_keluar."'";
		mysql_query($sql);
	   	break;
}
...
if($statusProses=='Error') {
	...
	$sql="select * from b_ms_cara_keluar ".$filter." ORDER BY ".$sorting;   
    ...
    while ($rows=mysql_fetch_array($rs)) {
		$i++;
		$dt.=$rows["id"]."|".$rows["nama"]."|".$rows["aktif"].chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$pilih[$rows["aktif"]].chr(6);
    }
