...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body style="margin-top:0px">
        <table width="900" border="0" cellspacing="0" cellpadding="0" align="left" class="kwi">
			...
			<?php
			$sqlc="SELECT DISTINCT d.* FROM (SELECT * FROM b_hasil_lab WHERE id_pelayanan='$idPel' AND verifikasi > 0) AS a 
...
INNER JOIN b_ms_kelompok_lab d ON c.kelompok_lab_id=d.id";
			...
			?>
			<tr height="25">
                    <td align="left" colspan="7" style="padding-left:10px;font-size:12px;border-bottom:1px solid;border-top:1px solid" >&nbsp;<i><b><?php echo $spasi.strtoupper($dt1["nama_kelompok"]); ?></b></i></td>
            </tr>
			<?php
...
FROM (SELECT * FROM b_hasil_lab WHERE id_pelayanan='$idPel' AND verifikasi > 0) AS a 
INNER JOIN b_ms_normal_lab b ON a.id_normal=b.id
INNER JOIN b_ms_pemeriksaan_lab c ON b.id_pemeriksaan_lab=c.id
...
WHERE c.kelompok_lab_id='".$dt1['id']."'";
			...
			?>
                ...
                <td width="293" style="font-weight:bold;font-size:12px">Sidoarjo, <?php echo gmdate('d F Y',mktime(date('H')+7));?><br/>
				Penanggungjawab
