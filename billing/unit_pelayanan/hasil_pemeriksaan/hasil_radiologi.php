...
$txtHasilAsli=str_replace("&amp;","&",$txtHasilAsli);
$ArrtxtHasil=explode("\n",$txtHasilAsli);
$jmlDataIsi=count($ArrtxtHasil);
...
function printjZebra(){
	...
	<?php 
		...
		$pjgLblHIsiKanan=27;
		$strGaris="";
		for ($i=0;$i<$pjgPerLine;$i++){
			$strGaris .="- ";
		}
		if ($jmlDataIsi>$pjgLineIsi){
			$jmlHal=$jmlDataIsi/$pjgLineIsi;
			if ($jmlHal>floor($jmlDataIsi/$pjgLineIsi)) $jmlHal=floor($jmlHal)+1;
		}//echo $jmlDataIsi."|".$jmlHal."\n";
		for ($j=0;$j<$jmlHal;$j++){
			...
			echo "applet.append(\"\\x1B\\x4F\")"; echo chr(59);echo "\n\t\t\t";//ESC O -> Cancel Top & Bottom margin
			//echo "applet.append(\"\\x1B\\x4E\\x00\")"; echo chr(59);echo "\n\t\t\t";//ESC N -> Set Bottom margin
			...
			//echo "applet.append(\"\\x7F\")"; echo chr(59);echo "\n\t\t\t";//DEL -> Delete Last Char In Buffer
			
			for ($i=0;$i<$marginTopLine;$i++){
				echo "applet.append(\"\\n\")"; echo chr(59);echo "\n\t\t\t";
			}
			if ($j==0){
				echo "applet.append(\"                                             HASIL PEMERIKSAAN RADIOLOGI\\n\")"; echo chr(59);echo "\n\t\t\t";
				echo "applet.append(\"\\n\")"; echo chr(59);echo "\n\t\t\t";
				echo "applet.append(\"".StrAddSpace("Tgl. Kunjung",$pjgLblHKiri).": ".StrAddSpace($rw['tgl_kunjungan'],$pjgLblHIsiKiri).StrAddSpace("Rujukan Dari",$pjgLblHKanan).": ".StrAddSpace($rw['nama'],$pjgLblHIsiKanan)."\\n\")"; echo chr(59);echo "\n\t\t\t";
				echo "applet.append(\"".StrAddSpace("No. RM",$pjgLblHKiri).": ".StrAddSpace($rw['no_rm'],$pjgLblHIsiKiri).StrAddSpace("Dokter Pengirim",$pjgLblHKanan).": ".StrAddSpace($rw['dpeng'],$pjgLblHIsiKanan)."\\n\")"; echo chr(59);echo "\n\t\t\t";
				echo "applet.append(\"".StrAddSpace("Nama Pasien",$pjgLblHKiri).": ".StrAddSpace($rw['pasien'],$pjgLblHIsiKiri).StrAddSpace("Tgl. Periksa",$pjgLblHKanan).": ".StrAddSpace($rw['tgl'],$pjgLblHIsiKanan)."\\n\")"; echo chr(59);echo "\n\t\t\t";
				echo "applet.append(\"".StrAddSpace("Tgl. Lahir",$pjgLblHKiri).": ".StrAddSpace($rw['tgl_lahir'],$pjgLblHIsiKiri).StrAddSpace("Tgl. Hasil",$pjgLblHKanan).": ".StrAddSpace($rw['tgl_act'],$pjgLblHIsiKanan)."\\n\")"; echo chr(59);echo "\n\t\t\t";
				echo "applet.append(\"".StrAddSpace("Status Px",$pjgLblHKiri).": ".StrAddSpace($rw['status'],$pjgLblHIsiKiri).StrAddSpace("Hak Kelas",$pjgLblHKanan).": ".StrAddSpace($rw['hakkelas'],$pjgLblHIsiKanan)."\\n\")"; echo chr(59);echo "\n\t\t\t";
			...
			if ($jmlHal>1){
				echo "applet.append(\"Uraian Hasil Pemeriksaan (hal ".($j+1).")\\n\")"; echo chr(59);echo "\n\t\t\t";
			}else{
				echo "applet.append(\"Uraian Hasil Pemeriksaan\\n\")"; echo chr(59);echo "\n\t\t\t";
			}
			echo "applet.append(\"".$strGaris."\\n\")"; echo chr(59);echo "\n\t\t\t";
			if ($jmlDataIsi>$pjgLineIsi){
				$printPerHal=$pjgLineIsi;
				$jmlDataIsi=$jmlDataIsi-$pjgLineIsi;
			}else{
				$printPerHal=$jmlDataIsi;
			}
			/*for ($i=0;$i<count($ArrtxtHasil)-1;$i++){
				echo "applet.append(\"".$ArrtxtHasil[$i]."\\n\")"; echo chr(59);echo "\n\t\t\t";
			}
			for ($i=0;$i<($pjgLineIsi-count($ArrtxtHasil));$i++){
				echo "applet.append(\"\\n\")"; echo chr(59);echo "\n\t\t\t";
			}*/
			for ($i=0;$i<$printPerHal;$i++){
				echo "applet.append(\"".$ArrtxtHasil[$j*$pjgLineIsi+$i]."\\n\")"; echo chr(59);echo "\n\t\t\t";
			}
			for ($i=0;$i<($pjgLineIsi-$printPerHal);$i++){
				echo "applet.append(\"\\n\")"; echo chr(59);echo "\n\t\t\t";
			}
			echo "applet.append(\"".$strGaris."\\n\")"; echo chr(59);echo "\n\t\t\t";
			echo "applet.append(\"\\n\")"; echo chr(59);echo "\n\t\t\t";
			if ($j==($jmlHal-1)){
				...
				$pjgDok=strlen($rw['drad'])+4;
				if ($pjgDok>20){
					$pjgDok=$pjgTtd+floor(($pjgDok-20)/2);
				}else{
			...
			echo "applet.print()"; echo chr(59); echo "\n\t\t\t";
			//echo "applet.append(\"\\n\")"; echo chr(59);echo "\n\t\t\t";
		}
		...
		echo "setTimeout('window.close()','15000');";
	?>
	}
}
</script>
