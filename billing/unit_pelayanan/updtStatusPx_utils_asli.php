...
while ($rw=mysql_fetch_array($rs)){
	...
	if ($cstatusPx=="1"){//UMUM
		...
		while ($rw1=mysql_fetch_array($rs1)){
			...
			//$sql="UPDATE b_tindakan SET kso_id='$cstatusPx',kso_kelas_id=1,biaya_kso=biaya,biaya_pasien=0 WHERE id='".$rw1['id']."'";
			//echo $sql."<br>";
			$rs2=mysql_query($sql);
			if (mysql_errno()>0){
				$dt="Proses Update Gagal !";
		...
		while ($rw1=mysql_fetch_array($rs1)){
			...
			//$sql="UPDATE b_tindakan_kamar SET kso_id='$cstatusPx',beban_kso=tarip,beban_pasien=0 WHERE id='".$rw1['id']."'";
			//echo $sql."<br>";
			$rs2=mysql_query($sql);
			if (mysql_errno()>0){
				$dt="Proses Update Gagal !";
		...
		$resep = "select * from b_resep where id_pelayanan = ".$rw['id'];
		$qresep = mysql_query($resep);
		if(mysql_num_rows($qresep)>0){
			//$dresep = mysql_fetch_array($qresep);
			$uresep = "update b_resep
						set kso_id = '{$cstatusPx}', cara_bayar = 1 
						where id_pelayanan = ".$rw['id'];
			$quresep = mysql_query($uresep);
			if(mysql_errno()>0){
				$dt="Proses Update Resep Gagal ! ".mysql_error();
			}
		}
		
		$ksoAp = "select * from $dbapotek.a_mitra where kso_id_billing = '{$cstatusPx}'";
		$qksoAp = mysql_query($ksoAp);
		$dksoAp = mysql_fetch_array($qksoAp);
		if(mysql_errno()>0){
			$dt="Proses Update Penjualan Apotek Gagal ! ".mysql_error();
		} else {
			$apotek = "select * from $dbapotek.a_penjualan where NO_KUNJUNGAN = ".$rw['id'];
			$qApotek = mysql_query($apotek);
			if(mysql_num_rows($qApotek) > 0){
				//$dApotek = mysql_fetch_array($qApotek);
				$uApot = "UPDATE $dbapotek.a_penjualan
							SET KSO_ID = ".$dksoAp['IDMITRA'].", CARA_BAYAR = 4
							WHERE NO_KUNJUNGAN = ".$rw['id'];
				$qApot = mysql_query($uApot);
				if(mysql_errno()>0){
					$dt="Proses Update Penjualan Apotek Gagal ! ".mysql_error();
				}
			}
		...
		if(mysql_num_rows($qresep)>0){
			//$dresep = mysql_fetch_array($qresep);
			$uresep = "update b_resep
						set kso_id = '{$cstatusPx}', cara_bayar = 2 
						where id_pelayanan = ".$rw['id'];
			$quresep = mysql_query($uresep);
			if(mysql_errno()>0){
				$dt="Proses Update Resep Gagal ! ".mysql_error();
		...
		if(mysql_errno()>0){
			...
			if(mysql_num_rows($qApotek) > 0){
				//$dApotek = mysql_fetch_array($qApotek);
				$uApot = "UPDATE $dbapotek.a_penjualan
							SET KSO_ID = ".$dksoAp['IDMITRA'].", CARA_BAYAR = 2
							WHERE NO_KUNJUNGAN = ".$rw['id'];
				$qApot = mysql_query($uApot);
				if(mysql_errno()>0){
					$dt="Proses Update Penjualan Apotek Gagal ! ".mysql_error();
...
//====================================================
		...
		if (mysql_errno()>0){
			...
			$query="insert into b_update_status (status_pasien, jenis_kunjungan, tgl_sjp, no_sjp, no_anggota, hak_kelas, nama_peserta, pasien_id, pelayanan_id, kunjungan_id, tgl_act, user_act) values
			('$cstatusPxLama', '$cJenisKunjLama', '$cTglSJPLama', '$cNoSJPLama', '$cNoJaminanLama', '$ccmbHakKelasLama', '$cnmPesertaLama', '$IdPasien', '$idPel', '$idKunj', NOW(), '$idUser')";
			mysql_query($query);
			if (mysql_errno()>0){
			$dt="Proses Update Gagal !";
			}
