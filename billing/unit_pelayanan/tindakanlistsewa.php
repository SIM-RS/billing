...
if($ksoId == 63 && $unitId == 232){
	...
	/**
 	 ...
 	 */
	$sql="SELECT * FROM (SELECT * from (SELECT mt.id, mt.unit_id, mt.kode, mt.nama, tk.ms_kelas_id, tk.tarip, tk.tarip_askes, tk.ms_tindakan_id, tk.id AS idtk,k.nama as kelas,kl.nama AS kelompok,kla.nama AS klasifikasi,mt.ext_tind,mt.ak_ms_unit_id, p.nama as penjamin
	FROM b_ms_tindakan mt
	INNER JOIN b_ms_tindakan_kelas tk ON tk.ms_tindakan_id = mt.id
	...
	{
		$i++;
		$arfvalue=$rows['idtk']."*|*".$rows['nama']."*|*".$rows["kode"]."*|*".$rows["tarip"]."*|*".$rows["id"]."*|*".$rows['kelas']."*|*".$rows['tarip_askes']."*|*".$rows['ext_tind']."*|*".$rows['ak_ms_unit_id'];
?>
	<tr id="lstTind<?php echo $i; ?>" lang="<?php echo $arfvalue; ?>" class="itemtableReq" onMouseOver="this.className='itemtableMOverReq'" onMouseOut="this.className='itemtableReq'" onClick="fSetTindakanSewa(this.lang);">
