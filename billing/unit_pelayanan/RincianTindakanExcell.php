...
$workbook->send('RincianTindakan_'.$rw['nmKso'].'_'.$rw['nmPas'].'.xls');
...
INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE p.kunjungan_id='$idKunj' AND mu.kategori<>5 AND p.unit_id<>$idUnitAmbulan AND p.unit_id<>$idUnitJenazah ORDER BY p.id";
}else {
	$sql="SELECT distinct p.id,mu.nama nmUnit ,mu.id idUnit,mu.parent_id,mu.inap FROM b_pelayanan p
	INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE p.id='$idPel' AND mu.kategori<>5 ORDER BY p.id";
}
//echo $sql."<br>";
$rs=mysql_query($sql);
$sByr=0;
...
while ($rows=mysql_fetch_array($rs)){
	...
	LEFT JOIN b_ms_kelas mKls ON tk.kelas_id=mKls.id WHERE kunjungan_id='$idKunj' AND p.id='".$rows['id']."'";
