...
FROM b_tindakan_kamar WHERE pelayanan_id='".$pelayanan_id."'";
	...
	WHERE pd.kso_paket_id=$idpaket AND t1.tgl='".$rwcekTindAsli["tgl"]."'";
				...
				if ($icd9cm=="true"){
					...
					$sqlUpdate="UPDATE b_tindakan SET ms_tindakan_kelas_id = '".$_REQUEST['idTind']."',ms_tindakan_unit_id='".$_REQUEST['unitId']."', ket = '".$_REQUEST['ket']."', qty=".$_REQUEST['qty'].", biaya = '".$biayaRS."', user_act = $userId, user_id=".$_REQUEST['idDok'].",type_dokter='".$isDokPengganti."', biaya_pasien = '$biaya_pasien', biaya_kso = '$biaya_kso' WHERE id = '".$_REQUEST['id']."'";
...
WHERE t.pelayanan_id = '".$pelayanan_id."'
	...
	*/
    ...
    elseif($grd2 == "true") {
	    ...
	    ,b.jenis_layanan,b.unit_id,b.dokter_id,p.nama as dokter,b.ket, b.type_dokter, tk.kamar_id, tk.kelas_id, b.type_dokter_tujuan,b.dokter_tujuan_id, b.cito
	    FROM b_pelayanan b
	    ...
	    $sql="select * from (SELECT distinct date_format(b.tgl,'%d-%m-%Y') as tgl,b.id,u.nama as unit,b.jenis_layanan,b.unit_id,b.dokter_id,p.nama as dokter,b.ket, b.type_dokter, b.type_dokter_tujuan,b.dokter_tujuan_id,b.cito
	    FROM b_pelayanan b
	    inner join b_ms_unit u on b.unit_id=u.id
	    left join b_ms_pegawai p on b.dokter_id=p.id
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
		  ...
		  if(mysql_num_rows($rs_an) > 0){
            ...
            $sisip = $rows['id']."|".$rows['ms_tindakan_kelas_id']."|".$rows['ms_tindakan_unit_id']."|".$rwJns['parent_id']."|".$rows['user_id']."|".$rows['kunjungan_id']."|".$rows['unit_act']."|".$rows['type_dokter']."|".$rows['klasifikasi_id'].'|'.$dokter_an.'|'.$rows['ms_tindakan_id_icd9cm'].'|'.$rows["nama"].'|'.$rows["kelas"].'|'.$rows["biaya"].'|'.$rows["qty"].'|'.$rows["ket"].'|'.tglJamSQL($rows["tanggal"]).'|'.$rows["sampah"];//joker;
            $i++;
            ...
            $dt.=$sisip.chr(3)."<span $css>".number_format($i,0,",","")."</span>".chr(3)."<span $css>".tglJamSQL($rows["tanggal"])."</span>".chr(3)."<span $css>".$rows["nama"]."</span>".chr(3)."<span $css>".$rows["kelas"]."</span>".chr(3)."<span $css>".$rows["biaya"]."</span>".chr(3)."<span $css>".$rows["qty"]."</span>".chr(3)."<span $css>".$rows["subtotal"]."</span>".chr(3)."<span $css>".$dokter."</span>".chr(3)."<span $css>".$rows["petugas"]."</span>".chr(3)."<span $css>".$rows["ket"]."</span>".chr(6);
    ...
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            $i++;
            $dt.=$rows["id"]."|".$rows['jenis_layanan']."|".$rows['unit_id']."|".$rows['dokter_id']."|".$rows['type_dokter'].'|'.$rows['kelas_id'].'|'.$rows['kamar_id'].'|'.$rows["dilayani"].'|'.$rows["dokter_tujuan_id"].'|'.$rows["type_dokter_tujuan"].'|'.$rows["cito"].chr(3).number_format($i,0,",","").chr(3).$rows['tgl'].chr(3).$rows["unit"].chr(3).$rows["dokter"].chr(3).$rows["ket"].chr(6);
        }
    }
    elseif($grd3 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
