...
$dokter = $_REQUEST['idDokter'];

$queryK = "SELECT * FROM b_tindakan_kamar WHERE pelayanan_id='$idPel'";
...
<table border="0" cellpadding="2" cellspacing="2" align="left" class="kwi">
    ...
    <?php
    $sql1 = "select no_rm, p.nama AS nama_pasien, p.alamat AS alamat_pasien, peg.nama AS nama_dokter, uj.nama  AS jenis_layanan, ut.nama  AS tempat_layanan from b_tindakan t inner join b_pelayanan pe on t.pelayanan_id = pe.id inner join b_kunjungan k on pe.kunjungan_id = k.id inner join b_ms_tindakan_kelas tk on t.ms_tindakan_kelas_id = tk.id inner join b_ms_tindakan mt on tk.ms_tindakan_id = mt.id inner join b_ms_kelas ke on tk.ms_kelas_id = ke.id inner join b_ms_pasien p on k.pasien_id = p.id inner join b_ms_pegawai peg on t.user_id = peg.id inner join b_ms_unit uj on pe.jenis_layanan = uj.id inner join b_ms_unit ut on pe.unit_id = ut.id where pe.id='".$idPel."' AND peg.id='".$dokter."'  GROUP BY peg.id";
	//echo $sql1."<br>";
	...
	<tr>
		<td colspan="2">
			<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;">
				<tr>
					<td width="5%" style="text-align:center; font-weight:bold; border-top:1px solid #000000; border-bottom:1px solid #000000; background-color:#00FFFF;" height="25">NO</td>
					<td width="20%" style="text-align:center; font-weight:bold; border-top:1px solid #000000; border-bottom:1px solid #000000; background-color:#00FFFF;">TANGGAL</td>
					<td width="55%" style="text-align:center; font-weight:bold; border-top:1px solid #000000; border-bottom:1px solid #000000; background-color:#00FFFF;">PELAYANAN</td>
					<td width="20%" style="text-align:center; font-weight:bold; border-top:1px solid #000000; border-bottom:1px solid #000000; background-color:#00FFFF;">BIAYA</td>
				</tr>	
				<?php
						$qTin = "SELECT DATE_FORMAT(b_tindakan.tgl,'%d-%m-%Y') AS tgl, b_ms_tindakan.nama AS tindakan, b_tindakan.biaya FROM b_tindakan INNER JOIN b_ms_tindakan_kelas ON b_ms_tindakan_kelas.id=b_tindakan.ms_tindakan_kelas_id INNER JOIN b_ms_tindakan ON b_ms_tindakan.id=b_ms_tindakan_kelas.ms_tindakan_id WHERE b_tindakan.pelayanan_id = '".$idPel."' AND b_tindakan.user_id = '".$dokter."' ORDER BY b_tindakan.tgl ";
						$sTin = mysql_query($qTin);
						$no=1;
						$total=0;
						while($wTin = mysql_fetch_array($sTin))
						{
				?>
				<tr>
					<td style="text-align:center;"><?php echo $no;?></td>
					<td style="text-align:center;"><?php echo $wTin['tgl']?></td>
					<td style="padding-left:5px;"><?php echo $wTin['tindakan']?></td>
					<td style="text-align:right; padding-right:10px;"><?php echo number_format($wTin['biaya'],0,",",".")?></td>
				</tr>
				<?php
						$no++;
						$total = $total + $wTin['biaya'];
						}
				?>
				<tr>
					<td colspan="3"style="text-align:center; font-weight:bold; border-bottom:1px solid #000000; border-top:1px solid #000000; background-color:#FFFF00;">TOTAL</td>
					<td style="text-align:right; padding-right:10px; font-weight:bold; border-bottom:1px solid #000000; border-top:1px solid #000000; background-color:#FFFF00;"><?php echo number_format($total,0,",",".");?></td>
				</tr>
			</table>
		</td>
	</tr>
