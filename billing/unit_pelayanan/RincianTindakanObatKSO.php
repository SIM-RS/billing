...
WHERE k.id='$idKunj' AND p.id='$idPel'";
		...
		$sqlIsInap="SELECT DATE_FORMAT(IFNULL(tk.tgl_out,NOW()),'%d-%m-%Y %H:%i') tglOut
					FROM b_pelayanan p
					...
					LIMIT 1";
		$rsIsInap=mysql_query($sqlIsInap);
		if (mysql_num_rows($rsIsInap)>0){
			$aslinyaInap=1;
			$rwIsInap=mysql_fetch_array($rsIsInap);
			$tglOut=$rwIsInap['tglOut'];
		}
		
		$sqlPas="SELECT no_rm,mp.nama nmPas,mp.alamat, mp.rt,mp.rw,mp.sex,mk.nama kelas, 
kso.nama nmKso,CONCAT(DATE_FORMAT(k.tgl,'%d-%m-%Y'),' ',DATE_FORMAT(k.tgl_act,'%H:%i')) tgljam, 
DATE_FORMAT(IFNULL(k.tgl_pulang,CONCAT(k.tgl,' ',DATE_FORMAT(k.tgl_act,'%H:%i'))),'%d-%m-%Y %H:%i') tglP, mp.desa_id,mp.kec_id, 
(SELECT nama FROM b_ms_wilayah WHERE id=mp.kec_id) nmKec, 
(SELECT nama FROM b_ms_wilayah WHERE id=mp.desa_id) nmDesa, k.kso_id,k.kso_kelas_id 
FROM b_kunjungan k INNER JOIN b_ms_pasien mp ON k.pasien_id=mp.id
LEFT JOIN b_ms_kelas mk ON k.kso_kelas_id=mk.id INNER JOIN b_ms_kso kso ON k.kso_id=kso.id
WHERE k.id='$idKunj'";
	}
}
...
if ($ksoid==$idKsoUmum) {
	$sqlUpdtkoreksi="UPDATE b_tindakan
					  INNER JOIN b_pelayanan
						ON b_tindakan.pelayanan_id = b_pelayanan.id
					SET b_tindakan.biaya_pasien = b_tindakan.biaya,
					  b_tindakan.biaya_kso = 0
					WHERE b_pelayanan.kunjungan_id = '$idKunj'
						AND b_tindakan.kso_id = '$idKsoUmum'
						AND b_tindakan.biaya_pasien = 0";
	$rsUpdtkoreksi=mysql_query($sqlUpdtkoreksi);
}
?>
                ...
                <td height="30" colspan="2">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
                        <tr>
                            ...
                            <td align="center" width="110" style="border:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px"> <?php if ($for<>"1"){?>Biaya<?php }?></td>
                            <td align="center" width="110" style="border:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px">Dijamin KSO</td>
                            <td align="center" width="110" style="border:#000000 solid 1px; border-left:none; font-weight:bold;font-size:12px">Iur Biaya</td>
                        </tr>
                        <?php
						$idUnitAmbulan=0;
						$idUnitJenazah=0;
						...
						if ($tipe=="2") {
                        	$sUnit="SELECT DISTINCT p.id,mu.nama nmUnit,mu.id idUnit,mu.parent_id FROM b_pelayanan p
						INNER JOIN b_ms_unit mu ON p.unit_id=mu.id LEFT JOIN b_tindakan t ON p.id=t.pelayanan_id
						WHERE p.kunjungan_id='$idKunj' AND p.unit_id<>$idUnitAmbulan AND p.unit_id<>$idUnitJenazah";
                        }elseif ($tipe=="1") {
                        	$sUnit="SELECT DISTINCT p.id,mu.nama nmUnit,mu.id idUnit,mu.parent_id 
FROM b_pelayanan p INNER JOIN b_ms_unit mu ON p.unit_id=mu.id LEFT JOIN b_tindakan t ON p.id=t.pelayanan_id 
WHERE p.kunjungan_id='$idKunj' AND p.jenis_kunjungan=3 AND p.unit_id<>$idUnitAmbulan AND p.unit_id<>$idUnitJenazah";
                        }elseif ($tipe=="0") {
                        		$sUnit="SELECT DISTINCT p.id,mu.nama nmUnit,mu.id idUnit,mu.parent_id 
FROM b_pelayanan p INNER JOIN b_ms_unit mu ON p.unit_id=mu.id LEFT JOIN b_tindakan t ON p.id=t.pelayanan_id 
WHERE p.kunjungan_id='$idKunj' AND p.jenis_kunjungan<>3 AND p.unit_id<>$idUnitAmbulan AND p.unit_id<>$idUnitJenazah";
...
IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in))*tk.beban_kso) biaya_kso,tk.bayar,tk.bayar_kso, 
...
LEFT JOIN b_ms_kelas mKls ON tk.kelas_id=mKls.id WHERE kunjungan_id='$idKunj' AND p.id='".$rwUnit['id']."'";
                        ...
                        <tr>
                            ...
                            <td align="right" style="font-size:12px"><?php if ($for<>"1") echo number_format($rwKmr['biaya'],0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-size:12px"><?php echo number_format($rwKmr['biaya_kso'],0,",",".")?>&nbsp;</td>
                            <td align="right" style="font-size:12px"><?php echo number_format($sKmrIur,0,",",".")?>&nbsp;</td>
                        </tr>
                                <?php 
							}
...
WHERE t.kunjungan_id=".$idKunj." AND t.pelayanan_id=".$rwUnit['id']." AND user_id='".$rwKonsul['user_id']."' GROUP BY mt.nama ORDER BY t.id";
                        ...
                        <tr>
                            ...
                            <td align="right" style="font-size:12px"><?php if ($for<>"1") echo number_format($rwTind['biaya'],0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-size:12px"><?php echo number_format($rwTind['biaya_kso'],0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-size:12px"><?php echo number_format($rwTind['iurbiaya'],0,",",".");?>&nbsp;</td>
                        </tr>
                                    <?php
                                    $sTot+=$rwTind['biaya'];
                        ...
                        <tr>
                            ...
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px;font-size:12px"><?php if ($for<>"1") echo number_format($bTot,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px;font-size:12px"><?php echo number_format($bTotKSO,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px;font-size:12px"><?php echo number_format($bTotIur,0,",",".");?>&nbsp;</td>
                        </tr>
                        <tr>
                          <td align="left" style="padding-left:60px;font-size:12px">&nbsp;</td>
                          <td align="center">&nbsp;</td>
                          <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px">Bayar&nbsp;</td>
                          <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px"><?php if ($for<>"1") echo number_format($sSdhBayar,0,",",".");?>&nbsp;</td>
                          <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px"><?php echo number_format($sSdhBayarKSO,0,",",".");?>&nbsp;</td>
						...
						if ($ksoid==1){
							...
							$selisih=$sBiaya-$sByr-$keringanan-$titipan-$sJaminKso;
						}else{
                        ...
                        <tr>
                            ...
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px"><?php if ($for<>"1") echo number_format($sBiaya,0,",","."); else echo number_format($sJaminKso+$sIurBiaya,0,",",".");?>&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px"><span style="font-weight:bold;font-size:12px"><?php echo number_format($sJaminKso,0,",","."); ?></span>&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px"><span style="font-weight:bold;font-size:12px"><?php echo number_format($sIurBiaya,0,",","."); ?></span>&nbsp;</td>
                        </tr>
                        <?php
                        if ($sJaminKso>0) {
                            ?>
                        ...
                        <tr>
                            <td align="left" height="20" style="padding-left:60px;font-size:12px">&nbsp;</td>
                            <td align="center">&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px;font-size:12px">Kurang (Tind)&nbsp;</td>
...
WHERE p.kunjungan_id='$idKunj' AND p.jenis_kunjungan<>3) AS t2 INNER JOIN $dbapotek.a_penjualan ap ON t2.id=ap.NO_KUNJUNGAN WHERE ap.NO_PASIEN='".$rw['no_rm']."'";
                        ...
                        <tr>
							...
							//echo $klsId."<br>";
							$sKonsul="SELECT DISTINCT ap.NO_PENJUALAN,ap.DOKTER,ap.CARA_BAYAR,ac.nama,ap.UNIT_ID FROM $dbapotek.a_penjualan ap inner join $dbapotek.a_cara_bayar ac on ap.CARA_BAYAR=ac.id WHERE ap.NO_PASIEN='".$rw['no_rm']."' AND ap.NO_KUNJUNGAN='".$rwUnit['id']."'";
                            //echo $sKonsul."<br>";
                            $qKonsul=mysql_query($sKonsul);
                            while($rwKonsul=mysql_fetch_array($qKonsul)) {
                            	$sTot=0;
                        ...
                        <tr>
                            ...
                            <td></td>
								...
								ao.OBAT_KELOMPOK,
								SUM(t2.BAYAR_PASIEN) BAYAR_PASIEN 
FROM (SELECT * FROM $dbapotek.a_penjualan WHERE NO_KUNJUNGAN='".$rwUnit['id']."' AND NO_PENJUALAN='".$rwKonsul['NO_PENJUALAN']."' AND DOKTER='".$rwKonsul['DOKTER']."' AND UNIT_ID = '".$rwKonsul['UNIT_ID']."') AS t2 
...
GROUP BY ao.OBAT_ID";
                        ...
                        <tr>
                            <td align="left" style="padding-left:85px"><?php echo strtolower($rwObat['OBAT_NAMA']).' @ Rp '.number_format($rwObat['HARGA_SATUAN'],2,",",".");?></td>
                        ...
                        <tr>
                            ...
                            <td align="center">&nbsp;</td>
                            <td align="center">&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px">Jamin KSO&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px"><?php echo number_format($sJaminKSO,0,",",".");?>&nbsp;</td>
                        </tr>
                        <?php 
						}else{
						?>
                        <tr>
                            ...
                            <td align="center">&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px">Bayar&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-bottom:solid #000000 1px;font-size:12px"><?php echo number_format($sbyrTot,0,",",".");?>&nbsp;</td>
                        </tr>
                        <?php 
						}
						?>
                        ...
                        <tr>
                            <td align="left">&nbsp;</td>
                            <td align="center">&nbsp;</td>
                            <td colspan="2" align="right" style="font-weight:bold; font-size:12px">Total Jamin KSO&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px"><?php echo number_format($jaminKSOTot,0,",",".");?>&nbsp;</td>
                        </tr>
                        <?php 
						}
						?>
                        <tr>
                            ...
                            <td align="center">&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px">Total Bayar&nbsp;</td>
                            <td align="right" style="font-weight:bold; font-size:12px"><?php echo number_format($byrTot,0,",",".");?>&nbsp;</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td align="center">&nbsp;</td>
                            <td align="center">&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px">Kurang (Obat)&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px"><?php echo number_format($krgObat,0,",",".");?>&nbsp;</td>
                        </tr>
                        <tr>
                            <td align="left" height="20">&nbsp;</td>
                            <td align="center">&nbsp;</td>
                            <td colspan="2" align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px">Kurang (Tind + Obat)&nbsp;</td>
                            <td align="right" style="font-weight:bold; border-top:solid #000000 1px; font-size:12px"><?php echo number_format($krgTindObat,0,",",".");?>&nbsp;</td>
                        </tr>
                        <tr>
                            <td align="left" style="padding-left:60px;font-size:12px;" height="2"></td>
                            <td align="center"></td>
