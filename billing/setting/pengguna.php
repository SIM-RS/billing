...
<table width="1000" border="0" cellpadding="0" cellspacing="0" class="hd2">
	<tr>
		<td height="30">&nbsp;SETTING PETUGAS / OPERATOR</td>
	</tr>
</table>
...
<tr>
	<td>
		...
		<tr>
			<td>Nip</td>
			<td><input type="text" id="nipR" name="nipR" size="20" class="txtinput"  />&nbsp;User ID INACBG&nbsp;<input type="text" id="user_id_inacbg" name="user_id_inacbg" size="5" class="txtinput" readonly="readonly" />&nbsp;<input type="button" class="btninput" onClick="openUserINACBG();"></td>
		</tr>
		<tr>
			<td>Nama</td>
			<td><input type="text" name="namaR" id="namaR" size="50"  class="txtinput" /></td>
		</tr>
		<tr>
			<td>Tempat Lahir</td>
			<td><input type="text" id="tmpLahirR" name="tmpLahirR" size="50" class="txtinput"  /></td>
		</tr>
		<tr>
			<td>Tanggal Lahir</td>
			<td><input type="text" id="tglLahirR" name="tglLahirR" value="<?php echo $date_now; ?>" readonly="true" size="10" class="txtinput"  />&nbsp;<img src="../icon/archive1.gif" width="20" align="absmiddle" style="cursor:pointer" onclick="gfPop.fPopCalendar(document.getElementById('tglLahirR'),depRange);" title="calender" /></td>
		</tr>
		<tr>
			<td>Alamat</td>
			<td><input type="text" id="almtR" name="almtR" size="50" class="txtinput"  /></td>
		</tr>
		<tr>
			<td>No Handpone</td>
			<td><input type="text" id="noTlpR" name="noTlpR" size="15" maxlength="15" class="txtinput"  />&nbsp;&nbsp;Telp&nbsp;<input id="tlpR" name="tlpR" size="15" maxlength="15" class="txtinput" /></td>
		</tr>
		<tr>
			<td>Jenis Kelamin</td>
			<td>
			<select id="sexR" name="sexR" class="txtinput" >
		...
		<tr>
			<td>Agama</td>
			<td><select id="agamaR" name="agamaR" class="txtinput" >
				<?php
	...
	<td valign="top">
	<table width="100%" align="center" cellpadding="3" cellspacing="0">
		<tr id="trStatAp" style="visibility:collapse">
			<td>Status Dalam Aplikasi</td>
			<td>
			<select id="statusR" name="statusR" class="txtinput" onchange="StatApChange(this.value);">
				<option value="1" selected="selected">Pengguna Aplikasi</option>
				<option value="2">Pelaksana dan Pengguna Aplikasi</option>
			</select>
			</td>
		</tr>
		<tr id="trSpe" style="visibility:collapse">
			<td>Spesialisasi</td>
			<td>
			<select id="SpeR" name="SpeR" class="txtinput" >
			<?php
		...
		<tr>
			...
			<td><select id="cmbjnsR" name="cmbjnsR"  class="txtinput" >
				<?php
				$sqljns="select * from b_ms_pegawai_jenis /* where KodeInduk<>'' */  order by id";
				$rsjns=mysql_query($sqljns);
				while($rwjns=mysql_fetch_array($rsjns)){
					?>
					<option value="<?php echo $rwjns['id'];?>"><?php echo $rwjns['Nama'];?></option>
					<?php
				}
				?>
				</select>
		...
		<tr>
			...
			<td><select id="cmbstsR" name="cmbstsR" class="txtinput">
				<?php
		...
		<!--tr>
			<td>Group Karyawan</td>
			<td>
				<select id="cmbgroup2R" name="cmbgroup2R" class="txtinput" >
				<?php
				$sqlgrp="select * from b_ms_group where aktif=1 order by nama";
				$rsgrp=mysql_query($sqlgrp);
				while($rwgrp=mysql_fetch_array($rsgrp)){
					?>
					<option value="<?php echo $rwgrp['id'];?>"><?php echo $rwgrp['nama'];?></option>
					<?php
				}
				mysql_free_result($rsgrp);
				?>
				</select>
			</td>
		</tr-->
		<tr>
			<td>User Name</td>
			<td><input type="text" id="userNameR" name="userNameR" size="35" class="txtinput" /></td>
		</tr>
		<tr>
			<td>Password</td>
			<td><input type="password" id="passR" name="passR" class="txtinput" size="35" /></td>
		</tr>
		<tr>
			<td>Flag</td>
			<td><select id="cmbFlag" name="cmbFlag"  class="txtinput"  >
				<option value="0" name="konsolidasi">Konsolidasi</option>
				<option value="1" name="rspelindo">RS Pelindo</option>
				<option value="2" name="klinik_krakatau">Klinik Krakatau</option>
				<option value="3" name="apotek">Apotek Prima Husada Cipta</option>
				</select></td>
		</tr>
		<tr>
...
<script>
	...
	mTab.setTabCaption("SETTING APLIKASI,GROUP KARYAWAN,HAK AKSES,PETUGAS,TEMPAT LAYANAN,REPORT RM");
	mTab.setTabCaptionWidth("160,160,160,160,0,0");
	mTab.setTabDisplay("true,true,true,true,false,false,0");
	mTab.onLoaded(showgrid);
	mTab.setTabPage("settingaplikasi.php,grupkaryawan.php,groupakses.php,petugas.php,tmptlayanan.php,menu_report_rm.php");
	...
	function setGroup(addrs,val){
		Request ( 'http://'+addrs+'<?php echo $base_addr; ?>/billing/setting/grouptree.php?gid='+val , 'divtree3', '', 'GET');
	}
	...
	var tangkap,proses,tombolSimpan,tombolHapus;
		...
		//if (val!=undefined && val!='' && val!='*|*'){
			...
			gd2.loadURL("group_petugasUtils.php?grd=2&idGroup="+document.getElementById('group').value,"","GET");
			alert(val);
		}
	}
	...
	function showgrid()
	{
		...
		tab2.setIDColHeader(",kode,nama,ket,aktif");
		...
		gd1.setColHeader("&nbsp;,No,User Name,Nama, Jenis");
		gd1.setIDColHeader(",,username,mp.nama,pj.nama");
		gd1.setColWidth("30,40,90,200,150");
		gd1.setCellAlign("center,center,left,left,left");
		gd1.setCellType("txt,txt,txt,txt,txt");
		gd1.setCellHeight(20);
		gd1.setImgPath("icon");
		gd1.setIDPaging("paging1");
		gd1.attachEvent("onRowClick","ambil1");
		gd1.onLoaded(konfirmasi);
...
<!------------------------------------------------------------------------------------------------------------------------------>		
		...
		tab5.setColWidth("50,100,100,500");
	...
	function ambil2(){
		...
		if(gd2.cellsGetValue(gd2.getSelRow(),3)==''){
			...
			mTab.setTabDisplay("true,true,true,true,true,false,3");
			mTab.setTabCaptionWidth("160,160,160,160,160,0");
		}
...
function del(){
	...
	gd1.loadURL("group_petugasUtils.php?grd=1&act=hapus&idGroup="+document.getElementById('group').value+"&txtId="+idPeg,"","GET");/* 
...
function act1(parameter){
	var niP = document.getElementById('nipR').value;
	var user_id_inacbg = document.getElementById("user_id_inacbg").value;
	var namaR = document.getElementById('namaR').value;
	var tmpLahirR = document.getElementById("tmpLahirR").value;
	var tglLahirR = document.getElementById("tglLahirR").value;
