	...
	WHERE p.kunjungan_id='".$rw['kunj_id']."' AND b.aktif=1";
					...
					//UMUM dan KSO LAINNYA
						...
						if ($kasir==127){
							...
							//Merubah sql lama jadi pemgambilan biaya tindakan nya per user_id
							//ini dilakukan untuk mendapatkan diskon dan id dokter yang memberikan diskon
							$sqlTin="SELECT 
							IFNULL(sum((t.biaya)*t.qty),0) as total,
							IFNULL(sum((t.biaya_kso)*t.qty),0) as dijamin,
							t.user_id
		...
		//echo $sqlTin.";<br/>";
						...
						$totalTindakan = 0;
						$totalDijamin = 0;
						$diskonTindakanDokter = 0;
						$dokterPemberiDiskon = "";
						//dilakukan penjumlahan untuk mendapatkan total tindakan dan total dijamin serta mendapatkan diskon dokter
						while($rwTin=mysql_fetch_array($rsTin)){
							$totalTindakan += $rwTin['total'];
							$totalDijamin += $rwTin['dijamin'];
							$diskonTindakanDokter += $rwTin['diskon'];
							$dokterPemberiDiskon .= $rwTin['user_id'] . ':' . $rwTin['diskon'] .'**';
						}

						$dokterPemberiDiskon = rtrim($dokterPemberiDiskon,"**");
...
-- Biaya retribusi hanya dikenakan dipertama masuk rawat inap* decyber
	...
	(IF(DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)=0,0,DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)+1))*(beban_kso))),0) AS kamar_kso
						...
						$total=($totalTindakan+$rwKamar['kamar'])-($totalDijamin+$rwKamar['kamar_kso'])+$rwKamar['retribusi'];
						if($total<0){
							$total=0;
						}
			...
			//akhir cek konsul+MRS
				...
				$rw['sex']."|".				//5
				$rw['no_billing']."|".		//6
				($total - $diskonTindakanDokter)."|".					//7
		   		($kurang - $diskonTindakanDokter)."|".				//8
				$rw['kunj_id']."|".			//9
				$tgl."|".					//10
				$pelayanan_id."|".			//11
				$rwTitip['titip']."|".		//12
				...
				$jmldata."|".				//40
				$sisabilling . "|".			//41
				$diskonTindakanDokter . "|". //42
				$dokterPemberiDiskon;		//43
			   ...
			   <td>&nbsp;<?php echo number_format($sisa_tagihan - $diskonTindakanDokter,0,',','.');?></td>
               </tr>
               <?php
		   			$i++;
				//}
