...
$userId = $_GET['idUser'];
	...
	if ($bayarId==0 || $bayarId==""){
		$sql="SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS tagihan,IFNULL(SUM(t.bayar_pasien),0) AS bayar 
...
IF(status_out=0,(IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in))), 
(IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)))) AS qtyHari, 
IF(status_out=0,(IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)))*(beban_kso+beban_pasien), 
(IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)))*(beban_kso+beban_pasien)) AS biayaKamar, 
IF(status_out=0,(IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,1,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)))*(beban_kso), 
(IF(DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)=0,0,DATEDIFF(IFNULL(tk.tgl_out,NOW()),tk.tgl_in)))*(beban_kso)) AS biayaKSO 
...
FROM b_tindakan t INNER JOIN b_bayar_tindakan bt ON t.id=bt.tindakan_id
INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND mu.parent_id=60";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaRad,IFNULL(SUM(t.biaya_kso),0) AS biayaRadKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id
WHERE p.kunjungan_id=$kunjId AND mu.parent_id=60";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
FROM b_tindakan t INNER JOIN b_bayar_tindakan bt ON t.id=bt.tindakan_id
INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND p.unit_id=65";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaHD,IFNULL(SUM(t.biaya_kso),0) AS biayaHDKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
WHERE p.kunjungan_id=$kunjId AND p.unit_id=65";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
FROM b_tindakan t INNER JOIN b_bayar_tindakan bt ON t.id=bt.tindakan_id
INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND mu.parent_id=66";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaEND,IFNULL(SUM(t.biaya_kso),0) AS biayaENDKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id
WHERE p.kunjungan_id=$kunjId AND mu.parent_id=66";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
//to get operation fee
    /*$query = "SELECT SUM(t5.biayaOP) AS biayaOP,SUM(t5.biayaOPKSO) biayaOPKSO FROM (SELECT IFNULL((t.biaya_kso+t.biaya_pasien),0) AS biayaOP,IFNULL((t.biaya_kso),0) AS biayaOPKSO 
FROM b_tindakan t INNER JOIN b_bayar_tindakan bt ON t.id=bt.tindakan_id INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id
INNER JOIN b_ms_tindakan_operasi mto ON tk.ms_tindakan_id = mto.ms_tindakan_id
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 GROUP BY bt.id) AS t5";*/
	if ($bayarId>0){
...
FROM b_tindakan t INNER JOIN b_bayar_tindakan bt ON t.id=bt.tindakan_id INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id 
WHERE bt.bayar_id=$bayarId AND bt.tipe=0) AS t1 INNER JOIN (SELECT DISTINCT ms_tindakan_id FROM b_ms_tindakan_operasi) mto ON t1.ms_tindakan_id = mto.ms_tindakan_id";
	}else{
		$query = "SELECT SUM(t1.biayaOP) biayaOP,SUM(t1.biayaOPKSO) biayaOPKSO FROM (
SELECT tk.ms_tindakan_id,IFNULL((t.biaya_kso+t.biaya_pasien),0) AS biayaOP,IFNULL((t.biaya_kso),0) AS biayaOPKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id 
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id 
WHERE p.kunjungan_id=$kunjId) AS t1 INNER JOIN (SELECT DISTINCT ms_tindakan_id FROM b_ms_tindakan_operasi) mto ON t1.ms_tindakan_id = mto.ms_tindakan_id";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND mt.klasifikasi_id=13";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaVisit,IFNULL(SUM(t.biaya_kso),0) AS biayaVisitKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id
INNER JOIN b_ms_tindakan mt ON tk.ms_tindakan_id = mt.id
WHERE p.kunjungan_id=$kunjId AND mt.klasifikasi_id=13";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id
INNER JOIN b_ms_tindakan mt ON tk.ms_tindakan_id = mt.id
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND mt.klasifikasi_id=14";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaKonsul,IFNULL(SUM(t.biaya_kso),0) AS biayaKonsulKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id
INNER JOIN b_ms_tindakan mt ON tk.ms_tindakan_id = mt.id
WHERE p.kunjungan_id=$kunjId AND mt.klasifikasi_id=14";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
FROM b_tindakan t INNER JOIN b_bayar_tindakan bt ON t.id=bt.tindakan_id
INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND p.unit_id=16";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaMed,IFNULL(SUM(t.biaya_kso),0) AS biayaMedKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id
WHERE p.kunjungan_id=$kunjId AND p.unit_id=16";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id
INNER JOIN b_ms_tindakan mt ON tk.ms_tindakan_id = mt.id
WHERE bt.bayar_id=$bayarId AND bt.tipe=0 AND mt.nama LIKE '%salin%' AND mt.klasifikasi_id=1";
	}else{
    	$query = "SELECT IFNULL(SUM(t.biaya_kso+t.biaya_pasien),0) AS biayaSalin,IFNULL(SUM(t.biaya_kso),0) AS biayaSalinKSO 
FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id
INNER JOIN b_ms_tindakan_kelas tk ON t.ms_tindakan_kelas_id = tk.id
INNER JOIN b_ms_tindakan mt ON tk.ms_tindakan_id = mt.id
WHERE p.kunjungan_id=$kunjId AND mt.nama LIKE '%salin%' AND mt.klasifikasi_id=1";
	}
	//echo $query."<br>";
    $rs = mysql_query($query);
    $rowDetil = mysql_fetch_array($rs);
...
<?php
    ...
    <body>
        <table width="<?php echo $lebar;?>" class="kwi" border="0" cellpadding="1" cellspacing="0" style="font-size:12px;">
			...
			?>
            <tr>
              <td colspan="5" style="border-bottom: 1px solid black; font-size: 14px" align="right">Sudah Bayar Kamar</td>
              <td style="border-bottom: 1px solid black; font-size: 14px">:&nbsp;Rp.</td>
              <td  align="right"><?php echo number_format($byrKamar,0,",",".");?></td>
            </tr>
            <?php }*/?>
            <tr>
