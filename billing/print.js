...
function printPage() {
    if (typeof(jsPrintSetup) == 'undefined') {
        ...
        if (navigator.appVersion.indexOf("Win") != -1) {
            printWindow.document.write("<head><link rel=\"stylesheet\" type=\"text/css\" href=\"/ecivil/css/windowsFont.css\"/>");
        }
...
//    jsPrintSetup.definePaperSize(9, 9, 'iso_a4', 'iso_a4_210x297mm', 'A4', 210, 297, jsPrintSetup.kPaperSizeMillimeters);
    ...
    for(var i=0;i<namaPrinter.length;i++){
       // if(namaPrinter[i].search('RP80 Printer')!=-1){
		   //alert(namaPrinter[i]);
		   if(namaPrinter[i].indexOf('RP80')!=-1){
            //alert(namaPrinter[i]);
			//alert(namaPrinter[i]+"/n"+i);
