...
function GetAll($idKunj, $idPasien)
{
  ...
  $query = "SELECT * FROM rm_penolakan_tindakan_kedokteran WHERE id_kunjungan = '$idKunj' AND id_pasien ='$idPasien'";
...
function GetOne($id)
{
  ...
  $query = "SELECT * FROM  `rm_penolakan_tindakan_kedokteran` WHERE  `id` =  '$id'";
  $exe = mysql_query($query);
  while ($data = mysql_fetch_array($exe)) {
    $datas[] = array(
      'id' => $data['id'],
...
function Delete($id)
{
  ...
  $query = "DELETE FROM `rm_penolakan_tindakan_kedokteran` WHERE `id` = '$id'";
  $exe = mysql_query($query);
  if ($exe) {
    // kalau berhasil
