<?php 
    ...
    $no = 1;
    $query = mysql_query("SELECT * FROM tb_kala WHERE id_pasien = '{$dataPasien['id']}' AND tipe = '2'");
?>
...
<body style="margin:15px"><br>
    ...
    <br><br>
    <center><h4>JAM 2 : TIAP 30 Menit</h4></center>
    <table class="text-center" border="1px" width="100%">
        ...
        <tr>
            ...
            <td>
                <a href="edit.php?domain=index2.php&id=<?= $row['id'] ?>&id_kunjungan=<?= $_REQUEST['idKunj'] ?>&idPel=<?= $_REQUEST['idPel'] ?>&idPasien=&idUser=<?= $_REQUEST['idUser'] ?>&tmpLay=<?= $_REQUEST['tmpLay'] ?>">
                    <button type="button"><img src="../../../icon/edit.gif" width="16" height="16"> Edit</button>
                    </a>&emsp;
                    <a href="credel.php?domain=index2.php&delete=<?= $row['id'] ?>&id_kunjungan=<?= $_REQUEST['idKunj'] ?>&idPel=<?= $_REQUEST['idPel'] ?>&idPasien=&idUser=<?= $_REQUEST['idUser'] ?>&tmpLay=<?= $_REQUEST['tmpLay'] ?>">
  ...
  <div class="modal-dialog modal-lg">
    ...
    <form action="credel.php" method="POST">
      <div class="row">
        ...
        <input type="hidden" name="domain" value="index2.php">
