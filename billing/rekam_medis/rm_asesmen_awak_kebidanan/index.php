<?php
    ...
    if (isset($_REQUEST["id"])) {
        mysql_query("DELETE FROM rm_asesmen_awak_kebidanan WHERE id = '{$_REQUEST['id']}'");
...
<body>
    <div align="center">
        ...
        <table width="1000" border="0" cellpadding="0" cellspacing="0" class="hd2">
            <tr>
                <td height="30">&nbsp;RM 5.1 Asesmen Awak Kebidanan</td>
            </tr>
        </table>
        <table width="1000" border="0" cellspacing="0" cellpadding="0" align="center" class="tabel">
            ...
            <tr>
                <td align="left">
                    ...
                    <a style="color: white;" target="_blank" href="form.php?idKunj=<?= $_REQUEST['idKunj'] ?>&idPel=<?= $_REQUEST['idPel'] ?>&idPasien=&idUser=<?= $_REQUEST['idUser'] ?>&tmpLay=<?= $_REQUEST['tmpLay'] ?>"><button type="button"><img src="../../icon/add.gif" width="16" height="16">Tambah</button></a>&emsp;
                    <a style="color: white;" target="_blank" href="tb_kala/index.php?idKunj=<?= $_REQUEST['idKunj'] ?>&idPel=<?= $_REQUEST['idPel'] ?>&idPasien=&idUser=<?= $_REQUEST['idUser'] ?>&tmpLay=<?= $_REQUEST['tmpLay'] ?>"><button type="button"><img src="../../icon/edit.gif" width="16" height="16">Edit Tabel KALA</button></a> &emsp;
                    <a style="color: white;" target="_blank" href="tb_riwayat_persalinan/index.php?idKunj=<?= $_REQUEST['idKunj'] ?>&idPel=<?= $_REQUEST['idPel'] ?>&idPasien=&idUser=<?= $_REQUEST['idUser'] ?>&tmpLay=<?= $_REQUEST['tmpLay'] ?>"><button type="button"><img src="../../icon/edit.gif" width="16" height="16">Edit Table Riwayat Persalinan</button></a>
                    <br><br>
    ...
    <script>
        ...
        function spawn(){
            ...
            gridTarifDokter.setHeader("RM Asesmen Awak Kebidanan");
            gridTarifDokter.setColHeader("No,Tanggal Kunjungan,Tanggal Pelayanan,Tanggal Anamnesa,Tempat Layanan,Action");
            gridTarifDokter.setIDColHeader(",k.tgl,p.tgl,pd.tgl,");
            gridTarifDokter.setColWidth("20,100,100,100,100,100");
            gridTarifDokter.setCellAlign("right,center,center,center,left");
            gridTarifDokter.setCellHeight(30);
            gridTarifDokter.setImgPath("../../icon");
            gridTarifDokter.setIDPaging("pagingDataRmPdalam");
            gridTarifDokter.baseURL("../function/grid_4.php?namaTable=rm_asesmen_awak_kebidanan&idPasien="+<?= $dataPasien['id'] ?>+"&idKunj="+<?= $_REQUEST['idKunj'] ?>+"&idPel="+<?= $_REQUEST['idPel'] ?>+"&idUser="+<?= $_REQUEST['idUser'] ?>+"&tmpLay="+<?= $_REQUEST['tmpLay'] ?>);
