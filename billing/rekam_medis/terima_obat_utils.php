...
$queryUnit = "SELECT unit_id FROM dbapotek_tangerang.a_unit WHERE unit_billing = $idunit";
...
END AS status_kirim 
 ...
 FROM 
 (SELECT * FROM dbapotek_tangerang.a_penerimaan 
 WHERE UNIT_ID_TERIMA=$idunit AND TIPE_TRANS=1 AND MONTH(TANGGAL)=$bulan AND YEAR(TANGGAL)=$ta) AS ap 
 INNER JOIN dbapotek_tangerang.a_minta_obat am ON ap.FK_MINTA_ID=am.permintaan_id 
 GROUP BY ap.NOKIRIM,am.no_bukti 
 ORDER BY ap.TANGGAL DESC) AS t1 
 INNER JOIN dbapotek_tangerang.a_unit au ON t1.UNIT_ID_KIRIM=au.UNIT_ID) AS t2 ".$filter." order by ".$sorting;
