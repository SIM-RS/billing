...
{	
	...
	if($rows["status"]==0){
		$status = "Menunggu";
		$isview = "false";
		$proses = "<img src='../icon/lihat.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Melihat Detail Peminjaman' onClick=terima('".$rows['NOKIRIM']."','".$rows['UNIT_ID']."','".$rows['tgl1']."','".$rows['NOTERIMA']."','".$isview."','".$rows['tgl2']."','".$rows['no_bukti']."','".$unitId."') />";
	} elseif($rows["status"]==1) {
		$status = "Diterima";
		$isview = "true";
		$proses = "<img src='../icon/lihat.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Melihat Detail Peminjaman' onClick=terima2('".$rows['no_bukti']."','".$isview."','".$unitId."','".$rows['tgl2']."') />";
	}
	//echo "<br/>";unitId
	
	$dt.=$id.chr(3).number_format($i,0,",","").chr(3).$rows["tgl1"].chr(3).$rows["NOKIRIM"].chr(3).$rows["NOTERIMA"].chr(3).$rows["UNIT_NAME"].chr(3).$status.chr(3).$proses.chr(6);
}
