...
INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE p.kunjungan_id='$idKunj' AND mu.kategori<>5 AND p.unit_id<>$idUnitAmbulan AND p.unit_id<>$idUnitJenazah";
}else {
	$sql="SELECT distinct p.id,mu.nama nmUnit ,mu.id idUnit,mu.parent_id,mu.inap FROM b_pelayanan p
	INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE p.id='$idPel' AND mu.kategori<>5";
}
//echo $sql."<br>";
$rs=mysql_query($sql);
$sByr=0;
