...
if($statusProses=='Error') {
    ...
    if($grd == "true") {
		...
		$sql="SELECT
			...
			WHERE pl.kunjungan_id='".$_REQUEST['kunjungan_id']."'";	
	}
	else if($grd2 == "true"){
...
WHERE LEVEL=2 AND parent_id='".$_REQUEST['parent_id']."' ORDER BY nomor_dokumen,nama";	
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $dt.=$sisip.chr(3)."<font color='$warna'>".number_format($i,0,",","")."</font>".chr(3)."<font color='$warna'>".$rows["tgl"]."</font>".chr(3)."<font color='$warna'>".$rows["no_rm"]."</font>".chr(3)."<font color='$warna'>".$rows["nama"]."</font>".chr(3)."<font color='$warna'>".$rows["penjamin"]."</font>".chr(3)."<font color='$warna'>".$rows['alamat']."</font>".chr(6);
        }
    }
	else if($grdH == "true") {
		while ($rows=mysql_fetch_array($rs)) {
			...
			$sisip=$rows["id"]."|".$rows['status_medik']."|".$rows["no_rm"]."|".$rows['pasien_id'];
		...
		while ($rows=mysql_fetch_array($rs)) {
			...
			$sisip=$rows["id"]."|".$rows["unit_id"]."|".$rows['jenis_layanan'];
			...
			*
        	...
        	$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["tgl"].chr(3).$rows["unit"].chr(3).$rows["dokter"].chr(3).$rows["asal"].chr(3).$stsKoding.chr(6);
		}	
	}
	else if($grd2 == "true") {
		while ($rows=mysql_fetch_array($rs)) {
		...
		while ($rows=mysql_fetch_array($rs)) {
            $i++;
			$sisip=$rows["id"]."|".$rows["pil"]."|".$rows["analisa"]."|".$rows["ceklist_id"]."|".$rows["legend"]."|".$rows["link"]."?report=1";
			$report_rm="<img src='../icon/zoom_in.png' width='24' height='24' title='Klik untuk melihat detil report rm' style='cursor:pointer;' onclick='viewReportRM($i);' />";
        	$dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["pil"].chr(3).$rows["analisa"].chr(3).$report_rm.chr(3).$rows["nomor_dokumen"].chr(3).$rows["nama"].chr(6);
		}
	}
