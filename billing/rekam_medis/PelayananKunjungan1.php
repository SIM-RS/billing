...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<div id="divResep" style="display:none; width: 900px;" class="popup">
		...
		<tr id="trNoResep" style="visibility:collapse">
			...
			<td>&nbsp;<input id="noResep" name="noResep" size="12" class="txtinput" disabled="disabled" /></td>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
...
<!-- ============= Div Permintaan Bank Darah ============= -->                        
    ...
    <script type="text/JavaScript" language="JavaScript">
        ...
        var jenisUnitId=0;
        var getIdPasien,getIdPel,getIdKunj,getIdUnit,getIdKelas,getUmur,getKsoId,getKsoKelasId,getIdKelasKunj,getDilayani,getKelas_id,getKamarNama,getKamarId,getTgl_sjp,getTgl_Inap,getNoSJP,getNoJaminan;
	...
	function cekInap(label){
        ...
        function cekSentPar(){
            if('<?php echo $_POST['sentPar']?>' != '' && '<?php echo isset($_POST['sentPar'])?>' == 1){
                document.getElementById('aharef').href = "http://<?php echo $_SERVER['HTTP_HOST']?>/simrs-tangerang/billing/informasi/riwayat_kunj.php";
	...
	function gantiDokter(comboDokter,statusCek){
		if(statusCek==true){
			isiCombo('cmbDokPengganti',document.getElementById('cmbTmpLay').value,'','cmbDokRujukUnit');
			isiCombo('cmbDokPengganti',document.getElementById('cmbTmpLay').value,'','cmbDokRujukRS');
			isiCombo('cmbDokPengganti',document.getElementById('cmbTmpLay').value,'','cmbDokDiag');
			isiCombo('cmbDokPengganti',document.getElementById('cmbTmpLay').value,'','cmbDokResep');
			isiCombo('cmbDokPengganti',document.getElementById('cmbTmpLay').value,'','cmbDokTind');
			//isiCombo('cmbDokPengganti',document.getElementById('cmbTmpLay').value,'',comboDokter);
		}
		else{
			isiCombo('cmbDok',document.getElementById('cmbTmpLay').value,'','cmbDokRujukUnit');
			isiCombo('cmbDok',document.getElementById('cmbTmpLay').value,'','cmbDokRujukRS');
	...
	function showgrid()
	{
		...
		aDet.setColHeader("NO,NAMA OBAT,JUMLAH,RACIKAN,DOSIS,DOKTER");
		aDet.setIDColHeader(",OBAT_NAMA,,,,");
		aDet.setColWidth("30,200,40,110,180,150");
		aDet.setCellAlign("center,left,center,center,left,left");
		aDet.setCellType("txt,txt,txt,txt,txt,txt");
	...
	function tutupLab(){
		setTimeout("document.getElementById('divDlm').innerHTML = ''","1000");
		setTimeout("document.getElementById('txtLab').value = ''","1000");
	...
	function ambilDataRujukUnit(){
		...
		var sisipan=c.getRowId(c.getSelRow()).split("|");
		var p ="idRujukUnit*-*"+sisipan[0]+"*|*JnsLayanan*-*"+sisipan[1]+"*|*TmpLayanan*-*"+sisipan[2]+"*|*btnHapusRujukUnit*-*false";
	...
	function hapus(id)
	{
		...
		switch(id)
		{
			...
			case 'btnHapusResep':
				...
				{
					document.getElementById(id).disabled = true;
					//alert("tindiag_utils.php?grdResep=true&pelayanan_id="+getIdPel+"&act=hapus&hps="+id+"&rowid="+rowidResep+"&noResep="+document.getElementById('noResep').value);
					r.loadURL("tindiag_utils.php?grdResep=true&pelayanan_id="+getIdPel+"&act=hapus&hps="+id+"&rowid="+rowidResep+"&noResep="+document.getElementById('noResep').value,"","GET");
				}
				break;
	...
	function konfirmasi(key,val){
		...
		if(key=='Error'){
			...
			if(proses=='tambah'){
				...
				else if(tombolSimpan=='btnSimpanResep'){
					document.getElementById('txtNmObat').focus();
					resep_baru=0;
