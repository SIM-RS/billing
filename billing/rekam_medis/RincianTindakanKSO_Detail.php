                ...
                <td height="30" colspan="2">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
						...
						WHERE p.kunjungan_id='$idKunj'";
                        	...
                        	/*$sUnit="SELECT DISTINCT p.id,mu.nama nmUnit,mu.id idUnit 
FROM b_pelayanan p INNER JOIN b_ms_unit mu ON p.unit_id=mu.id INNER JOIN b_tindakan t ON p.id=t.pelayanan_id 
WHERE p.kunjungan_id='$idKunj' AND p.id>=(SELECT b_pelayanan.id FROM b_pelayanan INNER JOIN b_ms_unit ON b_pelayanan.unit_id=b_ms_unit.id 
...
WHERE p.kunjungan_id='$idKunj' AND p.jenis_kunjungan=3";
							...
							/*$scekInap="SELECT b_pelayanan.id FROM b_pelayanan INNER JOIN b_ms_unit ON b_pelayanan.unit_id=b_ms_unit.id 
WHERE b_pelayanan.kunjungan_id='$idKunj' AND b_ms_unit.inap=1 ORDER BY b_pelayanan.id LIMIT 1";
							$rsCekInap=mysql_query($scekInap);
							if (mysql_num_rows($rsCekInap)>0){
                        		$sUnit="SELECT DISTINCT p.id,mu.nama nmUnit,mu.id idUnit 
FROM b_pelayanan p INNER JOIN b_ms_unit mu ON p.unit_id=mu.id INNER JOIN b_tindakan t ON p.id=t.pelayanan_id 
WHERE p.kunjungan_id='$idKunj' AND p.id<(SELECT b_pelayanan.id FROM b_pelayanan INNER JOIN b_ms_unit ON b_pelayanan.unit_id=b_ms_unit.id 
WHERE b_pelayanan.kunjungan_id='$idKunj' AND b_ms_unit.inap=1 ORDER BY b_pelayanan.id LIMIT 1)";
...
LEFT JOIN b_ms_kelas mKls ON tk.kelas_id=mKls.id WHERE kunjungan_id='$idKunj' AND p.id='".$rwUnit['id']."'";
                        ...
                        <tr>
                            ...
                            <td></td>
                                ...
                                <?php
                                if ($ksoid==1) {
										$sTind="SELECT DATE_FORMAT(t.tgl,'%d-%m-%Y') tgl,mt.nama nmTind, t.biaya*t.qty AS biaya, t.biaya*t.qty AS iurbiaya,0 AS biaya_kso,t.bayar bayar,t.bayar_kso bayarKSO,t.qty cTind 
...
WHERE t.pelayanan_id='".$rwUnit['id']."' AND user_id='".$rwKonsul['user_id']."' ORDER BY t.id";
										...
										$sTind="SELECT DATE_FORMAT(t.tgl,'%d-%m-%Y') tgl,mt.nama nmTind, (t.biaya)*t.qty AS biaya,t.biaya_kso*t.qty AS biaya_kso,t.biaya_pasien*t.qty AS iurbiaya,t.bayar bayar,t.bayar_kso bayarKSO,t.qty cTind FROM b_tindakan t INNER JOIN b_ms_tindakan_kelas mtk ON t.ms_tindakan_kelas_id=mtk.id INNER JOIN b_ms_tindakan mt ON mtk.ms_tindakan_id=mt.id 
