...
if($statusProses=='Error') {
	...
	WHERE d.pelayanan_id = '".$pelayanan_id."') as gab $filter order by $sorting ";*/
		$sql="SELECT * FROM (SELECT d.diagnosa_id AS id,d.ms_diagnosa_id,md.kode,md.nama as nama1,IF(d.diagnosa_manual IS NULL,md.nama,d.diagnosa_manual) AS nama,
...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."' ORDER BY $dbapotek.a_obat.OBAT_ID";
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
		  ...
		  if(mysql_num_rows($rs_an) > 0){
			...
			if ($dataicd9cm=="-" || $dataicd9cm=="") $dataicd9cm="[          -          ]";
            ...
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).tglJamSQL($rows["tanggal"]).chr(3).$rows["nama"].chr(3).$icd9rm.chr(3).$rows["kelas"].chr(3).$rows["biaya"].chr(3).$rows["qty"].chr(3).$rows["subtotal"].chr(3).$dokter.chr(3).$rows["petugas"].chr(3).$rows["ket"].chr(6);
        }
    }
	elseif($grdINACBG == "true") {
        while ($rows=mysql_fetch_array($rs)) {
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$icdrm.chr(3).$rows["dokter"].chr(3).$rows["utama"].chr(3).$rows['d_akhir'].chr(3).$rows['klinis'].chr(3).$rows['banding'].chr(3).$rows['unit'].chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
