...
if (isset($_REQUEST['idx'])) {
  ...
  if (isset($_REQUEST['id'])) {
    $data = mysql_fetch_assoc(mysql_query("SELECT * FROM rm_identitas_pasien WHERE id = '{$_REQUEST['id']}'"));
    $arr_data = [];
    $all = $data['data'];
	  $str = "";
		...
		<td rowspan="2">
			...
			AGAMA : <div class="row">
						...
						<div class="col-4">
							...
							<input <?php if($arr_data[$i-1] == 'BUDDHA'){echo "checked";};?> type="checkbox" value="1" name="data_<?=$i;?>" <?=checked($arr_data[$i-1]);$i++;?> /> BUDDHA
						</div>
						<div class="col-4">
