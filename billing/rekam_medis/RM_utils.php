    ...
    if($grd == "true") {
		if($cmbKunjungan=='0'){
			...
			* 
			...
			CONCAT(p.nama,' (',(SELECT IFNULL(SUM(jml),0) FROM b_print_res_rm WHERE id_kunjungan=k.id),')') nama,
...
$jmlh=mysql_num_rows(mysql_query("SELECT * FROM
...
LEFT JOIN (SELECT * FROM b_diagnosa_rm WHERE degger_id=0 AND kunjungan_id='".$_REQUEST['kunjungan_id']."') drm ON drm.diagnosa_id=d.diagnosa_id
...
INNER JOIN b_ms_unit mu ON pl.unit_id = mu.id
WHERE d.kunjungan_id = '".$_REQUEST['kunjungan_id']."'  GROUP BY d.diagnosa_id) AS gab2
...
FROM (SELECT * FROM b_diagnosa_rm WHERE degger_id=0 AND kunjungan_id='".$_REQUEST['kunjungan_id']."') drm 
LEFT JOIN b_ms_diagnosa mdrm ON drm.ms_diagnosa_id=mdrm.id
LEFT JOIN b_ms_pegawai p3 ON p3.id = drm.user_act
...
WHERE drm.kunjungan_id = '".$_REQUEST['kunjungan_id']."') AS gab2"));
