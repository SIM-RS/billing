...
pk.cara_keluar,IF(tk.status_out=0,IF(tgl_out IS NULL,IF(DATEDIFF(NOW(),tgl_in)=0,1,DATEDIFF(NOW(),tgl_in)), IF(DATEDIFF(tgl_out,tgl_in)=0,1,DATEDIFF(tgl_out,tgl_in))),
IF(tgl_out IS NULL,DATEDIFF(NOW(),tgl_in), DATEDIFF(tgl_out,tgl_in))) los,p.id pelId,(SELECT SUM(qty * (biaya_kso+biaya_pasien))
