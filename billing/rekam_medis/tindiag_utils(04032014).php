...
WHERE b_resep.id_pelayanan = '".$pelayanan_id."' AND b_resep.no_resep='".$_REQUEST['no_resep']."' AND b_resep.apotek_id='".$_REQUEST['apotek_id']."' AND b_resep.tgl='".$_REQUEST['tgl_resep']."'";
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
		  ...
		  if(mysql_num_rows($rs_an) > 0){
            ...
            $i++;
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).tglJamSQL($rows["tanggal"]).chr(3).$rows["nama"].chr(3).$xxxicd9cm.$icd9rm.chr(3).$rows["kelas"].chr(3).$rows["biaya"].chr(3).$rows["qty"].chr(3).$rows["subtotal"].chr(3).$dokter.chr(3).$rows["petugas"].chr(3).$rows["ket"].chr(6);
        }
    }
	elseif($grdINACBG == "true") {
        while ($rows=mysql_fetch_array($rs)) {
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            ...
            $dt.=$sisip.chr(3).number_format($i,0,",","").chr(3).$rows["nama"].chr(3).$xxxicdrm.$icdrm.chr(3).$rows["dokter"].chr(3).$rows["utama"].chr(3).$kasusicdrm.chr(3).$rows['d_akhir'].chr(3).$rows['klinis'].chr(3).$rows['banding'].chr(3).$rows['unit'].chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
