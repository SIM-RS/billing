...
?>    
<table width="700" id="tblDiagnosaAsterix_v2" border="0" cellpadding="1" cellspacing="0">
	<tr class="headtable">
...
<?php 
	...
	WHERE d.AKTIF=1 AND d.degger<>1 $filter AND du.ms_unit_id='$unitId' ORDER BY d.id LIMIT 200";
		}
	}
	//echo $sql;
	$rs=mysql_query($sql);
	...
	{
		$i++;
		$arfvalue=$no."*|*".$rows['id']."*|*".$rows['nama']."*|*".$rows["kode"];
?>
	<tr id="lstDiagAsterix_v2<?php echo $i; ?>" lang="<?php echo $arfvalue; ?>" class="itemtableReq" onMouseOver="this.className='itemtableMOverReq'" onMouseOut="this.className='itemtableReq'" onClick="fSetICDX_RM_Asterix_v2(this.lang);">
