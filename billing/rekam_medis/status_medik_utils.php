...
$dipinjam = ($_REQUEST['dipinjam'] > 0) ? $_REQUEST['dipinjam'] : 0;
$jnsLayanan = $tmpLayanan = $dokterId = $pengganti = '';
...
switch(strtolower($_REQUEST['act'])) {
	case 'save':
		...
		$sIns="insert into b_status_medik (kunjungan_id,pasien_id,tgl_act,user_act,status,jnsLayanan,tmpLayanan,sm_dokter,sm_dokter_pengganti,dipinjam,flag) values ('".$kunjungan_id."','".$pasien_id."',now(),'".$userId."','".$statusKeluar."','{$jnsLayanan}','{$tmpLayanan}','{$dokterId}','{$pengganti}',{$dipinjam},'{$dipinjam}')";
		$qIns=mysql_query($sIns);
		
		if(mysql_affected_rows()){
			$sSave="update b_ms_pasien set 
						status_medik='".$statusKeluar."',flag='$flag'
					where id='".$pasien_id."'";
			mysql_query($sSave);
		}
		break;
...
if($statusProses=='Error') {
    ...
    if($grd == "true") {
		$sql="SELECT * FROM (SELECT DISTINCT * 
			...
			WHERE ((b_kunjungan.tgl='".tglSQL($_REQUEST['tgl'])."') OR (b_kunjungan.tgl<>'".tglSQL($_REQUEST['tgl'])."' AND b_kunjungan.pulang=0 AND b_tindakan_kamar.tgl_out IS NULL)) AND b_kunjungan.pasien_id=p.id LIMIT 1) IS NULL,'1','0') AS aktif
			FROM b_ms_pasien p 
