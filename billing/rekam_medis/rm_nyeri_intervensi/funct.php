  ...
  function area($data, $name, $class, $style, $event){

    if (isset($_REQUEST['id'])) {

      if ($data == "" || $data == "#") {
        return "<textarea style='{$style}' {$event} class='form-control {$class}' name='{$name}'></textarea>";
      } else {
        return "<textarea style='{$style}' {$event} class='form-control {$class}' name='{$name}'>{$data}</textarea>";
      }
