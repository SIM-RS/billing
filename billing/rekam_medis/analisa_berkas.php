...
<?php
            ...
            <table width="1000" border="0" cellpadding="0" cellspacing="0" bgcolor="#008484"  align="center" class="hd2">
                <tr>
                    <td height="30">&nbsp;FORM ANALISA BERKAS</td>
					<td align="right">&nbsp;</td>
                </tr>
            </table>
    ...
    <script type="text/JavaScript" language="JavaScript">
		...
		function showGrid(){
			...
			a1.baseURL("analisa_berkas_utils.php?grd=true&tgl="+document.getElementById('txtTgl').value+"&tmpLay="+document.getElementById('cmbJnsLay').value);
            a1.Init();
			...
			a2.baseURL("analisa_berkas_utils.php?grd2=true&tgl="+document.getElementById('txtTgl').value+"&tmpLay="+document.getElementById('cmbJnsLay').value);
            a2.Init();
            ...
            //a1.attachEvent("onRowDblClick","editNoSJP");
            //a2.onLoaded(konfirmasi);
			a3.baseURL("analisa_berkas_utils.php?grd3=true&tgl="+document.getElementById('txtTgl').value+"&tmpLay="+document.getElementById('cmbJnsLay').value);
            a3.Init();
		}
		...
		function cekList(){
			...
			var idReport = a3.getRowId(parseInt(a3.getSelRow()-1)+1);
			...
			Request('analisa_berkas_utils.php?grd3=false&act='+act+'&id='+idReport+'&pelayanan_id='+idPel+'&formulir_id='+document.getElementById('formulir_id').value+'&userId=<?php echo $userId; ?>','spanSukRM','','GET');
		}
		
		function ambilDataReportRM(){
			var sisip = a2.getRowId(a2.getSelRow());
			document.getElementById('formulir_id').value=sisip;
			a3.loadURL("analisa_berkas_utils.php?grd3=true&parent_id="+sisip+'&pelayanan_id='+document.getElementById('pelayanan_id').value,'','GET');
		}
		...
		function ambilDataPasien(){
			...
			if(document.getElementById('pelayanan_id').value==''){
				a2.loadURL("analisa_berkas_utils.php?grd2=true&pelayanan_id=0",'','GET');	
			}
			else{
				a2.loadURL("analisa_berkas_utils.php?grd2=true&pelayanan_id="+document.getElementById('pelayanan_id').value,'','GET');
			}
		}
		...
		function saring(){
			...
			var url = "analisa_berkas_utils.php?grd=true&tgl="+tgl+"&tmpLay="+tmpLay+"&no_rm="+no_rm;
			//alert(url);
			a1.loadURL(url,"","GET");
		}
		...
		function filterNoRM(ev,par){
			if(ev.which==13){
				...
				var tmpLay=document.getElementById('cmbJnsLay').value;
				var no_rm=document.getElementById('txtFilter').value;
				var url = "analisa_berkas_utils.php?act=view&grd=true&tgl="+tgl+"&tmpLay="+tmpLay+"&no_rm="+no_rm;
				a1.loadURL(url,"","GET");
			}
		}
		...
		function goFilterAndSort(grd){
			...
			if (grd=="gridbox"){	
				url="analisa_berkas_utils.php?grd=true&tgl="+tgl+"&tmpLay="+tmpLay+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
				a1.loadURL(url,"","GET");
			}
		}
