	...
	case 'save':
		...
		if(mysql_affected_rows()){
			$sSave="update b_ms_pasien set 
						...
						jnsLayanan='{$jnsLayanan}',
						tmpLayanan='{$tmpLayanan}',
						sm_dokter='{$dokterId}',
						sm_dokter_pengganti='{$pengganti}'
					where id='".$pasien_id."'";
			mysql_query($sSave);
		}
		break;
...
if($statusProses=='Error') {
    ...
    if($grd == "true") {
		$sql="SELECT * FROM (SELECT DISTINCT * 
			...
			p.status_medik,
			(SELECT nama FROM b_ms_unit WHERE id = p.jnsLayanan) jnsLay,
			(SELECT nama FROM b_ms_unit WHERE id = p.tmpLayanan) tmpLay,
			(SELECT nama FROM b_ms_pegawai WHERE id = p.sm_dokter) dokter,
			p.sm_dokter_pengganti,
