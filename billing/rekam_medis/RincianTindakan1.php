...
WHERE p.kunjungan_id='$idKunj' AND p.id>=$idPel AND mu.kategori<>5";
							}else {
								$sUnit="SELECT DISTINCT p.id,mu.nama nmUnit ,mu.id idUnit,mu.parent_id,mu.inap FROM b_pelayanan p INNER JOIN b_ms_unit mu ON p.unit_id=mu.id 
WHERE (p.id='$idPel' OR p.unit_id_asal=(SELECT unit_id FROM b_pelayanan WHERE id='$idPel')) AND p.kunjungan_id='$idKunj' AND mu.inap=0 AND mu.kategori<>5";
							}
						}else{
							if ($inap=="1") {
							...
							WHERE p.kunjungan_id='$idKunj' AND mu.kategori<>5 AND p.unit_id<>$idUnitAmbulan AND p.unit_id<>$idUnitJenazah";
								...
								WHERE p.id='$idPel' AND mu.kategori<>5";
							}
						}
                        //echo $sUnit."<br>";
