...
if (isset($_REQUEST['cetak'])) {
    ...
    $data = mysql_fetch_assoc(mysql_query("SELECT * FROM rm_edukasi_tindakan_anestesi_sedasi WHERE id = '{$_REQUEST['cetak']}'"));
	$arr_data = explode("|", $data['data']);
  }

  if (isset($_REQUEST['pdf'])) {
    $print_mode = true;
    $data = mysql_fetch_assoc(mysql_query("SELECT * FROM rm_edukasi_tindakan_anestesi_sedasi WHERE id = '{$_REQUEST['pdf']}'"));
    $arr_data = explode("|", $data['data']);
...
<?php endif; ?>
	...
	<script>
		<?php if (isset($_REQUEST["pdf"])): ?>
			...
			var opt = {
				margin: 0,
				filename: "RM 21.8 <?=$dataPasien["nama"]?>.pdf",
