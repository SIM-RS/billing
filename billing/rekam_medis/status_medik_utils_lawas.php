...
switch(strtolower($_REQUEST['act'])) {
	case 'save':
		...
		$sIns="insert into b_status_medik (kunjungan_id,pasien_id,tgl,user_act,status) values ('".$kunjungan_id."','".$pasien_id."',now(),'".$userId."','".$statusKeluar."')";
		$qIns=mysql_query($sIns);
		
		if(mysql_affected_rows()){
			$sSave="update b_ms_pasien set status_medik='".$statusKeluar."' where id='".$pasien_id."'";
			mysql_query($sSave);
		}
		break;
}
