...
LEFT JOIN (SELECT * FROM b_diagnosa_rm WHERE kunjungan_id = '".$kunjungan_id."' AND degger_id=0) drm ON drm.diagnosa_id=d.diagnosa_id
...
LEFT JOIN (SELECT * FROM b_diagnosa_rm WHERE kunjungan_id = '".$kunjungan_id."' AND degger_id=0) drm ON drm.diagnosa_id=d.b_diagnosa_id
LEFT JOIN b_ms_diagnosa mdrm ON drm.ms_diagnosa_id=mdrm.id
LEFT JOIN b_ms_pegawai p3 ON p3.id = drm.user_act
    ...
    elseif($grd1 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
			...
			if($rows["act"]=='0'){
				...
				$kasusicdrm="<span style='color:#0000FF' title='Klik Untuk Mengubah Kasus ICD-10' onclick='fSetKasusICD_RM(".$rows["id"].",".$ksg.");'>".$datakasusicdrm."</span>";
			}
		...
		while ($isix=mysql_fetch_array($que2)) {
            ...
            $sisip=$rows["id"]."|".$rows['user_id']."|".$rows['ms_diagnosa_id']."|".$rows['type_dokter']."|".$rows['primer']."|".$rows['akhir']."|".$gol_id."|".$gol."|".$isPK."|".$rows['kode']."|".$rows['id_diag_rm']."|".$rows['cekdegger']."|".$rows['ms_diag_rm']."|".$rows['nama2']."|".$tot."|".$isinya."|".$vb."|".$isi;;
            $i++;
            ...
            $dt.=$sisip.chr(3)."<span $css>".number_format($i,0,",","")."</span>".chr(3)."<span $css>".$rows["tgldiag"]."</span>".chr(3)."<span $css>".$dig."</span>".chr(3)."<span $css>".$xxxicdrm.$icdrm."</span>".chr(3)."<span $css>".$rows["dokter"]."</span>".chr(3)."<span $css>".$rows["coder"]."</span>".chr(3)."<span $css>".$rows["petugas"]."</span>".chr(3)."<span $css>".$rows["utama"]."</span>".chr(3)."<span $css>".$kasusicdrm."</span>".chr(3)."<span $css>".$rows['d_akhir']."</span>".chr(3)."<span $css>".$rows['klinis']."</span>".chr(3)."<span $css>".$rows['banding']."</span>".chr(3)."<span $css>".$rows['unit']."</span>".chr(6);
        }
    }
    elseif($grd2 == "true") {
        while ($rows=mysql_fetch_array($rs)) {
