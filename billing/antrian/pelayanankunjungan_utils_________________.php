...
r.nama_pasien,r.alamat,u.unit_billing,IFNULL(u.UNIT_ID,0) UNIT_ID,CURDATE() tgl,NOW() tgl_act,u.UNIT_NAME AS tempat_layanan,m.NAMA AS bayar,u.`UNIT_NAME` AS nama_unit,k.`NAMA` AS kepemilikan
FROM rspelindo_billing.b_resep r INNER JOIN rspelindo_billing.b_pelayanan p ON r.id_pelayanan=p.id
LEFT JOIN {$dbapotek}.a_unit u ON u.unit_billing=p.unit_id
LEFT JOIN {$dbapotek}.a_mitra m ON r.kso_id=m.kso_id_billing
LEFT JOIN {$dbapotek}.`a_kepemilikan` k ON k.`ID`=r.`kepemilikan_id`
...
LEFT JOIN b_ms_kso kso ON kso.id = pl.kso_id ".$saringan.") as gab ".$filter." order by ".$sorting;
    ...
    if($grd == "true") {
        while ($rows=mysql_fetch_array($rs)) {
            $sisip = $rows['no_rm']."|".$rows['idp']."|".$rows['idk'];
            $i++;
            $dt.=$rows['id'].chr(3).number_format($i,0,",","").chr(3).$rows["no_resep"].chr(3).$rows["no_rm"].chr(3).$rows["nama_pasien"].chr(3).$rows["tempat_layanan"].chr(3).$rows["bayar"].chr(3).$rows["kepemilikan"].chr(3).$rows["nama_unit"].chr(6);
        }
    }
    else if($grd == "tarik"){
        while ($rows=mysql_fetch_array($rs)) {
