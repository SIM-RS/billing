         ...
         <table id="pasien_table">
			...
			if ($status==""){
				$sql="SELECT   
						...
						IF((SELECT  u.nama FROM b_pelayanan p
						INNER JOIN `b_ms_unit` u ON p.unit_id = u.id
						WHERE `kunjungan_id`=k.`id` AND u.inap=1 ORDER BY p.id DESC LIMIT 1 )IS NULL,
						(SELECT  u.nama FROM  `b_ms_unit` u
						WHERE u.id=k.`unit_id`),
						(SELECT  u.nama FROM b_pelayanan p
						INNER JOIN `b_ms_unit` u ON p.unit_id = u.id
						WHERE `kunjungan_id`=k.`id` AND u.inap=1 ORDER BY p.id DESC LIMIT 1 )
						...
						WHERE
						mp.nama LIKE '$kataKunci%' 
						OR mp.no_rm = '$kataKunci'";
			}elseif($status=="all"){
				$sql="SELECT   
						DISTINCT k.id AS kunj_id,
						k.id AS kunj_id,
					...
					if ($ksoid==$idKsoBPJS){
						...
						//cek jika pavilyun HCU --> naik kelas pavilyun
							...
							$sqlCekNaikKelas="SELECT id,level,nama,tipe FROM (
												...
												AND kso.id=6
												AND p.unit_id='$idPavilyunHCU'
												) AS tbl ORDER BY level DESC LIMIT 1";
							//echo $sqlCekNaikKelas.";<br/>";
							$rsCekNaikKelas=mysql_query($sqlCekNaikKelas);
							if (mysql_num_rows($rsCekNaikKelas)>0){
