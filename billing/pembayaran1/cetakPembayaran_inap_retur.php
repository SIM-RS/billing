<?php
	...
	$fromModul=$_REQUEST["fromModul"];
	
   	if ($fromModul=="1"){
   		$sql="SELECT r.*,b.nilai,b.kunjungan_id,b.user_act FROM $dbbilling.b_return_pembayaran r INNER JOIN $dbbilling.b_bayar b ON r.bayar_id=b.id WHERE r.id='".$_GET['id']."'";
		//echo $sql."<br>";
   		$rsReturn=mysql_query($sql);
		$rwReturn=mysql_fetch_array($rsReturn);
		$noReturn=$rwReturn["no_return"];
		$tglReturn=tglSQL($rwReturn["tgl_return"]);
		$nilaiReturn=$rwReturn["nilai_retur"];
		$idKunj=$rwReturn["kunjungan_id"];
		$kunjungan_id = $rwReturn["kunjungan_id"];
		$idbayar = $rwReturn["bayar_id"];
		//$userId = $_GET['idUser'];
	}else{
   		$sql="SELECT r.*,b.nilai,b.kunjungan_id,b.user_act FROM $dbbilling.b_return_pembayaran r INNER JOIN $dbbilling.b_bayar b ON r.bayar_id=b.id WHERE r.bayar_id='".$_GET['idbayar']."'";
   		$rsReturn=mysql_query($sql);
		$rwReturn=mysql_fetch_array($rsReturn);
		$noReturn=$rwReturn["no_return"];
		$tglReturn=tglSQL($rwReturn["tgl_return"]);
	...
	if ($fromModul=="1"){
		$queryPetugas = "SELECT nama, DATE_FORMAT(NOW(), '%d-%m-%Y') AS tgl_now, TIME_FORMAT(NOW(), '%H:%i') AS time_now
						FROM $dbkeuangan.k_ms_user WHERE id = '{$userId}'";
	}else{
		$queryPetugas = "SELECT nama, DATE_FORMAT(NOW(), '%d-%m-%Y') AS tgl_now, TIME_FORMAT(NOW(), '%H:%i') AS time_now
						FROM $dbbilling.b_ms_pegawai WHERE id = '{$userId}'";
	}
	...
	$bayar = $row['nilai'];
	$nBayar_Retur=$bayar-$nilaiReturn;
	
	if ($nilaiReturn>0){
		$no_kwitansi .=" ( Rp. ".currency($bayar).",- )";
		$noReturn .=" ( Rp. ".currency($nilaiReturn).",- )";
	}
...
?>
 ...
 <span id="cetak_count" style="display:none;"></span>
	<table width="717" id="tabel-isi" style="border:0px dashed #000;">
		...
		<tr id="heading-container">
			<td width="158" class="kiri">No. Bukti Bayar</td>
			<td width="9">:</td>
		...
		?>
		<tr id="heading-container">
			<td width="158" class="kiri">No. Bukti Retur</td>
			<td width="9">:</td>
			<td colspan="3" style="padding-left:10px; font-weight:bold;"><?=$noReturn;?></td>
		</tr>
        <?php 
		}
		?>
		...
		<tr>
			...
			<td colspan="3" class="isi" style="padding-left:10px;" >
				<?="# ".ucwords(terbilang($nBayar_Retur)." ".$koma." Rupiah #")?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td height="25" colspan="3" class="kiri" style="font-weight:bold; font-size:25px;">Rp. <?=currency($nBayar_Retur).",-&nbsp;&nbsp;&nbsp;";?></td>
            <td width="155">&nbsp;</td>
			<td width="230" align="center" ><?="Sidoarjo, ".$row['tgl_bayar']?></td>
		</tr>
        <tr>
			<td colspan="2"></td>
