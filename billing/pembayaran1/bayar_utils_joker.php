						...
						if($_REQUEST['jenisKasir']=='0'){
							...
							if ($rwRef=mysql_fetch_array($rsRef)){
								...
								if ($bayarIGD=="0"){
									...
									*/
									$sqlTind = "SELECT t.* FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE p.kunjungan_id='".$_REQUEST['idKunj']."' AND (p.unit_id=45 OR p.unit_id_asal=45) AND mu.inap=0 AND t.bayar_pasien < (t.biaya_pasien*t.qty) ORDER BY t.tgl_act";
									/*
									$sqlObat = "SELECT ap.* FROM ".$dbapotek.".a_penjualan ap
											...
											AND (p.unit_id=45 OR p.unit_id_asal=45) 
											AND p.unit_id<>$idUnitAmbulan 
											AND p.unit_id<>$idUnitJenazah 
											AND mu.inap=0 
											AND ap.BAYAR_PASIEN<((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN) 
											AND ap.CARA_BAYAR=4
											ORDER BY ap.ID";
									*/		
									$sqlObat = "SELECT ap.* FROM ".$dbapotek.".a_penjualan ap
									...
									if ($_REQUEST['idKasir']==127){
										...
										*/
										$sqlTind = "SELECT t.* FROM b_tindakan t INNER JOIN b_pelayanan p ON t.pelayanan_id=p.id INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE p.kunjungan_id='".$_REQUEST['idKunj']."' AND t.bayar_pasien < (t.biaya_pasien*t.qty) ORDER BY t.tgl_act";
									}
...
WHERE k.id='".$_REQUEST['idKunj']."' AND mu.inap=1 AND p.dilayani>0 AND k.tgl_pulang IS NULL";
							...
							if($nilai>0){
								...
								while($rwObat=mysql_fetch_array($rsObat)){
									...
									$sub_total = ($rwObat['QTY_JUAL'] - $rwObat['QTY_RETUR']) * $rwObat['HARGA_SATUAN'];
									$biaya = ($sub_total - $rwObat['BAYAR_PASIEN']);						
									
									$nilai = $nilai - $biaya;
									
									if($nilai >= 0 ){
										$sqlIn = "INSERT INTO b_bayar_tindakan(bayar_id,tindakan_id,kso_id,nilai,tipe) VALUES('$idBayar','$idPenj','$kso_id','$biaya','2')";
										$rsIn = mysql_query($sqlIn);
										//echo $sqlIn."<br>";
										
										$sqlUpdate="UPDATE ".$dbapotek.".a_penjualan SET TGL_BAYAR='$tgl', TGL_BAYAR_ACT=now(), USER_ID_BAYAR='$userId', BAYAR_PASIEN=BAYAR_PASIEN+$biaya, SUDAH_BAYAR=1 WHERE ID='$idPenj'";
										//echo $sqlUpdate."<br>";
										...
										$sqlIn = "INSERT INTO b_bayar_tindakan(bayar_id,tindakan_id,kso_id,nilai,tipe) VALUES('$idBayar','$idPenj','$kso_id','$nilai','2')";
										$rsIn = mysql_query($sqlIn);
										//echo $sqlIn."<br>";
										
										$sqlUpdate="UPDATE ".$dbapotek.".a_penjualan SET TGL_BAYAR='$tgl', TGL_BAYAR_ACT=now(), USER_ID_BAYAR='$userId', BAYAR_PASIEN=BAYAR_PASIEN+$nilai, SUDAH_BAYAR='$lunas' WHERE ID='$idPenj'";
										//echo $sqlUpdate."<br>";
										$rsUpdate=mysql_query($sqlUpdate);
									}
								}
								...
								if ($ok == 'true'){
									...
									if($_REQUEST['jenisKasir']=='0'){
										//kujungan_id
										$sql="INSERT INTO b_bayar_tindakan(bayar_id,tindakan_id,kso_id,nilai,tipe)
										SELECT $idBayar,ap.ID,ap.KSO_ID,0,2 
										FROM ".$dbapotek.".a_penjualan ap
										INNER JOIN b_pelayanan p ON p.id=ap.NO_KUNJUNGAN
										WHERE p.kunjungan_id='".$_REQUEST['idKunj']."' 
										AND ap.SUDAH_BAYAR=0
										AND ap.CARA_BAYAR=4 
										AND ap.ID NOT IN (SELECT tindakan_id FROM b_bayar_tindakan bt 
										INNER JOIN b_bayar b ON bt.bayar_id=b.id 
										WHERE b.kunjungan_id='".$_REQUEST['idKunj']."' AND bt.tipe=2)
										ORDER BY ap.ID";
									}
									else{
										//pelayanan_id
			...
			break;
	case 'hapus':
		// Insert into b_bayar_log -> histori penghapusan pembayaran
		$sqlBayarInsert = "insert into b_bayar_log (select *,NOW(),$userId from b_bayar where b_bayar.id = '".$_REQUEST['rowid']."')";
		$queryLog = mysql_query($sqlBayarInsert);
		$sqlBayarTindInsert = "insert into b_bayar_tindakan_log (select *,NOW() from b_bayar_tindakan where b_bayar_tindakan.bayar_id = '".$_REQUEST['rowid']."')";
		mysql_query($sqlBayarTindInsert);
		// end
		...
		mysql_query($sqlHapus);
		$sqlHapus="UPDATE ".$dbapotek.".a_penjualan t,b_bayar_tindakan bt SET t.BAYAR_PASIEN=t.BAYAR_PASIEN-bt.nilai, t.SUDAH_BAYAR=0 WHERE t.ID=bt.tindakan_id AND bt.bayar_id='".$_REQUEST['rowid']."' AND bt.tipe=2";
		mysql_query($sqlHapus);
