...
switch (strtolower($_REQUEST['act']))
{
				...
				*
				...
				FROM
				  ...
				  INNER JOIN b_ms_kso kso
					ON kso.id = t.kso_id 
				WHERE (mp.nama LIKE '$kataKunci%' OR mp.no_rm = '$kataKunci')
				UNION
				SELECT
				  DISTINCT 
				  p.jenis_kunjungan, 
				...
				GROUP BY kunj_id,jenis_kunjungan,kso_id
				ORDER BY kunj_id DESC,jenis_kunjungan ASC";
			}elseif($status=="all"){
				$sql="SELECT
				*
				FROM
				...
				else
				{
					if ($ksoid==$idKsoBPJS){
						...
						$sHakKelas="SELECT id,level,nama FROM (
									...
									p.kunjungan_id='".$rw['kunj_id']."'
									AND kso.id=6
									$fJenisKunjungan) AS tblhakkelas ORDER BY level DESC LIMIT 1";
						//echo $sHakKelas."<br/>";
						...
						$sqlCekNaikKelas="SELECT id,level,nama,tipe FROM (
											...
											AND kso.id=6
											AND mk.tipe<>1
											$fJenisKunjungan) AS tbl ORDER BY level DESC LIMIT 1";
						...
						if ($isNaikKelas==1){
							...
							$sNaekKlsObat="SELECT
									  ...
									  SUM(ap.QTY_JUAL * ap.HARGA_SATUAN) AS SUBTOTAL, ap.DOKTER, ap.UTANG
									  /* ,IF(ap.DIJAMIN=0,SUM(IF(ap.CARA_BAYAR=2,((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN),0)),'0') AS JAMINANOBAT */
									  ,IF(ap.DIJAMIN=0,SUM(IF(ap.CARA_BAYAR=2,(ap.QTY_JUAL * ap.HARGA_SATUAN),0)),'0') AS JAMINANOBAT, b_pelayanan.id as pel_id
						...
						elseif ($isNaikKelas==2){
							...
							if ($TipeBayar==1){
								...
								$sPavObatU="SELECT
										  /* SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN) AS SUBTOTAL, */ 
										  (ap.QTY_JUAL * ap.HARGA_SATUAN) AS SUBTOTAL, ap.DOKTER, ap.NO_PENJUALAN
										  /* ,IF(ap.DIJAMIN=0,SUM(IF(ap.CARA_BAYAR=2,((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN),0)),'0') AS JAMINANOBAT */
										  ,IF(ap.DIJAMIN=0,(IF(ap.CARA_BAYAR=2,(ap.QTY_JUAL * ap.HARGA_SATUAN),0)),'0') AS JAMINANOBAT
								...
								/* $sPavObatUw="SELECT
										  SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN) AS SUBTOTAL, ap.DOKTER, ap.NO_PENJUALAN
										  ,IF(ap.DIJAMIN=0,SUM(IF(ap.CARA_BAYAR=2,((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN),0)),'0') AS JAMINANOBAT
										FROM
										  $dbapotek.a_penjualan ap 
										  INNER JOIN b_pelayanan 
								...
								$rwPavObatUw=mysql_fetch_array($qPavObatUw); */
								...
								/* $sPavObatU2="SELECT
										  SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN) AS SUBTOTAL, ap.DOKTER, ap.NO_PENJUALAN, ap.UTANG, b_pelayanan.id as pel_id
										FROM
										  $dbapotek.a_penjualan ap 
										  INNER JOIN b_pelayanan 
											ON ap.NO_KUNJUNGAN = b_pelayanan.id
								...
								$sPavObatU2="SELECT
										  /* SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN) AS SUBTOTAL, */
										  SUM(ap.QTY_JUAL * ap.HARGA_SATUAN) AS SUBTOTAL, ap.DOKTER, ap.UTANG, b_pelayanan.id as pel_id
										  ,ap.ID AS idpenjualan
								...
								$sPavObat2="SELECT
										  /* SUM((ap.QTY_JUAL-ap.QTY_RETUR) * ap.HARGA_SATUAN) AS SUBTOTAL, */
										  SUM(ap.QTY_JUAL * ap.HARGA_SATUAN) AS SUBTOTAL, ap.DOKTER, ap.UTANG, b_pelayanan.id as pel_id, ap.ID AS idpenjualan
										  , ap.NO_PENJUALAN, ap.UNIT_ID, ap.NO_KUNJUNGAN, ap.NO_PASIEN
										FROM
										  $dbapotek.a_penjualan ap 
										  INNER JOIN b_pelayanan 
					...
					// JASA RAHARJA
						...
						if ($kasir==127){
							$sqlTin="select 
								...
								AND t.ms_tindakan_kelas_id IN (7513) $fJenisKunjungan $umum_stlh_krs2
								AND t.id NOT IN 
							...
							$sqlTin="select 
								...
								inner join b_pelayanan p on t.pelayanan_id=p.id
								where p.kunjungan_id='".$rw['kunj_id']."' AND t.lunas=0
								$fJenisKunjungan $umum_stlh_krs2
								AND t.id NOT IN 
								(SELECT tindakan_id FROM b_bayar_tindakan bt INNER JOIN b_bayar b ON bt.bayar_id=b.id 
								INNER JOIN b_tindakan t ON bt.tindakan_id=t.id 
								WHERE b.kunjungan_id='".$rw['kunj_id']."' AND bt.tipe=0 AND t.lunas=1)";
	...
	IFNULL(SUM(IF(b.status_out=0,(IF(DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)=0,1,DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)))*(beban_pasien)-bayar,
	(IF(DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)=0,0,DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)))*(beban_pasien)-bayar)),0) AS kurang_kamar,SUM(IFNULL(bayar,0)) bayar 
	FROM b_tindakan_kamar b INNER JOIN b_pelayanan p ON b.pelayanan_id=p.id 
	WHERE p.kunjungan_id='".$rw['kunj_id']."' AND b.aktif=1 $fJenisKunjungan";
						//echo $sqlKamar.";<br/>";
	...
	# IF(t2.KRONIS=2,'0',t2.UTANG) sisa 
	t2.UTANG as sisa, t2.DOKTER, t2.NO_PENJUALAN, t2.pel_id
	FROM (SELECT ap.*, p.id as pel_id FROM $dbapotek.a_penjualan ap INNER JOIN b_pelayanan p ON ap.NO_KUNJUNGAN=p.id 
	WHERE p.kunjungan_id='".$rw['kunj_id']."' AND ap.CARA_BAYAR=2 and ap.KRONIS<>2 and ap.DIJAMIN=0 $fJenisKunjungan) AS t2";
						//echo $sqlObat.";<br/>";
						...
						$sqlObat="SELECT 
							  ...
							  IFNULL(SUM(t2.QTY_JUAL * t2.HARGA_SATUAN),0) AS SUBTOTAL,
							  /* IF(t2.KRONIS=2,'0',t2.UTANG) sisa */
							  t2.UTANG AS sisa,
							  t2.DOKTER,
							...
							WHERE p.kunjungan_id='".$rw['kunj_id']."' AND ap.CARA_BAYAR=2 and ap.KRONIS<>2 and ap.DIJAMIN=0 $fJenisKunjungan) AS t2 GROUP BY t2.NO_PENJUALAN, t2.USER_ID";
						//echo $sqlObat.";<br/>";
						$rsObat=mysql_query($sqlObat);
						$bayar2=0;
						...
						/* $sqlObat2="SELECT IFNULL(SUM(t2.QTY_JUAL),0) QTY_JUAL,IFNULL(SUM(t2.QTY_RETUR),0) QTY_RETUR,
	IFNULL(SUM((t2.QTY_JUAL-t2.QTY_RETUR) * t2.HARGA_SATUAN),0) AS SUBTOTAL, 
	# IF(t2.KRONIS=2,'0',t2.UTANG) sisa  
	t2.UTANG as sisa, t2.DOKTER, t2.NO_PENJUALAN
	FROM (SELECT ap.* FROM $dbapotek.a_penjualan ap INNER JOIN b_pelayanan p ON ap.NO_KUNJUNGAN=p.id 
	WHERE p.kunjungan_id='".$rw['kunj_id']."' AND ap.CARA_BAYAR=2 and ap.KRONIS<>2 and ap.DIJAMIN=1 $fJenisKunjungan) AS t2";
						//echo $sqlObat2.";<br/>";
						$rsObat2=mysql_query($sqlObat2);
						$rwObat2=mysql_fetch_array($rsObat2); */
						
						$sqlObat2="SELECT 
							  IFNULL(t2.QTY_JUAL, 0) QTY_JUAL,
							  IFNULL(t2.QTY_RETUR, 0) QTY_RETUR,
							  IFNULL(t2.QTY_JUAL * t2.HARGA_SATUAN,0) AS SUBTOTAL,
							  /* IF(t2.KRONIS=2,'0',t2.UTANG) sisa */
							  t2.UTANG AS sisa,
							  t2.DOKTER,
						...
						//echo "numrows=".mysql_num_rows($rsTin1)."<br>";
						/*if (mysql_num_rows($rsTin1)>0){
							$rwTin1=mysql_fetch_array($rsTin1);
							if($rwTin1['biaya_kso'] > 0){
								$jaminan=$rwTin1['biaya_kso'];
								$kurang=$rwTin1['biaya_px'];
							}else{
						...
						//echo "Byr1=".$bayar."Byr2=".$byrObat."<br>";
						//echo "totTind=".$rwTin['total'].", TotKamar=".$rwKamar['kamar']."subTot=".$subTotal."jaminan2=".$jaminan2."<br>";
						//$total=$rwTin['total']+$rwKamar['kamar']+$rwObat['SUBTOTAL']-$bayar2-$jaminan2;
						$total=$rwTin['total']+$rwKamar['kamar']+$subTotal-$bayar2;
						//echo "KrgJRH=".$total."--tind".$rwTin['total']."--kmr".$rwKamar['kamar']."--sub".$subTotal."--byr".$bayar2."--jam".$jaminan2;
						if($total<0){$total=0;}
	...
	(IF(DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)=0,0,DATEDIFF(IFNULL(tgl_out,NOW()),tgl_in)))*(beban_kso))),0) AS kamar_kso
						...
						$total=($rwTin['total']+$rwKamar['kamar'])-($rwTin['dijamin']+$rwKamar['kamar_kso']);
						if($total<0){
							$total=0;
						}
					...
					if($ksoid != $idKsoBPJS && $ksoid !=9){
						...
						while($rwKonsul=mysql_fetch_array($qKonsul)){
							$sObat="SELECT * 
								  ...
								  SUM(IF(ap.CARA_BAYAR=2,(ap.QTY_JUAL * ap.HARGA_SATUAN),0)) AS DIJAMIN,
								 ...
								 /*  IF(ap.KRONIS=2,'0',ap.UTANG) sisa */
								  ap.UTANG as sisa, b_pelayanan.id as pel_id, ap.id idpenjualan
								FROM
								  (SELECT 
									dba_ap.* 
								  FROM
			...
			//akhir cek konsul+MRS
			   ...
			   <td>&nbsp;<?php echo number_format($sisa_tagihan,0,',','.');?></td>
               </tr>
               <?php
		   			$i++;
				}
