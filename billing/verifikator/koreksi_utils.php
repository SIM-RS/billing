...
switch(strtolower($tabel)){
    case 'kunjungan':
        if($_REQUEST['act']=='save'){
            $sqlUpdate="update b_kunjungan set pulang='$pulang',tgl_sjp=".(($tglSJP=='00-00-0000'||$tglSJP=='')?"(NULL)":"'".tglSQL($tglSJP)."'").",tgl=".(($tglMsk=='00-00-0000'||$tglMsk=='')?"(NULL)":"'".tglSQL($tglMsk)."'").",tgl_pulang=".(($tglPlg=='00-00-0000 00:00:00'||$tglPlg=='')?"(NULL)":"'".tglJamSQL($tglPlg)."'").",kelas_id='$kelas_id',isbaru='$baruLama',kso_id='$kso_id',kso_kelas_id='$kso_kelas_id',umur_thn='".$_REQUEST['uThn']."',umur_bln='".$_REQUEST['uBln']."',umur_hr='".$_REQUEST['uHr']."' where id='$kid'";
        ...
        $sql="SELECT k.id,k.jenis_layanan,k.umur_thn,k.umur_bln,k.umur_hr,n.nama AS jenis,unit_id,u.nama AS unit,u.inap,
        ...
        <table width="100%" border="0" style="font-size:12px; font-family:verdana;" class="tepi">
            ...
            <tr>
                ...
                <td>Umur</td><td>:</td><td><input type="text" id="uThn" name="uThn" size="5" value="<?php echo $rw['umur_thn'];?>" onchange="simpanKunjungan('<?php echo $rw['id'];?>');"  /> Thn &nbsp; <input type="text" id="uBln" name="uBln" size="5" value="<?php echo $rw['umur_bln'];?>" onchange="simpanKunjungan('<?php echo $rw['id'];?>');"/> Bln &nbsp; <input type="text" id="uHari" name="uHari" size="5" value="<?php echo $rw['umur_hr'];?>" onchange="simpanKunjungan('<?php echo $rw['id'];?>');"/> Hari </td>
    ...
    case 'pelayanan':
        if($_REQUEST['act']=='save'){
			...
			if (mysql_num_rows($rsdTind)>0){
				...
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act,flag) VALUES('Menghapus Data Pelayanan','$sqlUpdate',now(),'$IdUser','$flag')";
				mysql_query($sql);
			}
        }
        if($jnsRwt=='0'){
    ...
    case 'tindakan':
		$fUser="";
		if ($IdUser!="") $fUser=",verifikator='$IdUser'";
        if($_REQUEST['act']=='save'){
            ...
            if(mysql_query($sqlUpdate)){
				...
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act,flag) VALUES('Mengubah Data Tindakan','$sqlUpdate',now(),'$IdUser','$flag')";
				mysql_query($sql);
            }
            else{
                $notice='Simpan Gagal!';
...
INNER JOIN b_ms_unit mu ON p.unit_id=mu.id WHERE t.id='$id'";
            ...
            if(mysql_query($sqlDelete)){
				...
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act,flag) VALUES('Menghapus Data Tindakan','$sqlDelete',now(),'$IdUser','$flag')";
				mysql_query($sql);
            }
            else{
                $notice='Hapus Gagal!';
	...
	if($tglOut == "-- "){
        ...
        if($_REQUEST['act']=='save_kamar'){
            ...
            $sql="update b_tindakan_kamar set kamar_id='$kamar',kode='".$rwKmr['kode']."',nama='".$rwKmr['nama']."',tarip='".$rwKmr['tarip']."', kelas_id='$kelas', tgl_in=".(($tglIn=='00-00-0000 00:00:00'||$tglIn=='')?"(NULL)":"'".tglJamSQL($tglIn)."'").",tgl_out=".(($tglOut=='00-00-0000 00:00:00'||$sTglOut==true||$tglOut=='')?"(NULL)":"'".tglJamSQL($tglOut)."'").",beban_kso='$bebanKso',beban_pasien='$bebanPasien',status_out='$statusOut' where id='$id'";
            mysql_query($sql);           
			if (mysql_errno()==0){
				$sqlUpdate=str_replace("'","''",$sql);
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act,flag) VALUES('Mengubah Data Kamar','$sqlUpdate',now(),'$IdUser','$flag')";
				mysql_query($sql);
			}
			...
			if (mysql_errno()==0){
				$sql.=";ket -> ".$cket;
				$sqlUpdate=str_replace("'","''",$sql);
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act,flag) VALUES('Menghapus Data Kamar','$sqlUpdate',now(),'$IdUser','$flag')";
				mysql_query($sql);
			}
            ...
            $insert="insert into b_tindakan_kamar (pelayanan_id,unit_id_asal,tgl_in,kamar_id,kode,nama,tarip,beban_kso,beban_pasien,kelas_id,bayar,bayar_pasien,lunas,flag)
            values ('".$pelayanan[0]."','".$pelayanan[2]."','".$pelayanan[3]."','".$kamar."','".$rwKmr['kode']."','".$rwKmr['nama']."','".$tarip."',".($pelayanan[5]!='1'?$tarip:'0').",".($pelayanan[5]=='1'?$tarip:'0').",".$pelayanan[4].",".$bayar.",".$bayarPasien.",".($bayar<=$bayarPasien?'1':'0').",'$flag') ";
            mysql_query($insert);
			if (mysql_errno()==0){
				$sqlUpdate=str_replace("'","''",$insert);
				$sql="INSERT INTO b_log_act(action,query,tgl_act,user_act,flag) VALUES('Menambah Data Kamar','$sqlUpdate',now(),'$IdUser','$flag')";
				mysql_query($sql);
			}
        }
...
WHERE b_pelayanan.kunjungan_id='$idKunj' AND b_ms_unit.inap=1 ORDER BY b_pelayanan.id LIMIT 1) ORDER BY tk.id";
    ...
    case 'save_catatan':
        ...
        if(mysql_num_rows($rsNote)>0){
            ...
            $sql="insert into b_catatan (kunjungan_id,jenis_rawat,note,flag) value ('$idKunj','$jnsRwt','$note','$flag') ";
        }        
        mysql_query($sql);        
