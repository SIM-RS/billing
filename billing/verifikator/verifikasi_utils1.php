...
if($act == 'verifikasi'){
    if($kamar==''){
        $uTin="update b_tindakan set verifikasi='$value' where id='$tindakan_id'";
        mysql_query($uTin);
    ...
    if($jnsRwt=='0'){
        ...
        OR p.unit_id IN (SELECT u.id FROM $dbbilling.b_ms_unit u WHERE u.inap=0 AND u.parent_id = 44)) AND p.kunjungan_id='$kunjId'";
        $rs=mysql_query($sql);
        $rw=mysql_fetch_array($rs);
        if($rw['total']>0){
            if($rw['belum']>0){
