...
if($_REQUEST['act']=='hapus'){
	$hps = "delete from b_return where no_return='".$_REQUEST['no_return']."'";
	mysql_query($hps);
}
...
while ($rows=mysql_fetch_array($rs)){
		...
		$id=$rows['id']."|".$rows['nama']."|".$alamat."|".tglSQL($rows['tgl_lahir'])."|".tglSQL($rows['tgl'])."|".$rows['umur_thn']."|".$rows['sex']."|".tglSQL($rows['tgl'])."|".$rows['kunjungan_id']."|";
		$i++;
		$dt.=$id.chr(3).$i.chr(3).tglSQL($rows["tgl_return"]).chr(3).$rows["no_return"].chr(3).tglSQL($rows["tgl"]).chr(3).$rows["unit"].chr(3).$rows["no_rm"].chr(3).$rows["nama"].chr(3).$rows["alamat"].chr(6);
}
