...
<?php 
	...
	$sql="select * from (select * from (SELECT mt.id, mt.unit_id, mt.kode, mt.nama, tk.ms_kelas_id, tk.tarip, tk.tarip_askes, tk.ms_tindakan_id, tk.id AS idtk,k.nama as kelas
...
WHERE mt.aktif = 1 AND tu.ms_unit_id='".$unitId."' ".$tambahan." AND (mt.nama LIKE '$aKeyword%' OR mt.kode LIKE '$aKeyword%') order by mt.nama) as t1
) as gab limit 200";
	//echo $sql;
	$rs=mysql_query($sql);
	//$arfvalue="";
	...
	{
		$i++;
		$arfvalue=$rows['idtk']."*|*".$rows['nama']."*|*".$rows["kode"]."*|*".$rows["tarip"]."*|*".$rows["id"]."*|*".$rows['kelas']."*|*".$rows['tarip_askes']."*|*".$rows['ms_kelas_id'];
?>
	<tr id="lstTind<?php echo $i; ?>" lang="<?php echo $arfvalue; ?>" class="itemtableReq" onMouseOver="this.className='itemtableMOverReq'" onMouseOut="this.className='itemtableReq'" onClick="fSetTindakan('<?php echo $id;?>',this.lang);" <?php if ($allKelas!="1"){?>title="Tekan (F12): menampilkan semua kelas<?php }?>">
