...
$inap=$_REQUEST['inap'];
$idDokTujuan=$_REQUEST['idDokTujuan'];
$isDokTujuanPengganti=$_REQUEST['isDokTujuanPengganti'];

/*if($inap==1){
	$que=", dokter_tujuan_id='$idDokTujuan', type_dokter_tujuan='$isDokTujuanPengganti'";
}
*/
$que=", dokter_tujuan_id='$idDokTujuan', type_dokter_tujuan='$isDokTujuanPengganti'";
//===============================
...
if($_REQUEST['act']=='save'){
	$sqlUpdate="update b_pelayanan set kso_id='$pKsoId',kelas_id='$pKelasId',tgl=".(($pTgl=='00-00-0000'||$pTgl==''||$pTgl=='--')?"(NULL)":"'".tglSQL($pTgl)."'").",tgl_krs=".(($pTglKRS=='00-00-0000'||$pTglKRS==''||$pTglKRS=='--')?"(NULL)":"'".tglSQL($pTglKRS)."'").",dilayani='$dilayani' $que,flag='$flag' where id='$pid'";
	if(mysql_query($sqlUpdate)){ 
		if($sCout==1)
		{
			$sqlUpdate2="update b_pelayanan set checkout=1, checkoutact=now(), checkoutuserid='".$usrIdN."',flag='$flag' where id='$pid'";
...
$sql="SELECT l.id,l.jenis_layanan,n.nama AS jenis,l.unit_id,u.nama AS unit,u.inap,l.unit_id_asal,m.nama AS asal,
	l.kso_id,l.kelas_id,l.tgl,l.tgl_krs,l.dilayani,l.dokter_id,l.no_lab,l.verifikasi,l.verifikator,u.penunjang,l.dokter_tujuan_id,IF(u.inap=1,IFNULL(p.nama,'-'),'-') AS namadokter,l.type_dokter_tujuan,l.batal,l.checkout
	FROM b_pelayanan l
	INNER JOIN b_ms_unit u ON u.id=l.unit_id
	INNER JOIN b_ms_unit n ON n.id=l.jenis_layanan
	INNER JOIN b_ms_unit m ON m.id=l.unit_id_asal
	left join b_ms_pegawai p on p.id=l.dokter_tujuan_id
	WHERE l.jenis_kunjungan='$jnsKunj' AND l.kunjungan_id='$idKunj'";
...
INNER JOIN b_ms_tindakan_kelas mtk ON t.ms_tindakan_kelas_id=mtk.id INNER JOIN b_ms_kelas mk ON mtk.ms_kelas_id=mk.id";
		...
		$id=$rows['id']."|".$rows['dilayani']."|".$rows['unit_id']."|".$rows['jenis_layanan']."|".$ckelasId."|".$rows['inap']."|".$rows['kso_id']."|".$rows['dokter_tujuan_id']."|".$rows['type_dokter_tujuan'];
		$i++;
		$dt.=$id.chr(3).$i.chr(3).$rows["jenis"].chr(3).$rows["unit"].chr(3).$rows["asal"].chr(3).$rwKKSO["nama"].chr(3).$ckelas.chr(3).tglSQL($rows["tgl"]).chr(3).tglSQL($rows["tgl_krs"]).chr(3).$rows["namadokter"].chr(3).$dilayani.chr(3).$proses.chr(6);
}
