...
<span class="jdltable"></span><br>
  <table width="99%" border="0" cellpadding="2" cellspacing="0" class="txtinput" style="display:none">
  	<tr>
  	...
  	<tr>
		<td>Buku Besar</td>
		<td>:</td>
		<td><select name="bulan" id="bulan" class="txtinput" onchange="location='?f=bukubesar_neraca.php&idma='+idma.value+'&kode_ma=<?php echo $kode_ma; ?>&ccrvpbfumum=<?php echo $ccrvpbfumum; ?>&idccrvpbfumum=<?php echo $idccrvpbfumum; ?>&islast=<?php echo $islast; ?>&ta='+ta.value+'&bulan='+bulan.value+'&detail=<?php echo $detail; ?>&cc_islast=<?php echo $cc_islast; ?>'">
            ...
            <select name="ta" id="ta" class="txtinput" onchange="location='?f=bukubesar_neraca.php&idma='+idma.value+'&kode_ma=<?php echo $kode_ma; ?>&ccrvpbfumum=<?php echo $ccrvpbfumum; ?>&idccrvpbfumum=<?php echo $idccrvpbfumum; ?>&islast=<?php echo $islast; ?>&ta='+ta.value+'&bulan='+bulan.value+'&detail=<?php echo $detail; ?>&cc_islast=<?php echo $cc_islast; ?>'">
  ...
  <table width="99%" border="0" cellpadding="2" cellspacing="0">
    <tr class="headtable"> 
	  ...
	  if ($kode_ma=="") $kode_ma=chr(9);
	  	if ($ccrvpbfumum=="0"){
	   		$sql="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma and month(tgl)=$bulan and year(tgl)=".$_GET[ta]." and j.flag = '$flag' order by TGL,TR_ID"; //decyber_jurnal
		}elseif ($ccrvpbfumum=="1"){
	   		$sql="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma AND j.CC_RV_ID='$idccrvpbfumum' and month(tgl)=$bulan and year(tgl)=".$_GET[ta]." and j.flag = '$flag' order by TGL,TR_ID"; //decyber_jurnal
		}else{
	   		$sql="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma AND j.CC_RV_KSO_PBF_UMUM_ID='$idccrvpbfumum' and month(tgl)=$bulan and year(tgl)=".$_GET[ta]." and j.flag = '$flag' order by TGL,TR_ID"; //decyber_jurnal
		}
	   //echo $sql;
	  	$rs=mysql_query($sql);
...
?>
	  ...
	  if ($kode_ma=="") $kode_ma=chr(9);
		...
		$stotk=0;
		if ($ccrvpbfumum=="0"){
			$sql="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal inner join ma_sak on FK_SAK=MA_ID where MA_KODE like '$kode_ma%' and month(TGL)=$bulan and year(TGL)=$ta and flag = '$flag'";
			// echo $sql."<br>".$flag; //decyber_jurnal
			$rs=mysql_query($sql);
			$rwsTot=mysql_fetch_array($rs);
			$stotd=$rwsTot["subTotD"];
			$stotk=$rwsTot["subTotK"];
	   		...
	   		switch($ccrvpbfumum){
				case 1:
					if (substr($kode_ma,0,1)=="4"){
						$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_ID=mu.id where MA_ID='$idma' AND mu.kode LIKE '$kode_cc%' AND LEFT(mu.kode,2)<>'08' and month(j.TGL)=$bulan and year(j.TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
						
						$sql="SELECT * FROM ak_ms_unit WHERE parent_id='$idccrvpbfumum' AND LEFT(kode,2)<>'08' ORDER BY kode";
					}else{
						$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_ID=mu.id where MA_ID='$idma' AND mu.kode LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
					...
					break;
				case 2:
					$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN $dbbilling.b_ms_kso kso ON j.CC_RV_KSO_PBF_UMUM_ID=kso.id where MA_ID='$idma' AND kso.kode_ak LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
					$rs=mysql_query($sql5);
					$rwsTot=mysql_fetch_array($rs);
					$stotd=$rwsTot["subTotD"];
					$stotk=$rwsTot["subTotK"];
				...
				case 3:
					if ($idccrvpbfumum==0){
						$sql="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal inner join ma_sak on FK_SAK=MA_ID where MA_ID='$idma' and month(TGL)=$bulan and year(TGL)=$ta and flag = '$flag'"; //decyber_jurnal
					}else{
...
WHERE ma.MA_ID='$idma' AND pbf.PBF_KODE_AK LIKE '$kode_cc%' AND MONTH(j.TGL)=$bulan AND YEAR(j.TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
					}
					//echo $sql."<br>";
					$rs=mysql_query($sql);
					$rwsTot=mysql_fetch_array($rs);
					...
					break;
				case 4:
					$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN $dbaset.as_ms_rekanan rek ON j.CC_RV_KSO_PBF_UMUM_ID=rek.idrekanan where MA_ID='$idma' AND rek.koderekanan LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
					$rs=mysql_query($sql5);
					$rwsTot=mysql_fetch_array($rs);
					$stotd=$rwsTot["subTotD"];
					$stotk=$rwsTot["subTotK"];
	  ...
	  $i=($page-1)*$perpage;
		...
		while ($rows=mysql_fetch_array($rs)){
			...
			if ($ccrvpbfumum=="0"){
				...
				$dma=$rows['MA_NAMA'];
				$cc_islast=0;
				$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT from jurnal inner join ma_sak on FK_SAK=MA_ID where MA_KODE like '$ckode%' and month(TGL)=$bulan and year(TGL)=$ta and flag = '$flag'"; //decyber_jurnal
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$cdebit=$rows1['DEBIT'];
				...
				switch($ccrvpbfumum){
					case 1:
						...
						$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT FROM jurnal j INNER JOIN ma_sak ON FK_SAK=MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_ID=mu.id where MA_ID='$idma' AND mu.kode LIKE '".$rows['kode']."%' and month(TGL)=$bulan and year(TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
						break;
					case 2:
						$idccrvpbfumum=$rows['id'];
						...
						$dma=$rows['nama'];
						$cc_islast=1;
						$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT FROM jurnal j INNER JOIN ma_sak ON FK_SAK=MA_ID where MA_ID='$idma' AND j.CC_RV_KSO_PBF_UMUM_ID='".$idccrvpbfumum."' and month(TGL)=$bulan and year(TGL)=$ta and flag = '$flag'"; //decyber_jurnal
						break;
					case 3:
						...
						$dma=$rows['PBF_NAMA'];
						$cc_islast=1;
						$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT FROM jurnal j INNER JOIN ma_sak ON FK_SAK=MA_ID where MA_ID='$idma' AND j.CC_RV_KSO_PBF_UMUM_ID='".$idccrvpbfumum."' and month(TGL)=$bulan and year(TGL)=$ta and j.flag = '$flag'"; //decyber_jurnal
						break;
					case 4:
	  ...
	  ?>
        <tr class="itemtable" onMouseOver="this.className='itemtableMOver'" onMouseOut="this.className='itemtable'"> 
          <td class="tdisikiri"><?php echo $i; ?></td>
          <td align="left" class="tdisi"><a href="?f=bukubesar_neraca&idma=<?php echo $cid; ?>&ccrvpbfumum=<?php echo $tccrvpbfumum; ?>&idccrvpbfumum=<?php echo $idccrvpbfumum; ?>&cc_islast=<?php echo $cc_islast; ?>&kode_ma=<?php echo $ckode; ?>&bulan=<?php echo $bulan; ?>&ta=<?php echo $ta; ?>&islast=<?php echo $cislast; ?>&detail=true"><?php echo $dkode; ?></a></td>
          <td align="left" class="tdisi"><a href="?f=bukubesar_neraca&idma=<?php echo $cid; ?>&ccrvpbfumum=<?php echo $tccrvpbfumum; ?>&idccrvpbfumum=<?php echo $idccrvpbfumum; ?>&cc_islast=<?php echo $cc_islast; ?>&kode_ma=<?php echo $ckode; ?>&bulan=<?php echo $bulan; ?>&ta=<?php echo $ta; ?>&islast=<?php echo $cislast; ?>&detail=true"><?php echo $dma; ?></a></td>
          <td class="tdisi" align="right"><?php echo number_format($cdebit,2,",","."); ?></td>
          <td class="tdisi" align="right"><?php echo number_format($ckredit,2,",","."); ?></td>
        </tr>
        <? 
