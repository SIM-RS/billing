...
<?php	
  ...
  while ($rows=mysql_fetch_array($rs)){
  	  if ($rows["CC_RV_KSO_PBF_UMUM"]==0){
		 ...
		 if ($c_islast==1){
			$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['MA_ID']."*|*".$par[1]."*-*".$rows['MA_KODE']."*|*".$par[2]."*-*".$rows['MA_NAMA']."*|*".$par[3]."*-*0');window.close();";
 		 }else
		 	 $tree[$cnt][2] = null;
		 ...
		 $cnt++;
			switch ($rows["CC_RV_KSO_PBF_UMUM"]){
				case 1:
					if (substr($rows["MA_KODE"],0,1)=="4"){
					 ...
					 while ($rows1=mysql_fetch_array($rs1)){
						 ...
						 if ($c_islast==1){
							$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['MA_ID']."*|*".$par[1]."*-*".$rows['MA_KODE'].$mpkode."*|*".$par[2]."*-*".$rows1['nama']."*|*".$par[3]."*-*".$rows1['id']."');window.close();";
						 }else
							 $tree[$cnt][2] = null;
				...
				case 2:
					 ...
					 $sql="SELECT * FROM $dbbilling.b_ms_kso WHERE aktif=1 AND id>1".$ckso_filter." ORDER BY kode_ak";
					 $rs1 = mysql_query($sql);
					 while ($rows1=mysql_fetch_array($rs1)){
						 $c_level = $s_level + 1;
						 $c_islast = 1;
				...
				case 3:
					 ...
					 while ($rows1=mysql_fetch_array($rs1)){
						 ...
						 if ($c_islast==1){
							$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['MA_ID']."*|*".$par[1]."*-*".$rows['MA_KODE'].$mpkode."*|*".$par[2]."*-*".$rows1['PBF_NAMA']."*|*".$par[3]."*-*".$rows1['PBF_ID']."');window.close();";
						 }else
							 $tree[$cnt][2] = null;
				...
				case 4:
					 ...
					 while ($rows1=mysql_fetch_array($rs1)){
						 ...
						 if ($c_islast==1){
							$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['MA_ID']."*|*".$par[1]."*-*".$rows['MA_KODE'].$mpkode."*|*".$par[2]."*-*".$rows1['namarekanan']."*|*".$par[3]."*-*".$rows1['idrekanan']."');window.close();";
						 }else
							 $tree[$cnt][2] = null;
