			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_affected_rows()>0){
					...
					//$idpost=gmdate('YmdHis',mktime(date('H')+7)).rand(1,1000);
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,TIPE_JURNAL,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
		VALUES('$idpost',$notrans,1,'$pymad','$tanggal','$nokw','$uraian','$npymad',0,now(),$idUser,'D',3,0,$jenistrans,$jenistrans,1,'',$kso_id,'$unit_id_billing',1)";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
					...
					while ($rwTrans=mysql_fetch_array($rsTrans)){
						...
						if ($unit_id_billing>0){
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,TIPE_JURNAL,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
		VALUES('$idpost',$notrans,$no_psg,'$ma_pend','$tanggal','$nokw','$uraian',0,$nilaiPend,now(),$idUser,'K',3,0,$jenistrans,$jenistrans,1,'','$unit_id_billing','$unit_id_billing',1)";
							//echo $sql.";<br>";
							$rsPost=mysql_query($sql);
							/*if ($biayaKSO>0){
			...
			VALUES($id_k_trans,$notrans,$no_psg,$cfksak_bpasien,'$tanggal','$nokw','$uraian',$biayaPx,0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$kso_id,$idunit,1)";
							...
							if ($cisFarmasi==3){
								//============ PPN =============
								$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,TIPE_JURNAL,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
			VALUES('$idpost',$notrans,$no_psg,'$akunppn','$tanggal','$nokw','$uraian',0,$biayaKSO,now(),$idUser,'K',3,0,$jenistrans,$jenistrans,1,'','0','0',1)";
								//echo $sql.";<br>";
								$rsPost=mysql_query($sql);
							}else{
								//============ UTIP =============
								$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,TIPE_JURNAL,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
			VALUES('$idpost',$notrans,$no_psg,'$akun_utip','$tanggal','$nokw','$uraian',0,$biayaKSO,now(),$idUser,'K',3,0,$jenistrans,$jenistrans,1,'','0','0',1)";
								//echo $sql.";<br>";
								$rsPost=mysql_query($sql);
							}
						}
	...
	switch ($grd){
		...
		case "pendBilling":
			...
			$sql = "SELECT
					...
					WHERE kp.tglP = '$tanggal' AND kp.tipe=0 /*$fkso*/ $fposting $filter
