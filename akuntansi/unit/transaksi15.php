	...
	case "edit":
		...
		//Maka Lakukan replace nilai data baru..
			for ($i = 0; $i < count($nilai); $i++){
				...
				if (($fk_TipeTrans==0) || ($fk_TipeTrans=="0")){
					$sql2="update JURNAL set fk_sak=$id_sak[$i],tgl='$tgl',no_kw='$nokw',uraian='$uraian',debit=$nilai1,kredit=$nilai2,tgl_act='$sekarang',fk_iduser=$iduser,d_k='$dk[$i]',CC_RV_KSO_PBF_UMUM_ID='$id_CC[$i]' WHERE tr_id=".$id_tr[$i];
					//echo $sql2."<br>";
					$rs=mysql_query($sql2);
				}else{
					if($nilai1>0 OR $nilai2>0){
						...
						if ($rsrow=mysql_fetch_array($rs)){
							...
							$sql2="update JURNAL set no_kw='$nokw',tgl='$tgl',uraian='$uraian',debit=$nilai1,kredit=$nilai2,tgl_act='$sekarang',fk_iduser=$iduser,d_k='$dk[$i]' WHERE fk_sak=$id_sak[$i] and FK_TRANS=$fk_jnsTrans and NO_TRANS=$tnotrans and NO_KW='$nokw_lama' AND CC_RV_KSO_PBF_UMUM_ID='$id_CC[$i]'";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
							$sql2="update JURNAL set no_kw='$nokw',tgl='$tgl',uraian='$uraian',debit=$nilai2,kredit=$nilai1,tgl_act='$sekarang',fk_iduser=$iduser,d_k='$dk_out' WHERE fk_sak=$fk_IdRek and FK_TRANS=$fk_jnsTrans and NO_TRANS=$tnotrans and NO_KW='$nokw_lama' AND FK_LAST_TRANS='$tlasttrans'";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
						}else{
							$tnotrans=$tmaxnotrans;
							$tmaxnotrans++;
							$sql2="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID) values($tnotrans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$fkBebanJenis[$i]','$id_CC[$i]')";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
							$sql2="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS) values($tnotrans,$fk_IdRek,'$tgl','$nokw','$uraian',$nilai2,$nilai1,'$sekarang',$iduser,'$dk_out',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i])";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
						}
					}
		...
		if ($cdata=="1"){
			...
			for ($i = 0; $i < count($nilai); $i++){
				...
				if($nilai1>0 OR $nilai2>0){
					...
					if ($fk_TipeTrans!=0){
						$sql="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS) values($no_trans,$fk_IdRek,'$tgl','$nokw','$uraian',$nilai2,$nilai1,'$sekarang',$iduser,'$dk_out',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i])";
						//echo $sql."<br>";
						$rs=mysql_query($sql);
						//echo mysql_error()."<br>";					
					}
					$sql="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,KSO_ID,PBF_ID,VENDOR_ID) values($no_trans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$fkBebanJenis[$i]',$id_CC[$i],'$cc_rv_id[$i]',$ksoid,$pbfid,$vendorid)";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
					//echo mysql_error()."<br>";
				}
...
<div id="input" style="display:block;">
    <table width="980" border="0" cellspacing="0" cellpadding="0" class="txtinput">
      ...
      <tr> 
        ...
        <td>:</td>
        <td>
          <? if($show1['TIPE']==0) echo $show1['nama_tipe']; else echo $show1['nama_tipe']." - Pada Kode Rekening: ".$show1['korek']." - ".$show1['MA_NAMA'];?>
        </td>
      </tr>
      <tr> 
        <?php
	...
	?>
        ...
        <td><input name="tgl" type="text" id="tgl" size="11" maxlength="10" readonly="true" value="<?php echo $tskrg; ?>" class="txtcenter" /> 
          <input type="button" name="Button2" value=" V " class="txtcenter" onClick="gfPop.fPopCalendar(this.form.tgl,depRange);" /> 
        </td>
      </tr>
      <tr> 
        <td>&nbsp;</td>
	...
	?>
		...
		while ($show3=mysql_fetch_array ($exe3)){
            ...
            <tr class="itemtable"> 
              ...
              <td class="tdisi"><?php echo $show['namaBebanJenis']; ?>&nbsp;
              </td>
              <td class="tdisi"> 
                <?php if ($show['dk']=='D'){ echo "Debet";}elseif ($show['dk']=='K'){ echo "Kredit";}?></td>
              <?php 
			  if ($fk_TipeTrans==0){
			  	if ($show['dk']=='D'){
...
<div id="listjurnal" style="margin-top:15px"><br>
    <table width="99%" border="0" cellpadding="2" cellspacing="0">
      ...
      <tr class="itemtable" onMouseOver="this.className='itemtableMOver'" onMouseOut="this.className='itemtable'">
        ...
        <td class="tdisi"><span id="idNoTrans<?php echo $rows['TR_ID']; ?>"<?php echo $StyleEvSpan; ?> title="Klik Untuk Mencetak Bukti Jurnal" style="cursor:pointer;text-decoration:underline;color:#0000FF" onClick="spnNoTransClick('<?php echo $rows['NO_TRANS']."|".$rows['NO_KW']; ?>');"><?php echo $rows['NO_TRANS']; ?></span></td>
...
<script>
	...
	function spnNoTransClick(noTrans_noKw){
		//alert(noTrans_noKw);
