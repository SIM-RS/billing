...
switch ($act){
				...
				if (($fk_TipeTrans==0) || ($fk_TipeTrans=="0")){
					...
					if($nilai1>0 OR $nilai2>0){
						...
						if ($rsrow=mysql_fetch_array($rs)){
							...
							$tmaxnotrans++;
							$sql2="insert into JURNAL_SEM(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,KEPADA,ALAMAT,flag,move) values($tnotrans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$fkBebanJenis[$i]','$id_CC[$i]','$kepada','$alamat','$flag',0)"; //decyber_jurnal
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
							$sql2="insert into JURNAL_SEM_SEM(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,KEPADA,ALAMAT,flag,move) values($tnotrans,$fk_IdRek,'$tgl','$nokw','$uraian',$nilai2,$nilai1,'$sekarang',$iduser,'$dk_out',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$kepada','$alamat','$flag',0)";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
						}
					}
	...
	case "save":
		...
		if ($cdata=="1"){
			...
			//perhitungan no trans *decyber
			$sql1="SELECT IFNULL(MAX(no_trans)+1,1) AS no_trans FROM jurnal_sem";
			$rs1=mysql_query($sql1);
			$rs=mysql_query($sql);
			$no_trans=1;
			if ($rows=mysql_fetch_array($rs)) $cek=$rows["no_trans"];
			if ($rows1=mysql_fetch_array($rs1)) $cek1=$rows1["no_trans"];
			...
			for ($i = 0; $i < count($nilai); $i++){
				...
				if($nilai1>0 OR $nilai2>0){
					...
					if ($fk_TipeTrans!=0){
						$sql="insert into JURNAL_SEM(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,KEPADA,ALAMAT,TIPE_JURNAL,flag,move) values($no_trans,$fk_IdRek,'$tgl','$nokw','$uraian',$nilai2,$nilai1,'$sekarang',$iduser,'$dk_out',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$kepada','$alamat','$fk_TipeJurnal','$flag',0)"; //decyber_jurnal
						//echo $sql."<br>";
						$rs=mysql_query($sql);
						//echo mysql_error()."<br>";					
					}
					$sql="insert into JURNAL_SEM(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,KSO_ID,PBF_ID,VENDOR_ID,KEPADA,ALAMAT,TIPE_JURNAL,flag,move) values($no_trans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$fkBebanJenis[$i]',$id_CC[$i],'$cc_rv_id[$i]',$ksoid,$pbfid,$vendorid,'$kepada','$alamat','$fk_TipeJurnal','$flag',0)"; //decyber jurnal
					//echo $sql."<br>";
					$rs=mysql_query($sql);
					//echo mysql_error()."<br>";
				}
...
<div id="input" style="display:block;">
    <table width="980" border="0" cellspacing="0" cellpadding="0" class="txtinput">
      ...
      <tr> 
		...
		$sql="SELECT * FROM jurnal_sem WHERE NO_TRANS=$notrans AND NO_KW='$nokw' AND FK_LAST_TRANS=$pid_jnsTrans";
		//echo $sql."<br>";
		$rs=mysql_query($sql);
      ...
      <tr> 
        <?php
		if ($arfvalue=='edit'){
			$sql2="SELECT * FROM jurnal_sem WHERE FK_TRANS=".$fk_jnsTrans." AND NO_TRANS=".$notrans;
			//echo $sql2;
	...
	?>
	  ...
	  $sql="SELECT *, ma_sak.MA_KODE AS MA_SAK_KODE, ma_sak.MA_NAMA AS MA_SAK_NAMA, ma_sak.CC_RV_KSO_PBF_UMUM, jenis_transaksi.JTRANS_ID AS Expr5, detil_transaksi.fk_ma_sak AS Expr6, detil_transaksi.id_detil_trans AS Expr8,ak_ms_beban_jenis.kode as kodeBebanJenis,ak_ms_beban_jenis.nama as namaBebanJenis
		...
		if (($arfvalue=='edit') && ($jml!=$jmle)){
			if ($fk_TipeTrans==0)
			  $sql="SELECT *,b.CC_RV_KSO_PBF_UMUM cc_rv_kso_pbf_umum,a.CC_RV_KSO_PBF_UMUM_ID id_cc_rv_kso_pbf_umum,MA_KODE AS MA_SAK_KODE,MA_NAMA AS MA_SAK_NAMA,FK_TRANS AS Expr5,MA_ID AS Expr6,D_K AS dk FROM jurnal_sem a INNER JOIN ma_sak b ON a.FK_SAK=b.MA_ID WHERE a.NO_TRANS=$notrans AND a.FK_LAST_TRANS=$pid_jnsTrans AND NO_KW='$nokw' ORDER BY TR_ID";
		}
	// echo $sql."<br>";//decyber
	$exe= mysql_query ($sql);
	...
	?>
		...
		if ($fk_TipeTrans==0)
			$sql3="SELECT * FROM jurnal_sem a INNER JOIN ma_sak b ON a.FK_SAK=b.MA_ID WHERE a.NO_TRANS=$notrans AND a.FK_LAST_TRANS=$pid_jnsTrans ORDER BY TR_ID";
		else
			$sql3="SELECT * FROM jurnal_sem a INNER JOIN ma_sak b ON a.FK_SAK=b.MA_ID WHERE a.NO_TRANS=$notrans AND a.FK_TRANS=$fk_jnsTrans  AND a.FK_LAST_TRANS=$pid_jnsTrans ORDER BY TR_ID";
		//echo $sql3."<br>";
		...
		while ($show3=mysql_fetch_array ($exe3)){
            ...
            <tr class="itemtable"> 
              <?php 
					...
					$kodeBebanJenis="";
					if ($id_Bjenis>0){
						$kodeBebanJenis=" .".$show['kodeBebanJenis']."";
					}
              ...
              <?php 
			  	...
			  	$ndk="";
			  	if($arfvalue=="edit"){
			  		$strsql="SELECT * FROM jurnal_sem WHERE FK_SAK='$idsak' AND NO_KW='$tnokw' AND FK_TRANS=$fk_jnsTrans AND CC_RV_KSO_PBF_UMUM_ID='$idCC'";
					//echo $strsql."<br>";
      ...
      <td height="40" colspan="5" align="center" valign="bottom"><BUTTON type="button" onClick="if (ValidateForm('fk_jnsTrans,tgl,nokw,uraian','ind')){document.form1.submit();}"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Simpan</BUTTON>	<BUTTON type="button" onClick="if (ValidateForm('fk_jnsTrans,tgl,nokw,uraian','ind')){document.form1.submit();}"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Cetak</BUTTON>&nbsp;<BUTTON type="reset" onClick="location='?f=transaksi&arfvalue=0&notrans=0&tgl=<?php echo $th5; ?>&nokw= &uraian= &'"><IMG SRC="../icon/cancel.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;<strong>Reset</strong>&nbsp;&nbsp;&nbsp;&nbsp;</BUTTON></td>
      </tr>
    </table>
</div>
...
<div id="listjurnal" style="margin-top:15px"><br>
    <table width="99%" border="0" cellpadding="2" cellspacing="0">
	  ...
	  $sqlQ="Select jurnal.*, ma_sak.MA_KODE, ma_sak.MA_KODE_KP, ma_sak.MA_NAMA, ma_sak.FK_MA, ma_sak.CC_RV_KSO_PBF_UMUM From jurnal Inner Join ma_sak ON jurnal.FK_SAK = ma_sak.MA_ID where TGL between '$tgl_1' and '$tgl_2' AND VERIFIKASI=1 ".$fuser.$fver.$filter." AND flag = '$flag' order by ".$sorting;
	  //echo $sqlQ."<br>";
		$rs=mysql_query($sqlQ);
      ...
      <tr class="itemtable" onMouseOver="this.className='itemtableMOver'" onMouseOut="this.className='itemtable'">
		...
		?>
		<!-- decyber, edit dihilangkan, dan hapus
...
<script>
	...
	function spnNoTransClick(noTrans_noKw,tipeJ){
		...
		switch(tipeJ){
			case 0:
			case 1:
				window.open("../report/voucher_penerimaan_print.php?notrans_kw="+noTrans_noKw);
				break;
			case 2:
				window.open("../report/voucher_pengeluaran_print.php?notrans_kw="+noTrans_noKw);
				break;
			case 3:
			...
			case 4:
				...
				break;
			case 5:
				window.open("../report/voucher_pembelian_print.php?notrans_kw="+noTrans_noKw);
				break;
		}
	}
