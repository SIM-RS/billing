	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Return Pelayanan</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td style="padding-right:20px; text-align:right;" height="25">Jenis Layanan</td>
...
<script>
	...
	a1.setHeader("DATA RETURN PELAYANAN");
	a1.setColHeader("NO,UNIT PELAYANAN,KSO,NILAI,PILIH<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	//a1.setIDColHeader(",nama,,");
	a1.setColWidth("50,200,200,100,75");
	a1.setCellAlign("center,left,left,right,center");
	a1.setCellType("txt,txt,txt,txt,chk");
	a1.setCellHeight(20);
	a1.setImgPath("../icon");
	a1.setIDPaging("paging");
	a1.onLoaded(konfirmasi);
	//a.attachEvent("onRowClick");
	//alert("../unit/return_pelayanan_utils.php?grd=ReturnPelayanan&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value);
	a1.baseURL("../unit/return_pelayanan_utils.php?grd=ReturnPelayanan&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value);
	a1.Init();
	...
	function goFilterAndSort(grd){
		...
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/return_pelayanan_utils.php?grd=ReturnPelayanan&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	var cekbox=(p==true)?1:0;
		...
		for (var i=0;i<a1.getMaxRow();i++){
			a1.cellsSetValue(i+1,5,cekbox);
		}
	}
	
	function kirim(){
	var url;
		url="../unit/return_pelayanan_utils.php?grd=ReturnPelayanan&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var tmp2='';
	var url;
		url="../unit/return_pelayanan_utils.php?grd=ReturnPelayanan&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[4].childNodes[0].checked){
		...
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingReturnPelayanan&fdata="+tmp;
				//alert(url);
				//alert(tmp2);
				a1.loadURL(url,"","GET");
			}
