	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Pengurang Pendapatan <?php echo $captionbyr; ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td height="30" align="right" style="padding-right:45px;"><BUTTON type="button" id="btnPosting" onClick="PostingJurnal();"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal</BUTTON></td>
                </tr>
                <tr>
...
<script>
	...
	a1.setHeader("DATA PENDAPATAN BILLING");
	a1.setColHeader("NO,UNIT PELAYANAN,TARIP PERDA,BIAYA PASIEN,TARIP KSO,IUR BAYAR,SELISIH,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",nama,,,,,,");
	a1.setColWidth("50,200,100,100,100,100,100,75");
	a1.setCellAlign("center,left,right,right,right,right,right,center");
	...
	a1.baseURL("../unit/utils.php?grd=PengurangPendBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value)+"&posting="+document.getElementById('cmbPost').value+"&bayar=<?php echo $bayar; ?>";
	a1.Init();
	...
	function konfirmasi(key,val){
		...
		if(key=='Error'){
			if(val=='postingPengurangBilling'){
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/utils.php?grd=PengurangPendBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&bayar=<?php echo $bayar; ?>"+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function kirim(){
	var url;
		url="../unit/utils.php?grd=PengurangPendBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&bayar=<?php echo $bayar; ?>";
		a1.loadURL(url,"","GET");
	}
	...
	var tmp='',tmp2='';
	var url;
		url="../unit/utils.php?grd=PengurangPendBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&bayar=<?php echo $bayar; ?>"+"&idUser=<?php echo $iduser; ?>";
		...
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[7].childNodes[0].checked){
				...
				tmp2+=a1.getRowId(i+1)+"|"+ValidasiText(a1.cellsGetValue(i+1,3))+"|"+ValidasiText(a1.cellsGetValue(i+1,4))+"|"+ValidasiText(a1.cellsGetValue(i+1,5))+"|"+ValidasiText(a1.cellsGetValue(i+1,6));
			}
		}
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingPengurangBilling&fdata="+tmp;
				//alert(url);
				//alert(tmp2);
				a1.loadURL(url,"","GET");
			}
