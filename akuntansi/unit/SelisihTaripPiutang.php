	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Selisih Tarip Piutang</td>
                </tr>
                <tr id="trKso">
                    ...
                    <td width="60%">:&nbsp;<select id="cmbKso" name="cmbKso" class="txtinput" onChange="kirim()">
                        <?php
                            $qkso = mysql_query("SELECT $dbbilling.b_ms_kso.id, $dbbilling.b_ms_kso.nama FROM $dbbilling.b_ms_kso WHERE aktif=1 AND id<>1");
                            $flag=0;
...
<script>
	...
	a1.setHeader("SELISIH TARIP PIUTANG");
	a1.setColHeader("NO,TANGGAL KUNJUNGAN,TANGGAL PULANG,NO RM,NAMA,KSO,KUNJUNGAN AWAL,TARIF REGULER,BIAYA PASIEN,BIAYA KSO (SIMRS),BIAYA KSO (VERIF KLAIM),SELISIH TARIP,VERIFIKASI<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	...
	a1.setColWidth("30,70,70,60,170,100,140,70,70,90,95,70,50");
	...
	a1.baseURL("../unit/SelisihTaripPiutang_utils.php?grd=selisihPiutang&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/SelisihTaripPiutang_utils.php?grd=selisihPiutang&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	var url;
		document.getElementById('btnPosting').disabled=true;
		url="../unit/SelisihTaripPiutang_utils.php?grd=selisihPiutang&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var url;
		document.getElementById('btnPosting').disabled=true;
		url="../unit/SelisihTaripPiutang_utils.php?grd=selisihPiutang&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		...
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingSelisihPiutang&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}else{
