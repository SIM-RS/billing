  	  ...
  	  if ($rows["CC_RV_KSO_PBF_UMUM"]==0){
		 ...
		 $cnt++;
			switch ($rows["CC_RV_KSO_PBF_UMUM"]){
				case 1:
					if (substr($rows["MA_KODE"],0,3)=="810"){
						$sql="SELECT * FROM ak_ms_unit WHERE tipe=2 AND aktif=1 ORDER BY kode";
					}else{
				...
				case 4:
					 ...
					 while ($rows1=mysql_fetch_array($rs1)){
						 ...
						 if ($c_islast==1){
							$tree[$cnt][2]= "javascript:fSetValueArray(window.opener,'".$par[0]."*-*".$rows['MA_ID']."*|*".$par[1]."*-*".$rows['MA_KODE'].$mpkode."*|*".$par[2]."*-*".$rows1['namarekanan']."*|*".$par[3]."*-*".$rows1['idrekanan']."',$noidx);window.close();";
						 }else
							 $tree[$cnt][2] = null;
