  ...
  <table width="99%" border="0" cellpadding="2" cellspacing="0">
    <tr class="headtable"> 
	  ...
	  if ($kode_ma=="") $kode_ma=chr(9);
	  	if ($ccrvpbfumum=="0"){
	   		$sql="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma AND j.flag=$flag and month(tgl)=$bulan and year(tgl)=".$_GET[ta]." order by TGL,TR_ID";
		}elseif ($ccrvpbfumum=="1"){
	   		$sql="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma AND j.flag=$flag AND j.CC_RV_ID='$idccrvpbfumum' and month(tgl)=$bulan and year(tgl)=".$_GET[ta]." order by TGL,TR_ID";
		}else{
	   		$sql="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma AND j.flag=$flag AND j.CC_RV_KSO_PBF_UMUM_ID='$idccrvpbfumum' and month(tgl)=$bulan and year(tgl)=".$_GET[ta]." order by TGL,TR_ID";
		}
	   // echo $sql;
	  	$rs=mysql_query($sql);
...
?>
	  ...
	  if ($kode_ma=="") $kode_ma=chr(9);
		...
		$stotk=0;
		if ($ccrvpbfumum=="0"){
			$sql="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal inner join ma_sak on FK_SAK=MA_ID where MA_KODE like '$kode_ma%' AND jurnal.flag=$flag and month(TGL)=$bulan and year(TGL)=$ta";
			// echo $sql."<br>";
			$rs=mysql_query($sql);
			$rwsTot=mysql_fetch_array($rs);
			$stotd=$rwsTot["subTotD"];
	   		...
	   		switch($ccrvpbfumum){
				case 1:
					if (substr($kode_ma,0,1)=="4"){
						$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_ID=mu.id where MA_ID='$idma'AND j.flag=$flag AND mu.kode LIKE '$kode_cc%' AND LEFT(mu.kode,2)<>'08' and month(j.TGL)=$bulan and year(j.TGL)=$ta";
						
						$sql="SELECT * FROM ak_ms_unit WHERE parent_id='$idccrvpbfumum' AND LEFT(kode,2)<>'08' ORDER BY kode";
					}else{
						$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_ID=mu.id where MA_ID='$idma' AND j.flag=$flag AND mu.kode LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta";
					...
					break;
				case 2:
					$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN $dbbilling.b_ms_kso kso ON j.CC_RV_KSO_PBF_UMUM_ID=kso.id where MA_ID='$idma' AND j.flag=$flag AND kso.kode_ak LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta";
					$rs=mysql_query($sql5);
					$rwsTot=mysql_fetch_array($rs);
					$stotd=$rwsTot["subTotD"];
					$stotk=$rwsTot["subTotK"];
				...
				case 3:
					if ($idccrvpbfumum==0){
						$sql="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal inner join ma_sak on FK_SAK=MA_ID where MA_ID='$idma' AND j.flag=$flag and month(TGL)=$bulan and year(TGL)=$ta";
					}else{
...
WHERE ma.MA_ID='$idma' AND j.flag=$flag AND pbf.PBF_KODE_AK LIKE '$kode_cc%' AND MONTH(j.TGL)=$bulan AND YEAR(j.TGL)=$ta";
					}
					//echo $sql."<br>";
					$rs=mysql_query($sql);
					$rwsTot=mysql_fetch_array($rs);
	  ...
	  $i=($page-1)*$perpage;
		...
		while ($rows=mysql_fetch_array($rs)){
			...
			if ($ccrvpbfumum=="0"){
				...
				$dma=$rows['MA_NAMA'];
				$cc_islast=0;
				$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT from jurnal inner join ma_sak on FK_SAK=MA_ID where MA_KODE like '$ckode%' AND flag=$flag and month(TGL)=$bulan and year(TGL)=$ta";
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$cdebit=$rows1['DEBIT'];
				...
				switch($ccrvpbfumum){
					case 1:
						...
						$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT FROM jurnal j INNER JOIN ma_sak ON FK_SAK=MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_ID=mu.id where MA_ID='$idma' AND j.flag=$flag AND mu.kode LIKE '".$rows['kode']."%' and month(TGL)=$bulan and year(TGL)=$ta";
						break;
					case 2:
						$idccrvpbfumum=$rows['id'];
						...
						$dma=$rows['nama'];
						$cc_islast=1;
						$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT FROM jurnal j INNER JOIN ma_sak ON FK_SAK=MA_ID where MA_ID='$idma' AND j.flag=$flag AND j.CC_RV_KSO_PBF_UMUM_ID='".$idccrvpbfumum."' and month(TGL)=$bulan and year(TGL)=$ta";
						break;
					case 3:
