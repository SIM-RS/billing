	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Pendapatan / Piutang Resep Farmasi</td>
                </tr>
                <tr id="trKso">
                ...
                <tr>
                    <td style="padding-right:20px; text-align:right;" height="25">Tanggal KRS</td>
                ...
                <tr>
                    ...
                    <td height="30" align="right" style="padding-right:5px;"><BUTTON type="button" id="btnPosting" onClick="PostingJurnal();"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal</BUTTON></td>
                </tr>
                <tr>
                    <td colspan="2" align="center">
                        <div id="gridbox" style="width:980px; height:340px; background-color:white; overflow:hidden;"></div>
...
<script>
	...
	a1.setHeader("DATA PENDAPATAN / PIUTANG PELAYANAN RESEP");
	a1.setColHeader("NO,TANGGAL KUNJUNGAN,TANGGAL PULANG,NO RM,NAMA,KSO,KUNJUNGAN AWAL,TARIF PERDA,BIAYA PASIEN,BIAYA KSO,BAYAR PASIEN,PIUTANG PASIEN,VERIFIKASI<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setCellType("txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,chk");
	a1.setSubTotal(",,,,,,SubTotal :&nbsp;,0,0,0,0,0,");
	a1.setIDColHeader(",,,pas.no_rm,pas.nama,kso.nama,mu.nama,,,,,,");
	a1.setColWidth("30,70,70,60,170,100,140,70,70,70,70,70,50");
	a1.setCellAlign("center,center,center,center,left,center,left,right,right,right,right,right,center");
	a1.setSubTotalAlign("center,center,center,center,center,center,right,right,right,right,right,right,center");
	a1.setCellHeight(20);
	a1.setImgPath("../icon");
	a1.setIDPaging("paging");
	a1.onLoaded(konfirmasi);
	//a.attachEvent("onRowClick");
	a1.baseURL("../unit/pendapatan_piutangFarmasi_utils.php?grd=pend_piutangFarmasi&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value);
	a1.Init();
	...
	function konfirmasi(key,val){
		...
		if (val!=undefined){
			...
			a1.cellSubTotalSetValue(8,tmp36[0]);
			a1.cellSubTotalSetValue(9,tmp36[1]);
			a1.cellSubTotalSetValue(10,tmp36[2]);
			a1.cellSubTotalSetValue(11,tmp36[3]);
			a1.cellSubTotalSetValue(12,tmp36[4]);
			if (tmp36[5]!=""){
				alert(tmp36[5]);
			}
			/* if(key=='Error'){
			...
			a1.cellSubTotalSetValue(10,0);
			a1.cellSubTotalSetValue(11,0);
			a1.cellSubTotalSetValue(12,0);
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/pendapatan_piutangFarmasi_utils.php?grd=pend_piutangFarmasi&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function chkKlik(p){
		var cekbox=(p==true)?1:0;
			...
			for (var i=0;i<a1.getMaxRow();i++){
				a1.cellsSetValue(i+1,13,cekbox);
			}
	}
	...
	var url;
		document.getElementById('btnPosting').disabled=true;
		url="../unit/pendapatan_piutangFarmasi_utils.php?grd=pend_piutangFarmasi&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var url;
		document.getElementById('btnPosting').disabled=true;
		url="../unit/pendapatan_piutangFarmasi_utils.php?grd=pend_piutangFarmasi&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[12].childNodes[0].checked){
		...
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingPiutangFarmasi&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}else{
