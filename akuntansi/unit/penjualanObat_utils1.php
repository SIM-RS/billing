	...
	case "postingPenjObat":
		...
		if ($posting==0){
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$sql="SELECT nama FROM $dbbilling.b_ms_kso kso WHERE kso.id='$ckso_id_billing'";
				$rsPost=mysql_query($sql);
				$rwPost=mysql_fetch_array($rsPost);
				$nKsoBilling=$rwPost["nama"];
				
				$uraian="Pemakaian Persediaan Obat (Resep) : ".$unitFarm." - ".$nKsoBilling." - ".$nUnitBilling;
				...
				if (mysql_errno()==0 && mysql_affected_rows()>0){
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_AK_POSTING,FK_ID_POSTING,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
	VALUES($id_ak_posting,$id_transaksi,$notrans,$no_psg,$id_sak_biaya,'$tglP','$nokw','$uraian','$nilaiHPP',0,now(),$idUser,'D',0,$jenistrans_jual,$jenistrans_jual,1,'$cno_bukti',$ckso_id_billing,'$id_cc_rv',1)";
					//echo $sql.";<br>";
					$rsPost=mysql_query($sql);
					...
					if (mysql_errno()==0){
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_AK_POSTING,FK_ID_POSTING,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
		VALUES($id_ak_posting,$id_transaksi,$notrans,$no_psg,$id_sak_stok,'$tglP','$nokw','$uraian',0,'$nilaiHPP',now(),$idUser,'K',0,$jenistrans_jual,$jenistrans_jual,1,'$cno_bukti','$ckso_id_billing','$cmbFarmasi',1)";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
						...
						//=====Berhasil -> Update dbapotek.a_penjualan====
							$sql="UPDATE $dbapotek.a_penjualan SET POSTING=1,NO_POST='$id_transaksi' 
								WHERE UNIT_ID='$cmbFarmasi' AND KSO_ID='$ckso_id_farmasi' AND TGL='$tglP' 
									AND RUANGAN='$cruangan_farmasi' AND POSTING=0";
							//echo $sql.";<br>";
							$rsPost=mysql_query($sql);
							if (mysql_errno()>0){
								$statusProses='Error';
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_num_rows($rsDel)>0){//======berhasil===========
					...
					if (mysql_affected_rows()>0){
						$sDel = "DELETE FROM jurnal WHERE FK_ID_AK_POSTING=$cid_ak_posting";
						//echo $sDel."<br>";
						$rsDel=mysql_query($sDel);
						if (mysql_errno()==0 && mysql_affected_rows()>0){
							$sDel = "DELETE FROM ak_posting WHERE id=$cid_ak_posting";
							//echo $sDel."<br>";
							$rsDel=mysql_query($sDel);
						}else{					
		...
		case "penjualanObat":
			...
			if ($posting==0){
				$sql="SELECT * FROM (SELECT
					  ap.KSO_ID,
					  ap.RUANGAN,
					  IFNULL(SUM(ap.QTY_JUAL*ap.HARGA_SATUAN),0) AS hJual,
					...
					GROUP BY ap.KSO_ID,ap.RUANGAN) AS t".$filter." ORDER BY ".$sorting;
			}else{
				$sql="SELECT * FROM (SELECT
						  akp.id,
						  akp.no_bukti,
		...
		case "penjualanObat":
			if ($posting==0){
				...
				while ($rows=mysql_fetch_array($rs)) {
					...
					$dt.=$sisip.chr(3).$i.chr(3).$tanggalAsli.chr(3).$no_bukti.chr(3).$rows["nFarmasi"].chr(3).$rows["nKso"].chr(3).$rows["nUnit"].chr(3).number_format($nJual,0,",",".").chr(3).number_format($bJual,0,",",".").chr(3)."0".chr(6);
				}
			}else{
				while ($rows=mysql_fetch_array($rs)) {
					$i++;
