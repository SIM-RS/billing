		 ...
		 if ($c_level>0){
			...
			if ($c_islast==1){
				...
				if (mysql_num_rows($rsBJenis)>0){
					$c_islast = 0;
					//$tree[$cnt][2] = null;
					$tree[$cnt][2]= "javascript:window.opener.location='main.php?f=transaksi&fk_jnsTrans=".$rows['JTRANS_ID']."&idBebanJenis=0"."&fk_kodeTrans=".$rows['JTRANS_KODE']."&islast=".$rows['JTRANS_ISLAST']."&kodepilih=".$kodepilih."&arfvalue=".$arfvalue."&cuser=".$cuser."&ganti=1';window.close();";
					$tree[$cnt][3]= "";
					$tree[$cnt][4]= 0;
					...
					while ($rwBJenis=mysql_fetch_array($rsBJenis)){
						...
						$tree[$cnt][2]= "javascript:window.opener.location='main.php?f=transaksi&fk_jnsTrans=".$rows['JTRANS_ID']."&idBebanJenis=".$rwBJenis["id"]."&fk_kodeTrans=".$rows['JTRANS_KODE']."&islast=".$rows['JTRANS_ISLAST']."&kodepilih=".$kodepilih."&arfvalue=".$arfvalue."&cuser=".$cuser."&ganti=1';window.close();";
						$tree[$cnt][3]= "";
						$tree[$cnt][4]= 0;
						if ($tree[$cnt][0] > $maxlevel) 
							$maxlevel=$tree[$cnt][0];    
