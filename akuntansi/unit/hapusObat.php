	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Penghapusan Obat</td>
                </tr>
                <tr>
...
<script>
	...
	a1.setHeader("DATA PENGHAPUSAN OBAT");
	a1.setColHeader("NO,TGL HAPUS,NO HAPUS,NAMA OBAT,KEPEMILIKAN,QTY,NILAI,ALASAN,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",,,OBAT_NAMA,,,,,");
	a1.setColWidth("30,80,100,250,80,40,75,75,50");
	a1.setCellAlign("center,center,center,left,center,center,right,left,center");
	...
	a1.baseURL("../unit/hapusObat_utils.php?grd=HapusObat&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value);
	//alert("../unit/hapusObat_utils.php?grd=HapusObat&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/hapusObat_utils.php?grd=HapusObat&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function kirim(){
	var url;
		url="../unit/hapusObat_utils.php?grd=HapusObat&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&posting="+document.getElementById('cmbPost').value;
		a1.loadURL(url,"","GET");
	}
	...
	var tmp='',tmp2='',cdata='';
	var url;
		url="../unit/hapusObat_utils.php?grd=HapusObat&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		...
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[8].childNodes[0].checked){
				...
				tmp+=cdata[0]+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,3))+String.fromCharCode(5)+cdata[1]+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,4))+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,6))+String.fromCharCode(5)+cdata[2]+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,8))+String.fromCharCode(5)+cdata[3]+String.fromCharCode(6);
				tmp2+=cdata[0]+"|"+ValidasiText(a1.cellsGetValue(i+1,3))+"|"+cdata[1]+"|"+ValidasiText(a1.cellsGetValue(i+1,4))+"|"+ValidasiText(a1.cellsGetValue(i+1,6))+"|"+cdata[2]+"|"+ValidasiText(a1.cellsGetValue(i+1,8))+"|"+cdata[3];
			}
		}
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingHapusObat&fdata="+tmp;
				//alert(url);
				//alert(tmp2);
				a1.loadURL(url,"","GET");
			}
