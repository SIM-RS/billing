...
<script>
	...
	a1.setColHeader("NO,UNIT PELAYANAN,NILAI RETURN,NILAI HPP,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",nama,,,");
	a1.setColWidth("50,400,100,100,75");
	a1.setCellAlign("center,left,right,right,center");
	...
	a1.baseURL("../unit/returnObat_utils.php?grd=ReturnJualObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/returnObat_utils.php?grd=ReturnJualObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function kirim(){
	var url;
		url="../unit/returnObat_utils.php?grd=ReturnJualObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var tmp='',tmp2='',idata='';
	var url;
		url="../unit/returnObat_utils.php?grd=ReturnJualObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		...
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[4].childNodes[0].checked){
				...
				tmp2+=idata[0]+"|"+idata[1]+"|"+ValidasiText(a1.cellsGetValue(i+1,3))+"|"+ValidasiText(a1.cellsGetValue(i+1,4));
			}
		}
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingReturnJual&fdata="+tmp;
				//alert(url);
				//alert(tmp2);
				a1.loadURL(url,"","GET");
			}
