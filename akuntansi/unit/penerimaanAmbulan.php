	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2"><?php if ($tipe=="SetorManual") echo 'Transaksi Penerimaan Billing ( Setoran Manual )'; else echo 'Transaksi Penerimaan Ambulan'; ?></td>
                </tr>
                <tr>
...
<script>
	...
	a1.setHeader("DATA PENERIMAAN AMBULAN");
	a1.setColHeader("NO,TGL TERIMA,NO BUKTI,PENYETOR,NILAI,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",,no_bukti,nama,nilai,");
	...
	a1.baseURL("../unit/penerimaanParAm_utils.php?grd=PenerimaanAmbulan&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/penerimaanParAm_utils.php?grd=PenerimaanAmbulan&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function kirim(){
	var url;
		url="../unit/penerimaanParAm_utils.php?grd=PenerimaanAmbulan&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var url;
		...
		url="../unit/penerimaanParAm_utils.php?grd=PenerimaanAmbulan&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		...
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postPenerimaanAmbulan&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}else{
