...
switch ($act){
	case "edit":
		...
		if(($sum1==$sum2) || (($fk_TipeTrans!=0) && ($fk_TipeTrans!="0"))){
			if (($fk_TipeTrans!=0) && ($fk_TipeTrans!="0")){
				$sql2="SELECT MAX(NO_TRANS) AS ntrans FROM jurnal WHERE NO_KW='$nokw_lama' AND FK_TRANS=$fk_jnsTrans";
				//echo $sql2."<br>";
				$rs=mysql_query($sql2);
		...
		//Maka Lakukan replace nilai data baru..
			for ($i = 0; $i < count($nilai); $i++){
				...
				if (($fk_TipeTrans==0) || ($fk_TipeTrans=="0")){
					$sql2="update JURNAL set fk_sak=$id_sak[$i],tgl='$tgl',no_kw='$nokw',uraian='$uraian',debit=$nilai1,kredit=$nilai2,tgl_act='$sekarang',fk_iduser=$iduser,d_k='$dk[$i]',CC_RV_KSO_PBF_UMUM_ID='$id_CC[$i]',KEPADA='$kepada',ALAMAT='$alamat' WHERE tr_id=".$id_tr[$i];
					//echo $sql2."<br>";
					$rs=mysql_query($sql2);
				}else{
					if($nilai1>0 OR $nilai2>0){
						$sql2="select * from jurnal WHERE fk_sak=$id_sak[$i] and NO_KW='$nokw_lama' AND FK_TRANS=$fk_jnsTrans AND  CC_RV_KSO_PBF_UMUM_ID='$id_CC[$i]'";
						//echo $sql2."<br>";
						$rs=mysql_query($sql2);
						if ($rsrow=mysql_fetch_array($rs)){
							$tnotrans=$rsrow["NO_TRANS"];
							$tlasttrans=$rsrow["FK_LAST_TRANS"];
							$sql2="update JURNAL set no_kw='$nokw',tgl='$tgl',uraian='$uraian',debit=$nilai1,kredit=$nilai2,tgl_act='$sekarang',fk_iduser=$iduser,d_k='$dk[$i]',KEPADA='$kepada',ALAMAT='$alamat' WHERE fk_sak=$id_sak[$i] and FK_TRANS=$fk_jnsTrans and NO_TRANS=$tnotrans and NO_KW='$nokw_lama' AND CC_RV_KSO_PBF_UMUM_ID='$id_CC[$i]'";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
							$sql2="update JURNAL set no_kw='$nokw',tgl='$tgl',uraian='$uraian',debit=$nilai2,kredit=$nilai1,tgl_act='$sekarang',fk_iduser=$iduser,d_k='$dk_out',KEPADA='$kepada',ALAMAT='$alamat' WHERE fk_sak=$fk_IdRek and FK_TRANS=$fk_jnsTrans and NO_TRANS=$tnotrans and NO_KW='$nokw_lama' AND FK_LAST_TRANS='$tlasttrans'";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
						}else{
							$tnotrans=$tmaxnotrans;
							$tmaxnotrans++;
							$sql2="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,KEPADA,ALAMAT) values($tnotrans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$fkBebanJenis[$i]','$id_CC[$i]','$kepada','$alamat')";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
							$sql2="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,KEPADA,ALAMAT) values($tnotrans,$fk_IdRek,'$tgl','$nokw','$uraian',$nilai2,$nilai1,'$sekarang',$iduser,'$dk_out',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$kepada','$alamat')";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
						}
					}
	...
	case "save":
		...
		//}
		//Looping Value dari Multiple Form
		$sum1=0;
		$sum2=0;
		$cdata="";
		for ($i = 0; $i < count($nilai); $i++)
			{
				//echo "Nilai : $nilai[$i]<br>id_sak : $id_sak[$i]<br>";
			//Nilainya masuk kemana?, debet atau kredit??
				if ($dk[$i]=='D') {			//Debet
					$nilai1=$nilai[$i];
					$sum1=$sum1+$nilai1;
				...
				if ($nilai[$i]!="" && $id_sak[$i]=="") $cdata="1";
			}
		if ($cdata=="1"){
			echo "<script>alert ('Maaf, Ada Kode Rekening Yang Belum Dipilih');</script>";			
		}else{
			//Jika Nilai Total Debet dan Kredit sama,	
			if(($sum1==$sum2) || ($fk_TipeTrans!=0))
			{
			//Maka Lakukan Insert data..
			$sql="SELECT IFNULL(MAX(no_trans)+1,1) AS no_trans FROM jurnal";
			$rs=mysql_query($sql);
			$no_trans=1;
			if ($rows=mysql_fetch_array($rs)) $no_trans=$rows["no_trans"];
			for ($i = 0; $i < count($nilai); $i++){
				//Nilainya masuk kemana?, debet atau kredit??
				if ($dk[$i]=='D'){			//Debet
					$nilai1=$nilai[$i];
				...
				if ($pCC_RV_KSO_PBF_UMUM[$i]==3){
					$pbfid=$id_CC[$i];
				}
				
				if($nilai1>0 OR $nilai2>0){
					//$sql="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,fk_trans) values($no_trans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang','$iduser','$dk[$i]',$fk_jnsTrans)";
					if ($fk_TipeTrans!=0){
						$sql="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,KEPADA,ALAMAT,TIPE_JURNAL) values($no_trans,$fk_IdRek,'$tgl','$nokw','$uraian',$nilai2,$nilai1,'$sekarang',$iduser,'$dk_out',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$kepada','$alamat','$fk_TipeJurnal')";
						//echo $sql."<br>";
						$rs=mysql_query($sql);
						//echo mysql_error()."<br>";					
					}
					$sql="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,MS_BEBAN_JENIS_ID,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,KSO_ID,PBF_ID,VENDOR_ID,KEPADA,ALAMAT,TIPE_JURNAL) values($no_trans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$fkBebanJenis[$i]',$id_CC[$i],'$cc_rv_id[$i]',$ksoid,$pbfid,$vendorid,'$kepada','$alamat','$fk_TipeJurnal')";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
					//echo mysql_error()."<br>";
				}
			...
			<script>
			alert ("Maaf, Total Debet dan Kredit Anda Haruslah Sama, Silahkan Masukkan Data Lagi");
			</script>
		<? }
		// Looping Berakhir
			}
	...
	case "delete":
		...
		$sql="delete from jurnal where no_trans=$no_trans and NO_KW='$nokw1'";
		//echo $sql."<br>";
		$rs=mysql_query($sql);
		$pid_jnsTrans=0;
		break;
...
<div id="input" style="display:block;">
    <table width="980" border="0" cellspacing="0" cellpadding="0" class="txtinput">
      <tr> 
        <td height="29" colspan="4" class="jdltable">Transaksi Jurnal Khusus</td>
      </tr>
      <tr> 
        <td>&nbsp;</td>
        <td class="txtinput">Kode Jns.Trans </td>
        <?php
		//$sql="SELECT * FROM jurnal WHERE NO_TRANS=$notrans AND NO_KW='$nokw' AND FK_TRANS=$fk_jnsTrans";
		if ($act=="paging") $pid_jnsTrans=0;
		$sql="SELECT * FROM jurnal WHERE NO_TRANS=$notrans AND NO_KW='$nokw' AND FK_LAST_TRANS=$pid_jnsTrans";
		//echo $sql."<br>";
		$rs=mysql_query($sql);
		$jml=mysql_num_rows($rs);

		$sql="SELECT * FROM jenis_transaksi a INNER JOIN detil_transaksi b ON a.JTRANS_ID=b.fk_jenis_trans INNER JOIN ma_sak c ON b.fk_ma_sak=c.MA_ID WHERE a.JTRANS_KODE LIKE '$fk_kodeTrans%'";
		$rs=mysql_query($sql);
		$jmle=mysql_num_rows($rs);
		
		//$sql1="SELECT * from jenis_transaksi where JTRANS_KODE='$fk_kodeTrans'";
		$sql1="SELECT jenis_transaksi.*,nama_tipe,korek,MA_NAMA from jenis_transaksi left join m_tipe_jurnal on TIPE=id_tipe left join ma_sak on MA_ID=idrek where JTRANS_KODE='$fk_kodeTrans'";
		//echo $sql1;
		$exe1 = mysql_query ($sql1);
		$show1=mysql_fetch_array($exe1);
		//$fk_jnsTrans=$show1['JTRANS_ID'];
		$fk_TipeTrans=$show1['TIPE'];
		if ($fk_TipeTrans=="") $fk_TipeTrans=0;
		if (($arfvalue=='edit') && ($jml==$jmle)){
			$fk_jnsTrans=$show1['JTRANS_ID'];
			$fk_TipeJurnal=$show1['TIPE_JURNAL'];
		}
		?>
        <td>:</td>
        <td> <input name="goKode" id="goKode" value="<?php echo $show1['JTRANS_KODE']; ?>" size="5" class="txtinput" /> 
          <button type="button" class="txtinput" onClick="location='main.php?f=transaksi&fk_jnsTrans=<?php echo $show1['JTRANS_ID']; ?>&fk_kodeTrans='+goKode.value+'&islast=<?php echo $show1['JTRANS_ISLAST']; ?>&kodepilih=<?php echo $kodepilih; ?>'"><img src="../icon/go.png" height="16" width="16"/></button></td>
      </tr>
      <tr> 
        <td width="4">&nbsp;</td>
        <td width="114" class="txtinput">Jenis Transaksi</td>
        <td>:</td>
        <td width="687"> <input type="hidden" name="fk_jnsTrans" id="fk_jnsTrans" value="<?php echo $fk_jnsTrans;?>" />
			<input type="hidden" name="fk_TipeTrans" id="fk_TipeTrans" value="<?php echo $fk_TipeTrans;?>" />
            <input type="hidden" name="fk_TipeJurnal" id="fk_TipeJurnal" value="<?php echo $fk_TipeJurnal;?>" />
          <input type="text" size="65" value="<?php if ($fk_kodeTrans==0) echo 'Silahkan Pilih Jenis Transaksi'; else echo $show1['JTRANS_NAMA']; ?>" readonly="true" /> 
          <input type="button" value=".." onClick="OpenWnd('tree_jTrans.php?kodepilih=<?php echo $kodepilih; ?>',800,500,'msma',true)" /></td>
      </tr>
      <tr> 
        <td>&nbsp;</td>
        <td height="25">Tipe Trans</td>
        <td>:</td>
        <td>
          <? if($show1['TIPE']==0) echo $show1['nama_tipe']; else echo $show1['nama_tipe']." - Pada Kode Rekening: ".$show1['korek']." - ".$show1['MA_NAMA'];?>        </td>
      </tr>
      <tr> 
        <?php
		if ($arfvalue=='edit'){
			$sql2="SELECT * FROM jurnal WHERE FK_TRANS=".$fk_jnsTrans." AND NO_TRANS=".$notrans;
			//echo $sql2;
			$exe2 = mysql_query ($sql2);
			if (mysql_num_rows($exe2)>0){
				$show2=mysql_fetch_array ($exe2);
				$tskrg=date("d-m-Y",strtotime($show2['TGL']));
			}
		}
	?>
        <td>&nbsp;</td>
        <td>Tgl Transaksi</td>
        <td width="10">:</td>
        <td><input name="tgl" type="text" id="tgl" size="11" maxlength="10" readonly="true" value="<?php echo $tskrg; ?>" class="txtcenter" /> 
      ...
      <tr>
        ...
        <td>Kepada</td>
        <td>:</td>
        <td><textarea name="kepada" cols="45" rows="1" id="kepada" class="txtinput" ><?php if ($_REQUEST[arfvalue]=='edit') echo $show2['KEPADA']; ?></textarea><!--input name="kepada" type="text" id="kepada" size="32"  class="txtinput" value="<?php if ($_REQUEST[arfvalue]=='edit') echo $show2['KEPADA']; ?>" /--></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
        <td>Alamat</td>
        <td>:</td>
        <td><textarea name="alamat" cols="45" rows="1" id="alamat" class="txtinput" ><?php if ($_REQUEST[arfvalue]=='edit') echo $show2['ALAMAT']; ?></textarea></td>
      </tr>
      <tr> 
        <td>&nbsp;</td>
        <td>No Bukti</td>
        <td width="10">:</td>
        <td><input class="txtinput" name="nokw" type="text" id="nokw" size="25" maxlength="25" value="<?php if ($_REQUEST[arfvalue]=='edit') echo $show2['NO_KW']; else echo $tnokw;?>" /> 
          <input type="hidden" name="nokw_lama" id="nokw_lama" value="<?php echo $tnokw;?>" /></td>
      </tr>
      <tr> 
        <td>&nbsp;</td>
        <td>Uraian</td>
        <td width="10">:</td>
        <td><textarea name="uraian" cols="65" rows="3" id="uraian" class="txtinput"><? if ($_GET[arfvalue]=='edit') echo $show2['URAIAN']; else echo $turaian;?></textarea></td>
      </tr>
      <tr> 
        <td>&nbsp;</td>
        <td>Pos Nilai</td>
        <td width="10">:</td>
        <?php 
	  $sql="SELECT *, ma_sak.MA_KODE AS MA_SAK_KODE, ma_sak.MA_NAMA AS MA_SAK_NAMA, ma_sak.CC_RV_KSO_PBF_UMUM, jenis_transaksi.JTRANS_ID AS Expr5, detil_transaksi.fk_ma_sak AS Expr6, detil_transaksi.id_detil_trans AS Expr8,ak_ms_beban_jenis.kode as kodeBebanJenis,ak_ms_beban_jenis.nama as namaBebanJenis
			FROM detil_transaksi INNER JOIN jenis_transaksi ON detil_transaksi.fk_jenis_trans = jenis_transaksi.JTRANS_ID INNER JOIN ma_sak ON detil_transaksi.fk_ma_sak = ma_sak.MA_ID LEFT JOIN ak_ms_beban_jenis ON detil_transaksi.fk_ms_beban_jenis = ak_ms_beban_jenis.id WHERE jenis_transaksi.JTRANS_KODE LIKE '$fk_kodeTrans%'".$strBJenis." order by detil_transaksi.dk, detil_transaksi.fk_ms_beban_jenis, ma_sak.MA_KODE,detil_transaksi.id_detil_trans";
		if (($arfvalue=='edit') && ($jml!=$jmle)){
			if ($fk_TipeTrans==0)
			  $sql="SELECT *,b.CC_RV_KSO_PBF_UMUM cc_rv_kso_pbf_umum,a.CC_RV_KSO_PBF_UMUM_ID id_cc_rv_kso_pbf_umum,MA_KODE AS MA_SAK_KODE,MA_NAMA AS MA_SAK_NAMA,FK_TRANS AS Expr5,MA_ID AS Expr6,D_K AS dk FROM jurnal a INNER JOIN ma_sak b ON a.FK_SAK=b.MA_ID WHERE a.NO_TRANS=$notrans AND a.FK_LAST_TRANS=$pid_jnsTrans AND NO_KW='$nokw' ORDER BY TR_ID";
		}
	//echo $sql."<br>";
	$exe= mysql_query ($sql);
	...
	$divHeight1=200;
	$divHeight2=130;
	if ($_REQUEST["fk_jnsTrans"]!=""){
		if ($jmldata<6){
			$divHeight2=$jmldata*55;
			$divHeight1=$divHeight2+70;		
		}else{
			$divHeight1=400;
			$divHeight2=330;
		}
	...
	?>
        <td>
        	<div style="height:<?php echo $divHeight1; ?>px; overflow:hidden;">
            <!--div id="divTblHeader" style="height:400px; overflow:hidden;"-->
        	<table width="100%" border="0">
            <tr class="headtable"> 
              <td width="25" class="tblheaderkiri">No.</td>
              <td width="111" class="tblheader">Kode Rek </td>
              <td class="tblheader">Nama Rek </td>
              <td width="170" class="tblheader">Keterangan<br>(Jenis Beban)</td>
              <td width="60" class="tblheader">Debet/ Kredit </td>
              <td width="105" class="tblheader">Nilai</td>
            </tr>
            </table>
            <div id="divTblTrans" style="height:<?php echo $divHeight2; ?>px; overflow:scroll;">
            <!--div id="divTblTrans"-->
        	<table width="100%" border="0">
            <tr class="headtable" style="visibility:collapse;"> 
              <td width="25" class="tblheaderkiri">No.</td>
              <td width="111" class="tblheader">Kode Rek </td>
              <td class="tblheader">Nama Rek </td>
              <td width="170" class="tblheader">Keterangan</td>
              <td width="60" class="tblheader">Debet/ Kredit </td>
              <td width="80" class="tblheader">Nilai</td>
            </tr>
            <?php
			//echo $fk_TipeTrans."<br>";
		if ($fk_TipeTrans==0)
			$sql3="SELECT * FROM jurnal a INNER JOIN ma_sak b ON a.FK_SAK=b.MA_ID WHERE a.NO_TRANS=$notrans AND a.FK_LAST_TRANS=$pid_jnsTrans ORDER BY TR_ID";
		else
			$sql3="SELECT * FROM jurnal a INNER JOIN ma_sak b ON a.FK_SAK=b.MA_ID WHERE a.NO_TRANS=$notrans AND a.FK_TRANS=$fk_jnsTrans  AND a.FK_LAST_TRANS=$pid_jnsTrans ORDER BY TR_ID";
		//echo $sql3."<br>";
		$exe3=mysql_query($sql3);
		$idtr="";
		while ($show3=mysql_fetch_array ($exe3)){
			if ($show3['DEBIT']>0){ $nilai1=$show3['DEBIT'];}else{ $nilai1=$show3['KREDIT'];}
			$nilaiDK[]=$nilai1;
			$arkdma[]=$show3['MA_KODE'];
			$aridsak[]=$show3['FK_SAK'];
			$aridBebanJenis[]=$show3['MS_BEBAN_JENIS_ID'];
			$arCC_RV_PBF_UMUM_ID[]=$show3['CC_RV_KSO_PBF_UMUM_ID'];
			$arcc_rv_id[]=$show3['CC_RV_ID'];
			$arnamasak[]=$show3['MA_NAMA'];
			$idtr[]=$show3['TR_ID'];
		}
			$nilai_DK=0;
			$cnt=0;
			...
			while ($show = mysql_fetch_array ($exe)) {
				...
				$ketCC="";
				$namasak1="";
				$pCC_RV_KSO_PBF_UMUM=$show["CC_RV_KSO_PBF_UMUM"];
				//echo $show["cc_rv_kso_pbf_umum"]."<br>";
				//switch ($show["cc_rv_kso_pbf_umum"]){
				switch ($show["CC_RV_KSO_PBF_UMUM"]){
					case 0:
						...
						break;
					case 1:
						if (($arfvalue=='edit') && ($show["id_cc_rv_kso_pbf_umum"]==0)){
							if ($arCC_RV_PBF_UMUM_ID[$cnt]>0){
								$psql="SELECT * FROM ak_ms_unit WHERE id=".$arCC_RV_PBF_UMUM_ID[$cnt];
							}else{
								//$psql="SELECT * FROM ak_ms_unit WHERE id='".$show["id_cc_rv_kso_pbf_umum"]."'";
								$psql="SELECT * FROM ak_ms_unit WHERE id='".$show["CC_RV_KSO_PBF_UMUM_ID"]."'";
							}
						}else{
							//$psql="SELECT * FROM ak_ms_unit WHERE id='".$show["id_cc_rv_kso_pbf_umum"]."'";
							$psql="SELECT * FROM ak_ms_unit WHERE id='".$show["CC_RV_KSO_PBF_UMUM_ID"]."'";
						...
						break;
					case 2:
						$psql="SELECT * FROM $dbbilling.b_ms_kso WHERE id='".$show["id_cc_rv_kso_pbf_umum"]."'";
						//echo $psql."<br>";
						$rsCC=mysql_query($psql);
						$rwCC=mysql_fetch_array($rsCC);
						$idCC=$rwCC["id"];
						...
						break;
					case 3:
						$psql="SELECT * FROM $dbapotek.a_pbf WHERE PBF_ID='".$show["id_cc_rv_kso_pbf_umum"]."'";
						//echo $psql."<br>";
						$rsCC=mysql_query($psql);
						$rwCC=mysql_fetch_array($rsCC);
						$idCC=$rwCC["PBF_ID"];
						...
						break;
					case 4:
						$psql="SELECT * FROM $dbaset.as_ms_rekanan WHERE idrekanan='".$show["id_cc_rv_kso_pbf_umum"]."'";
						//echo $psql."<br>";
						$rsCC=mysql_query($psql);
						$rwCC=mysql_fetch_array($rsCC);
						$idCC=$rwCC["idrekanan"];
				...
				//echo $show['MA_ISLAST']."<br>";
				//echo $psql."<br>";
			?>
            <tr class="itemtable"> 
              <?php 
			  		$idBebanJenis=$show['fk_ms_beban_jenis'];
					$id_Bjenis=$show['fk_ms_beban_jenis'];
					$kodeBebanJenis="";
					if ($id_Bjenis>0){
						$kodeBebanJenis=" .".$show['kodeBebanJenis'].".04";
					}

			  		if ($show['MA_ISLAST']==1){ 
			  		$kdma=$show['MA_SAK_KODE'];
					$idsak=$show['Expr6'];
					$namasak=$show['MA_SAK_NAMA'];
					if ($show['CC_RV_KSO_PBF_UMUM']==0){
						$namasak1=$namasak;
					}else{
						//$namasak1="";
              ...
              <input type="hidden" name="dk[]" id="dk" value="<?=$show['dk'];?>" />
              <input type="hidden" name="id_tr[]" id="id_tr" value="<?php echo $idtr[$cnt]; ?>" />
			  <input type="hidden" name="id_sak[]" id="id_sak" value="<?php echo $idsak; ?>" />
              <input type="hidden" name="pCC_RV_KSO_PBF_UMUM[]" id="pCC_RV_KSO_PBF_UMUM" value="<?php echo $pCC_RV_KSO_PBF_UMUM; ?>" />
              <input type="hidden" name="id_CC[]" id="id_CC" value="<?php echo $idCC; ?>" />
              <input type="hidden" name="cc_rv_id[]" id="cc_rv_id" value="<?php echo $cc_rv_id; ?>" />
              <input type="hidden" name="id_BebanJenis[]" id="id_BebanJenis" value="<?php echo $idBebanJenis; ?>" />
              <input type="hidden" name="id_Bjenis[]" id="id_Bjenis" value="<?php echo $id_Bjenis; ?>" />
              <input type="hidden" name="pid_jnsTrans[]" id="pid_jnsTrans" value="<?php if ($fk_TipeTrans==0) echo $show['Expr5']; else echo $show['Expr8']; ?>" />
              <td class="tdisikiri"> 
                <?php echo $i++; ?>              </td>
              <td class="tdisi" align="center"> <input class="txtinput" type="<?php if (($show['MA_ISLAST']==0) || ($show['MA_ISLAST']==1 && $show['CC_RV_KSO_PBF_UMUM']==3)){echo 'text';}else{echo 'hidden';}?>" name="kode_sak[]" id="kode_sak" value="<?=$kdma;?>" size="8" readonly="true"/> 
                <?php if (($show['MA_ISLAST']==1 && $show['CC_RV_KSO_PBF_UMUM']<>3) || ($fk_TipeTrans>0)) echo $kdma.$kodeBebanJenis;?>
                <?php if (($show['MA_ISLAST']==0) || ($show['MA_ISLAST']==1 && $show['CC_RV_KSO_PBF_UMUM']==3)){?>
                <input type="button" class="txtcenter" name="Button222" value=".." title="Pilih Rekening Anak dari &quot;<?=$show['MA_SAK_NAMA'];?>&quot;" onClick="OpenWnd('nilaiAnak.php?<?php echo $qstr_ma_sak."&cnt=$cnt"; ?>&kode_induknya=<?=$show['MA_SAK_KODE'];?>',800,500,'msma',true)"> 
                <?php }?>              </td>
              <td class="tdisi"><?php if ($show['MA_ISLAST']==1) echo $namasak;?><? if (($show['MA_ISLAST']==0) || ($show['MA_ISLAST']==1 && $show['CC_RV_KSO_PBF_UMUM']==3)) echo "<br>";?><input type="<?php if (($show['MA_ISLAST']==0) || ($show['MA_ISLAST']==1 && $show['CC_RV_KSO_PBF_UMUM']==3)){echo 'text';}else{echo 'hidden';}?>" class="txtinput" name="nama_sak[]" id="nama_sak" value="<?=$namasak1;?>" readonly="true" size="35" />              </td>
              <td class="tdisi"><?php echo $show['namaBebanJenis']; ?>&nbsp;              </td>
              <td class="tdisi"> 
                <?php if ($show['dk']=='D'){
					echo "Debet";
					}elseif ($show['dk']=='K'){
			  ...
			  if ($fk_TipeTrans==0){
			  	if ($show['dk']=='D'){
			  		$tdebit+=$nilaiDK[$nilai_DK];
				}else{
					$tkredit+=$nilaiDK[$nilai_DK];
				}
			  ?>
              <td class="tdisi" width="80"> <input class="txtright" type="text" name="nilai[]" id="nilai" size="10" autocomplete="off" value="<?=$nilaiDK[$nilai_DK++];?>" onKeyUp="cari(event,this);" /></td>
              <?php 
			  }else{
			  	$ndk="";
			  	if($arfvalue=="edit"){
			  		$strsql="SELECT * FROM jurnal WHERE FK_SAK='$idsak' AND NO_KW='$tnokw' AND FK_TRANS=$fk_jnsTrans AND CC_RV_KSO_PBF_UMUM_ID='$idCC'";
					//echo $strsql."<br>";
					$showdk=mysql_query($strsql);
					if ($rowsdk=mysql_fetch_array($showdk)){
						if ($show['dk']=='D'){
							$ndk=$rowsdk["DEBIT"];
							$tdebit+=$ndk;
						}else{
			  ...
			  ?>
              <td class="tdisi" width="80"> <input class="txtright" type="text" name="nilai[]" id="nilai" size="10" autocomplete="off" value="<?=$ndk;?>" onKeyUp="cari(event,this);" /></td>	
            <?php
			  }
			  ?></tr>
            <?php
            ...
            <!--tr class="txtinput">
              ...
              <td>&nbsp;</td>
              <td>&nbsp;</td>
              <td colspan="2" align="right">Total Debet&nbsp;</td>
              <td id="totdebit" align="right"><?php echo number_format($tdebit,2,",","."); ?></td>
            </tr>
            <tr class="txtinput">
              <td>&nbsp;</td>
              <td>&nbsp;</td>
              ...
              <td id="totkredit" align="right"><?php echo number_format($tkredit,2,",","."); ?></td>
            </tr-->
            <input type="hidden" name="cidTotK" id="cidTotK" value="<?=$cidTotK;?>" />
          </table>
          </div>
        	<table width="100%" border="0">
            <!--tr class="txtinput"> 
              <td width="25">No.</td>
              <td width="111">Kode Rek </td>
              <td >Nama Rek </td>
              <td width="170">Keterangan</td>
              <td width="60">Debet/ Kredit </td>
              <td width="105">Nilai</td>
            </tr-->
            <tr class="txtinput">
              <td width="25">&nbsp;</td>
              <td width="111">&nbsp;</td>
              <td>&nbsp;</td>
              <td width="230" colspan="2" align="right">Total Debet&nbsp;</td>
              <td width="105" id="totdebit" align="right"><?php echo number_format($tdebit,2,",","."); ?></td>
              <td width="15">&nbsp;</td>
            </tr>
            <tr class="txtinput">
              <td>&nbsp;</td>
      ...
      <td height="40" colspan="5" align="center" valign="bottom"><BUTTON type="button" onClick="if (ValidateForm('fk_jnsTrans,tgl,nokw,uraian','ind')){document.form1.submit();}"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Simpan</BUTTON>
          <BUTTON type="reset" onClick="location='?f=transaksi&arfvalue=0&notrans=0&tgl=<?php echo $th5; ?>&nokw= &uraian= &'"><IMG SRC="../icon/cancel.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;<strong>Reset</strong>&nbsp;&nbsp;&nbsp;&nbsp;</BUTTON></td>
      </tr>
    </table>
</div>
...
<div id="listjurnal" style="margin-top:15px"><br>
    <table width="99%" border="0" cellpadding="2" cellspacing="0">
      <tr> 
        <td colspan="11"><div align="center"><span class="jdltable">Transaksi Journal 
            </span></div></td>
      </tr>
      <tr> 
        <td colspan="11">&nbsp;</td>
	  ...
	  <td colspan="11" valign="middle" class="txtinput">Data dientry Oleh : 
		...
		<button type="button" onClick="location='?f=transaksi.php&tgl_s='+tgl_s.value+'&tgl_d='+tgl_d.value+'&cuser='+cuser.value+'&kodepilih=<?php echo $kodepilih; ?>'">
	  ...
	  $sqlQ="Select jurnal.*, ma_sak.MA_KODE, ma_sak.MA_KODE_KP, ma_sak.MA_NAMA, ma_sak.FK_MA, ma_sak.CC_RV_KSO_PBF_UMUM From jurnal Inner Join ma_sak ON jurnal.FK_SAK = ma_sak.MA_ID where TGL between '$tgl_1' and '$tgl_2' AND VERIFIKASI=1 ".$fuser.$fver.$filter." order by ".$sorting;
	  //echo $sqlQ."<br>";
		$rs=mysql_query($sqlQ);
	  ...
	  while ($rows=mysql_fetch_array ($rs)){
		...
		if ($rows['MS_BEBAN_JENIS_ID']>0){
			...
			$rwcc=mysql_fetch_array($rscc);
			$ketcc=" - ".$rwcc["nama"];
			$txtKodeMA.=" .".$rwcc["kode"].".04";
		}else{
			$txtKodeMA.=" .00.00.00.04";
		}
		switch ($ccrvpbf){
			/*case 1:
...
<script>
	...
	function spnNoTransClick(noTrans_noKw,tipeJ){
		...
		switch(tipeJ){
			case 0:
			case 1:
				window.open("../../keuangan/laporan/jurnal/bukti_penerimaan_kas_bank.php?notrans_kw="+noTrans_noKw);
				break;
			case 2:
				window.open("../../keuangan/laporan/jurnal/bukti_pengeluaran_kas_bank.php?notrans_kw="+noTrans_noKw);
				break;
			case 3:
				window.open("../../keuangan/laporan/jurnal/bukti_jurnal_rupa_rupa.php?notrans_kw="+noTrans_noKw);
				break;
			case 4:
				window.open("../../keuangan/laporan/jurnal/bukti_jurnal_rupa_rupa_penjualan_obat.php?notrans_kw="+noTrans_noKw);
				break;
			case 5:
				window.open("../../keuangan/laporan/jurnal/bukti_jurnal_pembelian.php?notrans_kw="+noTrans_noKw);
				break;
		}
	}
