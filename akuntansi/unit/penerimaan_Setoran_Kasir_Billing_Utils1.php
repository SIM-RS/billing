	...
	case "posting":
		...
		if ($posting==0)
		{
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$sql="update ".$dbkeuangan.".k_transaksi set posting='1',no_post='$id_k_trans' 
				...
				{						
					...
					$sql="SELECT
							  ...
							  kso_id,
							  tgl,
  							  SUM(nilai_sim)      AS nilai_sim,
							  SUM(nilai)      AS nilai
							FROM $dbkeuangan.k_transaksi
							WHERE kasir_id = '$user_kasir'
								AND no_bukti = '$nslip'
								AND tgl_klaim = '$tanggalSlip'
								AND id_trans = '$IdTrans'
							GROUP BY unit_id_billing,kso_id,tgl";
					//echo $sql.";<br>";
					...
					while ($rwData=mysql_fetch_array($rsData)){
						...
						//======insert into jurnal========
						$sql="SELECT rv.* FROM ".$dbbilling.".b_ms_unit mu INNER JOIN ".$dbakuntansi.".ak_ms_unit rv ON mu.kode_ak=rv.kode WHERE mu.id=$unit_id";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
						$rwPost=mysql_fetch_array($rsPost);
						$idunit=$rwPost["id"];
						...
						if ($kso==1)
						{
							...
							$id_pendapatan=488;	//Pendapatan Px Umum (Tunai)
							$id_pendapatan=488;
							//$jenistrans=7;	//Saat Px Umum Bayar
							$jenistrans=258;	//1112a - Terjadinya Transaksi Pendapatan (Tunai) - Tipe: Normal
						}
						else
						{
							$id_sak=8;	//Kas di Bendahara Penerimaan
							...
							if ($ctipeKso==1){
								...
								$id_pendapatan=493;	//Pendapatan Jasa Layanan Pasien Kerja Sama dengan KSO
								//$id_selisih_pendapatan=491; //Selisih Tarif Jasa Layanan Pasien Kerja Sama dengan KSO
								//$jenistrans=14;	//Saat Px KSO Bayar
								$jenistrans=16;	//1122a - Terjadinya Transaksi Pendapatan Pasien KSO (Beban Pasien - Tunai) - Tipe: Normal
							}else{
								//$id_piutang=45;	//Piutang KSO beban Px
								$id_pendapatan=490;	//Pendapatan Jasa Layanan Pasien Kerja Sama dengan KSO
								$id_selisih_pendapatan=491; //Selisih Tarif Jasa Layanan Pasien Kerja Sama dengan KSO
								//$jenistrans=14;	//Saat Px KSO Bayar
								$jenistrans=396;	//1122a - Terjadinya Transaksi Pendapatan Pasien KSO (Beban Pasien - Tunai) - Tipe: Normal
							}
						}
			...
			VALUES('$id_k_trans','$user_kasir',$notrans,$no_psg,$id_sak,'$tglP','$nokw','$uraian',$nilai_slip,0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'$no_bukti',$kso,$idunit,1)";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
						
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
		VALUES('$id_k_trans','$user_kasir',$notrans,$no_psg,$id_pendapatan,'$tglP','$nokw','$uraian',0,$nilai_slip,now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'$no_bukti',$kso,$idunit,1)";
						//echo $sql.";<br>";
						$rsPost=mysql_query($sql);
					}
					...
					$uraian="Penerimaan Kasir : ".$ckasir.", Setor ke Bank";
					
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
		VALUES('$id_k_trans','$user_kasir',$notrans,$no_psg,$id_sak_bank,'$tanggalSlip','$nokw','$uraian',$ntotal,0,now(),$idUser,'D',0,$jTransSetor,$jTransSetor,1,'$no_bukti',0,0,1)";
					//echo $sql.";<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(FK_ID_POSTING,FK_ID_USER_APP,NO_TRANS,NO_PASANGAN,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,CC_RV_ID,POSTING) 
		VALUES('$id_k_trans','$user_kasir',$notrans,$no_psg,$id_sak,'$tanggalSlip','$nokw','$uraian',0,$ntotal,now(),$idUser,'K',0,$jTransSetor,$jTransSetor,1,'$no_bukti',0,0,1)";
					//echo $sql.";<br>";
					$rsPost=mysql_query($sql);
				}
				else
		...
		else
		{
			...
			for ($i=0;$i<count($arfdata);$i++)
			{
				...
				$sql="update ".$dbkeuangan.".k_transaksi set posting='0', no_post='0'
					where kasir_id='$user_kasir' AND tgl_klaim='$tanggalSlip' AND no_post='$no_post' AND id_trans='$IdTrans' AND no_bukti = '$nslip'";
				//echo $sql."<br>";
				$rs=mysql_query($sql);
				if (mysql_errno()==0 && mysql_affected_rows()>0){
					$sDel = "DELETE FROM jurnal WHERE FK_ID_USER_APP='$user_kasir' AND FK_ID_POSTING='$no_post' AND NO_BUKTI='$nslip'";
					//echo $sDel."<br>";
					mysql_query($sDel);
				}
			}
	...
	switch ($grd){
		...
		case "penerimaanBillingKasir":			
			...
			/*$sql="SELECT
					  ...
					  gab.no_rm,
					  gab.pasien,
					  GROUP_CONCAT(gab.kso_id SEPARATOR ',') AS kso_id,
					  GROUP_CONCAT(gab.kso SEPARATOR ',') AS kso,
					  gab.user_act,
					  gab.nama,
					  SUM(gab.nilai_sim) AS nilai_sim,
					  SUM(gab.nilai_slip) AS nilai_slip,
					  gab.tgl,
					  gab.tgl_klaim,
					  gab.posting,
					  gab.id_trans
					FROM (SELECT
						  ...
						  WHERE t.posting = '$posting'
							  ...
							  $fkasir
						  GROUP BY t.fk_id,kso.id) AS gab
					WHERE 1=1 $filter
					GROUP BY gab.fk_id
					$sorting";*/
