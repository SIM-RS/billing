...
<?php	
  ...
  $maxlevel=0;
  $cnt=0;
  $strSQL = "select * from cost where cost_aktif=1 order by cost_kode";
  $rs = mysql_query($strSQL);
  while ($rows=mysql_fetch_array($rs)){
		 $c_level = $rows["COST_LEVEL"];
//		 $mpkode=trim($rows['COST_MKODE']);
		 $tree[$cnt][0]= $c_level;
		 $tree[$cnt][1]= $rows["COST_KODE"]." - ".($mpkode==""?"":$mpkode." - ").$rows["COST_NAMA"];
		 if ($c_level>0)
			 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['COST_ID']."*|*".$par[1]."*-*".$rows['COST_KODE']."*|*".$par[2]."*-*".$rows['COST_NAMA']."*|*".$par[3]."*-*".$rows['COST_LEVEL']."');window.close();";
 		 else
		 	 $tree[$cnt][2] = null;
