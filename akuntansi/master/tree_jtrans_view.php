...
<?php	
  ...
  while ($rows=mysql_fetch_array($rs)){
		 ...
		 $mpkode=trim($rows['JTRANS_MKODE']);
		 $tree[$cnt][0]= $c_level;
		 $tree[$cnt][1]= $rows["JTRANS_KODE"]." - ".($mpkode==""?"":$mpkode." - ").$rows["JTRANS_NAMA"];
		 if ($c_level>0)
			 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['JTRANS_ID']."*|*".$par[1]."*-*".$rows['JTRANS_KODE']."*|*".$par[2]."*-*".$rows['JTRANS_NAMA']."*|*".$par[3]."*-*".$rows['JTRANS_LEVEL']."');window.close();";
 		 else
		 	 $tree[$cnt][2] = null;
