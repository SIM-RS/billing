...
<?php	
  ...
  $maxlevel=0;
  $cnt=0;
  $strSQL = "SELECT * FROM ak_ms_unit WHERE tipe=1 AND aktif=1 ORDER BY kode";
  ...
  while ($rows=mysql_fetch_array($rs)){
		 ...
		 if ($c_level>0)
			 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['id']."*|*".$par[1]."*-*".$rows['kode']."*|*".$par[2]."*-*".$rows['nama']."*|*".$par[3]."*-*".$rows['level']."');window.close();";
 		 else
		 	 $tree[$cnt][2] = null;
