...
var vtpar;
	...
	for (var i=0;i<tpar.length;i++){
		...
		//alert(vtpar[1]);
		if(obj.document.getElementById(vtpar[0]) == undefined) continue;
		if (obj.document.getElementById(vtpar[0]).length > 1){
...
var vtpar;
	...
	//for (var i=0;i<tpar.length;i++){
		...
		vtpar=tpar[3].split('*-*');
		obj.document.forms[0].id_CC[idx].value=vtpar[1];
		//obj.document.getElementById(vtpar[0])[idx].value=vtpar[1];
		//alert(vtpar[0]+" - "+obj.document.getElementByName(vtpar[0][idx]).value);
	//}
	//window.close();
}
