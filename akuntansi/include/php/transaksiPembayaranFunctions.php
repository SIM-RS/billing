...
function updateTransactionStatus($id, $newStatus, $newTglProses, $table = TRANSAKSI, $additionalValue = array()) {
    $sql = "SELECT `status`, uraian,tgl_proses FROM $table WHERE id =" . $id;
    $result = mysql_query($sql);
    $result = mysql_fetch_assoc($result);
    $oldStatus = $result['status'];
    $keterangan = $result['uraian'];
	$oldTglProses = $result['tgl_proses'];

    $additionalValue['status'] = $newStatus;
	$additionalValue['tgl_proses'] = $newTglProses;
    $values = '';
