  ...
  while ($cnt<count($tree))
  {
    ...
    if ($visible[$cnt])
    {
      ...
      if ($tree[$cnt][2]=="")
          ...
          echo "<td colspan=".($maxlevel-$tree[$cnt][0])." class='tree_ma'><a href=\"".$tree[$cnt][2]."\" target=\"".$tree[$cnt][3]."\">".$tree[$cnt][1]."</a>".(($tree[$cnt][5]>0)?"  [  <img src='../icon/del.gif' border='0' width='16' height='16' align='absmiddle' class='proses' title='Klik Untuk Menghapus' onClick=\"if (confirm('Yakin Ingin Menghapus Data ?')){fSetValue(window,'act*-*delete*|*idma*-*".$tree[$cnt][5]."');document.form1.submit();}\">   ]":"").(($tree[$cnt][6]!="")?$tree[$cnt][6]:"")."</td>";
