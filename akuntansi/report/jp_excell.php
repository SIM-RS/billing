...
if($tipe==4){
$jdl_lap="LAP JRR Penjualan Obat";
}elseif($tipe==5){
$jdl_lap="LAP JRR Pembelian Pemborongan";
}else{
$jdl_lap="LAP JP";
}
...
$workbook->send('JP.xls');

//Creating a worksheet
$worksheet=&$workbook->addWorksheet('JP');
...
$worksheet->write($row, 3, "RUMAH SAKIT PRIMA HUSADA CIPTA MEDAN", $sheetTitleFormatC);
$row += 1;
...
$worksheet->write($row, $column+4, "Debit", $columnTitleFormat);
$worksheet->write($row, $column+5, "Kredit", $columnTitleFormat);
...
$sql="SELECT a.no_kw, a.uraian,DATE_FORMAT(a.tgl,'%d-%m-%Y') tanggal,a.debit,a.kredit FROM jurnal a 
 ...
 WHERE a.`TGL` BETWEEN '$tgl_s1' AND '$tgl_d1' AND a.TIPE_JURNAL='$tipe' AND a.flag='$flag' AND NO_KW LIKE '%JP%'";
$rs=mysql_query($sql);
$i=0;
while ($rows=mysql_fetch_array($rs)){
	...
	$worksheet->write($row, 4, $rows['debit'], $regularFormatR);
	$worksheet->write($row, 5, $rows['kredit'], $regularFormatR);
	$row++;
	$i++;
}
...
$worksheet->write($row, 5, '=Sum(F'.$rowStart.':F'.$row.')', $regularFormatR);
