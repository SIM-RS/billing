...
<table width="1038" height="258" border="0" align="center" cellpadding="0" cellspacing="0" >
	<tr align="center">
		<td colspan="2"><p style="font-size:12px">PT. PELABUHAN INDONESIA I (Persero) RS. PELABUHAN MEDAN </p>
		...
		<!-- ---TABLE AKTIVE --------- -->		
			<table width="515" height="53"  cellpadding="0" cellspacing="0" style="font-family:Arial, Helvetica, sans-serif; font-size:11px;" align="right">
				...
				<?php
					...
					$sql = "select * from lap_neraca where isaktif=1 and aktiva_passiva=0 order by id ";
					//echo $sql;
					...
					while ($t = mysql_fetch_array($hasil)){
						...
						if ($ikode!=""){
							...
							inner join ma_sak ms ON j.FK_SAK = ms.MA_ID  WHERE ms.MA_KODE LIKE '$ikode%' AND MONTH(j.TGL)='$ibulan1' AND YEAR(j.TGL)='$ita1'"; 
							...
							if ($tgl2=="1"){
								...
								$sql= "select IFNULL(SUM(j.DEBIT),'-') D, IFNULL(SUM(j.KREDIT),'-') K, j.CC_RV_KSO_PBF_UMUM_ID  from jurnal j
							inner join ma_sak ms ON j.FK_SAK = ms.MA_ID  WHERE ms.MA_KODE LIKE '$ikode%' AND MONTH(j.TGL)='$ibulan2' AND YEAR(j.TGL)='$ita2'";
								$rs2=mysql_query($sql);
		...
		<td>
			...
			<table width="515" height="53"  cellpadding="0" cellspacing="0"  style="font-family:Arial, Helvetica, sans-serif; font-size:11px;" align="left">
				<tr height="17" align="center" style="font-weight:bold">
					...
					$sql = "select * from lap_neraca where isaktif=1 and aktiva_passiva=1 order by id ";
