...
//===============================
$idma=$_REQUEST['idma'];if ($idma=="") $idma="4135";
...
//if (($cislast=="1" && $ccrvpbfumum=="0") || ($cc_islast=="1" && $ccrvpbfumum!="0")){
	...
	$row += 1;
	//$worksheet->write($row, 1, "IKHTISAR :", $sheetTitleFormat);
	//$worksheet->write($row, 3, strtoupper($ma)." (".$kode_ma.")", $sheetTitleFormat);
	$row += 1;
	$worksheet->write($row, 1, "PRIODE : ".$ta, $sheetTitleFormat);
	$row += 1;
	...
	$worksheet->write($row, 0, "Bulan", $columnTitleFormat);
	...
	$worksheet->write($row, 1, "JUMLAH", $columnTitleFormat);
	//$row++;
...
$worksheet->write($row, 0, "Total", $regularFormatRF_TLBR);
$worksheet->write($row, 1, $total, $regularFormatRF_TLBR);
