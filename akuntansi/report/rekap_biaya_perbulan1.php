<?php
	...
	// if($_POST['export']=='excel'){
		...
		header('Content-Disposition: attachment; filename="Lap Biaya Berdasarkan Jenis.xls"');
	// }
	...
	if($waktu == 'Harian'){
		...
		$waktu = " AND MONTH(j2.TGL_ACT) = '$bln' AND YEAR(j2.TGL_ACT) = '$thn' ";
        ...
        $waktu = " AND j2.TGL_ACT between '$tglAwal2' and '$tglAkhir2' ";
        
        $Periode = "Periode : ".$tglAwal[0]." ".$arrBln[$tglAwal[1]]." ".$tglAwal[2].' s/d '.$tglAkhir[0]." ".$arrBln[$tglAkhir[1]]." ".$tglAkhir[2];
    }
	
	$sLevel = "SELECT MAX(`level`) lvl FROM ak_ms_beban_jenis";
	$qLevel = mysql_query($sLevel);
	$level = 1;
	$colspanHead = "";
	if($qLevel){
		$dLevel = mysql_fetch_array($qLevel);
		$level = $dLevel['lvl'];
		$colspanHead = "colspan='{$level}'";
	}
?>
<!DOCTYPE HTML>
<html lang="en-US">
...
<body>
	<div id="container">
		<header id="judul">
			<b>
				REKAP BIAYA BERDASARKAN JENIS TRANSAKSI<br />
				<?php echo $namaRS; ?><br />
				<?php echo $Periode; ?>
			</b>
		</header>
		<table id="dataNilai">
			<tr>
				<th class="borderfull" rowspan="2">No Rek</th>
				<th class="borderfull" rowspan="2" width="600" <?php echo $colspanHead; ?> >Uraian</th>
				<?php 
					$caseQuery = array();
						...
						switch($param['type']){
							case "coloum-head":
								...
								if($qChild && mysql_num_rows($qChild) > 0){
									...
									{
										if($dChild->islast != 0){
											$result["value"] .= "<th class='borderfull' width='200px' >".ucwords(strtolower($dChild->nama))."<br />".str_replace($dChild->parent_kode, $dChild->parent_kode.".", $dChild->kode)."</th>";
											$caseQuery[$dChild->kode] = "SUM(CASE WHEN j.CC_RV_ID = '".$dChild->id."' THEN j.nilai ELSE 0 END) AS `". $dChild->kode ."`";
										} else {
									...
									$result["value"] .= "<th class='borderfull' width='200px' >Jumlah<br />".$parentKode."</th>";
								}
								break;
							case "coloum-row":
								break;
					...
					while($qHead && $dHead = mysql_fetch_object($qHead))
					{
						...
						echo "<th class='borderfull' $colspan >".ucwords(strtolower($dHead->nama))."</th>";
						$result[] = $data;
					}
				?>
			</tr>
			...
			<tr style="background:#F6F6F6;">
				<?php
					echo "<th class='borderfull' >1</th>";
					for($j = 1; $j < $level; $j++){
						echo "<th style='width:1% !important;' class='topbottom'></th>";
					}
					echo "<th class='topbottom' style='text-align:left; padding-left:90px;' >2</th>";
					$col = 1;
					for($i=0; $i < count($caseQuery); $i++){
						echo "<th class='borderfull' >".(3+$i)."</th>";
					}
				?>
			</tr>
			<?php
				...
				while($query && $data = mysql_fetch_object($query)){
					...
					if($parent_kode != "kosong" && strlen($parent_kode) > 2){
						...
						if($parent_kode != $data->parent_kode && $subTotal[$parent_kode]['jml'] > 1){ // && $data->islast != 0
							echo "<tr><td class='borderbottom'></td><td class='borderbottom' align='right' {$colspanHead} >Jumlah {$parent_kode}</td>";
							foreach($subTotal[$parent_kode]['nilai'] as $key => $val){
								echo "<td class='borderbottom' align='right'>".number_format($val,0,",",".")."&nbsp;</td>";
							}
							echo "</tr><tr>";
							$subTotal = array();
						}
					...
					if($data->level == 1 && $data->islast == 0){
						if($no != 1){
							...
							echo "<td class='borderbottom' style='font-weight:bold;' align='right' {$colspanHead} >Jumlah {$head}</td>";
							foreach($subTotalHead as $key => $val){
								echo "<td class='borderbottom' align='right' style='font-weight:bold;' >".number_format($val,0,",",".")."&nbsp;</td>";
								$globalTotal[$key] += $val;
							}
							echo "</tr><tr>";
							$head = $data->kode ." - ". $data->nama;
							$headkode = $data->kode;
							$subTotalHead = array();
						} else {
							$head = $data->kode ." - ". $data->nama;
						...
						echo "<td class='borderbottom' style='font-weight:bold;' colspan='{$level}'>".$data->nama."</td>";
					} else { 
						echo "<td class='borderbottom'></td>";
						$number = str_replace($data->parent_kode.".", "", $data->kode);
						// $padding = (strlen($data->parent_kode) == 2 ? "5px;" : (strlen($data->parent_kode) == 5 ? "20px" : ($padding*2)."px"));
						// echo "<td class='borderbottom' style='padding-left:{$padding};'>". $number .". ".$data->nama."</td>";
						$lvlSpan = ($data->level - 1);
						$colspanNama = "";
						if($data->level != $level){
							$colspanNama = "colspan='". (($level - $data->level)+1) ."'";
						}
						echo "<td class='borderbottom' style='text-align:right;' colspan='". $lvlSpan ."'>". $number .". </td>";
						echo "<td class='borderbottom' {$colspanNama} style='width:10%'>".$data->nama."</td>"; // ." | ".$data->level."|".$level
					}
					
					$subTotalRow = 0;
					$subTotal[$data->parent_kode]['jml'] += 1;
					foreach($caseQuery as $key => $val){
						if(strpos($key, 'jumlah') === false && $data->islast == 1){
						...
						} else if(strpos($key, 'jumlah') !== false && $data->islast == 1){
			...
			<tr>
				<?php 
					echo "<td class='borderbottom' align='right' style='font-weight:bold;'></td>
						<td class='borderbottom' align='right' style='font-weight:bold;' {$colspanHead} >TOTAL BIAYA</td>";
