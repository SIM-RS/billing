			...
			<tr>
				...
				<?php 
						...
						switch($param['type']){
							case "coloum-head":
								$sChild = "SELECT id, kode, nama, `level`, parent_id, parent_kode, islast, tipe, aktif 
											FROM ak_ms_unit_new WHERE parent_id = ".$param['parent_id']." AND aktif = 1";
					...
					$cHead = "SELECT id, kode, nama, `level`, parent_id, parent_kode, islast, tipe, aktif 
								FROM ak_ms_unit_new WHERE parent_id = 0 AND level = 1";
			...
			<?php
				...
				while($query && $data = mysql_fetch_object($query)){
					...
					if($data->level == 1 && $data->islast == 0){
						...
						$padding = (strlen($data->parent_kode) == 2 ? "5px;" : (strlen($data->parent_kode) == 5 ? "20px" : "30px"));
