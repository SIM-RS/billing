	...
	switch ($ccrvpbfumum){
		...
		case 2:
			...
			if ($rows=mysql_fetch_array($rscc)){
				...
				$kode_ma.=$kode_cc;
				$ma=trim($rows["nama"]);
				$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID INNER JOIN billing.b_ms_kso mu ON s.CC_RV_KSO_PBF_UMUM_ID=mu.id WHERE s.bulan=$bulan[0] AND s.tahun=$ta AND s.FK_MAID=$idma AND mu.id='$idccrvpbfumum'";
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$saldoawal=$rows1['SALDO_AWAL'];
				}
	...
	if ($ccrvpbfumum=="0"){
		...
		switch($ccrvpbfumum){
			case 1:
				...
				break;
			case 2:
				$strSQL="SELECT * FROM billing.b_ms_kso WHERE id>1 ORDER BY kode_ak";
				break;
			case 3:
