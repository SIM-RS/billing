...
if ($idccrvpbfumum!="0"){
	switch ($ccrvpbfumum){
		case 1:
			...
			if ($rows=mysql_fetch_array($rscc)){
				...
				$kode_ma.=$kode_cc;
				$ma=trim($rows["nama"]);
				$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID INNER JOIN ak_ms_unit mu ON s.CC_RV_KSO_PBF_UMUM_ID=mu.id WHERE s.bulan=$bulan[0] AND s.tahun=$ta AND s.flag = '$flag' AND s.FK_MAID=$idma AND mu.kode LIKE '$kode_cc%'";
				$rs1=mysql_query($sql);
		...
		case 2:
			...
			if ($rows=mysql_fetch_array($rscc)){
				...
				$kode_ma.=$kode_cc;
				$ma=trim($rows["nama"]);
				$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID INNER JOIN $dbbilling.b_ms_kso mu ON s.CC_RV_KSO_PBF_UMUM_ID=mu.id WHERE s.bulan=$bulan[0] AND s.tahun=$ta AND s.FK_MAID=$idma AND s.flag = '$flag' AND mu.id='$idccrvpbfumum'";
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$saldoawal=$rows1['SALDO_AWAL'];
				}
		...
		case 3:
			...
			if ($rows=mysql_fetch_array($rscc)){
				...
				$kode_ma.=$kode_cc;
				$ma=trim($rows["nama"]);
				$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID INNER JOIN $dbapotek.a_pbf mu ON s.CC_RV_KSO_PBF_UMUM_ID=mu.PBF_ID WHERE s.bulan=$bulan[0] AND s.tahun=$ta AND s.FK_MAID=$idma AND s.flag = '$flag' AND mu.PBF_ID='$idccrvpbfumum'";
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$saldoawal=$rows1['SALDO_AWAL'];
				}
		...
		case 4:
			...
			if ($rows=mysql_fetch_array($rscc)){
				...
				$kode_ma.=$kode_cc;
				$ma=trim($rows["nama"]);
				$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID INNER JOIN $dbaset.as_ms_rekanan mu ON s.CC_RV_KSO_PBF_UMUM_ID=mu.idrekanan WHERE s.bulan=$bulan[0] AND s.tahun=$ta AND s.flag = '$flag' AND s.FK_MAID=$idma AND mu.idrekanan='$idccrvpbfumum'";
				//echo $sql."<br>";
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$saldoawal=$rows1['SALDO_AWAL'];
	...
	$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo INNER JOIN ma_sak ON FK_MAID=MA_ID WHERE bulan=$bulan[0] AND tahun=$ta AND flag = '$flag' AND MA_KODE LIKE '$kode_ma%'";
	//echo $sql."<br>";
	$rs=mysql_query($sql);
...
//if (($cislast=="1" && $ccrvpbfumum=="0") || ($cc_islast=="1" && $ccrvpbfumum!="0")){
	...
	$worksheet->write($row, 0, "PT. PRIMA HUSADA CIPTA MEDAN", $sheetTitleFormat_TL);
	$row += 1;
	...
	$row += 1;
	$worksheet->write($row, 3, "BULAN ".strtoupper($bulan[1])." ".$ta, $sheetTitleFormat);
	$row += 1;
	...
	if ($ccrvpbfumum=="0"){
		if ($cma_islast==1){
			$strSQL="SELECT
					  ...
					  ma_sak.MA_KODE_VIEW,
					  ma_sak.MA_NAMA,
					  IFNULL((SELECT SALDO_AWAL FROM saldo WHERE BULAN=$bulan[0] AND TAHUN=$ta AND FK_MAID=$idma AND flag = '$flag'),0) AS SALDO_AWAL,
					  IFNULL(SUM(j.DEBIT),0) AS DEBET,
					  IFNULL(SUM(j.KREDIT),0) AS KREDIT
					FROM jurnal j
					  INNER JOIN ma_sak
						ON j.FK_SAK = ma_sak.MA_ID
					WHERE MA_ID = $idma AND j.flag = '$flag'
						AND MONTH(tgl) = $bulan[0]
						AND YEAR(tgl) = $ta";
		}else{
			...
			$strSQL="SELECT
					  ...
					  ma_sak.MA_KODE_VIEW,
					  ma_sak.MA_NAMA,
					  IFNULL((SELECT IFNULL(SUM(SALDO_AWAL),0) FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID WHERE s.BULAN=$bulan[0] AND s.flag = '$flag' AND s.TAHUN=$ta AND ma.MA_KODE LIKE CONCAT(ma_sak.MA_KODE,'%')),0) AS SALDO_AWAL,
					  IFNULL((SELECT IFNULL(SUM(j.DEBIT),0) FROM jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID WHERE MONTH(j.TGL)=$bulan[0] AND j.flag = '$flag' AND YEAR(j.TGL)=$ta AND ma.MA_KODE LIKE CONCAT(ma_sak.MA_KODE,'%')),0) AS DEBET,
					  IFNULL((SELECT IFNULL(SUM(j.KREDIT),0) FROM jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID WHERE MONTH(j.TGL)=$bulan[0] AND j.flag = '$flag' AND YEAR(j.TGL)=$ta AND ma.MA_KODE LIKE CONCAT(ma_sak.MA_KODE,'%')),0) AS KREDIT
					FROM ma_sak
					WHERE MA_PARENT = $idma
					ORDER BY ma_sak.MA_KODE";
		}
	}else{
		$strSQL="Select ma_sak.MA_ID, ma_sak.MA_KODE, ma_sak.MA_NAMA,ma_sak.MA_PARENT,1 as MA_ISLAST,date_format(TGL,'%d/%m/%Y') as tgl1,j.TR_ID,j.NO_TRANS,j.TGL,j.NO_KW,j.URAIAN,j.DEBIT,j.KREDIT,j.D_K From jurnal j Inner Join ma_sak ON j.FK_SAK = ma_sak.MA_ID where MA_ID = $idma AND j.flag = '$flag' AND j.CC_RV_KSO_PBF_UMUM_ID='$idccrvpbfumum' and month(tgl)=$bulan[0] and year(tgl)=$ta order by TGL,TR_ID";
	}
	//echo $strSQL."<br>";
	...
	while ($rows=mysql_fetch_array($rs)){
		...
		$worksheet->write($row, 4, $rows['KREDIT'], $regularFormatRF_L);
