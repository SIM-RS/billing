...
while ($rows=mysql_fetch_array($rs)){
	if ($rows['MA_LEVEL']==1){
		if ($num>0){
			...
			$ttot=(($stot1tmp<0)?"(". -$stot1tmp.")":$stot1tmp);
			$worksheet->write($row, 3, $ttot, $sTotFormatR);			
			$worksheet->write($row+2, 1, "TOTAL $tmpTotCOA", $sTotFormatC);
			$worksheet->write($row+2, 2, "", $sTotFormatR);
			$ttot=(($stot1<0)?"(". -$stot1.")":$stot1);
			$worksheet->write($row+2, 3, $ttot, $sTotFormatR);
			if ($tgl2=="1"){
				$ttot=(($stot2tmp<0)?"(". -$stot2tmp.")":$stot2tmp);
				$worksheet->write($row, 4, $ttot, $sTotFormatR);
				$ttot=(($stot2<0)?"(". -$stot2.")":$stot2);
				$worksheet->write($row+2, 4, $ttot, $sTotFormatR);
			}
			$row +=4;
		}
		$num=0;
		...
		if ($tgl2=="1"){
			...
			$stot2 +=$nilai2;
			$stot2tmp +=$nilai2;
			$ttot=(($nilai1<0)?"(". -$nilai1.")":$nilai1);
			$worksheet->write($row, 3, $ttot, $regularFormatR);
			$ttot=(($nilai2<0)?"(". -$nilai2.")":$nilai2);
			$worksheet->write($row, 4, $ttot, $regularFormatR);
		}else{
			$mkode=$rows['MA_KODE'];
			$sql="select sum(SALDO_AWAL) as saldo from saldo s inner join ma_sak m on s.FK_MAID=m.MA_ID where s.BULAN=$bln1[0] and s.TAHUN=$ta1 and m.MA_KODE like '$mkode%' group by s.FK_MAID";
			$rs1=mysql_query($sql);
...
$ttot=(($tot1<0)?"(". -$tot1.")":$tot1);
$worksheet->write($row+4, 3, $ttot, $sTotFormatR);
if ($tgl2=="1"){
	$ttot=(($stot2tmp<0)?"(". -$stot2tmp.")":$stot2tmp);
	$worksheet->write($row, 4, $ttot, $sTotFormatR);
	$ttot=(($stot2<0)?"(". -$stot2.")":$stot2);
	$worksheet->write($row+2, 4, $ttot, $sTotFormatR);
	$ttot=(($tot2<0)?"(". -$tot2.")":$tot2);
	$worksheet->write($row+4, 4, $ttot, $sTotFormatR);
}
