	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Pembelian Obat</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td height="30" style="padding-left:45px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="changePost(this.value);kirim()">
                        <option value="0">BELUM POSTING</option>
                        <option value="1">SUDAH POSTING</option>
                    </select></td>
                    <td height="30" align="right" style="padding-right:45px;"><BUTTON id="btnPost" type="button" onClick="PostingJurnal()"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal</BUTTON></td>
                </tr>
                <tr>
...
<script>
 	function kirim(){
	var url;
		url="../unit/utils.php?grd1=true&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value+"&posting="+document.getElementById('cmbPost').value;
	...
	var tmp='';
	var url;
		url="../unit/utils.php?grd1=true&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		
		for (var i=0;i<aGrid.getMaxRow();i++){
			if (aGrid.obj.childNodes[0].childNodes[i].childNodes[8].childNodes[0].checked){
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/utils.php?grd1=true&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+aGrid.getFilter()+"&sorting="+aGrid.getSorting()+"&page="+aGrid.getPage();
			//alert(url);
			aGrid.loadURL(url,"","GET");
		}
	}
	...
	aGrid.setColHeader("NO,TANGGAL,NO GUDANG,NO FAKTUR,PBF,DPP,PPN,DPP + PPN,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
	aGrid.setIDColHeader(",tgl1,noterima,nobukti,b.pbf_nama,,,,");
	aGrid.setColWidth("30,75,150,130,200,100,100,100,75");
	aGrid.setCellAlign("center,center,center,center,left,right,right,right,center");
	aGrid.setCellType("txt,txt,txt,txt,txt,txt,txt,txt,chk");
	aGrid.setCellHeight(20);
	aGrid.setImgPath("../icon");
	aGrid.setIDPaging("paging");
	aGrid.onLoaded(konfirmasi);
	//a.attachEvent("onRowClick");
	//alert("../unit/utils.php?grd1=true&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value);
	aGrid.baseURL("../unit/utils.php?grd1=true&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value);
	aGrid.Init();
</script>
</body>
</html>