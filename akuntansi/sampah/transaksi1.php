				...
				if (($fk_TipeTrans==0) || ($fk_TipeTrans=="0")){
					...
					if($nilai1>0 OR $nilai2>0){
						...
						if ($rsrow=mysql_fetch_array($rs)){
							...
							$tmaxnotrans++;
							$sql2="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID) values($tnotrans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],'$id_CC[$i]')";
							//echo $sql2."<br>";
							$rs1=mysql_query($sql2);
		...
		if ($cdata=="1"){
			...
			for ($i = 0; $i < count($nilai); $i++){
				...
				if($nilai1>0 OR $nilai2>0){
					...
					$sql="insert into JURNAL(no_trans,fk_sak,tgl,no_kw,uraian,debit,kredit,tgl_act,fk_iduser,d_k,JENIS,fk_trans,FK_LAST_TRANS,CC_RV_KSO_PBF_UMUM_ID) values($no_trans,$id_sak[$i],'$tgl','$nokw','$uraian',$nilai1,$nilai2,'$sekarang',$iduser,'$dk[$i]',$fk_TipeTrans,$fk_jnsTrans,$pid_jnsTrans[$i],$id_CC[$i])";
					//echo $sql."<br>";
					$rs=mysql_query($sql);
					//echo mysql_error()."<br>";
				}
...
<div id="popGrPet" style="display:none; width:500px;height:auto" class="popup">
        ...
        <tr>
            ...
            <td>:&nbsp;<input type="text" id="newKRek" readonly="readonly" size="50" value="Pilih Kode Rekening" />&nbsp;<input type="button" value=".." onClick="OpenWnd('tree_ma_ajax.php?par=<?php echo $vpar; ?>',800,500,'mstre',true)" /></td>
        </tr>
        <tr>
...
<div id="input" style="display:block;">
	...
	?>
	  ...
	  $sql="SELECT *, detil_transaksi.fk_jenis_trans AS Expr1, ma_sak.MA_KODE AS MA_SAK_KODE, ma_sak.MA_NAMA AS MA_SAK_NAMA, jenis_transaksi.JTRANS_NAMA AS Expr4, jenis_transaksi.JTRANS_ID AS Expr5, detil_transaksi.fk_ma_sak AS Expr6, detil_transaksi.dk AS Expr7, detil_transaksi.id_detil_trans AS Expr8, detil_transaksi.*
			FROM detil_transaksi INNER JOIN jenis_transaksi ON detil_transaksi.fk_jenis_trans = jenis_transaksi.JTRANS_ID INNER JOIN ma_sak ON detil_transaksi.fk_ma_sak = ma_sak.MA_ID WHERE jenis_transaksi.JTRANS_KODE LIKE '$fk_kodeTrans%' order by detil_transaksi.id_detil_trans";
		if (($arfvalue=='edit') && ($jml!=$jmle)){
			if ($fk_TipeTrans==0)
	...
	?>
		...
		while ($show3=mysql_fetch_array ($exe3)){
			...
			while ($show = mysql_fetch_array ($exe)) {
				...
				switch ($show["cc_rv_kso_pbf_umum"]){
					...
					case 1:
						...
						break;
					case 2:
						$psql="SELECT * FROM billing.b_ms_kso WHERE id='".$show["id_cc_rv_kso_pbf_umum"]."'";
						//echo $psql."<br>";
						$rsCC=mysql_query($psql);
						$rwCC=mysql_fetch_array($rsCC);
						$idCC=$rwCC["id"];
            ...
            <tr class="itemtable"> 
              ...
              <td class="tdisikiri"> 
                <?php echo $i++; ?>              </td>
              <td class="tdisi" align="center"> <input class="txtinput" type="<?php if (($show['MA_ISLAST']==0 || ($show['CC_RV_KSO_PBF_UMUM']>0 && $idCC==0))){echo 'text';}else{echo 'hidden';}?>" name="kode_sak[]" id="kode_sak" value="<?=$kdma;?>" size="8" readonly="true"/> 
                <?php if (($show['MA_ISLAST']==1 && $show['CC_RV_KSO_PBF_UMUM']==0) || ($fk_TipeTrans>0) || ($show['CC_RV_KSO_PBF_UMUM']>0 && $idCC!=0)) echo $kdma;?>
                <?php if (($show['MA_ISLAST']==0 || ($show['CC_RV_KSO_PBF_UMUM']>0 && $idCC==0))){?>
                ...
                <?php }?>              </td>
              <td class="tdisi"><?php if ($show['MA_ISLAST']==1) echo $namasak;?><? if (($show['MA_ISLAST']==0 || ($idCC==0 && $show['CC_RV_KSO_PBF_UMUM']>0))) echo "<br>";?><input type="<?php if (($show['MA_ISLAST']==0 || ($show['CC_RV_KSO_PBF_UMUM']>0 && $idCC==0))){echo 'text';}else{echo 'hidden';}?>" class="txtinput" name="nama_sak[]" id="nama_sak" value="<?=$namasak1;?>" readonly="true" size="35" />              </td>
              <td class="tdisi"><?php if ($idCC!=0 && $ketCC!=""){echo $ketCC;}else{if ($fk_TipeTrans==0){if ($show['MA_ISLAST']==1) echo $show['JTRANS_NAMA']; else echo $show['JTRANS_NAMA']."<br>(".$show['MA_SAK_NAMA'].")";}}?>&nbsp;</td>
              <td class="tdisi"> 
...
<div id="listjurnal" style="margin-top:15px"><br>
    <table width="99%" border="0" cellpadding="2" cellspacing="0">
      <tr> 
        <td colspan="11"><div align="center"><span class="jdltable">Laporan Journal 
            </span></div></td>
      </tr>
      <tr> 
        <td colspan="11">&nbsp;</td>
      ...
      <tr class="headtable">
        ...
        <td id="MA_KODE" width="70" class="tblheader" onClick="ifPop.CallFr(this);">Kode 
          Rek.</td>
        <td width="250" class="tblheader" id="MA_NAMA" onClick="ifPop.CallFr(this);">MA 
          SAK</td>
	  ...
	  $sqlQ="Select jurnal.*, ma_sak.MA_KODE, ma_sak.MA_NAMA, ma_sak.FK_MA, ma_sak.CC_RV_KSO_PBF_UMUM From jurnal Inner Join ma_sak ON jurnal.FK_SAK = ma_sak.MA_ID where TGL between '$tgl_1' and '$tgl_2'".$fuser.$fver.$filter." order by ".$sorting;
	  //echo $sqlQ."<br>";
		$rs=mysql_query($sqlQ);
	  ...
	  while ($rows=mysql_fetch_array ($rs)){
		...
		switch ($ccrvpbf){
			case 1:
				...
				break;
			case 2:
				$sql="SELECT * FROM billing.b_ms_kso WHERE id=".$rows['CC_RV_KSO_PBF_UMUM_ID'];
				$rscc=mysql_query($sql);
				$rwcc=mysql_fetch_array($rscc);
				$ketcc=" - ".$rwcc["nama"];
      ...
      <tr class="itemtable" onMouseOver="this.className='itemtableMOver'" onMouseOut="this.className='itemtable'">
        ...
        <td class="tdisi"><?php echo $rows['NO_TRANS']; ?></td>
		...
		?>
        <td width="25" class="tdisi"><IMG SRC="../icon/edit.gif" border="0" width="16" height="16" ALIGN="absmiddle" class="proses" title="Klik Untuk Mengubah Data Transaksi" onClick="location='?f=transaksi&fk_jnsTrans=<?=$ijtrans;?>&fk_kodeTrans=<?=$exe['JTRANS_KODE'];?>&arfvalue=edit&notrans=<?=$rows['NO_TRANS'];?>&pid_jnsTrans=<?php echo $rows['FK_LAST_TRANS']; ?>&nokw=<?php echo $rows['NO_KW']; ?>&page=<?php echo $page; ?>&tgl_s=<?=$tgl_s;?>&tgl_d=<?=$tgl_d;?>&kodepilih=<?php echo $kodepilih; ?>&cuser=<?php echo $cuser; ?>';"></td>
        <td width="25" class="tdisi"><IMG SRC="../icon/del.gif" border="0" width="16" height="16" ALIGN="absmiddle" class="proses" title="Klik Untuk Menghapus Data Transaksi" onClick="if (confirm('Yakin Ingin Menghapus Data Dengan No. Transaksi <?=$rows['NO_TRANS'];?> ?')){fSetValue(window,'<?php echo $arfhapus; ?>');document.form1.submit();}"></td>
      </tr>
      <?php 
	  }
