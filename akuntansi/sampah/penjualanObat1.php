...
<script>
	...
	a1.setColHeader("NO,KSO,CARA BAYAR,UNIT PELAYANAN,SHIFT,NILAI JUAL,NILAI SLIP,NILAI HPP,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setSubTotal(",,,SubTotal :&nbsp;,,0,0,0,");
	a1.setIDColHeader(",KSO_NAMA,cara_bayar,nama,SHIFT,,,,");
	a1.setColWidth("50,170,90,180,50,90,90,90,50");
	...
	//alert("../unit/utils.php?grd=penjualanObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value);
	a1.baseURL("../unit/utils.php?grd=penjualanObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value);
	a1.Init();
	...
	function kirim(){
	var url;
		url="../unit/utils.php?grd=penjualanObat&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&cmbFarmasi="+document.getElementById('cmbFarmasi').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var url;
		...
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[8].childNodes[0].checked){
				...
				tmp+=idata[0]+String.fromCharCode(5)+idata[1]+String.fromCharCode(5)+idata[2]+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,6))+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,8))+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,7))+String.fromCharCode(5)+idata[3]+String.fromCharCode(5)+idata[4]+String.fromCharCode(6);
				tmp2+=idata[0]+"|"+idata[1]+"|"+idata[2]+"|"+ValidasiText(a1.cellsGetValue(i+1,5))+"|"+ValidasiText(a1.cellsGetValue(i+1,7))+"|"+ValidasiText(a1.cellsGetValue(i+1,6));
			}
		}
		if (tmp!=""){
