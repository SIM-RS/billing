...
INNER JOIN ma_sak ma ON dt.fk_ma_sak = ma.MA_ID WHERE jt.JTRANS_ID = ".$id_trans."";
				*/
				...
				while($rw=mysql_fetch_array($q)){
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) VALUES($notrans,$id_sak,'$tanggal','$no_kw','$uraian','$debet','$kredit',now(),$idUser,'$d_k',0,'$jenistrans','$jenistrans',1,'$no_bukti',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}
	...
	VALUES($notrans,488,'$tanggal','$nokw','$uraian',0,$cdata[1],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
						$rsPost=mysql_query($sql);
					
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,44,'$tanggal','$nokw','$uraian',$cdata[1],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$kso,1)";
						$rsPost=mysql_query($sql);
					}else{						
						if ($ckso_type==1){
							$cfksak_pendptn=493;
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$cfksak_pendptn,'$tanggal','$nokw','$uraian',0,$cdata[1],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
						$rsPost=mysql_query($sql);
					
						if ($cdata[3]>0){
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$cfksak_bkso,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$kso,1)";
							$rsPost=mysql_query($sql);
						}
						if ($iurPx>0){
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$cfksak_bpasien,'$tanggal','$nokw','$uraian',$iurPx,0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$kso,1)";
							$rsPost=mysql_query($sql);
						}
						if ($cselisih>0){
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$cfksak_selisih,'$tanggal','$nokw','$uraian',$cselisihD,$cselisihK,now(),$idUser,'$dk',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
							$rsPost=mysql_query($sql);
						}
					}
				}else{
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$sql = "SELECT 
					  ...
					  rv.id 
					FROM
					  akuntansi.ak_posting p 
					  INNER JOIN ".$dbbilling.".b_ms_unit mu 
						ON mu.id = p.id_cc_rv 
					  INNER JOIN akuntansi.ak_ms_unit rv 
				...
				$sUnit = "SELECT rv.* FROM ".$dbbilling.".b_ms_unit mu INNER JOIN akuntansi.ak_ms_unit rv ON mu.kode_ak=rv.kode WHERE mu.id='".$id_unit."'";
				$qUnit = mysql_query($sUnit);
				$rUnit = mysql_fetch_array($qUnit);
				$unit = $rUnit['nama'];
					
				$sDel = "DELETE FROM akuntansi.ak_posting WHERE id=".$cdata[0];
				mysql_query($sDel);
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_errno()<=0){
					...
					if ($kso==1){
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[1],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',0,1)";
						$rsPost=mysql_query($sql);

						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_pend,'$tanggal','$nokw','$uraian',$iurPx,0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
						$rsPost=mysql_query($sql);
					}else{
						if ($bayar==1){
							$id_sak=8;
							...
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
			VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$iurPx,now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',0,1)";
							$rsPost=mysql_query($sql);
							//if ($iurPx>0){
								$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
			VALUES($notrans,$id_pend_kso,'$tanggal','$nokw','$uraian',$iurPx,0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
								$rsPost=mysql_query($sql);
							//}
						}else{
							$id_sak=45;
							...
							$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
			VALUES($notrans,$id_pend_kso,'$tanggal','$nokw','$uraian',$cdata[1],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
							$rsPost=mysql_query($sql);
							if ($cdata[3]>0){
								$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
			VALUES($notrans,$id_sak_kso,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
								$rsPost=mysql_query($sql);
							}
							if ($iurPx>0){
								$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
			VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$iurPx,now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
								$rsPost=mysql_query($sql);
							}
							if ($cselisih>0){
								$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
			VALUES($notrans,491,'$tanggal','$nokw','$uraian',$cselisihD,$cselisihK,now(),$idUser,'$dk',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
								$rsPost=mysql_query($sql);
							}
						}
					}
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$sql = "SELECT * FROM akuntansi.ak_posting WHERE id=".$cdata[0];
				$kueri = mysql_query($sql);
				$row = mysql_fetch_array($kueri);
		...
		if ($posting==0){
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_errno()<=0){
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',$cdata[2],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_piutang,'$tanggal','$nokw','$uraian',0,$cdata[2],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_Bank_BLUD,'$tanggal','$nokw','$uraian',$cdata[2],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[2],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}else{
					$statusProses='Error';
	...
	VALUES($notrans,$id_piutang,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_Bank_BLUD,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}else{
					$statusProses='Error';
	...
	case "postingKlaimKSO":
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_errno()<=0){
					...
					if (mysql_errno()<=0){
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,8,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
						$rsPost=mysql_query($sql);
							
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
						$rsPost=mysql_query($sql);
		...
		VALUES($notrans,$id_sak_Bank_BLUD,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,8,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',0,1)";
						$rsPost=mysql_query($sql);
					}else{
						$statusProses='Error';
					}
	...
	case "postingPenLain":
		...
		VALUES($notrans,8,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$id_cc_rv,1)";
						//echo $sql."<br>";
						$rsPost=mysql_query($sql);
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				if ($rwBeli=mysql_fetch_array($rsBeli)){
					...
					if (mysql_errno()<=0){
						...
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$fksak,'$cdata[1]','$cdata[3]','$pbfNama',$biayaRS,0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',0,1)";
						//echo $sql."<br>";
						$rsPost=mysql_query($sql);
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$k_sak,'$cdata[1]','$cdata[3]','$pbfNama',0,$biayaRS,now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$cdata[0],1)";
						//echo $sql."<br>";
						$rsPost=mysql_query($sql);
					}else{
						$statusProses='Error';
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$sJ = "DELETE FROM akuntansi.jurnal WHERE TGL = '".$cdata[1]."' AND NO_KW='".$cdata[3]."'";
				mysql_query($sJ);
			}
			$actPosting='Unposting';
		}
	...
	case "postingPenjObat":
		...
		if ($posting==0){
			for ($i=0;$i<count($arfdata);$i++){
				...
				//echo $sql."<br>";
				$sql="UPDATE keuangan.k_transaksi SET posting=1 WHERE id=".$id_transaksi;
				$rsPost=mysql_query($sql);
				if (mysql_errno()<=0){
					//======insert into jurnal========
					...
					$uraian="Pemakaian Obat : ".$unit." - ".$rwCek["nama"]." - ".$unitFarm. " - Shift ".$shift;
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$kso,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_pend,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
					
					if ($id_sak==8){
						$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
		VALUES($notrans,$id_sak_Bank_BLUD,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'','$kso',1)";
						//echo $sql."<br>";
						$rsPost=mysql_query($sql);
		...
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'','$kso',1)";
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_biaya,'$tanggal','$nokw','$uraian',$cdata[4],0,now(),$idUser,'D',0,$jenistrans_jual,$jenistrans_jual,1,'',$idunit,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_stok,'$tanggal','$nokw','$uraian',0,$cdata[4],now(),$idUser,'K',0,$jenistrans_jual,$jenistrans_jual,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
		...
		VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[2],now(),$idUser,'K',0,$jenistrans_byr,$jenistrans_byr,1,'',$kso,1)";
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				*/
				...
				$sDel = "UPDATE keuangan.k_transaksi SET posting=0 WHERE id=".$id_transaksi;
				mysql_query($sDel);
				...
				$sJ = "DELETE FROM jurnal WHERE TGL = '$tanggal' AND FK_SAK IN ($id_sak,$id_sak_pend,$id_sak_Bank_BLUD) AND FK_TRANS = $jenistrans AND (DEBIT='$cdata[3]' OR KREDIT='$cdata[3]') AND URAIAN like '$uraian'";
				mysql_query($sJ);
				//echo $sJ."<br>";
		...
		if ($posting==0){
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_errno()<=0){
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',0,$cdata[2],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',$kso,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);

					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_pend,'$tanggal','$nokw','$uraian',$cdata[2],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$idunit,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_biaya,'$tanggal','$nokw','$uraian',0,$cdata[3],now(),$idUser,'K',0,$jenistrans_return,$jenistrans_return,1,'',$idunit,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_stok,'$tanggal','$nokw','$uraian',$cdata[3],0,now(),$idUser,'D',0,$jenistrans_return,$jenistrans_return,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
		...
		if ($posting==0){
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_errno()<=0){
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_biaya,'$tanggal','$nokw','$uraian',$cdata[2],0,now(),$idUser,'D',0,$jenistrans_fs,$jenistrans_fs,1,'',$idunit,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_stok,'$tanggal','$nokw','$uraian',0,$cdata[2],now(),$idUser,'K',0,$jenistrans_fs,$jenistrans_fs,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}else{
					$statusProses='Error';
		...
		if ($posting==0){
			for ($i=0;$i<count($arfdata);$i++){
				...
				if (mysql_errno()<=0){
					...
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak,'$tanggal','$nokw','$uraian',$cdata[4],0,now(),$idUser,'D',0,$jenistrans,$jenistrans,1,'',$id_kso_pbf_umum,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
					$sql="INSERT INTO ".$dbakuntansi.".jurnal(NO_TRANS,FK_SAK,TGL,NO_KW,URAIAN,DEBIT,KREDIT,TGL_ACT,FK_IDUSER,D_K,JENIS,FK_TRANS,FK_LAST_TRANS,STATUS,NO_BUKTI,CC_RV_KSO_PBF_UMUM_ID,POSTING) 
	VALUES($notrans,$id_sak_stok,'$tanggal','$nokw','$uraian',0,$cdata[4],now(),$idUser,'K',0,$jenistrans,$jenistrans,1,'',0,1)";
					//echo $sql."<br>";
					$rsPost=mysql_query($sql);
				}else{
					$statusProses='Error';
	...
	case "VerifikasiPengeluaranLain":
		...
		if ($posting==0){
			...
			for ($i=0;$i<count($arfdata);$i++){
				...
				$cdata=explode(chr(5),$arfdata[$i]);
				$sel = "select * from keuangan.k_transaksi where id='$cdata[0]'";
				$qu = mysql_query($sel);
...
FROM keuangan.k_transaksi t LEFT JOIN keuangan.k_transaksi_detail td ON t.id=td.transaksi_id 
WHERE t.id='$cdata[0]' AND (td.pajak_id=0 OR td.pajak_id IS NULL)";
					...
					if ($arfdata[0]!=""){
						...
						for ($i=0;$i<count($arpajak);$i++){
							...
							$sql="delete from k_transaksi_detail where transaksi_id='$cdata[0]'";
							mysql_query($sql);
							$nilai2 +=$nilai1;
						}
						...
						if ($TglBayarPajak!=""){
							...
							for ($i=0;$i<count($arpajak);$i++){
								...
								$sql="delete from k_transaksi_detail where transaksi_id='$id'";
								mysql_query($sql);
								$nilai2 +=$nilai1;
							}
		...
		case "ReturnPelayanan":
			...
			$sql="SELECT 
				...
				FROM
				  ...
				  FROM
					billing.b_return r 
					INNER JOIN billing.b_bayar_tindakan bt 
					  ON bt.id = r.bayar_tindakan_id 
					INNER JOIN billing.b_tindakan t 
					  ON t.id = bt.tindakan_id 
					INNER JOIN billing.b_ms_tindakan_kelas mtk 
					  ON mtk.id = t.ms_tindakan_kelas_id 
					INNER JOIN billing.b_ms_tindakan mt 
					  ON mt.id = mtk.ms_tindakan_id 
					INNER JOIN billing.b_pelayanan pl 
					  ON pl.id = t.pelayanan_id 
					INNER JOIN billing.b_ms_pasien ps 
					  ON ps.id = pl.pasien_id 
					INNER JOIN billing.b_ms_unit u 
					  ON u.id = pl.unit_id 
					INNER JOIN billing.b_ms_kso kso 
					  ON kso.id = pl.kso_id 
				  WHERE bt.tipe = 0 
					$ftgl
					$fkso
				  ...
				  SELECT
					...
					SUM(bt.nilai) AS nilai 
				  FROM
					billing.b_pelayanan p 
					INNER JOIN billing.b_ms_unit mu 
					  ON p.unit_id = mu.id 
					INNER JOIN billing.b_tindakan_kamar t 
					  ON p.id = t.pelayanan_id 
					LEFT JOIN billing.b_ms_unit n 
					  ON n.id = mu.parent_id 
					INNER JOIN billing.b_ms_kamar tk 
					  ON tk.id = t.kamar_id 
					INNER JOIN billing.b_bayar_tindakan bt 
					  ON bt.tindakan_id = t.id 
					INNER JOIN billing.b_tindakan ti 
					  ON ti.id = bt.tindakan_id 
					INNER JOIN billing.b_ms_tindakan_kelas btk 
					  ON btk.id = ti.ms_tindakan_kelas_id 
					INNER JOIN billing.b_ms_tindakan mt 
					  ON mt.id = btk.ms_tindakan_id 
					INNER JOIN billing.b_ms_kelas mk 
					  ON mk.id = btk.ms_kelas_id 
					INNER JOIN billing.b_return r 
...
WHERE t.kso_id='$kso' AND t.tgl='$tanggal' AND t.tipe_pendapatan=0 GROUP BY p.unit_id) AS t4 ON t3.id=t4.unit_id ".$filter." ORDER BY $sorting";
		...
		case "pembelianObat":
			...
			if ($posting==0){
				...
				if ($tipe!=0){
					...
					$sql2="SELECT * FROM (SELECT b.*,c.id FROM (SELECT data.PBF_ID,data.TANGGAL,data.tgl1, data.noterima, data.nobukti, data.nobukti no_faktur, data.KET no_spk, data.pbf_nama, SUM(data.dpp) AS dpp, 
		...
		WHERE id IS NULL) as t1 ".$filter;
				...
				$sql2="SELECT * FROM (SELECT p.*,p.no_terima noterima,p.no_bukti no_spk,DATE_FORMAT(p.tgl_trans,'%d/%m/%Y') as tgl1,p.tgl_trans AS TANGGAL,pbf.pbf_nama FROM ak_posting p INNER JOIN ".$dbapotek.".a_pbf pbf ON p.id_kso_pbf_umum=pbf.PBF_ID WHERE p.tipe=3".$ftipe." AND p.tgl_trans BETWEEN '$tanggalAwal' AND '$tanggalAkhir') as t1 $filter";
					...
					FROM
					  ...
					  LEFT JOIN 
						...
						FROM
						  akuntansi.ak_posting 
						WHERE tipe = 4 
						  AND tgl_trans = '$tanggal') ap 
						ON (
						  tbl.unit_billing = ap.id_cc_rv 
						  AND tbl.kso_id_billing = ap.id_kso_pbf_umum 
						  AND tbl.CARABAYAR_ID = ap.jenis
						) 
					WHERE ap.id IS NULL";*/
				
				/*
				$sql="SELECT * FROM (SELECT gab3.* FROM (SELECT gab2.*,gab1.KSO_NAMA,gab1.nama,gab1.kso_id_billing,gab1.kode_ak FROM (SELECT am.IDMITRA,am.kso_id_billing,am.NAMA KSO_NAMA,mu.id,mu.nama,mu.kode_ak FROM ".$dbapotek.".a_mitra am,
(SELECT id,nama,IFNULL(kode_ak,'') kode_ak FROM ".$dbbilling.".b_ms_unit mu WHERE mu.kategori=2 AND mu.level=2 
UNION SELECT 126,'Instalasi Farmasi','0502') mu) AS gab1,
(SELECT t1.KSO_ID,t1.CARABAYAR_ID,t1.cara_bayar,IFNULL(t1.unit_billing,126) unit_billing,SUM(t1.tot_jual) AS nJual,SUM(nBahan) nBahan 
FROM (SELECT TGL,TGL_ACT,NO_PENJUALAN,SHIFT,ap.KSO_ID,am.NAMA,ak.ID AS KPID,ak.NAMA AS KPNAMA, 
ac.id AS CARABAYAR_ID,ac.nama AS cara_bayar,u.unit_billing,FLOOR(HARGA_TOTAL) tot_jual, 
(ap.QTY_JUAL * IF(((p.`TIPE_TRANS` = 4) OR (p.`TIPE_TRANS` = 5) 
OR (p.`TIPE_TRANS` = 100) OR (p.`NILAI_PAJAK` <= 0)),(p.HARGA_BELI_SATUAN * (1-(p.DISKON/100))),
1.1 * (p.HARGA_BELI_SATUAN * (1-(p.DISKON/100))))) nBahan 
FROM ".$dbapotek.".a_penjualan ap INNER JOIN ".$dbapotek.".a_mitra am ON ap.KSO_ID=am.IDMITRA 
INNER JOIN ".$dbapotek.".a_penerimaan p ON ap.PENERIMAAN_ID=p.ID INNER JOIN ".$dbapotek.".a_cara_bayar ac ON ap.CARA_BAYAR=ac.id 
INNER JOIN ".$dbapotek.".a_kepemilikan ak ON ap.JENIS_PASIEN_ID=ak.ID LEFT JOIN ".$dbapotek.".a_unit u ON ap.RUANGAN=u.UNIT_ID 
WHERE ap.UNIT_ID=".$cmbFarmasi." AND ap.TGL = '$tanggal'".$fkso."
GROUP BY SHIFT,NO_PENJUALAN,TGL,ap.KSO_ID,ap.UNIT_ID,ap.JENIS_PASIEN_ID,ap.CARA_BAYAR,ap.NO_PASIEN,u.unit_billing ORDER BY TGL_ACT DESC) AS t1 
GROUP BY t1.TGL,t1.KSO_ID,t1.SHIFT,t1.NAMA,t1.KPNAMA,t1.cara_bayar,t1.unit_billing) AS gab2 
WHERE gab1.IDMITRA=gab2.KSO_ID AND gab1.id=gab2.unit_billing) gab3 
LEFT JOIN (SELECT * FROM ak_posting WHERE tipe=4 AND tgl_trans='$tanggal') ap 
ON (gab3.unit_billing=ap.id_cc_rv AND gab3.kso_id_billing=ap.id_kso_pbf_umum AND gab3.CARABAYAR_ID=ap.jenis)
WHERE ap.id IS NULL) gab".$filter." ORDER BY ".$sorting;
				*/
			//}else{
				//$sql = "SELECT ap.*,mu.nama FROM ak_posting ap INNER JOIN ak_ms_unit mu ON ap.id_cc_rv=mu.id WHERE ap.tipe=4 AND ap.tgl_trans='$tanggal'".$fkso;
				
				/*
				$sql="SELECT ap.id,ap.jenis CARABAYAR_ID,cb.nama cara_bayar,ap.biayaRS_DPP_PPN nJual,ap.biayaPx_PPN nBahan,ap.biayaKSO_DPP nSlip,mu.nama,kso.nama KSO_NAMA,ap.id_cc_rv as unit_billing,ap.id_kso_pbf_umum as kso_id_billing
FROM ak_posting ap INNER JOIN billing.b_ms_unit mu ON ap.id_cc_rv=mu.id 
INNER JOIN billing.b_ms_kso kso ON ap.id_kso_pbf_umum=kso.id INNER JOIN dbapotek.a_cara_bayar cb ON ap.jenis=cb.id 
WHERE ap.tipe=4 AND ap.tgl_trans='$tanggal'".$fkso;
				*/
...
WHERE p.tipe=15 AND MONTH(p.tgl_trans)=$bulan AND YEAR(p.tgl_trans)=$tahun";
	...
	AND t.kso_id = '".$kso."' AND t.aktif=1) AS t1) AS t2";
		...
		case "penjualanObat":
			...
			if ($posting==0){
				while ($rows=mysql_fetch_array($rs)) {
					$sql="SELECT * FROM ak_posting WHERE tipe=4 AND id_cc_rv=".$rows["id"]." AND id_kso_pbf_umum=$kso AND tgl_trans='$tanggal'";
					$rsCek=mysql_query($sql);
					if (mysql_num_rows($rsCek)<=0){
						$rwCek=mysql_fetch_array($rsCek);
						$sqlJual="SELECT SUM(t1.tot_jual) AS nJual,SUM(nBahan) nBahan FROM 
(SELECT TGL,TGL_ACT,NO_PENJUALAN,SHIFT,ap.KSO_ID,am.NAMA,ak.ID AS KPID,ak.NAMA AS KPNAMA,
ac.id AS CARABAYAR_ID,ac.nama AS cara_bayar,FLOOR(HARGA_TOTAL) tot_jual,
(ap.QTY_JUAL * IF(p.NILAI_PAJAK=0,(p.HARGA_BELI_SATUAN * (1-(p.DISKON/100))),1.1 * (p.HARGA_BELI_SATUAN * (1-(p.DISKON/100))))) nBahan 
FROM ".$dbapotek.".a_penjualan ap INNER JOIN ".$dbapotek.".a_mitra am ON ap.KSO_ID=am.IDMITRA 
INNER JOIN ".$dbapotek.".a_penerimaan p ON ap.PENERIMAAN_ID=p.ID
INNER JOIN ".$dbapotek.".a_cara_bayar ac ON ap.CARA_BAYAR=ac.id 
INNER JOIN ".$dbapotek.".a_kepemilikan ak ON ap.JENIS_PASIEN_ID=ak.ID INNER JOIN ".$dbapotek.".a_unit u ON ap.RUANGAN=u.UNIT_ID 
WHERE ap.UNIT_ID=$cmbFarmasi AND u.kdunitfar='".$rows["kode"]."' AND ap.TGL = '$tanggal' AND ap.KSO_ID='".$rwKsoF["id"]."'
...
GROUP BY t1.TGL,t1.SHIFT,t1.NAMA,t1.KPNAMA,t1.cara_bayar";
						...
						if ($nJual>0){
							...
							$ntot+=$nJual;
							$ntot1+=$bJual;
							$dt.=$rows["id"]."|".$tanggal.chr(3).$i.chr(3).$rows["nama"].chr(3).number_format($nJual,0,",",".").chr(3).number_format($bJual,0,",",".").chr(3)."0".chr(6);
						}
					}
				}
			}else{
				while ($rows=mysql_fetch_array($rs)) {
					...
					if (mysql_num_rows($rsCek)>0){
						$rwCek=mysql_fetch_array($rsCek);
						$nJual=$rwCek["biayaPx_PPN"];
						$bJual=$rwCek["biayaRS_DPP_PPN"];
			
						if ($nJual>0){
							$i++;
							$ntot+=$nJual;
							$ntot1+=$bJual;
							$dt.=chr(3).$i.chr(3).$rows["nama"].chr(3).number_format($nJual,0,",",".").chr(3).number_format($bJual,0,",",".").chr(3)."0".chr(6);
						}
					}
				}
			}*/
			...
			while ($rows=mysql_fetch_array($rs)) {
				...
				$cid=$cidBilling."|".$rows["kso_id_billing"]."|".$rows["CARABAYAR_ID"]."|".$rows["id"]."|".$rows["SHIFT"];
				$dt.=$cid.chr(3).$i.chr(3).$rows["KSO_NAMA"].chr(3).$rows["cara_bayar"].chr(3).$rows["nama"].chr(3).$rows["SHIFT"].chr(3).number_format($nJual,0,",",".").chr(3).number_format($nSlip,0,",",".").chr(3).number_format($bJual,0,",",".").chr(3)."0".chr(6);
			}
			break;
		case "ReturnJualObat":
			if ($posting==0){
		...
		case "pengeluaranLain":
			while ($rows=mysql_fetch_array($rs)){
				...
				//if ($rows["id_trans"]==8 || $rows["id_trans"]==9 || $rows["id_trans"]==10 || $rows["id_trans"]==15){
					$sql2="SELECT * FROM keuangan.k_transaksi_detail td WHERE td.transaksi_id=".$rows["id"]." AND pajak_id=0";
