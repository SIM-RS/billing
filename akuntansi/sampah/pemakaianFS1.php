	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Pemakaian Obat Oleh Floor Stock</td>
                </tr>
                <!--tr>
                ...
                <tr>
                    <td height="30" style="padding-left:95px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="if(this.value==0){document.getElementById('btnPosting').disabled=false;}else{document.getElementById('btnPosting').disabled=true;};kirim();">
                        <option value="0">BELUM POSTING</option>
                        <option value="1">SUDAH POSTING</option>
                    </select></td>
                    <td height="30" align="right" style="padding-right:95px;"><BUTTON type="button" id="btnPosting" onClick="PostingJurnal()"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal</BUTTON></td>
                </tr>
                <tr>
                    <td colspan="2" align="center">
                        <div id="gridbox" style="width:800px; height:300px; background-color:white; overflow:hidden;"></div>
                        <div id="paging" style="width:800px;"></div>
                    </td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
...
<script>
	...
	a1.setColHeader("NO,UNIT PELAYANAN,NILAI HPP,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",nama,,");
	a1.setColWidth("50,475,100,75");
	...
	a1.baseURL("../unit/utils.php?grd=PemakaianFS&tanggal="+document.getElementById('tgl').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/utils.php?grd=PemakaianFS&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function kirim(){
	var url;
		url="../unit/utils.php?grd=PemakaianFS&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var tmp='',idata='';
	var url;
		url="../unit/utils.php?grd=PemakaianFS&tanggal="+document.getElementById('tgl').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		...
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingObatFS&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}else{
