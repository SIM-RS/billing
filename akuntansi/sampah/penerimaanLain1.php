...
$kso=$_REQUEST['kso'];
$tipe=$_REQUEST['tipe'];//echo $tipe."<br>";
	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2"><?php if ($tipe=="SetorManual") echo 'Transaksi Penerimaan Billing ( Setoran Manual )'; else echo 'Transaksi Penerimaan Lain - Lain'; ?></td>
                </tr>
                <!--tr>
                ...
                <tr>
                    <td height="30" style="padding-left:60px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="if(this.value==0){document.getElementById('btnPosting').disabled=false;}else{document.getElementById('btnPosting').disabled=true;};kirim();">
                        <option value="0">BELUM POSTING</option>
                        <option value="1">SUDAH POSTING</option>
                    </select>
                    </td>
                    <td height="30" align="right" style="padding-right:65px;"><BUTTON id="btnPosting" type="button" onClick="PostingJurnal();"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal</BUTTON></td>
                </tr>
                <tr>
...
<script>
	...
	a1.setHeader("DATA PENERIMAAN <?php if ($tipe=="SetorManual") echo 'BILLING (SETORAN MANUAL)'; else echo 'LAIN - LAIN'; ?>");
	a1.setColHeader("NO,TGL TERIMA,NO BUKTI,<?php if ($tipe=="SetorManual") echo 'KSO'; else echo 'JENIS PENERIMAAN'; ?>,NILAI,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",tgl_terima,no_bukti,nama,nilai,");
	a1.setColWidth("50,80,120,400,100,75");
	...
	a1.baseURL("../unit/utils.php?grd=<?php if ($tipe=="SetorManual") echo 'PenerimaanBillingManual'; else echo 'PenerimaanLain'; ?>&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/utils.php?grd=<?php if ($tipe=="SetorManual") echo 'PenerimaanBillingManual'; else echo 'PenerimaanLain'; ?>&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	function kirim(){
	var url;
		url="../unit/utils.php?grd=<?php if ($tipe=="SetorManual") echo 'PenerimaanBillingManual'; else echo 'PenerimaanLain'; ?>&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var tmp='',idata='';
	var url;
		url="../unit/utils.php?grd=<?php if ($tipe=="SetorManual") echo 'PenerimaanBillingManual'; else echo 'PenerimaanLain'; ?>&bulan="+document.getElementById('bulan').value+"&tahun="+document.getElementById('ta').value+"&tipe=<?php echo $tipe; ?>&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		...
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[5].childNodes[0].checked){
				...
				tmp+=idata[0]+String.fromCharCode(5)+idata[1]+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,3))+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,5))+String.fromCharCode(5)+idata[2]+String.fromCharCode(6);
			}
		}
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=<?php if ($tipe=="SetorManual") echo 'postingPenerimaanBillingManual'; else echo 'postingPenLain'; ?>&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}else{
