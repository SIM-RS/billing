...
//===============================
	...
	$sql = "select * from (SELECT 
	  ...
	  (SELECT 
		nama 
	  FROM
		akuntansi.ak_ms_unit 
	  WHERE id = u.parent_id) AS parent 
	FROM
	  akuntansi.ak_ms_unit u 
	WHERE islast = 1 
	...
	while ($rows=mysql_fetch_array($rs)) {
		$sisip = $rows['id'];
		$i++;
		$nilai = "<input type='text' id='nilai_$rows[id]' lang='$rows[parent_id]*$rows[id]' size='20' style='text-align:right' onkeyup='zxc(this);setSubTotal()' />";
