...
if ($idccrvpbfumum!="0"){
	switch ($ccrvpbfumum){
		case 1:
			...
			break;
		case 2:
			$sql="SELECT kode_ak kode,nama FROM billing.b_ms_kso WHERE id='$idccrvpbfumum'";
			//echo $sql."<br>";
			$rscc=mysql_query($sql);
			if ($rows=mysql_fetch_array($rscc)){
				$kode_cc=$rows["kode"];
				$kode_ma.=$kode_cc;
				$ma=trim($rows["nama"]);
				$sql="SELECT IFNULL(SUM(SALDO_AWAL),0) AS SALDO_AWAL FROM saldo s INNER JOIN ma_sak ma ON s.FK_MAID=ma.MA_ID INNER JOIN billing.b_ms_kso mu ON s.CC_RV_KSO_PBF_UMUM_ID=mu.id WHERE s.bulan=$bulan AND s.tahun=$ta AND s.FK_MAID=$idma AND mu.id='$idccrvpbfumum'";
				$rs1=mysql_query($sql);
				if ($rows1=mysql_fetch_array($rs1)){
					$saldoawal=$rows1['SALDO_AWAL'];
				}
...
<span class="jdltable">Buku Besar </span><br>
  <table width="99%" border="0" cellpadding="2" cellspacing="0" class="txtinput">
  	...
  	<tr>
		<td>Buku Besar</td>
		<td>:</td>
		<td><select name="bulan" id="bulan" class="txtinput" onchange="location='?f=bukubesar.php&idma='+idma.value+'&kode_ma=<?php echo $kode_ma; ?>&ccrvpbfumum=<?php echo $ccrvpbfumum; ?>&idccrvpbfumum=<?php echo $idccrvpbfumum; ?>&islast=<?php echo $islast; ?>&ta='+ta.value+'&bulan='+bulan.value">
            ...
            <select name="ta" id="ta" class="txtinput" onchange="location='?f=bukubesar.php&idma='+idma.value+'&kode_ma=<?php echo $kode_ma; ?>&ccrvpbfumum=<?php echo $ccrvpbfumum; ?>&idccrvpbfumum=<?php echo $idccrvpbfumum; ?>&islast=<?php echo $islast; ?>&ta='+this.value+'&bulan='+bulan.value">
	   		...
	   		switch($ccrvpbfumum){
				case 1:
					if (substr($kode_ma,0,1)=="4"){
						$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_KSO_PBF_UMUM_ID=mu.id where MA_ID='$idma' AND mu.kode LIKE '$kode_cc%' AND LEFT(mu.kode,2)<>'08' and month(j.TGL)=$bulan and year(j.TGL)=$ta";
						
						$sql="SELECT * FROM ak_ms_unit WHERE parent_id='$idccrvpbfumum' AND LEFT(kode,2)<>'08' ORDER BY kode";
					}else{
						$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_KSO_PBF_UMUM_ID=mu.id where MA_ID='$idma' AND mu.kode LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta";
					...
					break;
				case 2:
					$sql5="select IFNULL(SUM(DEBIT),0) AS subTotD,IFNULL(SUM(KREDIT),0) AS subTotK from jurnal j INNER JOIN ma_sak ma ON j.FK_SAK=ma.MA_ID INNER JOIN billing.b_ms_kso kso ON j.CC_RV_KSO_PBF_UMUM_ID=kso.id where MA_ID='$idma' AND kso.kode_ak LIKE '$kode_cc%' and month(j.TGL)=$bulan and year(j.TGL)=$ta";
					$rs=mysql_query($sql5);
					$rwsTot=mysql_fetch_array($rs);
					$stotd=$rwsTot["subTotD"];
					$stotk=$rwsTot["subTotK"];
			...
			if ($ccrvpbfumum=="0"){
				...
				switch($ccrvpbfumum){
					case 1:
						...
						$sql="select if (sum(DEBIT) is null,0,sum(DEBIT)) as DEBIT,if (sum(KREDIT) is null,0,sum(KREDIT)) as KREDIT FROM jurnal j INNER JOIN ma_sak ON FK_SAK=MA_ID INNER JOIN ak_ms_unit mu ON j.CC_RV_KSO_PBF_UMUM_ID=mu.id where MA_ID='$idma' AND mu.kode LIKE '".$rows['kode']."%' and month(TGL)=$bulan and year(TGL)=$ta";
						break;
					case 2:
						$idccrvpbfumum=$rows['id'];
