	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                <tr>
                    <td class="jdltable" height="29" colspan="2">Transaksi Penerimaan Billing</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td height="30" style="padding-left:150px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="if(this.value==0){document.getElementById('btnPosting').disabled=false;}else{document.getElementById('btnPosting').disabled=true;};kirim();">
                        <option value="0">BELUM POSTING</option>
                        <option value="1">SUDAH POSTING</option>
                    </select>
                    </td>
                    <td height="30" align="right" style="padding-right:145px;"><BUTTON type="button" id="btnPosting" onClick="PostingJurnal();"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal</BUTTON></td>
                </tr>
                <tr>
                    <td colspan="2" align="center">
                        <div id="gridbox" style="width:700px; height:300px; background-color:white; overflow:hidden;"></div>
                        <div id="paging" style="width:700px;"></div>
                    </td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
...
<script>
	...
	a1.setHeader("DATA PENERIMAAN BILLING");
	a1.setColHeader("NO,UNIT PELAYANAN,NILAI,POSTING<BR><input type='checkbox' name='chkAll' id='chkAll' onClick='chkKlik(this.checked);'>&nbsp;&nbsp;&nbsp;&nbsp;");
	a1.setIDColHeader(",nama,,,,,,");
	a1.setColWidth("50,400,100,75");
	a1.setCellAlign("center,left,right,center");
	a1.setCellType("txt,txt,txt,chk");
	a1.setCellHeight(20);
	a1.setImgPath("../icon");
	a1.setIDPaging("paging");
	a1.onLoaded(konfirmasi);
	//a.attachEvent("onRowClick");
	a1.baseURL("../unit/utils.php?grd=penerimaanBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value);
	a1.Init();
	...
	function konfirmasi(key,val){
		...
		if(key=='Error'){
			if(val=='postingPenerimaanBilling'){
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/utils.php?grd=penerimaanBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	var cekbox=(p==true)?1:0;
		...
		for (var i=0;i<a1.getMaxRow();i++){
			a1.cellsSetValue(i+1,4,cekbox);
		}
	}
	
	function kirim(){
	var url;
		url="../unit/utils.php?grd=penerimaanBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value;
		//alert(url);
		a1.loadURL(url,"","GET");
	}
	...
	var tmp='',idata='';
	var url;
		url="../unit/utils.php?grd=penerimaanBilling&kso="+document.getElementById('cmbKso').value+"&tanggal="+document.getElementById('tgl').value+"&tempat="+document.getElementById('cmbTmp').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
		
		for (var i=0;i<a1.getMaxRow();i++){
			if (a1.obj.childNodes[0].childNodes[i].childNodes[3].childNodes[0].checked){
				//alert(a1.cellsGetValue(i+1,2));
				idata=a1.getRowId(i+1).split("|");
				tmp+=idata[0]+String.fromCharCode(5)+idata[1]+String.fromCharCode(5)+ValidasiText(a1.cellsGetValue(i+1,3))+String.fromCharCode(6);
			}
		}
		if (tmp!=""){
			...
			if (tmp.length>1024){
				...
				url+="&act=postingPenerimaanBilling&fdata="+tmp;
				//alert(url);
				a1.loadURL(url,"","GET");
			}
		}else{
