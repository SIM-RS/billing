	...
	<td align="left" height="430">
        <div align="center">
            <table width="980" border="0" cellpadding="0" cellspacing="0" class="txtinput">
                ...
                <tr valign="bottom">
                    <td height="50" style="padding-left:45px;"><select id="cmbPost" name="cmbPost" class="txtinput" style="background-color:#FFFFCC; color:#990000;" onChange="if(this.value==0){document.getElementById('btnPosting').disabled=false;document.getElementById('btnPosting1').disabled=false;}else{document.getElementById('btnPosting').disabled=true;document.getElementById('btnPosting1').disabled=true;};kirim()">
                        <option value="0">BELUM POSTING</option>
                        <option value="1">SUDAH POSTING</option>
                    </select></td>
                    <td height="30" align="right" style="padding-right:45px;"><BUTTON type="button" id="btnPosting1" onClick="PostingJurnal(0)"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal<br>Blm Terbayar</BUTTON>&nbsp;<BUTTON type="button" id="btnPosting" onClick="PostingJurnal(1)"><IMG SRC="../icon/save.gif" border="0" width="16" height="16" ALIGN="absmiddle">&nbsp;Posting >> Jurnal<br>Sdh Terbayar</BUTTON></td>
                </tr>
                <tr>
...
<script>
	...
	a1.setColWidth("30,300,75,150,75,150,200,75,75,100,150,75");
	...
	a1.baseURL("../unit/utils.php?grd=ReturnPBF&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value);
	a1.Init();
	...
	var url;
		//alert(grd);
		if (grd=="gridbox"){		
			url="../unit/utils.php?grd=ReturnPBF&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value+"&posting="+document.getElementById('cmbPost').value+"&filter="+a1.getFilter()+"&sorting="+a1.getSorting()+"&page="+a1.getPage();
			//alert(url);
			a1.loadURL(url,"","GET");
		}
	}
	...
	var cekbox=(p==true)?1:0;
 	...
 	function kirim(){
	var url;
		url="../unit/utils.php?grd=ReturnPBF&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value+"&posting="+document.getElementById('cmbPost').value;
		a1.loadURL(url,"","GET");
	} 
	...
	var tmp='',idata='';
	var url;
		url="../unit/utils.php?grd=ReturnPBF&tglAwal="+document.getElementById('tglAwal').value+"&tglAkhir="+document.getElementById('tglAkhir').value+"&posting="+document.getElementById('cmbPost').value+"&idUser=<?php echo $iduser; ?>";
