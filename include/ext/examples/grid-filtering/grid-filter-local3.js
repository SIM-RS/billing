 ...
 function extGrid (id){
    ...
    this.loadURL=function(urlLoad,successText,reload){
    	...
    	Ext.Ajax.request({
    	   url: urlLoad,
    	   success: received,
    	   failure: function (response) { alert("Gagal");}
    	});
    ...
    this.init=function(){
        ...
        for(var i=0;i<this.headerCount;i++){
            ...
            if(this.colType[i].indexOf(":")>-1){//alert('a');
                ...
                this.coloumModel.push({dataIndex:this.coloumId[i],width:new Number(this.coloumWidth[i]),header:"<b>"+this.headerName[i]+"</b>",id:this.coloumId[i],filter:{type:type[0],options:option,phpMode:true}});
            }else if(this.colType[i].length==0){               
                this.filterCol.push();   
                this.coloumModel.push({dataIndex:this.coloumId[i],width:new Number(this.coloumWidth[i]),header:"<b>"+this.headerName[i]+"</b>",id:this.coloumId[i]});
            }else{
                this.filterCol.push({type:this.colType[i],dataIndex:this.coloumId[i]});   
                this.coloumModel.push({dataIndex:this.coloumId[i],width:new Number(this.coloumWidth[i]),header:"<b>"+this.headerName[i]+"</b>",id:this.coloumId[i],filter:{type:this.colType[i]}});    
            }
