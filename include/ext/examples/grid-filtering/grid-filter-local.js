...
//if(Ext.isIE)alert('a');
 ...
 function extGrid (id){
    ...
    this.baseURL=function(str){
        this.url=str; //setTimeout("tt()",100);
    }
    ...
    this.setRowDoubleClickEvent = function(evt) {
        this.rowDoubleClick = evt;
    }
   
    this.getMaxRow=function(){
		//var gridstore=this.grid.getStore().getCount();
		//var bar = gridstore.data.length;
    ...
    this.init=function(){
        ...
        for(var i=0;i<this.headerCount;i++){
            ...
            if(this.colType[i].indexOf(":")>-1){//alert('a');
				...
				this.coloumModel.push({dataIndex:this.coloumId[i],width:parseFloat(this.coloumWidth[i]),align:alignCol,header:"<div align='left'><b>"+this.headerName[i]+"</b></div>",id:this.coloumId[i],filter:{type:this.colType[i]}});    
            }
        ...
        var evtDblCl = this.rowDoubleClick;
        //alert(getID());
