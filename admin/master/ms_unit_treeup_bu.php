                           ...
                           $strSQL = "select * from ms_unit order by kodeunit";
                            ...
                            while ($rows=mysql_fetch_array($rs)) {
                               ...
                               if ($lvl>0)
							   {
									...
									if($act=='add')
									{
										$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['idunit']."*|*".$par[1]."*-*".$rows['kodeunit']."*|*".$par[2]."*-*".$rows['namaunit']."*|*".$par[3]."*-*".($lvl+1)."*|*".$par[5]."*-*".$rows['billing_id']."');
									window.close();window.opener.get_kode_anak('$kode_parent',$level_anak);window.opener.get_id_billing2('$rows[billing_id]','$billing_level');";
									}
									else
									{
										$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['idunit']."*|*".$par[1]."*-*".$rows['kodeunit']."*|*".$par[2]."*-*".$rows['namaunit']."*|*".$par[3]."*-*".($lvl+1)."*|*".$par[5]."*-*".$rows['billing_id']."');
										window.close();window.opener.get_kode_anak('$kode_parent',$level_anak,'$kode_parent_awal','$kode_unit_awal'); window.opener.get_id_billing2('$rows[billing_id]','$billing_level')";
