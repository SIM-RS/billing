...
<html>
    ...
    <body style="border-width:0px;" bgcolor="#CCCCCC" topmargin="0" leftmargin="0" onLoad="javascript:if (window.focus) window.focus();" >
        <div align="center">
                ...
                <tr bgcolor="whitesmoke">
                    <td nowrap>
						...
						$canRead = true;
                            $maxlevel=0;
                            $cnt=0;
                           $strSQL = "select * from $rspelindo_db_billing.b_ms_unit where level=1 order by kode"; //echo $strSQL;
                            ...
                            while ($rows=mysql_fetch_array($rs)) {
                               ...
                               if ($lvl>0)
							   {
									...
									if($act=='add')
									{
										$tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['id']."*|*".$par[1]."*-*".$rows['kode']."*|*".$par[2]."*-*".$rows['nama']."');window.opener.get_id_billing('$act','$rows[kode]','$rows[level]');window.close();";
									}
			...
			if($act=='add')
			{
				$qq = "select id from $rspelindo_db_billing.b_ms_unit order by id desc limit 1";
				$ss = mysql_query($qq);
			...
			?>
			<button type="button" onClick="set_parent('<?=$new_kode;?>','<?=$new_id;?>');">Top Parent</button>
        </div>
    </body>
    <script type="text/javascript" language="javascript">
		...
		function set_parent(a,b)
		{
			//alert(a);
			window.close();
			window.opener.document.form1.billing_parent_id.value = '';
			window.opener.document.form1.billing_parent_kode.value = '';
			window.opener.document.form1.billing_parent_kode_lama.value = '';
			window.opener.document.form1.billing_parent_nama.value = '';
			window.opener.document.form1.billing_kode.value = a;
			window.opener.document.form1.billing_kode_lama.value = a;
			if(b != "")
			{
				window.opener.document.form1.billing_id.value = b;
				window.opener.document.form1.billing_level.value = 1;
			}
		}
    </script>
</html>
