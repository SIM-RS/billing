...
<body>
        <div id="wrapper">
			...
			<div id="content" align="center">
				...
				<div style="text-align:center; font-size:18px; font-weight:bold; font-family:'Comic Sans MS', cursive">.: Fasilitas :.</div>
				<br>
					...
					<input type="hidden" id="complain_id" name="complain_id" value="">
						<table>
							<tr>
								...
								<?php
									$sql="select distinct mu.`idunit`, mu.`namaunit` from rspelindo_saran.`tb_complain` tc
										inner join rspelindo_hcr.`ms_unit` mu on
										mu.`idunit` = tc.`complainunit`
										where tc.`complaintype` ='Facility'";
									$rs=mysql_query($sql);
							...
							<tr>
								<td>Parent</td>
								<td>:</td>
								<td><select id="parent_id" name="complainparentid" disabled>
									<option value="0">-- Silahkan Pilih Parent --</option>";
								</select></td>
							</tr>
							<tr>
								<td>Keterangan</td>
...
gd1.setTitle(".: Fasilitas :.");
gd1.setHeader("No,Nama Unit,Keterangan,Tipe,Status");
gd1.setColId("NO,namaunit,complainket,complaintype,complainaktif");
gd1.setColType("string,string,string,string,string");
	...
	$('#unit_id').live('change',function(){
		if($('#unit_id').val()=='0')
		{
			...
			$.ajax({
				type: 'post',
				url: 'utils.php',
				data: 'type=ajaxoption&id='+$('#unit_id').val(),
