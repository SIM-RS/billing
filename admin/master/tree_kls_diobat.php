...
<?php	
  ...
  $maxlevel=0;
  $cnt=0;
  $strSQL = "select * from $rspelindo_db_apotek.a_kelas order by kls_kode";
  ...
  while ($rows=mysql_fetch_array($rs)){
		 ...
		 if (($rows["KLS_ISLAST"]==1))
			 $tree[$cnt][2]= "javascript:fSetValue(window.opener,'".$par[0]."*-*".$rows['KLS_ID']."*|*".$par[1]."*-*".$rows['KLS_NAMA']."');window.close();";
 		 }else
		 	 $tree[$cnt][2] = null;
		 $tree[$cnt][3]= "";
		 $tree[$cnt][4]= 0;
