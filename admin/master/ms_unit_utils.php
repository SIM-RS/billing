...
if (is_array($filters)) {
 ...
 switch($_REQUEST['act']) {
	case 'hapus':
	$sqlSelect="SELECT parentunit FROM ms_unit WHERE idunit='".$_REQUEST['id']."'";
	$rs=mysql_query($sqlSelect);
	$row=mysql_fetch_array($rs);
	
	$jum = mysql_num_rows(mysql_query("select parentunit from ms_unit where parentunit='$row[0]'"));
	
	if($jum=='1'){
		//$upSel="SELECT PARENT_KODE_JABATAN FROM ms_jabatan_pegawai WHERE PARENT_KODE_JABATAN LIKE '%".$row[0]."%'";
		$up="UPDATE ms_unit set islast=1 WHERE idunit='$row[0]'";
		mysql_query($up); 
		
	}
		$sqlHapus="DELETE FROM ms_unit WHERE idunit='".$_REQUEST['id']."'";
...
while($obj=mysql_fetch_array($rs)){
	...
	if ($obj['aktif']==1) $ch = "checked"; else {$ch ="";}
    $arr[$i]=array('NO'=>($no).$idobj,'kodeunit'=>$obj["kodeunit"],'namaunit'=>$obj["namaunit"],'nickunit'=>$obj["nickunit"],'level'=>$obj["level"],'aktif'=>"<input type=checkbox $ch />");
    $i++;
    $no++;
}
