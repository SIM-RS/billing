...
function DSGridObject(id){
	...
	r.insertCell(1).innerHTML = "Img";r.childNodes[1].style.width = '60%';r.childNodes[1].align = 'right';
	...
	this.setCaptionColHeader=function(col,str){
		document.getElementById('hdrcol_'+id).childNodes[col-1].innerHTML = str;
	}
	...
	this.setColWidth=function(str){
		...
		if (document.getElementById('hdrcaption_'+id).style.visibility=='collapse'){
			document.getElementById('griditem_'+id).style.height=(parseInt(this.grid.gridboxh)-parseInt(this.grid.hdrcolh)+parseInt(this.grid.hdrcaptionh)-20) + 'px';
		}else{
	...
	this.setIDPaging=function(str){
		...
		document.getElementById(str).childNodes[0].childNodes[0].childNodes[0].childNodes[1].innerHTML = p;
	}
