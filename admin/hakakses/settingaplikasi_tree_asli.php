<?php
	...
	$act = $_REQUEST['act'];
	  ...
	  $pars='parent_kode*kode*namax*urlx*status';
      $par=explode("*",$pars);
?>
<div id="divtreeview">
				...
				<tr bgcolor="whitesmoke">
					<td>
						<?php	
						  ...
						  $PATH_INFO ='http://'.$_SERVER['HTTP_HOST'].'/simrs-pelindo/admin/hakakses/settingaplikasi_tree.php';
						  ...
						  $cnt=0;
						  $target='divtreeview';
						  $strSQL = "select * from ms_menu where aktif=1 and modul_id=1 order by kode";
						  ...
						  while ($rows=mysql_fetch_array($rs)){
								 ...
								 $arfvalue=$par[0]."*-*".$rows['id']."*|*".$par[1]."*-*".$mkode."*|*".$par[2]."*-*".$rows['nama']."*|*".$par[3]."*-*".$rows['url']."*|*".$par[4]."*-*".($rows['level']+1)."*|*".$par[5]."*-*".$rows['parent_id']."*|*".$par[6]."*-*"."Update";
								 if ($c_level>0)
