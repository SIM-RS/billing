...
switch ($act) {
    ...
    case "getPelayanan":
        if ($_REQUEST['tipe'] == 1) {
            $where = " tgl BETWEEN " . "'" . htmlspecialchars($_REQUEST['tanggalAwal']) . "'" . " AND " . "'" . htmlspecialchars($_REQUEST['tanggalAkhir']) . "'";
            $group = " tgl";
            $field = " tgl,";
        } else if ($_REQUEST['tipe'] == 2) {
            $where = " YEAR(tgl) = {$_REQUEST['tahun']}";
            $group = " MONTH(tgl)";
            $field = " MONTH(tgl) bulan,";
        } else if ($_REQUEST['tipe'] == 3) {
            $where = " MONTH(tgl) = {$_REQUEST['bulan']} AND YEAR(tgl) = {$_REQUEST['tahun_filter_bulan']}";
            $field = " tgl,";
            $group = " tgl";
        }
        $filter = "SELECT
    ...
    case 'getAll':
        ...
        if ($_REQUEST['tipe'] == 1) {
            ...
            $where = " AND YEAR(j.tgl) = {$_REQUEST['tahun']}";
            $wherePelayanan = " YEAR(tgl) = {$_REQUEST['tahun']}";
            $group = " MONTH(j.tgl)";
            $field = " MONTH(j.tgl) bulan,";
        } else if ($_REQUEST['tipe'] == 3) {
            $wherePelayanan = " MONTH(tgl) = {$_REQUEST['bulan']} AND YEAR(tgl) = {$_REQUEST['tahun_filter_bulan']}";
            $where = " AND MONTH(j.tgl) = {$_REQUEST['bulan']} AND YEAR(j.tgl) = {$_REQUEST['tahun_filter_bulan']}";
        }
        ...
        $filterLaba = $pendapatan->getDataPendapatan("SELECT IFNULL(SUM(jumlah2 - jumlah),0) jumlahLaba FROM (
                        ...
                        SELECT  
                            ...
                            WHERE
                                ma.MA_KODE LIKE '4%' 
                                {$where}
                                AND j.flag = '1') as q")->fetch_assoc();
        $filterPelayanan = $pendapatan->getDataPendapatan("SELECT
                                COUNT(*) AS jumlah 
                            FROM
                                b_pelayanan  
                            WHERE
                                {$wherePelayanan}
                                AND flag = '1'")->fetch_assoc();
        echo json_encode(['pendapatan' => $filterPendapatan['jumlah_pendapatan'], 'biaya' => $filterBiaya['jumlahBiaya'], 'laba' => $filterLaba['jumlahLaba'], 'pelayanan' => $filterPelayanan['jumlah']]);
        exit;
        break;
}
